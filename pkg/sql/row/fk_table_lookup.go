...
type tableLookupQueue struct {
	...
	// TODO lixinze  write for lixinze
	accessCheckFn CheckAccessFunction
...
// check the privileges of the current user to access specific table columns.
type CheckPrivilegeFunction func(context.Context, *sqlbase.TableDescriptor, tree.NameList, privilege.Kind) error

// CheckAccessFunction checks database access privilege
type CheckAccessFunction func(context.Context, sqlbase.DescriptorProto) error

// NoCheckPrivilege is a stub that can be used to not actually verify privileges.
// This can be used when the FK work is initialized from a pre-populated
// FkTableMetadata map.
func NoCheckPrivilege(
	_ context.Context, _ *sqlbase.TableDescriptor, _ tree.NameList, _ privilege.Kind,
) error {
	return nil
}

// NoCheckAccess means don't check access privilege
func NoCheckAccess(_ context.Context, _ sqlbase.DescriptorProto) error {
	return nil
}
...
func (q *tableLookupQueue) getTable(ctx context.Context, tableID TableID) (TableEntry, error) {
	...
	if !tableEntry.IsAdding && tableEntry.Desc != nil {
		...
		// TODO write for lixinze
		if err := q.accessCheckFn(ctx, tableEntry.Desc); err != nil {
			return TableEntry{}, err
		}

		// If we have a real table, we need first to verify the user has permission.
		if err := q.privCheckFn(ctx, tableEntry.Desc.TableDesc(), nil, privilege.REFERENCES); err != nil {
			return TableEntry{}, err
		}
...
func (q *tableLookupQueue) enqueue(ctx context.Context, tableID TableID, usage FKCheckType) error {
	...
	case CheckDeletes:
		...
		// delete check table privilege directly
		if err := q.privCheckFn(ctx, tableEntry.Desc.TableDesc(), nil, privilege.DELETE); err != nil {
			return err
		}
	case CheckUpdates:
		// TODO write for lixinze
	...
	// not check column UPDATE privilege
