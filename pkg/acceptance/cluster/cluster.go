...
type Cluster interface {
	...
	// ExecCLI runs `./znbase <args>`, while filling in required flags such as
	...
	// Kill terminates the znbase process running on the given node number.
	// The given integer must be in the range [0,NumNodes()-1].
	Kill(context.Context, int) error
	// Restart terminates the znbase process running on the given node
