...
var (
	...
	// node's storage (number, size and health of ranges) back to ZNBaseDB.
	...
	// env var ZNBASE_SKIP_ENABLING_DIAGNOSTIC_REPORTING.
	//
	...
	DiagnosticsReportingEnabled = settings.RegisterBoolSetting(
		...
		"enable reporting diagnostic metrics to znbase labs",
		false,
	)
	...
	// data with ZNBase Labs and various cloud services.
	ReportSensitiveDetails = envutil.EnvOrDefaultBool("ZNBASE_REPORT_SENSITIVE_DETAILS", false)
)
...
var crashReportURL = func() string {
	...
	if build.SeemsOfficial() {
		defaultURL = "https://ignored:ignored@errors.znbasedb.com/sentry"
	}
	return envutil.EnvOrDefaultString("ZNBASE_CRASH_REPORTS", defaultURL)
}()
