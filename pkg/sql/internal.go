...
func (ie *internalExecutorImpl) GetCaseSensitive() bool {
	if ie.sessionData != nil {
		return ie.sessionData.CaseSensitive
	}
	return false
}
...
func (ie *internalExecutorImpl) initConnEx(
	...
	ex.sessionData.IsInternalSQL = true
	ex.planner.extendedEvalCtx.IsInternalSQL = true
	if !sd.ReplicaionSync {
		log.Info(ctx, "replicationsync not inheritq")
	}
...
// fields (e.g. the user).
// And it's session is internalExecNodeSeesion.
func (ie *InternalExecutor) ExecEx(
	ctx context.Context, opName string, txn *client.Txn, stmt string, qargs ...interface{},
) (int, error) {
	res, err := ie.execInternal(
		ctx, opName, txn, internalExecNodeSeesion, &SessionArgs{}, stmt, qargs...,
	)
	if err != nil {
		return 0, err
	}
...
func (ie *InternalExecutor) GetExecCfg() (interface{}, error) {
	return ie.s.GetExecutorConfig(), nil
}
...
func (ie *InternalExecutor) GetCaseSensitive() bool {
	if ie.sessionData != nil {
		return ie.sessionData.CaseSensitive
	}
	return false
...
// ExecEx is like Exec, but allows the caller to override some session data
// fields (e.g. the user).
// But it's unimplement now.
func (ie *SessionBoundInternalExecutor) ExecEx(
...
// GetExecCfg return An ExecutorConfig,encompasses the auxiliary objects and configuration
// required to create an executor.
func (ie *SessionBoundInternalExecutor) GetExecCfg() (interface{}, error) {
	return ie.impl.s.GetExecutorConfig(), nil
}
...
func (ie *internalExecutorImpl) execInternal(
	...
	case internalExecInheritSession, internalExecRootSession:
		if sargs.isDefined() {
			...
			return result{}, errors.Errorf(fmt.Sprintln(sargs))
		}
	...
	case internalExecRootSession:
		sargs.SessionDefaults = SessionDefaults{SessionDefaultsMp: map[string]string{
			"database":         "system",
			"application_name": InternalAppNamePrefix + "-" + opName,
		},
		}
	case internalExecNodeSeesion:
		sargs = &SessionArgs{User: security.NodeUser}
		sargs.SessionDefaults.SessionDefaultsMp = map[string]string{
			"database":         "system",
			"application_name": InternalAppNamePrefix + "-" + opName,
		}
	}
	...
	parsed, err := parser.ParseOne(stmt, ie.GetCaseSensitive())
	if err != nil {
		return result{}, err
	}
	...
	// 内部执行，default_transaction_serializable = "serializable"
	sargs.SessionDefaults.Set("default_transaction_isolation", "serializable")
	sargs.SessionDefaults.Set("replicate_tables_in_sync", "on")
	if ie.sessionData != nil {
		ie.sessionData.ReplicaionSync = true
	}
