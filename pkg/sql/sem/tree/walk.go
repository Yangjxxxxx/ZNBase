...
func (expr *CharExpr) copyNode() *CharExpr {
	exprCopy := *expr
	return &exprCopy
}

// copyNode makes a copy of this Expr without recursing in any child Exprs.
func (expr *NvlExpr) copyNode() *NvlExpr {
	exprCopy := *expr
	return &exprCopy
}
...
// Walk implements the Expr interface.
func (expr *CharExpr) Walk(v Visitor) Expr {
	ret := expr
	exprs, changed := walkExprSlice(v, expr.Exprs)
	if changed {
		if ret == expr {
...
// Walk implements the Expr interface.
func (expr *NvlExpr) Walk(v Visitor) Expr {
	ret := expr
	exprs, changed := walkExprSlice(v, expr.Exprs)
	if changed {
		if ret == expr {
...
// Walk implements the Expr interface.
func (expr *AdditionalExpr) Walk(_ Visitor) Expr { return expr }
...
func (n *Backup) copyNode() *Backup {
	stmtCopy := *n
	stmtCopy.IncrementalFrom = append(Exprs(nil), n.IncrementalFrom...)
	stmtCopy.Options = append(KVOptions(nil), n.Options...)
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (n *Backup) walkStmt(v Visitor) Statement {
	ret := n
	if n.AsOf.Expr != nil {
		e, changed := WalkExpr(v, n.AsOf.Expr)
		if changed {
			if ret == n {
				ret = n.copyNode()
			}
	...
	for i, expr := range n.IncrementalFrom {
...
// walkStmt is part of the walkableStmt interface.
func (n *Delete) walkStmt(v Visitor) Statement {
	ret := n
	if n.Where != nil {
		e, changed := WalkExpr(v, n.Where.Expr)
		if changed {
			ret = n.copyNode()
			ret.Where.Expr = e
		}
	}
	returning, changed := walkReturningClause(v, n.Returning)
	if changed {
		if ret == n {
			ret = n.copyNode()
		}
...
// copyNode makes a copy of this Statement without recursing in any child Statements.
func (n *Explain) copyNode() *Explain {
	stmtCopy := *n
	stmtCopy.Options = append([]string(nil), n.Options...)
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (n *Explain) walkStmt(v Visitor) Statement {
	s, changed := walkStmt(v, n.Statement)
	if changed {
		n = n.copyNode()
		n.Statement = s
	}
...
// copyNode makes a copy of this Statement without recursing in any child Statements.
func (n *Insert) copyNode() *Insert {
	stmtCopy := *n
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (n *Insert) walkStmt(v Visitor) Statement {
	ret := n
	if n.Rows != nil {
		rows, changed := walkStmt(v, n.Rows)
		if changed {
			ret = n.copyNode()
...
// copyNode makes a copy of this Statement without recursing in any child Statements.
func (c *CreateTable) copyNode() *CreateTable {
	stmtCopy := *c
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (c *CreateTable) walkStmt(v Visitor) Statement {
	ret := c
	if c.AsSource != nil {
		rows, changed := walkStmt(v, c.AsSource)
		if changed {
			ret = c.copyNode()
...
// copyNode makes a copy of this Statement without recursing in any child Statements.
func (n *CancelQueries) copyNode() *CancelQueries {
	stmtCopy := *n
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (n *CancelQueries) walkStmt(v Visitor) Statement {
	sel, changed := walkStmt(v, n.Queries)
	if changed {
		n = n.copyNode()
		n.Queries = sel.(*Select)
	}
	return n
}

// copyNode makes a copy of this Statement without recursing in any child Statements.
func (n *CancelSessions) copyNode() *CancelSessions {
	stmtCopy := *n
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (n *CancelSessions) walkStmt(v Visitor) Statement {
	sel, changed := walkStmt(v, n.Sessions)
	if changed {
		n = n.copyNode()
		n.Sessions = sel.(*Select)
	}
	return n
}

// copyNode makes a copy of this Statement without recursing in any child Statements.
func (c *ControlJobs) copyNode() *ControlJobs {
	stmtCopy := *c
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (c *ControlJobs) walkStmt(v Visitor) Statement {
	sel, changed := walkStmt(v, c.Jobs)
	if changed {
		c = c.copyNode()
		c.Jobs = sel.(*Select)
	}
...
// walkStmt is part of the walkableStmt interface.
func (n *ParenSelect) walkStmt(v Visitor) Statement {
	sel, changed := walkStmt(v, n.Select)
	if changed {
...
// walkStmt is part of the walkableStmt interface.
func (n *Select) walkStmt(v Visitor) Statement {
	ret := n
	sel, changed := walkStmt(v, n.Select)
	if changed {
		ret = n.copyNode()
	...
	order, changed := walkOrderBy(v, n.OrderBy)
	if changed {
		if ret == n {
			ret = n.copyNode()
		}
	...
	if n.Limit != nil {
		if n.Limit.Offset != nil {
			e, changed := WalkExpr(v, n.Limit.Offset)
			if changed {
				if ret == n {
					ret = n.copyNode()
				}
		...
		if n.Limit.Count != nil {
			e, changed := WalkExpr(v, n.Limit.Count)
			if changed {
				if ret == n {
					ret = n.copyNode()
				}
...
// copyNode makes a copy of this Statement without recursing in any child Statements.
func (n *SelectClause) copyNode() *SelectClause {
	stmtCopy := *n
	stmtCopy.Exprs = append(SelectExprs(nil), n.Exprs...)
	stmtCopy.From = &From{
		Tables:     append(TableExprs(nil), n.From.Tables...),
		AsOf:       n.From.AsOf,
		AsSnapshot: n.From.AsSnapshot,
	}
	if n.Where != nil {
		wCopy := *n.Where
		stmtCopy.Where = &wCopy
	}
	stmtCopy.GroupBy = append(GroupBy(nil), n.GroupBy...)
	if n.Having != nil {
		hCopy := *n.Having
		stmtCopy.Having = &hCopy
	}
	n.Window = append(Window(nil), n.Window...)
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (n *SelectClause) walkStmt(v Visitor) Statement {
	ret := n
	...
	if n.From != nil && n.From.AsOf.Expr != nil {
		e, changed := WalkExpr(v, n.From.AsOf.Expr)
		if changed {
			if ret == n {
				ret = n.copyNode()
			}
	...
	if n.Having != nil {
		e, changed := WalkExpr(v, n.Having.Expr)
		if changed {
			if ret == n {
				ret = n.copyNode()
			}
	...
	for i := range n.Window {
		w, changed := walkWindowDef(v, n.Window[i])
		if changed {
			if ret == n {
				ret = n.copyNode()
			}
...
// copyNode makes a copy of this Statement without recursing in any child Statements.
func (n *SetVar) copyNode() *SetVar {
	stmtCopy := *n
	stmtCopy.Values = append(Exprs(nil), n.Values...)
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (n *SetVar) walkStmt(v Visitor) Statement {
	ret := n
...
// walkStmt is part of the walkableStmt interface.
func (n *SetZoneConfig) walkStmt(v Visitor) Statement {
	ret := n
	if n.YAMLConfig != nil {
		e, changed := WalkExpr(v, n.YAMLConfig)
		if changed {
			newStmt := *n
			ret = &newStmt
			ret.YAMLConfig = e
	...
	if n.Options != nil {
		newOpts, changed := walkKVOptions(v, n.Options)
		if changed {
			if ret == n {
				newStmt := *n
				ret = &newStmt
...
// copyNode makes a copy of this Statement without recursing in any child Statements.
func (n *SetTracing) copyNode() *SetTracing {
	stmtCopy := *n
	stmtCopy.Values = append(Exprs(nil), n.Values...)
	return &stmtCopy
}
...
// walkStmt is part of the walkableStmt interface.
func (n *SetTracing) walkStmt(v Visitor) Statement {
	ret := n
	for i, expr := range n.Values {
		e, changed := WalkExpr(v, expr)
		if changed {
...
// copyNode makes a copy of this Statement without recursing in any child Statements.
func (n *SetClusterSetting) copyNode() *SetClusterSetting {
	stmtCopy := *n
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (n *SetClusterSetting) walkStmt(v Visitor) Statement {
	ret := n
	if n.Value != nil {
...
// copyNode makes a copy of this Statement without recursing in any child Statements.
func (n *Update) copyNode() *Update {
	stmtCopy := *n
	stmtCopy.Exprs = make(UpdateExprs, len(n.Exprs))
...
// walkStmt is part of the walkableStmt interface.
func (n *Update) walkStmt(v Visitor) Statement {
	ret := n
	for i, expr := range n.Exprs {
		e, changed := WalkExpr(v, expr.Expr)
		if changed {
...
// walkStmt is part of the walkableStmt interface.
func (n *ValuesClause) walkStmt(v Visitor) Statement {
	ret := n
	for i, tuple := range n.Rows {
		exprs, changed := walkExprSlice(v, tuple)
		if changed {
			if ret == n {
				ret = &ValuesClause{append([]Exprs(nil), n.Rows...), AliasClause{"", nil}}
			}
			ret.Rows[i] = exprs
		}
	}
...
func (n *BeginTransaction) copyNode() *BeginTransaction {
	stmtCopy := *n
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (n *BeginTransaction) walkStmt(v Visitor) Statement {
	ret := n
	if n.Modes.AsOf.Expr != nil {
		e, changed := WalkExpr(v, n.Modes.AsOf.Expr)
		if changed {
			ret = n.copyNode()
...
// copyNode makes a copy of this Statement.
func (q *QueryLockstat) copyNode() *QueryLockstat {
	stmtCopy := *q
	return &stmtCopy
}

// walkStmt is part of the walkableStmt interface.
func (q *QueryLockstat) walkStmt(visitor Visitor) Statement {
	sel, changed := walkStmt(visitor, q.Txnid)
	if changed {
		q = q.copyNode()
		q.Txnid = sel.(*Select)
	}
...
var _ walkableStmt = &Dump{}
var _ walkableStmt = &QueryLockstat{}

//var _ walkableStmt = &Load{}
