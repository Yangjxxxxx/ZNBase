...
rocksdb::Status CTRCipherStreamCreator::InitSettingsAndCreateCipherStream(
  ...
  if (key == nullptr || key->info().encryption_type() == enginepbicl::Plaintext) {
    // Plaintext algorithm: only encryption_type is specified.
    enc_settings.set_encryption_type(enginepbicl::Plaintext);
...
rocksdb::Status CTRCipherStreamCreator::CreateCipherStreamFromSettings(
  ...
  if (settings.size() == 0 || enc_settings.encryption_type() == enginepbicl::Plaintext) {
...
CTRCipherStream::CTRCipherStream(std::shared_ptr<enginepbicl::SecretKey> key,
...
CTRCipherStream::InitCipher(std::unique_ptr<rocksdb_utils::BlockCipher>* cipher) const {
  ...
  if (key_->info().encryption_type() != enginepbicl::AES128_CTR &&
      key_->info().encryption_type() != enginepbicl::AES192_CTR &&
      key_->info().encryption_type() != enginepbicl::AES256_CTR) {
