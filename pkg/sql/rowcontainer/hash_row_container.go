...
type RowMarkerIterator interface {
	...
	ResetOpt(ctx context.Context, row sqlbase.EncDatumRow) error
...
type HashRowContainer interface {
	...
	// NewBucketIteratorOpt returns a RowMarkerIterator that iterates over a bucket
	// of rows that match the given row on equality columns. This iterator can
	// also be used to mark rows.
	// Rows are marked because of the use of this interface by the hashJoiner.
	// Given a row, the hashJoiner does not necessarily want to emit all rows
...
func (e *columnEncoder) encodeEqualityColsOpt(
	ctx context.Context, row sqlbase.EncDatumRow, eqCols columns,
) ([]byte, error) {
	var enc []byte
	var alloc = new(sqlbase.DatumAlloc)
	encoded, hasNull, err := encodeColumnsOfRow(
		alloc, enc, row, eqCols, e.keyTypes, e.encodeNull,
	)
	if err != nil {
		return nil, err
	}
...
// NewBucketIteratorOpt for hashjoin opt
func (h *HashMemRowContainer) NewBucketIteratorOpt(
	ctx context.Context, row sqlbase.EncDatumRow, probeEqCols columns,
) (RowMarkerIterator, error) {
	...
	if err := ret.ResetOpt(ctx, row); err != nil {
		return nil, err
	}
	return ret, nil
}

// FlushDir for hashopt
func (h *HashMemRowContainer) FlushDir() error {
	panic("implement me")
}

// IsDisk for hashopt
func (h *HashMemRowContainer) IsDisk() bool {
	return false
}
...
func (i *hashMemRowBucketIterator) ResetOpt(ctx context.Context, row sqlbase.EncDatumRow) error {
	var encoded []byte
	var err error
	encoded, err = i.encodeEqualityColsOpt(ctx, row, i.probeEqCols)
	if err != nil {
		return err
	}
...
func (i *hashMemRowBucketIterator) RowOpt(enc *sqlbase.EncDatumRow) (sqlbase.EncDatumRow, error) {
	return i.EncRowOpt(i.rowIdxs[i.curIdx], enc), nil
}
...
func (i *hashMemRowIterator) RowOpt(enc *sqlbase.EncDatumRow) (sqlbase.EncDatumRow, error) {
	panic("implement me")
}
...
// NewBucketIteratorOpt for hashjoin opt
func (h *HashDiskRowContainer) NewBucketIteratorOpt(
	ctx context.Context, row sqlbase.EncDatumRow, probeEqCols columns,
) (RowMarkerIterator, error) {
	panic("implement me")
}

// FlushDir for hashopt flush
func (h *HashDiskRowContainer) FlushDir() error {
	return h.bufferedRows.Flush()
}

// IsDisk for hashopt in disk
func (h *HashDiskRowContainer) IsDisk() bool {
	return true
}
...
func MakeHashDiskRowContainer(
	diskMonitor *mon.BytesMonitor, e diskmap.Factory, isFile bool,
...
func (h *HashDiskRowContainer) Init(
	...
	h.DiskRowContainer = MakeDiskRowContainer(h.diskMonitor, storedTypes, ordering, h.engine, h.isFile)
	return nil
}
...
func (i *hashDiskRowBucketIterator) ResetOpt(ctx context.Context, row sqlbase.EncDatumRow) error {
	var encoded []byte
	var err error
	encoded, err = i.encodeEqualityCols(ctx, row, i.probeEqCols)
	if err != nil {
...
// NewBucketIteratorOpt for hashjoin opt
func (h *HashDiskBackedRowContainer) NewBucketIteratorOpt(
	ctx context.Context, row sqlbase.EncDatumRow, probeEqCols columns,
) (RowMarkerIterator, error) {
	return h.src.NewBucketIteratorOpt(ctx, row, probeEqCols)
}

// FlushDir for hashopt
func (h *HashDiskBackedRowContainer) FlushDir() error {
	return h.src.FlushDir()
}

// IsDisk for hashopt use disk compute
func (h *HashDiskBackedRowContainer) IsDisk() bool {
	return h.src.IsDisk()
}
...
func (h *HashDiskBackedRowContainer) SpillToDisk(ctx context.Context) error {
	...
	hdrc := MakeHashDiskRowContainer(h.diskMonitor, h.engine, h.isFile)
