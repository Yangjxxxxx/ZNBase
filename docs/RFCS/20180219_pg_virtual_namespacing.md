...
rules used by ZNBaseDB so that tools that use virtual tables to
...
stored database/table descriptors in ZNBaseDB. In particular it
...
- The changes proposed here will unblock proper user experience of
  ZNBaseDB for users of (graphical or non-graphical) DB inspection
  tools.

- The changes proposed here will enable the alignment of the
  terminology used in ZNBaseDB with that used with PostgreSQL's
  documentation, so that pg's documentation becomes more readily
  applicable to ZNBaseDB.
...
- a ZNBaseDB cluster contains multiple *catalogs*, or
...
- each catalog contains one *physical schema* called `public`,
  and some additional *virtual schemas*, currently including `pg_catalog`, `information_schema` and `zbdb_internal`.
  - a future version of ZNBaseDB may support multiple logical schemas per catalog besides `public`.
...
- the session variable `search_path` contains a list of schema names
  ...
  kv`, ZNBaseDB will search for table `kv` first in the `public`
  schema of catalog `myapp2`, then in the `pg_catalog` schema for
  catalog `myapp2`.

- As a specific ZNBaseDB extension, a SQL client can specify
  a table name as `dbname.tblname` in some conditions to
  provide compatibility with previous ZNBaseDB versions.
...
- We'll adopt the word "catalog" as a synonym for "database" to
  ...
  meaning in PostgreSQL which ZNBaseDB does not yet support.

  - Except for what was called "virtual schema" in ZNBaseDB; these
...
The rule 2.2 is a ZNBaseDB extension (not present in PostgreSQL)
which provides compatibility with previous ZNBaseDB versions.
...
- `SELECT x FROM pg_tables` with empty `database` (ZNBaseDB extension)
  - rule 1 fails
  - rule 2 fails
  - rule 3 applies
  - rule 3.1 applies with C="", N=public
...
ZNBaseDB extensions for compatibility with previous ZNBaseDB versions:
...
- `SELECT x FROM pg_catalog.pg_tables` with empty `database` (ZNBaseDB extension)
  - rule 2 applies
...
The rule 2.4.1 is a new ZNBaseDB extension (not present in PostgreSQL)
which provides compatibility with previous ZNBaseDB versions.
...
The rule 2.2 is a new ZNBaseDB extension.
...
- the vtable generator functions in
  ...
  `sql/zbdb_internal.go` are modified to list the database descriptor
...
A ["problems with ZNBaseDB"
section](#current-problems-with-znbasedb) spells out what are the
...
ZNBaseDB"](#current-problems-with-znbasedb)) if you already know
...
In ZNBaseDB, the mechanical transformation of a name to a table ID
...
Currently ZNBaseDB does not support compound types, so the logic
...
## Current problems with ZNBaseDB

ZNBaseDB currently has several problems that this RFC aims to address:
...
- the name resolution algorithms are different than pg's. This means
  ...
  ZNBaseDB, and vice-versa.

  The specific phrasing of the problem is the following:

  - ZNBaseDB currently uses the logical schema part of a qualified
    name as a key to look up a database ID.
  - ZNBaseDB fails to recognize FQN relation, column and function names.
...
- the introspection tables are insufficiently populated for admin users.
  ...
  `information_schema` tables. Currently, ZNBaseDB will
...
These various problems compound and cause ZNBaseDB to confuse
...
- the deviation in name resolution algorithms is resolved by changing
  ...
  Backward compatibility with previous ZNBaseDB versions is ensured by a "catch"
...
Consequences on other areas of ZNBaseDB: internal queries
ran by ZNBaseDB against itself should use the new naming rules.
