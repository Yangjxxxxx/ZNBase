...
var genManCmd = &cobra.Command{
	Use:   "man",
	Short: "generate man pages for ZNBaseDB",
	Long: `This command generates man pages for ZNBaseDB.
...
func runGenManCmd(cmd *cobra.Command, args []string) error {
	...
	header := &doc.GenManHeader{
		Section: "1",
		Manual:  "ZNBaseDB Manual",
		Source:  fmt.Sprintf("ZNBaseDB %s", info.Tag),
	}
	...
	fmt.Println("Generated ZNBaseDB man pages in", manPath)
	return nil
}
...
var genAutocompleteCmd = &cobra.Command{
	...
	Short: "generate autocompletion script for ZNBaseDB",
	Long: `Generate autocompletion script for ZNBaseDB.

If no arguments are passed, or if 'bash' is passed, a bash completion file is
written to ./znbase.bash. If 'zsh' is passed, a zsh completion file is written
to ./_znbase. Use "--out=/path/to/file" to override the output file location.
...
func runGenAutocompleteCmd(cmd *cobra.Command, args []string) error {
	...
	case "bash":
		if autoCompletePath == "" {
			autoCompletePath = "znbase.bash"
		}
		err = cmd.Root().GenBashCompletionFile(autoCompletePath)
	case "zsh":
		if autoCompletePath == "" {
			autoCompletePath = "_znbase"
		}
