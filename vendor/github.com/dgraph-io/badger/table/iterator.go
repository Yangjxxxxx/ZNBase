...
func (itr *blockIterator) SeekToLast() {
	itr.err = nil
	for itr.Init(); itr.Valid(); itr.Next() { //TODO performance?
	}
	itr.Prev()
}
...
func (itr *Iterator) SeekForPrev(target []byte) {
	itr.seekForPrev(target)
}
...
func (s *ConcatIterator) SeekForPrev(key []byte) {
	s.seek(key, true)
}

// Seek brings us to element >= key if reversed is false. Otherwise, <= key.
func (s *ConcatIterator) seek(key []byte, reversed bool) {
	var idx int
	if !reversed {
...
func (s *ConcatIterator) next(reversed bool) {
	s.cur.Next()
	if s.cur.Valid() {
