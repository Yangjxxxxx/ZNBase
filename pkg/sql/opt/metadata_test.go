...
func TestMetadata(t *testing.T) {
	...
	colID := md.AddColumn("col", types.Int, nil)
	...
	if md.AddColumn("col2", types.Int, nil) != colID {
	...
	md.AddDataSourceDependency(tab.Name(), tab, nil, privilege.CREATE, "")
...
func TestMetadataColumns(t *testing.T) {
	...
	colID := md.AddColumn("alias", types.Int, nil)
	if colID != 1 {
	...
	// Add another column.
	colID = md.AddColumn("alias2", types.String, nil)
	if colID != 2 {
		t.Fatalf("unexpected column id: %d", colID)
	}
