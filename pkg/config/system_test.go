...
func partitionSpaceConfig(
	descriptorID uint32, spans ...*roachpb.LocationMap_PartitionSpace,
) roachpb.KeyValue {
	kv := roachpb.KeyValue{
		Key: config.MakeLocationKey(descriptorID),
	}
	k := keys.MakeTablePrefix(descriptorID)
	for _, v := range spans {
		startKey := append(roachpb.Key{}, k...)
		v.StartKey = roachpb.RKey(append(startKey, v.StartKey...))
		endKey := append(roachpb.Key{}, k...)
		v.EndKey = roachpb.RKey(append(endKey, v.EndKey...))
	}
	if err := kv.Value.SetProto(&roachpb.LocationMap{PartitionSpace: spans}); err != nil {
		panic(err)
	}
	return kv
}

func subPartitionSpace(start, end string) *roachpb.LocationMap_PartitionSpace {
	startKey, endKey := []byte(start), []byte(end)
	if end == "" {
		endKey = roachpb.Key(startKey).PrefixEnd()
	}
	return &roachpb.LocationMap_PartitionSpace{StartKey: startKey, EndKey: endKey}
}
...
func TestComputeSplitKeyTableIDs(t *testing.T) {
	...
	subPartitionSpaceSQL := append(userSQL,
		partitionSpaceConfig(start+1, subPartitionSpace("a", ""), subPartitionSpace("c", "e")),
		partitionSpaceConfig(start+5, subPartitionSpace("b", ""), subPartitionSpace("c", "d"), subPartitionSpace("d", "")))

	sort.Sort(roachpb.KeyValueByKey(userSQL))
	//sort.Sort(roachpb.KeyValueByKey(subzoneSQL))
	sort.Sort(roachpb.KeyValueByKey(subPartitionSpaceSQL))
	...
	testCases := []struct {
		...
		{subPartitionSpaceSQL, tkey(start), roachpb.RKeyMax, tkey(start + 1)},
		{subPartitionSpaceSQL, tkey(start), tkey(start + 1), nil},
		// first one
		{subPartitionSpaceSQL, tkey(start + 1), tkey(start + 2), tkey(start+1, "a")},
		{subPartitionSpaceSQL, tkey(start+1, "a"), tkey(start + 2), tkey(start+1, "b")},
		{subPartitionSpaceSQL, tkey(start+1, "b"), tkey(start + 2), tkey(start+1, "c")},
		{subPartitionSpaceSQL, tkey(start+1, "b"), tkey(start+1, "c"), nil},
		{subPartitionSpaceSQL, tkey(start+1, "ba"), tkey(start+1, "bb"), nil},
		{subPartitionSpaceSQL, tkey(start+1, "c"), tkey(start + 2), tkey(start+1, "e")},
		{subPartitionSpaceSQL, tkey(start+1, "e"), tkey(start + 2), nil},
		{subPartitionSpaceSQL, tkey(start + 4), tkey(start + 6), tkey(start + 5)},
		{subPartitionSpaceSQL, tkey(start + 5), tkey(start + 5), nil},
		{subPartitionSpaceSQL, tkey(start + 5), tkey(start + 6), tkey(start+5, "b")},
		{subPartitionSpaceSQL, tkey(start+5, "a"), tkey(start+5, "ae"), nil},
		{subPartitionSpaceSQL, tkey(start+5, "b"), tkey(start + 6), tkey(start+5, "c")},
		{subPartitionSpaceSQL, tkey(start+5, "c"), tkey(start + 6), tkey(start+5, "d")},
		{subPartitionSpaceSQL, tkey(start+5, "d"), tkey(start + 6), tkey(start+5, "e")},
		{subPartitionSpaceSQL, tkey(start+5, "e"), tkey(start + 6), nil},
