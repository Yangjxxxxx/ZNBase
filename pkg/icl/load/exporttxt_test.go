...
func setupExportableBankTXT(t *testing.T, nodes, rows int) (*sqlutils.SQLRunner, string, func()) {
...
func TestExportImportBankTXT(t *testing.T) {
	defer leaktest.AfterTest(t)()

	db, dir, cleanup := setupExportableBankTXT(t, 3, 100)
	defer cleanup()
	...
	for _, null := range []string{"", "NULL", `\N`} {
		...
		t.Run("null="+null, func(t *testing.T) {
			...
			for _, row := range db.QueryStr(t,
				fmt.Sprintf(`DUMP TO TXT 'nodelocal:///t'
			...
			db.Exec(t, fmt.Sprintf(`LOAD TABLE bank3 %s TXT DATA (%s) WITH delimiter = '|'%s`, schema, fileList, nullIf))
...
func TestMultiNodeExportStmtTXT(t *testing.T) {
	defer leaktest.AfterTest(t)()

	nodes := 5
	exportRows := 100
	db, _, cleanup := setupExportableBankTXT(t, nodes, exportRows*2)
	defer cleanup()
	...
	for tries := 0; tries < maxTries; tries++ {
		chunkSize := 13
		rows := db.Query(t,
			`DUMP TO TXT 'nodelocal:///t'  FROM SELECT * FROM bank WHERE id >= $1 and id < $2 WITH chunk_rows = $3`,
...
func TestExportJoinTXT(t *testing.T) {
	defer leaktest.AfterTest(t)()
	dir, cleanupDir := testutils.TempDir(t)
	defer cleanupDir()
	...
	sqlDB.Exec(t, `DUMP TO TXT 'nodelocal:///join' FROM SELECT * FROM t, t as u`)
}

func TestExportOrderTXT(t *testing.T) {
	defer leaktest.AfterTest(t)()
	dir, cleanupDir := testutils.TempDir(t)
	defer cleanupDir()
	...
	sqlDB.Exec(t, `DUMP TO TXT 'nodelocal:///order' from select * from foo order by y asc limit 2`)
	content, err := ioutil.ReadFile(filepath.Join(dir, "order", "n1.0.txt"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestExportShowTXT(t *testing.T) {
	defer leaktest.AfterTest(t)()
	dir, cleanupDir := testutils.TempDir(t)
	defer cleanupDir()
	...
	sqlDB.Exec(t, `DUMP TO TXT 'nodelocal:///show' FROM SELECT * FROM [SHOW DATABASES]`)
	content, err := ioutil.ReadFile(filepath.Join(dir, "show", "n1.0.txt"))
	if err != nil {
		t.Fatal(err)
	}
