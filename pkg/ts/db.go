...
// DB provides ZNBase's Time Series API.
type DB struct {
	db      *client.DB
	st      *cluster.Settings
...
// StoreData writes the supplied time series data to the znbase server.
...
// storeRollup writes the supplied time series rollup data to the znbase
// server.
