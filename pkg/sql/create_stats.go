...
func (n *createStatsNode) startJob(ctx context.Context, resultsCh chan<- tree.Datums) error {
	...
	if tableDesc.IsVirtualTable() {
		return pgerror.NewError(pgcode.WrongObjectType, "cannot create statistics on virtual tables")
	}

	if tableDesc.IsView() {
		return pgerror.NewError(pgcode.WrongObjectType, "cannot create statistics on views")
	}
	...
	if len(n.ColumnNames) == 0 {
		...
		createStatsColLists = []jobspb.CreateStatsDetails_ColList{{IDs: columnIDs, HasHistogram: true}}
	}
	...
	statement := tree.AsStringWithFlags(n, tree.FmtAlwaysQualifyTableNames|tree.FmtVisableType)
	...
	if err = <-errCh; err != nil {
		...
		if ok && pgerr.Code == pgcode.LockNotAvailable {
...
func createStatsDefaultColumns(
	...
	columns = append(columns, jobspb.CreateStatsDetails_ColList{
		IDs:                 []sqlbase.ColumnID{pkCol},
		HasHistogram:        true,
	...
	for i := range desc.Indexes {
		...
		if !requestedCols.Contains(int(idxCol)) {
			columns = append(
				columns, jobspb.CreateStatsDetails_ColList{
					IDs:                 []sqlbase.ColumnID{idxCol},
					HasHistogram:        true,
					HistogramMaxBuckets: defaultHistogramBuckets,
				},
			)
	...
	for _, row := range rows {
		...
		if payload.Type() == jobspb.TypeCreateStats || payload.Type() == jobspb.TypeAutoCreateStats {
			...
			return pgerror.NewError(
				pgcode.LockNotAvailable, "another CREATE STATISTICS job is already running",
			)
		}
	}
	return nil
...
func (r *createStatsResumer) OnSuccess(ctx context.Context, _ *client.Txn, job *jobs.Job) error {
	...
	return r.evalCtx.ExecCfg.DB.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		return MakeEventLogger(r.evalCtx.ExecCfg).InsertEventRecord(
			...
			timeutil.Now(),
			txn,
			string(EventLogCreateStatistics),
