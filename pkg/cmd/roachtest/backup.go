...
func registerBackup(r *registry) {
	r.Add(testSpec{
		...
		Run: func(ctx context.Context, t *test, c *cluster) {
			...
			// Created via:
			// roachtest --znbase znbase-v2.0.4-fb6939 store-gen --stores=10 bank \
			//           --payload-bytes=10240 --ranges=0 --rows=65104166
			location := `gs://znbase-fixtures/workload/bank/version=1.0.0,payload-bytes=10240,ranges=0,rows=65104166,seed=1`
			...
			m.Go(func(ctx context.Context) error {
				...
				BACKUP bank.bank TO 'gs://znbasedb-backup-testing/`+c.name+`'"`)
				return nil
			})
			m.Wait()
		},
	...
	r.Add(testSpec{
		...
		Run: func(ctx context.Context, t *test, c *cluster) {
			...
			warehouses := 1
			backupDir := "gs://znbasedb-backup-testing/" + c.name
