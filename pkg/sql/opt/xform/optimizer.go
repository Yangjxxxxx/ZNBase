...
// OptimizerHint 强制使用第几条计划
type OptimizerHint struct {
	// Hint sign for plan_nth
	ForcePlanTh bool
	PlanTh      int
	OpenForce   bool
}
...
// InitXQ inits the default costerXQ
func (o *Optimizer) InitXQ(locatespace roachpb.LocationName) {

	o.defaultCosterXQ = costerxq{
		&o.defaultCoster,
		locatespace,
	}

	o.coster = &o.defaultCosterXQ
}
...
func (o *Optimizer) Optimize() (_ opt.Expr, err error) {
	...
	if o.ForcePlanTh && o.PlanTh > 0 {
		r := o.trimTheTreeWithIndex(root, &o.PlanTh)
		if len(r) >= o.PlanTh {
			root = r[o.PlanTh-1]
			o.OpenForce = true
			defer func() {
...
func (o *Optimizer) optimizeGroup(grp memo.RelExpr, required *physical.Required) *groupState {
	if o.OpenForce {
		...
		o.optimizeGroupMember(state, grp, required)
		return state
	}
...
func (o *Optimizer) setLowestCostTree(parent opt.Expr, parentProps *physical.Required) opt.Expr {
	...
	case memo.RelExpr:
		if o.OpenForce {
			relParent = t
			state := o.lookupOptState(t, parentProps)
...
//    |                     |
// [Memo3] - [Memo3']   [Memo3'']
// 在生成的memo森林中，可以表示出很多种不同的计划，我们现在对每个计划进行标记，并且使计划的执行可控
// 可控的含义是 可以按照输入的sql进行对计划的生成进行控制
// ex : explain planth(2) analyze selectStmt 这个语句规定按照第二种计划运行
// 在上面的例子中，我们可以通过下面函数得到的结构为，并以此结构进行计划生成
// [Memo1]
//    |
// [Memo2]
//    |
// [Memo3']
func (o *Optimizer) trimTheTreeWithIndex(grp memo.RelExpr, index *int) []memo.RelExpr {
	//对于每个输入的memo 都可能发掘很多不同路径的memo，所以我们要全部返回
	ret := make([]memo.RelExpr, 0)
	// 记录一共有多少种路径
	total := 0
	// 对于每个next 进行记录（横向统计）
	for i, member := 0, grp; member != nil; i, member = i+1, member.NextExpr() {
		...
		if childCount == 0 {
			//o.Construct(member,nil)
			ret = append(ret, member)
			total++
			continue
		}
		// treeSlice用来记录一个member的每个children的状态
		treeSlice := make([][]memo.RelExpr, childCount)
		count := 1
		com := make([]int, 0)
		for j := 0; j < childCount; j++ {
			if e, ok := member.Child(j).(memo.RelExpr); ok {
				treeSlice[j] = o.trimTheTreeWithIndex(e, index)
				com = append(com, len(treeSlice[j]))
				count *= len(treeSlice[j])
			}
		}
		//如果index为0 那么默认前面
		//当每个child都发掘后，如果组合数已经大于我们需要的组合，直接返回，并且将index置为0
		total += count

		remainders, cnt := calculateCombinationIndex(*index, com)
		combinaMeta := make([][]memo.RelExpr, *index) //存储当层所有的可能,为index或最大可能

		if len(com) == 1 {
			for i, remainder := range remainders {
				com1 := make([][]memo.RelExpr, 0)
				for j := 0; j < remainder; j++ {
					m := append(combinaMeta[j], treeSlice[i][j])
					com1 = append(com1, m)
				}
				combinaMeta = com1
			}
				...
				if i == 0 {
					...
					for r := 0; r <= s1; r++ {
						for j := 0; j < com[0]; j++ {
							...
							m := append(combinaMeta[k], treeSlice[i][j])
							com1 = append(com1, m)
							k++
						}
					}
		...
		for k := 0; k < cnt; k++ {
			constructExpr := o.construct(member, combinaMeta[k])
			ret = append(ret, constructExpr)
		}
	}
	return ret
}

//返回第total种对应的列值和这一层能统计出的最多种数
//例：
//6, [1, 8] 返回 [1, 6], 6
//8, [5, 7] 返回 [3, 2],  8
//5, [5, 7] 返回 [5, 1],  5
func calculateCombinationIndex(total int, com []int) ([]int, int) {
	fakeTotal := 1
	for _, sum := range com {
		fakeTotal *= sum
	}
	if total >= fakeTotal {
		return com, fakeTotal
	}
	l := len(com)
	ret := make([]int, 0)
	if l == 1 {
		...
		s1 := total % com[0]
		if s1 == 0 { //当total 是第一列的倍数时为不同情况
			ret = append(ret, com[0])
			s2 := total / com[0]
			ret = append(ret, s2)
		} else {
			ret = append(ret, s1)
...
func (o *Optimizer) construct(expr memo.RelExpr, children []memo.RelExpr) memo.RelExpr {
	switch t := expr.(type) {
