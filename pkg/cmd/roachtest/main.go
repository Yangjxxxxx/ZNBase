...
func main() {
	...
	var rootCmd = &cobra.Command{
		...
		Short: "roachtest tool for testing znbase clusters",
		Long: `roachtest is a tool for testing znbase clusters.
`,
	...
	rootCmd.PersistentFlags().StringVar(
		&znbase, "znbase", "", "path to znbase binary to use")
	rootCmd.PersistentFlags().StringVar(
...
`,
	...
	var runCmd = &cobra.Command{
		Use:   "run [tests]",
		Short: "run automated tests on znbase cluster",
		Long: `Run automated tests on existing or ephemeral znbase clusters.
...
`,
	...
	var benchCmd = &cobra.Command{
		...
		Short: "run automated benchmarks on znbase cluster",
		Long:  `Run automated benchmarks on existing or ephemeral znbase clusters.`,
		RunE: func(_ *cobra.Command, args []string) error {
			if count <= 0 {
				return fmt.Errorf("--count (%d) must by greater than 0", count)
			}
...
ZNBase cluster with existing data.
`,
