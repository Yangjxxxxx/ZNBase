...
func (h *txnHeartbeater) SendLocked(
	...
	if ba.Header.Txn != nil {
		h.mu.txn.IsolationLevel = ba.Header.Txn.IsolationLevel
	}
	...
	arg, haveEndTxn := ba.GetArg(roachpb.EndTransaction)

	if haveEndTxn && h.HeartBeat {
		arg.(*roachpb.EndTransactionRequest).Heartbeat = true
	}
...
func (h *txnHeartbeater) heartbeat(ctx context.Context) bool {
	...
	ba.Header = roachpb.Header{BackFill: h.backFill}
	ba.Txn = txn
	...
	if pErr != nil {
		...
		case *roachpb.TransactionAbortedError, *roachpb.TransactionDeadlockError:
...
func (h *txnHeartbeater) abortTxnAsyncLocked(ctx context.Context) {
	...
	ba := roachpb.BatchRequest{}
	ba.Header = roachpb.Header{Txn: txn, BackFill: h.backFill}
