...
func TestStmtBuf(t *testing.T) {
	...
	ctx := context.TODO()
	s1, err := parser.ParseOne("SELECT 1", false)
	if err != nil {
		t.Fatal(err)
	}
	s2, err := parser.ParseOne("SELECT 2", false)
	if err != nil {
		t.Fatal(err)
	}
	s3, err := parser.ParseOne("SELECT 3", false)
	if err != nil {
		t.Fatal(err)
	}
	s4, err := parser.ParseOne("SELECT 4", false)
	if err != nil {
		t.Fatal(err)
	}
	buf := NewStmtBuf()
...
func TestStmtBufLtrim(t *testing.T) {
	...
	buf := NewStmtBuf()
	for i := 0; i < 5; i++ {
		stmt, err := parser.ParseOne(fmt.Sprintf("SELECT %d", i), false)
		if err != nil {
			t.Fatal(err)
		}
...
func TestStmtBufClose(t *testing.T) {
	...
	ctx := context.TODO()
	buf := NewStmtBuf()
	stmt, err := parser.ParseOne("SELECT 1", false)
	if err != nil {
		t.Fatal(err)
	}
	mustPush(ctx, t, buf, ExecStmt{Statement: stmt})
