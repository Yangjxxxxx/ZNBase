...
func CmdHelper(
	...
	const znbaseDefaultURL = `postgres://root@localhost:26257?sslmode=disable`
...
// read / write blocks of random data into a table in znbase DB. The function
...
func runInit(gen workload.Generator, urls []string, dbName string) error {
	...
	initDB, err := gosql.Open(`znbase`, strings.Join(urls, ` `))
	if err != nil {
		return err
	}
...
func startPProfEndPoint(ctx context.Context) {
	b := envutil.EnvOrDefaultInt64("ZNBASE_BLOCK_PROFILE_RATE",
		10000000 /* 1 sample per 10 milliseconds spent blocking */)

	m := envutil.EnvOrDefaultInt("ZNBASE_MUTEX_PROFILE_RATE",
