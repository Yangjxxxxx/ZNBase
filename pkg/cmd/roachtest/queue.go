...
func registerQueue(r *registry) {
	// One node runs the workload generator, all other nodes host ZNBaseDB.
	const numNodes = 2
	r.Add(testSpec{
		//Skip:    "https://github.com/znbasedb/znbase/issues/17229",
...
func runQueue(ctx context.Context, t *test, c *cluster) {
	...
	// Distribute programs to the correct nodes and start ZNBaseDB.
	c.Put(ctx, znbase, "./znbase", c.Range(1, dbNodeCount))
