...
func _ROWS_TO_COL_VEC(
	...
	datumToPhysicalFn := sem.GetDatumToPhysicalFn(columnType)
	for i := range rows {
		row := rows[i]
		if row[columnIdx].Datum == nil {
			ct, _ := sqlbase.DatumType1ToColumnType(*columnType)
			if err = row[columnIdx].EnsureDecoded(&ct, alloc); err != nil {
				break
			}
		}
