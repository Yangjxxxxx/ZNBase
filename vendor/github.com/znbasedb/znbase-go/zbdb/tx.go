...
// Package zbdb provides helpers for using ZNBaseDB in client
// applications.
package zbdb
...
//
// For more information about ZNBaseDB's transaction model see
// https://znbaselabs.com/docs/stable/transactions.html.
//
...
//
//    znbase.ExecuteTx(ctx, db, txopts, func (tx *sql.Tx) error {
...
func ExecuteInTx(ctx context.Context, tx Tx, fn func() error) (err error) {
	...
	// Specify that we intend to retry this txn in case of ZNBaseDB retryable
	// errors.
	if _, err = tx.ExecContext(ctx, "SAVEPOINT znbase_restart"); err != nil {
		return err
	}
	...
	for {
		...
		err = fn()
		if err == nil {
			// RELEASE acts like COMMIT in ZNBaseDB. We use it since it gives us an
			// opportunity to react to retryable errors, whereas tx.Commit() doesn't.
			released = true
			if _, err = tx.ExecContext(ctx, "RELEASE SAVEPOINT znbase_restart"); err == nil {
				return nil
			}
		}
		...
		//  - the ZNBase extension errcode RetriableError:CR000. This extension
		//    has been removed server-side, but support for it has been left here for
		//    now to maintain backwards compatibility.
		code := errCode(err)
		if retryable := (code == "CR000" || code == "40001" || code == "XX000"); !retryable {
			if released {
		...
		if _, retryErr := tx.ExecContext(ctx, "ROLLBACK TO SAVEPOINT znbase_restart"); retryErr != nil {
