...
//PreferredEncoding is encCodeing DatumEncoding_ASCENDING_KEY
	...
	for _, b := range builtins {
		...
		if match {
			...
			constructAgg := func(evalCtx *tree.EvalContext) tree.WindowFunc {
				return b.WindowFunc(datumTypes, evalCtx, nil)
			}
...
var _ runbase.Processor = &windower{}
var _ runbase.RowSource = &windower{}
...
func newWindower(
	...
	ctx := evalCtx.Ctx()
	memMonitor := runbase.NewMonitor(ctx, evalCtx.Mon, "windower-mem")
	...
	w.outputTypes = make([]sqlbase.ColumnType, len(w.inputTypes)+len(windowFns))
	...
	for _, windowFn := range windowFns {
		...
		argTypes := make([]sqlbase.ColumnType, len(windowFn.ArgsIdxs))
	...
	if err := w.InitWithEvalCtx(
		...
		output,
		memMonitor,
		runbase.ProcStateOpts{InputsToDrain: []runbase.RowSource{w.input},
...
func (w *windower) GetBatch(ctx context.Context, respons chan roachpb.BlockStruct) context.Context {
	return ctx
}

//SetChan is part of the RowSource interface.
func (w *windower) SetChan() {}

// Next is part of the RowSource interface.
func (w *windower) Next() (sqlbase.EncDatumRow, *distsqlpb.ProducerMetadata) {
	for w.State == runbase.StateRunning {
...
func (w *windower) processPartition(
	...
	for _, windowFnIdx := range w.orderOfWindowFnsProcessing {
		...
		if windowFn.frame != nil {
			...
			frameRun.Frame.IsIgnoreNulls = windowFn.frame.IsIgnoreNulls
			...
			if frameRun.RangeModeWithOffsets() {
				...
				if !found {
					return pgerror.NewErrorf(pgcode.Windowing, "given logical offset cannot be combined with ordering column")
				}
