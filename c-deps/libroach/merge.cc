...
znbase::roachpb::ValueType GetTag(const std::string& val) {
  if (val.size() < kHeaderSize) {
    return znbase::roachpb::UNKNOWN;
  }
  return znbase::roachpb::ValueType(val[kTagPos]);
}

void SetTag(std::string* val, znbase::roachpb::ValueType tag) { (*val)[kTagPos] = tag; }
...
void SerializeProtoToValue(std::string* val, const google::protobuf::MessageLite& msg) {
  ...
  SetTag(val, znbase::roachpb::BYTES);
  msg.AppendToString(val);
}

// Method used to sort InternalTimeSeriesSamples.
bool TimeSeriesSampleOrdering(const znbase::roachpb::InternalTimeSeriesSample* a,
                              const znbase::roachpb::InternalTimeSeriesSample* b) {
...
bool IsTimeSeriesData(const std::string& val) {
  return GetTag(val) == znbase::roachpb::TIMESERIES;
}

void SerializeTimeSeriesToValue(std::string* val,
                                const znbase::roachpb::InternalTimeSeriesData& ts) {
  SerializeProtoToValue(val, ts);
  SetTag(val, znbase::roachpb::TIMESERIES);
}
...
WARN_UNUSED_RESULT bool MergeTimeSeriesValues(std::string* left, const std::string& right,
  ...
  znbase::roachpb::InternalTimeSeriesData left_ts;
  znbase::roachpb::InternalTimeSeriesData right_ts;
  ...
  if (use_column_format) {
    ...
    znbase::roachpb::InternalTimeSeriesData new_ts;
    ...
    while (left_front != left_end || right_front != right_end) {
      ...
      znbase::roachpb::InternalTimeSeriesSample* ns = new_ts.add_samples();
      ...
      znbase::roachpb::InternalTimeSeriesSample src;
...
WARN_UNUSED_RESULT bool ConsolidateTimeSeriesValue(std::string* val, rocksdb::Logger* logger) {
  // Attempt to parse TimeSeriesData from both Values.
  znbase::roachpb::InternalTimeSeriesData val_ts;
  ...
  if (val_ts.offset_size() > 0) {
    ...
    using sample = znbase::roachpb::InternalTimeSeriesSample;
    auto it =
...
class DBMergeOperator : public rocksdb::MergeOperator {
  virtual const char* Name() const { return "znbase_merge_operator"; }
 ...
 private:
  bool MergeOne(znbase::storage::engine::enginepb::MVCCMetadata* meta,
                const rocksdb::Slice& operand, bool full_merge,
                rocksdb::Logger* logger) const WARN_UNUSED_RESULT {
    znbase::storage::engine::enginepb::MVCCMetadata operand_meta;
...
void convertToColumnar(znbase::roachpb::InternalTimeSeriesData* data) {
...
void sortAndDeduplicateColumns(znbase::roachpb::InternalTimeSeriesData* data,
                               int first_unsorted) {
...
WARN_UNUSED_RESULT bool MergeValues(znbase::storage::engine::enginepb::MVCCMetadata* left,
                                    const znbase::storage::engine::enginepb::MVCCMetadata& right,
                                    bool full_merge, rocksdb::Logger* logger) {
...
DBStatus MergeResult(znbase::storage::engine::enginepb::MVCCMetadata* meta, DBString* result) {
