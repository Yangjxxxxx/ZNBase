...
import (
	...
	_ "github.com/znbasedb/znbase/pkg/keys" // hook up pretty printer
...
func newTestRegistration(
	span roachpb.Span, ts hlc.Timestamp, catchup engine.SimpleIterator, withDiff bool,
) *testRegistration {
	s := newTestStream()
...
func TestRegistrationBasic(t *testing.T) {
	...
	go noCatchupReg.runOutputLoop(context.Background(), hlc.Timestamp{})
	...
	go catchupReg.runOutputLoop(context.Background(), hlc.Timestamp{WallTime: 1})
	...
	go disconnectReg.runOutputLoop(context.Background(), hlc.Timestamp{})
	...
	go overflowReg.runOutputLoop(context.Background(), hlc.Timestamp{})
	...
	go streamErrReg.runOutputLoop(context.Background(), hlc.Timestamp{})
	...
	go streamCancelReg.runOutputLoop(context.Background(), hlc.Timestamp{})
...
func TestRegistrationCatchUpScan(t *testing.T) {
	...
	require.NoError(t, r.runCatchupScan(hlc.Timestamp{WallTime: 4}))
...
func TestRegistryBasic(t *testing.T) {
	...
	ev1.MustSetValue(&roachpb.RangeFeedValue{Value: val, PrevValue: val})
	ev2.MustSetValue(&roachpb.RangeFeedValue{Value: val, PrevValue: val})
	ev3.MustSetValue(&roachpb.RangeFeedValue{Value: val, PrevValue: val})
	ev4.MustSetValue(&roachpb.RangeFeedValue{Value: val, PrevValue: val})
	...
	go rAB.runOutputLoop(context.Background(), hlc.Timestamp{})
	go rBC.runOutputLoop(context.Background(), hlc.Timestamp{})
	go rCD.runOutputLoop(context.Background(), hlc.Timestamp{})
	go rAC.runOutputLoop(context.Background(), hlc.Timestamp{})
...
func TestRegistryPublishBeneathStartTimestamp(t *testing.T) {
	...
	go r.runOutputLoop(context.Background(), hlc.Timestamp{})
