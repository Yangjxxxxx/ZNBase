...
var _ runbase.Processor = &sstWriter{}
...
func (sp *sstWriter) Run(ctx context.Context) {
	...
	err := func() error {
		...
		metrics := sp.flowCtx.Cfg.JobRegistry.MetricsStruct().Load.(*Metrics)
		...
		defer iter.Close()
		iter.Rewind()
		maxSize := storageicl.MaxImportBatchSize(sp.settings)
		for i, span := range sp.spec.Spans {
			...
			group.GoCtx(func(ctx context.Context) error {
				...
				for sst := range contentCh {
					...
					if sp.spec.Destination == "" {
						...
						before := timeutil.Now()
						if _, err := bulk.AddSSTable(ctx, sp.db, sst.span.Key, sst.span.EndKey, sst.data,
							true /* disallowShadowing */, enginepb.MVCCStats{} /* ms */, sp.settings); err != nil {
							return err
						}
						after := timeutil.Now()
						//log.Info(ctx, "----------------------------------Time to add sst----------------------------------", after.Sub(before).Nanoseconds())
						//记录AddSSTable所消耗的时间
						metrics.AddSSTableTime.RecordValue(after.Sub(before).Nanoseconds())
					} else {
						checksum, err = storageicl.SHA512ChecksumData(sst.data)
						if err != nil {
							return err
						}
				...
				metrics.AddSSTableTime.RecordValue(0)
				return nil
			})
			...
			if err := job.FractionProgressed(ctx, func(ctx context.Context, details jobspb.ProgressDetails) float32 {
				...
				progressErr = func() error {
					...
					for _, spanProgress := range d.SpanProgress {
						if spanProgress.Table.ID == sp.spec.Table.ID {
							sg.Add(spanProgress.SpanProgress...)
							sg.Add(finished)
							spanProgress.SpanProgress = sg.Slice()
						}
					}
					return nil
				}()
...
func init() {
	runbase.NewSSTWriterProcessor = newSSTWriterProcessor
}
