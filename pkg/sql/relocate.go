...
func (n *relocateNode) Next(params runParams) (bool, error) {
	...
	if n.relocateLease {
		...
		if !(data[0].ResolvedType().Equivalent(types.TArray{Typ: types.Int}) || data[0].ResolvedType().Equivalent(types.TArray{Typ: types.Int2}) || data[0].ResolvedType().Equivalent(types.TArray{Typ: types.Int4})) {
