...
#define digitbuf_alloc(size) ((NumericDigit *) TYPES_alloc(size))
...
numeric *
TYPESnumeric_new(void)
{
	numeric    *var;

	if ((var = (numeric *) TYPES_alloc(sizeof(numeric))) == NULL)
		return NULL;
...
decimal *
TYPESdecimal_new(void)
{
	decimal    *var;

	if ((var = (decimal *) TYPES_alloc(sizeof(decimal))) == NULL)
		return NULL;
...
get_str_from_var(numeric *var, int dscale)
{
	 ...
	 */
	if ((str = (char *) TYPES_alloc(Max(0, dscale) + Max(0, var->weight) + 4)) == NULL)
		return NULL;
	cp = str;
...
numeric *
TYPESnumeric_from_asc(char *str, char **endptr)
{
	numeric    *value = (numeric *) TYPES_alloc(sizeof(numeric));
	int			ret;
...
char *
TYPESnumeric_to_asc(numeric *num, int dscale)
{
	numeric    *numcopy = TYPESnumeric_new();
	char	   *s;

	if (numcopy == NULL)
		return NULL;

	if (TYPESnumeric_copy(num, numcopy) < 0)
	{
		TYPESnumeric_free(numcopy);
		return NULL;
	}
 ...
 */
int
TYPESnumeric_add(numeric *var1, numeric *var2, numeric *result)
{
	/*
 ...
 */
int
TYPESnumeric_sub(numeric *var1, numeric *var2, numeric *result)
{
	/*
	 * Decide on the signs of the two variables what to do
	 */
 ...
 */
int
TYPESnumeric_mul(numeric *var1, numeric *var2, numeric *result)
{
	NumericDigit *res_buf;
	NumericDigit *res_digits;
	int			res_ndigits;
...
int
TYPESnumeric_div(numeric *var1, numeric *var2, numeric *result)
{
	NumericDigit *res_digits;
	int			res_ndigits;
	int			res_sign;
...
int
TYPESnumeric_cmp(numeric *var1, numeric *var2)
{
...
int
TYPESnumeric_from_int(signed int int_val, numeric *var)
{
	/* implicit conversion */
	signed long int long_int = int_val;

	return TYPESnumeric_from_long(long_int, var);
}

int
TYPESnumeric_from_long(signed long int long_val, numeric *var)
{
...
int
TYPESnumeric_copy(numeric *src, numeric *dst)
{
	int			i;
...
int
TYPESnumeric_from_double(double d, numeric *dst)
{
	...
	if ((tmp = TYPESnumeric_from_asc(buffer, NULL)) == NULL)
		return -1;
	i = TYPESnumeric_copy(tmp, dst);
	TYPESnumeric_free(tmp);
	if (i != 0)
		return -1;
...
numericvar_to_double(numeric *var, double *dp)
{
	...
	double		val;
	char	   *endptr;
	numeric    *varcopy = TYPESnumeric_new();

	if (varcopy == NULL)
		return -1;

	if (TYPESnumeric_copy(var, varcopy) < 0)
	{
		TYPESnumeric_free(varcopy);
		return -1;
	}
...
int
TYPESnumeric_to_double(numeric *nv, double *dp)
{
	double		tmp;
...
int
TYPESnumeric_to_int(numeric *nv, int *ip)
{
	long		l;
	int			i;

	if ((i = TYPESnumeric_to_long(nv, &l)) != 0)
		return i;
...
int
TYPESnumeric_to_long(numeric *nv, long *lp)
{
	char	   *s = TYPESnumeric_to_asc(nv, 0);
	char	   *endptr;
...
int
TYPESnumeric_to_decimal(numeric *src, decimal *dst)
{
	int			i;
...
int
TYPESnumeric_from_decimal(decimal *src, numeric *dst)
{
	int			i;
