...
func registerInterleaved(r *registry) {
	...
	runInterleaved := func(
		...
		znbaseWest := c.Range(1, 3)
		workloadWest := c.Node(4)
		znbaseEast := c.Range(5, 7)
		workloadEast := c.Node(8)
		znbaseCentral := c.Range(9, 11)
		workloadCentral := c.Node(12)

		znbaseNodes := znbaseWest.merge(znbaseEast.merge(znbaseCentral))
		workloadNodes := workloadWest.merge(workloadEast.merge(workloadCentral))

		c.l.Printf("znbase nodes: %s", znbaseNodes.String()[1:])
		...
		c.Run(ctx, znbaseEast.randNode(), cmdInit)

		duration := " --duration " + ifLocal("10s", "10m")
		histograms := " --histograms logs/stats.json"

		createCmd := func(locality string, znbaseNodes nodeListOption) string {
			return fmt.Sprintf(
		...
		runLocality("west", workloadWest, createCmd("west", znbaseWest))
		runLocality("east", workloadEast, createCmd("east", znbaseEast))
