...
type windowNode struct {
	...
	// The window functions handled by this windowNode.s
...
// function application by comznbaseng specific window definition from a
...
func (p *planner) constructWindowDefinitions(
	...
	for _, windowDef := range sc.Window {
		...
		if _, ok := namedWindowSpecs[name]; ok {
			return pgerror.NewErrorf(pgcode.Windowing, "window %q is already defined", name)
		}
	...
	for windowFnIdx, windowFn := range n.funcs {
		...
		if windowFn.expr.Filter != nil {
			...
			col, renderExpr, err := p.computeRender(ctx, tree.SelectExpr{Expr: filterExpr}, types.Bool, s.sourceInfo, s.ivarHelper, autoGenerateRenderOutputName, -1)
			if err != nil {
				return err
			}
		...
		for _, partition := range windowDef.Partitions {
			if funcExpr, ok := partition.(*tree.FuncExpr); ok {
				if funcExpr.IsWindowFunctionApplication() {
					return pgerror.NewErrorf(pgcode.Windowing, "window functions are not allowed in window definitions")
				}
			}
			cols, exprs, _, err := p.computeRenderAllowingStars(ctx, tree.SelectExpr{Expr: partition}, types.Any, nil, s.sourceInfo, s.ivarHelper, autoGenerateRenderOutputName)
			if err != nil {
				return err
			}
		...
		for _, orderBy := range windowDef.OrderBy {
			...
			cols, exprs, _, err := p.computeRenderAllowingStars(ctx, tree.SelectExpr{Expr: orderBy.Expr}, types.Any, nil, s.sourceInfo, s.ivarHelper, autoGenerateRenderOutputName)
			if err != nil {
				return err
			}
...
func constructWindowDef(
	...
	if !ok {
		return def, pgerror.NewErrorf(pgcode.UndefinedObject, "window %q does not exist", refName)
	}
	if !modifyRef {
		...
		case t.IsWindowFunctionApplication():
			...
			for _, argExpr := range t.Exprs {
				if v.subWindowVisitor.ContainsWindowFunc(argExpr) {
					v.err = pgerror.NewErrorf(pgcode.Windowing, "window function calls cannot be nested")
					return false, expr
				}
			}
...
func (c *windowNodeColAndAggContainer) GetVisibleType(idx int) string {
	return ""
}

// SetForInsertOrUpdate implements the parser.IndexedVarContainer interface.
func (c *windowNodeColAndAggContainer) SetForInsertOrUpdate(b bool) {
	c.IsInsertOrUpdate = b
}

// GetForInsertOrUpdate implements the parser.IndexedVarContainer interface.
func (c *windowNodeColAndAggContainer) GetForInsertOrUpdate() bool {
