...
func (l *lexer) initLastErr() {
	if l.lastError == nil {
		l.lastError = pgerror.NewError(pgcode.Syntax, "syntax error")
	}
}
...
func (l *lexer) setErr(err error) {
	if pgErr, ok := err.(*pgerror.Error); ok {
		...
		l.lastError = pgerror.NewErrorf(pgcode.Syntax, "syntax error: %v", err)
	}
	l.populateErrorDetails()
}

func (l *lexer) Error(e string) {
	l.lastError = pgerror.NewError(pgcode.Syntax, e)
	l.populateErrorDetails()
}
