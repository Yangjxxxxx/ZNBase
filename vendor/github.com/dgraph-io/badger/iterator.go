...
func (item *Item) SafeValue() ([]byte, error) {
	var val_copy []byte
	val_copy, err := item.ValueCopy(nil)
	return val_copy, err
}

func (item *Item) UnsafeValue() []byte {
	item.wg.Wait()
	if item.status != prefetched {
		item.prefetchValue()
	}
...
func (txn *Txn) NewIterator(opt IteratorOptions) *Iterator {
	...
	if itr := txn.newPendingWritesIterator(); itr != nil {
...
func (it *Iterator) parseItem(reversed bool) bool {
	mi := it.iitr
	key := mi.Key()
...
FILL:
	...
	if !reversed || !mi.Valid() { // Forward direction, or invalid.
		setItem(item)
		return true
	}
...
func (it *Iterator) prefetch(reversed bool) {
	prefetchSize := 2
...
// iterating backwards.
func (it *Iterator) seek(key []byte, reversed bool) {
