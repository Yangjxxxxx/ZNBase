...
const backgroundEnvVar = "ZNBASE_BACKGROUND_RESTART"

func init() {
	initCLIDefaults()

	// Every command but start will inherit the following setting.
	znbaseCmd.PersistentPreRunE = func(cmd *cobra.Command, _ []string) error {
		extraClientFlagInit()
	...
	// top-level znbase command.
	pf := znbaseCmd.PersistentFlags()
...

	{
		...
		VarFlag(f, &serverCfg.LocationName, cliflags.Space)
	...
	for _, cmd := range []*cobra.Command{demoCmd, StartCmd} {
		...
		VarFlag(f,
			pflag.PFlagFromGoFlag(flag.Lookup(logflags.LogFilesArchiveOnName)).Value,
			cliflags.LogFileArchiveOn)
		VarFlag(f,
	...
	clientCmds = append(clientCmds, storeCmds...)
	...
	tableOutputCommands = append(tableOutputCommands, storeCmds...)
