...
import (
	...
	"github.com/znbasedb/znbase/pkg/testutils/zerofields"
	...
	"github.com/znbasedb/znbase/pkg/util/shuffle"
...
var (
	...
	txn1         = &roachpb.Transaction{TxnMeta: enginepb.TxnMeta{Key: roachpb.Key("a"), ID: txn1ID, Epoch: 1, WriteTimestamp: hlc.Timestamp{Logical: 1}}, ReadTimestamp: hlc.Timestamp{Logical: 1}}
	txn1Commit   = &roachpb.Transaction{TxnMeta: enginepb.TxnMeta{Key: roachpb.Key("a"), ID: txn1ID, Epoch: 1, WriteTimestamp: hlc.Timestamp{Logical: 1}}, ReadTimestamp: hlc.Timestamp{Logical: 1}, Status: roachpb.COMMITTED}
	txn1Abort    = &roachpb.Transaction{TxnMeta: enginepb.TxnMeta{Key: roachpb.Key("a"), ID: txn1ID, Epoch: 1}, Status: roachpb.ABORTED}
	txn1e2       = &roachpb.Transaction{TxnMeta: enginepb.TxnMeta{Key: roachpb.Key("a"), ID: txn1ID, Epoch: 2, WriteTimestamp: hlc.Timestamp{Logical: 1}}, ReadTimestamp: hlc.Timestamp{Logical: 1}}
	txn1e2Commit = &roachpb.Transaction{TxnMeta: enginepb.TxnMeta{Key: roachpb.Key("a"), ID: txn1ID, Epoch: 2, WriteTimestamp: hlc.Timestamp{Logical: 1}}, ReadTimestamp: hlc.Timestamp{Logical: 1}, Status: roachpb.COMMITTED}
	txn2         = &roachpb.Transaction{TxnMeta: enginepb.TxnMeta{Key: roachpb.Key("a"), ID: txn2ID, WriteTimestamp: hlc.Timestamp{Logical: 1}}, ReadTimestamp: hlc.Timestamp{Logical: 1}}
	txn2Commit   = &roachpb.Transaction{TxnMeta: enginepb.TxnMeta{Key: roachpb.Key("a"), ID: txn2ID, WriteTimestamp: hlc.Timestamp{Logical: 1}}, ReadTimestamp: hlc.Timestamp{Logical: 1}, Status: roachpb.COMMITTED}
...
func unwrapMVCCScan(
	...
	res, err := MVCCScan(ctx, reader, key, endKey, max, timestamp, opts)
	return res.KVs, res.ResumeSpan, res.Intents, err
}

// createTestRocksDBEngine returns a new in-memory RocksDB engine with 1MB of
// storage capacity.
func createTestRocksDBEngine(attrs roachpb.Attributes, cacheSize int64) Engine {
	return newRocksDBInMem(base.StoreSpec{Attributes: attrs}, cacheSize)
}

// createTestPebbleEngine returns a new in-memory Pebble storage engine.
func createTestPebbleEngine(attrs roachpb.Attributes, cacheSize int64) Engine {
	return newPebbleInMem(context.Background(), attrs, cacheSize)
}

var mvccEngineImpls = []struct {
	name   string
	create func(attrs roachpb.Attributes, cacheSize int64) Engine
}{
...
func TestMVCCEmptyKey(t *testing.T) {
	...
	if _, _, _, err := unwrapMVCCScan(ctx, engine, key, testKey1, math.MaxInt64, ts, MVCCScanOptions{}); err != nil {
		t.Errorf("empty key allowed for start key in scan; got %s", err)
	}
	if _, _, _, err := unwrapMVCCScan(ctx, engine, testKey1, key, math.MaxInt64, ts, MVCCScanOptions{}); err == nil {
		t.Error("expected empty key error")
	}
	if _, err := MVCCResolveWriteIntent(ctx, engine, nil, roachpb.LockUpdate{}); err == nil {
		t.Error("expected empty key error")
	}
}
...
func TestMVCCScanWriteIntentError(t *testing.T) {
	...
	for i, scan := range scanCases {
		...
		kvs, _, intents, err := unwrapMVCCScan(ctx, engine, testKey1, testKey4.Next(), math.MaxInt64,
...
func TestMVCCInvalidateIterator(t *testing.T) {
	...
	for _, which := range []string{"get", "scan", "findSplitKey", "computeStats"} {
		t.Run(which, func(t *testing.T) {
			...
			case "scan":
				_, _, _, err = unwrapMVCCScan(ctx, batch, key, roachpb.KeyMax, math.MaxInt64, ts2, MVCCScanOptions{})
			case "findSplitKey":
...
func TestMVCCScan(t *testing.T) {
	...
	kvs, resumeSpan, _, err := unwrapMVCCScan(ctx, engine, testKey2, testKey4, math.MaxInt64,
	...
	kvs, resumeSpan, _, err = unwrapMVCCScan(ctx, engine, testKey4, keyMax, math.MaxInt64,
		hlc.Timestamp{WallTime: 1}, MVCCScanOptions{})
	if err != nil {
		t.Fatal(err)
	}
	...
	kvs, _, _, err = unwrapMVCCScan(ctx, engine, keyMin, testKey2, math.MaxInt64,
		hlc.Timestamp{WallTime: 1}, MVCCScanOptions{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestMVCCScanMaxNum(t *testing.T) {
	...
	kvs, resumeSpan, _, err := unwrapMVCCScan(ctx, engine, testKey2, testKey4, 1,
		hlc.Timestamp{WallTime: 1}, MVCCScanOptions{})
	if err != nil {
		t.Fatal(err)
	}
	...
	kvs, resumeSpan, _, err = unwrapMVCCScan(ctx, engine, testKey2, testKey4, 0,
		hlc.Timestamp{WallTime: 1}, MVCCScanOptions{})
	if err != nil {
		t.Fatal(err)
	}
	...
	kvs, resumeSpan, _, err = unwrapMVCCScan(ctx, engine, testKey4, testKey5, 1,
		hlc.Timestamp{WallTime: 1}, MVCCScanOptions{})
	if err != nil {
		t.Fatal(err)
	}
	...
	kvs, resumeSpan, _, err = unwrapMVCCScan(ctx, engine, testKey5, testKey6.Next(), 1,
...
func TestMVCCScanWithKeyPrefix(t *testing.T) {
	...
	kvs, _, _, err := unwrapMVCCScan(ctx, engine, roachpb.Key("/a"), roachpb.Key("/b"), math.MaxInt64,
...
func TestMVCCScanInTxn(t *testing.T) {
	...
	kvs, _, _, err := unwrapMVCCScan(ctx, engine, testKey2, testKey4, math.MaxInt64,
...
func TestMVCCScanInconsistent(t *testing.T) {
	...
	kvs, _, intents, err := unwrapMVCCScan(
...
func TestMVCCDeleteRange(t *testing.T) {
	...
	kvs, _, _, _ := unwrapMVCCScan(ctx, engine, keyMin, keyMax, math.MaxInt64,
		hlc.Timestamp{WallTime: 2}, MVCCScanOptions{})
	if len(kvs) != 4 ||
		!bytes.Equal(kvs[0].Key, testKey1) ||
	...
	kvs, _, _, _ = unwrapMVCCScan(ctx, engine, keyMin, keyMax, math.MaxInt64, hlc.Timestamp{WallTime: 2},
		MVCCScanOptions{})
	if len(kvs) != 4 ||
		!bytes.Equal(kvs[0].Key, testKey1) ||
		!bytes.Equal(kvs[1].Key, testKey4) ||
...
func TestMVCCUncommittedDeleteRangeVisible(t *testing.T) {
	...
	txn.Epoch++
	kvs, _, _, _ := unwrapMVCCScan(ctx, engine, testKey1, testKey4, math.MaxInt64,
...
func TestMVCCDeleteRangeInline(t *testing.T) {
	...
	kvs, _, _, err := unwrapMVCCScan(ctx, engine, keyMin, keyMax, math.MaxInt64, hlc.Timestamp{WallTime: 2},
		MVCCScanOptions{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestMVCCConditionalPutWithTxn(t *testing.T) {
	...
	if _, err := MVCCResolveWriteIntent(ctx, engine, nil, roachpb.LockUpdate{
...
func TestMVCCReverseScan(t *testing.T) {
	...
	kvs, resumeSpan, _, err = unwrapMVCCScan(ctx, engine, testKey2, testKey4, 1, hlc.Timestamp{WallTime: 1},
	...
	kvs, resumeSpan, _, err = unwrapMVCCScan(ctx, engine, testKey2, testKey4, 0, hlc.Timestamp{WallTime: 1},
		MVCCScanOptions{Reverse: true})
	...
	// latest.
	kvs, _, _, err = unwrapMVCCScan(ctx, engine, testKey2, testKey3, 1, hlc.Timestamp{WallTime: 4},
		MVCCScanOptions{Reverse: true})
	...
	kvs, _, _, err = unwrapMVCCScan(ctx, engine, testKey4, testKey6, 1, hlc.Timestamp{WallTime: 1},
		MVCCScanOptions{Reverse: true})
	...
	kvs, _, _, err = unwrapMVCCScan(ctx, engine, testKey1, testKey1.Next(), 1, hlc.Timestamp{WallTime: 1},
		MVCCScanOptions{Reverse: true})
...
func TestMVCCReverseScanFirstKeyInFuture(t *testing.T) {
	...
	kvs, _, _, err := unwrapMVCCScan(ctx, engine, testKey1, testKey4, math.MaxInt64,
...
func TestMVCCReverseScanSeeksOverRepeatedKeys(t *testing.T) {
	...
	kvs, _, _, err := unwrapMVCCScan(ctx, engine, testKey1, testKey3, math.MaxInt64,
		hlc.Timestamp{WallTime: 1}, MVCCScanOptions{Reverse: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestMVCCResolveWithDiffEpochs(t *testing.T) {
	...
	num, _, err := MVCCResolveWriteIntentRange(ctx, engine, nil, roachpb.LockUpdate{
...
func TestMVCCResolveTxnRange(t *testing.T) {
	...
	num, resumeSpan, err := MVCCResolveWriteIntentRange(ctx, engine, nil, roachpb.LockUpdate{
...
func TestMVCCIntentHistory(t *testing.T) {
	...
	kvs, _, _, err := unwrapMVCCScan(ctx, engine, key, key.Next(), math.MaxInt64, ts2, MVCCScanOptions{Txn: txn})
	if err != nil {
	...
	// but we're including tombstones in our search.
	kvs, _, _, err = unwrapMVCCScan(ctx, engine, key, key.Next(), math.MaxInt64, ts2, MVCCScanOptions{Txn: txn, Tombstones: true})
	if err != nil {
		t.Fatalf("MVCCScan failed with error: %v", err)
	}
