...
func TestFailedReplicaChange(t *testing.T) {
	...
	if _, err := repl.ChangeReplicas(
		...
		storagepb.ReasonRangeUnderReplicated,
		"",
		false); !testutils.IsError(err, "boom") {
...
func TestReplicateAfterRemoveAndSplit(t *testing.T) {
	...
	splitKey := roachpb.Key("m")
	splitArgs := adminSplitArgs(splitKey)
	if _, err := rep1.AdminSplit(context.Background(), *splitArgs, "test", false); err != nil {
		t.Fatal(err)
	}
...
func TestReportUnreachableHeartbeats(t *testing.T) {
	...
	cb := mtc.transport.GetCircuitBreaker(mtc.stores[followerIdx].Ident.NodeID, rpc.DefaultClass)
	cb.Break()
...
func TestStoreRangeRemoveDead(t *testing.T) {
	...
	t.Skip("https://github.com/znbasedb/znbase/issues/34081")
...
func TestReplicateReAddAfterDown(t *testing.T) {
	...
	// https://github.com/znbasedb/znbase/issues/2873.
...
func TestFailedPreemptiveSnapshot(t *testing.T) {
	...
	if _, err := rep.ChangeReplicas(
		...
		storagepb.ReasonRangeUnderReplicated,
		"",
		false); !testutils.IsError(err, expErr) {
...
// https://github.com/znbasedb/znbase/issues/13506 has been
// fixed.
