...
//
// See https://github.com/znbasedb/znbase/issues/12154.
...
func DecodeIndexPrefix(key roachpb.Key) ([]byte, uint32, uint32, error) {
	key, tableID, err := DecodeTablePrefix(key)
	if err != nil {
		return nil, 0, 0, err
	}
	if encoding.PeekType(key) != encoding.Int {
	...
	return key, uint32(tableID), uint32(indexID), err
}
...
// below is key for cursor feature

// 由于variable和cursor的key都以sessionID开头，所以中间加入常量用于在范围删除时候区分
const (
	CursorDescKey = iota
	VariableDescKey
)

// MakeCursorMetaKey make a meta key for cursor
func MakeCursorMetaKey(suffix, detail roachpb.RKey) roachpb.Key {
	key := make(roachpb.Key, 0, len(suffix)+len(detail))
	key = append(key, suffix...)
	key = append(key, CursorDescKey)
	key = append(key, detail...)
	return key
}

// MakeCursorDataKey make a data key for cursor
func MakeCursorDataKey(suffix, detail roachpb.RKey) roachpb.Key {
	key := make(roachpb.Key, 0, len(suffix)+len(detail))
	key = append(key, suffix...)
	key = append(key, detail...)
	return key
...
// MakeVariableMetaKey make a meta key for cursor
func MakeVariableMetaKey(suffix, detail roachpb.RKey) roachpb.Key {
	key := make(roachpb.Key, 0, len(suffix)+len(detail))
	key = append(key, suffix...)
	key = append(key, VariableDescKey)
	key = append(key, detail...)
	return key
}

// MakeVariableDataKey make a data key for cursor
func MakeVariableDataKey(k roachpb.RKey) roachpb.Key {
	key := make(roachpb.Key, 0, len(k))
	key = append(key, k...)
	return key
}
