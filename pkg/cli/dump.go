...
var dumpCmd = &cobra.Command{
	...
	Long: `
Dump SQL tables of a znbase database. If the table name
...
// in https://github.com/znbasedb/znbase/issues/28948.
func runDump(cmd *cobra.Command, args []string) error {
	conn, err := getPasswordAndMakeSQLClient("znbase dump")
	if err != nil {
		return err
	}
	defer conn.Close()
...
func getMetadataForTable(conn *sqlConn, md basicMetadata, ts string) (tableMetadata, error) {
	...
	rows, err := conn.Query(makeQuery("ZNBASE_SQL_TYPE")+` AND IS_HIDDEN = 'NO'`,
		[]driver.Value{md.name.Catalog(), md.name.Schema(), md.name.Table()})
	if err != nil {
		// IS_HIDDEN was introduced in the first 2.1 beta. ZNBASE_SQL_TYPE
		// some time after that.  To ensure `znbase dump` works across
		...
		if strings.Contains(err.Error(), "column \"znbase_sql_type\" does not exist") {
			// Pre-2.1 ZNBASE_SQL_HIDDEN did not exist in
...
func dumpSequenceData(w io.Writer, conn *sqlConn, clusterTS string, bmd basicMetadata) error {
	...
	// TODO(knz,vilterp): This could use a shortcut via zbdb_internal.
		...
		for vals := range valsCh {
			...
			for si, sv := range vals {
				...
				case time.Time:
					...
					case coltypes.Timestamp, coltypes.TimestampWithoutTZ:
						d = tree.MakeDTimestamp(t, time.Nanosecond)
					case coltypes.TimestampTZ, coltypes.TimestampWithTZ:
