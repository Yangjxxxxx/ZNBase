...
func (b *Builder) analyzeOrderBy(
	...
	//b.evalCtx.SessionData.IsWhereOrOrderBy = true
	for i := range orderBy {
...
func findIndexs(table cat.Table) []string {
	vecCols := make([]string, 0)
	for i, n := 0, table.IndexCount(); i < n; i++ {
		idx := table.Index(i)
		if idx.IsUnique() {
			s := bytes.Buffer{}
			for i, col := range idx.GetColumnNames() {
				if i == 0 {
					s.WriteString(col)
				} else {
					s.WriteString(";")
			...
			vecCols = append(vecCols, s.String())
		}
	}
	return vecCols
}
...
func (b *Builder) analyzeOrderByIndex(
	...
	tab, _, _ := b.resolveTableOrderBy(&order.Table, privilege.SELECT)
