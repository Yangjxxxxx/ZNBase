...
/**
 * Warning when multiple versions of ZNBaseDB are detected on the cluster.
...
/**
 * Notification when a new version of ZNBaseDB is available.
 */
export const newVersionNotificationSelector = createSelector(
  ...
  (newerVersions, newVersionDismissedPersistentLoaded, newVersionDismissedPersistent, newVersionDismissedLocal): Alert => {
    ...
    //   text: "A new version of ZNBaseDB is available.",
    //   link: docsURL.upgradeZNBaseVersion,
    //   dismiss: (dispatch) => {
...
export const disconnectedAlertSelector = createSelector(
  ...
  (health, disconnectedDismissed): Alert => {
    ...
    return {
      ...
      title: "我们目前在获取更新的数据时遇到了一些麻烦。 如果这种情况持续存在，则最好检查您与ZNBaseDB集群的网络连接.",
      dismiss: (dispatch) => {
...
export function alertDataSync(store: Store<AdminUIState>) {
  ...
  return () => {
    ...
    // Load potential new versions from ZNBaseDB cluster. This is the
