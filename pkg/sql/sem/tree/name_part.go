...
//ConstructUnresolvedName use for StartWith'recursive to construct UnresolvedName simply
func (u UnresolvedName) ConstructUnresolvedName(
	numParts int, star bool, tableName string,
) UnresolvedName {
	return UnresolvedName{NumParts: numParts, Star: star, Parts: NameParts{u.Parts[0], tableName}}
}
...
// NewUnresolvedSchemaName creates an unresolved object name, verifying that it
// is well-formed.
func NewUnresolvedSchemaName(numParts int, parts [3]string) (*UnresolvedObjectName, error) {
	u := &UnresolvedObjectName{
		NumParts: numParts,
		Parts:    parts,
	}
	if u.NumParts > 2 {
		return nil, newInvSchemaNameError(u)
	}
...
// ToSchemaName is transferred to schema name format
func (u *UnresolvedObjectName) ToSchemaName() SchemaName {
	return SchemaName{
		name:            Name(u.Parts[0]),
		catalog:         Name(u.Parts[1]),
