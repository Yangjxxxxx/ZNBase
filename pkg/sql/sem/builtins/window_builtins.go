...
var windows = map[string]builtinDefinition{
	...
	"ntile": makeBuiltin(winProps(),
		makeWindowOverload(tree.ArgTypes{{Name: "n", Typ: types.Int}}, types.Int, newNtileWindow,
	...
	"lag": collectOverloads(
		...
		func(t types.T) tree.Overload {
			return makeWindowOverload(tree.ArgTypes{{Name: "val", Typ: t}}, t,
		...
		func(t types.T) tree.Overload {
			return makeWindowOverload(tree.ArgTypes{{Name: "val", Typ: t}, {Name: "n", Typ: types.Int}}, t,
	...
	"nth_value": collectOverloads(winProps(), types.AnyNonArray,
		func(t types.T) tree.Overload {
			return makeWindowOverload(tree.ArgTypes{
				{Name: "val", Typ: t}, {Name: "n", Typ: types.Int}}, t, newNthValueWindow,
...
func makeWindowOverload(
	in tree.ArgTypes,
	ret types.T,
	f func([]types.T, *tree.EvalContext, *mon.BoundAccount) tree.WindowFunc,
	info string,
) tree.Overload {
...
func NewAggregateWindowFunc(
	aggConstructor func(*tree.EvalContext, tree.Datums, *mon.BoundAccount) tree.AggregateFunc,
) func(*tree.EvalContext) tree.WindowFunc {
	return func(evalCtx *tree.EvalContext) tree.WindowFunc {
		return &aggregateWindowFunc{agg: aggConstructor(evalCtx, nil /* arguments */, nil)}
	}
}
...
func (w *framableAggregateWindowFunc) Compute(
	...
	*w.agg = aggregateWindowFunc{w.aggConstructor(evalCtx, nil /* arguments */, nil), tree.DNull}
	...
	for i := frameStartIdx; i < frameEndIdx; i++ {
		...
		if wfr.Frame.IsIgnoreNulls && value != nil && value.String() == "NULL" {

		} else if err := w.agg.agg.Add(ctx, value, others...); err != nil {
			return nil, err
		}
	}
...
func newRowNumberWindow([]types.T, *tree.EvalContext, *mon.BoundAccount) tree.WindowFunc {
...
func newRankWindow([]types.T, *tree.EvalContext, *mon.BoundAccount) tree.WindowFunc {
	return &rankWindow{}
}
...
func newDenseRankWindow([]types.T, *tree.EvalContext, *mon.BoundAccount) tree.WindowFunc {
...
func newPercentRankWindow([]types.T, *tree.EvalContext, *mon.BoundAccount) tree.WindowFunc {
...
func newCumulativeDistWindow([]types.T, *tree.EvalContext, *mon.BoundAccount) tree.WindowFunc {
...
func newNtileWindow([]types.T, *tree.EvalContext, *mon.BoundAccount) tree.WindowFunc {
	return &ntileWindow{}
}
...
) func([]types.T, *tree.EvalContext, *mon.BoundAccount) tree.WindowFunc {
	return func([]types.T, *tree.EvalContext, *mon.BoundAccount) tree.WindowFunc {
...
func newNthValueWindow([]types.T, *tree.EvalContext, *mon.BoundAccount) tree.WindowFunc {
