...
func ClassifyConversion(
	...
	//Temp:Just to determine the ENUM type
	if !isEqualEnum(oldType, newType) {
	...
	fromPlaceholder, err := (&tree.Placeholder{Idx: 0}).TypeCheck(&ctx, oldType.ToDatumType(), false)
	if err != nil {
		return ColumnConversionImpossible, err
	}
	...
	if newColType, err := coltypes.DatumTypeToColumnType(newType.ToDatumType()); err == nil {
		if cast, err := tree.NewTypedCastExpr(fromPlaceholder, newColType); err == nil {
			if _, err := cast.TypeCheck(&ctx, nil, false); err == nil {
				return ColumnConversionGeneral, nil
			}
		}
	}
	...
	return ColumnConversionImpossible,
		pgerror.NewErrorf(pgcode.CannotCoerce, "cannot convert %s to %s", oldType.SQLString(), newType.SQLString())
}

func isEqualEnum(oldType, newType *sqlbase.ColumnType) bool {
	oldName := oldType.VisibleTypeName
	newName := newType.VisibleTypeName
	if oldName == "ENUM()" && newName != "ENUM()" {
		return false
	}
	if oldName != "ENUM()" && newName == "ENUM()" {
		return false
	}
	if oldName == "SET()" && newName != "SET()" {
		return false
	}
	if oldName != "SET()" && newName == "SET()" {
		return false
