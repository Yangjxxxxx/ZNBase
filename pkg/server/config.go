...
const (
	...
	// See https://github.com/znbasedb/znbase/issues/20310.
	DefaultMetricsSampleInterval   = 10 * time.Second
	DefaultHistogramWindowInterval = 6 * DefaultMetricsSampleInterval
	defaultStorePath               = "znbase-data"
	// TempDirPrefix is the filename prefix of any temporary subdirectory
	// created.
	TempDirPrefix       = "znbase-temp"
	CursorTempDirPrefix = "znbase-cursor-temp"
	...
	CursorTempDirsRecordFilename = "cursor-temp-dirs-record.txt"
...
type Config struct {
	...
	*base.Config
	...
	// Cursor
	CursorTempStorageConfig base.TempStorageConfig
	...
	// Environment Variable: ZNBASE_EXPERIMENTAL_LINEARIZABLE
	Linearizable bool
	...
	// Environment Variable: ZNBASE_SCAN_INTERVAL
	...
	// stores.
	// Environment Variable: ZNBASE_SCAN_MIN_IDLE_TIME
	ScanMinIdleTime time.Duration

	// ScanMaxIdleTime is the maximum time the scanner will be idle between ranges.
	// If enabled (> 0), the scanner may complete in less than ScanInterval for small
	// stores.
	// Environment Variable: ZNBASE_SCAN_MAX_IDLE_TIME
	...
	// LocationName is used for replace Locality
	...
	// EventLogEnabled is a switch which enables recording into znbase's SQL
...
func MakeConfig(ctx context.Context, st *cluster.Settings) Config {
	...
	disableWebLogin := envutil.EnvOrDefaultBool("ZNBASE_DISABLE_WEB_LOGIN", false)
	...
	cfg := Config{
		...
		CursorTempStorageConfig: base.TempStorageConfigFromEnv(
			ctx, st, storeSpec, "" /* parentDir */, base.DefaultTempStorageMaxSizeBytes, 0),
	}
...
func (cfg *Config) CreateEngines(ctx context.Context) (Engines, error) {
	...
	details = append(details, fmt.Sprintf("%s cache size: %s", cfg.StorageEngine.String(), humanizeutil.IBytes(cfg.CacheSize)))
	...
	for i, spec := range cfg.Stores.Specs {
		...
		if spec.InMemory {
			...
			engines = append(engines, engine.NewInMemByType(ctx, cfg.StorageEngine, spec, sizeInBytes))
		} else {
			if spec.Size.Percent > 0 {
			...
			details = append(details, fmt.Sprintf("store %d: %s, max size %s, max open file limit %d",
				i, cfg.StorageEngine.String(), humanizeutil.IBytes(sizeInBytes), openFileLimitPerStore))
			...
			eng, err := engine.NewEngine(cfg.StorageEngine, cfg.CacheSize, rocksDBConfig)
...
func (cfg *Config) readEnvironmentVariables() {
	cfg.Linearizable = envutil.EnvOrDefaultBool("ZNBASE_EXPERIMENTAL_LINEARIZABLE", cfg.Linearizable)
	cfg.ScanInterval = envutil.EnvOrDefaultDuration("ZNBASE_SCAN_INTERVAL", cfg.ScanInterval)
	cfg.ScanMinIdleTime = envutil.EnvOrDefaultDuration("ZNBASE_SCAN_MIN_IDLE_TIME", cfg.ScanMinIdleTime)
	cfg.ScanMaxIdleTime = envutil.EnvOrDefaultDuration("ZNBASE_SCAN_MAX_IDLE_TIME", cfg.ScanMaxIdleTime)
}
