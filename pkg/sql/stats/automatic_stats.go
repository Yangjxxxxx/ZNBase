...
// CheckStatisticsCreateTime collect stats
var CheckStatisticsCreateTime = settings.RegisterBoolSetting(
	"sql.status.check_stat_time.enabled",
	"check stats expired time",
	true,
)
...
func (r *Refresher) ensureAllTables(
	...
	getAllTablesQuery := fmt.Sprintf(
		`SELECT table_id FROM zbdb_internal.tables AS OF SYSTEM TIME '-%s' WHERE schema_name = 'public'`,
	...
	for _, row := range rows {
		...
		// identified differently from tables in zbdb_internal.tables.
...
func (r *Refresher) maybeRefreshStats(
	...
	if stat := mostRecentAutomaticStat(tableStats); stat != nil {
		...
		if timeutil.Now().After(maxTimeBetweenRefreshes) && CheckStatisticsCreateTime.Get(&r.st.SV) {
			mustRefresh = true
		}
