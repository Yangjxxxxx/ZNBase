...
func (c *CustomFuncs) EnsureKey(in memo.RelExpr) memo.RelExpr {
	...
	colID := c.f.Metadata().AddColumn("rownum", types.Int, nil)
...
func (c *CustomFuncs) EnsureCanaryCol(in memo.RelExpr, aggs memo.AggregationsExpr) opt.ColumnID {
	for i := range aggs {
		if !opt.AggregateIgnoresNulls(aggs[i].Agg.Op()) {
			...
			return c.f.Metadata().AddColumn("canary", types.Bool, nil)
		}
	}
	return 0
}
	...
	for i := range aggs {
		...
		default:
			if !opt.AggregateIgnoresNulls(t.Op()) {
				...
				newCol = md.AddColumn(colMeta.Alias, colMeta.Type, nil)
			}
		}
		if newAggs == nil {
...
func (r *subqueryHoister) constructGroupByExists(subquery memo.RelExpr) memo.RelExpr {
	trueColID := r.f.Metadata().AddColumn("true", types.Bool, nil)
	aggColID := r.f.Metadata().AddColumn("true_agg", types.Bool, nil)
	existsColID := r.f.Metadata().AddColumn("exists", types.Bool, nil)
...
func (r *subqueryHoister) constructGroupByAny(
	...
	if scalar.Op() != opt.VariableOp && !opt.IsConstValueOp(scalar) {
		typ := scalar.DataType()
		scalarColID := r.f.Metadata().AddColumn("scalar", typ, nil)
	...
	notNullColID := r.f.Metadata().AddColumn("notnull", types.Bool, nil)
	aggColID := r.f.Metadata().AddColumn("bool_or", types.Bool, nil)
	aggVar := r.f.ConstructVariable(aggColID)
	caseColID := r.f.Metadata().AddColumn("case", types.Bool, nil)
