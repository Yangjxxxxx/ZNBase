...
func QueryTableID(t testing.TB, sqlDB DBHandle, dbName, tableName string) uint32 {
	var tableIDQuery string
	if dbName != "system" {
		tableIDQuery = `
 SELECT tables.id FROM system.namespace tables
	 JOIN system.namespace sc ON (sc.id = tables."parentID")
   JOIN system.namespace dbs ON (dbs.id = sc."parentID")
   WHERE dbs.name = $1 AND tables.name = $2
 `
	} else {
		tableIDQuery = `
 SELECT sc.id FROM system.namespace tables
	 JOIN system.namespace sc ON (sc.id = tables."parentID")
   WHERE sc.name = $1 AND tables.name = $2
 `
	}
	var tableID uint32
