...
func MakeBulkAdder(
	...
	b := &BufferingAdder{
		sink:      SSTBatcher{db: db, maxSize: sstBytes, rc: rangeCache, disallowShadowing: disallowShadowing},
		timestamp: timestamp,
...
//GetFlushTs 获取最近一次推送数据的时间
func (b *BufferingAdder) GetFlushTs() hlc.Timestamp {
	return b.flushTs
}
...
func (b *BufferingAdder) Flush(ctx context.Context) error {
	...
	for i, kv := range b.curBuf {
		if b.skipDuplicates && i > 0 && bytes.Equal(b.curBuf[i-1].key, kv.key) {
			log.Errorf(ctx, "duplicate key: %v", kv.key)
			continue
		}
