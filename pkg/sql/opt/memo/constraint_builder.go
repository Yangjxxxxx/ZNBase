...
func (cb *constraintsBuilder) buildSingleColumnConstraint(
	...
	if opt.BoolOperatorRequiresNotNullArgs(op) {
		...
		vv, IsManualCast := IsManualCastVariable(val)
		if IsManualCast {
			res = res.Intersect(cb.evalCtx, cb.notNullSpan(vv.Col))
		}
...
//BuildConstraints is for build Constraints
...
func (cb *constraintsBuilder) buildConstraints(e opt.ScalarExpr) (_ *constraint.Set, tight bool) {
	...
	// if castExpr, ok1 := child0.(*CastExpr); ok1 {
	// 	if v2, ok2 := castExpr.Input.(*VariableExpr); ok2 {
	// 		return cb.buildSingleColumnConstraint(v2.Col, e.Op(), child1)
	// 	}
	// }
	OriginVar, IsManualCast := IsManualCastVariable(child0)
	if v, ok := child0.(*VariableExpr); ok || IsManualCast {
		if IsManualCast {
			v = OriginVar
		}
