...
func (fe *fieldExtract) interpretNumber(numbers []numberChunk, idx int, textMonth bool) error {
	...
	case !fe.Wants(fieldYear) && !fe.Wants(fieldMonth) && fe.Wants(fieldDay):
		...
		// the Separator between year and month must same as the Separator between month and day,
		// or the previous is empty char but the latter not empty.
		if fe.has.Has(fieldYear) && fe.has.Has(fieldMonth) && idx == int(fieldDay) {
			year, _ := fe.Get(fieldYear)
			month, _ := fe.Get(fieldMonth)
			if year == numbers[int(fieldYear)].v && month == numbers[int(fieldMonth)].v {
				monthSepara := numbers[int(fieldMonth)].separator
				daySepara := chunk.separator
				if !((monthSepara == daySepara) || (monthSepara == ' ' && (daySepara == '/' || daySepara == '-' || daySepara == '.'))) {
					return inputErrorf("invalid input syntax")
				}
			}
		}
		return fe.SetChunk(fieldDay, chunk)
