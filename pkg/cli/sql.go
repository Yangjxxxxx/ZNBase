...
const (
	infoMessage = `# Welcome to the znbase SQL interface.
...
var sqlShellCmd = &cobra.Command{
	...
	Long: `
Open a sql shell running against a znbase database.
`,
	Args: cobra.NoArgs,
...
func printCliHelp() {
	fmt.Printf(`You are using 'znbase sql', ZNBaseDB's lightweight SQL client.
Type:
...
More documentation about our SQL dialect and the CLI shell is available in:
%s`,
...
func (c *cliState) handleFunctionHelp(cmd []string, nextState, errState cliStateEnum) cliStateEnum {
	...
	if funcName == "" {
		...
		_, err := parser.Parse(fmt.Sprintf("select %s(??", funcName), false)
...
var cmdHistFile = envutil.EnvOrDefaultString("ZNBASE_SQL_CLI_HISTORY", ".znbasesql_history")
	...
	for {
		...
		case cliStart:
			...
			if cliCtx.isInteractive && cliCtx.terminalOutput {
				...
				c.ins, c.exitErr = readline.InitFiles("znbase",
					true, /* wideChars */
...
func checkInteractive() {
	...
	// serious hunch they are. For now, only `znbase sql` *without*
...
func runTerm(cmd *cobra.Command, args []string) error {
	...
	conn, err := getPasswordAndMakeSQLClient("znbase sql")
	if err != nil {
		return err
	}
	defer conn.Close()
...
func (c *cliState) serverSideParse(sql string) (stmts []string, pgErr *pgerror.Error) {
	...
	if err != nil {
		...
		if pgErr, ok := err.(*pgerror.Error); ok {
			...
			return nil, pgerror.NewError(
				pgcode.MakeCode(string(pqErr.Code)), pqErr.Message).SetHintf("%s", pqErr.Hint).SetDetailf("%s", pqErr.Detail)
		}
		return nil, pgerror.NewErrorf(pgcode.DataException,
	...
	if len(rows) >= 1 && rows[0][0] == "error" {
		...
		return nil, pgerror.NewError(pgcode.MakeCode(code), message).SetHintf("%s", hint).SetDetailf("%s", detail)
	}
