...
func (s *Service) PutStream(stream blobspb.Blob_PutStreamServer) error {
	...
	err := s.localStorage.Write(filename[0], reader)
	return err
}
...
// RecursionLogList traverses the wal directory recursively.
func (s *Service) RecursionLogList(
	ctx context.Context, req *blobspb.WalRequest,
) (*blobspb.WalResponse, error) {
	matches, err := s.localStorage.RecursionWalList(ctx, req.Pattern)
	return &blobspb.WalResponse{Files: matches}, err
}
