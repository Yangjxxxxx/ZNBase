...
func TestColumnConversions(t *testing.T) {
	...
	columnConversionInfo := map[testKey]map[testKey]ColumnConversionKind{
		...
		{SemanticType: sqlbase.ColumnType_FLOAT, Precision: 16}: {
			{SemanticType: sqlbase.ColumnType_FLOAT, Precision: 8}:  ColumnConversionTrivial,
			{SemanticType: sqlbase.ColumnType_FLOAT, Precision: 32}: ColumnConversionTrivial,
		},
		{SemanticType: sqlbase.ColumnType_FLOAT, Width: 4, Precision: 32}: {
			{SemanticType: sqlbase.ColumnType_FLOAT, Width: 2, Precision: 16}: ColumnConversionTrivial,
			{SemanticType: sqlbase.ColumnType_FLOAT, Width: 8, Precision: 64}: ColumnConversionTrivial,
		},
			...
			for to, expected := range mid {
				...
				if err != nil {
					if pgErr, ok := err.(*pgerror.Error); ok {
						if pgErr.Code != pgcode.CannotCoerce {
			...
			for to, kind := range mid {
				...
				t.Run(fmt.Sprint(from.SemanticType, "->", to.SemanticType), func(t *testing.T) {
					...
					case sqlbase.ColumnType_FLOAT:
						...
						if util.EnableAQKK && from.Precision >= 1 && from.Precision <= 20 {
							insert = insert32
						}
						switch to.SemanticType {
						case sqlbase.ColumnType_FLOAT:
							if util.EnableAQKK && to.Precision >= 1 && to.Precision <= 20 {
								expect = insert32
							} else {
								expect = insert
							}
					...
					case sqlbase.ColumnType_TIME,
						...
						const withZone = "2006-01-01 03:04:05 +0000"
