...
import (
	...
	"github.com/znbasedb/znbase/pkg/cmd/docgen/extract"
	...
	cmdSVG := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if railroadJar != "" {
				...
				if err != nil {
					if envutil.EnvOrDefaultBool("ZNBASE_REQUIRE_RAILROAD", false) {
...
// https://github.com/znbasedb/docs/wiki/SQL-Grammar-Railroad-Diagram-Changes#structure
...
var specs = []stmtSpec{
	...
	{name: "create_database_stmt", inline: []string{"encoding_clause"}, replace: map[string]string{"'SCONST'": "encoding"}, unlink: []string{"name", "encoding"}},
	{
	...
	},
	{
		...
		inline: []string{"opt_with", "user_options"},
		replace: map[string]string{
	...
	},
	{
		...
		replace: map[string]string{"'TRANSACTION'": "", "'TO'": "'TO' 'SAVEPOINT'", "savepoint_name": "znbase_restart"},
		unlink:  []string{"znbase_restart"},
	},
	{
		name:   "limit_clause",
