...
// Gossip .settings .
var (
	GossipMaxhops = settings.RegisterIntSetting(
		"gossip.maxhops",
		"the maxhops which gossip's distance can reach",
		5)
	CurrentGossipMaxhops = (int)(GossipMaxhops.Default())
)
...
func maxPeers(nodeCount int) int {
	...
	maxPeers := int(math.Ceil(math.Exp(math.Log(float64(nodeCount)) / float64(CurrentGossipMaxhops-2))))
...
func (g *Gossip) RecomputeMaxPeersLocked() {
	...
	ctx := g.AnnotateCtx(context.TODO())
	log.Infof(ctx, "MaxPeers Recomputed as %d", maxPeers)
...
func (g *Gossip) TightenNetwork(ctx context.Context) {
	...
	if g.outgoing.hasSpace() {
		...
		if distantHops <= (uint32)(CurrentGossipMaxhops) {
			return
		}
		if nodeAddr, err := g.getNodeIDAddressLocked(distantNodeID); err != nil {
			...
			log.Infof(ctx, "starting client to n%d (%d > %d) to tighten network graph",
				distantNodeID, distantHops, CurrentGossipMaxhops)
