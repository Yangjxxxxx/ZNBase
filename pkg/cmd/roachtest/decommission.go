...
func runDecommission(t *test, c *cluster, nodes int, duration time.Duration) {
	...
	waitReplicatedAwayFrom := func(downNodeID string) error {
		...
		for {
			...
			if err := db.QueryRow(
				...
				"SELECT count(*) FROM zbdb_internal.ranges WHERE array_position(replicas, $1) IS NOT NULL",
				downNodeID,
			).Scan(&count); err != nil {
				return err
			}
	...
	waitUpReplicated := func(targetNodeID string) error {
		...
		for ok := false; !ok; {
			stmtReplicaCount := fmt.Sprintf(
				`SELECT count(*) = 0 FROM zbdb_internal.ranges WHERE array_position(replicas, %s) IS NULL and database_name = 'kv';`, targetNodeID)
	...
	m.Go(func() error {
		nodeID := func(node int) (string, error) {
			...
			defer dbNode.Close()
			var nodeID string
			if err := dbNode.QueryRow(`SELECT node_id FROM zbdb_internal.node_runtime_info LIMIT 1`).Scan(&nodeID); err != nil {
				return "", nil
			}
			return nodeID, nil
		}
		...
		stop := func(node int) error {
			...
			return c.RunE(ctx, c.Node(node), "./znbase quit --insecure --host=:"+port)
		}

		decom := func(id string) error {
			port := fmt.Sprintf("{pgport:%d}", nodes) // always use last node
			t.Status("decommissioning node", id)
			return c.RunE(ctx, c.Node(nodes), "./znbase node decommission --insecure --wait=live --host=:"+port+" "+id)
		}
...
func runDecommissionAcceptance(ctx context.Context, t *test, c *cluster) {
	...
	if err := retry.ForDuration(time.Minute, func() error {
		...
		// unavailable
		//
		// Seen in https://teamcity.znbasedb.com/viewLog.html?buildId=344802.
		db := c.Conn(ctx, 2)
		defer db.Close()
