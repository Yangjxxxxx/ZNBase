...
func (s *CrossRangeTxnWrapperSender) Send(
	...
	err := s.db.Txn(ctx, func(ctx context.Context, txn *Txn) error {
		err := txn.SetDebugName("auto-wrap")
		if err != nil {
			return err
		}
		b := txn.NewBatch()
...
// DB is a database handle to a single znbase cluster. A DB is safe for
...
// ScatterForSplit is a helper that wraps AdminSplit + AdminScatter.
func (db *DB) ScatterForSplit(ctx context.Context, key roachpb.Key) error {
	if err := db.AdminSplit(ctx, key, key); err != nil {
		return err
	}
...
// TxnWithSteppingEnabled same as Txn but with stepping open
func (db *DB) TxnWithSteppingEnabled(
	ctx context.Context, retryable func(context.Context, *Txn) error,
) error {
	// TODO(radu): we should open a tracing Span here (we need to figure out how
	// to use the correct tracer).

	txn := NewTxnWithSteppingEnabled(ctx, db, db.ctx.NodeID.Get(), RootTxn)
	err := txn.SetDebugName("unnamed")
	if err != nil {
		return err
	}
