...
// CaseSensitiveG is cluster switch for casesensitive
var CaseSensitiveG bool
...
func classifyTablePattern(n *UnresolvedName) (TablePattern, error) {
	...
	// We allow this in e.g. `select * from "".zbdb_internal.tables`.
...
func classifyColumnItem(n *UnresolvedName) (VarName, error) {
	...
	// `select "".zbdb_internal.tables.table_id from "".zbdb_internal.tables`.
	lastCheck := n.NumParts
	if lastCheck > 3 {
		lastCheck = 3
	}
...
type ColumnItemResolver interface {
	...
	// modified.
	FindSourceProvidingColumn(ctx context.Context, col Name, original string) (prefix *TableName, srcMeta ColumnSourceMeta, colHint int, err error)
...
func (a *AllColumnsSelector) Resolve(
	...
	if res == NoResults && a.TableName.NumParts == 2 {
		...
		// Special rule for compatibility with ZNBaseDB v1.x:
...
type TableNameExistingResolver interface {
	LookupObject(ctx context.Context, requireMutation bool, requireFunction bool, dbName, scName, obName string) (
...
func (t *TableName) ResolveExisting(
	...
	if t.ExplicitSchema {
		...
		if t.SchemaName == sessiondata.PgTempSchemaName {
			t.SchemaName = Name(scName)
		}
		if t.ExplicitCatalog {
			// Already 3 parts: nothing to search. Delegate to the resolver.
			found, objMeta, err := r.LookupObject(ctx, requireMutable, t.ResolvedAsFunction, t.Catalog(), scName, t.Table())
			if found && err == nil {
				err = searchPath.CheckTemporarySchema(t.Schema())
				if err != nil {
					return false, nil, err
				}
			}
		...
		//
		// Note: we test this even if curDb == "", because ZNBaseDB
		...
		if found, objMeta, err := r.LookupObject(ctx, requireMutable, t.ResolvedAsFunction, curDb, scName, t.Table()); found || err != nil {
			if err == nil {
				err = searchPath.CheckTemporarySchema(t.Schema())
				if err != nil {
					return false, nil, err
		...
		// No luck so far. Compatibility with ZNBaseDB v1.1: try D.public.T instead.
		if found, objMeta, err := r.LookupObject(ctx, requireMutable, t.ResolvedAsFunction, t.Schema(), PublicSchema, t.Table()); found || err != nil {
			if err == nil {
				err = searchPath.CheckTemporarySchema(t.Schema())
				if err != nil {
					return false, nil, err
	...
	for next, ok := iter.Next(); ok; next, ok = iter.Next() {
		if found, objMeta, err := r.LookupObject(ctx, requireMutable, t.ResolvedAsFunction, curDb, next, t.Table()); found || err != nil {
			if err == nil {
				t.CatalogName = Name(curDb)
...
func (t *TableName) ResolveTarget(
	...
	if t.ExplicitSchema {
		...
		if t.ExplicitCatalog {
			...
			found, scMeta, err := r.LookupSchema(ctx, t.Catalog(), scName)
			// Verify if session has access to temporary schema
			if found && err == nil {
				err = searchPath.CheckTemporarySchema(t.Schema())
				if err != nil {
					return false, nil, err
		...
		// No luck so far. Compatibility with ZNBaseDB v1.1: use D.public.T instead.
...
func (tp *TableNamePrefix) Resolve(
	...
	if tp.ExplicitSchema {
		...
		if tp.SchemaName == sessiondata.PgTempSchemaName {
			tp.SchemaName = Name(scName)
		}
		if tp.ExplicitCatalog {
			// Catalog name is explicit; nothing to do.
			found, scMeta, err := r.LookupSchema(ctx, tp.Catalog(), scName)
			// Verify if session has access to temporary schema
			if found && err == nil {
				err = searchPath.CheckTemporarySchema(tp.Schema())
				if err != nil {
					return false, nil, err
				}
			}
...
// Resolve is used for SHOW SEQUENCES.
//func (n *ShowSequences) Resolve(
//	ctx context.Context,
...
//	iter := searchPath.IterWithoutImplicitPGCatalog()
//	for scName, ok := iter.Next(); ok; scName, ok = iter.Next() {
//		if found, _, err = r.LookupSchema(ctx, curDatabase, scName); found || err != nil {
//			if err == nil {
//				schema = scName
//			}
//			break
...
func (n *UnresolvedName) ResolveFunction(
	searchPath sessiondata.SearchPath, casesensitive bool,
	...
	if n.NumParts > 3 || len(n.Parts[0]) == 0 || n.Star {
		...
		return nil, pgerror.NewErrorf(pgcode.InvalidName,
	...
	// independently of the database/catalog prefix.contains unknown column
	if casesensitive || CaseSensitiveG {
		n.Parts[0] = strings.ToLower(n.Parts[0])
	}
	...
	if !ok {
		...
		if !found {
			...
			// At lease reoslve function name
			ResolveNameDef := &FunctionDefinition{
				Name:       function,
				Definition: binOpOverload{},
			}
			return ResolveNameDef, pgerror.NewErrorf(
...
// UDRFunctionResolver offer a method to resolve UDR function with function name
type UDRFunctionResolver interface {
	// ResolveUDRFunction will searches for UDR Functions with given name, and return a FunctionDefinition
	ResolveUDRFunction(ctx context.Context, function *FunctionDefinition) (*FunctionDefinition, error)
}

func newInvColRef(fmt string, n *UnresolvedName) error {
	return pgerror.NewErrorWithDepthf(1, pgcode.InvalidColumnReference, fmt, n)
}

func newInvTableNameError(n fmt.Stringer) error {
	return pgerror.NewErrorWithDepthf(1, pgcode.InvalidName,
		"invalid table name: %s", n)
}

func newInvSchemaNameError(n fmt.Stringer) error {
	return pgerror.NewErrorWithDepthf(1, pgcode.InvalidName,
		"invalid schema name: %s", n)
}

func newSourceNotFoundError(fmt string, args ...interface{}) error {
	return pgerror.NewErrorWithDepthf(1, pgcode.UndefinedTable, fmt, args...)
}
