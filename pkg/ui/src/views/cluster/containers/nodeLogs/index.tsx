...
class Logs extends React.Component<LogProps & RouterState, {}> {
  componentWillMount() {
    ...
    this.props.refreshLogs(new protos.znbase.server.serverpb.LogsRequest({ node_id: this.props.params[nodeIDAttr] }));
  }
  ...
  renderContent = () => {
    ...
    const columns = [
      ...
      {
        title: "等级",
        cell: (index: number) => protos.znbase.util.log.Severity[logEntries[index].severity],
      },
      {
        title: "日志",
        cell: (index: number) => (
  ...
  render() {
    ...
    const title = this.props.currentNode
      ? `日志 | ${getDisplayName(this.props.currentNode)} | 节点`
      : `日志 | 节点 ${this.props.params[nodeIDAttr]} | 节点`;
      ...
      return (
        <div>
          ...
          <div className="section section--heading">
            <h2>节点{ this.props.params[nodeIDAttr] } / { nodeAddress }的日志</h2>
          </div>
