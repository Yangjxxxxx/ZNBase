...
func (tc *txnCommitter) SendLocked(
	...
	// to the EndTransaction request to the LockSpans and clear the in-flight
	...
	if len(et.InFlightWrites) > 0 && !tc.canCommitInParallelWithWrites(ba, et) {
		et.LockSpans = mergeIntoSpans(et.LockSpans, et.InFlightWrites)
	...
	if pErr != nil {
		...
		if br != nil {
			if len(br.Responses) > 0 {
				if br.Responses[len(br.Responses)-1].GetEndTransaction() != nil {
					br.Responses = br.Responses[:len(br.Responses)-1]
				}
			}
		}
		return br, pErr
	...
	mergedLockSpans := mergeIntoSpans(et.LockSpans, et.InFlightWrites)
	tc.makeTxnCommitExplicitAsync(ctx, br.Txn, mergedLockSpans, ba.CanForwardRead, ba.BackFill)
...
func (tc *txnCommitter) sendLockedWithElidedEndTransaction(
	...
	if deadline != nil && !br.Txn.WriteTimestamp.Less(*deadline) && br.Txn.IsolationLevel != util.ReadCommittedIsolation {
...
func (tc *txnCommitter) makeTxnCommitExplicitAsync(
	...
	if err := tc.stopper.RunAsyncTask(
		context.Background(), "txnCommitter: making txn commit explicit", func(ctx context.Context) {
			tc.mu.Lock()
			defer tc.mu.Unlock()
			if err := makeTxnCommitExplicitLocked(ctx, tc.wrapped, txn, lockSpans, noRefreshSpans, backFill); err != nil {
...
func makeTxnCommitExplicitLocked(
	...
	ba.Header = roachpb.Header{Txn: txn, CanForwardRead: noRefreshSpans, BackFill: backFill}
