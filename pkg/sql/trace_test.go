	...
	testData := []struct {
	...
	}{
		{
			...
			getRows: func(t *testing.T, sqlDB *gosql.DB) (*gosql.Rows, error) {
				...
				return sqlDB.Query(
					"SELECT DISTINCT operation AS op FROM zbdb_internal.session_trace " +
		...
		},
		{
			...
			getRows: func(t *testing.T, sqlDB *gosql.DB) (*gosql.Rows, error) {
				...
				rows, err := sqlDB.Query(
					"SELECT count(message) FROM zbdb_internal.session_trace " +
						"WHERE message LIKE '%znbase.stat.tablereader.input.rows: 0%'",
				)
				if err != nil {
					t.Fatal(err)
				}
			...
			// spans.
			optionalSpans: []string{
				"/znbase.sql.distsqlrun.DistSQL/SetupFlow",
				"noop",
			},
		},
		{
				...
				t.Run(name, func(t *testing.T) {
					for i := 0; i < numNodes; i++ {
						t.Run(fmt.Sprintf("node-%d", i), func(t *testing.T) {
							...
							row := sqlDB.QueryRow("SELECT count(1) FROM zbdb_internal.session_trace")
							var count int
							if err := row.Scan(&count); err != nil {
								t.Fatal(err)
							}
							if count != 0 {
								t.Fatalf("expected zbdb_internal.session_trace to be empty "+
