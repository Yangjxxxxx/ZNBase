...
// RaftLogQueueConcurrency is the setting of queue concurrency.
// Allow a limited number of Raft log truncations to be processed
// concurrently.
var RaftLogQueueConcurrency = settings.RegisterPositiveIntSetting(
	"kv.raft_log.queue_concurrency",
	"raftLogQueue manages a queue of replicas slated to have their raft logs "+
		"truncated by removing unneeded entries. This cluster setting is used to adjust "+
...
func newRaftLogQueue(store *Store, db *client.DB, gossip *gossip.Gossip) *raftLogQueue {
	...
	rlq.baseQueue = newBaseQueue(
		...
		queueConfig{
			...
			maxConcurrency:       RaftLogQueueConcurrency.Get(&store.ClusterSettings().SV),
			needsLease:           false,
	...
	confChanged := func() {
		rlq.maxConcurrency = RaftLogQueueConcurrency.Get(&store.ClusterSettings().SV)
		rlq.confCh <- struct{}{}
	}
	RaftLogQueueConcurrency.SetOnChange(&store.ClusterSettings().SV, confChanged)
	return rlq
}
