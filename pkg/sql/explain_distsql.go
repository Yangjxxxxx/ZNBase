...
func (n *explainDistSQLNode) startExec(params runParams) error {
	...
	if n.analyze && len(n.subqueryPlans) > 0 {
		...
		if n.optimizeSubqueries {
			...
			for i := range n.subqueryPlans {
				...
				params.p.setUnlimited(n.subqueryPlans[i].plan.planNode)
			}
		}
	...
	for _, proc := range plan.Processors {
		...
		if !ok {
			flowSpec = distsqlplan.NewFlowSpec(flowID, params.extendedEvalCtx.NodeID)
			flows[proc.Node] = flowSpec
			//log.Infof(planCtx.ctx, "plan.flowSpec %v %v", proc.Node,  len(flowSpec.Processors))
		}
		flowSpec.Processors = append(flowSpec.Processors, proc.Spec)
	}

	//flows := plan.GenerateFlowSpecs(params.extendedEvalCtx.NodeID)
	//log.Info(planCtx.ctx, "*******get pool flows %v*********", params.extendedEvalCtx.NodeID)
...
func (n *explainDistSQLNode) Close(ctx context.Context) {
	...
	for i := range n.subqueryPlans {
		...
		// plan.
		if n.subqueryPlans[i].plan.planNode != nil {
			n.subqueryPlans[i].plan.planNode.Close(ctx)
			n.subqueryPlans[i].plan.planNode = nil
		}
	}
}
