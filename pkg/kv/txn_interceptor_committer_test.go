...
func TestTxnCommitterElideEndTransaction(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "commit", func(t *testing.T, commit bool) {
		...
		ba.Add(&roachpb.EndTransactionRequest{Commit: commit, LockSpans: nil})
...
func TestTxnCommitterAttachesTxnKey(t *testing.T) {
	...
	// Attach LockSpans to each EndTransaction request to avoid the elided
	...
	ba.Header = roachpb.Header{Txn: &txn}
	ba.Add(&roachpb.PutRequest{RequestHeader: roachpb.RequestHeader{Key: keyA}})
	ba.Add(&roachpb.EndTransactionRequest{Commit: true, LockSpans: intents})
	...
	ba.Requests = nil
	ba.Add(&roachpb.EndTransactionRequest{Commit: false, LockSpans: intents})
