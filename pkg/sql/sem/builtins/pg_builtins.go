...
func initPGBuiltins() {
	...
	// Make zbdb_internal.create_regfoo builtins.
	for _, typ := range []types.TOid{types.RegType, types.RegProc, types.RegProcedure, types.RegClass, types.RegNamespace} {
		typName := typ.SQLName()
		builtins["zbdb_internal.create_"+typName] = makeCreateRegDef(typ)
	}
}

var errUnimplemented = pgerror.NewError(pgcode.FeatureNotSupported, "unimplemented")
...
func makeTypeIOBuiltins(builtinPrefix string, typ types.T) map[string]builtinDefinition {
	...
	return map[string]builtinDefinition{
		builtinPrefix + "send": makeTypeIOBuiltin(tree.ArgTypes{{Name: typname, Typ: typ}}, types.Bytes),
		...
		// matter.
		builtinPrefix + "recv": makeTypeIOBuiltin(tree.ArgTypes{{Name: "input", Typ: types.Any}}, typ),
		// Note: PG returns 'cstring' for these builtins, but we don't support that.
		builtinPrefix + "out": makeTypeIOBuiltin(tree.ArgTypes{{Name: typname, Typ: typ}}, types.Bytes),
		// Note: PG takes 'cstring' for these builtins, but we don't support that.
		builtinPrefix + "in": makeTypeIOBuiltin(tree.ArgTypes{{Name: "input", Typ: types.Any}}, typ),
	}
}
	...
	return tree.Overload{
		...
		Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
			...
			r, err := ctx.InternalExecutor.QueryRow(
				...
				ctx.Txn,
				"SELECT indexdef FROM pg_catalog.pg_indexes WHERE znbase_oid = $1", args[0])
			if err != nil {
				return nil, err
			}
...
// pg_class.relowner & pg_views.viewowner both are null
	...
	return tree.Overload{
		...
		Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
			r, err := ctx.InternalExecutor.QueryRow(
				...
				ctx.Txn,
				"SELECT definition FROM pg_catalog.pg_views v JOIN pg_catalog.pg_class c ON c.relname=v.viewname "+
					"JOIN pg_catalog.pg_namespace n ON c.relnamespace = n.oid and v.schemaname = n.nspname WHERE c.oid=$1 limit 1", args[0])
			if err != nil {
				return nil, err
			}
			if len(r) == 0 {
...
func makePGGetConstraintDef(argTypes tree.ArgTypes) tree.Overload {
	return tree.Overload{
		...
		Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
			...
			if len(r) == 0 {
				return nil, pgerror.NewErrorf(pgcode.InvalidParameterValue, "unknown constraint (OID=%s)", args[0])
			}
			return r[0], nil
		},
		Info: notUsableInfo,
...
func makePGPrivilegeInquiryDef(
	...
	for _, typ := range strOrOidTypes {
		argTypes = append(argTypes, tree.ArgTypes{{Name: "user", Typ: typ}})
	}
	...
	for _, objSpecArg := range objSpecArgs {
		...
		for _, argType := range prevArgTypes {
			for _, typ := range objSpecArg.Typ {
				argTypeVariant := append(argType, tree.ArgTypes{{Name: objSpecArg.Name, Typ: typ}}...)
	...
	for i, argType := range argTypes {
		argTypes[i] = append(argType, tree.ArgTypes{{Name: "privilege", Typ: types.String}}...)
	}
...
func evalPrivilegeCheck(
	...
	for _, p := range privChecks {
		query := fmt.Sprintf(`
			SELECT bool_or(privilege_type='%s') IS TRUE, bool_or(privilege_type='%s' and is_grantable='YES') IS TRUE
		...
		case tree.DBoolTrue:
			if withGrantOpt {
				...
				case tree.DBoolTrue:
				default:
					panic(fmt.Sprintf("unexpected privilege check result %v", r[1]))
				}
			}
			continue
		default:
...
var pgBuiltins = map[string]builtinDefinition{
	...
	// of an expression", which is provided in the pg_node_tree type. In ZNBase's
	...
	"pg_get_constraintdef": makeBuiltin(tree.FunctionProperties{DistsqlBlacklist: true},
		...
		makePGGetConstraintDef(tree.ArgTypes{{Name: "constraint_oid", Typ: types.Oid}}),
	),
	...
	"pg_get_indexdef": makeBuiltin(tree.FunctionProperties{DistsqlBlacklist: true},
		makePGGetIndexDef(tree.ArgTypes{{Name: "index_oid", Typ: types.Oid}}),
		makePGGetIndexDef(tree.ArgTypes{{Name: "index_oid", Typ: types.Oid}, {Name: "column_no", Typ: types.Int}, {Name: "pretty_bool", Typ: types.Bool}}),
	),
	...
	"pg_get_viewdef": makeBuiltin(tree.FunctionProperties{DistsqlBlacklist: true},
		makePGGetViewDef(tree.ArgTypes{{Name: "view_oid", Typ: types.Oid}}),
		makePGGetViewDef(tree.ArgTypes{{Name: "view_oid", Typ: types.Oid}, {Name: "pretty_bool", Typ: types.Bool}}),
	),
	...
	"pg_sequence_parameters": makeBuiltin(tree.FunctionProperties{DistsqlBlacklist: true},
		...
		tree.Overload{
			Types:      tree.ArgTypes{{Name: "sequence_oid", Typ: types.Oid}},
			ReturnType: tree.FixedReturnType(types.String),
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				r, err := ctx.InternalExecutor.QueryRow(
				...
				if len(r) == 0 {
					return nil, pgerror.NewErrorf(pgcode.UndefinedTable, "unknown sequence (OID=%s)", args[0])
				}
	...
	"format_type": makeBuiltin(tree.FunctionProperties{NullableArgs: true},
		tree.Overload{
			Types:      tree.ArgTypes{{Name: "type_oid", Typ: types.Oid}, {Name: "typemod", Typ: types.Int}},
			ReturnType: tree.FixedReturnType(types.String),
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				oidArg := args[0]
	...
	"col_description": makeBuiltin(defProps(),
		tree.Overload{
			Types:      tree.ArgTypes{{Name: "table_oid", Typ: types.Oid}, {Name: "column_number", Typ: types.Int}},
			ReturnType: tree.FixedReturnType(types.String),
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
	...
	"obj_description": makeBuiltin(defProps(),
		tree.Overload{
			Types:      tree.ArgTypes{{Name: "object_oid", Typ: types.Oid}},
			ReturnType: tree.FixedReturnType(types.String),
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
		...
		tree.Overload{
			Types:      tree.ArgTypes{{Name: "object_oid", Typ: types.Oid}, {Name: "catalog_name", Typ: types.String}},
			ReturnType: tree.FixedReturnType(types.String),
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
	...
	"oid": makeBuiltin(defProps(),
		tree.Overload{
			Types:      tree.ArgTypes{{Name: "int", Typ: types.Int}},
	...
	// ZNBaseDB supports just UTF8 for now.
	...
	"pg_table_is_visible": makeBuiltin(defProps(),
		tree.Overload{
			Types:      tree.ArgTypes{{Name: "oid", Typ: types.Oid}},
			ReturnType: tree.FixedReturnType(types.Bool),
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				oid := args[0]
				t, err := ctx.InternalExecutor.QueryRow(
	...
	"has_column_privilege": makePGPrivilegeInquiryDef(
		...
		func(ctx *tree.EvalContext, args tree.Datums, user string) (tree.Datum, error) {
			...
			return parsePrivilegeStr(args[2], pgPrivList{
				...
				// todo column references use select
				"REFERENCES": func(withGrantOpt bool) (tree.Datum, error) {
					if retNull {
						return tree.DNull, nil
					}
	...
	"has_database_privilege": makePGPrivilegeInquiryDef(
		...
		func(ctx *tree.EvalContext, args tree.Datums, user string) (tree.Datum, error) {
			...
			if db == "" {
				switch dbArg.(type) {
				case *tree.DString:
					return nil, pgerror.NewErrorf(pgcode.InvalidCatalogName,
			...
			return parsePrivilegeStr(args[1], pgPrivList{
				"CREATE": func(withGrantOpt bool) (tree.Datum, error) {
					...
					return evalPrivilegeCheck(ctx, "database_privileges",
	...
	"has_sequence_privilege": makePGPrivilegeInquiryDef(
		...
		func(ctx *tree.EvalContext, args tree.Datums, user string) (tree.Datum, error) {
			...
			return parsePrivilegeStr(args[1], pgPrivList{
				...
				"DROP": func(withGrantOpt bool) (tree.Datum, error) {
					if retNull {
						return tree.DNull, nil
					}
					return evalPrivilegeCheck(ctx, "table_privileges",
						user, pred, privilege.DROP, withGrantOpt)
				},
			})
		},
	),
		...
		func(ctx *tree.EvalContext, args tree.Datums, user string) (tree.Datum, error) {
			...
			return parsePrivilegeStr(args[1], pgPrivList{
				...
				"REFERENCES": func(withGrantOpt bool) (tree.Datum, error) {
					...
					return evalPrivilegeCheck(ctx, "table_privileges",
						user, pred, privilege.REFERENCES, withGrantOpt)
				},
	...
	"current_setting": makeBuiltin(
		...
		tree.Overload{
			Types:      tree.ArgTypes{{Name: "setting_name", Typ: types.String}},
			ReturnType: tree.FixedReturnType(types.String),
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
		...
		tree.Overload{
			Types:      tree.ArgTypes{{Name: "setting_name", Typ: types.String}, {Name: "missing_ok", Typ: types.Bool}},
			ReturnType: tree.FixedReturnType(types.String),
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
	...
	"set_config": makeBuiltin(
		...
		tree.Overload{
			Types:      tree.ArgTypes{{Name: "setting_name", Typ: types.String}, {Name: "new_value", Typ: types.String}, {Name: "is_local", Typ: types.Bool}},
			ReturnType: tree.FixedReturnType(types.String),
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
