...
func (ts *txnState) resetForNewSQLTxn(
	...
	ts.mu.Lock()
	if txn == nil {
		ts.mu.txn = client.NewTxnWithSteppingEnabled(ts.Ctx, tranCtx.db, tranCtx.nodeID, client.RootTxn)
		if debugName != "" {
			_ = ts.mu.txn.SetDebugName(debugName)
		} else {
			_ = ts.mu.txn.SetDebugName(opName)
		}
	} else {
		ts.mu.txn = txn
	}
	...
	ts.setIsolationLevel(isolationLevel)
	...
	ts.retry = util.MaxTxnRefreshAttempts.Get(&ts.settings.SV)
}
...
func (ts *txnState) setIsolationLevel(isolationLevel tree.IsolationLevel) {
	if isolationLevel == tree.UnspecifiedIsolation {
		return
	}
	ts.mu.Lock()
	ts.mu.txn.SetIsolationLevel(util.IsolationLevel(isolationLevel))
	ts.mu.Unlock()
	ts.isolationLevel = isolationLevel
}
...
func (ts *txnState) setDebugName(name tree.Name) error {
	...
	ts.mu.Lock()
	err := ts.mu.txn.SetDebugName(string(name))
	ts.mu.Unlock()
	if err != nil {
		return err
	}
...
// 设置事务是否支持DDL
func (ts *txnState) setTxnDDL(ddl tree.SUPDDL) error {
	ts.mu.Lock()
	switch ddl {
	case tree.SupportDDL:
		ts.mu.txn.SetTxnDDL(true)
	case tree.NotSupportDDL:
		ts.mu.txn.SetTxnDDL(false)
	default:
		return errors.Errorf("unknown support ddl: %s", ddl)
	}
	ts.mu.Unlock()
	return nil
}
