...
// NewOneInputNode returns an distsqlrun.OpNode with a single Operator input.
func NewOneInputNode(input Operator) OneInputNode {
	return OneInputNode{input: input}
}

// OneInputNode is an distsqlrun.OpNode with a single Operator input.
...
func (n OneInputNode) Child(nth int, verbose bool) runbase.OpNode {
	if nth == 0 {
		return n.input
	}
...
// ZeroInputNode is an distsqlrun.OpNode with no inputs.
...
// Child implements the distsqlrun.OpNode interface.
func (ZeroInputNode) Child(nth int, verbose bool) runbase.OpNode {
	execerror.VectorizedInternalPanic(fmt.Sprintf("invalid index %d", nth))
	// This code is unreachable, but the compiler cannot infer that.
	return nil
}

// newTwoInputNode returns an distsqlrun.OpNode with two Operator inputs.
...
func (n *twoInputNode) Child(nth int, verbose bool) runbase.OpNode {
	switch nth {
	case 0:
		return n.inputOne
	case 1:
