...
// - Attaching lock tent spans to EndTransaction requests, for intent cleanup.
		...
		if !asyncRequest {
			if gs.requestInFlight {
				return nil, roachpb.NewError(
					errors.Errorf("concurrent txn use detected. ba: %s", ba))
			}
...
func (tcf *TxnCoordSenderFactory) TransactionalSender(
	...
	if typ == client.LeafTxn {
		meta.Txn.WriteTooOld = false
	}
...
func (tc *TxnCoordSender) commitReadOnlyTxnLocked(
	...
	if deadline != nil && !tc.mu.txn.WriteTimestamp.Less(*deadline) && tc.mu.txn.IsolationLevel != util.ReadCommittedIsolation {
		txn := tc.mu.txn.Clone()
...
func (tc *TxnCoordSender) Send(
	...
	if ba.IsSingleEndTransactionRequest() && !tc.interceptorAlloc.txnPipeliner.hasAcquiredLocks() {
...
func (tc *TxnCoordSender) handleRetryableErrLocked(
	...
	if rcRetry && tc.mu.txn.IsolationLevel == util.ReadCommittedIsolation {
		return roachpb.NewTransactionRCRetryWithProtoRefreshError(
			pErr.Message,
			errTxnID, // the id of the transaction that encountered the error
			tc.mu.txn)
	}
...
func (tc *TxnCoordSender) SetDebugName(name string) error {
	tc.mu.Lock()
	defer tc.mu.Unlock()
	...
	if tc.mu.active {
		return errors.New("cannot change the debug name of a running transaction")
	}
	tc.mu.txn.Name = name
	return nil
}

// SetTxnMetaKeyIsSystemConfig is part of the client.TxnSender interface.
func (tc *TxnCoordSender) SetTxnMetaKeyIsSystemConfig() {
	tc.mu.Lock()
	defer tc.mu.Unlock()
	if err := tc.setTxnAnchorKeyLocked(keys.SystemConfigSpan.Key); err != nil {
		log.Errorf(context.Background(), "cannot set SystemConfigSpan")
	}
}
...
func (tc *TxnCoordSender) SetTimeForRC(ts hlc.Timestamp) {
	tc.mu.Lock()
	tc.mu.txn.IsolationLevel = util.ReadCommittedIsolation
	// 读写时间前推
	ts.Forward(tc.mu.txn.WriteTimestamp)
	tc.mu.txn.WriteTimestamp.Forward(ts)
	tc.mu.txn.ReadTimestamp.Forward(ts)
	tc.mu.txn.MinTimestamp.Forward(ts)
	tc.mu.txn.MaxTimestamp = ts.Add(tc.clock.MaxOffset().Nanoseconds(), 0)
	tc.mu.Unlock()
}
...
func (tc *TxnCoordSender) IsSerializablePushAndRefreshNotPossible() bool {
	...
	if tc.mu.txn.IsolationLevel == util.ReadCommittedIsolation {
		return false
	}
...
// GetTxnSpans 获取事务的读取和DML操作的数据范围
func (tc *TxnCoordSender) GetTxnSpans() client.TxnSpans {
	tc.mu.Lock()
	defer tc.mu.Unlock()
	pipelinerSpans := tc.interceptorAlloc.txnPipeliner.getPipelinerSpans()
	refresherSpans := tc.interceptorAlloc.txnSpanRefresher.getRefresherSpans()
	txnSpans := client.TxnSpans{
		PipelinerSpans: pipelinerSpans,
		RefresherSpans: refresherSpans,
	}
	return txnSpans
}
