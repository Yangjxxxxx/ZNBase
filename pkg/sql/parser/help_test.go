...
func TestContextualHelp(t *testing.T) {
	testData := []struct {
		...
		{`ALTER SCHEMA foo ??`, `ALTER SCHEMA`},
		{`ALTER SCHEMA foo RENAME ??`, `ALTER SCHEMA`},
		{`ALTER SCHEMA foo RENAME TO bar ??`, `ALTER SCHEMA`},
		...
		{`SHOW CREATE TRIGGER blah ??`, `SHOW CREATE TRIGGER`},
		{`SHOW TRIGGERS ??`, `SHOW TRIGGERS`},

		{`ALTER PROCEDURE ??`, `ALTER PROCEDURE/FUNCTION`},
		...
		{`CREATE SCHEMA blih ??`, `CREATE SCHEMA`},
		...
		{`DROP SCHEMA IF ??`, `DROP SCHEMA`},
		{`DROP SCHEMA IF EXISTS blah ??`, `DROP SCHEMA`},

		{`DROP INDEX blah, ??`, `DROP INDEX`},
		{`DROP INDEX blah@blih ??`, `DROP INDEX`},

		{`DROP FUNCTION ??`, `DROP PROCEDURE/FUNCTION`},
		...
		{`SHOW LOAD_KAFKA ??`, `SHOW LOAD_KAFKA`},
		...
		{`DUMP foo TO SST 'bar' ??`, `DUMP`},
		{`DUMP DATABASE ??`, `DUMP`},
		{`DUMP foo TO SST 'bar' AS OF ??`, `DUMP`},
		...
		{`SHOW FLASHBACK ??`, `SHOW FLASHBACK`},
		{`FLASHBACK TABLE ??`, `FLASHBACK`},
		{`FLASHBACK DATABASE ??`, `FLASHBACK`},

		{`DUMP ??`, `DUMP`},
		{`LOAD ??`, `LOAD`},

		{`CREATE SCHEDULE FOR DUMP ??`, `CREATE SCHEDULE FOR DUMP`},

		{`CLEAR ??`, `CLEAR`},

		{`QUERY LOCKSTAT ??`, `QUERY LOCKSTAT`},
	}
