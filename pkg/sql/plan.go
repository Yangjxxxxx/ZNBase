...
// PlanStmtAST() gives convenient access to the runParam's PlanStmtAST().
func (r *runParams) PlanStmtAST() tree.Statement {
	if r.p != nil && r.p.stmt != nil {
		return r.p.stmt.AST
	}
	return nil
}
...
// UDRRunParam is PLSQL runtime parmam
type UDRRunParam struct {
	// MagicCode is a random number from UINTMIN to UINTMAX
	ParamsEngineIndex uint32
	MagicCode         int
	Ctx               context.Context
	ExecCfg           *ExecutorConfig
	EvalCtx           *extendedEvalContext
	CachedPlan        map[uint32]UDRPlanner
	FoundIdx          int
	Params            tree.UDRVars
	Result            []tree.Datum
	CursorFetchRow    tree.Datums
	CursorColNum      int
	Return            tree.Datum
...
// UDRPlanner work for UDR to store plan/memo after prepare
type UDRPlanner struct {
	Planner  *planner
	Memo     *memo.Memo
	Expr     tree.TypedExpr
	Result   tree.Datum
...
func (p *planner) GetAuditInfo() *server.AuditInfo {
	p.curPlan.auditInfo = &server.AuditInfo{}
	return p.curPlan.auditInfo
}
...
// StatusServer gives convenient access to the runParam's StatusServer.
func (r *runParams) StatusServer() *serverpb.StatusServer {
	return &r.extendedEvalCtx.StatusServer
}
...
type planNode interface {
	//startExec(params runParams) (error, interface{})
...
// PlanNode is the exported name for planNode. Useful for ICL hooks.
...
var _ planNode = &declareCursorNode{}
var _ planNode = &cursorAccessNode{}
...
var _ planNodeFastPath = &alterUserNode{}
var _ planNodeFastPath = &alterUserDefaultSchemaNode{}
...
// DataStoreEngine interface used to set or get datastoreengine info.
type DataStoreEngine interface {
	SetDataStoreEngine(ds cat.DataStoreEngine)
...
func (p *planner) makePlan(ctx context.Context) error {
	...
	var err error
	p.curPlan.main.planNode, err = p.newPlan(ctx, stmt.AST, nil /*desiredTypes*/)
	if err != nil {
		return err
	}
	cols := planColumns(p.curPlan.main.planNode)
	if stmt.ExpectedTypes != nil {
		if !stmt.ExpectedTypes.TypesEqual(cols) {
			return pgerror.NewError(pgcode.FeatureNotSupported,
	...
	p.curPlan.main.planNode, err = p.hideHiddenColumns(ctx, p.curPlan.main.planNode, cols)
	if err != nil {
		p.curPlan.close(ctx)
		return err
	}
	...
	needed := allColumns(p.curPlan.main.planNode)
	p.curPlan.main.planNode, err = p.optimizePlan(ctx, p.curPlan.main.planNode, needed)
	if err != nil {
		p.curPlan.close(ctx)
		return err
	}
	...
	if log.V(3) {
		log.Infof(ctx, "statement %s compiled to:\n%s", stmt,
			planToString(ctx, p.curPlan.main.planNode, p.curPlan.subqueryPlans))
	}
...
func (p *planTop) collectSpans(params runParams) (readSpans, writeSpans roachpb.Spans, err error) {
	readSpans, writeSpans, err = collectSpans(params, p.main.planNode)
	if err != nil {
		return nil, nil, err
	}
	for i := range params.p.curPlan.subqueryPlans {
		reads, writes, err := collectSpans(params, params.p.curPlan.subqueryPlans[i].plan.planNode)
		if err != nil {
			return nil, nil, err
		}
...
func (p *planner) delegateQuery(
	...
	if p.SessionData() != nil {
		casesensitive = p.SessionData().CaseSensitive
	}
	stmt, err := parser.ParseOne(sql, casesensitive)
	if err != nil {
		return nil, err
	}
...
func (p *planner) newPlan(
	...
	if p.EvalContext().TxnReadOnly {
		if canModifySchema || tree.CanWriteData(stmt) {
			return nil, pgerror.NewErrorf(pgcode.ReadOnlySQLTransaction,
	...
	case *tree.AlterUser:
		return p.AlterUser(ctx, n)
	case *tree.AlterSchema:
		return p.AlterSchema(ctx, n)
	case *tree.AlterUserDefaultSchma:
		return p.AlterUserDefaultSchma(ctx, n)
	case *tree.AlterViewAS:
		return p.AlterView(ctx, n)
	case *tree.CallStmt:
		return p.CallStmt(ctx, n)
	...
	case *tree.ControlSchedules:
		return p.ControlScheduleJobs(ctx, n)
	case *tree.Scrub:
	...
	case *tree.SetReplica:
		return p.SetReplica(ctx, n)
	case *tree.ShowCursor:
		return p.ShowCursors(ctx, n)
	case *tree.ShowClusterSetting:
		return p.ShowClusterSetting(ctx, n)
	case *tree.ShowVar:
		return p.ShowVar(ctx, n) //delegate
	case *tree.ShowColumns:
		return p.ShowColumns(ctx, n) //delegate
	case *tree.ShowConstraints:
		return p.ShowConstraints(ctx, n) //delegate
	case *tree.ShowCreateTrigger:
		return p.ShowCreateTrigger(ctx, n) //delegate
	case *tree.ShowCreate:
		return p.ShowCreate(ctx, n) //delegate
	case *tree.ShowDatabases:
		return p.ShowDatabases(ctx, n) //delegate
	case *tree.ShowGrants:
		return p.ShowGrants(ctx, n)
	case *tree.ShowHistogram:
		return p.ShowHistogram(ctx, n)
	case *tree.ShowIndexes:
		return p.ShowIndex(ctx, n) //delegate
	case *tree.ShowQueries:
		return p.ShowQueries(ctx, n) //delegate
	case *tree.ShowJobs:
		return p.ShowJobs(ctx, n) //delegate
	case *tree.ShowSchedules:
		return p.showSchedules(ctx, n)
	case *tree.ShowRoleGrants:
		return p.ShowRoleGrants(ctx, n) //delegate
	case *tree.ShowRoles:
		return p.ShowRoles(ctx, n) //delegate
	case *tree.ShowSpaces:
		return p.ShowSpaces(ctx, n) //delegate
	case *tree.ShowSessions:
		return p.ShowSessions(ctx, n) //delegate
	case *tree.ShowTableStats:
		return p.ShowTableStats(ctx, n)
	case *tree.ShowSavepointStatus:
		return p.ShowSavepointStatus(ctx)
	case *tree.ShowSyntax:
		return p.ShowSyntax(ctx, n) //delegate
	case *tree.ShowTables:
		return p.ShowTables(ctx, n) //delegate
	case *tree.ShowFunctions:
		return p.ShowFunctions(ctx, n) //delegate
	case *tree.ShowSchemas:
		return p.ShowSchemas(ctx, n) //delegate
	case *tree.ShowSequences:
		return p.ShowSequences(ctx, n) //delegate
	case *tree.ShowTraceForSession:
		return p.ShowTrace(ctx, n)
	case *tree.ShowTransactionStatus:
		return p.ShowTransactionStatus(ctx) //delegate
	case *tree.ShowTriggers:
		return p.ShowTriggers(ctx, n) //delegate
	case *tree.ShowTriggersOnTable:
		return p.ShowTriggersOnTable(ctx, n) //delegate
	case *tree.ShowUsers:
		return p.ShowUsers(ctx, n) //delegate
	case *tree.ShowZoneConfig:
		return p.ShowZoneConfig(ctx, n) //delegate
	...
	case *tree.RenameFunction:
		return p.RenameFunction(ctx, n)
	default:
...
func (p *planner) doPrepare(ctx context.Context, stmt tree.Statement) (planNode, error) {
	...
	case *tree.ShowSpaces:
		return p.ShowSpaces(ctx, n)
	case *tree.ShowSessions:
		return p.ShowSessions(ctx, n)
	case *tree.ShowTables:
		return p.ShowTables(ctx, n)
	case *tree.ShowFunctions:
		return p.ShowFunctions(ctx, n)
	case *tree.ShowSchemas:
	...
	case *tree.ShowTriggersOnTable:
		return p.ShowTriggersOnTable(ctx, n)
	case *tree.ShowTransactionStatus:
