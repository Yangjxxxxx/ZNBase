...
func ZoneSpecifierFromID(
	...
	parentID, sc, err := resolveID(parentID)
	if err != nil {
		return tree.ZoneSpecifier{}, err
	}
	if parentID == keys.RootNamespaceID {
		return tree.ZoneSpecifier{
			TableOrIndex: tree.TableIndexName{
				Table: tree.MakeTableName(tree.Name(sc), tree.Name(name)),
			},
		}, nil
	}
	...
	return tree.ZoneSpecifier{
		TableOrIndex: tree.TableIndexName{
			Table: tree.MakeTableNameWithSchema(tree.Name(db), tree.Name(sc), tree.Name(name)),
		},
	}, nil
}
...
func ResolveZoneSpecifier(
	...
	parentID, err := resolveName(keys.RootNamespaceID, tn.Catalog())
	if err != nil {
		return 0, err
	}

	if tn.Catalog() != "system" {
		// if schema name is not public then parentID is schemaID
		parentID, err = resolveName(parentID, tn.Schema())
		if err != nil {
			return 0, err
		}
	}

	return resolveName(parentID, tn.Table())
}
...
func (z *ZoneConfig) ValidateTandemFields() error {
	for _, s := range z.Subzones {
		if err := s.Config.ValidateTandemFields(); err != nil {
			return err
		}
	}
