...
import (
	...
	_ "github.com/znbasedb/znbase/pkg/sql/sem/builtins"
...
func TestTypeCheck(t *testing.T) {
	testData := []struct {
		...
		{`IFNULL(1, 2.0)`, `IFNULL(MANUALCAST(1:::DECIMAL AS DECIMAL), MANUALCAST(2.0:::DECIMAL AS DECIMAL))`},
		...
		{`NVL(1, 2)`, `NVL(1:::INT8, 2:::INT8)`},
		{`NVL(1, 2.0)`, `NVL(MANUALCAST(1:::DECIMAL AS DECIMAL), MANUALCAST(2.0:::DECIMAL AS DECIMAL))`},
		{`NVL(NULL, 2)`, `NVL(NULL, 2:::INT8)`},
		{`NVL(2, NULL)`, `NVL(2:::INT8, NULL)`},
		{`NVL((1, 2), (1, 3))`, `NVL((1:::INT8, 2:::INT8), (1:::INT8, 3:::INT8))`},
		...
		{`true IS NULL`, `1:::DECIMAL IS NULL`},
		{`true IS NOT NULL`, `1:::DECIMAL IS NOT NULL`},
		...
		{`4 BETWEEN 2.4 AND 5.5::float`, `4:::DECIMAL BETWEEN 2.4:::DECIMAL AND MANUALCAST(5.5:::FLOAT8::FLOAT8 AS DECIMAL)`},
	...
	for _, d := range testData {
		t.Run(d.expr, func(t *testing.T) {
			...
			typeChecked, err := tree.TypeCheck(expr, &ctx, types.Any, false)
			if err != nil {
...
func TestTypeCheckError(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.expr, func(t *testing.T) {
			...
			if _, err := tree.TypeCheck(expr, nil, types.Any, false); !testutils.IsError(err, regexp.QuoteMeta(d.expected)) {
