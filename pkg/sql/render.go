...
type renderNode struct {
	...
	// FunctionDescCollection will store all functions desc of this render clause part
...
func (p *planner) Select(
	...
	if n.ForLocked != nil {
		return nil, errors.New("for-upDate cannot run in a non-optimizer")
	}
...
func (p *planner) SelectClause(
	...
	getTableDesc := func(name tree.TableName) (*sqlbase.TableDescriptor, error) {
		objDesc, err := p.PhysicalSchemaAccessor().GetObjectDesc(ctx, p.txn, &name,
	...
	andList, err := optbuilder.BuildPartitionExprs(parsed.From.Tables, getTableDesc)
	if err != nil {
		return nil, err
	}
	...
	if parsed.Where != nil {
		if partitionExprs != nil {
			parsed.Where.Expr = sqlbase.AndExpr(partitionExprs, parsed.Where.Expr)
		}
		...
		where, err = p.initWhere(ctx, r, partitionExprs)
		if err != nil {
			return nil, err
		}
	}
...
func (r *renderNode) GetVisibleType(idx int) string {
	return r.sourceInfo[0].SourceColumns[idx].VisibleTypeName
}

// SetForInsertOrUpdate implements the parser.IndexedVarContainer interface.
func (r *renderNode) SetForInsertOrUpdate(b bool) {
	r.IsInsertOrUpdate = b
}

// GetForInsertOrUpdate implements the parser.IndexedVarContainer interface.
func (r *renderNode) GetForInsertOrUpdate() bool {
...
func (p *planner) initTargets(
	...
	for i, target := range targets {
		...
		cols, exprs, hasStar, err := p.computeRenderAllowingStars(ctx, target, desiredType, desiredTypes, r.sourceInfo, r.ivarHelper, outputName)
		if err != nil {
			return err
		}
	...
	if asOf.Expr != nil {
		...
		if p.semaCtx.AsOfTimestamp == nil {
			return hlc.MaxTimestamp, false,
				pgerror.NewErrorf(pgcode.Syntax,
...
func (r *renderNode) SetDataStoreEngine(ds cat.DataStoreEngine) {
	r.storeEngine = ds
}

func (r *renderNode) GetDataStoreEngine() cat.DataStoreEngine {
	return r.storeEngine
}
