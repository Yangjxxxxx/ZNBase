...
const baseUpdatesURL = `https://register.znbasedb.com/api/clusters/updates`
const baseReportingURL = `https://register.znbasedb.com/api/clusters/report`
...
func init() {
	...
	updatesURL, err = url.Parse(
		envutil.EnvOrDefaultString("ZNBASE_UPDATE_CHECK_URL", baseUpdatesURL),
	)
	if err != nil {
		panic(err)
	}
	reportingURL, err = url.Parse(
		envutil.EnvOrDefaultString("ZNBASE_USAGE_REPORT_URL", baseReportingURL),
	)
	if err != nil {
		panic(err)
	}
...
func addInfoToURL(ctx context.Context, url *url.URL, s *Server, n diagnosticspb.NodeInfo) {
	internal := strings.Contains(sql.ClusterOrganization.Get(&s.st.SV), "ZNBase Labs")
	q := url.Query()
	b := n.Build
	q.Set("version", b.Tag)
