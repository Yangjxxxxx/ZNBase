...
func testAggregateResultDeepCopy(
	t *testing.T,
	aggFunc func([]types.T, *tree.EvalContext, tree.Datums, *mon.BoundAccount) tree.AggregateFunc,
	vals []tree.Datum,
) {
	evalCtx := tree.NewTestingEvalContext(cluster.MakeTestingClusterSettings())
	defer evalCtx.Stop(context.Background())
	aggImpl := aggFunc([]types.T{vals[0].ResolvedType()}, evalCtx, nil, nil)
...
func testArrayAggAliasedTypeOverload(t *testing.T, expected types.T) {
	...
	typedExpr, err := tree.TypeCheck(expr, nil, types.TArray{Typ: expected}, false)
	if err != nil {
...
func runBenchmarkAggregate(
	...
	for i := 0; i < b.N; i++ {
		func() {
			aggImpl := aggFunc(params, evalCtx, nil, nil)
			defer aggImpl.Close(context.Background())
			for i := range vals {
				if err := aggImpl.Add(context.Background(), vals[i]); err != nil {
					b.Fatal(err)
