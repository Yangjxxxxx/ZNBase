...
// RaftSnapshotQueueConcurrency is the setting of queue concurrency.
var RaftSnapshotQueueConcurrency = settings.RegisterPositiveIntSetting(
	"kv.raft_snapshot.queue_concurrency",
	"raftSnapshotQueue manages a queue of replicas which may need to "+
		"catch a replica up with a snapshot to their range. "+
...
func newRaftSnapshotQueue(store *Store, g *gossip.Gossip) *raftSnapshotQueue {
	...
	rq.baseQueue = newBaseQueue(
		...
		queueConfig{
			...
			maxConcurrency: RaftSnapshotQueueConcurrency.Get(&store.ClusterSettings().SV),
	...
	confChanged := func() {
		rq.maxConcurrency = RaftSnapshotQueueConcurrency.Get(&store.ClusterSettings().SV)
		rq.confCh <- struct{}{}
	}
	RaftSnapshotQueueConcurrency.SetOnChange(&store.ClusterSettings().SV, confChanged)
	return rq
}
