...
const (
	// MVCCVersionTimestampSize The size of the timestamp portion of MVCC version keys (used to update stats).
...
func mvccPutInternal(
	...
	var backFill bool
	switch opt := engine.(type) {
	case *OpLoggerBatch:
		backFill = opt.BackFill
	case *DistinctOpLoggerBatch:
		backFill = opt.parent.BackFill
	}
	...
	if putIsInline {
		...
		if err == nil {
			engine.LogLogicalOp(logicalOp, MVICLogicalOpDetails{
				Key:      key,
				Safe:     true,
				BackFill: backFill,
			})
	...
	if ok {
		...
		if meta.Txn != nil {
			...
			// timestamp.
			if txn != nil {
				if txn.IsolationLevel == util.ReadCommittedIsolation {
					// 不要遗留write too old
	...
	logicalOpDetails := MVICLogicalOpDetails{
		Key:       key,
...
func MVCCDeleteRange(
	...
	res, err := MVCCScan(ctx, engine, key, endKey, max, timestamp, MVCCScanOptions{
		FailOnMoreRecent: true, Txn: scanTxn,
	})
	if err != nil {
...
// MVCCClearIntent 清理写意图数据
func MVCCClearIntent(
	ctx context.Context,
	engine ReadWriter,
	ms *enginepb.MVCCStats,
	beginKey, endKey roachpb.Key,
	max int64,
	timestamp hlc.Timestamp, // 当前写入时间戳
	txn *roachpb.Transaction,
	returnKeys bool,
	clearIntents []roachpb.Intent,
) (*roachpb.Span, int64, *[]roachpb.Intent, error) {
	var resume *roachpb.Span
	var rIntents []roachpb.Intent

	iter := engine.NewIterator(IterOptions{LowerBound: beginKey, UpperBound: endKey})
	defer iter.Close()

	iter.Seek(MakeMVCCMetadataKey(beginKey))

	// 发现未知状态的写意图,需要返回重新检查状态
	findUnknownIntent := len(rIntents) == 0
	...
	for ; ; iter.NextKey() {
		...
		if !unsafeKey.IsValue() {
			...
			// 快照恢复期间遇到其他未提交的事务
			if meta.Txn.ID != txn.ID {
				// 清理已提交事物的写意图
				for _, v := range clearIntents {
					if v.Txn.ID == meta.Txn.ID {
				...
				// 如果未被清理,需要进行一次排重,然后返回给上游检查写意图状态
				if !isCleared {
					// 未知状态写意图需要记录返回确认状态
					isExists := false
					for _, v := range rIntents {
						if v.Txn.ID == meta.Txn.ID {
							isExists = true
						}
					}

					if findUnknownIntent && !isExists {
						rIntents = append(rIntents, roachpb.MakeIntent(meta.Txn, append([]byte{}, meta.Txn.Key...)))
					}
				}
	...
	if findUnknownIntent && len(rIntents) > 0 {
		resume = &roachpb.Span{Key: append([]byte{}, beginKey...), EndKey: endKey}
		return resume, 0, &rIntents, nil
	}

	return nil, int64(clearedIntentNums), &rIntents, nil
}

// MVCCRevertRange 恢复range数据到某个快照时间点
func MVCCRevertRange(
	ctx context.Context,
	engine ReadWriter,
	ms *enginepb.MVCCStats,
	beginKey, endKey roachpb.Key,
	max int64,
	timestamp hlc.Timestamp, // 当前写入时间戳
	snapshotTs hlc.Timestamp, // 快照时间点
	txn *roachpb.Transaction,
	returnKeys bool,
	usePutInternal bool,
	clearIntents []roachpb.Intent,
) ([]roachpb.Key, *roachpb.Span, int64, *[]roachpb.Intent, error) {
	var resume *roachpb.Span
	var snapKVs []MVCCKeyValue
	var rIntents []roachpb.Intent
	...
	iter.Seek(MakeMVCCMetadataKey(beginKey))
	preKv := MVCCKeyValue{}
	// nextKey为true表示处理下一条记录;
	// nextKey为false表示处理该记录下一个版本,如果当前正好是该记录的最后一个版本,就跳到下一条记录
	nextKey := false
	iterNext := func() {
		if nextKey {
			iter.NextKey()
	...
	for ; ; iterNext() {
		...
		if !unsafeKey.IsValue() {
			...
			if meta.Txn.ID != txn.ID {
				// 清理已提交事物的写意图
				for _, v := range clearIntents {
					//fmt.Printf("v.Txn.ID:%v, meta.Txn.ID:%v\n",v.Txn.ID, meta.Txn.ID)
					if v.Txn.ID == meta.Txn.ID {
						if err := engine.Clear(iter.Key()); err != nil {
							return nil, nil, 0, &rIntents, err
						}
		...
		// 如果出现WriteIntent本次就只返回需要清理的Intent
		if hasUnknownIntent {
			nextKey = true
			preKv = MVCCKeyValue{}
			continue
		...
		if !unsafeKey.Key.Equal(preKv.Key.Key) { // 处理新的key
			if int64(len(snapKVs)) > max {
				resume = &roachpb.Span{Key: append([]byte{}, iter.Key().Key...), EndKey: endKey}
				break
			}
	...
	if hasUnknownIntent {
		resume = &roachpb.Span{Key: append([]byte{}, beginKey...), EndKey: endKey}
		return nil, resume, 0, &rIntents, nil
	}

	if preKv.Key.Key != nil && snapshotTs.Less(preKv.Key.Timestamp) && len(preKv.Value) != 0 {
		// 最后的key的时间戳都大于快照点，表示新增加数据，需要删除
		// 如果最新的数据已经delete，则不处理
		snapKVs = append(snapKVs, MVCCKeyValue{Key: preKv.Key, Value: nil})
	}
	...
	buf := newPutBuffer()
	defer buf.release()
	pIter := engine.NewIterator(IterOptions{Prefix: true})
	defer pIter.Close()

	if usePutInternal { // 使用mvccPutInternal可以保证一致性,但会插入写意图导致性能下降
		for i := range snapKVs {
			err := mvccPutInternal(
				ctx, engine, pIter, ms, snapKVs[i].Key.Key, timestamp, snapKVs[i].Value, txn, buf, nil)
			if err != nil {
				return nil, nil, 0, &rIntents, err
			}
		}
	} else {
		for i := range snapKVs { // 无法保证一致性,会有中间状态,性能更高
			versionKey := MakeMVCCMetadataKey(snapKVs[i].Key.Key)
			versionKey.Timestamp = timestamp

			if err := engine.Put(versionKey, snapKVs[i].Value); err != nil {
				return nil, resume, 0, &rIntents, err
			}
		}
	}

	var rKeys []roachpb.Key
	if returnKeys && len(snapKVs) > 0 {
		rKeys = make([]roachpb.Key, len(snapKVs))
		for i := range snapKVs {
			rKeys[i] = snapKVs[i].Key.Key
		}
	}

	return rKeys, resume, int64(len(snapKVs)), &rIntents, nil
}
...
func mvccScanToKvs(
	...
	res, err := iter.MVCCScan(key, endKey, max, timestamp, opts)
	if err != nil {
...
func MVCCScanAsTxn(
	...
	return MVCCScan(ctx, reader, key, endKey, int64(math.MaxInt64), timestamp, MVCCScanOptions{
		Txn: &roachpb.Transaction{
			TxnMeta:       txnMeta,
			Status:        roachpb.PENDING,
			ReadTimestamp: txnMeta.WriteTimestamp,
...
func MVCCResolveWriteIntent(
	ctx context.Context, engine ReadWriter, ms *enginepb.MVCCStats, intent roachpb.LockUpdate,
) (bool, error) {
	iterAndBuf := GetBufUsingIter(engine.NewIterator(IterOptions{Prefix: true}))
	ok, err := MVCCResolveWriteIntentUsingIter(ctx, engine, iterAndBuf, ms, intent)
	// Using defer would be more convenient, but it is measurably slower.
	iterAndBuf.Cleanup()
	return ok, err
}
...
func MVCCResolveWriteIntentUsingIter(
	...
	return mvccResolveWriteIntent(ctx, engine, iterAndBuf.iter, ms, intent, iterAndBuf.buf)
}
...
func mvccResolveWriteIntent(
	...
	if commit || pushed || rolledBackVal != nil {
		...
		if opLoggerBatch, ok := rw.(*OpLoggerBatch); ok {
			backFill = opLoggerBatch.BackFill
		} else {
			if logicalOp == MVCCCommitIntentOpType {
				if opLoggerBatch, ok := rw.(*DistinctOpLoggerBatch); ok {
					if opLoggerBatch.parent != nil {
						backFill = opLoggerBatch.parent.BackFill
					}
				}
			}

		}
		rw.LogLogicalOp(logicalOp, MVICLogicalOpDetails{
			Txn:       intent.Txn,
			Key:       intent.Key,
	...
	// Log the logical MVCC operation.
	rw.LogLogicalOp(MVCCAbortIntentOpType, MVICLogicalOpDetails{
		Txn: intent.Txn,
		Key: intent.Key,
	})
...
func MVCCResolveWriteIntentRangeUsingIter(
	...
	for {
		...
		if !key.IsValue() {
			intent.Key = key.Key
			ok, err = mvccResolveWriteIntent(ctx, engine, iterAndBuf.iter, ms, intent, iterAndBuf.buf)
		}
		if err != nil {
...
func MVCCGarbageCollect(
	...
	for _, gcKey := range keys {
		...
		if (gcKey.Timestamp == zero && hlc.Timestamp(meta.Timestamp) == zero) ||
			(!gcKey.Timestamp.Less(hlc.Timestamp(meta.Timestamp)) && gcKey.From.Less(hlc.Timestamp(meta.Timestamp))) {
		...
		for ; ; iter.Next() {
			...
			if (gcKey.Timestamp == zero && hlc.Timestamp(meta.Timestamp) == zero) ||
				(!gcKey.Timestamp.Less(unsafeIterKey.Timestamp) && gcKey.From.Less(unsafeIterKey.Timestamp)) {
				if ms != nil {
