...
func TestInboundStreamTimeoutIsRetryable(t *testing.T) {
	...
	if err := fr.RegisterFlow(
		context.Background(), distsqlpb.FlowID{}, &FlowBase{}, inboundStreams, 0, /* timeout */
	); err != nil {
		t.Fatal(err)
	}
	wg.Wait()
...
func TestTimeoutPushDoesntBlockRegister(t *testing.T) {
	...
	if err := fr.RegisterFlow(
		ctx, distsqlpb.FlowID{}, &FlowBase{}, inboundStreams, 0, /* timeout */
	); err != nil {
		t.Fatal(err)
	}
	...
	if err := fr.RegisterFlow(
		ctx, distsqlpb.FlowID{UUID: uuid.MakeV4()}, &FlowBase{}, nil /* inboundStreams */, time.Hour, /* timeout */
	); err != nil {
		t.Fatal(err)
	}
...
func TestFlowCancelPartiallyBlocked(t *testing.T) {
	...
	ctx := context.Background()
	fr := NewFlowRegistry(0)
	left := &runbase.RowChannel{}
	left.InitWithBufSizeAndNumSenders(nil /* types */, 1, 1)
	right := &runbase.RowChannel{}
