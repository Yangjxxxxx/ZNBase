...
`); err != nil {
	...
	tb2NameKey := sqlbase.MakeNameMetadataKey(scDesc.ID, "kv2")
...
func TestDropIndexWithZoneConfigOSS(t *testing.T) {
	...
	// Hack in zone configs for the primary and secondary indexes. (You need a ICL binary to do this properly.) Dropping the index will thus require
	// regenerating the zone config's SubzoneSpans, which will fail with a "ICL
	// required" error.
	...
	// Verify that dropping the index fails with a "ICL required" error.
	_, err = sqlDBRaw.Exec(`DROP INDEX t.kv@foo`)
	if err != nil {
		t.Fatalf("expected success, but Got ICL required error %v !", err)
	}
	tableDesc = sqlbase.GetTableDescriptor(kvDB, "t", "public", "kv")
	if _, _, err := tableDesc.FindIndexByName("foo"); err == nil {
		t.Fatalf("table descriptor still contains index after index is dropped")
...
func TestDropIndexInterleaved(t *testing.T) {
	...
	tableDesc = sqlbase.GetTableDescriptor(kvDB, "t", "public", "intlv")
...
func TestDropTable(t *testing.T) {
	...
	tableDesc := sqlbase.GetTableDescriptor(kvDB, "t", "public", "kv")
	nameKey := sqlbase.MakeNameMetadataKey(keys.MinNonPredefinedUserDescID+1, "kv")
...
func TestDropTableDeleteData(t *testing.T) {
	...
	for i := 0; i < numTables; i++ {
		...
		descs = append(descs, sqlbase.GetTableDescriptor(kvDB, "t", "public", tableName))

		schemaRange := 1
		nameKey := sqlbase.MakeNameMetadataKey(sqlbase.ID(keys.MinNonPredefinedUserDescID+schemaRange), tableName)
		gr, err := kvDB.Get(ctx, nameKey)
		if err != nil {
			t.Fatal(err)
		}
...
func TestDropTableInterleavedDeleteData(t *testing.T) {
	...
	tableDesc := sqlbase.GetTableDescriptor(kvDB, "t", "public", "kv")
	tableDescInterleaved := sqlbase.GetTableDescriptor(kvDB, "t", "public", "intlv")
	tableSpan := tableDesc.TableSpan()
...
func TestDropNameReuse(t *testing.T) {
	defer leaktest.AfterTest(t)()
	t.Skip("err will occur sometime during logic test")
...
func TestDropSchemaRestrict(t *testing.T) {
	defer leaktest.AfterTest(t)()
	params, _ := tests.CreateTestServerParams()
	params.Knobs = base.TestingKnobs{
		SQLSchemaChanger: &sql.SchemaChangerTestingKnobs{
...
`); err != nil {
	...
	scNameKey := sqlbase.MakeNameMetadataKey(dbDesc.ID, scDesc.Name)
	...
	if _, err := sqlDB.Exec(`SET sql_safe_updates = true`); err != nil {
		t.Fatal(err)
	}
	if _, err := sqlDB.Exec(`DROP SCHEMA t`); !testutils.IsError(err,
		`rejected: DROP SCHEMA on non-empty schema without explicit CASCADE \(sql_safe_updates = true\)`) {
		t.Fatal(err)
	}

	if _, err := sqlDB.Exec(`DROP SCHEMA t RESTRICT`); !testutils.IsError(err,
		`pq:  schema "t" is not empty and RESTRICT was specified`) {
		t.Fatal(err)
	}

	if _, err := sqlDB.Exec(`DROP SCHEMA t CASCADE`); err != nil {
		t.Fatal(err)
	}
	...
	if gr, err := kvDB.Get(ctx, tbNameKey); err != nil {
		...
		t.Fatalf("table descriptor key still exists after schema is dropped")
	}

	if err := descExists(sqlDB, false, scDesc.ID); err != nil {
		t.Fatal(err)
	}
	// Database zone config is removed once all table data and zone configs are removed.
	if err := zoneExists(sqlDB, &cfg, dbDesc.ID); err != nil {
	...
	if gr, err := kvDB.Get(ctx, scNameKey); err != nil {
		t.Fatal(err)
	} else if gr.Exists() {
		t.Fatalf("database descriptor key still exists after database is dropped")
	}
	...
	if err := jobutils.VerifyRunningSystemJob(t, sqlRun, 0, jobspb.TypeSchemaChange, jobs.RunningStatusWaitingGC, jobs.Record{
		Username:    security.RootUser,
		Description: "DROP SCHEMA t CASCADE",
		DescriptorIDs: sqlbase.IDs{
			tbDesc.ID,
		},
	}); err != nil {
