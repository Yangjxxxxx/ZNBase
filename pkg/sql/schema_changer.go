...
type SchemaChanger struct {
	...
	isFunc               bool
	internalSessiondata  *sessiondata.SessionData
}
...
func (sc *SchemaChanger) refreshMaterializedView(
	ctx context.Context, table *sqlbase.TableDescriptor, refresh *sqlbase.MaterializedViewRefresh,
) error {
	...
	tableToRefresh := protoutil.Clone(table).(*sqlbase.TableDescriptor)
...
func (sc *SchemaChanger) backfillQueryIntoTable(
	ctx context.Context, table *sqlbase.TableDescriptor, query string, ts hlc.Timestamp, desc string,
) error {
	if fn := sc.testingKnobs.RunBeforeBackfill; fn != nil {
		if err := fn(); err != nil {
			return err
		}
	}
	...
	return sc.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		var cleanup func()
		if sc.internalSessiondata == nil {
			localPlanner, cleanup = newInternalPlanner(desc, txn, security.RootUser, &MemoryMetrics{}, sc.execCfg)
		} else {
			localPlanner, cleanup = newMViewInternalPlanner(desc, txn, sc.internalSessiondata, &MemoryMetrics{}, sc.execCfg)
		}
		defer cleanup()
		var stmt parser.Statement
		var err error
		if strings.Contains(query, "rownum") {
			query = sqlutil.SolveRownumInCreateView(query)
			stmt, err = parser.ParseOne(query, false)
		} else {
			stmt, err = parser.ParseOne(query, false)
		}
		if err != nil {
		...
		localPlanner.extendedEvalCtx.InternalExecutor = sc.ieFactory(ctx, localPlanner.SessionData())
		...
		willDistribute := shouldDistributePlan(
			ctx,
			localPlanner.extendedEvalCtx.SessionData.DistSQLMode,
			localPlanner.DistSQLPlanner(),
		...
		localPlanner.runWithOptions(resolveFlags{skipCache: true}, func() {
			...
			} else if len(localPlanner.curPlan.planComponents.subqueryPlans) != 0 {
				if !sc.distSQLPlanner.PlanAndRunSubqueries(
					ctx, localPlanner, localPlanner.ExtendedEvalContextCopy,
					localPlanner.curPlan.planComponents.subqueryPlans, recv, willDistribute,
				) {
					if planAndRunErr = rw.Err(); planAndRunErr != nil {
						return
					}
			...
			sc.distSQLPlanner.PlanAndRunCTAS(ctx, localPlanner,
...
// mviewInternalPlanner creates a new planner instance for internal usage.
// This planner is created using the sessionData from SQL session, in order
// to achieve the creation of materialized views using partitioned tables
// and pg_buildtins.
//
// Since it can't be reset, the planner can be used only for planning a
// single statement.
//
// Returns a cleanup function that must be called once the caller is done
// with the planner.
func newMViewInternalPlanner(
	opName string,
	txn *client.Txn,
	internalSessionData *sessiondata.SessionData,
	...
	plannerMon := mon.MakeUnlimitedMonitor(ctx,
		fmt.Sprintf("internal-planner.%s.%s", sd.User, opName),
		mon.MemoryResource,
...
// DropFuncDesc removes a descriptor from the KV database.
func (sc *SchemaChanger) DropFuncDesc(
	ctx context.Context, funcDesc *sqlbase.FunctionDescriptor, traceKV bool,
) error {
	descKey := sqlbase.MakeDescMetadataKey(funcDesc.ID)
	zoneKeyPrefix := config.MakeZoneKeyPrefix(uint32(funcDesc.ID))
	...
	return sc.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		if funcDesc.GetDropJobID() != 0 {
			if err := sc.updateDropTableJob(
				ctx,
				txn,
				funcDesc.GetDropJobID(),
				funcDesc.ID,
				jobspb.Status_DONE,
				func(ctx context.Context, txn *client.Txn, job *jobs.Job) error {
					// Delete the zone config entry for the dropped database associated
				...
				log.Warningf(ctx, "failed to update job %d: %+v", funcDesc.GetDropJobID(), err)
			}
		}
		return txn.Run(ctx, b)
	})
...
// maybeBackfillCreateTableAs maybe backfill a created table by executing the AS query. Return nil if
...
func (sc *SchemaChanger) maybeBackfillCreateTableAs(
	...
	return sc.backfillQueryIntoTable(ctx, table, table.CreateQuery, table.CreateAsOfTime, "ctasBackfill")
}
...
func (sc *SchemaChanger) maybeBackfillMaterializedView(
	...
	return sc.backfillQueryIntoTable(ctx, table, table.ViewQuery, table.CreateAsOfTime, "materializedViewBackfill")
}
...
// maybe Drop a table. Return nil if successfully dropped.
func (sc *SchemaChanger) maybeDropFunction(
	ctx context.Context,
	inSession bool,
	function *sqlbase.FunctionDescriptor,
	evalCtx *extendedEvalContext,
) error {
	if !function.Dropped() || inSession {
		return nil
	}

	// This can happen if a change other than the drop originally
	// scheduled the changer for this function. If that's the case,
	// we still need to wait for the deadline to expire.
	if function.DropTime != 0 {
		var timeRemaining time.Duration
		if err := sc.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
			timeRemaining = 0
			_, zoneCfg, _, err := GetZoneConfigInTxn(ctx, txn, uint32(function.ID),
				&sqlbase.IndexDescriptor{}, "", false /* getInheritedDefault */)
			if err != nil {
				return err
			}
			deadline := function.DropTime + int64(zoneCfg.GC.TTLSeconds)*time.Second.Nanoseconds()
			timeRemaining = timeutil.Since(timeutil.Unix(0, deadline))
			return nil
		}); err != nil {
			return err
	...
	// Do all the hard work of deleting the function data and the function ID.
	// if err := sc.truncateTable(ctx, &lease, function, evalCtx); err != nil {
	// 	return err
	// }

	if err := sc.DropFuncDesc(ctx, function, false /* traceKV */); err != nil {
		return err
	}
	// The descriptor was deleted.
	needRelease = false
...
// // maybe make a table PUBLIC if it's in the ADD state.
// func (sc *SchemaChanger) maybeMakeAddFuncPublic(
// 	ctx context.Context, function *sqlbase.FunctionDescriptor,
// ) error {
// 	if function.Adding() {
// 		if _, err := sc.leaseMgr.Publish(
// 			ctx,
	...
	_, err := sc.leaseMgr.Publish(
		...
		func(txn *client.Txn) error {
			...
			for _, drain := range namesToReclaim {
				tbKey := tableKey{parentID: drain.ParentID, name: drain.Name}.Key()
				b.Del(tbKey)
				err := sqlbase.RemoveObjectNamespaceEntry(ctx, txn, drain.ParentID,
...
// Drain old names from the cluster.
func (sc *SchemaChanger) drainFuncNames(ctx context.Context) error {
	// Publish a new version with all the names drained after everyone
	// has seen the version with the new name. All the draining names
	// can be reused henceforth.
	var namesToReclaim []sqlbase.FunctionDescriptor_NameInfo
	var dropJobID int64
	_, err := sc.leaseMgr.PublishFunction(
		ctx,
		sc.tableID,
		func(desc *sqlbase.MutableFunctionDescriptor) error {
			if sc.testingKnobs.OldNamesDrainedNotification != nil {
				sc.testingKnobs.OldNamesDrainedNotification()
			}
			// Free up the old name(s) for reuse.
		...
		func(txn *client.Txn) error {
			b := txn.NewBatch()
			for _, drain := range namesToReclaim {
				index := strings.Index(drain.Name, "(")
				funcName := ""
				if index > -1 {
					funcName = drain.Name[:index]
				} else {
					return pgerror.NewErrorf(pgcode.InvalidName, "Unknown function: %s", drain.Name)
				}
				tbKey := functionKey{parentID: drain.ParentID, name: drain.Name, funcName: funcName}.Key()
				b.Del(tbKey)
			}
...
func (sc *SchemaChanger) exec(
	...
	var err error
	if sc.isFunc {
		funcDesc, notFirst, err = sc.notFirstInLineFunc(ctx)
		...
		if funcDesc.HasDrainingNames() {
			if err := sc.drainFuncNames(ctx); err != nil {
				return err
			}
		}

		if err := sc.maybeDropFunction(ctx, inSession, funcDesc, evalCtx); err != nil {
			return err
		}

		// Find our job.
		foundJobID := false
		for _, g := range funcDesc.MutationJobs {
...
func (sc *SchemaChanger) done(ctx context.Context) (*sqlbase.ImmutableTableDescriptor, error) {
	...
	update := func(descs map[sqlbase.ID]*sqlbase.MutableTableDescriptor) error {
		...
		for _, mutation := range scDesc.Mutations {
			...
			if refresh := mutation.GetMaterializedViewRefresh(); refresh != nil {
				...
				if mutation.Direction == sqlbase.DescriptorMutation_ADD {
					...
					indexGCDetails := jobspb.SchemaChangeGCDetails{
						Indexes: []jobspb.SchemaChangeGCDetails_DroppedIndex{
							{
								IndexID:  scDesc.PrimaryIndex.ID,
								DropTime: now,
							},
						},
						ParentID: sc.tableID,
					...
					for i := range scDesc.Indexes {
						indexGCDetails := jobspb.SchemaChangeGCDetails{
							Indexes: []jobspb.SchemaChangeGCDetails_DroppedIndex{
								{
									IndexID:  scDesc.Indexes[i].ID,
									DropTime: now,
								},
							},
							ParentID: sc.tableID,
				...
				} else if mutation.Direction == sqlbase.DescriptorMutation_DROP {

					indexGCDetails := jobspb.SchemaChangeGCDetails{
						Indexes: []jobspb.SchemaChangeGCDetails_DroppedIndex{
							{
								IndexID:  refresh.NewPrimaryIndex.ID,
								DropTime: now,
							},
						},
						ParentID: sc.tableID,
					...
					for i := range scDesc.Indexes {
						indexGCDetails := jobspb.SchemaChangeGCDetails{
							Indexes: []jobspb.SchemaChangeGCDetails_DroppedIndex{
								{
									IndexID:  refresh.NewIndexes[i].ID,
									DropTime: now,
								},
							},
							ParentID: sc.tableID,
			...
			// update location Map at here
			if err := sc.updateLocationMap(ctx, &scDesc.TableDescriptor, mutation); err != nil {
				return err
			}
		}
		if i == 0 {
	...
	descs, err := sc.leaseMgr.PublishMultiple(ctx, tableIDsToUpdate, update, func(txn *client.Txn) error {
		...
		if err := sc.execCfg.AuditServer.LogAudit(
			ctx,
			false,
			&server.AuditInfo{
				EventTime: timeutil.Now(),
				EventType: string(schemaChangeEventType),
				TargetInfo: &server.TargetInfo{
					TargetID: int32(sc.tableID),
				},
				Info: struct {
					MutationID uint32
...
// updateLocationMap will update the locationMap of this table after index setup finished
func (sc *SchemaChanger) updateLocationMap(
	ctx context.Context, tabDesc *TableDescriptor, m sqlbase.DescriptorMutation,
) error {
	// first get index from Mutation
	var index *sqlbase.IndexDescriptor
	var newLocateNums int32
	switch m.Direction {
	case sqlbase.DescriptorMutation_ADD:
		// do add index job
		...
		// if index is nil then nothing to do direct return
		if index == nil {
			return nil
		}
		// first set location number by check locatespace name
		if index.LocateSpaceName != nil {
			newLocateNums = 1
		} else {
			newLocateNums = 0
		}
		newLocateNums += index.Partitioning.LocationNums
		// newLocateNums == 0 implicit thats this index job with out any locate in setting
		if newLocateNums == 0 {
			return nil
		}
		// find index from tab by name
		if indexInTab, found, err := tabDesc.FindIndexByName(index.Name); err != nil {
			return err
		} else if found {
			return errors.New("Index not found")
		} else {
			if err := updateIndexLocationNums(tabDesc, indexInTab, newLocateNums); err != nil {
				return err
			}
		}
	case sqlbase.DescriptorMutation_DROP:
		...
		if index == nil || index.LocationNums == 0 {
			// if index is nil then return directly
			// implicit than this index has not any locate in information
			return nil
		}
		newLocateNums = tabDesc.LocationNums - index.LocationNums
		if err := updateTableLocationNums(tabDesc, newLocateNums); err != nil {
			return err
		}
	}
	locationKey := config.MakeLocationKey(uint32(tabDesc.ID))
	if keyValue, err := sc.db.Get(ctx, locationKey); err != nil {
		return err
	} else if keyValue.Value == nil {
		return nil
	}
	// geo-partition feature add by jiye
	locationMap, err := GetLocationMapICL(tabDesc)
	if err != nil {
		return err
	}
	if locationMap == nil {
		return sc.db.Del(ctx, locationKey)
	}
	value, err := protoutil.Marshal(locationMap)
	if err != nil {
		return err
	}
	if err := sc.db.Put(ctx, config.MakeLocationKey(uint32(tabDesc.ID)), value); err != nil {
		return err
	}
	// first update location map
	sysconfig := sc.execCfg.Gossip.GetSystemConfig()
	sysconfig.UpdateLocationCache(uint32(tabDesc.ID), locationMap)
	return nil
}
...
// notFirstInLineFunc returns true whenever the schema change has been queued
// up for execution after another schema change.
func (sc *SchemaChanger) notFirstInLineFunc(
	ctx context.Context,
) (*sqlbase.FunctionDescriptor, bool, error) {
	var notFirst bool
	var desc *sqlbase.FunctionDescriptor
	err := sc.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		notFirst = false
		var err error
		desc, err = sqlbase.GetFunctionDescFromID(ctx, txn, sc.tableID)
		if err != nil {
			return err
		}
		// for i, mutation := range desc.Mutations {
...
func (sc *SchemaChanger) reverseMutations(ctx context.Context, causingError error) error {
	...
	_, err = sc.leaseMgr.PublishMultiple(ctx, tableIDsToUpdate, update, func(txn *client.Txn) error {
		...
		if err := sc.execCfg.AuditServer.LogAudit(
			...
			&server.AuditInfo{
				EventTime: timeutil.Now(),
				EventType: string(EventLogReverseSchemaChange),
				TargetInfo: &server.TargetInfo{
					TargetID: int32(sc.tableID),
				},
				Info: struct {
			...
			case <-gossipUpdateC:
				...
				cfgFilter.ForModified(cfg, func(kv roachpb.KeyValue) {
					...
					case *sqlbase.Descriptor_Function:
						...
						// function.MaybeFillInDescriptor()
						if err := function.ValidateFunction(s.execCfg.Settings); err != nil {
							log.Errorf(ctx, "%s: received invalid function descriptor: %s. Desc: %v",
								kv.Key, err, function,
							)
							return
						}

						schemaChanger.tableID = function.ID
						schemaChanger.mutationID = sqlbase.InvalidMutationID
						schemaChanger.execAfter = execAfter
						schemaChanger.dropTime = 0
						schemaChanger.isFunc = true

						var minDeadline int64
						if len(function.GCMutations) > 0 {
							zoneCfg, placeholder, _, err := ZoneConfigHook(cfg, uint32(function.ID))
							if err != nil {
								log.Errorf(ctx, "zone config for functionDesc: %d, err = %+v", function.ID, err)
								return
							}
							...
							for _, m := range function.GCMutations {
								// Initialize TTL without a zone config in case it's not present.
								ttlSeconds := defTTL
								if zoneCfg != nil {
									ttlSeconds = zoneCfg.GC.TTLSeconds
						...
						// Keep track of outstanding schema changes.
						// pendingChanges := function.Adding() ||
						// 	function.HasDrainingNames() || len(function.Mutations) > 0

						pendingChanges := function.Adding()
						if pendingChanges {
							if log.V(2) {
								log.Infof(ctx, "%s: queue up pending schema change; function: %d, version: %d",
									kv.Key, function.ID, function.Version)
							}

							// if len(function.Mutations) > 0 {
							// 	schemaChanger.mutationID = function.Mutations[0].MutationID
							// }

							s.schemaChangers[function.ID] = schemaChanger
						} else if function.Dropped() {
							// If the function is dropped add function to map forGC.
							if log.V(2) {
								log.Infof(ctx,
									"%s: queue up pending drop function GC; function: %d, version: %d",
									kv.Key, function.ID, function.Version)
							}

							if function.DropTime > 0 {
								schemaChanger.dropTime = function.DropTime
								zoneCfg, _, _, err := ZoneConfigHook(cfg, uint32(function.ID))
								if err != nil {
									log.Errorf(ctx, "zone config for functionDesc: %d, err: %+v", function.ID, err)
									return
								}
								...
								// Initialize deadline without a zone config in case it's not present.
								deadline := function.DropTime + int64(delay)
								if zoneCfg != nil {
									deadline += int64(zoneCfg.GC.TTLSeconds) * time.Second.Nanoseconds()
								} else {
									deadline += int64(defTTL) * time.Second.Nanoseconds()
						...
						if function.Dropped() || (!pendingChanges && len(function.GCMutations) > 0) {
							s.forGC[function.ID] = schemaChanger
							// Remove from schema change map if present because
							// this table has been dropped or only has other GC waiting mutations.
							delete(s.schemaChangers, function.ID)
						}
