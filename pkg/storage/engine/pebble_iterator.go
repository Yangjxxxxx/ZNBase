...
func (p *pebbleIterator) MVCCScan(
	...
	*mvccScanner = pebbleMVCCScanner{
	...
	kvData := [][]byte{bytes.Join(mvccScanner.results.finish(), nil)}
	...
	*p = pebbleIterator{
	...
	if opts.MaxTimestampHint != (hlc.Timestamp{}) {
		...
		p.options.TableFilter = func(userProps map[string]string) bool {
			tableMinTS := userProps["znbase.ts.min"]
			if len(tableMinTS) == 0 {
				if opts.WithStats {
			...
			tableMaxTS := userProps["znbase.ts.max"]
			if len(tableMaxTS) == 0 {
				if opts.WithStats {
					p.timeBoundNumSSTables++
				}
...
// TODO: zhangwg, pebble SeekReverse is < , rocksdb SeekReverse is <=
...
// Prev implements the Iterator interface.
// TODO:zhangwg, PrevKey now is same with Prev
