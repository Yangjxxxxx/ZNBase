...
// ReplicateGCQueueConcurrency is the setting of concurrency.
// We allow to adjust the parallelism of replicate gc queue process.
var ReplicateGCQueueConcurrency = settings.RegisterPositiveIntSetting(
	"kv.replicate_gc.queue_concurrency",
	"replicaGCQueue manages a queue of replicas to be considered "+
		"for garbage collections. The GC process asynchronously removes "+
		"local data for ranges that have been rebalanced away from this store. "+
...
func newReplicaGCQueue(store *Store, db *client.DB, gossip *gossip.Gossip) *replicaGCQueue {
	...
	rgcq.baseQueue = newBaseQueue(
		...
		queueConfig{
			...
			maxConcurrency:           ReplicateGCQueueConcurrency.Get(&store.ClusterSettings().SV),
			needsLease:               false,
	...
	confChanged := func() {
		rgcq.maxConcurrency = ReplicateGCQueueConcurrency.Get(&store.ClusterSettings().SV)
		rgcq.confCh <- struct{}{}
	}
	ReplicateGCQueueConcurrency.SetOnChange(&store.ClusterSettings().SV, confChanged)
	return rgcq
}
