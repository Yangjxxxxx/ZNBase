...
func (p *planner) groupBy(
	...
	for _, g := range groupByExprs {
		cols, exprs, hasStar, err := p.computeRenderAllowingStars(ctx, tree.SelectExpr{Expr: g}, types.Any, nil, r.sourceInfo, r.ivarHelper, autoGenerateRenderOutputName)
		if err != nil {
			return nil, nil, err
		}
...
func (n *groupNode) desiredAggregateOrdering(evalCtx *tree.EvalContext) sqlbase.ColumnOrdering {
	...
	f := n.funcs[0]
	impl := f.create(evalCtx, nil /* arguments */, nil)
	switch impl.(type) {
...
func (n *groupNode) SetDataStoreEngine(ds cat.DataStoreEngine) {
	n.storeEngine = ds
}

func (n *groupNode) GetDataStoreEngine() cat.DataStoreEngine {
...
func (v *extractAggregatesVisitor) VisitPre(expr tree.Expr) (recurse bool, newExpr tree.Expr) {
	...
	case *tree.FuncExpr:
		if agg := t.GetAggregateConstructor(); agg != nil {
			...
			if t.Filter != nil {
				...
				col, renderExpr, err := v.planner.computeRender(v.ctx, tree.SelectExpr{Expr: filterExpr}, types.Bool, v.preRender.sourceInfo, v.preRender.ivarHelper, autoGenerateRenderOutputName, -1)
				if err != nil {
					v.err = err
					return false, expr
				}
	...
	case *tree.IndexedVar:
		v.err = pgerror.NewErrorf(pgcode.Grouping,
