...
func (e *explainPlanNode) startExec(params runParams) error {
	if e.optimizeSubqueries {
		...
		for i := range e.subqueryPlans {
			...
			params.p.setUnlimited(e.subqueryPlans[i].plan.planNode)
		}
	}
...
func (e *explainPlanNode) Close(ctx context.Context) {
	e.plan.Close(ctx)
	for i := range e.subqueryPlans {
		e.subqueryPlans[i].plan.planNode.Close(ctx)
	}
...
func populateEntriesForObserver(
	...
	for i := range subqueryPlans {
		...
		if subqueryPlans[i].plan.planNode != nil {
			if err := walkPlan(ctx, subqueryPlans[i].plan.planNode, observer); err != nil && returnError {
				return err
			}
		} else if subqueryPlans[i].started {
			observer.expr(observeAlways, "subquery", "result", -1, subqueryPlans[i].result)
		}
		if err := observer.leaveNode("subquery", subqueryPlans[i].plan.planNode); err != nil && returnError {
			return err
		}
	}
