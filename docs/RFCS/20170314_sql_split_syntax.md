...
- RFC PR: [#14146](https://github.com/znbasedb/znbase/pull/14146)
- ZNBase Issue: [#13665](https://github.com/znbasedb/znbase/issues/13665)
...
[TestDistSQLPlanner](https://github.com/znbasedb/znbase/blob/cc5ba57/pkg/sql/distsql_physical_planner_test.go#L75).
...
### 4. `zbdb_internal.ranges` and `ranges_cached` system table ###

To facilitate testing the implementation of the new commands (as well as allow a
user to verify what the commands did), we introduce a `zbdb_internal.ranges`
...
A second table, `zbdb_internal.ranges_cached` has the same schema, but it
...
Alternatives considered for `zbdb_internal.ranges`:
