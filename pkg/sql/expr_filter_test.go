...
func TestSplitFilter(t *testing.T) {
	...
	testData := []struct {
		...
		{`(a, b, j, h) < (1, 2, 3, 4.0)`, []string{"a"}, `a <= 1`, `(a, b, j, MANUALCAST(h AS DECIMAL)) < (1, 2, 3, 4.0)`},
		{`(a, b, j, h) < (1, 2, 3, 4.0)`, []string{"a", "j"}, `a <= 1`, `(a, b, j, MANUALCAST(h AS DECIMAL)) < (1, 2, 3, 4.0)`},
		{`(a, b, j, h) < (1, 2, 3, 4.0)`, []string{"a", "b", "j"}, `(a, b, j) <= (1, 2, 3)`, `(a, b, j, MANUALCAST(h AS DECIMAL)) < (1, 2, 3, 4.0)`},
		{`(a, b, j, h) < (1, 2, 3, 4.0)`, []string{"a", "b", "h"}, `(a, b) <= (1, 2)`, `(a, b, j, MANUALCAST(h AS DECIMAL)) < (1, 2, 3, 4.0)`},
		...
		// Regression tests for #21243.
		{
			...
			[]string{"a", "b"},
			`(a, b) = (1, 2)`,
			`((a, b, j) = (1, 2, 6)) AND (MANUALCAST(h AS DECIMAL) > 8.0)`,
		},
		{
			`(((a, b) > (1, 2)) OR (((a, b) = (1, 2)) AND (j < 6))) OR (((a, b, j) = (1, 2, 6)) AND (h > 8.0))`,
			[]string{"a", "b"},
			`(((a, b) > (1, 2)) OR ((a, b) = (1, 2))) OR ((a, b) = (1, 2))`,
			`(((a, b) > (1, 2)) OR (((a, b) = (1, 2)) AND (j < 6))) OR (((a, b, j) = (1, 2, 6)) AND (MANUALCAST(h AS DECIMAL) > 8.0))`,
		},
	}
