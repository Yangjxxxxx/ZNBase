...
// znbaseToxiWrapper replaces the znbase binary. It modifies the listening port so
// that the nodes in the cluster will communicate through toxiproxy instead of
// directly.
const znbaseToxiWrapper = `#!/usr/bin/env bash
set -eu
...
// nodes. On these nodes, the znbase binary must already have been populated
...
func Toxify(ctx context.Context, c *cluster, node nodeListOption) *ToxiCluster {
	...
	c.Run(ctx, node, "mv znbase znbase.real")
	c.PutString(ctx, znbaseToxiWrapper, "./znbase", 0755, node)
	...
	for _, i := range node {
		...
		proxy, err := tc.toxClients[i].CreateProxy("znbase", fmt.Sprintf(":%d", tc.poisonedPort(port)), fmt.Sprintf("127.0.0.1:%d", port))
		if err != nil {
			c.t.Fatal(err)
		}
...
// of `./znbase sql`. This is simplistic and does not perform proper
...
func (tc *ToxiCluster) Measure(ctx context.Context, fromNode int, stmt string) time.Duration {
	...
	b, err := tc.cluster.RunWithBuffer(ctx, tc.cluster.l, tc.cluster.Node(fromNode), "time", "-p", "./znbase", "sql", "--insecure", "--port", strconv.Itoa(port), "-e", "'"+stmt+"'")
