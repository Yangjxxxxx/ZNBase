...
import (
	...
	"github.com/znbasedb/znbase/pkg/util/ring"
)
...
type RowIterator interface {
	...
	RowOpt(enc *sqlbase.EncDatumRow) (sqlbase.EncDatumRow, error)
...
// EncRowOpt for hashopt in mem
func (mc *MemRowContainer) EncRowOpt(idx int, enc *sqlbase.EncDatumRow) sqlbase.EncDatumRow {
	datums := mc.At(idx)
	for i, d := range datums {
		(*enc)[i] = sqlbase.EncDatum{Datum: d}
	}
	return *enc
}
...
func (i *memRowIterator) RowOpt(enc *sqlbase.EncDatumRow) (sqlbase.EncDatumRow, error) {
	panic("implement me")
}
...
func (i memRowFinalIterator) RowOpt(enc *sqlbase.EncDatumRow) (sqlbase.EncDatumRow, error) {
	panic("implement me")
}
...
func (f *DiskBackedRowContainer) SpillToDisk(ctx context.Context) error {
	...
	drc := MakeDiskRowContainer(f.diskMonitor, f.mrc.types, f.mrc.ordering, f.engine, f.isFile)
...
func MakeDiskBackedIndexedRowContainer(
	...
	d.DiskBackedRowContainer.Init(ordering, d.storedTypes, evalCtx, engine, memoryMonitor, diskMonitor, rowCapacity, false)
