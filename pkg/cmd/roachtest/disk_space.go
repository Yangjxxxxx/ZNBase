...
func runDiskUsage(t *test, c *cluster, duration time.Duration, tc diskUsageTestCase) {
	...
	for i := 1; i <= numNodes; i++ {
		...
		mFillBallast.Go(
			func() error {
				fillBallastCommand := []string{"./znbase", "debug", "ballast", ballastFilePath, "--size"}
...
func registerDiskUsage(r *registry) {
	...
	testCases := []diskUsageTestCase{
		...
		// nodes based on the underlying node configuration and znbase version. If the
		...
		// specific logic in znbase that uses specific %age of disk used(Mainly rebalancing).
		{
			// The workload involves both hot row update and mix rows with equal probability.
	...
	for i := range testCases {
		...
		r.Add(
			testSpec{
				...
				MinVersion: "v2.1.0", // znbase debug ballast
