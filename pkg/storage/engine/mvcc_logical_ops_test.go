...
func TestMVCCOpLogWriter(t *testing.T) {
	...
	if _, _, err := MVCCResolveWriteIntentRange(ctx, ol, nil, roachpb.LockUpdate{
	...
	if _, err := MVCCResolveWriteIntent(ctx, ol, nil, roachpb.LockUpdate{
	...
	makeOp := func(val interface{}) enginepb.MVICLogicalOp {
		var op enginepb.MVICLogicalOp
		op.MustSetValue(val)
		return op
	}
	exp := []enginepb.MVICLogicalOp{
