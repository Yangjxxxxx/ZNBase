	...
	for _, desc := range dropped {
		...
		for rowIdx, done := int64(0), false; !done; rowIdx += chunkSize {
			...
			if err := sc.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
				...
				txn.ConfigureStepping(ctx, client.SteppingEnabled)
...
func (sc *SchemaChanger) distBackfill(
	...
	g.GoCtx(func(ctx context.Context) error {
		...
		for {
			...
			if err := sc.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
				var err error
				spans, _, _, err = rowexec.GetResumeSpans(
	...
	for i, idx := range indexes {
		...
		grp.GoCtx(func(ctx context.Context) error {
			...
			row, err := evalCtx.InternalExecutor.QueryRow(ctx, "verify-inverted-idx-count", txn,
				fmt.Sprintf(
					`SELECT coalesce(sum_int(zbdb_internal.json_num_index_entries(%s)), 0) FROM [%d AS t]`,
					col, tableDesc.ID,
				),
			)
			if err != nil {
		...
		grp.GoCtx(func(ctx context.Context) error {
			...
			if idx.IsPartial() {
				row, err = newEvalCtx.InternalExecutor.QueryRow(ctx, "verify-idx-count", txn,
					fmt.Sprintf(`SELECT count(1) FROM [%d AS %s]@[%d] AS OF SYSTEM TIME %s WHERE %s`,
						tableDesc.ID, tableDesc.Name, idx.ID, readAsOf.AsOfSystemTime(), idx.PredExpr))
			} else {
				row, err = newEvalCtx.InternalExecutor.QueryRow(ctx, "verify-idx-count", txn,
			...
			case <-tableCountReady:
				if idxLen != tableRowCount && !idx.IsPartial() {
					if idx.ID == 1 {
						return pgerror.NewErrorf(
							pgcode.UniqueViolation,
							"%d entries, expected %d 违反了主键约束 %q",
					...
					return pgerror.NewErrorf(
						pgcode.UniqueViolation,
						"%d entries, expected %d 违反了唯一性约束 %q",
						idxLen, tableRowCount, idx.Name,
					)
				}
...
// runFunctionSchemaChangesInTxn runs all the schema changes immediately in a
// transaction. This is called when a CREATE TABLE is followed by
// schema changes in the same transaction. The CREATE TABLE is
// invisible to the rest of the cluster, so the schema changes
// can be executed immediately on the same version of the table.
func runFunctionSchemaChangesInTxn(
	ctx context.Context,
	txn *client.Txn,
	tc *TableCollection,
	execCfg *ExecutorConfig,
	evalCtx *tree.EvalContext,
	functionDesc *sqlbase.MutableFunctionDescriptor,
	traceKV bool,
) error {
	if len(functionDesc.DrainingNames) > 0 {
		// Reclaim all the old names. Leave the data and descriptor
		// cleanup for later.
		b := txn.NewBatch()
		for _, drain := range functionDesc.DrainingNames {
			tbKey := tableKey{parentID: drain.ParentID, name: drain.Name}.Key()
			b.Del(tbKey)
		}
		functionDesc.DrainingNames = nil
		if err := txn.Run(ctx, b); err != nil {
			return err
		}
	}
