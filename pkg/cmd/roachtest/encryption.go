...
func registerEncryption(r *registry) {
	...
	runEncryption := func(ctx context.Context, t *test, c *cluster) {
		...
		stop := func(node int) error {
			...
			if err := c.RunE(ctx, c.Node(node), "./znbase quit --insecure --host=:"+port); err != nil {
				return err
			}
		...
		testCLIGenKey := func(size int) error {
			// Generate encryption store key through `./znbase gen encryption-key -s=size aes-size.key`.
			if err := c.RunE(ctx, c.Node(nodes), fmt.Sprintf("./znbase gen encryption-key -s=%[1]d aes-%[1]d.key", size)); err != nil {
