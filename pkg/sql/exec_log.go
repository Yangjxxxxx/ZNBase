...
// I180211 07:30:48.832004 317 sql/exec_log.go:90  [client=127.0.0.1:62503,user=root,n1] 13 exec "znbase" {"ab"[53]:READ} "SELECT * FROM ab" {} 123.45 12 OK
// I180211 07:30:48.832004 317 sql/exec_log.go:90  [client=127.0.0.1:62503,user=root,n1] 13 exec "znbase" {"ab"[53]:READ} "SELECT nonexistent FROM ab" {} 0.123 12 ERROR
// Example execution log:
// I180211 07:30:48.832004 317 sql/exec_log.go:90  [client=127.0.0.1:62503,user=root,n1] 13 exec "znbase" {} "SELECT * FROM ab" {} 123.45 12 OK
// I180211 07:30:48.832004 317 sql/exec_log.go:90  [client=127.0.0.1:62503,user=root,n1] 13 exec "znbase" {} "SELECT nonexistent FROM ab" {} 0.123 0 "column \"nonexistent\" not found"
//
// Explanation of fields:
// I180211 07:30:48.832004 317 sql/exec_log.go:90  [client=127.0.0.1:62503,user=root,n1] 13 exec "znbase" {"ab"[53]:READ} "SELECT nonexistent FROM ab" {} 0.123 12 ERROR
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  \                                                                                                                                   .../
//   '- prefix generated by ZNBaseDB's standard log package. Contains:
//
// - date and time
...
func (p *planner) maybeLogStatementInternal(
	...
	auditEventsDetected := p.execCfg.AuditServer.IsAudit(p.curPlan.AST.StatAbbr())
	// for user data, check if enable table audit
	...
	age := timeutil.Now().Sub(startTime).Nanoseconds() / 1e6
	...
	if err != nil {
		...
		// if got error, append error code to error string
		...
		auditErrStr = sb.String()
	}

	// Now log!
	if auditEventsDetected {
		info := p.curPlan.auditInfo
		if info == nil {
			var id int32
			for _, ev := range p.curPlan.auditEvents {
				id = int32(ev.desc.GetID())
			}
			info = &server.AuditInfo{
				EventTime: timeutil.Now(),
				EventType: p.curPlan.AST.StatAbbr(),
				TargetInfo: &server.TargetInfo{
					TargetID: id,
					Desc:     p.curPlan.AST.StatObj()},
				Info: "User name:" + p.EvalContext().SessionData.User + " SHOW SYNTAX: " + stmtStr,
			}
		}
		info.ClientInfo = &server.ClientInfo{
			User: p.EvalContext().SessionData.User,
		}
		if p.EvalContext().SessionData.RemoteAddr != nil {
			info.ClientInfo.Client = p.EvalContext().SessionData.RemoteAddr.String()
		}
		info.AppName = appName
		info.Opt = stmtStr
		info.OptParam = plStr
		info.OptTime = age
		info.AffectedSize = rows
		info.Result = auditErrStr
		if err := p.execCfg.AuditServer.LogAudit(ctx, false, info); err != nil {
			log.Warningf(ctx, "got error when log audit info, err:%s", err)
		}
	}
	if logExecuteEnabled {
		logger := p.execCfg.ExecLogger
		logger.Logf(ctx, "%s %q %s %q %s %d %d %q",
			lbl, appName, logTrigger, stmtStr, plStr, age, rows, execErrStr)
	}
	if logV {
		// Copy to the main log.
		log.VEventf(ctx, 2, "%s %q %s %q %s %d %d %q",
			lbl, appName, logTrigger, stmtStr, plStr, age, rows, execErrStr)
	}
}
...
func (p *planner) maybeAudit(desc sqlbase.DescriptorProto, priv privilege.Kind) {
	if desc, ok := desc.(*FunctionDescriptor); ok {
