...
// initialization. Used by ICL code to install additional features.
//
...
// UpdateOnlineParams use rocksdb SetOptions()/SetDBOptions() API to
...
// representation is unfortunately duplicated in engine and engineicl. If this
...
//DBCheckForKeyCollisions在lockstep中运行迭代器，在
//第一个键冲突，其中冲突指的是
//相同的用户key，并且具有不同的时间戳或值。
//当碰撞key的最新版本是
//从MVCC删除现有数据中的逻辑删除。如果
//SST key大于或等于墓碑的时间戳，则
//不被视为冲突，我们从下一个键继续迭代
//现有数据
...
typedef struct {
  DBStatus status;
  DBSlice data;// roachpb.VecResult的序列化
  DBSlice intents;
  DBTimestamp write_too_old_timestamp;
  DBTimestamp uncertainty_timestamp;
  DBSlice resume_key;
...
DBVecResults VecScan(DBEngine *db, DBSlice start, DBSlice end, DBTimestamp timestamp,
                     DBSlice pushdown,// roachpb.PushDownExpr的序列化
                     int64_t max_keys, DBTxn txn, bool inconsistent, bool reverse,
                     bool tombstones, bool ignore_sequence, bool fail_on_more_recent);
...
typedef struct {
  // 数据表的schema信息，
  int32_t col_count;
  uint32_t *col_ids; //TODO 释放问题
  uint32_t *col_types;
  int32_t primary_count;
  uint32_t *primary_cols;
  int32_t secondary_count;
  uint32_t *secondary_cols;

  int32_t required_count;
  uint32_t *required_cols;
...
typedef struct {
  ...
  int64_t base_level_file_count;
  int64_t l0_bytes;
  int64_t base_level_bytes;
  int64_t l0_compaction_score;
  int64_t base_level_compaction_score;
  //增加写失速指标
  int64_t stall_l0_slowdown_micros;
  int64_t stall_memtable_compaction_micros;
  int64_t stall_l0_num_files_micros;
  int64_t stall_micros;
  int64_t stall_l0_slowdown_count;
  int64_t stall_memtable_compaction_count;
  int64_t stall_l0_num_files_count;
  int64_t write_stall;
} DBStatsResult;
...
typedef struct {
  ...
  // encryption status (ICL only).
  // This is a serialized enginepbicl/stats.proto:EncryptionStatus
...
// ingestion为true则创建的sst启用前缀bloom过滤器
