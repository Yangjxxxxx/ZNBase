...
const (
	...
	// administrative interface to the znbase cluster.
	...
	// ResultSuccess stands for success.
	ResultSuccess = serverpb.StoreStateResponse_ResponseState(1)

	// ResultFail stands for fail.
	ResultFail = serverpb.StoreStateResponse_ResponseState(0)
)
...
var errAdminAPIError = status.Errorf(codes.Internal, "An internal server error "+
	"has occurred. Please check your ZNBaseDB logs for more details.")
...
var noteworthyAdminMemoryUsageBytes = envutil.EnvOrDefaultInt64("ZNBASE_NOTEWORTHY_ADMIN_MEMORY_USAGE", 100*1024)
...
func (s *adminServer) Databases(
	...
	rows, _ /* cols */, err := s.server.internalExecutor.QueryWithUser(
		ctx, "admi-show-db", nil /* txn */, userName, "SHOW DATABASES",
	)
	if err != nil {
...
func (s *adminServer) DatabaseDetails(
	...
	privs, err := s.hasUserPrivileges(ctx, escDBName, "", "")
	if err != nil {
		return nil, err
	}
	if !privs {
		return nil, fmt.Errorf("user-privileges: user does not have privilege on database %s", escDBName)
	}
	userName, err := getUserName(ctx)
	if err != nil {
		return nil, err
	...
	var resp serverpb.DatabaseDetailsResponse
	{
		...
		for _, row := range rows {
			...
			if sql.CheckVirtualSchema(schemaName) {
				continue
			}
	...
	{
		path, err := s.queryDescriptorIDPath(ctx, s.getUser(req), []string{req.Database})
		if err != nil {
			return nil, s.serverError(err)
		}
		resp.DescriptorID = int64(path[1])

		id, zone, zoneExists, err := s.queryZonePath(ctx, s.getUser(req), path)
		if err != nil {
			return nil, s.serverError(err)
		}
...
func (s *adminServer) TableDetails(
	...
	privs, err := s.hasUserPrivileges(ctx, escDBName, req.Schema, escTableName)
	if err != nil {
		return nil, err
	}
	if !privs {
		return nil, fmt.Errorf("user-privileges: user does not have privilege on relation %s.%s.%s", escDBName, req.Schema, escTableName)
	}
	...
	// 查询列ID (cli 可以查询到结果，程序中没有结果)
	//id_rows, err := s.server.internalExecutor.Query(
	//	ctx, "admin-show-columns",
	//	nil /* txn */,
	//	fmt.Sprintf("select column_name , column_id  "+
	//		"from bidb_internal.table_columns as columns,bidb_internal.tables as tables "+
	//		"where tables.database_name='%s' and tables.name='%s' "+
	//		"and columns .descriptor_id =tables.table_id", escDBName, escTableName),
	//)
	//if s.isNotFoundError(err) {
	//	return nil, status.Errorf(codes.NotFound, "%s", err)
	//}
	...
	//	getColumnID(id_rows, nil, col.Name, &col.Id)
	//}
	...
	// for our API.
	{
		...
		if req.Schema != "" {
			tableDesc, err = s.getTableDescriptor(escDBName, req.Schema, escTableName)
			if err != nil {
				return nil, s.serverError(err)
			}
		}
		...
		for _, row := range rows {
			...
			if tableDesc != nil {
				fetchColumnID(tableDesc, col.Name, &col.Id, &col.ColumnType)
			}
	...
	}
	{
		...
		scanner := makeResultScanner(cols)
		grantMap := map[string]map[string]*serverpb.TableDetailsResponse_Grant{}
		for _, row := range rows {
			// Marshal grant, splitting comma-separated privileges into a proper slice.
			var grantee string
			var grantor string
			var privileges string
			if err := scanner.Scan(row, userCol, &grantee); err != nil {
				return nil, err
			}
			if err := scanner.Scan(row, grantorCol, &grantor); err != nil {
				return nil, err
			}
			if err := scanner.Scan(row, privilegesCol, &privileges); err != nil {
				return nil, err
			}
			if v, ok := grantMap[grantee]; ok {
				if vv, ok := v[grantor]; ok {
					vv.Privileges = append(vv.Privileges, privileges)
				} else {
					v[grantor] = &serverpb.TableDetailsResponse_Grant{
						User:       grantee,
						Grantor:    grantor,
				...
				grantMap[grantee] = make(map[string]*serverpb.TableDetailsResponse_Grant)
				grantMap[grantee][grantor] = &serverpb.TableDetailsResponse_Grant{
					User:       grantee,
					Grantor:    grantor,
					Privileges: []string{privileges},
				}
		...
		for _, v := range grantMap {
			for _, vv := range v {
				resp.Grants = append(resp.Grants, *vv)
			}
		}
	}
	...
	{
		path, err := s.queryDescriptorIDPath(ctx, s.getUser(req), []string{req.Database, req.Table})
		if err != nil {
			return nil, s.serverError(err)
		}
		tableID = path[2]
...
// hasUserPrivileges verify the user whether has the database or table privs.
func (s *adminServer) hasUserPrivileges(
	ctx context.Context, databaseName string, schemaName string, tableName string,
) (bool, error) {
	privs := false
	sessionUser, err := getUserName(ctx)
	if sessionUser == "root" {
		return true, nil
	}
	if tableName != "" {
		if schemaName != "" {
			rows, _ /* cols */, err := s.server.internalExecutor.QueryWithUser(
				ctx, "user-privileges", nil /* txn */, sessionUser,
				"SHOW GRANTS ON "+databaseName+"."+schemaName+"."+tableName+" for "+sessionUser,
			)
			if err != nil {
				return false, err
			}
			...
			rows, _ /* cols */, err := s.server.internalExecutor.QueryWithUser(
				ctx, "user-privileges", nil /* txn */, sessionUser,
				"SHOW GRANTS ON "+databaseName+"."+tableName+" for "+sessionUser,
			)
			if err != nil {
				return false, err
			}
	...
	rows, _ /* cols */, err := s.server.internalExecutor.QueryWithUser(
		ctx, "user-privileges", nil /* txn */, sessionUser,
		"SHOW GRANTS ON database "+databaseName+" for "+sessionUser,
	)
	if err != nil {
		return false, err
	}
...
address	5
*/
func getColumnID(rows []tree.Datums, cols sqlbase.ResultColumns, colName string, colID *int32) {
	for i := range rows {
		if name := rows[i][0].(*tree.DString); name != nil && name.String() == colName {
			*colID = int32(*rows[i][1].(*tree.DInt))
			break
		}
	}
	*colID = 0
}
func fetchColumnID(table *sqlbase.TableDescriptor, colName string, colID *int32, colType *int32) {
	for _, col := range table.Columns {
		if colName == col.Name {
			*colID = int32(col.ID)
			*colType = int32(col.Type.SemanticType)
			return
		}
	}
	*colID = 0
}
func (s *adminServer) getTableDescriptor(
	...
	tableNameKey := sqlbase.MakeNameMetadataKey(scDesc.ID, table)
	...
	tbDesc := &sqlbase.Descriptor{}
	if err := kvDB.GetProto(context.TODO(), descKey, tbDesc); err != nil || (*tbDesc == sqlbase.Descriptor{}) {
		log.Fatalf(context.TODO(), "proto with id %d missing. err: %v", gr.ValueInt(), err)
	}
	return tbDesc.GetTable(), nil
}
...
func (s *adminServer) TableStats(
	...
	privs, err := s.hasUserPrivileges(ctx, req.Database, "", req.Table)
	if err != nil {
		return nil, err
	}
	if !privs {
		return nil, fmt.Errorf("user-privileges: user does not have privilege on relation %s.%s", req.Database, req.Table)
	}
...
func (s *adminServer) Cluster(
	...
	enterpriseEnabled := base.CheckCommercialFeatureEnabled(s.server.st, clusterID, organization, "BACKUP") == nil
...
func (s *adminServer) QueryPlan(
	...
	// explain query.
	stmts, err := parser.Parse(req.Query, false)
	if err != nil {
		return nil, s.serverError(err)
	}
	if len(stmts) > 1 {
...
func (s *adminServer) Drain(req *serverpb.DrainRequest, stream serverpb.Admin_DrainServer) error {
	...
	al := &server.AuditInfo{
		...
		EventType:  string(sql.EventLogNodeQuit),
		ClientInfo: ci,
		TargetInfo: &server.TargetInfo{TargetID: int32(s.server.node.Descriptor.NodeID)},
		Opt:        string(sql.EventLogNodeQuit),
	}

	nowOn, err := s.server.Drain(ctx, on)
	if err != nil {
		al.Info = err
		s.AuditLog(ctx, al, start, err)
		return err
	}
	...
	if !req.Shutdown {
		al.Info = &infos.NodeInfo{
			NodeID:    int32(s.server.node.Descriptor.NodeID),
			LastUp:    s.server.node.lastUp,
			ClusterID: s.server.node.clusterID.Get(),
		}
		s.AuditLog(ctx, al, start, err)
		return nil
	}
	...
	case <-time.After(10 * time.Second):
		...
		// https://github.com/znbasedb/znbase/pull/31692
		//
...
func (s *adminServer) AuditLog(
	ctx context.Context, auditLog *server.AuditInfo, start time.Time, err error,
) {
	auditLog.OptTime = timeutil.SubTimes(timeutil.Now(), start)
	auditLog.Result = util.AuditResult(err)
	if err := s.server.auditServer.LogAudit(ctx, false, auditLog); err != nil {
		log.Warningf(ctx, "unable to log %s event, error:%s", auditLog.EventType, err)
	}
}
...
// UpdateStoreState gets nodeID from gossip to update the state from StoreStateRequest and return StoreStateResponse.
func (s *adminServer) UpdateStoreState(
	ctx context.Context, req *serverpb.StoreStateRequest,
) (*serverpb.StoreStateResponse, error) {
	nodeID, err := s.server.gossip.GetNodeIDForStoreID(req.StoreID)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "failed to lookup store %d: %s", req.StoreID, err)
	}
	admin, err := s.dialNode(ctx, nodeID)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "failed to create client %v", err)
	}
	return admin.StoreState(ctx, req)
}

// StoreState updates the state from StoreStateRequest and return StoreStateResponse.
func (s *adminServer) StoreState(
	ctx context.Context, req *serverpb.StoreStateRequest,
) (*serverpb.StoreStateResponse, error) {
	node := s.server.node
	var store *storage.Store
	var err error
	if s.server.node.stores.HasStore(req.StoreID) {
		if store, err = node.stores.GetStore(req.StoreID); err != nil {
			return &serverpb.StoreStateResponse{Result: ResultFail, Original: roachpb.UNKNOWN, New: roachpb.UNKNOWN}, err
		}
		orgnlState := store.Engine().State()
		store.Engine().UpdateState(req.State)
		return &serverpb.StoreStateResponse{Result: ResultSuccess, Original: orgnlState, New: req.State}, nil
	}
	return &serverpb.StoreStateResponse{Result: ResultFail, Original: roachpb.UNKNOWN, New: roachpb.UNKNOWN}, status.Errorf(codes.InvalidArgument, "The store:store id=%d is not in the current host", req.StoreID)
...
func (s *adminServer) DataDistribution(
	...
	// This relies on zbdb_internal.tables returning data even for newly added tables
	...
	tablesQuery := `SELECT name, table_id, database_name, drop_time FROM "".zbdb_internal.tables WHERE schema_name = 'public'`
...
func (s *adminServer) queryNamespaceID(
	ctx context.Context, userName string, parentID sqlbase.ID, name string, index int,
) (sqlbase.ID, error) {
	var query string
	if index == 0 || parentID == sqlbase.SystemDB.ID {
		query = `SELECT id FROM system.namespace WHERE "parentID" = $1 AND name = $2`
	} else {
		query = `SELECT tb.id FROM system.namespace sc JOIN system.namespace tb ON (sc.id=tb."parentID") WHERE sc."parentID" = $1 AND tb.name = $2`
	}
	rows, _ /* cols */, err := s.server.internalExecutor.QueryWithUser(
...
func (s *adminServer) queryDescriptorIDPath(
	...
	for i, name := range names {
		id, err := s.queryNamespaceID(ctx, userName, path[len(path)-1], name, i)
		if err != nil {
			return nil, err
		}
		path = append(path, id)
...
func (s *adminServer) TableRanges(
	ctx context.Context, req *serverpb.TableRangesRequest,
) (*serverpb.TableRangesResponse, error) {
	ctx = s.server.AnnotateCtx(ctx)
	resp := serverpb.TableRangesResponse{}
	var tableID sqlbase.ID
	privs, err := s.hasUserPrivileges(ctx, req.Database, "", req.Table)
	if err != nil {
		return nil, err
	...
	{
		tableSpan := generateTableSpan(tableID)
		ranges, serr := ScanMetaKVs(ctx, s.server.db.NewTxn(ctx, "ScanTableRanges"), roachpb.Span{
			Key:    tableSpan.Key,
			EndKey: tableSpan.EndKey,
		})

		if nil != serr {
			return nil, s.serverError(serr)
		}
		if len(ranges) == 0 {
			return nil, s.serverError(errors.New("There is no ranges of table " + req.Table))
		}

		info := serverpb.TableRangeInfo{}
		...
		for _, r := range ranges {
			...
			//status range的方法,比较低效
			req := serverpb.RangeRequest{}
			req.RangeId = int64(desc.RangeID)
			rangeResp, err := s.server.status.Range(ctx, &req)
			if err != nil {
				return nil, s.serverError(err)
			}

			for _, nodeResp := range rangeResp.ResponsesByNodeID {
				if len(nodeResp.Infos) > 0 {
					info.Lead = nodeResp.Infos[0].RaftState.Lead
				}
			}
...
func (s *adminServer) RawBatchLoad(
	ctx context.Context, req *serverpb.RawBatchLoadRequest,
) (*serverpb.RawBatchLoadResponse, error) {
	privs, err := s.hasUserPrivileges(ctx, req.Database, "public", req.Table)
	if err != nil {
		return nil, err
	}
	if !privs {
		return nil, fmt.Errorf("user-privileges: user does not have privilege on relation %s.public.%s", req.Database, req.Table)
	}

	tableDesc, err := s.getTableDescriptor(req.Database, "public", req.Table)
	if err != nil {
		return nil, s.serverError(err)
	}

	return loadutils.RawBatchLoadCallbackFunc(ctx, req, s.server.st, s.server.db,
		s.server.distSender.RangeDescriptorCache(), s.server.NodeID(), tableDesc, s.server.cfg.HistogramWindowInterval())

	// 手工执行TestAdminLoadData时打开,并屏蔽上面的return语句,否则有空指针/循环引用问题
	//return load.BatchLoadFromAdmin(ctx, req, s.server.st, s.server.db,
	//	s.server.distSender.RangeDescriptorCache(), s.server.NodeID(), tableDesc, s.server.cfg.HistogramWindowInterval())
}
...
// getUserName is used to get the user name.
func getUserName(ctx context.Context) (string, error) {
	userName, err := userFromContext(ctx)
	if err != nil {
		return "", err
	}
	if userName == "authentic_user" {
