...
import (
	...
	"github.com/znbasedb/znbase/pkg/internal/rsg"
...
func TestFormatExpr2(t *testing.T) {
	...
	testData := []struct {
		...
		{tree.NewDOidWithName(tree.DInt(10), coltypes.RegClass, "foo"),
			tree.FmtParsable, `zbdb_internal.create_REGCLASS(10,'foo'):::REGCLASS`},
		{tree.NewDOidWithName(tree.DInt(10), coltypes.RegProc, "foo"),
			tree.FmtParsable, `zbdb_internal.create_REGPROC(10,'foo'):::REGPROC`},
		{tree.NewDOidWithName(tree.DInt(10), coltypes.RegType, "foo"),
			tree.FmtParsable, `zbdb_internal.create_REGTYPE(10,'foo'):::REGTYPE`},
		{tree.NewDOidWithName(tree.DInt(10), coltypes.RegNamespace, "foo"),
			tree.FmtParsable, `zbdb_internal.create_REGNAMESPACE(10,'foo'):::REGNAMESPACE`},
	...
	for i, test := range testData {
		t.Run(fmt.Sprintf("%d %s", i, test.expr), func(t *testing.T) {
			ctx := tree.MakeSemaContext()
			typeChecked, err := tree.TypeCheck(test.expr, &ctx, types.Any, false)
			if err != nil {
				t.Fatal(err)
			}
			exprStr := tree.AsStringWithFlags(typeChecked, test.f)
...
func BenchmarkFormatRandomStatements(b *testing.B) {
	...
	for i := 0; i < 1000; {
		...
		stmt, err := parser.ParseOne(rdm, false)
		if err != nil {
	...
	b.Run("parse", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			for _, sql := range strs {
				_, err := parser.ParseOne(sql, false)
				if err != nil {
					b.Fatal(err)
				}
			}
