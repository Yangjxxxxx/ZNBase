...
0143`,
...

		{
			...
			create: `s STRING`,
			with:   `WITH fields_escaped_by = '\',nulldist`,
			typ:    "CSV",
			data:   "\\N\n\\\\N\nNULL",
			query: map[string][][]string{
				`SELECT s from t`: {{"NULL"}, {`\N`}, {"NULL"}},
			},
		},
		{
...
\.
		...
		},
		{
			...
			query: map[string][][]string{
				`SHOW TABLES`:        {{"a", "root"}, {"b", "root"}},
				`SELECT i, k FROM a`: {{"2", "2"}},
				`SELECT j FROM b`:    {{"2"}},

				`SELECT dependson_name
				FROM zbdb_internal.backward_dependencies ORDER BY dependson_name`: {
					{"a_i_fkey"},
					{"a_k_fkey"},
					{"b_j_fkey"},
				},
	...
	FAMILY "primary" (j)
		...
		},
		{
			...
			query: map[string][][]string{
				...
				`SELECT dependson_name FROM zbdb_internal.backward_dependencies`: {},
				`SHOW CONSTRAINTS FROM weather`:                                  {},
			},
		},
		{
		...
		},
		{
			...
			query: map[string][][]string{
				...
				`SHOW CREATE SEQUENCE i_seq`: {{"i_seq", "CREATE SEQUENCE i_seq MINVALUE 1 MAXVALUE 9223372036854775807 INCREMENT 1 START WITH 1"}},
			},
		},
		//{
	...
	for _, direct := range []bool{false, true} {
		...
		for i, tc := range tests {
			if direct {
				if tc.with == "" {
					tc.with = "WITH direct_ingestion,save_rejected"
				} else {
					tc.with += ", direct_ingestion,save_rejected"
				}
			}
			t.Run(fmt.Sprintf("%s: %s direct=%v", tc.typ, tc.name, direct), func(t *testing.T) {
				...
				var q string
				if tc.create != "" {
					q = fmt.Sprintf(`LOAD TABLE t (%s) %s DATA ($1) %s`, tc.create, tc.typ, tc.with)
				} else {
					q = fmt.Sprintf(`LOAD %s ($1) %s`, tc.typ, tc.with)
				}
				//t.Log(q)
				//dataString = tc.data
	...
	t.Run("mysqlout multiple", func(t *testing.T) {
		...
		sqlDB.Exec(t, `LOAD TABLE t (s STRING) MYSQLOUTFILE DATA ($1, $1)`, srv.URL)
		sqlDB.CheckQueryResults(t, `SELECT * FROM t`, [][]string{{"1"}, {"1"}})
	})
	//load 权限测试
	t.Run("load-with-privs", func(t *testing.T) {
		sqlDB.Exec(t, `USE defaultdb`)
		sqlDB.Exec(t, `CREATE USER foo`)
		sqlDB.Exec(t, `GRANT ALL ON SCHEMA public  TO foo`)
		//导入mysqldump文件
		sqlDB.Exec(t, `LOAD TABLE t (s STRING) MYSQLOUTFILE DATA ($1)`, srv.URL)
		//查验权限是否正确
		res := sqlDB.QueryStr(t, `SHOW COLUMNS FROM t`)
		expectedRows := [][]string{
			{`s`, `STRING`, `true`, `NULL`, ``, `{}`, `false`},
			{`rowid`, `INT`, `false`, `unique_rowid()`, ``, `{primary}`, `true`},
		}
		if !reflect.DeepEqual(expectedRows, res) {
			t.Fatalf("expected:\n%v\ngot:\n%v", expectedRows, res)
		}
	})
}
...
func makeCSVData(
...
) (files []string, filesWithOpts []string, filesWithDups []string, allfilepath []string) {
	if err := os.Mkdir(filepath.Join(in, "csv"), 0777); err != nil {
		t.Fatal(err)
	}
	if err := os.Mkdir(filepath.Join(in, "onlycsv"), 0777); err != nil {
		t.Fatal(err)
	}
	allfilepath = append(allfilepath, "onlycsv")
	...
	for fn := 0; fn < numFiles; fn++ {
		...
		onlycsvpath := filepath.Join("onlycsv", fmt.Sprintf("data-%d.csv", fn))
		fonlycsv, err := os.Create(filepath.Join(in, onlycsvpath))
		if err != nil {
			t.Fatal(err)
		}
		...
		for i := 0; i < rowsPerFile; i++ {
			...
			if _, err := fmt.Fprintf(fonlycsv, "%d,%c\n", x, 'A'+x%26); err != nil {
				t.Fatal(err)
			}
	...
	return files, filesWithOpts, filesWithDups, allfilepath
}
...
func TestImportCSVStmt(t *testing.T) {
	...
	sqlDB.Exec(t, `SET CLUSTER SETTING kv.load.batch_size = '10KB'`)
	...
	files, filesWithOpts, dups, allfilepath := makeCSVData(t, dir, numFiles, rowsPerFile)
	...
	allfilepath = nodelocalPrefix(allfilepath)
	...
	for i, tc := range []struct {
		...
		{
			"schema-in-file",
			`LOAD TABLE t CREATE USING $1 CSV DATA (%s)`,
			schema,
			files,
			``,
			"",
		},
		{
			...
			`LOAD TABLE csv1.t CREATE USING $1 CSV DATA (%s)`,
			schema,
			files,
			``,
			"",
		...
		{
			"schema-in-query",
			`LOAD TABLE t (a INT8 PRIMARY KEY, b STRING, INDEX (b), INDEX (a, b)) CSV DATA (%s)`,
			nil,
			files,
			``,
			"",
		},
		{
			...
			`LOAD TABLE t (a INT8 PRIMARY KEY, b STRING, INDEX (b), INDEX (a, b)) CSV DATA (%s) WITH delimiter = '|', comment = '#', nullif='', skip = '2'`,
			nil,
			filesWithOpts,
		...
		},
		{
			...
			`LOAD TABLE t CREATE USING $1 CSV DATA (%s) WITH sstsize = '10K'`,
			schema,
			files,
			` WITH sstsize = '10K'`,
			"",
		},
		{
			...
			`LOAD TABLE t (a INT8 PRIMARY KEY, b STRING, INDEX (b), INDEX (a, b)) CSV DATA (%s) WITH delimiter = '|', comment = '#', nullif='', skip = '2', transform = $1`,
			nil,
			filesWithOpts,
		...
		{
			"allfiles",
			`LOAD TABLE t (a INT8 PRIMARY KEY, b STRING, INDEX (b), INDEX (a, b)) CSV DATA (%s) WITH allfiles`,
			nil,
			allfilepath,
			` WITH allfiles`,
			"",
		},
		{
			...
			`LOAD TABLE t CREATE USING $1 CSV DATA (%s) WITH decompress = 'auto'`,
			schema,
			files,
		...
		},
		{
			...
			`LOAD TABLE t CREATE USING $1 CSV DATA (%s) WITH decompress = 'none'`,
			schema,
			files,
		...
		},
		{
			...
			`LOAD TABLE t CREATE USING $1 CSV DATA (%s) WITH decompress = 'gzip'`,
			schema,
			gzip,
		...
		},
		{
			...
			`LOAD TABLE t CREATE USING $1 CSV DATA (%s) WITH decompress = 'bzip'`,
			schema,
			bzip,
		...
		{
			"bad-opt-name",
			`LOAD TABLE t (a INT8 PRIMARY KEY, b STRING, INDEX (b), INDEX (a, b)) CSV DATA (%s) WITH foo = 'bar'`,
			nil,
			files,
			``,
			"invalid option \"foo\"",
		...
		{
			"bad-opt-no-arg",
			`LOAD TABLE t (a INT8 PRIMARY KEY, b STRING, INDEX (b), INDEX (a, b)) CSV DATA (%s) WITH transform`,
			nil,
			files,
			``,
		...
		{
			"bad-computed-column",
			`LOAD TABLE t (a INT8 PRIMARY KEY, b STRING AS ('hello') STORED, INDEX (b), INDEX (a, b)) CSV DATA (%s) WITH skip = '2', transform = $1`,
			nil,
			filesWithOpts,
			``,
		...
		{
			"primary-key-dup",
			`LOAD TABLE t CREATE USING $1 CSV DATA (%s) WITH transform = $2`,
			schema,
			dups,
			``,
		...
		{
			"no-database",
			`LOAD TABLE nonexistent.t CREATE USING $1 CSV DATA (%s)`,
			schema,
			files,
			``,
			`pq: Unable to resolve the current import target, please check whether the target schema or database exists: "nonexistent.t"`,
		},
		{
			"into-db-fails",
			`LOAD TABLE t CREATE USING $1 CSV DATA (%s) WITH into_db = 'test'`,
			schema,
			files,
			``,
			`pq: into_db only can be used while load schema`,
		},
		{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if hasTransform {
				...
				sqlDB.Exec(t, fmt.Sprintf(`CREATE DATABASE csv`))

				sqlDB.QueryRow(
					t, `LOAD TABLE csv.t FROM $1 WITH into_db = $2`, backupPath, intodb,
				).Scan(
					&unused, &unused, &unused, &restored.rows, &restored.idx, &restored.sys, &restored.bytes,
				)
	...
	//	sqlDB.Exec(t, fmt.Sprintf(`LOAD TABLE pk.t (a INT8, b STRING) CSV DATA (%s)`, strings.Join(files, ", ")))
	...
	// Verify a failed LOAD won't prevent a second LOAD.
	t.Run("checkpoint-leftover", func(t *testing.T) {
		...
		sqlDB.ExpectErr(
			...
			fmt.Sprintf(`LOAD TABLE t (a INT8 PRIMARY KEY) CSV DATA (%s)`, files[0]),
		)

		// Specify wrong table name; still shouldn't leave behind a checkpoint file.
		sqlDB.ExpectErr(
			t, `file specifies a schema for table t`,
			fmt.Sprintf(`LOAD TABLE bad CREATE USING $1 CSV DATA (%s)`, files[0]), schema[0],
		)

		// Expect it to succeed with correct columns.
		sqlDB.Exec(t, fmt.Sprintf(`LOAD TABLE t (a INT8 PRIMARY KEY, b STRING) CSV DATA (%s)`, files[0]))
		...
		sqlDB.ExpectErr(
			...
			fmt.Sprintf(`LOAD TABLE t (a INT8 PRIMARY KEY, b STRING) CSV DATA (%s)`, files[0]),
		)
	})
	...
	t.Run("allow-default", func(t *testing.T) {
		...
		//	sqlDB.ExpectErr(
		//		t, `row 1: parse "a" as SERIAL8: could not parse ""`,
		//		query, srv.URL,
		//	)
		//	sqlDB.ExpectErr(
...
func BenchmarkImport(b *testing.B) {
	...
	files, _, _, _ := makeCSVData(b, dir, numFiles, b.N*100)
	files = nodelocalPrefix(files)
	...
	sqlDB.Exec(b,
		fmt.Sprintf(
			`LOAD TABLE t (a INT8 PRIMARY KEY, b STRING, INDEX (b), INDEX (a, b))
...
func BenchmarkConvertRecord(b *testing.B) {
	...
	tableDesc, err := MakeSimpleTableDescriptor(ctx, st, create, sqlbase.ID(100), sqlbase.ID(100), NoFKs, 1, nil, sqlbase.AdminRole)
	if err != nil {
		b.Fatal(err)
	}
...
func TestImportControlJob(t *testing.T) {
	...
	makeSrv := func() *httptest.Server {
		return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			if r.Method == "GET" {
				...
				// CANCEL JOB is issued, the second stage of the LOAD (the shuffle,
	...
	t.Run("cancel", func(t *testing.T) {
		...
		query := fmt.Sprintf(`LOAD TABLE cancelimport.t (i INT8 PRIMARY KEY) CSV DATA (%s)`, csvURLs)
	...
	t.Run("pause", func(t *testing.T) {
		// Test that LOAD can be paused and resumed. This test also attempts to
		...
		csvURLs := strings.Join(urls, ", ")
		query := fmt.Sprintf(`LOAD TABLE pauseimport.t (i INT8 PRIMARY KEY) CSV DATA (%s) WITH sstsize = '50B'`, csvURLs)
...
// TestImportWorkerFailure tests that LOAD can restart after the failure
// of a worker node.
func TestImportWorkerFailure(t *testing.T) {
	...
	csvURLs := strings.Join(urls, ", ")
	query := fmt.Sprintf(`LOAD TABLE t (i INT8 PRIMARY KEY) CSV DATA (%s) WITH sstsize = '1B'`, csvURLs)
...
// during LOAD correctly resumes after the node executing the job
...
// iterations with differing constants (rows, sstsize, kv.load.batch_size)
...
func TestImportLivenessWithRestart(t *testing.T) {
	...
	sqlDB.Exec(t, `SET CLUSTER SETTING cloudsink.timeout = '3s'`)

	sqlDB.Exec(t, `SET CLUSTER SETTING kv.load.batch_size = '300B'`)
	...
	const query = `LOAD TABLE liveness.t (i INT8 PRIMARY KEY) CSV DATA ($1) WITH sstsize = '500B'`

	// Start an LOAD and wait until it's done one addsstable.
	allowResponse = make(chan struct{})
	errCh := make(chan error)
	go func() {
	...
	if !tableSpan.EqualValue(spans[0].SpanProgress[0]) {
...
// transition during LOAD doesn't cancel the job, but allows the
...
func TestImportMysql(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{`read data only`, expectSimple, `LOAD TABLE simple (i INT8 PRIMARY KEY, s text, b bytea) MYSQLDUMP DATA ($1)`, simple},
		{`single table dump`, expectSimple, `LOAD TABLE simple FROM MYSQLDUMP ($1)`, simple},
		{`second table dump`, expectSecond, `LOAD TABLE second FROM MYSQLDUMP ($1) WITH skip_foreign_keys`, second},
		{`simple from multi`, expectSimple, `LOAD TABLE simple FROM MYSQLDUMP ($1)`, multitable},
		{`second from multi`, expectSecond, `LOAD TABLE second FROM MYSQLDUMP ($1) WITH skip_foreign_keys`, multitable},
		{`all from multi`, expectAll, `LOAD MYSQLDUMP ($1)`, multitable},
		{`all from multi gzip`, expectAll, `LOAD MYSQLDUMP ($1)`, multitableGz},
		{`all from multi bzip`, expectAll, `LOAD MYSQLDUMP ($1)`, multitableBz},
	} {
...
func TestImportMysqlOutfile(t *testing.T) {
	...
	for i, cfg := range configs {
		t.Run(cfg.name, func(t *testing.T) {
			...
			cmd := fmt.Sprintf(`LOAD TABLE test%d (i INT8 PRIMARY KEY, s text, b bytea) MYSQLOUTFILE DATA ($1)`, i)
...
func TestImportPgCopy(t *testing.T) {
	...
	for i, cfg := range configs {
		t.Run(cfg.name, func(t *testing.T) {
			...
			cmd := fmt.Sprintf(`LOAD TABLE test%d (i INT8 PRIMARY KEY, s text, b bytea) PGCOPY DATA ($1)`, i)
			opts = append(opts, fmt.Sprintf("nodelocal://%s", strings.TrimPrefix(cfg.filename, baseDir)))
...
func TestImportPgDump(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{`single table dump`, expectSimple, `LOAD TABLE simple FROM PGDUMP ($1)`, simple},
		{`second table dump`, expectSecond, `LOAD TABLE second FROM PGDUMP ($1)`, second},
		{`simple from multi`, expectSimple, `LOAD TABLE simple FROM PGDUMP ($1)`, multitable},
		{`second from multi`, expectSecond, `LOAD TABLE second FROM PGDUMP ($1)`, multitable},
		{`all from multi`, expectAll, `LOAD PGDUMP ($1)`, multitable},
	} {
		t.Run(c.name, func(t *testing.T) {
	...
	a INTEGER NULL DEFAULT nextval('public.a_seq'),
	b INTEGER NULL,
	FAMILY "primary" (a, b, rowid)
)`,
				}})
				sqlDB.CheckQueryResults(t, `SHOW CREATE SEQUENCE a_seq`, [][]string{{
					"a_seq", `CREATE SEQUENCE a_seq MINVALUE 1 MAXVALUE 9223372036854775807 INCREMENT 1 START WITH 1`,
				}})
...
func TestImportCockroachDump(t *testing.T) {
	...
	sqlDB.Exec(t, "LOAD PGDUMP ($1)", "nodelocal:///znbasedbdump/dump.sql")
...
func TestCreateStatsAfterImport(t *testing.T) {
	...
	//todo 石梦飞
	t.Skip("todo 石梦飞 import success时没考虑osc问题造成table id找不到")
