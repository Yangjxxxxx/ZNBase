...
import (
	...
	"github.com/znbasedb/znbase/pkg/util/causer"
...
func (r *Replica) AdminSplit(
	ctx context.Context, args roachpb.AdminSplitRequest, reason string, backFill bool,
	...
	for retryable := retry.StartWithCtx(ctx, retryOpts); retryable.Next(); {
		...
		reply, lastErr = r.adminSplitWithDescriptor(ctx, args, r.Desc(), true /* delayable */, reason, backFill)
...
func (r *Replica) adminSplitWithDescriptor(
	...
	if err := r.store.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		if err := txn.SetDebugName(splitTxnName); err != nil {
			return err
		}
...
func (r *Replica) AdminMerge(
	ctx context.Context, args roachpb.AdminMergeRequest, reason string, backFill bool,
	...
	runMergeTxn := func(txn *client.Txn) error {
		...
		if err := txn.SetDebugName(mergeTxnName); err != nil {
			return err
		}
...
func (r *Replica) ChangeReplicas(
	...
	return r.changeReplicas(ctx, changeType, target, desc, SnapshotRequest_REBALANCE, reason, details, backFill)
}
...
func (r *Replica) changeReplicas(
	...
	if err := r.store.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		if err := txn.SetDebugName(replicaChangeTxnName); err != nil {
			return err
		}
...
func (s *Store) AdminRelocateRange(
	...
	locateSpace, err := sysCfg.GetLocateSpace(rangeDesc.StartKey)
	if err != nil {
		return err
	}
	ctx = setLocateSpace(ctx, locateSpace)
	...
	for len(addTargets) > 0 || len(removeTargets) > 0 {
		...
		if len(addTargets) > 0 && len(addTargets) >= len(removeTargets) {
			...
			newDesc, err := s.DB().AdminChangeReplicas(ctx, startKey, roachpb.ADD_REPLICA,
				[]roachpb.ReplicationTarget{target}, *rangeInfo.Desc, backFill)
			if err != nil {
...
func (r *Replica) adminScatter(
	ctx context.Context, args roachpb.AdminScatterRequest, backFill bool,
	...
	for re := retry.StartWithCtx(ctx, retryOpts); re.Next(); {
		requeue, err := rq.processOneChange(ctx, r, sysCfg, canTransferLease, false /* dryRun */, backFill)
		if err != nil {
