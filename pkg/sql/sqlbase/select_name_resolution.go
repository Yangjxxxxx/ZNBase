...
func (v *NameResolutionVisitor) VisitPre(expr tree.Expr) (recurse bool, newNode tree.Expr) {
	...
	case *tree.ColumnItem:
		...
		if err != nil {
			...
			if v.InUDR || v.InCursor {
				if v.InCursor {
					if curVar, ok := v.CurVarMap[varName]; ok {
						return true,
							&tree.CursorReplaceVar{
								CurVarName:  tree.Name(varName),
								CurVarType:  curVar.CurVarType,
								CurVarDatum: curVar.CurVarDatum,
							}
					}
				}
				for i := len(v.Params) - 1; i >= 0; i-- {
					if v.Params[i].VarName == varName || tree.FindNameInNames(varName, v.Params[i].AliasNames) {
						return true, &tree.PlpgsqlVar{VarName: tree.Name(varName), VarType: types.OidToType[v.Params[i].VarOid], Index: i}
					}
				}
			}
			v.err = err
		...
		if v.InCursor {
			varName := string(t.ColumnName)
			if _, ok := v.CurVarMap[varName]; ok {
				v.err = errors.Errorf("Fuzzy definition!\n"+
					"\"%s\" can point to a cursor parameter or a column in a table!\n", varName)
				return false, expr
			}
		}
	...
	case *tree.FuncExpr:
		fd, err := t.Func.Resolve(v.searchPath, false)
		if err != nil {
			v.err = err
			return false, expr
		}
...
func expandStar(
	...
	if len(src) == 0 || len(src[0].SourceColumns) == 0 {
		return nil, nil, pgerror.NewErrorf(pgcode.InvalidName,
...
func CheckRenderStar(
	...
	case tree.UnqualifiedStar, *tree.AllColumnsSelector:
		if target.As != "" {
			return false, nil, nil, pgerror.NewErrorf(pgcode.Syntax,
