...
//
// This is beneficial from the standpoint of transaction latency because it means
...
type txnPipeliner struct {
	...
	// a transaction's lock lockFootprint may grow without bound.
	ri       *RangeIterator
	...
	// The transaction's write lockFootprint contains spans where intent writes have
...
func (tp *txnPipeliner) SendLocked(
	...
	// in-flight writes and the write lockFootprint to it.
	...
	// Update the in-flight lock set and the write lockFootprint with the results
	// of the request.
...
// attachWritesToEndTxn attaches the in-flight writes and the write lockFootprint
...
func (tp *txnPipeliner) attachWritesToEndTxn(
	...
	// Augment et.LockSpans and et.InFlightWrites with writes from
	// the current batch.
...
// update the tracked in-flight write set and write lockFootprint. It does so by
...
//    the in-flight writes set to the write lockFootprint.
//
// After updating the write sets, the write lockFootprint is condensed to ensure
// that it remains under its memory limit.
//
// If no response is provided (indicating an error), all writes from the batch
// are added directly to the write lockFootprint to avoid leaking any intents when
// the transaction cleans up.
func (tp *txnPipeliner) updateWriteTracking(
	ctx context.Context, ba roachpb.BatchRequest, br *roachpb.BatchResponse,
) {
	// After adding new writes to the write lockFootprint, check whether we need to
	// condense the set to stay below memory limits.
	defer tp.lockFootprint.maybeCondense(ctx, tp.ri, trackedWritesMaxSize.Get(&tp.st.SV))

	// If the request failed, add all intent writes directly to the write
	// lockFootprint. This reduces the likelihood of dangling intents blocking
	...
	for i, ru := range ba.Requests {
		...
		if qiReq, ok := req.(*roachpb.QueryIntentRequest); ok {
			...
			if ba.AsyncConsensus {
				...
				// directly to our write lockFootprint.
...
func (tp *txnPipeliner) epochBumpedLocked() {
	// Move all in-flight writes into the write lockFootprint. These writes no
...
func (tp *txnPipeliner) rollbackToSavepointLocked(ctx context.Context, s savepoint) {
	...
	// write lockFootprint. We no longer care if these write succeed or fail, so we're
...
// hasAcquiredLocks returns whether the interceptor has observed any writes,
...
// getPipelinerSpans
func (tp *txnPipeliner) getPipelinerSpans() []roachpb.Span {
	writeSpanMap := make(map[string]roachpb.Span)
	for _, alloc := range tp.ifWrites.alloc {
		if alloc.Key != nil {
			writeSpanMap[alloc.Key.String()] = roachpb.Span{Key: alloc.Key, EndKey: alloc.Key.Next()}
		}
	}
	for _, provedWriteSpan := range tp.lockFootprint.s {
		if provedWriteSpan.EndKey == nil {
			writeSpanMap[provedWriteSpan.String()] = roachpb.Span{Key: provedWriteSpan.Key, EndKey: provedWriteSpan.Key.Next()}
		} else {
			writeSpanMap[provedWriteSpan.String()] = provedWriteSpan
		}
	}
	writeSpanSet := make([]roachpb.Span, 0)
	for _, span := range writeSpanMap {
		writeSpanSet = append(writeSpanSet, span)
	}
	return writeSpanSet
}
...
// "lockFootprint" of the set to grow, so the set should be thought of as on
// overestimate.
...
// lockFootprint of the span set. Not only will it merge overlapping spans, but
		...
		for _, s := range bucket.spans[1:] {
			...
			if !cs.Valid() {
				...
				log.Fatalf(ctx, "failed to condense lock spans: "+
					"comznbaseng span %s yielded invalid result", s)
			}
		}
		s.bytes += spanSize(cs)
		s.s = append(s.s, cs)
