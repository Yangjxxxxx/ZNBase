...
func TestConsumerClosed(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.typ.String() /* name */, func(t *testing.T) {
			leftInput := runbase.NewRowBuffer(leftTypes, tc.leftRows, runbase.RowBufferArgs{})
			rightInput := runbase.NewRowBuffer(rightTypes, tc.rightRows, runbase.RowBufferArgs{})
...
func BenchmarkMergeJoiner(b *testing.B) {
	...
	for _, inputSize := range []int{0, 1 << 2, 1 << 4, 1 << 8, 1 << 12, 1 << 16} {
		...
		b.Run(fmt.Sprintf("OneSideRepeatInputSize=%d", inputSize), func(b *testing.B) {
			leftInput := runbase.NewRepeatableRowSource(sqlbase.ColumnTypesToDatumTypes(sqlbase.OneIntCol), sqlbase.MakeIntRows(inputSize, numCols))
			rightInput := runbase.NewRepeatableRowSource(sqlbase.ColumnTypesToDatumTypes(sqlbase.OneIntCol), sqlbase.MakeRepeatedIntRows(numRepeats, inputSize, numCols))
			b.SetBytes(int64(8 * inputSize * numCols * 2))
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				m, err := newMergeJoiner(flowCtx, 0 /* processorID */, spec, leftInput, rightInput, post, disposer)
	...
	for _, inputSize := range []int{0, 1 << 2, 1 << 4, 1 << 8, 1 << 12, 1 << 16} {
		...
		b.Run(fmt.Sprintf("BothSidesRepeatInputSize=%d", inputSize), func(b *testing.B) {
			...
			leftInput := runbase.NewRepeatableRowSource(sqlbase.ColumnTypesToDatumTypes(sqlbase.OneIntCol), row)
			rightInput := runbase.NewRepeatableRowSource(sqlbase.ColumnTypesToDatumTypes(sqlbase.OneIntCol), row)
			b.SetBytes(int64(8 * inputSize * numCols * 2))
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				m, err := newMergeJoiner(flowCtx, 0 /* processorID */, spec, leftInput, rightInput, post, disposer)
...
func TestOptMergeJoiner(t *testing.T) {
	defer leaktest.AfterTest(t)()
