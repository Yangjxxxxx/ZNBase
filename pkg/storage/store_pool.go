...
const (
	...
	// The store is alive but it cannot execute rebalance and addreplica.
	storeStatusDisable
...
func (sd *storeDetail) status(
	...
	// Judge whether it is DISABLE according to state of Store
	if sd.desc.State == roachpb.DISABLE {
		return storeStatusDisable
	}
...
func (sp *StorePool) decommissioningReplicas(
	...
	for _, repl := range repls {
		...
		case storeStatusDecommissioning, storeStatusDisable:
...
// ClusterEnableNodeCount returns the number of nodes of the stores which state is enable
func (sp *StorePool) ClusterEnableNodeCount() int {
	sp.detailsMu.Lock()
	defer sp.detailsMu.Unlock()
	now := sp.clock.PhysicalTime()
	timeUntilStoreDead := TimeUntilStoreDead.Get(&sp.st.SV)
	nodeMap := make(map[roachpb.NodeID]struct{})
	for _, store := range sp.detailsMu.storeDetails {
		if store.desc != nil {
			if _, ok := nodeMap[store.desc.Node.NodeID]; !ok {
				if store.desc.State == roachpb.ENABLE && store.status(now, timeUntilStoreDead, 0, sp.nodeLivenessFn) != storeStatusDecommissioning {
					nodeMap[store.desc.Node.NodeID] = struct{}{}
				}
			}
		}
	}
...
func (sp *StorePool) liveAndDeadReplicas(
	...
	for _, repl := range repls {
		...
		case storeStatusAvailable, storeStatusThrottled, storeStatusDecommissioning, storeStatusDisable:
...
// getStoreList返回一个包含所有活动存储的storeList
// 所需的属性及其相关的统计信息。storeList是过滤的
// 根据提供的storeFilter。它还返回的总数
// 活的和节流的存储。传递的rangeID用于检查
// 损坏的副本。
...
func (sp *StorePool) getStoreListFromIDsRLocked(
	...
	for _, storeID := range storeIDs {
		...
		case storeStatusAvailable:
			aliveStoreCount++
			storeDescriptors = append(storeDescriptors, *detail.desc)
		case storeStatusDead, storeStatusUnknown, storeStatusDecommissioning, storeStatusDisable:
