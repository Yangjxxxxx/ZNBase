...
function queryFromProps(
  ...
  graphProps: MetricsDataComponentProps,
): protos.znbase.ts.tspb.IQuery {
    let derivative = protos.znbase.ts.tspb.TimeSeriesQueryDerivative.NONE;
    let sourceAggregator = protos.znbase.ts.tspb.TimeSeriesQueryAggregator.SUM;
    let downsampler = protos.znbase.ts.tspb.TimeSeriesQueryAggregator.AVG;

    // Compute derivative function.
    if (!_.isNil(metricProps.derivative)) {
      derivative = metricProps.derivative;
    } else if (metricProps.rate) {
      derivative = protos.znbase.ts.tspb.TimeSeriesQueryDerivative.DERIVATIVE;
    } else if (metricProps.nonNegativeRate) {
      derivative = protos.znbase.ts.tspb.TimeSeriesQueryDerivative.NON_NEGATIVE_DERIVATIVE;
    }
    ...
    if (!_.isNil(metricProps.downsampler)) {
      ...
      downsampler = protos.znbase.ts.tspb.TimeSeriesQueryAggregator.MAX;
    } else if (metricProps.downsampleMin) {
      downsampler = protos.znbase.ts.tspb.TimeSeriesQueryAggregator.MIN;
    }
    ...
    if (!_.isNil(metricProps.aggregator)) {
      ...
      sourceAggregator = protos.znbase.ts.tspb.TimeSeriesQueryAggregator.MAX;
    } else if (metricProps.aggregateMin) {
      sourceAggregator = protos.znbase.ts.tspb.TimeSeriesQueryAggregator.MIN;
    } else if (metricProps.aggregateAvg) {
      sourceAggregator = protos.znbase.ts.tspb.TimeSeriesQueryAggregator.AVG;
    }
