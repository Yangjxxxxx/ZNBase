...
func doExpandPlan(
	...
	case *deleteNode:
		...
		if isContainTrigger(ctx, p.txn, n.run.td.tableDesc()) {
			break
		}
	...
	case *createTriggerNode:
	case *dropDatabaseNode:
	case *dropIndexNode:
	case *dropTableNode:
	case *dropViewNode:
	case *dropTriggerNode:
	case *dropSequenceNode:
	case *dropFunctionNode:
	case *dropSchemaNode:
	case *DropUserNode:
	case *zeroNode:
	case *unaryNode:
...
func isContainTrigger(
	...
	for i := range triggers.Triggers {
		if triggers.Triggers[i].Tgtype&TriggerTypeDelete == TriggerTypeDelete {
			return true
		}
	}
	return false
