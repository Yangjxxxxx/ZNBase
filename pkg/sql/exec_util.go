...
// TODO(bob): Change this to 4 in v2.3; https://github.com/znbasedb/znbase/issues/32534
// TODO(bob): Remove or n-op this in v2.4: https://github.com/znbasedb/znbase/issues/32844
...
// SQLSessionTimeout use for canceling session
var SQLSessionTimeout = settings.RegisterIntSetting(
	"server.sql_session_timeout",
	"default duration the session is permitted to be idle before it is closed by server",
	0,
)

var replicationSyncClusterMode = settings.RegisterBoolSetting(
	"sql.defaults.replicate_table_in_sync.enabled",
	"default value for replicate_table_in_sync session setting; allows use of primary key changes by default",
	false,
)
...
var VectorizeClusterMode = settings.RegisterEnumSetting(
	...
	map[int64]string{
		...
		int64(sessiondata.VectorizeExperimentalOn):     "on",
		int64(sessiondata.VectorizeExperimentalAlways): "always",
	},
)
...
var VectorizeRowCountThresholdClusterValue = settings.RegisterValidatedIntSetting(
	...
	vecexec.DefaultVectorizeRowCountThreshold,
	func(v int64) error {
		if v < 0 {
...
// clusterIsolationLevel set cluster default transaction isolation level.
var clusterIsolationLevel = settings.RegisterIsolationSetting(
	"kv.transaction.cluster_isolationlevel",
	"transactional commits default with transactional cluster_isolationlevel",
	"SERIALIZABLE",
	map[string]string{
		"DEFAULT":          util.DefaultIsolation.String(),
		"SERIALIZABLE":     util.SerializableIsolation.String(),
		"READ COMMITTED":   util.ReadCommittedIsolation.String(),
		"READ UNCOMMITTED": util.ReadCommittedIsolation.String(),
		"SNAPSHOT":         util.SerializableIsolation.String(),
		"REPEATABLE READ":  util.SerializableIsolation.String(),
	},
)
...
var (
	...
	MetaRestartSavepoint = metric.Metadata{
		...
		Help:        "Number of `SAVEPOINT znbase_restart` statements",
		Measurement: "SQL Statements",
		Unit:        metric.Unit_COUNT,
	}
	MetaReleaseRestartSavepoint = metric.Metadata{
		Name:        "sql.restart_savepoint.release.count",
		Help:        "Number of `RELEASE SAVEPOINT znbase_restart` statements",
		Measurement: "SQL Statements",
		Unit:        metric.Unit_COUNT,
	}
	MetaRollbackToRestartSavepoint = metric.Metadata{
		Name:        "sql.restart_savepoint.rollback.count",
		Help:        "Number of `ROLLBACK TO SAVEPOINT znbase_restart` statements",
		Measurement: "SQL Statements",
		Unit:        metric.Unit_COUNT,
	}
...
// DropTableCache drop the table cache.
type DropTableCache struct {
	Gossip   *gossip.Gossip
	ClientDB *client.DB
	LeaseMgr *LeaseManager
}

// NewDropTableCache return the DropTableCache struct.
func NewDropTableCache(g *gossip.Gossip, db *client.DB, leaseMgr *LeaseManager) *DropTableCache {
	dropTableCache := &DropTableCache{
		Gossip:   g,
		ClientDB: db,
		LeaseMgr: leaseMgr,
	}
	...
	g.RegisterCallback(
		gossip.KeyTableDescDropCache,
		dropTableCache.tableDropCache,
		gossip.Redundant,
	)
	return dropTableCache
}

func (dt *DropTableCache) tableDropCache(key string, _ roachpb.Value) {
	tableID, err := TableIDFromKey(key)
	if err != nil {
		log.Fatalf(context.Background(), "wrong key received on dropTableCache callback: %s", key)
	}

	if err := purgeOldVersions(
		context.TODO(), nil, tableID, true, 0, dt.LeaseMgr); err != nil {
		log.Fatalf(context.Background(), "Unable to clean up table cache, ID: %v", tableID)
	}
...
type ExecutorConfig struct {
	...
	LocationName     roachpb.LocationName
...
func newDatabaseCacheHolder(c *databaseCache, s *schemaCache) *databaseCacheHolder {
...
func (dc *databaseCacheHolder) getSchemaCache() *schemaCache {
	dc.mu.Lock()
	defer dc.mu.Unlock()
	return dc.mu.s
}
...
func (dc *databaseCacheHolder) updateSystemConfig(cfg *config.SystemConfig) {
	...
	dc.mu.c.disable()
	dc.mu.s.disable()
	// use newDatabaseCache and newSchemaCache avoid race without lock
	dc.mu.c = newDatabaseCache(cfg)
	dc.mu.s = newSchemaCache(cfg)
	dc.mu.cv.Broadcast()
	dc.mu.Unlock()
}
...
func (p *planner) isAsOf(stmt tree.Statement) (*hlc.Timestamp, error) {
	...
	case *tree.Dump:
		if s.IsQuery && (s.ExpendQuery.Type == 0) {
			return p.isAsOf(s.ExpendQuery.Query)
		}
		return nil, nil
...
// GetSessions used to get sessions from SessionRegistry.
func (r *SessionRegistry) GetSessions() map[ClusterWideID]RegistrySession {
	return r.sessions
}

// GetUserName used to get user from SessionRegistry.
func (r *SessionRegistry) GetUserName(s registrySession) string {
	return s.user()
}
...
type registrySession interface {
	...
	queryTransaction(str string) (*serverpb.QueryLockstatResponse, bool)
	GetConnExecutor(r registrySession) *connExecutor
}

// RegistrySession for using registrySession in other pack
type RegistrySession registrySession
...
// QueryLockstat looks up the specified session in the session registry and queries it.
func (r *SessionRegistry) QueryLockstat(
	txnIDStr, username string,
) (*serverpb.QueryLockstatResponse, error) {
	r.Lock()
	defer r.Unlock()
	...
	for _, session := range r.sessions {
		...
		response, findTxn := session.queryTransaction(txnIDStr)
		if findTxn {
			return response, nil
		}
	}

	return nil, fmt.Errorf("query ID %s not found", txnIDStr)
}
...
func AnonymizeStatementsForReporting(action, sqlStmts string, r interface{}) error {
	...
	{
		stmts, err := parser.Parse(sqlStmts, false)
		if err == nil {
			for i := range stmts {
...
// the order in which they should be presented in the zbdb_internal.session_info
...
type sessionDataMutator struct {
	...
	// setCurTxnSupDDL is called when we execute SET transaction_sup_ddl = ...
	setCurTxnSupDDL func(val bool)
...
// SetParallelFlag set operator parallel flag
func (m *sessionDataMutator) SetParallel(idx sessiondata.ParallelFlags, flag bool) {
	m.data.PC[idx].Parallel = flag
	if flag {
		m.data.PC[idx].ValString = "on"
	} else {
		m.data.PC[idx].ValString = "off"
	}
}

// SetParallelNum set operator parallel flag
func (m *sessionDataMutator) SetParallelNum(idx sessiondata.ParallelFlags, num int64) {
	m.data.PC[idx].ParallelNum = num
}

func (m *sessionDataMutator) SetClientEncoding(charset string) {
	m.data.ClientEncoding = charset
}
...
func (m *sessionDataMutator) UpdateSearchPath(dbID uint32) {
	if sp, ok := m.data.DBDefaultSearchPath[dbID]; ok {
		m.data.SearchPath = sessiondata.SetSearchPath(m.data.SearchPath, sp)
		sqlbase.DefaultSearchPath = sessiondata.MakeSearchPath(sp)
	} else {
		m.data.SearchPath = sessiondata.SetSearchPath(m.data.SearchPath, m.data.UserDefaultSearchPath)
		sqlbase.DefaultSearchPath = sessiondata.MakeSearchPath(m.data.UserDefaultSearchPath)
	}
}
...
func (m *sessionDataMutator) SetDefaultIsolationLevel(isolationLevel tree.IsolationLevel) {
	m.data.DefaultIsolationLevel = util.IsolationLevel(isolationLevel)
}
...
func (m *sessionDataMutator) SetReplicaionSync(tr bool) {
	m.data.ReplicaionSync = tr
}
...
func (m *sessionDataMutator) SetCloseSessionTimeout(timeout int64) {
	m.data.CloseSessionTimeout = timeout
}
...
func (m *sessionDataMutator) SetCaseSensitive(val bool) {
	m.data.CaseSensitive = val
}
...
// TableIDFromKey returns the Table ID which parsing from key.
func TableIDFromKey(key string) (sqlbase.ID, error) {
	trimmedKey, err := removePrefixFromKey(key, gossip.KeyTableDescDropCache)
	if err != nil {
		return 0, err
	}
	nodeID, err := strconv.ParseInt(trimmedKey, 10 /* base */, 64 /* bitSize */)
	if err != nil {
		return 0, errors.Wrapf(err, "failed parsing TableID from key %q", key)
	}
	return sqlbase.ID(nodeID), nil
}
