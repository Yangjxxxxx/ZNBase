...
func init() {
	...
	scalarBuildFuncMap = [opt.NumOperators]buildFunc{
		...
		opt.NvlOp:             (*Builder).buildNvl,
		opt.CharOp:            (*Builder).buildChar,
		...
		opt.AdditionalOp:      (*Builder).buildAdditional,
...
func (b *Builder) buildScalar(ctx *buildScalarCtx, scalar opt.ScalarExpr) (tree.TypedExpr, error) {
	if fn := scalarBuildFuncMap[scalar.Op()]; fn != nil {
		...
		if b.evalCtx != nil && b.isConst(texpr) {
			...
			if err != nil {
				// for user define function, not just a simple eval process,
				// can also execute DDL, DML, DCL stmts and so on.
				// it may cause some data change in database, which not same as the builtin function.
				// so report UDF error here
...
// BuildScalarExprToTypedExpr build typedExpr from scalar, use in UDR
func (b *Builder) BuildScalarExprToTypedExpr(
...
func (b *Builder) buildFunction(
	...
	var isUDF bool
	if fn.Properties.Category == "the user define func" {
		funcRef = tree.ResolvableFunctionReference{FunctionReference: fn.Def}
		isUDF = true
	} else {
...
func (b *Builder) buildNvl(ctx *buildScalarCtx, scalar opt.ScalarExpr) (tree.TypedExpr, error) {
	nvl := scalar.(*memo.NvlExpr)
	exprs := make(tree.TypedExprs, len(nvl.Args))
	var err error
	for i := range exprs {
		exprs[i], err = b.buildScalar(ctx, nvl.Args[i])
		if err != nil {
			return nil, err
		}
	}
	return tree.NewTypedNvlExpr(exprs, nvl.Typ), nil
}

func (b *Builder) buildChar(ctx *buildScalarCtx, scalar opt.ScalarExpr) (tree.TypedExpr, error) {
	char := scalar.(*memo.CharExpr)
	char.Typ = types.String
	exprs := make(tree.TypedExprs, len(char.Args))
	var err error
	for i := range exprs {
		exprs[i], err = b.buildScalar(ctx, char.Args[i])
		if err != nil {
			return nil, err
		}
	}
	return tree.NewTypedCharExpr(exprs, char.Typ), nil
}
...
func (b *Builder) buildAdditional(
	ctx *buildScalarCtx, scalar opt.ScalarExpr,
) (tree.TypedExpr, error) {
	return scalar.(*memo.AdditionalExpr).Value, nil
}
...
func (b *Builder) isConst(expr tree.Expr) bool {
	// 判断是否是const通过 walk expr得到新旧expr进行对比，如果是一样的，就认为他是const
	// SignalExpr通过walk返回的值是他本身，如果当做const处理，Signal自定义报错会在计划阶段直接报错回滚
	// 与期望逻辑"数据满足某种条件时进行自定义报错"不符,因此需要特殊处理
	switch expr.(type) {
	case *tree.AdditionalExpr:
		if expr.(*tree.AdditionalExpr).Sig != nil {
			return false
		}
