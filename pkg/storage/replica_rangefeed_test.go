...
func TestReplicaRangefeedRetryErrors(t *testing.T) {
	...
	assertRangefeedRetryErr := func(
		...
		subT.Helper()
		expErr := roachpb.NewRangeFeedRetryError(expReason, nil)
		if pErr == nil {
...
func TestReplicaRangefeedNudgeSlowClosedTimestamp(t *testing.T) {
	...
	for i := range repls {
		ds := tc.Server(i).DistSender().(*kv.DistSender)
		...
		go func() {
			...
			rangeFeedErrC <- ds.RangeFeed(rangeFeedCtx, &req, false, rangeFeedCh)
		}()
	}
