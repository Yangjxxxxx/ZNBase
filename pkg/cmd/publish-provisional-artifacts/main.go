...
func main() {
	...
	pkg, err := build.Import("github.com/znbasedb/znbase", "", build.FindOnly)
	if err != nil {
		log.Fatalf("unable to locate ZNBase directory: %s", err)
	}
...
func run(svc s3I, execFn execRunner, flags runFlags) {
	...
	if len(*destBucket) > 0 {
		...
		bucketName = "binaries.znbasedb.com"
	} else {
		bucketName = "znbase"
	}
	log.Printf("Using S3 bucket: %s", bucketName)

	var znbaseBuildOpts []opts
	for _, target := range []struct {
		...
		for i, extraArgs := range []struct {
			...
			znbaseBuildOpts = append(znbaseBuildOpts, o)
		}
	}
	...
	if flags.doProvisional {
		for _, o := range znbaseBuildOpts {
			buildZNBase(svc, execFn, flags, o)
...
func buildZNBase(svc s3I, execFn execRunner, flags runFlags, o opts) {
	log.Printf("building znbase %s", pretty.Sprint(o))
	defer func() {
		log.Printf("done building znbase: %s", pretty.Sprint(o))
	}()
...
func putNonRelease(svc s3I, o opts, binary io.ReadSeeker) {
	const repoName = "znbase"
	remoteName, hasExe := TrimDotExe(o.Base)
	// TODO(tamird): do we want to keep doing this? No longer
	// doing so requires updating znbaselabs/production, and
	// possibly znbasedb/znbase-go.
...
func putRelease(svc s3I, o opts, binary *os.File) {
	...
	if _, hasExe := TrimDotExe(o.Suffix); hasExe {
		...
		zipHeader.Name = filepath.Join(targetArchiveBase, "znbase.exe")
		...
		tarHeader.Name = filepath.Join(targetArchiveBase, "znbase")
