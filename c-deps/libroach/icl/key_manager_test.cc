...
struct testKey {
  std::string id;
  std::string key;
  enginepbicl::EncryptionType type;
  ...
  testKey(std::string id) : testKey(id, "", enginepbicl::Plaintext, 0, "") {}
  testKey(std::string id, std::string key, enginepbicl::EncryptionType type, int64_t timestamp,
          std::string source)
...
enginepbicl::KeyInfo keyInfoFromTestKey(const testKey& k) {
  enginepbicl::KeyInfo ki;
...
enginepbicl::SecretKey secretKeyFromTestKey(const testKey& k) {
  enginepbicl::SecretKey sk;
  sk.set_key(k.key);
  sk.set_allocated_info(new enginepbicl::KeyInfo(keyInfoFromTestKey(k)));
  return sk;
}

rocksdb::Status compareNonRandomKeyInfo(const enginepbicl::KeyInfo& actual,
...
rocksdb::Status compareKeyInfo(const enginepbicl::KeyInfo& actual, const testKey& expected) {
...
rocksdb::Status compareKey(const enginepbicl::SecretKey& actual, const testKey& expected) {
...
TEST(FileKeyManager, BuildKeyFromFile) {
  ...
  std::vector<TestCase> test_cases = {
      {"plain", "plain", "plain", {testKey("plain", "", enginepbicl::Plaintext, now, "plain")}},
      {"16.key",
       "32.key",
       key_id_128,
       {testKey(key_id_128, key_128, enginepbicl::AES128_CTR, now, "16.key"),
        testKey(key_id_256, key_256, enginepbicl::AES256_CTR, now, "32.key")}},
      {
          "24.key",
          "plain",
          key_id_192,
          {testKey(key_id_192, key_192, enginepbicl::AES192_CTR, now, "24.key"),
           testKey("plain", "", enginepbicl::Plaintext, now, "plain")},
      },
  };
...
TEST(DataKeyManager, LoadKeys) {
  ...
  // Empty protobuf.
  {
    enginepbicl::DataKeysRegistry registry;
    std::string contents;
...
TEST(DataKeyManager, KeyFromKeyInfo) {
  ...
  std::vector<TestCase> test_cases = {
      {testKey("bad", "", enginepbicl::EncryptionType_INT_MIN_SENTINEL_DO_NOT_USE_, 0, ""),
       "unknown encryption type .* for key ID bad",
       {}},
      {testKey("plain", "", enginepbicl::Plaintext, now, "plain"), "",
       testKey("plain", "", enginepbicl::Plaintext, now, "data key manager", true, "plain")},
      {testKey(key_id_128, "", enginepbicl::AES128_CTR, now - 86400, "128.key"), "",
       testKey("someid", "somekey", enginepbicl::AES128_CTR, now, "data key manager", false,
               key_id_128)},
      {testKey(key_id_192, "", enginepbicl::AES192_CTR, now + 86400, "192.key"), "",
       testKey("someid", "somekey", enginepbicl::AES192_CTR, now, "data key manager", false,
               key_id_192)},
      {testKey(key_id_256, "", enginepbicl::AES256_CTR, 0, "256.key"), "",
       testKey("someid", "somekey", enginepbicl::AES256_CTR, now, "data key manager", false,
               key_id_256)},
  };
  ...
  for (auto t : test_cases) {
    ...
    case enginepbicl::Plaintext:
      expected_length = 0;
      break;
    case enginepbicl::AES128_CTR:
      expected_length = 16;
      break;
    case enginepbicl::AES192_CTR:
      expected_length = 24;
      break;
    case enginepbicl::AES256_CTR:
      expected_length = 32;
      break;
    default:
    ...
    if (t.new_key.type == enginepbicl::Plaintext) {
...
TEST(DataKeyManager, SetStoreKey) {
  ...
  std::vector<TestCase> test_cases = {
      ...
      },
      {
          testKey(key_id_128, "", enginepbicl::AES128_CTR, now - 86400, "128.key"),
          "",
          testKey(key_id_128, "", enginepbicl::AES128_CTR, now, "data key manager", false,
                  key_id_128),
      },
      {
      ...
      {
          testKey(key_id_256, "", enginepbicl::AES256_CTR, now - 86400, "256.key"),
          "",
          testKey(key_id_256, "", enginepbicl::AES256_CTR, now, "data key manager", false,
                  key_id_256),
      },
      {
          testKey(key_id_128, "", enginepbicl::AES128_CTR, now - 86400, "128.key"),
  ...
  for (auto t : test_cases) {
    ...
    auto store_info = std::unique_ptr<enginepbicl::KeyInfo>(
        new enginepbicl::KeyInfo(keyInfoFromTestKey(t.store_info)));
    ...
    if (status.ok()) {
      ...
      if (t.store_info.type == enginepbicl::Plaintext) {
...
TEST(DataKeyManager, RotateKeyAtStartup) {
  ...
  std::vector<TestCase> test_cases = {
      {
          testKey("plain", "", enginepbicl::Plaintext, 0, "plain"),
          0,
          testKey("plain", "", enginepbicl::Plaintext, 0, "data key manager", true, "plain"),
      },
      {
      ...
      },
      {
          ...
          testKey(key_id_128, "", enginepbicl::AES128_CTR, 0, "128.key"),
          20,
          testKey("not checked", "not checked", enginepbicl::AES128_CTR, 20, "data key manager",
                  false, key_id_128),
      },
      {
      ...
      },
      {
          ...
          testKey(key_id_128, "", enginepbicl::AES128_CTR, 0, "128.key"),
          30,
          testKey("not checked", "not checked", enginepbicl::AES128_CTR, 30, "data key manager",
                  false, key_id_128),
      },
  };
...

  {
    ...
    auto store_info = std::unique_ptr<enginepbicl::KeyInfo>(
        new enginepbicl::KeyInfo(keyInfoFromTestKey(tc.store_info)));
...
TEST(DataKeyManager, RotateKeyWhileRunning) {
  ...
  // Set new active store key.
  auto plain_key = testKey("plain", "", enginepbicl::Plaintext, 0, "plain");
  auto plain_info = std::unique_ptr<enginepbicl::KeyInfo>(
      new enginepbicl::KeyInfo(keyInfoFromTestKey(plain_key)));
  ...
  auto aes_key = testKey(key_id_128, "", enginepbicl::AES128_CTR, 0, "128.key");
  auto aes_info =
      std::unique_ptr<enginepbicl::KeyInfo>(new enginepbicl::KeyInfo(keyInfoFromTestKey(aes_key)));
  ...
  EXPECT_EQ(aes_key1->encryption_type(), enginepbicl::AES128_CTR);
  ...
  for (int i = 0; i < 9; i++) {
    ...
    EXPECT_EQ(aes_key2->encryption_type(), enginepbicl::AES128_CTR);
  }
  ...
  EXPECT_EQ(aes_key3->encryption_type(), enginepbicl::AES128_CTR);
}
