...
import (
	...
	"github.com/znbasedb/znbase/pkg/cli/syncbench"
...
// PopulateRocksDBConfigHook is a callback set by ICL code.
...
func OpenEngine(dir string, stopper *stop.Stopper, opts OpenEngineOptions) (engine.Engine, error) {
	...
	storageEngine := resolveStorageEngineType(engine.DefaultStorageEngine, cfg.Dir)
...
func runDebugCheckStoreDescriptors(ctx context.Context, db engine.Engine) error {
...
func runDebugCheckStoreRaft(ctx context.Context, db engine.Engine) error {
...
raw store data. 'znbase debug rocksdb' accepts the same arguments and flags
as 'ldb'.
...
a ZNBase Labs engineer. It is a last-resort option to recover data
...
func removeDeadReplicas(
	...
	for _, desc := range newDescs {
		...
		if wiErr, ok := err.(*roachpb.WriteIntentError); ok {
			...
			if _, err := engine.MVCCResolveWriteIntent(ctx, batch, nil, update); err != nil {
				return nil, err
			}
...
var debugMergeLogsOpts = struct {
	...
	program: regexp.MustCompile("^znbase.*$"),
...
// and need encryption flags (injected by ICL code).
...
// DebugCmd is the root of all debug commands. Exported to allow modification by ICL code.
