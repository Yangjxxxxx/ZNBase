...
func runBadgerTest(t *testing.T, opts *Options, test func(t *testing.T, db *DB)) string {
	return runBadgerTestWithDB(t, opts, "", false, test)
}
func runBadgerTestWithDB(t *testing.T, opts *Options, dbPath string, reuse bool, test func(t *testing.T, db *DB)) string {
	if dbPath == "" {
		dir, err := ioutil.TempDir("/data/test", "badger-test")
		require.NoError(t, err)
		dbPath = dir
	}
	if !reuse {
