...
func (pi *pendingWritesIterator) Prev() {
	if pi.nextIdx > 0 {
		pi.nextIdx--
	}
}

func (pi *pendingWritesIterator) SeekToFirst() {
	pi.rewind(false)
}
func (pi *pendingWritesIterator) SeekToLast() {
	pi.rewind(true)
}
func (pi *pendingWritesIterator) Rewind() {
	pi.rewind(false)
}
func (pi *pendingWritesIterator) rewind(reversed bool) {
	pi.nextIdx = 0
}
...
func (pi *pendingWritesIterator) SeekForPrev(key []byte) {
	pi.seek(key)
}
...
func (txn *Txn) newPendingWritesIterator() *pendingWritesIterator {
