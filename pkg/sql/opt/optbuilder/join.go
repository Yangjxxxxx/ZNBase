...
func (b *Builder) buildJoin(
	...
	rightScope := b.buildDataSource(join.Right, nil /* indexFlags */, locking, inScope)
	joinPrivate := buildPrivateJoinForHint(leftScope.expr, rightScope.expr)
	if !joinPrivate.Flags.DisallowHashJoin && joinPrivate.Flags.DisallowMergeJoin && joinPrivate.Flags.DisallowLookupJoin {
		join.Hint = tree.AstHash
	}
	if !joinPrivate.Flags.DisallowLookupJoin && joinPrivate.Flags.DisallowHashJoin && joinPrivate.Flags.DisallowMergeJoin {
		join.Hint = tree.AstLookup
	}
	if !joinPrivate.Flags.DisallowMergeJoin && joinPrivate.Flags.DisallowLookupJoin && joinPrivate.Flags.DisallowHashJoin {
		join.Hint = tree.AstMerge
	}
	...
	case *tree.OnJoinCond, nil:
		...
		if on, ok := cond.(*tree.OnJoinCond); ok {
			...
			b.semaCtx.Properties.Require("ON", tree.RejectSpecial|tree.RejectWindowApplications)
			outScope.context = "ON"
...
func (jb *usingJoinBuilder) buildUsingJoin(using *tree.UsingJoinCond) {
	...
	for _, name := range using.Cols {
		...
		if seenCols.Contains(int(leftCol.id)) {
			...
			panic(pgerror.NewErrorf(pgcode.DuplicateColumn,
