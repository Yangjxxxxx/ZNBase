...
func init() {
	...
	tcs = []hjTestCase{
		...
		},
		{
			...
			onExpr: distsqlpb.Expression{Expr: "@1 + @3 > 2 AND @1 + @3 < 8"},
			expectedTuples: tuples{
				{nil, nil},
				{1, nil},
			},
		},
		{
			...
			onExpr: distsqlpb.Expression{Expr: "@1 + @3 + @4 < 100"},
			expectedTuples: tuples{
				{nil, 2},
				{2, 4},
			},
...
func createSpecForHashJoiner(tc hjTestCase) *distsqlpb.ProcessorSpec {
	...
	lcts, _ := sqlbase.DatumType1sToColumnTypes(sem.ToColumnTypes(tc.leftTypes))
	rcts, _ := sqlbase.DatumType1sToColumnTypes(sem.ToColumnTypes(tc.rightTypes))
...
func TestHashJoinerProjection(t *testing.T) {
	...
	lcts, _ := sqlbase.DatumTypesToColumnTypes(leftTypes)
	rcts, _ := sqlbase.DatumTypesToColumnTypes(rightTypes)
