...
static const std::string kKeyRegistryFilename = "INSPURCLOUDDB_DATA_KEYS";
...
class KeyManager {
 public:
  ...
  std::unique_ptr<enginepbicl::KeyInfo> CurrentKeyInfo() {
    auto key = CurrentKey();
    if (key == nullptr) {
      return nullptr;
    }
    return std::unique_ptr<enginepbicl::KeyInfo>(new enginepbicl::KeyInfo(key->info()));
  }

  // CurrentKey returns the key itself.
  // **WARNING**: this must not be logged, displayed, or stored outside the key registry.
  virtual std::shared_ptr<enginepbicl::SecretKey> CurrentKey() = 0;

  // GetKeyInfo returns the KeyInfo about the key the requested `id`.
  // It does NOT include the key itself and can be logged, displayed, and stored.
  std::unique_ptr<enginepbicl::KeyInfo> GetKeyInfo(const std::string& id) {
    auto key = GetKey(id);
    if (key == nullptr) {
      return nullptr;
    }
  ...
  // **WARNING**: this must not be logged, displayed, or stored outside the key registry.
  virtual std::shared_ptr<enginepbicl::SecretKey> GetKey(const std::string& id) = 0;
};
...
class FileKeyManager : public KeyManager {
 ...
 private:
  ...
  std::shared_ptr<enginepbicl::SecretKey> active_key_;
  std::shared_ptr<enginepbicl::SecretKey> old_key_;
};
...
class DataKeyManager : public KeyManager {
 public:
  ...
  rocksdb::Status SetActiveStoreKeyInfo(std::unique_ptr<enginepbicl::KeyInfo> store_key);
  ...
  // be accurate here.
  std::unique_ptr<enginepbicl::KeyInfo> GetActiveStoreKeyInfo();

  // GetScrubbedRegistry returns a copy of the key registry with key contents scrubbed.
  std::unique_ptr<enginepbicl::DataKeysRegistry> GetScrubbedRegistry() const;
 ...
 private:
  ...
  std::shared_ptr<enginepbicl::SecretKey> CurrentKeyLocked();
  rocksdb::Status LoadKeysHelper(enginepbicl::DataKeysRegistry* registry);
  // Write the given registry to disk. If successful, update registry_ and current_key_.
  rocksdb::Status PersistRegistryLocked(std::unique_ptr<enginepbicl::DataKeysRegistry> reg);
  ...
  std::unique_ptr<enginepbicl::DataKeysRegistry> registry_;
  std::shared_ptr<enginepbicl::SecretKey> current_key_;
};
