...
func (s *Store) canApplySnapshotLocked(
	...
	if snapHeader.IsPreemptive() {
		return nil, errors.Errorf(`expected a raft or learner snapshot`)
	}
...
func (s *Store) shouldAcceptSnapshotData(
	...
	if snapHeader.IsPreemptive() {
		return errors.Errorf(`expected a raft or learner snapshot`)
	}
...
var rebalanceSnapshotRate = settings.RegisterByteSizeSetting(
	...
	envutil.EnvOrDefaultBytes("ZNBASE_PREEMPTIVE_SNAPSHOT_RATE", 8<<20),
)
...
var recoverySnapshotRate = settings.RegisterByteSizeSetting(
	...
	envutil.EnvOrDefaultBytes("ZNBASE_RAFT_SNAPSHOT_RATE", 8<<20),
)
