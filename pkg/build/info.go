...
var (
	...
	// Distribution is changed by the ICL init-time hook in non-APL builds.
	Distribution = "OSS"
	typ          string // Type of this build: <empty>, "development", or "release[-gnu|-musl]"
	channel      = "unknown"
	envChannel   = envutil.EnvOrDefaultString("ZNBASE_CHANNEL", "unknown")
)
...
func init() {
	...
	if tagOverride := envutil.EnvOrDefaultString(
		"ZNBASE_TESTING_VERSION_TAG", ""); tagOverride != "" {
		tag = tagOverride
	}
}
...
func (b Info) Short() string {
	...
	return fmt.Sprintf("ZNBaseDB %s %s (%s, built %s, %s)",
