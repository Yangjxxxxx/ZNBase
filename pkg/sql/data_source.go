...
func (p *planner) getDataSource(
	...
	case *tree.StatementSource:
		...
		if len(cols) == 0 {
			return planDataSource{}, pgerror.NewErrorf(pgcode.UndefinedColumn,
...
func (p *planner) getTableScanByRef(
	...
	if tref.Columns != nil && len(tref.Columns) == 0 {
		return planDataSource{}, pgerror.NewErrorf(pgcode.Syntax,
...
func (p *planner) getPlanForDesc(
	...
	if desc.IsView() && !desc.IsMaterializedView {
	...
	_, ok := p.curPlan.AST.(*tree.Update)
	if len(desc.InheritsBy) > 0 && !tn.IsOnly && !ok {
		var colNames = make([]string, 0, len(colCfg.wantedColumns))
		for _, id := range colCfg.wantedColumns {
			for _, col := range desc.Columns {
				if tree.ColumnID(col.ID) == id {
					colNames = append(colNames, col.Name)
				}
			}
		}
		var inhCols = make(map[string]struct{})
		for _, col := range scan.resultColumns {
			inhCols[col.Name] = struct{}{}
		}
		var nuionNode planNode

		var selectTableMap = make(map[sqlbase.ID]struct{})
		for _, id := range desc.InheritsBy {
			selectTableMap[id] = struct{}{}
		}

		for i, id := range desc.InheritsBy {
			inhTableDesc, err := p.Tables().getTableVersionByID(ctx, p.txn, id, ObjectLookupFlags{})
			if err != nil {
				return planDataSource{}, err
			}
			if inhTableDesc == nil {
				return planDataSource{}, fmt.Errorf(`cannot find inherit table id: %d`, id)
			}

			if i == 0 {
				nuionNode, err = p.buildInheritPlanNode(ctx, inhTableDesc, colCfg, colNames, scan, inhCols, selectTableMap)
				if err != nil {
					return planDataSource{}, err
				}
			} else {
				nuionNode, err = p.buildInheritPlanNode(ctx, inhTableDesc, colCfg, colNames, nuionNode, inhCols, selectTableMap)
				if err != nil {
					return planDataSource{}, err
				}
			}
...
func (p *planner) getViewPlan(
	...
	stmt, err := parser.ParseOne(desc.ViewQuery, p.EvalContext().GetCaseSensitive())
	if err != nil {
...
func (p *planner) buildInheritPlanNode(
	ctx context.Context,
	desc *sqlbase.ImmutableTableDescriptor,
	cfg scanColumnsConfig,
	names []string,
	leftNode planNode,
	inhcols map[string]struct{},
	tableMap map[sqlbase.ID]struct{},
) (planNode, error) {
	rightScan := p.Scan()
	if err := rightScan.initTable(ctx, p, desc, nil, cfg); err != nil {
		return nil, err
	}
	var resultCols = make(sqlbase.ResultColumns, 0, len(rightScan.resultColumns))
	for _, col := range rightScan.resultColumns {
		if _, ok := inhcols[col.Name]; ok {
			resultCols = append(resultCols, col)
		}
	}
	rightScan.resultColumns = resultCols

	nuionNode, err := p.newUnionNode(tree.UnionOp, true, leftNode, rightScan)
	if err != nil {
		return nil, err
	}
	...
	for _, id := range inheritsByTables {
		...
		if inhTableDesc == nil {
			return nil, fmt.Errorf(`cannot find inherit table id: %d`, id)
		}
		nuionNode, err = p.buildInheritPlanNode(ctx, inhTableDesc, cfg, names, nuionNode, inhcols, tableMap)
		if err != nil {
			return nil, err
		}
	}
	return nuionNode, nil
}
