...
ZNBaseDB key/value datastore. It is used to server basic metrics generated by
ZNBaseDB.
...
currently superior to ZNBaseDB for the purpose of time series data.

However, it is a broad goal of ZNBaseDB to provide a good experience for
...
With this in mind, ZNBaseDB downsamples data before storing it; the original
timestamp for each data point in a series is not recorded. ZNBaseDB instead
...
samples into "slab" values, with data for each slab stored in a ZNBaseDB key.
This is done by again dividing time into contiguous slots, but with a longer
duration; this is known as the "slab duration". For example, ZNBaseDB
...
however, ZNBaseDB is specialized for aggregation of the same series across
...
A hypothetical example from ZNBaseDB: we want to record the available
capacity of all stores in the cluster.

The series name is: znbase.capacity.available
...
The data is recorded into keys structurally similar to the following:
	...
	tsd.znbase.capacity.available.10s.403234.1
	tsd.znbase.capacity.available.10s.403234.2
	tsd.znbase.capacity.available.10s.403234.3
	tsd.znbase.capacity.available.10s.403235.1
	tsd.znbase.capacity.available.10s.403235.2
	tsd.znbase.capacity.available.10s.403235.3
