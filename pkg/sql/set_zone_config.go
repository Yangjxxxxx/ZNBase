...
func (n *setZoneConfigNode) startExec(params runParams) error {
	...
	if err := params.p.RequireAdminRole(params.ctx, "SET ZONE CONFIG"); err != nil {
		return err
	}
...
func writeZoneConfig(
	...
	if len(zone.Subzones) > 0 {
		...
		if !st.Version.IsActive(cluster.VersionPartitioning) {
			return 0, pgerror.NewError(pgcode.CheckViolation,
	...
	if err != nil {
		return 0, pgerror.NewErrorf(pgcode.CheckViolation,
...
func RemoveIndexZoneConfigs(
	...
	// Ignore ICL required error to allow schema change to progress.
	_, err = writeZoneConfig(ctx, txn, tableID, tableDesc, zone, execCfg, false /* hasNewSubzones */)
	if err != nil && !sqlbase.IsICLRequiredError(err) {
		return err
	}
	return nil
}
