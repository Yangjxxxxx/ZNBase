...
func (v *srfExtractionVisitor) lookupSRF(t *tree.FuncExpr) (*tree.FunctionDefinition, error) {
	fd, err := t.Func.Resolve(v.searchPath, v.p.ExtendedEvalContext().GetCaseSensitive())
	if err != nil {
		return nil, err
	}
	...
	*v = srfExtractionVisitor{
	...
	for i, target := range targets {
		...
		if len(v.srfs) != curSrfs && target.As == "" {
			...
			// original expression.
			_, newLabel, err := tree.ComputeColNameInternal(v.searchPath, target.Expr, p.ExtendedEvalContext().GetCaseSensitive())
			if err != nil {
				return nil, err
			}
