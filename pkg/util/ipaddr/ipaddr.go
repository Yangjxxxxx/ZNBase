...
var errResultOutOfRange = pgerror.NewError(pgcode.NumericValueOutOfRange, "result out of range")
...
func ParseINet(s string, dest *IPAddr) error {
	...
	if i < 0 {
		...
		ip := net.ParseIP(addr)
		if ip == nil {
			return pgerror.NewErrorf(pgcode.InvalidTextRepresentation, "could not parse %q as inet. invalid IP", s)
		}
	...
	if err != nil {
		return pgerror.NewErrorf(pgcode.InvalidTextRepresentation, "could not parse %q as inet. invalid mask", s)
	...
	if family == IPv4family {
		...
		if (octetCount+1)*8-1 < maskOnes {
			return pgerror.NewErrorf(pgcode.InvalidTextRepresentation, "could not parse %q as inet. mask is larger than provided octets", s)
		}
...
func (ipAddr *IPAddr) And(other *IPAddr) (IPAddr, error) {
	var newIPAddr IPAddr
	if ipAddr.Family != other.Family {
		return newIPAddr, pgerror.NewErrorf(pgcode.InvalidParameterValue,
...
func (ipAddr *IPAddr) SubIPAddr(other *IPAddr) (int64, error) {
	var diff int64
	if ipAddr.Family != other.Family {
		return diff, pgerror.NewErrorf(pgcode.InvalidParameterValue,
