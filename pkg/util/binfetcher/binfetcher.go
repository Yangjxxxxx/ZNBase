...
func (opts *Options) init() error {
	...
	var suffix string // only used when Binary == "znbase"
	switch opts.GOOS {
	case "darwin":
		suffix = ".tgz"
	case "linux":
	...
	if opts.URL == (url.URL{}) {
		...
		default:
			...
			opts.URL = url.URL{
				Scheme: "https",
				Host:   "edge-binaries.znbasedb.com",
...
// `version` can be:
//
// - a release, e.g. v1.0.5 (makes sense only for downloading `znbase`)
