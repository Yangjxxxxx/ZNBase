...
func (g *newRuleGen) genConstruct(construct *lang.FuncExpr) {
	if !construct.HasDynamicName() {
		...
		for _, arg := range construct.Args {
			if falseArg, ok := arg.(*lang.NameExpr); ok {
				if opName == "Cast" && *falseArg == "False" {
					g.w.writeIndent("")
					g.w.write("false,\n")
					continue
				}
...
func (g *newRuleGen) genCustomFunc(customFunc *lang.CustomFuncExpr) {
	if customFunc.Name == "OpName" {
		...
		if strings.Index(funcName, "StorageEngine") != -1 && !g.normalize {
			g.w.write("%s.CustomFuncs().%s(", g.factoryVar, funcName)
		} else {
