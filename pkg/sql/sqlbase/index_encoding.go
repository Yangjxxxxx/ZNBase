...
func EncodePartialIndexKey(
	...
	if len(index.Interleave.Ancestors) > 0 {
		for i, ancestor := range index.Interleave.Ancestors {
			...
			var n bool
			key, n, err = EncodeIndexColumns(colIDs[:length], tableDesc, index, dirs[:length], colMap, values, key)
			if err != nil {
	...
	var n bool
	key, n, err = EncodeIndexColumns(colIDs, tableDesc, index, dirs, colMap, values, key)
	containsNull = containsNull || n
	return key, containsNull, err
}
...
// MakeSpanFromEncDatumsForSpan creates a minimal index key span on the input
// values. A minimal index key span is a span that includes the fewest possible
// keys after the start key generated by the input values.
func MakeSpanFromEncDatumsForSpan(
	values EncDatumRow,
	types []ColumnType,
	dirs []IndexDescriptor_Direction,
	tableDesc TableDescriptor,
	...
	startKey, complete, containsNull, err := makeKeyFromEncDatumsForSpan(values, types, dirs, tableDesc, index, alloc, keyPrefix)
	if err != nil {
...
func NeededColumnFamilyIDs(
	...
	if table.NumFamilies() == 1 {
		return []FamilyID{table.GetFamilies()[0].ID}
	}
	...
	hasSecondaryEncoding := index.GetEncodingType(table.GetPrimaryIndexID()) == SecondaryIndexEncoding
	...
	if mvccColumnRequested {
		families := make([]FamilyID, 0, table.NumFamilies())
		_ = table.ForeachFamily(func(family *ColumnFamilyDescriptor) error {
...
// makeKeyFromEncDatumsForSpan creates an index key by concatenating keyPrefix with the
// encodings of the given EncDatum values. The values, types, and dirs
// parameters should be specified in the same order as the index key columns and
// may be a prefix. The complete return value is true if the resultant key
// fully constrains the index.
...
// key.  An example of one level of interleaving (a parent):
// /<parent_table_id>/<parent_index_id>/<field_1>/<field_2>/NullDesc/<table_id>/<index_id>/<field_3>/<family>
func makeKeyFromEncDatumsForSpan(
	values EncDatumRow,
	types []ColumnType,
	dirs []IndexDescriptor_Direction,
	tableDesc TableDescriptor,
	...
	if len(index.Interleave.Ancestors) > 0 {
		for i, ancestor := range index.Interleave.Ancestors {
			...
			key, n, err = appendEncDatumsToKeyForSpan(key, types[:length], values[:length], dirs[:length], alloc)
			if err != nil {
	...
	key, n, err = appendEncDatumsToKeyForSpan(key, types, values, dirs, alloc)
	if err != nil {
...
func findIndexColumnValue(
	...
	for i, columnid := range index.ColumnIDs {
		if columnid == column && len(index.IsFunc) > i {
			if index.IsFunc[i] {
				return values[int(column)-1]
			}
		}
	}
	return tree.DNull
...
// findFuncIndexColumnValue returns the value corresponding to the column. If
// the column isn't present return a NULL value.
func findFuncIndexColumnValue(
	desc *TableDescriptor,
	index *IndexDescriptor,
	column ColumnID,
	colMap map[ColumnID]int,
...
// appendEncDatumsToKey concatenates the encoded representations of
// the datums at the end of the given roachpb.Key.
func appendEncDatumsToKeyForSpan(
	key roachpb.Key,
	types []ColumnType,
	values EncDatumRow,
	dirs []IndexDescriptor_Direction,
...
func EncodeSecondaryIndex(
	...
	extraKey, _, err := EncodeIndexColumns(secondaryIndex.ExtraColumnIDs, tableDesc, secondaryIndex, nil,
		colMap, values, nil)
	if err != nil {
		return []IndexEntry{}, err
	}
	...
	for i, key := range secondaryKeys {
		...
		for _, col := range cols {
			val := findFuncIndexColumnValue(tableDesc, secondaryIndex, col.id, colMap, values)
			if val == tree.DNull || (col.isComposite && !val.(tree.CompositeDatum).IsComposite()) {
				continue
			}
			if lastColID > col.id {
...
func EncodeSecondaryIndexes(
	...
	var isfuncflag bool
	secondaryIndexEntries = secondaryIndexEntries[:0]
	for i, index := range indexes {
		if !ignoreIndexes.Contains(int(indexes[i].ID)) {
			...
			if isfuncflag {
				...
				entries, err := EncodeSecondaryIndex(tableDesc, &indexes[i], colMap, result)
				if err != nil {
					return secondaryIndexEntries, err
				}
				secondaryIndexEntries = append(secondaryIndexEntries, entries[0])
