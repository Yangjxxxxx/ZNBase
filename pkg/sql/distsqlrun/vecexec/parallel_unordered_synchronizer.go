...
var _ runbase.OpNode = &ParallelUnorderedSynchronizer{}
...
func (s *ParallelUnorderedSynchronizer) Child(nth int, verbose bool) runbase.OpNode {
	return s.inputs[nth]
}
...
func (s *ParallelUnorderedSynchronizer) init(ctx context.Context) {
	...
	for i, input := range s.inputs {
		...
		go func(input Operator, inputIdx int) {
			...
			for {
				if err := execerror.CatchVecRuntimeError(s.nextBatch[inputIdx]); err != nil {
					select {
