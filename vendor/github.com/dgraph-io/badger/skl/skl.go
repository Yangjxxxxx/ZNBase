...
func (s *Skiplist) NewUniIterator() *UniIterator {
	return &UniIterator{
		iter: s.NewIterator(),
		//reversed: reversed,
	}
...
func (s *UniIterator) SeekForPrev(key []byte) {
	s.iter.SeekForPrev(key)
}

func (s *UniIterator) SeekToFirst() {
	s.iter.SeekToFirst()
}

func (s *UniIterator) SeekToLast() {
	s.iter.SeekToLast()
}
