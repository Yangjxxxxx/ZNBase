...
func runSysbench(ctx context.Context, t *test, c *cluster, opts sysbenchOptions) {
	...
	c.Put(ctx, znbase, "./znbase", allNodes)
	c.Start(ctx, t, roachNodes)

	t.Status("installing haproxy")
	c.Install(ctx, loadNode, "haproxy")
	c.Run(ctx, loadNode, "./znbase gen haproxy --insecure --url {pgurl:1}")
	...
	m.Go(func(ctx context.Context) error {
		...
		c.Run(ctx, c.Node(1), `./znbase sql --insecure -e "CREATE DATABASE sysbench"`)
...
func registerSysbench(r *registry) {
	for w := sysbenchWorkload(0); w < numSysbenchWorkloads; w++ {
		...
		r.Add(testSpec{
			//Skip:    "https://github.com/znbasedb/znbase/issues/32738",
