	...
	t.Run("EncodeDecode", func(t *testing.T) {
		for i := 0; i < 100; i++ {
			...
			for _, ordering := range orderings {
				...
				func() {
					d := MakeDiskRowContainer(&diskMonitor, types, ordering, tempEngine, false)
					defer d.Close(ctx)
...
func TestDiskRowContainerDiskFull(t *testing.T) {
	...
	if pgErr, ok := pgerror.GetPGCause(err); !(ok && pgErr.Code == pgcode.DiskFull) {
...
func TestDiskRowContainerFinalIterator(t *testing.T) {
	...
	d := MakeDiskRowContainer(&diskMonitor, sqlbase.OneIntCol, nil /* ordering */, tempEngine, false)
	defer d.Close(ctx)
...
func TestDiskRowContainerUnsafeReset(t *testing.T) {
	...
	d := MakeDiskRowContainer(&monitor, sqlbase.OneIntCol, nil /* ordering */, tempEngine, false)
	defer d.Close(ctx)
