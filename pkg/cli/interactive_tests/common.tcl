...
set histfile "znbase_sql_history"

set ::env(ZNBASE_SQL_CLI_HISTORY) $histfile
# Set client commands as insecure. The server uses --insecure.
set ::env(ZNBASE_INSECURE) "true"
...
proc report {text} {
    ...
    # Also ZNBaseDB doesn't honor umask anyway.
    # So we simply come after the fact and adjust the permissions.
    #
    # The find may race with a znbase process shutting down in the
    # background; znbase might be deleting files as they are being
...
proc flush_server_logs {} {
    ...
    system "for i in `seq 1 3`; do
              grep 'hangup received, flushing logs' logs/db/logs/znbase.log && exit 0;
              echo still waiting
              sleep 1
            done
