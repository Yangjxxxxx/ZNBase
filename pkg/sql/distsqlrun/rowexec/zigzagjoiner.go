...
var _ runbase.Processor = &zigzagJoiner{}
var _ runbase.RowSource = &zigzagJoiner{}
...
func newZigzagJoiner(
	...
	err := z.joinerBase.init(
		...
		post,
		output,
		runbase.ProcStateOpts{}, // zigzagJoiner doesn't have any inputs to drain.
	)
	if err != nil {
		return nil, err
	}
...
func (z *zigzagJoiner) producerMeta(err error) *distsqlpb.ProducerMetadata {
	...
	if !z.Closed {
		...
		} else if trace := runbase.GetTraceData(z.Ctx); trace != nil {
...
func (z *zigzagJoiner) extractEqDatums(row sqlbase.EncDatumRow, side int) sqlbase.EncDatumRow {
	eqCols := z.infos[side].eqColumnIDs
...
func (z *zigzagJoiner) produceInvertedIndexKey(
	...
	keyBytes, _, err := sqlbase.EncodeIndexColumns(
...
func (zi *zigzagJoinerInfo) eqColTypes() []sqlbase.ColumnType {
	eqColTypes := make([]sqlbase.ColumnType, len(zi.eqColumnIDs))
	colTypes := zi.table.ColumnTypes()
	for i := range eqColTypes {
		eqColTypes[i] = colTypes[zi.eqColumnIDs[i]]
	}
	return eqColTypes
}
...
func (zi *zigzagJoinerInfo) eqOrdering() (sqlbase.ColumnOrdering, error) {
	...
	for i := range zi.eqColumnIDs {
		colID := zi.table.Columns[zi.eqColumnIDs[i]].ID
