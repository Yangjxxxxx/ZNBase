...
Before we get started, you need to download the ZNBaseDB source code and
...
to get the approval of someone from @znbasedb/sql-language.  There are also
some guidelines on adding new syntax that you can read about on [#17569](https://github.com/znbasedb/znbase/pull/17569).

## Adding a SQL Statement

ZNBaseDB supports for many different types of [SQL statements][statements].
...
We'll add a new statement to the ZNBase dialect of SQL: `FROBNICATE`.  This
...
```text
~/go/src/github.com/znbasedb/znbase$ make generate
...
Type checking sql.y
Compiling sql.go
...
...
```text
~/go/src/github.com/znbasedb/znbase$ make build
...
github.com/znbasedb/znbase
```

Now, let’s run a single-node ZNBase instance:

```text
$ rm -fr znbase-data/ && ./znbase start --insecure
...
status:     initialized new cluster
...
```

In another terminal window, use the `znbase sql` shell to try out our new
statement:

```text
$ ./znbase sql --insecure -e "frobnicate cluster"
...
```go
$ ./znbase sql --insecure -e 'hodgepodge bananas'
...
```text
$ ./znbase sql --insecure -e "frobnicate session; show application_name"
+------------------+
| application_name |
+------------------+
| fhqwhgads        |
...
That's it!  You've seen how to add new syntax and semantics to the ZNBaseDB
SQL parser and execution engine.

[CONTRIBUTING.md]: https://github.com/znbasedb/znbase/blob/master/CONTRIBUTING.md
[sql-function]: https://github.com/znbasedb/znbase/blob/master/docs/codelabs/00-sql-function.md
[statements]: https://www.znbaselabs.com/docs/stable/sql-statements.html
