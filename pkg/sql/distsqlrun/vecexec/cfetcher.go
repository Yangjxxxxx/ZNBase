...
import (
	...
	"github.com/znbasedb/znbase/pkg/sql/colencoding"
...
func (rf *cFetcher) Init(
	...
	for i := range typs {
		typs[i] = sqlbase.ToColType(&colDescriptors[i].Type)
	...
	cts, err := sqlbase.GetColumnTypes(table.desc.TableDesc(), indexColumnIDs)
	table.keyValTypes = sqlbase.ToTypes(cts)
	if err != nil {
		return err
	}
	if cHasExtraCols(&table) {
		...
		ctes, err := sqlbase.GetColumnTypes(table.desc.TableDesc(), table.index.ExtraColumnIDs)
		table.extraTypes = sqlbase.ToTypes(ctes)
	...
	keysPerRow := table.desc.KeysPerRow(table.index.ID)
	if err != nil {
		return err
	}
	...
	for {
		...
		case stateDecodeFirstKVOfRow:
			...
			if rf.mustDecodeIndexKey || rf.traceKV {
				...
				cts, _ := sqlbase.DatumTypesToColumnTypes(rf.table.keyValTypes)
...
func (rf *cFetcher) getDatumAt(colIdx int, rowIdx uint16, typ types.T) tree.Datum {
	return PhysicalTypeColElemToDatum(rf.machine.colvecs[colIdx], rowIdx, rf.table.da, sem.ToNewType(typ))
}
...
func (rf *cFetcher) processValue(
	...
	if rf.traceKV {
		...
		for _, idx := range rf.table.allIndexColOrdinals {
			buf.WriteByte('/')
			if idx != -1 {
				buf.WriteString(rf.getDatumAt(idx, rf.machine.rowIdx, rf.table.cols[idx].Type.ToDatumType()).String())
			} else {
				buf.WriteByte('?')
			}
		}
	...
	if !table.isSecondaryIndex {
		...
		case roachpb.ValueType_BYTES:
			...
			if cHasExtraCols(table) {
				...
				cts, _ := sqlbase.DatumTypesToColumnTypes(table.extraTypes)
...
func (rf *cFetcher) processValueSingle(
	...
	if needDecode {
		if idx, ok := table.colIdxMap.get(colID); ok {
			...
			if rf.traceKV {
				prettyValue = rf.getDatumAt(idx, rf.machine.rowIdx, typ.ToDatumType()).String()
			}
...
func (rf *cFetcher) processValueBytes(
	...
	for len(valueBytes) > 0 && rf.machine.remainingValueColsByIdx.Len() > 0 {
		...
		rf.machine.remainingValueColsByIdx.Remove(idx)
		if rf.traceKV {
			dVal := rf.getDatumAt(idx, rf.machine.rowIdx, valTyp.ToDatumType())
	...
	for i, ok := rf.machine.remainingValueColsByIdx.Next(0); ok; i, ok = rf.machine.remainingValueColsByIdx.Next(i + 1) {
		...
		if !table.cols[i].Nullable {
			...
			for _, idx := range table.indexColOrdinals {
				if idx != -1 {
					indexColValues = append(indexColValues, rf.getDatumAt(idx, rf.machine.rowIdx, rf.table.cols[idx].Type.ToDatumType()).String())
				} else {
...
func (rf *cFetcher) convertFetchError(ctx context.Context, err error) error {
	...
	err = execerror.NewStorageError(err)
	return err
}
...
func (rf *cFetcher) KeyToDesc(key roachpb.Key) (*sqlbase.ImmutableTableDescriptor, bool) {
	...
	types, err := sqlbase.DatumTypesToColumnTypes(rf.table.keyValTypes)
	if err != nil {
		return nil, false
	}
	_, ok, err := sqlbase.DecodeIndexKeyWithoutTableIDIndexIDPrefix(
		rf.table.desc.TableDesc(),
		rf.table.index,
		types,
		tableKeyVals,
		rf.table.indexColumnDirs,
