# context.Context and how it relates to logging and tracing in ZNBaseDB
...
## Motivations for contexts in ZNBaseDB

At ZNBase Labs, we use `Contexts` extensively, as a result of
...
**Go contexts enable ZNBaseDB to provide both services using a common interface.**
...
information for ZNBase operations and subsequently how the code around
tracing looks like.
...
There’s also the
`ZNBASE_TRACE_SQL` env var, which causes the distributed traces
...
- `Context` objects actually can carry a linked list of key/value pairs.
  ...
  of "current operations" in ZNBaseDB.
...
**do we track in znbase? Where do we track traces and spans?** below for
...
[extract the](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/util/log/structured.go#L111)
[“log tags”](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/util/log/structured.go#L111)
[from it](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/util/log/structured.go#L111)
...
[like so](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/storage/replica.go#L576):
...
[does](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/storage/replica.go#L1416):
...
[Enriching Log Messages Using Go Contexts](https://www.znbaselabs.com/blog/enriching-log-messages-using-go-contexts/)
blog post.
...
ZNBaseDB currently contains the LightStep tracer (a collector
...
enabled with the `ZNBASE_LIGHTSTEP_TOKEN=<project-specific token>`
env var.

Besides LightStep, ZNBaseDB also has its own tracer which, together
...
If the Lightstep integration is enabled, znbase uses both the Lightstep
and the internal tracers simultaneously - through a
`[TeeTracer](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/util/tracing/tee_tracer.go#L28)`.

As we’ve seen, all log messages also go to the current distributed
trace (if any). The logging code [passes messages to the
tracer](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/util/log/trace.go#L132),
...
it](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/util/log/trace.go#L110)
from there.

### What operations do we track in znbase? Where do we track traces and spans?

We define operations at multiple levels in znbase for tracing and
...
`[trace.EventLog](https://github.com/znbasedb/znbase/blob/e05f9ec8e47922c3ffb20570883da17544566953/pkg/sql/session.go#L199)`
[in the
session’s](https://github.com/znbasedb/znbase/blob/e05f9ec8e47922c3ffb20570883da17544566953/pkg/sql/session.go#L199)
`[Context](https://github.com/znbasedb/znbase/blob/e05f9ec8e47922c3ffb20570883da17544566953/pkg/sql/session.go#L199)`. (FIXME:
...
the sql
`[Executor](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/sql/session.go#L326)`. This
...
perform](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/kv/transport.go#L173)
an RPC and on the [server side of the
RPC](https://github.com/znbasedb/znbase/blob/281777256787cef83e7b1a8485648010442ffe48/pkg/server/node.go#L883)
...
1. Some structs are just “bags or arguments” for some method
   ...
   `[EvalContext](https://github.com/znbasedb/znbase/blob/e05f9ec/pkg/sql/parser/eval.go#L1605)`
   [](https://github.com/znbasedb/znbase/blob/e05f9ec/pkg/sql/parser/eval.go#L1605)is
...
https://github.com/znbasedb/znbase/pull/14128 to address this
...
3. `Context` are becoming more and more universal in the Go ecosystem
   ...
   ZNBaseDB might make interfacing with external code harder. (FIXME:
...
ZNBaseDB - one such open-source one is AppDash.
