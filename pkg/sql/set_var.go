...
func (p *planner) SetVar(ctx context.Context, n *tree.SetVar) (planNode, error) {
	...
	if len(n.Values) > 0 {
		...
		if !isReset {
			...
			for i, expr := range n.Values {
				...
				// maybe more exprs need to set expectedType = types.String
				switch expr.(type) {
				case *tree.Placeholder:
					expectedType = types.String
				}
				typedValue, err := p.analyzeExpr(
					ctx, expr, nil, dummyHelper, expectedType, false, "SET SESSION "+name)
				if err != nil {
	...
	if typedValues == nil {
		...
		if val := p.sessionDataMutator.defaults.Get(name); val == "" && v.GlobalDefault == nil {
			return nil, newCannotChangeParameterError(name)
		}
	}
	...
	p.curPlan.auditInfo = &server.AuditInfo{
		...
		EventType: string(EventLogSetVar),
		Result:    "OK",
		TargetInfo: &server.TargetInfo{
			Desc: struct {
				...
				queryID: p.stmt.queryID.String(),
				sql:     p.curPlan.AST.String(),
			},
		},
		Info: "User name:" + p.SessionData().User + " SHOW SYNTAX:" + p.curPlan.AST.String(),
	}
...
func (n *setVarNode) startExec(params runParams) error {
	...
	if desc, err := getDatabaseDesc(params.ctx, params.p.Txn(), strVal); err == nil {
		params.p.sessionDataMutator.UpdateSearchPath(uint32(desc.GetID()))
	}
...
func getSessionVarDefaultString(
	...
	if defVal := m.defaults.Get(varName); defVal != "" {
		return true, defVal
	}
...
func datumAsInt(evalCtx *tree.EvalContext, name string, value tree.TypedExpr) (int64, error) {
	if name == "sql_session_timeout" {
		if _, ok := value.(*tree.DString); ok {
			return 0, errors.Errorf("parameter \"server.sql_session_timeout\" requires an integer value")
		}
	}
	val, err := value.Eval(evalCtx)
	if err != nil {
	...
	if !ok {
		return 0, pgerror.NewErrorf(pgcode.InvalidParameterValue,
...
func timeZoneVarGetStringVal(
	...
	if offset == 0 { //执行set time zone "EST"(为英文缩写时)时,将偏移量写入
		timeNow := timeutil.Now().In(loc)
		_, off := timeNow.Zone()
		_ = timeutil.FixedOffsetTimeZoneToLocation(off, loc.String())
	}
...
func newVarValueError(varName, actualVal string, allowedVals ...string) *pgerror.Error {
	err := pgerror.NewErrorf(pgcode.InvalidParameterValue,
...
func newCannotChangeParameterError(varName string) error {
	return pgerror.NewErrorf(pgcode.CantChangeRuntimeParam,
