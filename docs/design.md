...
For a more approachable explanation of how ZNBaseDB works, consider reading
the [Architecture docs](https://www.znbaselabs.com/docs/stable/architecture/overview.html).

# Overview

ZNBaseDB is a distributed SQL database. The primary design goals
are **scalability**, **strong consistency** and **survivability**
(hence the name). ZNBaseDB aims to tolerate disk, machine, rack, and
even **datacenter failures** with minimal latency disruption and **no
manual intervention**. ZNBaseDB nodes are symmetric; a design goal is
...
in a ZNBaseDB cluster can act as a client SQL gateway. A SQL
...
necessary and returns results to the client. ZNBaseDB implements a
...
configurable number of additional ZNBaseDB nodes. Ranges are merged and
...
ZNBaseDB achieves horizontally scalability:
...
ZNBaseDB achieves strong consistency:
...
- logical mutations which affect multiple ranges employ distributed
  transactions for ACID semantics. ZNBaseDB uses an efficient
  **non-locking distributed commit** protocol.

ZNBaseDB achieves survivability:
...
performance. ZNBaseDB implements [a limited form of linearizability
](#strict-serializability-linearizability), providing ordering for any
observer or chain of observers.

Similar to
[Spanner](http://static.googleusercontent.com/media/research.google.com/en/us/archive/spanner-osdi2012.pdf)
directories, ZNBaseDB allows configuration of arbitrary zones of data.
...
ZNBaseDB implements a layered architecture. The highest level of
...
communicates with any number of physical znbase nodes. Each node
...
ZNBase keys are arbitrary byte arrays. Keys come in two flavors:
system keys and table data keys. System keys are used by ZNBase for
...
ZNBase maintains historical versions of values by storing them with
...
ZNBase provides distributed transactions without locks. ZNBase
...
no anomalous conditions. ZNBase’s SSI implementation is based on ideas from
...
aborting more transactions. ZNBase’s SI and SSI implementations
...
which is the source of much inspiration for ZNBase’s SSI.
...
Each ZNBase transaction is assigned a random priority and a
...
Each znbase node maintains a hybrid logical clock (HLC) as discussed
...
implementation is [here](https://github.com/znbasedb/znbase/blob/master/pkg/util/hlc/hlc.go).

ZNBase picks a Timestamp for a transaction using HLC time. Throughout this
...
the HLC time >= wall time. A read/write timestamp received in a znbase request
...
Please see [pkg/roachpb/data.proto](https://github.com/znbasedb/znbase/blob/master/pkg/roachpb/data.proto) for the up-to-date structures, the best entry point being `message Transaction`.
...
potentially limiting. ZNBase could also potentially use a global
...
interchangeably with <i>linearizability</i>) and ZNBaseDB's default
...
In practice, in ZNBaseDB many transactional workloads are actually
...
advantageous to pay for it only when it *is* needed. ZNBaseDB implements
...
by cloud providers, ZNBaseDB can provide global linearizability by doing
...
https://www.znbaselabs.com/blog/living-without-atomic-clocks/
...
elections. ZNBase weights random timeouts such that the replicas with
...
For these reasons, ZNBase introduces the concept of **Range Leases**:
...
via a HTTP endpoint, but ZNBaseDB also implements an internal timeseries
...
Please see
[pkg/config/zone.proto](https://github.com/znbasedb/znbase/blob/master/pkg/config/zone.proto)
...
Each node in a cluster can accept SQL client connections. ZNBaseDB
...
ZNBaseDB also attempts to emulate the flavor of SQL supported by
PostgreSQL, although it also diverges in significant ways:

- ZNBaseDB exclusively implements MVCC-based consistency for
...
- ZNBaseDB implements its own [SQL type system](RFCS/20160203_typing.md)
  ...
  will need to be massaged for ZNBaseDB anyways.
...
Every SQL table has a primary key in ZNBaseDB. (If a table is created
...
- _physical plan_ - a physical plan is conceptually a mapping of the _logical
  plan_ nodes to ZNBaseDB nodes. Logical plan nodes are replicated and
