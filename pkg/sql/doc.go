...
/*
Package sql provides the user-facing API for access to a ZNBase datastore. As
the name suggests, the API is based around SQL, the same SQL you find in
traditional RDBMS systems like Oracle, MySQL or Postgres. The core ZNBase
...
pair "key addressing". ZNBase's key addressing relies upon a primary key, and
...
primary key in the SQL sense, and is unrelated to the "key" in ZNBase's
...
column was non-NULL). Thus, ZNBase has the ability to group multiple columns
...
https://www.znbaselabs.com/blog/sql-znbasedb-column-families/
...
column. Instead, to process this query, ZNBase does two key-value reads,
...
transactional key-value operations. The ZNBase distributed transactional
...
description is here:
https://www.znbaselabs.com/blog/index-selection-znbasedb-2/, but back to
