...
func TestTxnPipeliner1PCTransaction(t *testing.T) {
	...
	mockSender.MockSend(func(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
		...
		require.IsType(t, &roachpb.EndTransactionRequest{}, ba.Requests[3].GetInner())

		etReq := ba.Requests[3].GetEndTransaction()
...
func TestTxnPipelinerTrackInFlightWrites(t *testing.T) {
	...
	mockSender.MockSend(func(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
		...
		etReq := ba.Requests[4].GetEndTransaction()
...
// to the write lockFootprint so they will be removed when the transaction finishes.
...
func TestTxnPipelinerEnableDisableMixTxn(t *testing.T) {
	...
	mockSender.MockSend(func(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
		...
		etReq := ba.Requests[1].GetEndTransaction()
		require.Equal(t, []roachpb.Span{{Key: keyA}}, etReq.LockSpans)
...
func TestTxnPipelinerSavepoints(t *testing.T) {
	...
	// and all rolled-back writes were moved to the write lockFootprint.
	...
	// Check that the lockFootprint was updated correctly. In addition to the "a"
