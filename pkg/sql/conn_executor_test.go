...
`); err != nil {
	...
	for _, state := range tests {
		t.Run(state, func(t *testing.T) {
			...
			if state == "CommitWait" {
				if _, err := tx.ExecContext(ctx, "SAVEPOINT znbase_restart", nil); err != nil {
					t.Fatal(err)
				}
			}
			...
			if state == "CommitWait" {
				_, err := tx.ExecContext(ctx, "RELEASE SAVEPOINT znbase_restart", nil)
				if state == "CommitWait" {
					if err != nil {
						t.Fatal(err)
					}
...
func TestAppNameStatisticsInitialization(t *testing.T) {
	...
	rows := sqlDB.Query(t, "SELECT application_name, key FROM zbdb_internal.node_statement_statistics")
	defer rows.Close()
...
func TestGetSearchPathList(t *testing.T) {
	defer leaktest.AfterTest(t)()

	var (
		in     = `s1, s2, "s,3", s4, "s*5", "s-6", "s'7", "s$7"`
		expect = []string{"s1", "s2", "s,3", "s4", "s*5", "s-6", "s'7", "s$7"}
	)
	actual := sql.GetSearchPathList(in)
	if !reflect.DeepEqual(actual, expect) {
		t.Fatalf("expected state %s, got: %s", expect, actual)
	}
}
