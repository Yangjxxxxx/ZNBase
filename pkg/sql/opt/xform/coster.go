...
// costerxq is coster for xq
type costerxq struct {
	*coster
	// locatespace for xq
	locatespace roachpb.LocationName
}
...
const (
	...
	// lookup join.
	// See https://github.com/znbasedb/znbase/pull/35561 for the initial
...
func (c *coster) ComputeCost(candidate memo.RelExpr, required *physical.Required) memo.Cost {
	...
	cost += cpuCostFactor
	//log.Errorf(context.Background(),"type: %s, cost: %f",candidate.Op(),cost)
...
func (c *costerxq) ComputeCost(candidate memo.RelExpr, required *physical.Required) memo.Cost {
	var cost memo.Cost
	cc := c.coster
	...
	case opt.SortOp:
		cost = cc.computeSortCost(candidate.(*memo.SortExpr), required)

	case opt.VirtualScanOp:
		cost = cc.computeVirtualScanCost(candidate.(*memo.VirtualScanExpr))

	case opt.SelectOp:
		cost = cc.computeSelectCost(candidate.(*memo.SelectExpr))

	case opt.ProjectOp:
		cost = cc.computeProjectCost(candidate.(*memo.ProjectExpr))

	case opt.ValuesOp:
		cost = cc.computeValuesCost(candidate.(*memo.ValuesExpr))

	case opt.InnerJoinOp, opt.LeftJoinOp, opt.RightJoinOp, opt.FullJoinOp,
		opt.SemiJoinOp, opt.AntiJoinOp, opt.InnerJoinApplyOp, opt.LeftJoinApplyOp,
		opt.RightJoinApplyOp, opt.FullJoinApplyOp, opt.SemiJoinApplyOp, opt.AntiJoinApplyOp:
		// All join ops use hash join by default.
		cost = cc.computeHashJoinCost(candidate)

	case opt.MergeJoinOp:
		cost = cc.computeMergeJoinCost(candidate.(*memo.MergeJoinExpr))

	case opt.IndexJoinOp:
		cost = cc.computeIndexJoinCost(candidate.(*memo.IndexJoinExpr))

	case opt.LookupJoinOp:
		cost = cc.computeLookupJoinCost(candidate.(*memo.LookupJoinExpr), required)

	case opt.ZigzagJoinOp:
		cost = cc.computeZigzagJoinCost(candidate.(*memo.ZigzagJoinExpr))

	case opt.UnionOp, opt.IntersectOp, opt.ExceptOp,
		opt.UnionAllOp, opt.IntersectAllOp, opt.ExceptAllOp:
		cost = cc.computeSetCost(candidate)

	case opt.GroupByOp, opt.ScalarGroupByOp, opt.DistinctOnOp:
		cost = cc.computeGroupingCost(candidate, required)

	case opt.LimitOp:
		cost = cc.computeLimitCost(candidate.(*memo.LimitExpr))

	case opt.OffsetOp:
		cost = cc.computeOffsetCost(candidate.(*memo.OffsetExpr))

	case opt.OrdinalityOp:
		cost = c.computeOrdinalityCost(candidate.(*memo.OrdinalityExpr))

	case opt.ProjectSetOp:
		cost = cc.computeProjectSetCost(candidate.(*memo.ProjectSetExpr))
	...
	cost += cpuCostFactor
	// log.Errorf(context.Background(), "type: %s, costxq: %f", candidate.Op(), cost)
...
func (c *coster) computeScanCost(scan *memo.ScanExpr, required *physical.Required) memo.Cost {
	...
	if scan.Flags.IndexHintType == keys.IndexHintForce && scan.Flags.HintIndexs[0] != scan.Index {
	...
	if scan.Flags.IndexHintType != keys.IndexNoHint {
		if scan.Flags.IndexHintType == keys.IndexHintForce {
			if scan.Index != scan.Flags.HintIndexs[0] {
				return hugeCost
			}
		}
		if scan.Flags.IndexHintType == keys.IndexHintUse {
			find := false
			for _, index := range scan.Flags.HintIndexs {
				if scan.Index == index {
					find = true
					break
				}
		...
		if scan.Flags.IndexHintType == keys.IndexHintIgnore {
			for _, index := range scan.Flags.HintIndexs {
				if scan.Index == index {
					return hugeCost
				}
...
func (c *costerxq) computeScanCost(scan *memo.ScanExpr, required *physical.Required) memo.Cost {
	// Scanning an index with a few columns is faster than scanning an index with
	// many columns. Ideally, we would want to use statistics about the size of
	// each column. In lieu of that, use the number of columns.
	if scan.Flags.IndexHintType == keys.IndexHintForce && scan.Flags.HintIndexs[0] != scan.Index {
...
func (c *coster) computeHashJoinCost(join memo.RelExpr) memo.Cost {
	...
	//if leftRowCount <= 1000 || rightRowCount <= 1000 {
	//	join.Private().(*memo.JoinPrivate).Flags.DisallowParallel = true
	//}
...
func (c *coster) computeIndexJoinCost(join *memo.IndexJoinExpr) memo.Cost {
	...
	switch t := join.Input.Private().(type) {
	case *memo.ScanPrivate:
		if t.PartialIndex {
			leftRowCount *= 0.1
		}
		break
	...
	switch t := join.Input.Private().(type) {
	case *memo.ScanPrivate:
		if c.mem.Metadata().Table(join.Table).Index(t.Index).IsFunc() {
			leftRowCount *= 0.01
		}
		break
	}
...
func (c *coster) computeLookupJoinCost(
	...
	//if required.LimitHint != 0 && leftRowCount > 0 {
	//	outputRows := join.Relational().Stats.RowCount
	//	unlimitedLookupCount := leftRowCount
	//	leftRowCount = lookupJoinInputLimitHint(unlimitedLookupCount, outputRows, required.LimitHint)
	...
	//	rowsProcessed = (rowsProcessed / unlimitedLookupCount) * leftRowCount
	//}
...
func (c *coster) rowScanCost(tabID opt.TableID, idxOrd int, numScannedCols int) memo.Cost {
	...
	if idx.IsFunc() {
		...
		if !ok {
			return memo.Cost(numCols+numScannedCols) * costFactor * 1.4
		}
		return hugeCost
...
// columns in the index and (to a lesser extent) on the number of columns we are
// scanning.
func (c *costerxq) rowScanCost(tabID opt.TableID, idxOrd int, numScannedCols int) memo.Cost {
	md := c.mem.Metadata()
	tab := md.TableXQ(tabID)
	idxXQ := tab.Index(idxOrd)
	idx := idxXQ.Index()
	numCols := idx.ColumnCount()
	...
	// zone constraints.
	var costFactor memo.Cost = cpuCostFactor
	if len(c.locatespace.Names) > 0 {
		// If 0% of locality tiers have matching constraints, then add additional
		// cost. If 100% of locality tiers have matching constraints, then add no
		// additional cost. Anything in between is proportional to the number of
		// matches.
		adjustment := 1.0 - locatespaceMatchScore(idxXQ.LocateSpace(), c.locatespace)
		costFactor += latencyCostFactor * memo.Cost(adjustment)
	}
...
// locatespaceMatchScore calculates the score of space
func locatespaceMatchScore(
	locatespace *roachpb.LocationValue, locationName roachpb.LocationName,
) float64 {
	res := 0.0
	if locatespace == nil {
		return 1
	}
	if len(locatespace.Spaces) == 0 {
		return 1
	}
	for _, space := range locationName.Names {
		for _, s := range locatespace.Spaces {
			if s == space {
				res++
			}
		}
	}
	return res / float64(len(locatespace.Spaces))
}
