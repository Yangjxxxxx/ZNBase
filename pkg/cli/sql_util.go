...
// These are translated to a message "perhaps this is not a ZNBaseDB node"
// at the top level.
...
func (c *sqlConn) getServerMetadata() (version, clusterID string, err error) {
	...
	rows, err := c.Query("SELECT * FROM zbdb_internal.node_build_info", nil)
	...
	if version == "" {
		...
		// ZNBaseDB. Use that below.
		version = "v1.0-" + v10fields[1]
		c.serverBuild = fmt.Sprintf("ZNBaseDB %s %s (%s, built %s, %s)",
...
func (c *sqlConn) getServerValue(what, sql string) (driver.Value, bool) {
	...
	if err != nil {
		if sql == `SHOW DATABASE` && err.Error() == "EOF" {
			user, dbName := cliCtx.sqlConnUser, cliCtx.sqlConnDBName
			if user == "" {
				user = security.RootUser
			}
			if dbName == "" {
				dbName = sessiondata.DefaultDatabaseName
			}
			fmt.Fprintf(stderr, "Fatal: %s\n", sqlbase.NewNoAccessDBPrivilegeError(user, dbName))
			os.Exit(1)
		}
...
// sqlTxnShim implements the zbdb.Tx interface.
//
// It exists to support zbdb.ExecuteInTxn. Normally, we'd hand zbdb.ExecuteInTxn
...
func (c *sqlConn) ExecTxn(fn func(*sqlConn) error) (err error) {
	...
	return zbdb.ExecuteInTx(context.TODO(), sqlTxnShim{c}, func() error {
		return fn(c)
	})
}
...
var sqlConnTimeout = envutil.EnvOrDefaultString("ZNBASE_CONNECT_TIMEOUT", "5")
...
func runQueryAndFormatResults(conn *sqlConn, w io.Writer, fn queryFunc) error {
	...
	for {
		...
		if sqlCtx.showTimes {
			...
			// the 1st result; this is subject to change once ZNBaseDB
...
func getNextRowStrings(rows *sqlRows, showMoreChars bool) ([]string, error) {
	...
	rowStrings := make([]string, len(cols))
	for i, v := range vals {
		rowStrings[i] = formatValue(&ValueWrapper{value: v, columnTypeDatabaseTypeName: rows.ColumnTypeDatabaseTypeName(i)},
...
// ValueWrapper format value for select column type is date, time.datetime.
type ValueWrapper struct {
	value                      driver.Value
	columnTypeDatabaseTypeName string
}

func formatValue(val *ValueWrapper, showPrintableUnicode bool, showNewLinesAndTabs bool) string {
	...
	case time.Time:
		switch val.columnTypeDatabaseTypeName {
		case "DATE":
			return t.Format(tree.DateFormat)
		case "TIME":
			return t.Format(tree.TimeFormat)
		case "TIMESTAMP":
			return t.Format(tree.TimestampFormat)
		}
