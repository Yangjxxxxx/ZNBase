...
// Specify an explicit empty prefix for zbdb_internal to avoid an error if
// the database we're connected to does not exist.
const numNodesQuery = `SELECT count(node_id) FROM "".zbdb_internal.gossip_liveness`
...
func importFixtureTable(
	...
	fmt.Fprintf(&buf, `LOAD TABLE "%s"."%s" %s CSV DATA (`, dbName, table.Name, table.Schema)
	...
	if directIngestion {
		buf.WriteString(`, direct_ingestion`)
	}
