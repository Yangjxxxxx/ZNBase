...
func (p *planner) ShowRanges(ctx context.Context, n *tree.ShowRanges) (planNode, error) {
	...
	return p.delegateQuery(ctx, "SHOW RANGES",
		fmt.Sprintf(`
SELECT 
  CASE WHEN r.start_key <= x'%s' THEN NULL ELSE zbdb_internal.pretty_key(r.start_key, 2) END AS start_key,
  CASE WHEN r.end_key >= x'%s' THEN NULL ELSE zbdb_internal.pretty_key(r.end_key, 2) END AS end_key,
  range_id,
  replicas,
  lease_holder,
  range_size / 1048576 as range_size_mb
FROM zbdb_internal.ranges AS r
