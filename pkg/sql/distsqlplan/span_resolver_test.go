...
func TestSpanResolverUsesCaches(t *testing.T) {
	...
	lr := distsqlplan.NewSpanResolver(
		s3.Cfg.Settings,
		s3.DistSender().(*kv.DistSender), s3.Gossip(), s3.GetNode().Descriptor, nil,
		replicastrategy.BinPackingChoice)
...
func TestSpanResolver(t *testing.T) {
	...
	lr := distsqlplan.NewSpanResolver(
		...
		s.DistSender().(*kv.DistSender), s.Gossip(),
...
func setupRanges(
	...
	tableDesc := sqlbase.GetTableDescriptor(cdb, "t", "public", "test")
