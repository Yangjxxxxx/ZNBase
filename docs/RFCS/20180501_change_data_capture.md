...
ZNBaseDB is an excellent system of record, but no technology exists in a
...
Changefeeds scale to any size ZNBaseDB cluster and are designed to impact
...
to reconstruct them. (See [Transaction grouped changes] for why ZNBaseDB
...
- INTO `<sink>` is a URI that contains all the configuration information needed
  ...
  - `kafka://host:port` Kafka is used as the sink. The bootstrap server is
    ...
    - `?confluent_schema_registry=<address>` The address of a schema registry
      ...
      registry is used.
  - `experimental-sql:///` A [SQL table sink] in the same ZNBaseDB cluster.
- `WITH <...>`
...
ZNBaseDB doesn't (yet) offer one, so MovR uses CDC to keep vehicle
descriptions and their current availability in [ElasticSearch].

The authoritative store for vehicles is a table in ZNBaseDB:
...
needs to run once per minute to keep prices up to date. ZNBaseDB is not (yet)
...
For efficiency, in ZNBaseDB and in other databases, truncate is implemented
...
sharded PostgreSQL (for example), ZNBaseDB supports transactions touching any
...
As a result, we’ve decided to give ZNBaseDB users the information necessary
...
ZNBaseDB's distributed ranges and replicas.
...
an alias if we build support for ZNBaseDB to ZNBaseDB replication.



[#2656]: https://github.com/znbasedb/znbase/issues/2656
[#6130]: https://github.com/znbasedb/znbase/issues/6130
[amazon redshift]: https://aws.amazon.com/redshift/
[avro]: #appendix-avro-background
[rangefeed]: https://github.com/znbasedb/znbase/blob/381e4dafa596c5f3621a48fcb5fce1f62b18c186/docs/RFCS/20170613_change_feeds_storage_primitive.md
[column family]: https://www.znbaselabs.com/docs/stable/column-families.html
...
[distsql]: https://github.com/znbasedb/znbase/blob/381e4dafa596c5f3621a48fcb5fce1f62b18c186/docs/RFCS/20160421_distributed_sql.md
...
[follower reads]: https://github.com/znbasedb/znbase/pull/26362
[full-text index example: elasticsearch]: #full-text-index-example-elasticsearch
[garbage collection]: https://www.znbaselabs.com/docs/stable/architecture/storage-layer.html#garbage-collection
[incremental backups]: https://www.znbaselabs.com/docs/stable/backup.html
...
[proposed and sent through raft]: https://github.com/znbasedb/znbase/blob/381e4dafa596c5f3621a48fcb5fce1f62b18c186/docs/RFCS/20160420_proposer_evaluated_kv.md
[schema resolution]: http://avro.apache.org/docs/current/spec.html#Schema+Resolution
[sql table sink]: #sql-table-sink
[system jobs]: https://github.com/znbasedb/znbase/blob/381e4dafa596c5f3621a48fcb5fce1f62b18c186/docs/RFCS/20170215_system_jobs.md
[transaction grouped changes]: #no-transaction-grouped-changes
[follower read]: https://github.com/znbasedb/znbase/pull/19222
