...
- RFC PR: [#20149](https://github.com/znbasedb/znbase/pull/20149)
- ZNBase Issue: [#20371](https://github.com/znbasedb/znbase/issues/20371)
...
ZNBase administrators can create a new role called `myrole` using:
```
CREATE ROLE myrole
```
...
* znbase administrator (`admin` role)
...
ZNBase administrators can delete a role called `myrole` using:
```
DROP ROLE myrole
```
...
Before this RFC, there is a single administrative user in a znbase cluster: the user `root`.
...
Closing this loop-hole is out of scope for this document. See [tracking issue #19277](https://github.com/znbasedb/znbase/issues/19277).
...
The `pg_authid` virtual table is not currently implemented in ZNBase.
...
[Existing issue](https://github.com/znbasedb/znbase/issues/2939).
