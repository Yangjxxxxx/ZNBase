...
rocksdb::Status KeyFromFile(rocksdb::Env* env, const std::string& path,
  ...
  if (path == kFilenamePlain) {
    key->set_key("");
    info->set_encryption_type(enginepbicl::Plaintext);
  ...
  case 16:
    ...
    break;
  case 24:
    info->set_encryption_type(enginepbicl::AES192_CTR);
    break;
  case 32:
    info->set_encryption_type(enginepbicl::AES256_CTR);
    break;
  default:
...
rocksdb::Status KeyFromKeyInfo(rocksdb::Env* env, const enginepbicl::KeyInfo& store_info,
                               enginepbicl::SecretKey* key) {
  int64_t now;
  auto status = env->GetCurrentTime(&now);
  if (!status.ok()) {
  ...
  if (store_info.encryption_type() == enginepbicl::Plaintext) {
    key->set_key("");
    info->set_key_id(kPlainKeyID);
...
rocksdb::Status ValidateRegistry(enginepbicl::DataKeysRegistry* registry) {
...
rocksdb::Status GenerateDataKey(rocksdb::Env* env, enginepbicl::DataKeysRegistry* reg) {
...
std::string AlgorithmEnumToString(const enginepbicl::EncryptionType t) {
  switch (t) {
  case enginepbicl::AES128_CTR:
    return "AES128-CTR";
    break;
...
std::string StoreKeyInfoSummary(const enginepbicl::KeyInfo& info) {
  if (info.encryption_type() == enginepbicl::Plaintext) {
    return "plain";
  }
...
std::string DataKeyInfoSummary(const enginepbicl::KeyInfo& info) {
  if (info.encryption_type() == enginepbicl::Plaintext) {
    return "plain";
  }
...
rocksdb::Status FileKeyManager::LoadKeys() {
  std::shared_ptr<enginepbicl::SecretKey> active(new enginepbicl::SecretKey());
  rocksdb::Status status = KeyManagerUtils::KeyFromFile(env_, active_key_path_, active.get());
  if (!status.ok()) {
    return status;
  }

  std::shared_ptr<enginepbicl::SecretKey> old(new enginepbicl::SecretKey());
...
std::shared_ptr<enginepbicl::SecretKey> FileKeyManager::CurrentKey() { return active_key_; }

std::shared_ptr<enginepbicl::SecretKey> FileKeyManager::GetKey(const std::string& id) {
...
rocksdb::Status DataKeyManager::LoadKeysHelper(enginepbicl::DataKeysRegistry* registry) {
...
rocksdb::Status DataKeyManager::LoadKeys() {
  ...
  std::unique_ptr<enginepbicl::DataKeysRegistry> registry(new enginepbicl::DataKeysRegistry());
...
std::shared_ptr<enginepbicl::SecretKey> DataKeyManager::CurrentKey() {
  std::unique_lock<std::mutex> l(mu_);
...
std::shared_ptr<enginepbicl::SecretKey> DataKeyManager::CurrentKeyLocked() {
  assert(registry_ != nullptr);
  ...
  return std::shared_ptr<enginepbicl::SecretKey>(new enginepbicl::SecretKey(iter->second));
}

std::shared_ptr<enginepbicl::SecretKey> DataKeyManager::GetKey(const std::string& id) {
  std::unique_lock<std::mutex> l(mu_);
  ...
  return std::shared_ptr<enginepbicl::SecretKey>(new enginepbicl::SecretKey(key->second));
}

std::unique_ptr<enginepbicl::KeyInfo> DataKeyManager::GetActiveStoreKeyInfo() {
  std::unique_lock<std::mutex> l(mu_);
  ...
  return std::unique_ptr<enginepbicl::KeyInfo>(new enginepbicl::KeyInfo(iter->second));
}

std::unique_ptr<enginepbicl::DataKeysRegistry> DataKeyManager::GetScrubbedRegistry() const {
  std::unique_lock<std::mutex> l(mu_);
  ...
  auto new_registry =
      std::unique_ptr<enginepbicl::DataKeysRegistry>(new enginepbicl::DataKeysRegistry(*registry_));
...
rocksdb::Status DataKeyManager::MaybeRotateKeyLocked() {
  ...
  if (current_key_->info().encryption_type() == enginepbicl::Plaintext) {
...
rocksdb::Status
DataKeyManager::SetActiveStoreKeyInfo(std::unique_ptr<enginepbicl::KeyInfo> store_info) {
  std::unique_lock<std::mutex> l(mu_);
...
rocksdb::Status
DataKeyManager::PersistRegistryLocked(std::unique_ptr<enginepbicl::DataKeysRegistry> reg) {
  if (read_only_) {
