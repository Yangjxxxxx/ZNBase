...
func verifySystemJob(
	...
	db.QueryRow(t, `
		...
		FROM zbdb_internal.jobs WHERE job_type = $1 ORDER BY created LIMIT 1 OFFSET $2`,
		filterType.String(),
		offset,
	).Scan(
	...
	suffix := "WITH allfiles"
	if !strings.HasSuffix(expected.Description, suffix) {
...
func GetJobID(t testing.TB, db *sqlutils.SQLRunner, offset int) int64 {
	var jobID int64
	db.QueryRow(t, `
	SELECT job_id FROM zbdb_internal.jobs ORDER BY created LIMIT 1 OFFSET $1`, offset,
	).Scan(&jobID)
	return jobID
}
...
func QueryRecentJobID(db *gosql.DB, offset int) (int64, error) {
	var jobID int64
	err := db.QueryRow(
		`SELECT job_id FROM zbdb_internal.jobs ORDER BY created DESC LIMIT 1 OFFSET $1`, offset,
	).Scan(&jobID)
	return jobID, err
}
