...
func TestOrderedSync(t *testing.T) {
	...
	for testIdx, c := range testCases {
		var sources []runbase.RowSource
		for _, srcRows := range c.sources {
			rowBuf := runbase.NewRowBuffer(sqlbase.ThreeIntCols, srcRows, runbase.RowBufferArgs{})
...
func TestOrderedSyncDrainBeforeNext(t *testing.T) {
	...
	var sources []runbase.RowSource
	for i := 0; i < 4; i++ {
		rowBuf := runbase.NewRowBuffer(sqlbase.OneIntCol, nil /* rows */, runbase.RowBufferArgs{})
		sources = append(sources, rowBuf)
...
func TestUnorderedSync(t *testing.T) {
	...
	mrc.InitWithNumSenders([]sqlbase.ColumnType{sqlbase.IntType}, 5)
	producerErr := make(chan error, 100)
	for i := 1; i <= 5; i++ {
		go func(i int) {
			for j := 1; j <= 100; j++ {
				a := sqlbase.DatumToEncDatum(sqlbase.IntType, tree.NewDInt(tree.DInt(i)))
				b := sqlbase.DatumToEncDatum(sqlbase.IntType, tree.NewDInt(tree.DInt(j)))
				row := sqlbase.EncDatumRow{a, b}
				if status := mrc.Push(row, nil /* meta */); status != runbase.NeedMoreRows {
