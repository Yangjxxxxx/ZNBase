...
func (p *joinPredicate) GetVisibleType(idx int) string {
	return ""
}
...
func (p *joinPredicate) SetForInsertOrUpdate(b bool) {
	p.IsInsertOrUpdate = b
}

// GetForInsertOrUpdate implements the parser.IndexedVarContainer interface.
func (p *joinPredicate) GetForInsertOrUpdate() bool {
...
// prepareRow prepares the output row by comznbaseng values from the
// input data sources.
...
func makeUsingColumns(
	...
	for _, syntaxColName := range usingColNames {
		...
		if _, ok := seenNames[colName]; ok {
			return nil, pgerror.NewErrorf(pgcode.DuplicateColumn,
...
func pickUsingColumn(
	...
	if idx == invalidColIdx {
		return idx, nil, pgerror.NewErrorf(pgcode.UndefinedColumn,
