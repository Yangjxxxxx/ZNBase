...
func main() {
	...
	if err := template.Must(template.New("").Parse(tmpl1)).Execute(os.Stdout, data1); err != nil {
		log.Fatal(err)
	}
}
...
var categories = map[string]string{
	...
	"znbasedb_extra_type_func_name_keyword:": "T",
	"reserved_keyword:":                      "R",
	"znbasedb_extra_reserved_keyword:":       "R",
}
...
const tmpl1 = `
// GetResKeywordID returns the lex id of the SQL reserved_keyword k or IDENT if k is
// not a reserved_keyword.
func GetResKeywordID(k string) int32 {
	// The previous implementation generated a map that did a string ->
	// id lookup. Various ideas were benchmarked and the implementation below
	// was the fastest of those, between 3% and 10% faster (at parsing, so the
	// scanning speedup is even more) than the map implementation.
