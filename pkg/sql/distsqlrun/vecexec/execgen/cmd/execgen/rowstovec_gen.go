...
func genRowsToVec(wr io.Writer) error {
	f, err := ioutil.ReadFile("pkg/sql/distsqlrun/vecexec/rowstovec_tmpl.go")
	if err != nil {
		return err
	}
	...
	s = strings.Replace(s, "_FAMILY", "types1.{{.Family}}", -1)
	...
	conversionsMap := make(map[types1.Family]*columnConversion)
	for _, ct := range types1.OidToType {
		t := sem.FromColumnType(ct)
