...
func (expr *NvlExpr) normalize(v *NormalizeVisitor) TypedExpr {
...
func (expr *CharExpr) normalize(v *NormalizeVisitor) TypedExpr {
	last := len(expr.Exprs) - 1
	for i := range expr.Exprs {
		subExpr := expr.TypedExprAt(i)
...
func (expr *UnaryExpr) normalize(v *NormalizeVisitor) TypedExpr {
	...
	case UnaryMinus:
		...
		if val.ResolvedType() != types.Float && val.ResolvedType() != types.Float4 && v.isNumericZero(val) {
			return val
		}
		switch b := val.(type) {
		// -(a - b) -> (b - a)
...
func (expr *ComparisonExpr) normalize(v *NormalizeVisitor) TypedExpr {
	...
	case EQ, GE, GT, LE, LT:
		...
		for {
			...
			// comparison comznbaseng portions that are const.
			...
			case expr.Operator == EQ && left.Operator == JSONFetchVal && v.isConst(left.Right) &&
				...
				typedJ, err := dj.TypeCheck(nil, types.JSON, false)
				if err != nil {
					break
				}
