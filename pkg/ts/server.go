...
//ReportQuery get ReportQueryRequest which contains the requests of the startTime and
//endTime from the AdminUI to import reports between time,the generated reports now
//can be seen in the ~/znbase-data/reports
func (s *Server) ReportQuery(
	ctx context.Context, req *tspb.ReportQueryRequest,
) (*tspb.ReportQueryResponse, error) {

	//the start and end time need to be format to UnixNano to match the request of Query
	startTime, err := time.Parse("2006-01-02T15:04:05.000Z", req.StartNanos)
	if err != nil {
		return nil, err
	}

	endTime, err := time.Parse("2006-01-02T15:04:05.000Z", req.EndNanos)
	if err != nil {
		return nil, err
	}

	//first send a test query to get the number of the Sources,because
	//the Sample data between the chosen period may not equal to the
	//number of nodes
	testRequest := testQuery(startTime.UnixNano(), endTime.UnixNano())

	testRes, err := s.Query(ctx, testRequest)
	if err != nil {
		return nil, err
	}
	sampleNum := len(testRes.Results[0].Query.Sources)

	//if there if at least one Sample in the source,generate the query list and send to the Query
	var ret tspb.ReportQueryResponse
	if sampleNum > 0 {
		request := defaultQuery(sampleNum, startTime.UnixNano(), endTime.UnixNano())
		...
		//write the report pdf
		err = PdfExporterTs(result, sampleNum, s)
		if err != nil {
			return nil, err
		}

		message := "report generated"
		ret = tspb.ReportQueryResponse{Info: &message}
	} else {
		message := "report can't generated"
		ret = tspb.ReportQueryResponse{Info: &message}
	}
	return &ret, nil
}
