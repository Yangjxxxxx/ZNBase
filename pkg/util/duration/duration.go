...
// DateTruncExtract date_trunc
func DateTruncExtract(sText string, dDuration Duration) (string, error) {
	nDays := dDuration.Days
	nMons := dDuration.Months
	nNanos6 := dDuration.Nanos() / nDTn2
	nNanos := nNanos6 / nDTn1
	sResult := "00:00:00"
	var sYears, sQuarters, sMonths, sDays, sTemp, sInterval string
	var nYears, nMonths, nHours, nMinutes, nSeconds int64 = 0, 0, 0, 0, 0
	sHours, sMinutes, sSeconds := "00", "00", "00"
	bFlag := false
	switch sText {
	case "millennium":
		{
			nYears = nMons / (MonthsPerYear * nanosInMicro) * nanosInMicro
			break
		}
	case "century":
		{
			nYears = nMons / (MonthsPerYear * nDTn2) * nDTn2
			break
		}
	case "decade":
		{
			nYears = nMons / (MonthsPerYear * nDTn3) * nDTn3
			break
		}
	case "year":
		{
			nYears = nMons / MonthsPerYear
			break
		}
	case "quarter":
		{
			nYears = nMons / MonthsPerYear
			nQuarters := (nMons - (MonthsPerYear * nYears)) / 3
			if nQuarters > 0 {
				sQuarters = strconv.FormatInt(nQuarters*3, 10) + " " + "mons"
			}
			break
		}
	case "month":
		{
			nYears = nMons / MonthsPerYear
			nMonths = (nMons - (MonthsPerYear * nYears))
			break
		}
	case "day":
		{
	...
	case "minute":
		{
			...
			bFlag = true
			nHours = nNanos / (nDTn4 * nDTn4)
			nMinutes = (nNanos - nHours*(nDTn4*nDTn4)) / nDTn4
			nMinutes = (nNanos - nHours*(nDTn4*nDTn4)) / nDTn4
			break
		}
	case "second":
		{
			...
			nHours = nNanos / (nDTn4 * nDTn4)
			nMinutes = (nNanos - nHours*(nDTn4*nDTn4)) / nDTn4
			nSeconds = (nNanos - nHours*(nDTn4*nDTn4) - nMinutes*nDTn4)
			break
		}
	case "milliseconds":
		{
			...
			nMinutes = (nNanos - nHours*(nDTn4*nDTn4)) / nDTn4
			nSeconds = (nNanos - nHours*(nDTn4*nDTn4) - nMinutes*nDTn4)
			nTemp := int64(float64((nNanos6-(nNanos*nDTn1))/(nanosInMicro*nDTn3)) + 0.5)
			sTemp = "." + strconv.FormatInt(nTemp, 10)
			break
		}
	case "microseconds":
		{
			...
			nMinutes = (nNanos - nHours*(nDTn4*nDTn4)) / nDTn4
			nSeconds = (nNanos - nHours*(nDTn4*nDTn4) - nMinutes*nDTn4)
			nTemp := int64(float64((nNanos6-(nNanos*nDTn1))/nDTn3) + 0.5)
			sTemp = "." + strconv.FormatInt(nTemp, 10)
			break
		}
	default:
		return "", pgerror.NewErrorf(pgcode.InvalidParameterValue, "Params Is Undefined")
	}
	if nYears == 1 {
		sYears = fmt.Sprintf("%s %s", strconv.FormatInt(nYears, 10), "year")
	} else if nYears > 1 {
		sYears = fmt.Sprintf("%s %s", strconv.FormatInt(nYears, 10), "years")
	}
	if nMonths == 1 {
		sMonths = fmt.Sprintf("%s %s", strconv.FormatInt(nMonths, 10), "mon")
	} else if nMonths > 1 {
		sMonths = fmt.Sprintf("%s %s", strconv.FormatInt(nMonths, 10), "mons")
	}
	if bFlag && nDays == 1 {
		sDays = fmt.Sprintf("%s %s", strconv.FormatInt(nDays, 10), "day")
	} else if bFlag && nDays > 1 {
		sDays = fmt.Sprintf("%s %s", strconv.FormatInt(nDays, 10), "days")
	}
	if nHours > 0 {
		sHours = fmt.Sprintf("%02d", nHours)
	}
	if nMinutes > 0 {
		sMinutes = fmt.Sprintf("%02d", nMinutes)
	}
	if nSeconds > 0 {
		sSeconds = fmt.Sprintf("%02d", nSeconds)
	}
	sInterval = fmt.Sprintf("%s:%s:%s%s", sHours, sMinutes, sSeconds, sTemp)
	if sText == "hour" || sText == "minute" || sText == "second" ||
		sText == "milliseconds" || sText == "microseconds" {
		sResult = fmt.Sprintf("%s %s %s %s %s", sYears, sQuarters, sMonths, sDays, sInterval)
	} else {
		sResult = fmt.Sprintf("%s %s %s %s", sYears, sQuarters, sMonths, sDays)
	}
