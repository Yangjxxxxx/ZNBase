...
func newReplica(rangeID roachpb.RangeID, desc *roachpb.RangeDescriptor, store *Store) *Replica {
	r := &Replica{
...
func (r *Replica) setDescLockedRaftMuLocked(ctx context.Context, desc *roachpb.RangeDescriptor) {
	...
	if bytes.HasPrefix(desc.StartKey, keys.NodeLivenessPrefix) {
		r.store.tickScheduler.SetPriorityID(desc.RangeID)
		r.store.readyrequestScheduler.SetPriorityID(desc.RangeID)
	}
}
