...
func wrapRowSources(
	...
	newToWrap func([]runbase.RowSource) (runbase.RowSource, error),
) (*Columnarizer, error) {
	var (
		toWrapInputs []runbase.RowSource
...
type NewColOperatorArgs struct {
	...
	ProcessorConstructor               runbase.ProcessorConstructor
}
...
func createJoiner(
	...
	) (*distsqlpb.Expression, filterPlanningState, []uint32, []uint32, error),
) error {
	spec := args.Spec
	inputs := args.Inputs
	...
	leftTypes, err = conv.FromColumnType1s(spec.Input[0].ColumnTypes)
	if err != nil {
		return err
	}
	rightTypes, err = conv.FromColumnType1s(spec.Input[1].ColumnTypes)
	if err != nil {
		return err
	}
...
func NewColOperator(
	ctx context.Context, flowCtx *runbase.FlowCtx, args NewColOperatorArgs,
	...
	supported := spec.VecComputeSupported
	if supported == 1 {
		...
		for _, input := range spec.Input {
			typs1, error := sqlbase.ToType1s(input.ColumnTypes)
			if error != nil {
				return result, errors.Newf("conv.ToType1s is not supported")
			}
			inputTypes = append(inputTypes, typs1)
		}
		...
		c, err = wrapRowSources(
			...
			inputTypes,
			streamingMemAccount,
			func(inputs []runbase.RowSource) (runbase.RowSource, error) {
				...
				proc, err := processorConstructor(
					...
					[]runbase.RowReceiver{nil}, /* outputs */
					nil,                        /* localProcessors */
				)
				if err != nil {
					return nil, err
				...
				var (
					rs runbase.RowSource
					ok bool
				)
				if rs, ok = proc.(runbase.RowSource); !ok {
					return nil, errors.Newf(
						"processor %s is not an RowSource", core.String(),
					)
				}
				...
				opTypes, error := sqlbase.ToType1s(rs.OutputTypes())
				if error != nil {
					return nil, nil
				}
				result.ColumnTypes = opTypes
				return rs, nil
			},
		)
		...
		case core.Noop != nil:
			...
			cts, error := sqlbase.ToType1s(spec.Input[0].ColumnTypes)
			if error != nil {
				return result, nil
			}
			result.ColumnTypes = cts
		case core.TableReader != nil:
			...
			cts, error := sqlbase.ToType1s(core.TableReader.Table.ColumnTypesWithMutations(returnMutations))
			if error != nil {
				return result, error
			}
			result.ColumnTypes = cts
		case core.Aggregator != nil:
			...
			if len(aggSpec.Aggregations) == 0 {
				...
				result.ColumnTypes = make([]types1.T, 0)
				break
			}
			if len(aggSpec.GroupCols) == 0 &&
				...
				result.ColumnTypes = []types1.T{*types1.Int}
				break
			}
			...
			result.ColumnTypes = make([]types1.T, len(aggSpec.Aggregations))
			for i, agg := range aggSpec.Aggregations {
				...
				aggFns[i] = agg.Func
				cts, err := sqlbase.DatumType1sToColumnTypes(aggTyps[i])
				if err != nil {
					return result, nil
				}
				_, retType, err := distsqlpb.GetAggregateInfo(agg.Func, cts...)
				if err != nil {
					return result, err
				}
				result.ColumnTypes[i] = *sqlbase.FromColumnTypeToT(retType)
			}
			var typs []coltypes.T
			typs, err = conv.FromColumnType1s(spec.Input[0].ColumnTypes)
			if err != nil {
				return result, err
			}
			if needHash {
				...
				result.Op, err = NewHashAggregator(
					...
					aggSpec.GroupCols, aggCols, distsqlpb.IsScalarAggregate(aggSpec),
				)
			} else {
		...
		case core.Distinct != nil:
			...
			ct1s, error := sqlbase.ToType1s(spec.Input[0].ColumnTypes)
			if error != nil {
				return result, error
			}
			result.ColumnTypes = ct1s
			var typs []coltypes.T
			typs = sqlbase.ToColTypes(spec.Input[0].ColumnTypes)
			if err != nil {
				return result, err
			}
		...
		case core.Ordinality != nil:
			...
			result.ColumnTypes = append(ct1s, *types1.Int)
			...
			createMergeJoinerWithOnExprPlanning := func(
				...
				if !core.MergeJoiner.OnExpr.Empty() {
					...
					case sqlbase.JoinType_LEFT_SEMI, sqlbase.JoinType_LEFT_ANTI:
						...
						cts1, _ := sqlbase.ToType1s(spec.Input[0].ColumnTypes)
						cts2, _ := sqlbase.ToType1s(spec.Input[1].ColumnTypes)
						filterConstructor = func(op Operator) (Operator, error) {
							r := NewColOperatorResult{
								Op:          op,
								ColumnTypes: append(cts1, cts2...),
							}
		...
		case core.Sorter != nil:
			...
			input := inputs[0]
			ct1s, _ := sqlbase.ToType1s(spec.Input[0].ColumnTypes)
			result.ColumnTypes = ct1s
			var inputTypes []coltypes.T
			inputTypes = sqlbase.ToColTypes(spec.Input[0].ColumnTypes)
			if err != nil {
				return result, err
			}
			...
			cts, _ := sqlbase.ToType1s(spec.Input[0].ColumnTypes)
			result.ColumnTypes = cts
	...
	if post.Projection {
		...
		for _, expr := range post.RenderExprs {
			var (
				helper            runbase.ExprHelper
				renderInternalMem int
			)
			cts, _ := sqlbase.DatumType1sToColumnTypes(result.ColumnTypes)
			err := helper.Init(expr, cts, flowCtx.EvalCtx)
			if err != nil {
				return result, err
			}
			var outputIdx int
		...
		newTypes := make([]types1.T, 0, len(renderedCols))
...
// from the right side.
func (p *filterPlanningState) isFilterOnlyOnLeft(filter distsqlpb.Expression) (bool, error) {
...
func (p *filterPlanningState) projectOutExtraCols(result *NewColOperatorResult) {
	...
	// Update output column types1 according to the projection.
	newTypes := make([]types1.T, 0, len(projection))
...
func (r *NewColOperatorResult) createBufferingMemAccount(
	ctx context.Context, flowCtx *runbase.FlowCtx, name string,
) *mon.BoundAccount {
	bufferingOpMemMonitor := runbase.NewLimitedMonitor(
...
// setProjectedByJoinerColumnTypes sets column types1 on r according to a
// joiner handled projection.
// NOTE: r.ColumnTypes is updated.
func (r *NewColOperatorResult) setProjectedByJoinerColumnTypes(
	spec *distsqlpb.ProcessorSpec, leftOutCols, rightOutCols []uint32,
) {
	r.ColumnTypes = make([]types1.T, 0, len(leftOutCols)+len(rightOutCols))
	for _, leftOutCol := range leftOutCols {
		r.ColumnTypes = append(r.ColumnTypes, *sqlbase.FromColumnTypeToT(spec.Input[0].ColumnTypes[leftOutCol]))
	}
	for _, rightOutCol := range rightOutCols {
		r.ColumnTypes = append(r.ColumnTypes, *sqlbase.FromColumnTypeToT(spec.Input[1].ColumnTypes[rightOutCol]))
	}
}
...
func (r *NewColOperatorResult) planFilterExpr(
	...
	var filterColumnTypes []types1.T
...
func planSelectionOperators(
	...
	input Operator,
	acc *mon.BoundAccount,
) (op Operator, resultIdx int, ct []types1.T, internalMemUsed int, err error) {
	switch t := expr.(type) {
	...
	case *tree.ComparisonExpr:
		...
		if constArg, ok := t.Right.(tree.Datum); ok {
			...
			constColType := t.TypedRight().ResolvedType()
			op, err := GetSelectionConstOperator(lTyp, sem.ToNewType(constColType), cmpOp, leftOp, leftIdx, constArg)
			return op, resultIdx, ct, internalMemUsedLeft, err
		}
...
func planTypedMaybeNullProjectionOperators(
	...
	if expr == tree.DNull {
		...
		op = NewConstNullOp(NewAllocator(ctx, acc), input, resultIdx, sem.FromColumnType(exprTyp))
...
// of the expression's result (if any, otherwise -1) and the column types1 of the
// resulting batches.
func planProjectionOperators(
	...
	case *tree.ComparisonExpr:
		return planProjectionExpr(ctx, evalCtx, t.Operator, sem.ToNewType(t.ResolvedType()), t.TypedLeft(), t.TypedRight(), columnTypes, input, acc)
	...
	case *tree.CastExpr:
		...
		if sem.ToNewType(expr.ResolvedType()) == types1.Unknown {
			op, resultIdx, ct, internalMemUsed, err = planTypedMaybeNullProjectionOperators(ctx, evalCtx, expr, sem.CastTypeToType1(t.Type), columnTypes, input, acc)
		} else {
		...
		outputIdx := len(ct)
		fromTyp := expr.ResolvedType()
		op, err = GetCastOperator(NewAllocator(ctx, acc), op, resultIdx, outputIdx, sem.ToNewType(fromTyp), sem.ToNewType(*sem.CastTypeToType(t.Type)))
		ct = append(ct, *sem.CastTypeToType1(t.Type))
	...
	case *tree.FuncExpr:
		...
		resultIdx = len(ct)
		ct = append(ct, *sem.ToNewType(funcOutputType))
	...
	case tree.Datum:
		...
		ct = columnTypes
		resultIdx = len(ct)
		ct1 := sem.ToNewType(datumType)
		ct = append(ct, *ct1)
		if ct1.Family() == types1.UnknownFamily {
			return nil, resultIdx, ct, internalMemUsed, errors.New("cannot plan null type unknown")
		}
		typ := sem.FromColumnType(ct1)
		constVal, err := sem.GetDatumToPhysicalFn(ct1)(t)
		if err != nil {
			return nil, resultIdx, ct, internalMemUsed, err
		}
	...
	case *tree.CaseExpr:
		...
		caseOutputType := sem.FromColumnType(sem.ToNewType(t.ResolvedType()))
		caseOutputIdx := len(columnTypes)
		ct = append(columnTypes, *sem.ToNewType(t.ResolvedType()))
		...
		for i, when := range t.Whens {
			...
			caseOps[i], resultIdx, ct, whenInternalMemUsed, err = planTypedMaybeNullProjectionOperators(
				ctx, evalCtx, when.Cond.(tree.TypedExpr), sem.ToNewType(t.ResolvedType()), ct, buffer, acc,
			)
			if err != nil {
				return nil, resultIdx, ct, internalMemUsed, err
			}
			...
			caseOps[i], thenIdxs[i], ct, thenInternalMemUsed, err = planTypedMaybeNullProjectionOperators(
				ctx, evalCtx, when.Val.(tree.TypedExpr), sem.ToNewType(t.ResolvedType()), ct, caseOps[i], acc,
			)
			if err != nil {
				return nil, resultIdx, ct, internalMemUsed, err
			}
		...
		elseOp, thenIdxs[len(t.Whens)], ct, elseInternalMemUsed, err = planTypedMaybeNullProjectionOperators(
			ctx, evalCtx, elseExpr.(tree.TypedExpr), sem.ToNewType(t.ResolvedType()), ct, buffer, acc,
		)
		if err != nil {
			return nil, resultIdx, ct, internalMemUsed, err
		}
...
func planProjectionExpr(
	...
	if lConst {
		...
		// to the input batch.
		lct := left.ResolvedType()
		lctOld := sem.ToOldType(&ct[rightIdx])
		op, err = GetProjectionLConstOperator(
			NewAllocator(ctx, acc), &lct, &lctOld, binOp,
	...
	if rConstArg, rConst := right.(tree.Datum); rConst {
		...
		if binOp == tree.Like || binOp == tree.NotLike {
			...
			rct := right.ResolvedType()
			loldType := sem.ToOldType(&ct[leftIdx])
			op, err = GetProjectionRConstOperator(
				NewAllocator(ctx, acc), &loldType, &rct, binOp,
	...
	resultIdx = len(ct)
	lOldTyp := sem.ToOldType(&ct[leftIdx])
	rOldTyp := sem.ToOldType(&ct[rightIdx])
	op, err = GetProjectionOperator(
		NewAllocator(ctx, acc), &lOldTyp, &rOldTyp, binOp, rightOp,
...
func planLogicalProjectionOp(
	...
	resultIdx = len(columnTypes)
	ct = append(columnTypes, *types1.Bool)
	var (
	...
	leftProjOpChain, leftIdx, ct, internalMemUsedLeft, err = planTypedMaybeNullProjectionOperators(
		ctx, evalCtx, typedLeft, types1.Bool, ct, &leftFeedOp, acc,
	)
	if err != nil {
		return nil, resultIdx, ct, internalMemUsed, err
	}
	rightProjOpChain, rightIdx, ct, internalMemUsedRight, err = planTypedMaybeNullProjectionOperators(
		ctx, evalCtx, typedRight, types1.Bool, ct, &rightFeedOp, acc,
	)
	if err != nil {
		return nil, resultIdx, ct, internalMemUsed, err
	}
