...
type SystemConfig struct {
	...
	mu struct {
		...
		locateSpaceCache map[uint32]*roachpb.LocationMap
...
func NewSystemConfig() *SystemConfig {
	...
	sc.mu.locateSpaceCache = map[uint32]*roachpb.LocationMap{}
...
func (s *SystemConfig) ComputeSplitKey(startKey, endKey roachpb.RKey) (rr roachpb.RKey) {
	...
	findSplitKey := func(startID, endID uint32) roachpb.RKey {
		...
		for id := startID; id <= endID; id++ {
			...
			if locationMap, err := s.getLocationRaw(id); err == nil && locationMap != nil {
				// endKey prefix 0
				elen := len(endKey)
				if elen > 0 && endKey[elen-1] == 0 {
					elen--
				}
				eKey := endKey[:elen]
				// todo optimize O(n) to O(log(n))
				for _, p := range locationMap.PartitionSpace {
					if startKey.Less(p.StartKey) {
						if p.StartKey.Less(eKey) {
							return p.StartKey
						}
					}
					if startKey.Less(p.EndKey) {
						if p.EndKey.Less(eKey) {
							return p.EndKey
						}
					}
				}
			} else if err != nil {
				log.Errorf(context.TODO(), "getLocationRaw ERROR: %s", err)
			}
