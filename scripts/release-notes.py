...
ext_contributors = individual_authors - znbase_folk
...
if not hideheader:
    ...
    print("toc: true")
    print("summary: Additions and changes in ZNBaseDB version", current_version, "since version", previous_version)
    print("---")
    print()
...
<div id="os-tabs" class="clearfix">
    <a href="https://binaries.znbasedb.com/znbase-""" + current_version + """.darwin-10.9-amd64.tgz"><button id="mac" data-eventcategory="mac-binary-release-notes">Mac</button></a>
    <a href="https://binaries.znbasedb.com/znbase-""" + current_version + """.linux-amd64.tgz"><button id="linux" data-eventcategory="linux-binary-release-notes">Linux</button></a>
    <a href="https://binaries.znbasedb.com/znbase-""" + current_version + """.windows-6.2-amd64.zip"><button id="windows" data-eventcategory="windows-binary-release-notes">Windows</button></a>
    <a href="https://binaries.znbasedb.com/znbase-""" + current_version + """.src.tgz"><button id="source" data-eventcategory="source-release-notes">Source</button></a>
</div>
""")
...
~~~shell
$ docker pull znbasedb/znbase""" + ("-unstable:" if "-" in current_version else ":") + current_version + """
~~~
""")
    print()
...
if len(notified_authors) > 0:
        print("We would like to thank the following contributors from the ZNBaseDB community:")
        print()
        for person in notified_authors:
            print("-", person, end='')
            if person in firsttime_contributors:
                annot = ""
                if person in znbase_folk:
                    annot = ", ZNBaseDB team member"
...
for pr in sorted(seenprs):
    print("[%s]: https://github.com/znbasedb/znbase/pull/%s" % (pr, pr[1:]))
for sha in sorted(seenshas):
    print("[%s]: https://github.com/znbasedb/znbase/commit/%s" % (sha, sha))
print()
