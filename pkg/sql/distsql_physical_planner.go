...
type DistSQLPlanner struct {
	...
	// processors.
	metadataTestTolerance runbase.MetadataTestLevel
...
// during initialization by ICL code to enable follower reads.
var ReplicaOraclePolicy = replicastrategy.BinPackingChoice

// If true, the plan diagram (in JSON) is logged for each plan (used for
// debugging).
var logPlanDiagram = envutil.EnvOrDefaultBool("ZNBASE_DISTSQL_LOG_PLAN", false)
...
//ServerZone record server zone
var ServerZone = settings.RegisterStringSetting(
	"sql.distsql.serverzone",
	"record server zone",
	"+08:00",
)

//ParallelSetting nums
var ParallelSetting = settings.RegisterIntSetting(
	"sql.distsql.opt.parallelsetting",
	"we set the concurrency here,the defalut value is 5,the most min value is 5",
	5,
)

// OrderedSyncOpt ParallelSetting nums
var OrderedSyncOpt = settings.RegisterBoolSetting(
	"sql.distsql.opt.orderedsync",
	"we set the orderedsync syncexec switch here",
	false,
)

// CaseSensitiveG controls case sensitivity
var CaseSensitiveG = settings.RegisterBoolSetting(
	"sql.casesensitive.enabled",
	"if enabled, all identifier will be casesensitve",
	false,
)
...
func NewDistSQLPlanner(
	...
	dsp := &DistSQLPlanner{
		...
		rpcCtx:                rpcCtx,
		metadataTestTolerance: runbase.NoExplain,
	}
...
type PlanningCtx struct {
	...
	// ReplcationTable indicates whether the plan involves replication tables.
	ReplcationTable bool
}
...
func getIndexIdx1(index *sqlbase.IndexDescriptor, desc *ImmutableTableDescriptor) (uint32, error) {
...
// convertOrdering maps the columns in props.ordering to the output columns of a
// processor.
func (dsp *DistSQLPlanner) convertOrdering1(
	reqOrdering sqlbase.ColumnOrdering, planToStreamColMap []int,
) distsqlpb.Ordering {
	if len(reqOrdering) == 0 {
		return distsqlpb.Ordering{}
	}
	result := distsqlpb.Ordering{
		Columns: make([]distsqlpb.Ordering_Column, len(reqOrdering)),
	}
...
func (dsp *DistSQLPlanner) createTableReaders(
	...
	if len(n.props.ordering) > 0 {
		planCtx.ExtendedEvalCtx.EvalContext.ShouldOrdered = true
	}
	...
	p.SetDataStoreEngine(n.storeEngine)
	return p, nil
}
...
func (dsp *DistSQLPlanner) planTableReaders(
	...
	returnMutations := info.scanVisibility == ScanVisibilityPublicAndNotPublic
	...
	typs := make([]sqlbase.ColumnType, 0, numCols)
	...
	p.AddNoInputStage(
		corePlacement, info.post, typs, dsp.convertOrdering1(sqlbase.ColumnOrdering(info.reqOrdering), info.colsToTableOrdinalMap),
	)
...
func (dsp *DistSQLPlanner) addPhySorters(
	p *PhysicalPlan, ordering sqlbase.ColumnOrdering, alreadyOrderedPrefix int,
) {
	// Sorting is needed; we add a stage of sorting processors.
	outputOrdering := distsqlpb.ConvertToMappedSpecOrdering(ordering, p.PlanToStreamColMap)
...
func (dsp *DistSQLPlanner) addAggregators(
	...
	if prevStageNode == 0 {
		...
		for _, e := range aggregations {
			...
			if _, ok := rowexec.DistAggregationTable[e.Func]; !ok {
				multiStage = false
				break
			}
		}
	...
	if !multiStage {
		...
		needRender := false
		for _, e := range aggregations {
			info := rowexec.DistAggregationTable[e.Func]
	...
	for i, agg := range aggregations {
		...
		var err error
		_, finalOutTypes[i], err = distsqlpb.GetAggregateInfo(agg.Func, argTypes...)
		if err != nil {
			return err
		}
	}
...
func (dsp *DistSQLPlanner) createPlanForIndexJoin(
	...
	plan.SetDataStoreEngine(n.storeEngine)
	return plan, nil
}
...
func (dsp *DistSQLPlanner) createPlanForJoin(
	...
	var leftReplicaTable, rightReplicaTable bool
	if planLeft, ok := n.left.plan.(*scanNode); ok {
		if planLeft.desc.TableDescriptor.ReplicationTable {
			leftReplicaTable = true
		}
	}
	if planRight, ok := n.right.plan.(*scanNode); ok {
		if planRight.desc.TableDescriptor.ReplicationTable {
			rightReplicaTable = true
		}
	}
	...
	leftPlan.ChangePlanForReplica = rowexec.ReplicaTableHashJoinerOpt.Get(&dsp.st.SV)
	// Create processor for replication table on all nodes involved in non-replicated
	// tables.
	if leftReplicaTable && leftPlan.ChangePlanForReplica {
		leftPlan.HaveReplicaTable = true
		var leftProcs []distsqlplan.Processor
		var leftResultRouters []distsqlplan.ProcessorIdx
		switch n.right.plan.(type) {
		case *scanNode:
			// If there is a scanNode on the right, traverse all its processors.
			for i, processor := range rightPlan.Processors {
				lPlan, err := dsp.createPlanForNode(planCtx, n.left.plan)
				if err != nil {
					return PhysicalPlan{}, err
				}
				lPlan.Processors[0].Node = processor.Node
				leftProcs = append(leftProcs, lPlan.Processors[0])
				leftResultRouters = append(leftResultRouters, distsqlplan.ProcessorIdx(i))
			}
		default:
			// In other cases, just look for the non-tablereader processor on the right.
			i := 0
			for _, processor := range rightPlan.Processors {
				if processor.Spec.Core.TableReader != nil {
					continue
				}
				lPlan, err := dsp.createPlanForNode(planCtx, n.left.plan)
				if err != nil {
		...
		leftPlan.Processors = leftProcs
		leftPlan.ResultRouters = leftResultRouters
	} else if rightReplicaTable && leftPlan.ChangePlanForReplica {
		rightPlan.HaveReplicaTable = true
		var rightProcs []distsqlplan.Processor
		var rightResultRouters []distsqlplan.ProcessorIdx
		switch n.left.plan.(type) {
		case *scanNode:
			// If there is a scanNode on the left, traverse all its processors.
			for i, processor := range leftPlan.Processors {
				rPlan, err := dsp.createPlanForNode(planCtx, n.right.plan)
				if err != nil {
					return PhysicalPlan{}, err
				}
				rPlan.Processors[0].Node = processor.Node
				rightProcs = append(rightProcs, rPlan.Processors[0])
				rightResultRouters = append(rightResultRouters, distsqlplan.ProcessorIdx(i))
			}
		default:
			// In other cases, just look for the non-tablereader processor on the left.
			i := 0
			for _, processor := range leftPlan.Processors {
				if processor.Spec.Core.TableReader != nil {
					continue
				}
				rPlan, err := dsp.createPlanForNode(planCtx, n.right.plan)
		...
		rightPlan.Processors, rightPlan.ResultRouters = rightProcs, rightResultRouters
	}
	...
	// If it involves replication table, then mark it in planCtx.
	if p.PhysicalPlan.HaveReplicaTable {
		planCtx.ReplcationTable = true
	}

	p.MaxEstimatedRowCount = leftPlan.MaxEstimatedRowCount
	if p.MaxEstimatedRowCount < rightPlan.MaxEstimatedRowCount {
		p.MaxEstimatedRowCount = rightPlan.MaxEstimatedRowCount
	}
	...
	if leftMergeOrd.Columns == nil {
		...
		if n.ParallelFlag && planCtx.ExtendedEvalCtx.SessionData.PC[sessiondata.HashJoinerSet].Parallel {
			core.HashJoiner.ParallelFlag = true
		}
		if n.forceside != 2 {
			core.HashJoiner.ForceSide = uint32(n.forceside)
		}
	} else {
...
func (dsp *DistSQLPlanner) createPhysPlan(
	...
	return dsp.createPlanForNode(planCtx, plan.planNode)
}

func (dsp *DistSQLPlanner) createPlanForNode(
	planCtx *PlanningCtx, node planNode,
) (plan PhysicalPlan, err error) {
	serverZone := ServerZone.Get(&dsp.st.SV)
	tree.ServerTimeZone = serverZone
	if rowexec.DistributedBatchOpt.Get(&dsp.st.SV) {
		runbase.OptTableReader = true
	} else {
		runbase.OptTableReader = false
	}

	runbase.OptTableReaderParallelSetting = int(rowexec.DistributedBatchParallelNums.Get(&dsp.st.SV))

	runbase.OptOrderedSync = OrderedSyncOpt.Get(&dsp.st.SV)

	runbase.ParallelSetting = int(ParallelSetting.Get(&dsp.st.SV))
	//if runbase.ParallelSetting < 5 {
	//	panic("ParallelSetting can not less than 5")
	//}
	planCtx.planDepth++
...
func createDistinctSpec1(
	distinctOnColIdxs exec.ColumnOrdinalSet, columnsInOrder exec.ColumnOrdinalSet, cols []int,
) *distsqlpb.DistinctSpec {
	var orderedColumns []uint32
...
func createProjectSetSpec1(
	...
	for i, col := range n.columns[n.numColsInSource:] {
		var err error
		spec.GeneratedColumns[i], err = sqlbase.DatumTypeToColumnType(col.Typ)
		if err != nil {
			return nil, err
		}
	}
...
func (dsp *DistSQLPlanner) createPlanForSetOp(
	...
	if !reflect.DeepEqual(leftPlan.PlanToStreamColMap, rightPlan.PlanToStreamColMap) {
		var l, r int
		for i := range leftPlan.PlanToStreamColMap {
			if leftPlan.PlanToStreamColMap[i] == -1 {
				l++
			}
		}
		for i := range rightPlan.PlanToStreamColMap {
			if rightPlan.PlanToStreamColMap[i] == -1 {
				r++
			}
		}
		if (len(leftPlan.PlanToStreamColMap) - l) != (len(rightPlan.PlanToStreamColMap) - r) {
...
func (dsp *DistSQLPlanner) createPlanForWindow(
	...
	for numWindowFuncProcessed < len(n.funcs) {
		...
		newResultTypes := make([]sqlbase.ColumnType, len(plan.ResultTypes)+len(samePartitionFuncs))
...
// NewPlanningCtx1 returns a new PlanningCtx return span resolver.
func (dsp *DistSQLPlanner) NewPlanningCtx1(
	ctx context.Context,
...
func (dsp *DistSQLPlanner) setProcessorVectorFlag(planCtx *PlanningCtx, plan *PhysicalPlan) {
	rowCountsMet := plan.MaxEstimatedRowCount >= planCtx.EvalContext().SessionData.VectorizeRowCountThreshold
	dsEngineType := plan.GetDataStoreEngine().ETypeSet & (cat.EngineTypeSet(cat.COLUMN))
	dsEngineType = 0
	for i := range plan.Processors {
		if dsEngineType > 0 {
			plan.Processors[i].Spec.VecComputeSupported = 2
		} else {
			if rowCountsMet {
				if isSupported(&plan.Processors[i].Spec) {
					plan.Processors[i].Spec.VecComputeSupported = 2
				} else {
					core := plan.Processors[i].Spec.Core
					if core.LocalPlanNode != nil ||
						core.MetadataTestSender != nil ||
						core.MetadataTestReceiver != nil {
						plan.Processors[i].Spec.VecComputeSupported = 1
					} else {
						plan.Processors[i].Spec.VecComputeSupported = 0
					}
				}
			} else {
				plan.Processors[i].Spec.VecComputeSupported = 0
...
func isSupported(spec *distsqlpb.ProcessorSpec) bool {
	core := spec.Core
	...
	case core.Aggregator != nil:
		...
		aggTyps := make([][]types1.T, len(aggSpec.Aggregations))
		for i, agg := range aggSpec.Aggregations {
			...
			aggTyps[i] = make([]types1.T, len(agg.ColIdx))
			for j, colIdx := range agg.ColIdx {
				aggTyps[i][j] = *sqlbase.FromColumnTypeToT(spec.Input[0].ColumnTypes[colIdx])
			}
			switch agg.Func {
			case distsqlpb.AggregatorSpec_SUM:
				switch aggTyps[i][0].Family() {
				case types1.IntFamily:
					// TODO(alfonso): plan ordinary SUM on integer types1 by casting to DECIMAL
					// at the end, mod issues with overflow. Perhaps to avoid the overflow
					// issues, at first, we could plan SUM for all types1 besides Int64.
					return false
				}
...
func (dsp *DistSQLPlanner) createPhysPlanForTuples(
	...
	return dsp.createValuesPlan(typs, numRows, rawBytes)
}

func getTypesFromResultColumns(cols sqlbase.ResultColumns) []sqlbase.ColumnType {
	typs := make([]sqlbase.ColumnType, len(cols))
	var err error
	for i, col := range cols {
		typs[i], err = sqlbase.DatumTypeToColumnType(col.Typ)
		if err != nil {
			panic(err)
		}
	}
...
func MakePhysicalPlan(gatewayNodeID roachpb.NodeID) PhysicalPlan {
	return PhysicalPlan{PhysicalPlan: distsqlplan.PhysicalPlan{GatewayNodeID: gatewayNodeID}}
}

// toTableOrdinals returns a mapping from column ordinals in cols to table
// reader column ordinals.
func toTableOrdinals(
	cols []sqlbase.ColumnDescriptor, desc *ImmutableTableDescriptor, visibility scanVisibility,
) []int {
...
func (dsp *DistSQLPlanner) planJoiners(
	info *joinPlanningInfo, reqOrdering exec.OutputOrdering,
) *PhysicalPlan {

	p := MakePhysicalPlan(dsp.nodeDesc.NodeID)
	leftRouters, rightRouters := distsqlplan.MergePlans1(
	...
	// Joiners may guarantee an ordering to outputs, so we ensure that
	// ordering is propagated through the input synchronizer of the next stage.
	p.SetMergeOrdering(dsp.convertOrdering1(sqlbase.ColumnOrdering(reqOrdering), p.PlanToStreamColMap))
	return &p
}

func (dsp *DistSQLPlanner) addDistinctProcessors(
	plan *PhysicalPlan, spec *distsqlpb.DistinctSpec, reqOrdering exec.OutputOrdering,
) {
	distinctSpec := distsqlpb.ProcessorCoreUnion{
		Distinct: spec,
	}
	defer func() {
		plan.SetMergeOrdering(dsp.convertOrdering1(sqlbase.ColumnOrdering(reqOrdering), plan.PlanToStreamColMap))
	}()
	...
	plan.AddStageOnNodes(
		nodes, distinctSpec, distsqlpb.PostProcessSpec{},
...
func (dsp *DistSQLPlanner) addProjectSet(
	...
	// Create the project set processor spec.
	projectSetSpec, err := createProjectSetSpec1(planCtx, info, plan.PlanToStreamColMap)
	if err != nil {
		return err
	}
	spec := distsqlpb.ProcessorCoreUnion{
...
type aggregatorPlanningInfo struct {
	...
	inputMergeOrdering   distsqlpb.Ordering
	reqOrdering          exec.OutputOrdering
}
...
func (dsp *DistSQLPlanner) planAggregators(
	...
	if !multiStage {
		...
		for _, e := range info.aggregations {
			...
			for i, localFunc := range info.LocalStage {
				...
				if isNewAgg {
					...
					for j, c := range e.ColIdx {
						argTypes[j] = inputTypes[c].ToDatumType()
					}
					coltyps, err := sqlbase.DatumTypesToColumnTypes(argTypes)
					if err != nil {
						return err
					}
					_, outputType, err := distsqlpb.GetAggregateInfo(localFunc, coltyps...)
					if err != nil {
						return err
					}
					intermediateTypes = append(intermediateTypes, outputType)
			...
			for _, finalInfo := range info.FinalStage {
				...
				if isNewAgg {
					...
					if needRender {
						...
						_, outputType, err := distsqlpb.GetAggregateInfo(finalInfo.Fn, argTypes...)
						if err != nil {
							return err
						}
						finalPreRenderTypes = append(finalPreRenderTypes, outputType.ToDatumType())
					}
				}
				finalIdx++
			}
		...
		ordCols := make([]distsqlpb.Ordering_Column, len(info.groupColOrdering))
		for i, o := range info.groupColOrdering {
			...
			if !found {
				return errutil.AssertionFailedf("group column ordering contains non-grouping column %d", o.ColIdx)
			}
			if o.Direction == encoding.Descending {
				ordCols[i].Direction = distsqlpb.Ordering_Column_DESC
			} else {
		...
		if needRender {
			// Build rendering expressions.
			renderExprs := make([]distsqlpb.Expression, len(info.aggregations))
	...
	finalOutTypes := make([]sqlbase.ColumnType, len(info.aggregations))
	for i, agg := range info.aggregations {
		...
		var err error
		coltyps, err := sqlbase.DatumTypesToColumnTypes(info.argumentsColumnTypes[i])
		if err != nil {
			return err
		}
		copy(argTypes[len(agg.ColIdx):], coltyps)
	...
	if len(finalAggsSpec.GroupCols) == 0 || len(p.ResultRouters) == 1 {
		...
		for bucket := 0; bucket < len(p.ResultRouters); bucket++ {
			pIdx := pIdxStart + distsqlplan.ProcessorIdx(bucket)
			p.MergeResultStreams1(p.ResultRouters, bucket, p.MergeOrdering, pIdx, 0, false /* forceSerialization */)
		}
		...
		p.ResultTypes = finalOutTypes
		p.SetMergeOrdering(dsp.convertOrdering1(sqlbase.ColumnOrdering(info.reqOrdering), p.PlanToStreamColMap))
	}
