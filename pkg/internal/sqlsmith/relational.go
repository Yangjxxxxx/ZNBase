...
type (
	...
	// accepts a list of tables generated from WITH expressions. Since znbase
...
func (s *scope) makeSelect(desiredTypes []types.T, refs colRefs) (*tree.Select, colRefs, bool) {
	...
	// Table references to CTEs can only be referenced once (znbase
	// limitation). Shuffle the tables and only pick each once.
	// TODO(mjibson): remove this when znbase supports full CTEs.
