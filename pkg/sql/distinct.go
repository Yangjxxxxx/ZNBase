...
func (p *planner) distinct(
	...
	for _, expr := range n.DistinctOn {
		...
		if index == -1 && r != nil {
			cols, exprs, hasStar, err := p.computeRenderAllowingStars(ctx, tree.SelectExpr{Expr: expr}, types.Any, nil, r.sourceInfo, r.ivarHelper, autoGenerateRenderOutputName)
			if err != nil {
				return nil, nil, err
			}
...
func (n *distinctNode) SetDataStoreEngine(ds cat.DataStoreEngine) {
	n.storeEngine = ds
}

func (n *distinctNode) GetDataStoreEngine() cat.DataStoreEngine {
