...
var (
	...
	metaDistSenderFollowerReadsCount = metric.Metadata{
		Name:        "follower_reads.count",
		Help:        "Number of follower reads processed by any replica",
...
// By default, without ICL code, this function returns false.
...
var followerReadSetHighPriority = settings.RegisterBoolSetting(
	"kv.follower_read.follower_high_priority",
	"whether follower read with high priority is allowed",
	false)
...
func makeDistSenderMetrics() DistSenderMetrics {
	return DistSenderMetrics{
		...
		FollowerReadsCount:      metric.NewCounter(metaDistSenderFollowerReadsCount),
	}
}
...
// A DistSender provides methods to access ZNBase's monolithic,
...
// ZNBase cluster via the supplied gossip instance. Supplying a
...
func NewDistSender(cfg DistSenderConfig, g *gossip.Gossip) *DistSender {
	...
	ds.asyncSenderSem = quotapool.NewIntPool("DistSender async concurrency", defaultSenderConcurrency)
...
func (ds *DistSender) sendSingleRange(
	...
	if isReplicaTable {
		for _, r := range desc.Replicas {
			if r.NodeID == ds.getNodeDescriptor().NodeID {
				nd := ds.getNodeDescriptor()
		...
		br, err = ds.sendRPC(ctx, ba, desc, replicas, cachedLeaseHolder, withCommit)
		if err != nil || br.Error != nil {
			replicas = []ReplicaInfo{}
		}
	}
	//get replica descriptor from store
	if len(replicas) == 0 {
		// When the table is not replicated or when there is no local replica
		// Try to send the call.
		...
		if (cachedLeaseHolder == roachpb.ReplicaDescriptor{}) {
			...
			if followerReadSetHighPriority.Get(&ds.st.SV) {
				if storeID, ok := ds.leaseHolderCache.Lookup(ctx, desc.RangeID); ok {
					if i := replicas.FindReplica(storeID); i >= 0 {
						replicas.MoveToEnd(i)
					}
				}
			}
		}
...
func (ds *DistSender) initAndVerifyBatch(
	...
	if ba.MaxSpanRequestKeys != 0 {
		...
		for _, req := range ba.Requests {
			inner := req.GetInner()
			switch inner.(type) {
			case *roachpb.ScanRequest, *roachpb.VecScanRequest, *roachpb.DeleteRangeRequest, *roachpb.RevertRangeRequest:
			...
			case *roachpb.EndTransactionRequest, *roachpb.ReverseScanRequest, *roachpb.RevertRequest:
				continue
...
// EndTransaction. If so, the EndTransactionRequest.CanCommitAtHigherTimestamp
...
func splitBatchAndCheckForRefreshSpans(
	...
	if et, ok := lastReq.(*roachpb.EndTransactionRequest); ok && ba.CanForwardRead {
...
func (ds *DistSender) Send(
	...
	var RetryTxn bool
	if et, ok := lastReq.(*roachpb.EndTransactionRequest); ok && et.Retrytxn {
		RetryTxn = true
	}
	...
	if ba.Txn != nil && (ba.Txn.Epoch > 0 || RetryTxn) && !require1PC {
		splitET = true
	}
...
// mergeErrors merges the two errors, comznbaseng their transaction state and
...
func (ds *DistSender) sendPartialBatch(
	...
	//in case add node has not refreshed RangeDescriptor in cache
	if ba.Txn != nil {
		if t, ok := ba.Requests[0].GetInner().(*roachpb.ScanRequest); ok && t.ReplicationTable {
			var getNewRangeDesc bool
			for _, r := range desc.Replicas {
				if r.NodeID == ds.getNodeDescriptor().NodeID {
					getNewRangeDesc = true
				}
			}
			if !getNewRangeDesc {
	...
	for r := retry.StartWithCtx(ctx, ds.rpcRetryOptions); r.Next(); {
		...
		if pErr == nil {
			if ba.DontNeedMeta {
				if c, ok := reply.Responses[0].GetInner().(*roachpb.ScanResponse); ok {
					c.BatchResponses = nil
				}
			}
	...
	for {
		if err != nil {
			...
			case nil:
				...
				// know held the lease.
				// When follower read have high priority, do not update leaseHolderCache to
				// prevent frequent change of leaseHolder.
				if !followerReadSetHighPriority.Get(&ds.st.SV) && cachedLeaseHolder != curReplica && ba.RequiresLeaseHolder() {
					if t, ok := ba.Requests[0].GetInner().(*roachpb.ScanRequest); ok && t.ReplicationTable {
						return br, nil
					}
