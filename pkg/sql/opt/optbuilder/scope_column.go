...
type scopeColumn struct {
	...
	// exprStr contains a string field representation of expr, or the original
	// column name if expr is nil. It is populated lazily inside getExprStr().
	exprStr string

	// visibleType 表示用户定义类型
	visibleType string

	// view is the ID of column's parent view if column is SELECT from a view.
	view sqlbase.ID

	// if column is rewrited due to updatable view, checkType will be owner
	checkType colType
}

// setCheckType mark if a column in scope is already checked by owner
func (c *scopeColumn) setCheckType(checkType colType) {
	c.checkType |= checkType
}

// isCheckOwner returns if a column is already checked by owner
func (c *scopeColumn) isCheckOwner() bool {
	s := c.checkType & CheckOwner
	if s == CheckOwner {
		return true
	}
	return false
...
func (c *scopeColumn) Format(ctx *tree.FmtCtx) {
	...
	if ctx.HasFlags(tree.FmtCheckEquivalence) {
		// Use double @ to distinguish from ZNBase column ordinals.
		ctx.Printf("@@%d", c.id)
		return
	}
