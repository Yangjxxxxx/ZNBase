...
func EncodeSQLStringWithFlags(buf *bytes.Buffer, in string, flags EncodeFlags) {
	...
	for i, r := range in {
		...
		// skip the chinese
		if ok := unicode.Is(unicode.Han, r); ok {
			continue
		}
		ch := byte(r)
		if r >= 0x20 && r < 0x7F {
...
func EncodeByteArrayToRawBytes(
	...
	case sessiondata.BytesEncodeEscape:
		...
		// ZNBaseDB's scanner. It only recognizes octal and \\ for the
		// backslash itself.
		...
		for _, c := range []byte(data) {
			if c == '\\' {
				...
				//
				// Note: ZNBaseDB only supports UTF-8 for which all values
...
func DecodeRawBytesToByteArray(data string, be sessiondata.BytesEncodeFormat) ([]byte, error) {
	...
	case sessiondata.BytesEncodeEscape:
		...
		for i := 0; i < len(data); i++ {
			...
			if i >= len(data)-1 {
				return nil, pgerror.NewError(pgcode.InvalidEscapeSequence,
