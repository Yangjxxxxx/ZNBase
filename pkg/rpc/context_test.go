...
func TestHeartbeatCB(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "compression", func(t *testing.T, compression bool) {
		...
		if _, err := clientCtx.GRPCDialNode(remoteAddr, 1, DefaultClass).Connect(context.Background()); err != nil {
			t.Fatal(err)
		}
...
//
// Prevents regression of https://github.com/znbasedb/znbase/issues/19991.
...
func TestHeartbeatHealthTransport(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		return clientCtx.TestingConnHealth(remoteAddr, 1)
	})
	...
	for then := timeutil.Now(); ; {
		err := func() error {
			if err := clientCtx.TestingConnHealth(remoteAddr, 1); !isUnhealthy(err) {
				return errors.Errorf("unexpected error: %v", err)
			}
			return nil
		}()
	...
	testutils.SucceedsSoon(t, func() error {
		err := clientCtx.TestingConnHealth(remoteAddr, 1)
...
func TestRemoteOffsetUnhealthy(t *testing.T) {
	...
	for i, clientNodeContext := range nodeCtxs {
		for j, serverNodeContext := range nodeCtxs {
			...
			if _, err := clientNodeContext.ctx.GRPCDialNode(serverNodeContext.ctx.Addr, serverNodeContext.ctx.NodeID.Get(), DefaultClass).Connect(context.Background()); err != nil {
				t.Fatal(err)
			}
		}
	}
...
func TestGRPCKeepaliveFailureFailsInflightRPCs(t *testing.T) {
	...
	for _, c := range testCases {
		...
		t.Run(kaName+"/"+pName, func(t *testing.T) {
			...
			conn, err := clientCtx.GRPCDialNode(remoteAddr, 1, DefaultClass).Connect(ctx)
			if err != nil {
				t.Fatal(err)
			}
...
func TestClusterIDMismatch(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		wg.Add(1)
		go func() {
			_, err := clientCtx.GRPCDialNode(remoteAddr, 1, DefaultClass).Connect(context.Background())
...
func BenchmarkGRPCDial(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			_, err := ctx.GRPCDialNode(remoteAddr, 1, DefaultClass).Connect(context.Background())
			if err != nil {
				b.Fatal(err)
			}
		}
