...
func TestSettingsSetAndShow(t *testing.T) {
	...
	db.Exec(t, fmt.Sprintf(setQ, intKey, "'a-str'"))
	...
	db.Exec(t, `CREATE USER testuser;GRANT USAGE ON DATABASE defaultdb TO testuser;`)
	...
	if _, err := testuser.Exec(`SELECT * FROM zbdb_internal.cluster_settings`); !testutils.IsError(err,
		`only users with the admin role are allowed to read zbdb_internal.cluster_settings`,
	) {
		t.Fatal(err)
	}
}
