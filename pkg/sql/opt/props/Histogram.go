...
func getRangesBeforeAndAfter(
	...
	rangeBefore, okBefore := cat.GetRange(bucketLowerBound, bucketUpperBound)
	rangeAfter, okAfter := cat.GetRange(spanLowerBound, spanUpperBound)
	ok = okBefore && okAfter
...
func isDiscrete(typ types.T) bool {
	switch typ {
	case types.Int, types.Date, types.Timestamp, types.TimestampTZ:
		return true
	}
	return false
}
