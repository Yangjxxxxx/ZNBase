...
func BenchmarkTypeCheck(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := tree.TypeCheck(expr, &ctx, types.Int, false)
		if err != nil {
...
func attemptTypeCheckSameTypedExprs(t *testing.T, idx int, test sameTypedExprsTestCase) {
	...
	forEachPerm(test.exprs, 0, func(exprs []copyableExpr) {
		...
		_, typ, err := tree.TypeCheckSameTypedExprs(&ctx, desired, false, false, buildExprs(exprs)...)
		if err != nil {
...
func TestTypeCheckSameTypedExprs(t *testing.T) {
	for i, d := range []sameTypedExprsTestCase{
		...
		{nil, types.Int, exprs(decConst("1.1")), types.Int, nil},
		...
		{nil, types.Int, exprs(intConst("1"), decConst("1.1")), types.Int, nil},
...
func TestTypeCheckSameTypedExprsError(t *testing.T) {
	...
	for i, d := range testData {
		...
		forEachPerm(d.exprs, 0, func(exprs []copyableExpr) {
			if _, _, err := tree.TypeCheckSameTypedExprs(&ctx, desired, false, false, buildExprs(exprs)...); !testutils.IsError(err, d.expectedErr) {
