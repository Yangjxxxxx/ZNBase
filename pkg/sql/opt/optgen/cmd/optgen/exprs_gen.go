...
import (
	...
	"github.com/znbasedb/znbase/pkg/sql/opt/optgen/lang"
)
...
func (g *exprsGen) generate(compiled *lang.CompiledExpr, w io.Writer) {
	...
	fmt.Fprintf(g.w, "\n")
	fmt.Fprintf(g.w, "  \"github.com/znbasedb/znbase/pkg/sql/coltypes\"\n")
	fmt.Fprintf(g.w, "  \"github.com/znbasedb/znbase/pkg/sql/opt/constraint\"\n")
	fmt.Fprintf(g.w, "  \"github.com/znbasedb/znbase/pkg/sql/opt\"\n")
	fmt.Fprintf(g.w, "  \"github.com/znbasedb/znbase/pkg/sql/opt/cat\"\n")
	fmt.Fprintf(g.w, "  \"github.com/znbasedb/znbase/pkg/sql/opt/props\"\n")
	fmt.Fprintf(g.w, "  \"github.com/znbasedb/znbase/pkg/sql/opt/props/physical\"\n")
	fmt.Fprintf(g.w, "  \"github.com/znbasedb/znbase/pkg/sql/pgwire/pgerror\"\n")
	fmt.Fprintf(g.w, "  \"github.com/znbasedb/znbase/pkg/sql/sem/tree\"\n")
	fmt.Fprintf(g.w, "  \"github.com/znbasedb/znbase/pkg/sql/sem/types\"\n")
	fmt.Fprintf(g.w, ")\n\n")
...
func (g *exprsGen) genExprDef(define *lang.DefineExpr) {
	...
	if define.Tags.Contains("List") {
		...
		g.generateDataEngineFuncs(define)
...
func (g *exprsGen) genExprStruct(define *lang.DefineExpr) {
	...
	fmt.Fprintf(g.w, "BlockOffset int\n")
	if define.Tags.Contains("Scalar") {
		...
		fmt.Fprintf(g.w, "  best  bestProps\n")
		fmt.Fprintf(g.w, "  StoreEngine cat.DataStoreEngine\n")
	} else {
		fmt.Fprintf(g.w, "\n")
...
func (g *exprsGen) genExprFuncs(define *lang.DefineExpr) {
	...
	// Generate the SetBlockOffset method
	fmt.Fprintf(g.w, "func (e *%s) SetBlockOffset(Offset int) {\n", opTyp.name)
	fmt.Fprintf(g.w, "  e.BlockOffset = Offset\n")
	fmt.Fprintf(g.w, "}\n\n")

	// Generate the GetBlockOffset method
	fmt.Fprintf(g.w, "func (e *%s) GetBlockOffset() int {\n", opTyp.name)
	fmt.Fprintf(g.w, "  return e.BlockOffset\n")
	fmt.Fprintf(g.w, "}\n\n")
	...
	if define.Tags.Contains("SetNext") {
		fmt.Fprintf(g.w, "func (exp *%s) SetNext() {\n", opTyp.name)
		fmt.Fprintf(g.w, "exp.next = nil\n")
		fmt.Fprintf(g.w, "}\n")
	}
	...
	if define.Tags.Contains("Scalar") {
		...
		if name := g.scalarPropsFieldName(define); name != "" {
			...
			fmt.Fprintf(g.w, "  if !e.scalar.Populated && mem != nil {\n")
...
func (g *exprsGen) generateSetDataStoreEngine(define *lang.DefineExpr) {
	opTyp := g.md.typeOf(define)
	if define.Tags.Contains("Scalar") {
		return
	}
	if strings.Index(opTyp.name, "Scan") == 0 {
		fmt.Fprintf(g.w, "  e.StoreEngine = (&e.ScanPrivate).EngineTypeSet \n")
		return
	}
	if g.hasTwoInput(define) {
		fmt.Fprintf(g.w, "  e.StoreEngine = cat.DataStoreEngine{ETypeSet: cat.EngineTypeSet(e.Left.GetDataStoreEngine().ETypeSet & e.Right.GetDataStoreEngine().ETypeSet)} \n")
	} else {
		if len(define.Fields) > 0 {
			firstFiledName := g.md.fieldName(define.Fields[0])
			if firstFiledName == "Input" {
				fmt.Fprintf(g.w, "  e.StoreEngine = e.Input.GetDataStoreEngine() \n")
			} else {
				fmt.Fprintf(g.w, "if e.StoreEngine.ETypeSet == 0 { \n")
				fmt.Fprintf(g.w, "	e.StoreEngine = cat.DataStoreEngine{ETypeSet: cat.EngineTypeSet(cat.KV)} \n")
				fmt.Fprintf(g.w, "} \n")
			}
		} else {
			fmt.Fprintf(g.w, "	e.StoreEngine = cat.DataStoreEngine{ETypeSet: cat.EngineTypeSet(cat.KV)} \n")
...
func (g *exprsGen) hasTwoInput(define *lang.DefineExpr) bool {
	opTyp := g.md.typeOf(define)
	var opName = opTyp.name
	if strings.Index(opName, "InnerJoin") != -1 || strings.Index(opName, "LeftJoin") != -1 ||
		strings.Index(opName, "RightJoin") != -1 || strings.Index(opName, "Full") != -1 ||
		strings.Index(opName, "SemiJoin") != -1 || strings.Index(opName, "AntiJoin") != -1 ||
		strings.Index(opName, "MergeJoin") != -1 {
		return true
	}
	return false
}

func (g *exprsGen) generateDataEngineFuncs(define *lang.DefineExpr) {
	opTyp := g.md.typeOf(define)
	// Generate the SetDataStoreEngine method
	fmt.Fprintf(g.w, "func (e *%s) SetDataStoreEngine() {\n", opTyp.name)
	g.generateSetDataStoreEngine(define)
	fmt.Fprintf(g.w, "}\n\n")
	// Generate the GetDataStoreEngine method
	fmt.Fprintf(g.w, "func (e *%s) GetDataStoreEngine() cat.DataStoreEngine {\n", opTyp.name)
	if define.Tags.Contains("Scalar") {
		fmt.Fprintf(g.w, "  return cat.DataStoreEngine{ETypeSet: cat.EngineTypeSet(cat.KV)} \n")
	} else {
		fmt.Fprintf(g.w, "  return e.StoreEngine \n")
	}
	//g.generateGetDataStoreEngine(define)
	fmt.Fprintf(g.w, "}\n\n")
}
