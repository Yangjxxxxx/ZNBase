...
func (p *planner) EvalSubquery(expr *tree.Subquery) (result tree.Datum, err error) {
	...
	if p.curPlan.subqueryPlans != nil {
		...
		if expr.Idx < 0 || expr.Idx-1 >= len(p.curPlan.planComponents.subqueryPlans) {
			return nil, pgerror.NewAssertionErrorf("invalid index %d for %q", expr.Idx, expr)
		}
	}
	var s *subquery
	if p.curPlan.subqueryPlans != nil {
		...
		s = &p.curPlan.planComponents.subqueryPlans[expr.Idx-1]
	}
...
// JudgePlannerStmtIfInsertOrUpdate is used to get planner's stmt to judge if the stmt is an insert statement or a update statement
func (p *planner) JudgePlannerStmtIfInsertOrUpdate() (bool, bool) {
	if p.stmt == nil || p.stmt.AST == nil {
		return false, false
	}
	_, ok := p.stmt.AST.(*tree.Insert)
	_, ok2 := p.stmt.AST.(*tree.Update)
	return ok, ok2
}
...
func (v *subqueryVisitor) subqueryAlreadyAnalyzed(t *tree.Subquery) bool {
	return t.Idx > 0 && t.Idx-1 < len(v.curPlan.subqueryPlans) && v.curPlan.subqueryPlans[t.Idx-1].plan.planNode != nil
}
...
func (v *subqueryVisitor) VisitPre(expr tree.Expr) (recurse bool, newExpr tree.Expr) {
	...
	case *tree.Subquery:
		...
		if t.Exists {
			result.plan.planNode = &limitNode{plan: result.plan.planNode, countExpr: tree.NewDInt(1)}
			result.execMode = rowexec.SubqueryExecModeExists
			t.SetType(types.Bool)
		} else {
			result.plan.planNode = &max1RowNode{
				plan: &limitNode{
					plan:      result.plan.planNode,
...
func (v *subqueryVisitor) extractSubquery(
	...
	var res planMaybePhysical
	res.planNode = plan

	v.curPlan.subqueryPlans = append(v.curPlan.subqueryPlans, subquery{subquery: sub, plan: res})
