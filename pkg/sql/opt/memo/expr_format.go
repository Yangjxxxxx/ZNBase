...
func (f *ExprFmtCtx) formatRelational(e RelExpr, tp treeprinter.Node) {
	...
	case *ScanExpr:
		...
		if !t.Flags.Empty() {
			...
			} else if t.Flags.IndexHintType == keys.IndexHintForce {
				idx := md.Table(t.Table).Index(t.Flags.HintIndexs[0])
				dir := ""
		...
		if t.Locking != nil {
			...
			wait := ""
			switch t.Locking.WaitPolicy.LockingType {
			...
			case tree.LockWaitError:
				wait = ",nowait"
			case tree.LockWait:
				wait = ",wait " + string(rune(t.Locking.WaitPolicy.WaitTime))
			}
	...
	default:
		if opt.IsJoinOp(t) {
			...
			if !p.Flags.DisallowParallel {
				tp.Childf("can parallel: %s", "true")
			} else {
				tp.Childf("can parallel: %s", "false")
			}
		}
	}
...
func (f *ExprFmtCtx) formatScalar(scalar opt.ScalarExpr, tp treeprinter.Node) {
	...
	if scalar.Op() != opt.ScalarListOp {
		...
		case opt.FiltersItemOp, opt.ProjectionsItemOp, opt.AggregationsItemOp,
			opt.ZipItemOp: // Use properties from the item, but otherwise omit it from output.
...
func frameBoundName(b tree.WindowFrameBoundType) string {
	...
	panic(pgerror.NewAssertionErrorf("unexpected bound"))
}
