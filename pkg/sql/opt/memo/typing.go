...
func init() {
	...
	typingFuncMap[opt.AdditionalOp] = typeAsTypedExpr
	typingFuncMap[opt.CoalesceOp] = typeCoalesce
	typingFuncMap[opt.NvlOp] = typeNvl
	typingFuncMap[opt.CharOp] = typeChar
...
// typeNvl returns the type of a nvl expression, which is equal to
// the type of its first non-null child.
func typeNvl(e opt.ScalarExpr) types.T {
	for _, arg := range e.(*NvlExpr).Args {
		childType := arg.DataType()
		if childType != types.Unknown {
			return childType
		}
...
// typeChar returns the type of a char expression, which is equal to
// the type of its first non-null child.
func typeChar(e opt.ScalarExpr) types.T {
	for _, arg := range e.(*CharExpr).Args {
		childType := arg.DataType()
		if childType != types.Unknown {
			return childType
		}
