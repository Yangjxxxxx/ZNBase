...
func Example_sql_format() {
	...
	// bare	withtz
	// 2016-01-25 10:10:10	2016-01-25 15:10:10+00:00
}
...
Available Commands:
  ...
  store       Modify the state of the store
  help        Help about any command

Flags:
  -h, --help                             help for znbase
      --log-archive-on                   archive log file or not (default true)
      --logtostderr Severity[=DEFAULT]   logs at or above this threshold go to stderr (default NONE)
      --no-color                         disable standard error log colorization

Use "znbase [command] --help" for more information about a command.
`
	helpExpected := fmt.Sprintf("ZNBaseDB command-line interface and server.\n\n%s", expUsage)
	...
	for _, test := range testCases {
		t.Run(strings.Join(test.flags, ","), func(t *testing.T) {
			...
			znbaseCmd.SetOutput(w)
			defer znbaseCmd.SetOutput(nil)
...
func checkNodeStatus(t *testing.T, c cliTest, output string, start time.Time) {
	...
	if nodeCtx.statusShowRanges || nodeCtx.statusShowAll {
		testcases = append(testcases,
			testCase{"leader_ranges", baseIdx, 24},
			testCase{"leaseholder_ranges", baseIdx + 1, 24},
			testCase{"ranges", baseIdx + 2, 24},
...
func TestGenAutocomplete(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{shell: "bash"},
		{shell: "zsh"},
		{shell: "bad", expErr: `invalid argument "bad" for "znbase gen autocomplete"`},
	} {
...
func TestJunkPositionalArguments(t *testing.T) {
	...
	for i, test := range []string{
		...
		exp := fmt.Sprintf("%s\nunknown command %q for \"znbase %s\"\n", line, junk, test)
		if exp != out {
			t.Errorf("expected:\n%s\ngot:\n%s", exp, out)
		}
	}
...
func TestZipContainsAllInternalTables(t *testing.T) {
	...
	rows, err := db.Query(`
SELECT concat('zbdb_internal.', table_name) as name FROM [ SHOW TABLES FROM zbdb_internal ] WHERE
    table_name NOT IN (
...
writing ` + os.DevNull + `
  ...
  debug/zbdb_internal.cluster_queries.txt
  debug/zbdb_internal.cluster_sessions.txt
  debug/zbdb_internal.cluster_settings.txt
  debug/zbdb_internal.databases.txt
  debug/zbdb_internal.jobs.txt
  debug/zbdb_internal.kv_node_status.txt
  debug/zbdb_internal.kv_store_status.txt
  debug/zbdb_internal.schema_changes.txt
  debug/zbdb_internal.partition_views.txt
  debug/zbdb_internal.partitions.txt
  debug/zbdb_internal.table_row_statistics.txt
  debug/zbdb_internal.zones.txt
  debug/nodes/1/status.json
  debug/nodes/1/zbdb_internal.feature_usage.txt
  debug/nodes/1/zbdb_internal.function_privileges.txt
  debug/nodes/1/zbdb_internal.gossip_alerts.txt
  debug/nodes/1/zbdb_internal.gossip_liveness.txt
  debug/nodes/1/zbdb_internal.gossip_network.txt
  debug/nodes/1/zbdb_internal.gossip_nodes.txt
  debug/nodes/1/zbdb_internal.leases.txt
  debug/nodes/1/zbdb_internal.node_statement_statistics.txt
  debug/nodes/1/zbdb_internal.node_build_info.txt
  debug/nodes/1/zbdb_internal.node_metrics.txt
  debug/nodes/1/zbdb_internal.node_queries.txt
  debug/nodes/1/zbdb_internal.node_runtime_info.txt
  debug/nodes/1/zbdb_internal.node_sessions.txt
  ...
  debug/schema/system/authentication.json
  debug/schema/system/comments.json
  debug/schema/system/descriptor.json
  debug/schema/system/eventlog.json
  debug/schema/system/flashback.json
  debug/schema/system/function_namespace.json
  debug/schema/system/hints.json
  debug/schema/system/jobs.json
  debug/schema/system/lease.json
  debug/schema/system/location.json
  ...
  debug/schema/system/scheduled_jobs.json
  debug/schema/system/settings.json
  debug/schema/system/snapshots.json
  debug/schema/system/table_statistics.json
  debug/schema/system/triggers.json
  debug/schema/system/ui.json
  debug/schema/system/user_options.json
