...
// This test runs hibernate-core's full test suite against an single znbase
// node.
...
func registerHibernate(r *registry) {
	runHibernate := func(
		...
		for attempt, r := 0, retry.StartWithCtx(ctx, opts); r.Next(); {
			...
			// In order to get Hibernate's test suite to connect to znbase, we have
     ...
     znbase : [
       ...
       'jdbc.url'   : 'jdbc:postgresql://localhost:26257/defaultdb?sslmode=disable'
		...
		for attempt, r := 0, retry.StartWithCtx(ctx, opts); r.Next(); {
			...
			if err := c.RunE(
				ctx, node, `cd /mnt/data1/hibernate/hibernate-core/ && ./../gradlew test -Pdb=znbase `+
		...
		version, err := fetchZNBaseVersion(ctx, c, node[0])
		if err != nil {
			t.Fatal(err)
		}
		blacklistName, expectedFailures := getHibernateBlacklistForVersion(version)
		if expectedFailures == nil {
			t.Fatalf("No hibernate blacklist defined for znbase version %s", version)
		}
		c.l.Printf("Running znbase version %s, using blacklist %s", version, blacklistName)
		...
		_ = c.RunE(ctx, node,
			`cd /mnt/data1/hibernate/hibernate-core/ && ./../gradlew test -Pdb=znbase`,
		)

		t.Status("collecting the test results")
		// Copy all of the test results to the znbase logs directory to be
		...
		fmt.Fprintf(&bResults, "Tests run on ZNBase %s\n", version)
...
func fetchZNBaseVersion(ctx context.Context, c *cluster, nodeIndex int) (string, error) {
	...
	if err := db.QueryRowContext(ctx,
		`SELECT value FROM zbdb_internal.node_build_info where field = 'Version'`,
...
func maybeAddGithubLink(issue string) string {
	...
	/*
		...
		return fmt.Sprintf("https://github.com/znbasedb/znbase/issues/%d", issueNum)
	*/
}
