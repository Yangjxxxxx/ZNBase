...
func (d *pgCopyReader) closeRejectCh(ctx context.Context) {
	if d.fileContext.rejected != nil {
		close(d.fileContext.rejected)
	}
}

func (d *pgCopyReader) saveRejectRecord(
...
func (d *pgCopyReader) readFiles(
	...
	return readInputFiles(ctx, cp, d.readFile, &d.fileContext, encryption, encoding, details)
}
...
func (d *pgCopyReader) readFile(
	...
	for count := int64(1); ; count++ {
		...
		if len(row) != len(d.conv.visibleColTypes) {
			return makeRowErr(inputName.Path, count, "expected %d values, got %d", len(d.conv.visibleColTypes), len(row))
		}
		for i, s := range row {
			if s == nil {
				d.conv.datums[i] = tree.DNull
			} else {
				d.conv.datums[i], err = tree.ParseDatumStringAs(d.conv.visibleColTypes[i], *s, d.conv.evalCtx, true)
				if err != nil {
