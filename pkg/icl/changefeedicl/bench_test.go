...
func BenchmarkCDCTicks(b *testing.B) {
	...
	runBench := func(b *testing.B, feedClock *hlc.Clock) {
		...
		for i := 0; i < b.N; i++ {
			b.StartTimer()
			sink, cancelFeed := createBenchmarkCDC(ctx, s, feedClock, `d`, `bank`)
...
func (s *benchSink) GetTopics() (map[string]string, error) {
...
func (s *benchSink) EmitRow(
	...
	return true, s.emit(int64(len(k) + len(v)))
}
...
// createBenchmarkCDC starts a stripped down cdc. It watches
...
// different timestamps beforehand and simulate the cdc going through
// them in steps.
//
// The returned sink can be used to count emits and the closure handed back
// cancels the cdc (blocking until it's shut down) and returns an error
// if the cdc had failed before the closure was called.
//
// This intentionally skips the distsql and sink parts to keep the benchmark
// focused on the core cdc work, but it does include the poller.
func createBenchmarkCDC(
	ctx context.Context,
	s serverutils.TestServerInterface,
	feedClock *hlc.Clock,
	database, table string,
) (*benchSink, func() error) {
	tableDesc := sqlbase.GetTableDescriptor(s.DB(), database, "public", table)
	...
	poller := makePoller(
		...
		leaseMgr, metrics, &mm, "", "", sink, nil, nil, nil,
	)
	...
	tickFn := emitEntrySlice(
		s.ClusterSettings(), details, sf, encoder, sink, rowsFn, TestingKnobs{}, metrics, nil, nil, poller)

	ctx, cancel := context.WithCancel(ctx)
	go func() { _ = poller.Run(ctx) }()
	go func() { _ = thUpdater.PollTableDescriptors(ctx) }()
	...
	for batchIdx := 0; batchIdx < table.InitialRows.NumBatches; batchIdx++ {
		...
		for _, row := range table.InitialRows.Batch(batchIdx) {
			...
			for colIdx, datum := range row {
				...
				_, _ = fmt.Fprintf(&insertStmtBuf, `$%d`, len(params))
			}
			insertStmtBuf.WriteString(`)`)
		}
