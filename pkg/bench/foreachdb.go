...
func benchmarkZNBase(b *testing.B, f func(b *testing.B, db *gosql.DB)) {
...
func benchmarkZNBaseOpt(b *testing.B, f func(b *testing.B, db *gosql.DB)) {
	s, db, _ := serverutils.StartServer(
		b, base.TestServerArgs{UseDatabase: "bench"})
	defer s.Stopper().Stop(context.TODO())
...
func benchmarkMultinodeZNBase(b *testing.B, f func(b *testing.B, db *gosql.DB)) {
...
func benchmarkPostgres(b *testing.B, f func(b *testing.B, db *gosql.DB)) {
	...
	// Postgres server has SSL enabled. To use ZNBase's checked-in
	...
	// ssl_cert_file = '$GOPATH/src/github.com/znbasedb/znbase/pkg/security/securitytest/test_certs/node.crt' # (change requires restart)
	// ssl_key_file = '$GOPATH/src/github.com/znbasedb/znbase/pkg/security/securitytest/test_certs/node.key' # (change requires restart)
	// ssl_ca_file = '$GOPATH/src/github.com/znbasedb/znbase/pkg/security/securitytest/test_certs/ca.crt' # (change requires restart)
	// ```
	// Where `$GOPATH/src/github.com/znbasedb/znbase`
	// is replaced with your local ZNBase source directory.
...
func ForEachDB(b *testing.B, fn func(*testing.B, *gosql.DB)) {
	for _, dbFn := range []func(*testing.B, func(*testing.B, *gosql.DB)){
		benchmarkZNBase,
		benchmarkZNBaseOpt,
		benchmarkMultinodeZNBase,
		benchmarkPostgres,
		benchmarkMySQL,
	} {
		dbName := runtime.FuncForPC(reflect.ValueOf(dbFn).Pointer()).Name()
		dbName = strings.TrimPrefix(dbName, "github.com/znbasedb/znbase/pkg/bench.benchmark")
