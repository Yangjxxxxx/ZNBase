...
// MakeFuncNameMetadataKey returns the key for the function name. Pass name == "" in order
// to generate the prefix key to use to scan over all of the names for the
// specified parentID.
func MakeFuncNameMetadataKey(parentID ID, name string, funcName string) roachpb.Key {
	k := keys.MakeTablePrefix(uint32(FunctionNamespaceTable.ID))
	k = encoding.EncodeUvarintAscending(k, uint64(FunctionNamespaceTable.PrimaryIndex.ID))
	k = encoding.EncodeUvarintAscending(k, uint64(parentID))
	if funcName != "" {
		k = encoding.EncodeBytesAscending(k, []byte(funcName))
	}
	if name != "" {
		k = encoding.EncodeBytesAscending(k, []byte(name))
		k = keys.MakeFamilyKey(k, uint32(FunctionNamespaceTable.Columns[3].ID))
	}
	return k
}

// MakeTrigMetadataKey returns the key of trigger: 29/1/tableID/trigger's name
func MakeTrigMetadataKey(parentID ID, name string) roachpb.Key {
	k := keys.MakeTablePrefix(uint32(TriggersTable.ID))
	k = encoding.EncodeUvarintAscending(k, uint64(TriggersTable.PrimaryIndex.ID))
	k = encoding.EncodeUvarintAscending(k, uint64(parentID))
	if name != "" {
		k = encoding.EncodeBytesAscending(k, []byte(name))
		return keys.MakeFamilyKey(k, uint32(TriggersTable.Families[0].ID))
	}
	return k
}
...
//KvRedundancy 符合条件时在insertNode中创建kv冗余数据，提高insert执行效率
type KvRedundancy struct {
	KvKey          []byte
	RawValueBuf    []byte
	SecondaryIndex []IndexEntry
	Err            error
}
