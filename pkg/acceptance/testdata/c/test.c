...
int main(int argc, char *argv[]) {
	...
	const char *version = PQparameterStatus(conn, "znbase_version");
	if (version == NULL) {
		fprintf(stderr, "ERROR PQparameterStatus: znbase_version not reported: %s\n", PQgeterror());
		return 1;
	}
	if (strncmp(version, "ZNBaseDB ", strlen("ZNBaseDB ")) != 0) {
		fprintf(stderr, "znbase_version mismatch: '%s' doesn't start with 'ZNBaseDB '\n", version);
		return 1;
	}
