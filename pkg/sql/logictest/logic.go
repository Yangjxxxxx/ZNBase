...
// against ZNBaseDB's SQL layer. It is typically used to run
// ZNBaseDB's own tests (stored in the `testdata` directory) during
...
// its input files are hosted at https://github.com/znbasedb/sqllogictest)
//
...
//
// The Test-Script language is extended here for use with ZNBaseDB. The
...
//
//  - skipif <mysql/mssql/postgresql/znbasedb>
//    Skips the following `statement` or `query` if the argument is postgresql
//    or znbasedb.
//
//  - onlyif <mysql/mssql/postgresql/znbasedb>
...
//            ZNBaseDB's fork of Sqllogictest.
//
// Configuration:
//
...
// TestClusterConfig cluster config for test
type TestClusterConfig struct {
	...
	// if true, will apply test config for audit
	audit bool
}
...
var logicTestConfigs = []TestClusterConfig{
	...
	{name: "local-insecure", numNodes: 1, overrideDistSQLMode: "off", overrideOptimizerMode: "off", insecure: true},
	{name: "local-insecure-opt", numNodes: 1, overrideDistSQLMode: "off", overrideOptimizerMode: "on", insecure: true},
	...
	if *rewriteSQL {
		if !hasVars {
			newSyntax, err := func(inSql string) (string, error) {
				...
				stmtList, err := parser.Parse(inSql, false)
				if err != nil {
					if ls.expectErr != "" {
...
func (t *logicTest) setUser(user string, insecure bool) func() {
	if t.clients == nil {
	...
	pgURL1 := sqlutils.PGUrl
	if insecure {
		pgURL1 = sqlutils.PGUrlInsecur
	}
	pgURL, cleanupFunc := pgURL1(t.t, addr, "TestLogic", url.User(user))
	pgURL.Path = "test"
	if insecure {
...
func (t *logicTest) setup(cfg TestClusterConfig) {
	...
	params := base.TestClusterArgs{
		ServerArgs: base.TestServerArgs{
			...
			LocationName: roachpb.LocationName{
				Names: []string{
					"TEST", "CHINA", "TJ", "inspur",
				},
			},
			SyncEventLog: cfg.audit,
		},
	...
	distSQLKnobs := &distsql.TestingKnobs{
		MetadataTestLevel: distsql.Off, DeterministicStats: true,
	}
	if cfg.distSQLUseDisk {
		distSQLKnobs.MemoryLimitBytes = 1
	}
	if cfg.distSQLMetadataTestEnabled {
		distSQLKnobs.MetadataTestLevel = distsql.On
	}
	...
	params.ServerArgs.Insecure = cfg.insecure
	...
	if params.ServerArgs.SyncEventLog {
		t.cluster.UpdateAudit()
	}
	...
	t.cleanupFuncs = append(t.cleanupFuncs, t.setUser(security.RootUser, params.ServerArgs.Insecure))
...
`); err != nil {
	...
	sqlStmt := `CREATE USER %s;`
	if _, err := t.db.Exec(fmt.Sprintf(sqlStmt, server.TestUser)); err != nil {
		t.Fatal(err)
	}
...
func (t *logicTest) processSubtest(
	...
	for s.Scan() {
		...
		case "user":
			...
			cleanupUserFunc := t.setUser(fields[1], false)
		...
		case "userInsecure":
			...
			cleanupUserFunc := t.setUser(fields[1], true)
			defer cleanupUserFunc()
...
var skipLogicTests = envutil.EnvOrDefaultBool("ZNBASE_LOGIC_TESTS_SKIP", false)
var logicTestsConfigExclude = envutil.EnvOrDefaultString("ZNBASE_LOGIC_TESTS_SKIP_CONFIG", "")
var logicTestsConfigFilter = envutil.EnvOrDefaultString("ZNBASE_LOGIC_TESTS_CONFIG", "")

// RunLogicTest is the main entry point for the logic test. The globs parameter
// specifies the default sets of files to run.
func RunLogicTest(t *testing.T, tcs []TestClusterConfig, globs ...string) {
	...
	if skipLogicTests {
		t.Skip("ZNBASE_LOGIC_TESTS_SKIP")
	}
	...
	// of paths per config.
	configPaths := make([][]string, len(tcs))
...
// ProcessTestFile used for parallel test
func ProcessTestFile(
	t *testing.T, cluster serverutils.TestClusterInterface, path string, nodeIdx int, db *gosql.DB,
) {
...
// getEventConfig return configs with audit config
func getEventConfig() []TestClusterConfig {
	tc := make([]TestClusterConfig, len(logicTestConfigs))
	for i, lt := range logicTestConfigs {
		lt.audit = true
		tc[i] = lt
	}
	return tc
...
// GetConfig return config for cluster test
func GetConfig() []TestClusterConfig {
	return logicTestConfigs
}
