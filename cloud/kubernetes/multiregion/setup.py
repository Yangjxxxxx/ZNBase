...
# example:
# contexts = {
#     'us-central1-a': 'gke_znbase-alex_us-central1-a_my-cluster',
#     'us-central1-b': 'gke_znbase-alex_us-central1-b_my-cluster',
#     'us-west1-b': 'gke_znbase-alex_us-west1-b_my-cluster',
# }
contexts = {
}
...
# clusters.
#
# Setting regions is optional, but recommended, because it improves znbase's
...
# Path to the znbase binary on your local machine that you want to use
# generate certificates. Defaults to trying to find znbase in your PATH.
znbase_path = 'znbase'
...
try:
    check_call(["which", znbase_path])
except:
    exit("no binary found at provided path '" + znbase_path + "'; please put a znbase binary in your path or change the znbase_path variable")
...
check_call([znbase_path, 'cert', 'create-ca', '--certs-dir', certs_dir, '--ca-key', ca_key_dir+'/ca.key'])
check_call([znbase_path, 'cert', 'create-client', 'root', '--certs-dir', certs_dir, '--ca-key', ca_key_dir+'/ca.key'])
...
for zone, context in contexts.items():
    ...
    check_call(['kubectl', 'create', 'secret', 'generic', 'znbasedb.client.root', '--from-file', certs_dir, '--context', context])
    check_call(['kubectl', 'create', 'secret', 'generic', 'znbasedb.client.root', '--namespace', zone, '--from-file', certs_dir, '--context', context])
    check_call([znbase_path, 'cert', 'create-node', '--certs-dir', certs_dir, '--ca-key', ca_key_dir+'/ca.key', 'localhost', '127.0.0.1', 'znbasedb-public', 'znbasedb-public.default' 'znbasedb-public.'+zone, 'znbasedb-public.%s.svc.cluster.local' % (zone), '*.znbasedb', '*.znbasedb.'+zone, '*.znbasedb.%s.svc.cluster.local' % (zone)])
    check_call(['kubectl', 'create', 'secret', 'generic', 'znbasedb.node', '--namespace', zone, '--from-file', certs_dir, '--context', context])
...
# Create a znbasedb-public service in the default namespace in each cluster.
for zone, context in contexts.items():
...
for zone in contexts:
    for i in range(3):
        join_addrs.append('znbasedb-%d.znbasedb.%s' % (i, zone))
join_str = ','.join(join_addrs)

# Create the znbase resources in each cluster.
for zone, context in contexts.items():
    if zone in regions:
        locality = 'region=%s,zone=%s' % (regions[zone], zone)
    else:
        locality = 'zone=%s' % (zone)
    yaml_file = '%s/znbasedb-statefulset-%s.yaml' % (generated_files_dir, zone)
    with open(yaml_file, 'w') as f:
        check_call(['sed', 's/JOINLIST/%s/g;s/LOCALITYLIST/%s/g' % (join_str, locality), 'znbasedb-statefulset-secure.yaml'], stdout=f)
