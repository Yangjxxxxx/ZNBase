		...
		for _, state := range []sqlbase.DescriptorMutation_State{sqlbase.DescriptorMutation_DELETE_ONLY,
			...
			t.Run(fmt.Sprintf("useUpsert=%t/state=%v", useUpsert, state),
				func(t *testing.T) {
					...
					// read table descriptor
					mTest.tableDesc = sqlbase.GetTableDescriptor(kvDB, "t", "public", "test")
...
`); err != nil {
	...
	mt.tableDesc = sqlbase.GetTableDescriptor(kvDB, "t", "public", "test")
	...
	mt.CheckQueryResults(t,
		...
		[][]string{
			{"test", "primary", "false", "1", "a", "ASC", "false", "false", "global", "NO"},
			{"test", "ufo", "true", "1", "d", "ASC", "false", "false", "global", "NO"},
			{"test", "ufo", "true", "2", "a", "ASC", "false", "true", "global", "NO"},
		},
	)
...
func TestAddingFKs(t *testing.T) {
	...
	ordersDesc := sqlbase.GetTableDescriptor(kvDB, "t", "public", "orders")
