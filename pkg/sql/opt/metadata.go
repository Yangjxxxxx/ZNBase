...
type Metadata struct {
	...
	// functions stores information about echo metadata of functions
...
type mdDep struct {
	...
	// upperOwner is not empty if the object is used as an underlying relation,
	// so that the object is used by owner of the upper level view.
	upperOwner string

	// colPrivileges store column privileges on columns for dml operation.
	colPrivileges tree.PrivilegeGroups
...
// ClearMetadate clear the metadata to an empty metadata.
//
// Table annotations are not transferred over; all annotations are unset on
// the copy.
func (md *Metadata) ClearMetadate() {
	md.schemas = nil
	md.cols = nil
	md.tables = nil
	md.sequences = nil
...
func (md *Metadata) AddDataSourceDependency(
	name *cat.DataSourceName, ds cat.DataSource, cols tree.NameList, priv privilege.Kind, user string,
) {
	// Search for the same name / object pair.
	for i := range md.deps {
		if md.deps[i].object == ds && md.deps[i].dsName().Equals(name) {
			if len(md.deps[i].upperOwner) == 0 {
				md.deps[i].upperOwner = user
			}
			// column privilege
			if cols != nil && privilege.IsColumnPrivilegeType(priv) {
				hasPriv := func() (int, bool) {
					for j := range md.deps[i].colPrivileges {
						if priv == md.deps[i].colPrivileges[j].Privilege {
							return j, true
						}
					}
					return -1, false
				...
				if j, has := hasPriv(); has {
					hasCol := func(col tree.Name) bool {
						for _, c := range md.deps[i].colPrivileges[j].Columns {
							if col == c {
								return true
							}
						}
					...
					for _, col := range cols {
						...
						md.deps[i].colPrivileges[j].Columns = append(md.deps[i].colPrivileges[j].Columns, col)
						return
					}
				} else {
					md.deps[i].colPrivileges = append(md.deps[i].colPrivileges, tree.PrivilegeGroup{Privilege: priv, Columns: cols})
					return
				}
			}
			// table privilege
	...
	// Not find the same name / object pair.
	var cp tree.PrivilegeGroups
	var p privilegeBitmap
	if cols != nil && privilege.IsColumnPrivilegeType(priv) {
		cp = append(cp, tree.PrivilegeGroup{Privilege: priv, Columns: cols})
		p = 0
	} else {
		p = (1 << priv)
	}
...
// AddFunctionDependency will check current query's dependency function group.
func (md *Metadata) AddFunctionDependency(
	name *cat.DataSourceName, f cat.UdrFunction, priv privilege.Kind,
) {
	for i := range md.deps {
		if md.deps[i].object == f && md.deps[i].dsName().Equals(name) {
			md.deps[i].privileges |= (1 << priv)
			return
		}
	}
	d := mdDep{object: f, privileges: (1 << priv)}
	d.name = *name
...
func (md *Metadata) CheckDependencies(
	...
	for i := range md.deps {
		...
		switch obj.(type) {
		// only udrfunction can change.
		case cat.UdrFunction:
			name := *md.deps[i].dsName()
			// Resolve function object
			new, err := catalog.ResolveUdrFunction(ctx, &name)
			if err != nil {
				return false, err
			}
			toCheck = new
		...
		for privs := md.deps[i].privileges; privs != 0; {
			...
			if !privilege.IsColumnPrivilegeType(priv) {
				if err := catalog.CheckDMLPrivilege(ctx, toCheck, nil, priv, md.deps[i].upperOwner); err != nil {
					return false, err
				}
			}
		...
		for _, cp := range md.deps[i].colPrivileges {
			if err := catalog.CheckDMLPrivilege(ctx, toCheck, cp.Columns, cp.Privilege, md.deps[i].upperOwner); err != nil {
				return false, err
			}
		}
	}
...
// AddFunction indexes a new reference to a function used by this query
func (md *Metadata) AddFunction(function cat.UdrFunction) FunctionID {
	if md.functions == nil {
		md.functions = make([]cat.UdrFunction, 0, 4)
	}
	...
	return FunctionID(len(md.functions))
}
...
func (md *Metadata) AddTableWithAlias(tab cat.Table, alias *tree.TableName) TableID {
	...
	for i := 0; i < colCount; i++ {
		col := tab.Column(i)
		colID := md.AddColumn(string(col.ColName()), col.DatumType(), nil)
...
// AddTableXQ is a helper that calls AddTableWithAlias with tab.Name() as the
// table's alias.
func (md *Metadata) AddTableXQ(tab cat.TableXQ) TableID {
	return md.AddTableWithAliasXQ(tab, tab.Table().Name())
}

// AddTableWithAliasXQ indexes a new reference to a table within the query.
// Separate references to the same table are assigned different table ids (e.g.
// in a self-join query). All columns are added to the metadata. If mutation
// columns are present, they are added after active columns. The table's alias
// is passed separately so that its original formatting is preserved for error
// messages, pretty-printing, etc.
func (md *Metadata) AddTableWithAliasXQ(tabXQ cat.TableXQ, alias *tree.TableName) TableID {
	tabID := makeTableID(len(md.tables), ColumnID(len(md.cols)+1))
	tab := tabXQ.Table()
	if md.tables == nil {
		md.tables = make([]TableMeta, 0, 4)
	}
	md.tables = append(md.tables, TableMeta{MetaID: tabID, Table: tab, TableXQ: tabXQ, Alias: *alias})
...
// AddTableWithAliasAll is the function that add table with alias all
func (md *Metadata) AddTableWithAliasAll(
	tab cat.Table, tabXQ cat.TableXQ, alias *tree.TableName,
) TableID {
	if tabXQ != nil {
		return md.AddTableWithAliasXQ(tabXQ, alias)
	}
	return md.AddTableWithAlias(tab, alias)
}

// TableMeta looks up the metadata for the table associated with the given table
// id. The same table can b added multiple times to the query metadata and
// associated with multiple table ids.
func (md *Metadata) TableMeta(tabID TableID) *TableMeta {
	return &md.tables[tabID.index()]
}

//TableNil is to tell if table is nil
func (md *Metadata) TableNil() bool {
	if md.tables == nil {
		return true
	}
	return false
...
// TableXQ is the helper to get the TabeXQ from TableMeta
func (md *Metadata) TableXQ(tabID TableID) cat.TableXQ {
	return md.TableMeta(tabID).TableXQ
}
...
func (md *Metadata) AddColumn(alias string, typ types.T, expr tree.TypedExpr) ColumnID {
	if alias == "" {
		alias = fmt.Sprintf("column%d", len(md.cols)+1)
	}
	colID := ColumnID(len(md.cols) + 1)
	md.cols = append(md.cols, ColumnMeta{MetaID: colID, Alias: alias, Type: typ, Expr: expr})
	return colID
}
...
// AllFunctions returns the metadata for all functions. The result must not be
// modified.
func (md *Metadata) AllFunctions() []cat.UdrFunction {
	return md.functions
}
...
// GetDepUpperOwner returns upperOwner of specified object from deps.
func (md *Metadata) GetDepUpperOwner(name *cat.DataSourceName, ds cat.DataSource) string {
	for i := range md.deps {
		if md.deps[i].object == ds && md.deps[i].dsName().Equals(name) {
			return md.deps[i].upperOwner
		}
	}
	return ""
}
