...
func createAndStartTestNode(
	...
	if err := node.start(ctx, addr, bootstrappedEngines, newEngines,
		roachpb.Attributes{}, locality, &roachpb.LocationName{}, cv, []roachpb.LocalityAddress{},
...
func TestBootstrapCluster(t *testing.T) {
	...
	res, err := engine.MVCCScan(context.Background(), e, keys.LocalMax, roachpb.KeyMax, math.MaxInt64, hlc.MaxTimestamp, engine.MVCCScanOptions{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCorruptedClusterID(t *testing.T) {
	...
	if err := node.start(
		...
		roachpb.Attributes{}, roachpb.Locality{}, &roachpb.LocationName{}, cv,
...
func TestStartNodeWithLocationName(t *testing.T) {
	defer leaktest.AfterTest(t)()
	ctx := context.Background()

	testLocationNameWithNewNode := func(locationName roachpb.LocationName) {
		e := engine.NewInMem(roachpb.Attributes{}, 1<<20)
		defer e.Close()
		cfg := bootstrapNodeConfig()
		if _, err := bootstrapCluster(
		...
		//)
		//_, _, node, stopper :=
		_, addr, cfg, node, stopper := createTestNode(util.TestAddr, []engine.Engine{e}, util.TestAddr, t)
		bootstrappedEngines, newEngines, cv, err := inspectEngines(
			ctx, []engine.Engine{e}, cfg.Settings.Version.MinSupportedVersion,
			cfg.Settings.Version.ServerVersion, node.clusterID)
		if err != nil {
			t.Fatal(err)
		}
		...
		cfg.NodeLiveness.StartHeartbeat(ctx, stopper, nil /* alive */)
		if err := node.start(ctx, addr, bootstrappedEngines, newEngines,
			roachpb.Attributes{}, roachpb.Locality{}, &locationName, cv, []roachpb.LocalityAddress{},
			nil, /*nodeDescriptorCallback */
		); err != nil {
			t.Fatal(err)
		}
		...
		// Check the node to make sure the locality was propagated to its
		// nodeDescriptor.
		if !reflect.DeepEqual(node.Descriptor.LocationName, locationName) {
			t.Fatalf("expected node locationName to be %q, but it was %q", locationName, node.Descriptor.LocationName)
		}
		...
		if err := node.stores.VisitStores(func(store *storage.Store) error {
			...
			if !reflect.DeepEqual(desc.Node.LocationName, locationName) {
				t.Fatalf("expected store's node locationName to be %q, but it was %q", locationName, desc.Node.LocationName)
			}
			return nil
		}); err != nil {
			t.Fatal(err)
	...
	testCases := []roachpb.LocationName{
		{},
		{
			Names: []string{"a"},
		},
	...
	for _, testCase := range testCases {
		testLocationNameWithNewNode(testCase)
	}
