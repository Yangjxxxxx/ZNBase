...
var StartCmd = &cobra.Command{
	...
	Short: "start a node",
	Long: `
Start a ZNBaseDB node, which will export data from one or more
...
`,
	Example: `  znbase start --insecure --store=attrs=ssd,path=/mnt/ssd1 [--join=host:port,[host:port]]`,
	Args:    cobra.NoArgs,
...
var maxSizePerProfile = envutil.EnvOrDefaultInt64(
	"ZNBASE_MAX_SIZE_PER_PROFILE", 100<<20 /* 100 MB */)
...
func initMemProfile(ctx context.Context, dir string) {
	...
	memProfileInterval := envutil.EnvOrDefaultDuration("ZNBASE_MEMPROF_INTERVAL", -1)
...
func initCPUProfile(ctx context.Context, dir string) {
	...
	cpuProfileInterval := envutil.EnvOrDefaultDuration("ZNBASE_CPUPROF_INTERVAL", -1)
...
func initBlockProfile() {
	...
	d := envutil.EnvOrDefaultInt64("ZNBASE_BLOCK_PROFILE_RATE",
...
func initMutexProfile() {
	...
	d := envutil.EnvOrDefaultInt("ZNBASE_MUTEX_PROFILE_RATE",
...
func initCursorTempStorageConfig(
	ctx context.Context,
	st *cluster.Settings,
	stopper *stop.Stopper,
	firstStore base.StoreSpec,
	...
	var recordPath string
	if !firstStore.InMemory {
		recordPath = filepath.Join(firstStore.Path, server.CursorTempDirsRecordFilename)
	}
	...
	// Create the temporary subdirectory for the temp engine.
	if tempStorageConfig.Path, err = engine.CreateTempDir(tempDir, server.CursorTempDirPrefix, stopper); err != nil {
		return base.TempStorageConfig{}, errors.Wrap(err, "could not create temporary directory for temp storage")
	}
...
// runStart starts the znbase node using --store as the list of
// storage devices ("stores") on this machine and --join as the list
// of other active nodes used to join this node to the znbase
...
func runStart(cmd *cobra.Command, args []string) error {
	...
	// in ZNBaseDB may use logging, and until logging has been
	...
	if serverCfg.CursorTempStorageConfig, err = initCursorTempStorageConfig(ctx, serverCfg.Settings, stopper, useStore, specIdx); err != nil {
		return err
	}
		...
		if waitForInit {
			log.Shout(ctx, log.Severity_INFO,
				"initial startup completed.\n"+
					"Node will now attempt to join a running cluster, or wait for `znbase init`.\n"+
...
If problems persist, please see ` + base.DocsURL("cluster-setup-troubleshooting.html") + "."
	...
	// ready to start.
	log.Info(ctx, "starting znbase node")
	...
	go func() {
		...
		if err := func() error {
			...
			if err := s.Start(ctx); err != nil {
				...
				return errors.Wrap(err, "znbase server exited with error")
			}
			...
			// in tests.
			if !envutil.EnvOrDefaultBool("ZNBASE_SKIP_UPDATE_CHECK", false) {
			...
			fmt.Fprintf(tw, "ZNBaseDB node starting at %s (took %0.1fs)\n", timeutil.Now(), timeutil.Since(tBegin).Seconds())
	...
	// message received from `znbase quit` / `znbase
	...
	case <-log.FatalChan():
		...
		// being written.
		// https://github.com/znbasedb/znbase/issues/23414
...
func setupAndInitializeLoggingAndProfiling(ctx context.Context) (*stop.Stopper, error) {
	...
	if logDir := startCtx.logDir.String(); logDir != "" {
		ls := znbaseCmd.PersistentFlags().Lookup(logflags.LogToStderrName)
		if !ls.Changed {
