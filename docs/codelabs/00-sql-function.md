...
Currently, ZNBaseDB only supports [built-in SQL functions][built-ins]. We’re
...
during ZNBaseDB initialization transformed into a
...
In another terminal window, use the `znbase sql` shell to execute our
built-in:

```text
$ ./znbase sql --insecure -e "select whois()"
...
```go
$ ./znbase sql --insecure -e 'select nonexistent()'
...
```text
$ ./znbase sql --insecure -e "select whois('pmattis')"
+------------------+
| whois('pmattis') |
+------------------+
| Peter Mattis     |
...
$ ./znbase sql --insecure -e "select whois('pmattis', 'bdarnell')"
+------------------------------+
...
$ ./znbase sql --insecure -e "select whois('non-existent')"
...
To test ZNBaseDB’s SQL functionality, we use a logic test framework that
...
[built-ins]: https://www.znbaselabs.com/docs/stable/functions-and-operators.html#built-in-functions
