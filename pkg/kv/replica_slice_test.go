...
func TestReplicaSliceMoveToEnd(t *testing.T) {
	defer leaktest.AfterTest(t)()
	rs := createReplicaSlice()
	rs.MoveToEnd(0)
	exp := []roachpb.StoreID{2, 3, 4, 5, 1}
	if stores := getStores(rs); !reflect.DeepEqual(stores, exp) {
		t.Errorf("expected order %s, got %s", exp, stores)
	}
	rs.MoveToEnd(2)
	exp = []roachpb.StoreID{2, 3, 5, 1, 4}
	if stores := getStores(rs); !reflect.DeepEqual(stores, exp) {
		t.Errorf("expected order %s, got %s", exp, stores)
	}
	rs.MoveToEnd(4)
	...
	assertPanic(t, rs.MoveToEnd, -1)
	assertPanic(t, rs.MoveToEnd, 5)
}

func assertPanic(t *testing.T, f func(i int), i int) {
	defer func() {
		if r := recover(); r == nil {
			t.Errorf("%s(%d) did not panic", runtime.FuncForPC(reflect.ValueOf(f).Pointer()).Name(), i)
		}
	}()
	f(i)
}
