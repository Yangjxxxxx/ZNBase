...
func (p *planner) ProjectSet(
	...
	for i, expr := range exprs {
		...
		if tFunc, ok := normalized.(*tree.FuncExpr); ok && tFunc.IsGeneratorApplication() {
			...
			fd, err := tFunc.Func.Resolve(p.semaCtx.SearchPath, p.ExtendedEvalContext().GetCaseSensitive())
			if err != nil {
				return planDataSource{}, err
			}
...
func (n *projectSetNode) SetDataStoreEngine(ds cat.DataStoreEngine) {
	n.storeEngine = ds
}

func (n *projectSetNode) GetDataStoreEngine() cat.DataStoreEngine {
