...
func TestSchemaChangeProcess(t *testing.T) {
	...
	var id = sqlbase.ID(keys.MinNonPredefinedUserDescID + 2 /* skip over DB ID */)
	var node = roachpb.NodeID(2)
...
`); err != nil {
	...
	if _, err := sqlDB.Exec(`
CREATE UNIQUE INDEX vidx ON t.test (v);
`); !testutils.IsError(err, `违反了唯一性约束 "vidx"`) {
...
func TestSchemaChangePurgeFailure(t *testing.T) {
	defer leaktest.AfterTest(t)()
	t.Skip("由于新增异步Schema Change GC造成概率过, 参考官方暂时跳过")
...
`); err != nil {
	...
	if _, err := sqlDB.Exec(
		"CREATE UNIQUE INDEX foo ON t.test (v)",
	); !testutils.IsError(err, `违反了唯一性约束 "foo"`) {
		t.Fatal(err)
	}
...
`); err != nil {
	...
	); !testutils.IsError(err, ` 违反了唯一性约束`) {
		t.Fatalf("err = %s", err)
	}
...
// sentinel kv pairs at the start of each table row. ZNBasedb used
...
`); err != nil {
	...
	// written in the past when znbasedb used to write sentinel
...
`); err != nil {
	...
	testCases := []struct {
		...
		// TODO https://forum.znbaselabs.com/t/renaming-database-in-a-transaction/2397/8
...
`); err != nil {
	...
	tableDesc := sqlbase.GetTableDescriptor(kvDB, "d", "public", "t")
...
`); err != nil {
	...
	newTableDesc := sqlbase.GetTableDescriptor(kvDB, "t", "public", "test")
...
`); err != nil {
	...
	fkTableDesc := sqlbase.GetTableDescriptor(kvDB, "t", "public", "pi")
...
func TestIndexBackfillAfterGC(t *testing.T) {
	...
	runGC := func(sp roachpb.Span) error {
		...
		_, err := client.SendWrapped(ctx, tc.Server(0).DistSender().(*kv.DistSender), &gcr)
		if err != nil {
			panic(err)
		}
		return nil
...
// needs to be retried it gets retried internal to znbase. This test
...
`); err != nil {
	...
	if _, err := sqlDB.Exec(`CREATE UNIQUE INDEX foo ON t.test (v)`); !testutils.IsError(
		err, fmt.Sprintf("%d entries, expected %d 违反了唯一性约束", maxValue, maxValue+1),
	) {
		t.Fatal(err)
	}
...
`); err != nil {
	...
	FROM
	zbdb_internal.check_consistency(false, '', '') as t
