...
function makeNodesState(...addresses: { id: number, address: string, status?: LivenessStatus }[]) {
  ...
  store.dispatch(livenessReducerObj.receiveData(new protos.znbase.server.serverpb.LivenessResponse(livenessData)));
...
describe("sumNodeStats", function() {
  it("sums stats from an array of nodes", function() {
    // Each of these nodes only has half of its capacity "usable" for znbase data.
    // See diagram for what these stats mean:
    // https://github.com/znbasedb/znbase/blob/31e4299ab73a43f539b1ba63ed86be5ee18685f6/pkg/storage/metrics.go#L145-L153
    const nodeStatuses: INodeStatus[] = [
      {
        desc: { node_id: 1 },
        metrics: {
