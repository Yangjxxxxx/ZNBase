...
func (p *planner) orderBy(
	...
	for _, o := range orderBy {
		...
		if index == -1 && s != nil {
			cols, exprs, hasStar, err := p.computeRenderAllowingStars(ctx, tree.SelectExpr{Expr: expr}, types.Any, nil, s.sourceInfo, s.ivarHelper, autoGenerateRenderOutputName)
			if err != nil {
				return nil, err
			}
...
func (n *sortNode) SetDataStoreEngine(ds cat.DataStoreEngine) {
	n.storeEngine = ds
}

func (n *sortNode) GetDataStoreEngine() cat.DataStoreEngine {
