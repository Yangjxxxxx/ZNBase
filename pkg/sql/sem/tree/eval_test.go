	...
	walk := func(t *testing.T, getExpr func(tree.TypedExpr) (tree.TypedExpr, error)) {
		datadriven.Walk(t, filepath.Join("testdata", "eval"), func(t *testing.T, path string) {
			datadriven.RunTest(t, path, func(d *datadriven.TestData) string {
				...
				typedExpr, err := expr.TypeCheck(nil, types.Any, false)
				if err != nil {
					t.Fatalf("%s: %v", d.Input, err)
				}
...
func TestTimeConversion(t *testing.T) {
	...
	for _, test := range tests {
		...
		typedExpr, err := expr.TypeCheck(nil, types.Timestamp, false)
		if err != nil {
			t.Errorf("%s: %v", exprStr, err)
			continue
		}
...
func TestEvalError(t *testing.T) {
	...
	for _, d := range testData {
		...
		typedExpr, err := tree.TypeCheck(expr, nil, types.Any, false)
		if err == nil {
