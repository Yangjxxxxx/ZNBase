...
func TestReadOnlyBasics(t *testing.T) {
	...
	for _, engineImpl := range mvccEngineImpls {
		t.Run(engineImpl.name, func(t *testing.T) {
			e := engineImpl.create(inMemAttrs, testCacheSize)
			defer e.Close()
			...
			successTestCases := []func(){
				...
				func() { _ = ro.Iterate(a, a, func(MVCCKeyValue) (bool, error) { return true, nil }) },
			...
			defer func() {
				...
				for i, f := range successTestCases {
					shouldPanic(t, f, string(rune(i)), "using a closed "+name)
				}
			}()

			for i, f := range successTestCases {
				shouldNotPanic(t, f, string(rune(i)))
			}
			...
			for i, f := range failureTestCases {
				shouldPanic(t, f, string(rune(i)), "not implemented")
			}
...
// keys. This should never happen with the way ZNBase uses
...
func TestBatchBuilder(t *testing.T) {
	...
	stopper := stop.NewStopper()
	defer stopper.Stop(context.TODO())
	e := newRocksDBInMem(base.StoreSpec{}, 1<<20)
	stopper.AddCloser(e)
...
func TestBatchIteration(t *testing.T) {
	...
	k0 := MakeMVCCMetadataKey(roachpb.Key("b"))
	...
	// SeekReverse works, but rocksdb reverse iteration is not supported. pebble reverse iteration is supported.
	iter.SeekReverse(k2)
	if ok, err := iter.Valid(); !ok {
		t.Fatal(err)
	}

	//pebble SeekReverse is < , rocksdb SeekReverse is <=
	if _, ok := e.(*RocksDB); ok {
		if !reflect.DeepEqual(iter.Key(), k2) {
			t.Fatalf("expected %s, got %s", k3, iter.Key())
		}
		if !reflect.DeepEqual(iter.Value(), v2) {
			t.Fatalf("expected %s, got %s", v2, iter.Value())
		}
		...
		if !reflect.DeepEqual(iter.Key(), k0) {
			t.Fatalf("expected %s, got %s", k0, iter.Key())
		}
		if !reflect.DeepEqual(iter.Value(), v0) {
			t.Fatalf("expected %s, got %s", v0, iter.Value())
		}
	}

}

// Test comznbaseng of concurrent commits of write-only batches, verifying that
...
func TestDecodeKey(t *testing.T) {
	...
	if _, ok := e.(*RocksDB); ok {
		tests = append(tests, MVCCKey{Key: []byte{}})
	}
