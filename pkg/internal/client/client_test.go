...
znbase cluster (a single node, but bootstrapped, gossiped, etc.).
*/
package client_test
	...
	for i := 0; i < 2; i++ {
		go func(i int) {
			...
			if err := db.Txn(context.TODO(), func(ctx context.Context, txn *client.Txn) error {
				err := txn.SetDebugName(fmt.Sprintf("test-%d", i))
				if err != nil {
					return err
				}
...
// https://groups.google.com/forum/#!topic/znbase-db/LdrC5_T0VNw
...
func TestScanTables(t *testing.T) {
	defer leaktest.AfterTest(t)()
	//store := base.StoreSpec{StorePath: dir}
	s, _, _ := serverutils.StartServer(t, base.TestServerArgs{})
	defer s.Stopper().Stop(context.TODO())
	db := createTestClient(t, s)
