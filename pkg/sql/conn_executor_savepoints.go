...
const commitOnReleaseSavepointName = "znbase_restart"
...
func (ex *connExecutor) execSavepointInOpenState(
	...
	// Sanity check for "SAVEPOINT znbase_restart".
	commitOnRelease := ex.isCommitOnReleaseSavepoint(s.Name)
	if commitOnRelease {
		// Validate the special savepoint znbase_restart. It cannot be nested
		...
		// SAVEPOINT znbase_restart
		//
		// Some of our examples use this pattern, issuing the SAVEPOINT znbase_restart
		...
		// ROLLBACK TO SAVEPOINT znbase_restart  -> the savepoint no longer exists here
		//
		// Although it would work for any other savepoint but znbase_restart. But
	...
	if !ex.server.cfg.Settings.Version.IsActive(cluster.VersionSavepoints) && !commitOnRelease {
...
func (ex *connExecutor) execRelease(
	...
	if entry.commitOnRelease {
		...
		if errIsRetriable(err) {
			...
			// znbase_restart (that's the whole point of commitOnRelease).
			env.push(*entry)
...
func (ex *connExecutor) execRollbackToSavepointInOpenState(
	...
	// are not yet enabled but we still support znbase_restart. In
	// that case, we can't process ROLLBACK TO SAVEPOINT
	// znbase_restart using ignored seqnum lists so we need to
	...
	if entry.kvToken.Initial() &&
		!ex.server.cfg.Settings.Version.IsActive(cluster.VersionSavepoints) {
...
func (ex *connExecutor) checkRollbackValidity(
	...
	if !entry.kvToken.Initial() {
		...
		// state.
		ev, payload = ex.makeErrEvent(errors.New(
			"unimplemented: ROLLBACK TO SAVEPOINT not yet supported after DDL statements"), s)
	...
	if ex.state.mu.txn.UserPriority() == roachpb.MaxUserPriority {
		...
		ev, payload = ex.makeErrEvent(errors.New(
			"unimplemented: cannot use ROLLBACK TO SAVEPOINT in a HIGH PRIORITY transaction containing DDL"), s)
		return ev, payload, false
	}
...
type savepoint struct {
	...
	// commitOnRelease is set if the special syntax "SAVEPOINT znbase_restart"
	...
	// retriable error and issue another "ROLLBACK TO SAVEPOINT znbase_restart"
