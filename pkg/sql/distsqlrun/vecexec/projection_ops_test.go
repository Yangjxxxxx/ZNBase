...
func TestGetProjectionConstOperator(t *testing.T) {
	...
	outputIdx := 5
	oldFloat := sem.ToOldType(types1.Float)
	op, err := GetProjectionRConstOperator(testAllocator, &oldFloat, &oldFloat, binOp, input, colIdx, constArg, outputIdx)
	if err != nil {
		t.Error(err)
	}
...
func TestGetProjectionConstMixedTypeOperator(t *testing.T) {
	...
	outputIdx := 5
	oldInt1 := sem.ToOldType(types1.Int)
	oldInt2 := sem.ToOldType(types1.Int2)
	op, err := GetProjectionRConstOperator(testAllocator, &oldInt1, &oldInt2, binOp, input, colIdx, constArg, outputIdx)
	if err != nil {
		t.Error(err)
	}
	expected := &projGEInt64Int64ConstOp{
		projConstOpBase: projConstOpBase{
			OneInputNode: NewOneInputNode(input),
			allocator:    testAllocator,
			colIdx:       colIdx,
...
func TestRandomComparisons(t *testing.T) {
	...
	for _, ct := range types1.Scalar {
		...
		for _, cmpOp := range []tree.ComparisonOperator{tree.EQ, tree.NE, tree.LT, tree.LE, tree.GT, tree.GE} {
			...
			oldct := sem.ToOldType(ct)
			input := newChunkingBatchSource(typs, []coldata.Vec{lVec, rVec, ret}, numTuples)
			op, err := GetProjectionOperator(testAllocator, &oldct, &oldct, cmpOp, input, 0, 1, 2)
			if err != nil {
				t.Fatal(err)
			}
			op.Init()
...
func TestGetProjectionOperator(t *testing.T) {
	...
	col2Idx := 7
	outputIdx := 9
	op, err := GetProjectionOperator(testAllocator, &oldct, &oldct, binOp, input, col1Idx, col2Idx, outputIdx)
	if err != nil {
		t.Error(err)
	}
	expected := &projMultInt64Int64Op{
		projOpBase: projOpBase{
			OneInputNode: NewOneInputNode(input),
			allocator:    testAllocator,
			col1Idx:      col1Idx,
