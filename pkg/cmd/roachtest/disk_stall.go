...
func runDiskStalledDetection(
	...
	go func() {
		...
		out, err := c.RunWithBuffer(ctx, l, n,
			fmt.Sprintf("timeout --signal 9 %ds env ZNBASE_ENGINE_MAX_SYNC_DURATION_FATAL=true "+
				"ZNBASE_ENGINE_MAX_SYNC_DURATION=%s ZNBASE_LOG_MAX_SYNC_DURATION=%s "+
				"./znbase start --insecure --logtostderr=INFO --store {store-dir}/%s --log-dir {store-dir}/%s",
