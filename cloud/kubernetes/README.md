# ZNBaseDB on Kubernetes as a StatefulSet

This example deploys ZNBaseDB on [Kubernetes](https://kubernetes.io) as a
...
repository](https://github.com/kubernetes/kubernetes/tree/master/examples/znbasedb).
...
Note that if all you want to do is run a single znbasedb instance for
...
```shell
kubectl run znbasedb --image=znbasedb/znbase --restart=Never -- start --insecure
```
...
dynamically provisioned remote persistent volumes. Note that ZNBaseDB already
...
like to deploy ZNBaseDB across multiple geographically distributed Kubernetes
...
roles that ZNBaseDB needs to manage certificates.
...
## Creating the znbase cluster

Once your kubernetes cluster is up and running, you can launch your znbase cluster.

### Insecure mode

To create the cluster, run:
```shell
kubectl create -f znbasedb-statefulset.yaml
```
...
Run: `kubectl create -f znbasedb-statefulset-secure.yaml`
...
default.node.znbasedb-0   4s        system:serviceaccount:default:default   Pending
default.node.znbasedb-1   4s        system:serviceaccount:default:default   Pending
default.node.znbasedb-2   4s        system:serviceaccount:default:default   Pending

# Examine the CSR:
$ kubectl describe csr default.node.znbasedb-0
Name:                   default.node.znbasedb-0
Labels:                 <none>
Annotations:            <none>
...
Subject Alternative Names:
        DNS Names:      localhost
                        znbasedb-0.znbasedb.default.svc.cluster.local
                        znbasedb-public
...
$ kubectl certificate approve default.node.znbasedb-0
certificatesigningrequest "default.node.znbasedb-0" approved

# Otherwise, deny the CSR:
$ kubectl certificate deny default.node.znbasedb-0
certificatesigningrequest "default.node.znbasedb-0" denied
```
...
with. In our example, we've called this service `znbasedb-public`.

In insecure mode, start up a client pod and open up an interactive, (mostly) Postgres-flavor
SQL shell:

```shell
kubectl run znbasedb -it --image=znbasedb/znbase --rm --restart=Never \
    -- sql --insecure --host=znbasedb-public
```

In secure mode, use our `client-secure.yaml` config to launch a pod that runs indefinitely with the `znbase` binary inside it:
...
```shell
kubectl exec -it znbasedb-client-secure -- ./znbase sql --certs-dir=/znbase-certs --host=znbasedb-public
```
...
[ZNBaseDB's documentation](https://www.znbaselabs.com/docs/stable/learn-znbasedb-sql.html).

## Accessing the admin UI

If you want to see information about how the cluster is doing, you can try
pulling up the ZNBaseDB admin UI by port-forwarding from your local machine
to one of the pods:

```shell
kubectl port-forward znbasedb-0 8080
```
...
If you created an insecure znbase cluster, run:
```shell
kubectl create -f example-app.yaml
```

If you created a secure znbase cluster, run:
```shell
...
Connect to the `znbasedb-client-secure` pod (created in the "Accessing the database" section):
```shell
kubectl exec -it znbasedb-client-secure -- ./znbase sql --certs-dir=/znbase-certs --host=znbasedb-public
```
...
```shell
"/request-cert -namespace=${POD_NAMESPACE} -certs-dir=/znbase-certs -type=client -user=myapp -symlink-ca-from=/var/run/secrets/kubernetes.io/serviceaccount/ca.crt"
```
...
```shell
"postgres://myapp@znbasedb-public:26257/myappdb?sslmode=verify-full&sslcert=/znbase-certs/client.myapp.crt&sslkey=/znbase-certs/client.myapp.key&sslrootcert=/znbase-certs/ca.crt"
```
...
Once approved, the init container copies the certificate and key to `/znbase-certs` and terminates. The app container now starts, using the mounted certificate directory.
...
```shell
kubectl exec znbasedb-0 -- /bin/bash -c "while true; do kill 1; done"
```
...
```shell
kubectl scale statefulset znbasedb --replicas=4
```
...
decommissioning](https://www.znbaselabs.com/docs/stable/remove-nodes.html).

## Doing a rolling upgrade to a different ZNBaseDB version

Open up the StatefulSet's current configuration in your default text editor
using the command below, find the line specifying the `znbasedb/znbase`
image being used, change the version tag to the new one, save the file, and
quit.

```shell
kubectl edit statefulset znbasedb
```
...
ZNBaseDB, see [our
docs](https://www.znbaselabs.com/docs/stable/upgrade-znbase-version.html).
...
Because all of the resources in this example have been tagged with the label `app=znbasedb`,
we can clean up everything that we created in one quick command using a selector on that label:

```shell
kubectl delete statefulsets,pods,persistentvolumes,persistentvolumeclaims,services,poddisruptionbudget,jobs,rolebinding,clusterrolebinding,role,clusterrole,serviceaccount -l app=znbasedb
```
