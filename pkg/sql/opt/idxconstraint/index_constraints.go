...
func (c *indexConstraintCtx) makeSpansForSingleColumn(
	...
	if op == opt.InOp && memo.CanExtractConstTuple(val) {
		...
		for _, child := range tupVal.Elems {
			...
			//newdatum := memo.ExtractConstDatum(child)
			//if c.md.Table(c.TableOrd).Index(c.IndexOrd).IsFunc(){
			//	if !c.colType(offset).Equivalent(datum.ResolvedType()) {
			//		newType,err := coltypes.DatumTypeToColumnType(c.colType(offset))
			//		if err != nil {
			//			datum = newdatum
			//		}
			//		datum,err = tree.PerformCast(c.evalCtx, datum, newType)
			//		if err != nil {
			//			datum = newdatum
			//		}
			//	}
...
func (c *indexConstraintCtx) makeSpansForSingleColumnDatum(
	...
	for !c.verifyType(offset, datum.ResolvedType()) {
		newdatum := datum
		if c.md.TableNil() {
			c.unconstrained(offset, out)
			return false
...
func (c *indexConstraintCtx) makeSpansForExpr(
	...
	// TODO: uncertain use originVar or CastExpr
	_, IsManualCast := memo.IsManualCastVariable(child0)
	if c.isIndexColumn(child0, offset) || IsManualCast || c.isFuncIndexColumn(child0, offset) {
	...
	// TODO: uncertain use originVar or CastExpr
	_, IsManualCast = memo.IsManualCastVariable(child1)
	if c.isNullable(offset) && (c.isIndexColumn(child1, offset) || IsManualCast) &&
...
// isIndexColumn returns true if ev is a variable on the n indexed var that
// corresponds to index column <offset>.
func (c *indexConstraintCtx) isFuncIndexColumn(nd opt.Expr, offset int) bool {
	if c.md.TableNil() {
		return false
	}
	index := c.md.Table(c.TableOrd).Index(c.IndexOrd)
	v, ok := nd.(*memo.CastExpr)
	if !ok {
		return false
	}
	f, ok := v.Input.(*memo.FunctionExpr)
	if !ok {
		return false
	}

	columnMaps := index.GetColumnID()

	if index.IsFunc() {
		for i, columnname := range index.GetColumnNames() {
			if index.ColumnIsFunc(i) {
				expr, err := parser.ParseExpr(columnname)
				if err != nil {
					return false
				}
				...
				unresolveexpr, ok := funcexpr.Func.FunctionReference.(*tree.UnresolvedName)
				if !ok {
					return false
				}
				var argParts []string
				for _, args := range funcexpr.Exprs {
					parts, ok := args.(*tree.UnresolvedName)
					if !ok {
						return false
					}
					for num := 0; num < parts.NumParts; num++ {
						argParts = append(argParts, parts.Parts[num])
					}
				}

				for i := 0; i < unresolveexpr.NumParts; i++ {
					if f.Name == unresolveexpr.Parts[i] {
						for j := range f.Args {
							col, ok := f.Args[j].(*memo.VariableExpr)
							if ok {
								if columnMaps[int(col.Col)] == argParts[j] {
									return true
								}
							}
						}
