...
func BenchmarkAggregation(b *testing.B) {
	...
	for _, aggFunc := range aggFuncs {
		b.Run(aggFunc.String(), func(b *testing.B) {
			...
			input := runbase.NewRepeatableRowSource(sqlbase.ColumnTypesToDatumTypes(sqlbase.OneIntCol), sqlbase.MakeIntRows(numRows, numCols))
...
func benchmarkAggregationWithGrouping(b *testing.B, numOrderedCols int) {
	...
	for _, aggFunc := range aggFuncs {
		b.Run(aggFunc.String(), func(b *testing.B) {
			...
			post := &distsqlpb.PostProcessSpec{}
			disposer := &runbase.RowDisposer{}
			input := runbase.NewRepeatableRowSource(sqlbase.ColumnTypesToDatumTypes(sqlbase.ThreeIntCols), makeGroupedIntRows(groupSize, numCols, groupedCols[:]))
