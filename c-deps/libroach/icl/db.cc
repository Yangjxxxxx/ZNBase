...
class ICLEnvStatsHandler : public EnvStatsHandler {
 public:
  explicit ICLEnvStatsHandler(DataKeyManager* data_key_manager)
      : data_key_manager_(data_key_manager) {}
  virtual ~ICLEnvStatsHandler() {}
  ...
  virtual rocksdb::Status GetFileEntryKeyID(const enginepb::FileEntry* entry,
    ...
    enginepbicl::EncryptionSettings enc_settings;
    ...
    if (enc_settings.encryption_type() == enginepbicl::Plaintext) {
      *id = kPlainKeyID;
    } else {
...
// DBOpenHookICL parses the extra_options field of DBOptions and initializes
// encryption objects if needed.
rocksdb::Status DBOpenHookICL(std::shared_ptr<rocksdb::Logger> info_log, const std::string& db_dir,
  ...
  if (opts.key_source() != znbase::icl::baseicl::KeyFiles) {
  ...
  std::unique_ptr<enginepbicl::KeyInfo> store_key = store_key_manager->CurrentKeyInfo();
  ...
  env_mgr->SetStatsHandler(new ICLEnvStatsHandler(data_key_manager));
...
void* DBOpenHookICL = (void*)znbase::DBOpenHookICL;
