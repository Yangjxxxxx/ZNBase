...
func (d testVarContainer) GetVisibleType(idx int) string {
	return ""
}
...
func (d testVarContainer) SetForInsertOrUpdate(b bool) {}

// GetForInsertOrUpdate implements the parser.IndexedVarContainer interface.
func (d testVarContainer) GetForInsertOrUpdate() bool {
	return false
}
...
func TestIndexedVars(t *testing.T) {
	...
	typedExpr, err := expr.TypeCheck(semaContext, types.Any, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !typ.Equivalent(types.Int) && !typ.Equivalent(types.Int2) && !typ.Equivalent(types.Int4) {
