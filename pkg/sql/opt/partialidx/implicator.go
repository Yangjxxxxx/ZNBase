...
func (im *Implicator) warmCache(filters memo.FiltersExpr) {
	for _, f := range filters {
		op := f.Condition.Op()
		if f.ScalarProps(nil).Constraints != nil && op != opt.AndOp && op != opt.OrOp && op != opt.RangeOp {
			im.cacheConstraint(f.Condition, f.ScalarProps(nil).Constraints, f.ScalarProps(nil).TightConstraints)
		}
	}
}
...
func (im *Implicator) simplifyFiltersExpr(
	...
	for i := range e {
		...
		if s != memo.TrueSingleton {
			filters = append(filters, memo.FiltersItem{Condition: s})
		}
	}
