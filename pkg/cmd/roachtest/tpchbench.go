...
const (
	// SQL20 queries: https://github.com/znbasedb/znbase/blob/master/pkg/workload/querybench/2.1-sql-20
...
// sql20bench runs the group of sql20 queries against ZNBaseDB clusters in
...
func runSQL20Bench(ctx context.Context, t *test, c *cluster, b tpchBenchSpec) {
	...
	url := `https://raw.githubusercontent.com/znbasedb/znbase/master/pkg/workload/querybench/2.1-sql-20`
...
func loadTPCHBench(
	...
	if _, err := db.ExecContext(ctx, `USE tpch`); err == nil {
		...
		if err := db.QueryRowContext(
			...
			if pqErr, ok := err.(*pq.Error); !(ok && pgcode.MakeCode(string(pqErr.Code)) == pgcode.UndefinedTable) {
				return err
			}
	...
	tpchURL := fmt.Sprintf("gs://znbase-fixtures/workload/tpch/scalefactor=%d/backup", b.ScaleFactor)
