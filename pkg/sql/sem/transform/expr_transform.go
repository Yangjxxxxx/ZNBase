...
type ExprTransformContext struct {
	...
	normalizeVisitors     [11]tree.NormalizeVisitor
...
// NormalizeExpr1 is a wrapper around EvalContex.NormalizeExpr which
// avoids allocation of a normalizeVisitor. See normalize.go for
// details.
func (t *ExprTransformContext) NormalizeExpr1(
	ctx *tree.EvalContext, i int, typedExpr tree.TypedExpr,
) (tree.TypedExpr, error) {
	if ctx.SkipNormalize {
		return typedExpr, nil
	}
	t.normalizeVisitors[i] = tree.MakeNormalizeVisitor(ctx)
	expr, _ := tree.WalkExpr(&t.normalizeVisitors[i], typedExpr)
	if err := t.normalizeVisitors[i].Err(); err != nil {
		return nil, err
	}
	return expr.(tree.TypedExpr), nil
}
