...
func (ex *connExecutor) populatePrepared(
	...
	protoTS, err := p.asOfSnapshot(ctx, stmt.AST)
	if err != nil {
		return 0, err
	}
	if protoTS != nil {
	...
	if p.curPlan.plan == nil {
		...
		//
		// Note: we're comznbaseng `flags` which comes from
...
func (ex *connExecutor) execBind(
	...
	if stmt, ok := ps.AST.(*tree.Execute); ok {
		ps, ok = ex.extraTxnState.prepStmtsNamespace.prepStmts[stmt.Name.String()]
		if !ok {
			return retErr(pgerror.NewErrorf(
				pgcode.InvalidSQLStatementName,
				"unknown prepared statement %q", stmt.Name))
		}
	}
	...
	if len(bindCmd.OutFormats) == 1 {
		...
		if numCols == 0 {
			//solve panic cause by when zndb connect by JDBC but cursor do not take the optimization path
			columnFormatCodes = *new([]pgwirebase.FormatCode)
		} else {
...
func (ex *connExecutor) execDescribe(
	...
	case pgwirebase.PreparePortal:
		...
		if stmtHasNoData(portal.Stmt.AST) {
			...
			// table
			for i := range portal.Stmt.Columns {
				for _, t := range ex.extraTxnState.tables.leasedTables {
					for _, k := range t.Columns {
						if k.Name == portal.Stmt.Columns[i].Name {
							portal.Stmt.Columns[i].Typ = types.MakeTypeByVisibleTypName(portal.Stmt.Columns[i].Typ, k.Type.VisibleTypeName)
							portal.Stmt.Columns[i].VisibleTypeName = k.VisibleType()
						}
					}
				}
			}
