...
func TestInternalError(t *testing.T) {
	...
	testData := []struct {
	...
	}{
		{
			...
			[]pred{
				...
				{"basefields", func(t *testing.T, e *Error) {
					...
					eq(t, e.Code.String(), pgcode.Internal.String())
		...
		},
		{
			...
			NewError(pgcode.Syntax, "syn %s"),
			[]pred{
				{"basefields", func(t *testing.T, e *Error) {
					eq(t, e.Message, "syn %s")
					eq(t, e.Code.String(), pgcode.Syntax.String())
					m(t, e.Source.File, ".*internal_errors_test.go")
					eq(t, e.Source.Function, "TestInternalError")
				}},
		...
		},
		{
			...
			Wrapf(NewError(pgcode.Syntax, "syn foo"), pgcode.AdminShutdown, "wrap %s", "waa"),
			[]pred{
				{"basefields", func(t *testing.T, e *Error) {
		...
		},
		{
			...
			[]pred{
				{"basefields", func(t *testing.T, e *Error) {
					...
					// New code was added.
					eq(t, e.Code.String(), pgcode.AdminShutdown.String())
					// Source info is stored.
					m(t, e.Source.File, ".*internal_errors_test.go")
					eq(t, e.Source.Function, "TestInternalError")
				}},
		...
		},
		{
			...
			NewAssertionErrorWithWrappedErrf(NewError(pgcode.Syntax, "syn err"), "iewrap %s", "waa"),
			[]pred{
				{"basefields", func(t *testing.T, e *Error) {
					// Wrap adds a prefix to the message.
...
func makeNormal() error {
	return NewError(pgcode.Syntax, "syn")
}
