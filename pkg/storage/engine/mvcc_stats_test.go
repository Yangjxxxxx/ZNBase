...
func TestMVCCStatsDeleteCommitMovesTimestamp(t *testing.T) {
	...
	if _, err := MVCCResolveWriteIntent(ctx, engine, aggMS, roachpb.LockUpdate{
...
//
// See https://github.com/znbasedb/znbase/issues/21112.
...
func (s *randomTest) step(t *testing.T) {
	...
	if s.Txn != nil {
		...
		s.Txn.WriteTimestamp.Forward(s.TS)
		s.Txn.Sequence++
	}
	s.cycle++
...
func TestMVCCStatsRandomized(t *testing.T) {
	...
	actions["EnsureTxn"] = func(s *state) string {
		if s.Txn == nil {
			s.Txn = &roachpb.Transaction{TxnMeta: enginepb.TxnMeta{ID: uuid.MakeV4(), WriteTimestamp: s.TS}}
		}
		return ""
	}
