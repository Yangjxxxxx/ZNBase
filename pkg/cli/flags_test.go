...
func TestNoLinkForbidden(t *testing.T) {
	...
	// Verify that the znbase binary doesn't depend on certain packages.
	buildutil.VerifyNoImports(t,
		"github.com/znbasedb/znbase/pkg/cmd/znbase", true,
		[]string{
			"testing",  // defines flags
			"go/build", // probably not something we want in the main binary
			"github.com/znbasedb/znbase/pkg/security/securitytest", // contains certificates
		},
		[]string{
			"github.com/znbasedb/znbase/pkg/testutils", // meant for testing code only
		})
}
	...
	for _, test := range testData {
		for _, cmdName := range test.cmds {
			t.Run(fmt.Sprintf("%s/%s", cmdName, strings.Join(test.flags, " ")), func(t *testing.T) {
				cmd, _, _ := znbaseCmd.Find([]string{cmdName})
...
func TestSpaceFlagValue(t *testing.T) {
	defer leaktest.AfterTest(t)()

	// Avoid leaking configuration changes after the tests end.
	defer initCLIDefaults()
	unOrderStringsEqual := func(a, b []string) bool {
		m := map[string]int{}
		for _, v := range a {
			m[v]++
		}
	...
	testData := []struct {
		...
		{[]string{"start", "--" + cliflags.Space.Name, "china"}, []string{"china"}},
		{[]string{"start", "--" + cliflags.Space.Name, "china,tianjin"}, []string{"china", "tianjin"}},
		{[]string{"start", "--" + cliflags.Space.Name, "china,tj,dongli"}, []string{"china", "tj", "dongli"}},
		{[]string{"start", "--" + cliflags.Space.Name, "china", "--" + cliflags.Space.Name, "tianjin"}, []string{"china", "tianjin"}},
		{[]string{"start", "--" + cliflags.Space.Name, "china,tj", "--" + cliflags.Space.Name, "dongli"}, []string{"china", "tj", "dongli"}},
		{[]string{"start", "--" + cliflags.Space.Name, "china,tj", "--" + cliflags.Space.Name, "dongli,tj"}, []string{"china", "tj", "dongli"}},
	}
	...
	for i, td := range testData {
		...
		if !unOrderStringsEqual(td.expected, serverCfg.LocationName.Duplicate().Names) {
			t.Errorf("%d. serverCfg.LocationName expected '%s', but got '%s'. td.args was '%#v'.", i, td.expected, serverCfg.LocationName.String(), td.args)
		}
	}
}
