...
func NewProtocolViolationErrorf(format string, args ...interface{}) error {
	return pgerror.NewErrorf(pgcode.ProtocolViolation, format, args...)
}
...
var invalidUTF8Error = pgerror.NewErrorf(pgcode.CharacterNotInRepertoire, "invalid UTF-8 sequence")
...
// OidGetDefaultDatum From oid get default datum
func OidGetDefaultDatum(id oid.Oid) (tree.Datum, error) {
	switch id {
	case oid.T_bool:
		return tree.MakeDBool(tree.DBool(false)), nil
	case oid.T_int2, oid.T_int4, oid.T_int8:
		return tree.NewDInt(tree.DInt(0)), nil
	case oid.T_float4, oid.T_float8:
		return tree.NewDFloat(tree.DFloat(0)), nil
	case oid.T_numeric:
		d, err := tree.ParseDDecimal(string("0"))
		if err != nil {
			return nil, errors.Errorf("could not parse string %q as decimal", d.String())
		}
		return d, nil
	}
