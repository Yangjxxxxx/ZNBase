...
const (
	// TStringVariantSTRING is the canonical ZNBaseDB string type.
	//
	...
	// TStringVariantCHARACTER is same as TStringVariantCHAR.
	TStringVariantCHARACTER
	...
	// TStringVariantVOID only use for UDR's return type
	TStringVariantVOID

	// TStringVariantSET only use for SET type
	TStringVariantSET
)
...
func (v TStringVariant) String() string {
	...
	case TStringVariantCHAR:
		return "CHAR"
	case TStringVariantCHARACTER:
		return "CHAR"
	case TStringVariantVARCHAR:
		return "VARCHAR"
	case TStringVariantQCHAR:
		return `"char"`
	case TStringVariantVOID:
		return "VOID"
	default:
		return "STRING"
	}
...
// TypeName implements the ColTypeFormatter interface.
func (node *TVoid) TypeName() string { return node.Variant.String() }

// Format implements the ColTypeFormatter interface.
func (node *TVoid) Format(buf *bytes.Buffer, f lex.EncodeFlags) {
	buf.WriteString(node.TypeName())
}
...
type TBytes struct{ VisibleType }
