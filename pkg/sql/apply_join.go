...
func (a *applyJoinNode) Next(params runParams) (bool, error) {
	...
	for {
		...
		if params.p.execCfg.LocationName.Names != nil {
			a.optimizer.InitXQ(params.p.execCfg.LocationName)
		}
...
func runPlanInsidePlan(
	...
	ExecInUDR = false
	params.p.semaCtx.SetInUDR(false)
	params.p.extendedEvalCtx.ExecCfg.DistSQLPlanner.PlanAndRun(
		params.ctx, evalCtx, planCtx, params.p.Txn(), plan.main, recv)
	if recv.commErr != nil {
		return recv.commErr
	}
	if evalCtx.RecursiveData != nil && evalCtx.RecursiveData.FormalJoin {
		evalCtx.RecursiveData.FormalJoin = false
	}
...
func (a *applyJoinNode) SetDataStoreEngine(ds cat.DataStoreEngine) {
	a.storeEngine = ds
}

func (a *applyJoinNode) GetDataStoreEngine() cat.DataStoreEngine {
	return a.storeEngine
}
