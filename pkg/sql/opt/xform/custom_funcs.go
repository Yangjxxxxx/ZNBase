...
import (
	...
	"github.com/znbasedb/znbase/pkg/sql/opt/idxconstraint"
	...
	"github.com/znbasedb/znbase/pkg/sql/opt/partialidx"
...
// GeneratePartialIndexScans generate PartialIndexScans
func (c *CustomFuncs) GeneratePartialIndexScans(
	...
	if tabMeta.PartialIndexPredicates == nil {
		return
	}
	...
	for iter.next() {
		...
		if isPartialIndex {
			...
			remainingFiltersDeal, ok := c.im.FiltersImplyPredicate(filters, *pred.(*memo.FiltersExpr))
			if !ok {
				// The filters do not imply the predicate, so the partial index
				// cannot be used.
				continue
			}
			remainingFilters = remainingFiltersDeal
		}
		...
		newScanPrivate.PartialIndex = isPartialIndex
...
func (c *CustomFuncs) GenerateIndexScans(grp memo.RelExpr, scanPrivate *memo.ScanPrivate) {
	...
	for iter.next() {
		...
		_, isPartialIndex := md.Table(scanPrivate.Table).Index(iter.indexOrdinal).Predicate()
		if isPartialIndex {
			pred := tabMeta.PartialIndexPredicates[iter.indexOrdinal].(*memo.FiltersExpr)
			if !pred.IsTrue() {
				continue
			}
		}
		...
		if !(scanPrivate.Flags.IndexHintType == keys.IndexHintForce) && scanPrivate.Flags.IndexHintType == keys.IndexNoHint {
			continue
		}
...
func (c *CustomFuncs) initIdxConstraintForIndex(
	...
	for i := range columns {
		col := index.ColumnForFunc(i)
		var colID opt.ColumnID
		if i != len(columns)-1 && index.ColumnIsFunc(i) {
			colID = opt.ColumnID(index.ColumnGetID(i, col.Ordinal))
		} else {
...
func (c *CustomFuncs) GenerateMergeJoins(
	...
	for _, o := range orders {
		...
		merge.SetDataStoreEngine()
		c.e.mem.AddMergeJoinToGroup(&merge, grp)
	}

	if (n == 0 || len(orders) == 0) && joinPrivate.Flags.DisallowHashJoin && joinPrivate.Flags.DisallowLookupJoin {
		merge := memo.MergeJoinExpr{Left: left, Right: right, On: remainingFilters}
		merge.JoinPrivate = *joinPrivate
		merge.JoinType = originalOp
		merge.LeftEq = make(opt.Ordering, n)
...
func (c *CustomFuncs) GenerateLookupJoins(
	...
	for iter.next() {
		...
		// Find the longest prefix of index key columns that are constrained by an equality with another column or a constant.
...
func (c *CustomFuncs) GenerateZigzagJoins(
	...
	for iter.next() {
		...
		for iter2.next() {
			...
			zigzagJoin.StoreEngine = scanPrivate.EngineTypeSet
...
func (it *scanIndexIter) next() bool {
	for {
		...
		if it.scanPrivate.Flags.IndexHintType == keys.IndexHintForce && it.scanPrivate.Flags.HintIndexs[0] != it.indexOrdinal {
			continue
		}
		if it.scanPrivate.Flags.IndexHintType == keys.IndexHintIgnore {
			continueSign := false
			for _, index := range it.scanPrivate.Flags.HintIndexs {
		...
		if it.scanPrivate.Flags.IndexHintType == keys.IndexHintUse {
			find := false
			for _, index := range it.scanPrivate.Flags.HintIndexs {
				if index == it.indexOrdinal {
					find = true
...
// ShouldReorderJoinsWithSingleExpr returns whether the optimizer should attempt to find
...
func (c *CustomFuncs) ShouldReorderJoinsWithSingleExpr(root memo.RelExpr) bool {
