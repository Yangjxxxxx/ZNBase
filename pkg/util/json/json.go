...
//GetJsons Get Jsons
func (b *ArrayBuilder) GetJsons() []JSON {
	return b.jsons
}
...
//GetArrayBuilder Get ArrayBuilder
func (b *ArrayBuilderWithCounter) GetArrayBuilder() *ArrayBuilder {
	return b.ab
}
...
var errTrailingCharacters = pgerror.NewError(pgcode.InvalidTextRepresentation, "trailing characters after JSON document")
...
func ParseJSON(s string) (JSON, error) {
	...
	if err != nil {
		return nil, pgerror.NewErrorf(pgcode.InvalidTextRepresentation, "error decoding JSON: %s", err.Error())
	}
	if decoder.More() {
...
func MakeJSON(d interface{}) (JSON, error) {
	...
	return nil, errors.AssertionFailedf("invalid value %s passed to MakeJSON", d.(fmt.Stringer).String())
}
...
var errCannotSetPathInScalar = pgerror.NewError(pgcode.InvalidParameterValue, "cannot set path in scalar")
...
var errCannotReplaceExistingKey = pgerror.NewError(pgcode.InvalidParameterValue, "cannot replace existing key")
...
var errCannotDeleteFromScalar = pgerror.NewError(pgcode.InvalidParameterValue, "cannot delete from scalar")
var errCannotDeleteFromObject = pgerror.NewError(pgcode.InvalidParameterValue, "cannot delete from object using integer index")
...
var errInvalidConcat = pgerror.NewError(pgcode.InvalidParameterValue, "invalid concatenation of jsonb objects")
...
var errCannotDeletePathInScalar = pgerror.NewError(pgcode.InvalidParameterValue, "cannot delete path in scalar")
