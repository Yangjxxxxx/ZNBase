...
func (d *mysqloutfileReader) closeRejectCh(ctx context.Context) {
	if d.fileContext.rejected != nil {
		close(d.fileContext.rejected)
	}
}

func (d *mysqloutfileReader) saveRejectRecord(
...
func (d *mysqloutfileReader) readFiles(
	...
	return readInputFiles(ctx, cp, d.readFile, &d.fileContext, nil, "", details)
}
...
func (d *mysqloutfileReader) readFile(
	...
	addField := func() error {
		if len(row) >= len(d.conv.visibleCols) {
			return makeRowErr(inputName.Path, count, "too many columns, expected %d: %#v", len(d.conv.visibleCols), row)
		}
		if gotNull {
			if len(field) != 0 {
				return makeRowErr(inputName.Path, count, "unexpected data after null encoding: %s", field)
			}
			...
			datum, err := tree.ParseStringAs(d.conv.visibleColTypes[len(row)], string(field), d.conv.evalCtx, true)
			if err != nil {
	...
	addRow := func() error {
		...
		if err := d.conv.row(ctx, inputIdx, count, d.metrics); err != nil {
	...
	for {
		...
		if finished {
			if nextLiteral {
				return makeRowErr(inputName.Path, count, "unmatched literal")
			}
			if readingField {
				return makeRowErr(inputName.Path, count, "unmatched field enclosure")
			}
			if len(field) > 0 {
		...
		if d.opts.HasEscape {
			if nextLiteral {
				...
				case 'N':
					if gotNull {
						return makeRowErr(inputName.Path, count, "unexpected null encoding")
					}
					gotNull = true
				default:
	...
	return d.conv.sendBatch(ctx, d.metrics)
}
