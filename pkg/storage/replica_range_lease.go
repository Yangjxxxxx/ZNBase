			...
			if reqLease.Type() == roachpb.LeaseEpoch && status.State == storagepb.LeaseState_EXPIRED &&
				...
				if status.Lease.OwnedBy(nextLeaseHolder.StoreID) &&
					...
					if live, liveErr := p.repl.store.cfg.NodeLiveness.IsLive(nextLeaseHolder.NodeID); !live || liveErr != nil {
						...
						//
						// https://github.com/znbasedb/znbase/issues/35986
...
// IsEndKey used for Debug
func (r *Replica) IsEndKey(key roachpb.Key) error {
	r.mu.RLock()
	defer r.mu.RUnlock()
	if r.mu.state.Desc.EndKey.Equal(key) {
		return newNotLeaseHolderError(nil, r.store.StoreID(), r.mu.state.Desc)
	}
	return nil
}
			...
			case storagepb.LeaseState_VALID, storagepb.LeaseState_STASIS:
				if !status.Lease.OwnedBy(r.store.StoreID()) {
					...
					if !stillMember {
						...
						//
						// However, this is possible if the `znbase debug
