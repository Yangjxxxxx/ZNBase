...
func initFetcher(
	...
	if err := fetcher.Init(reverseScan, false /*reverse*/, false, /* isCheck */
		alloc, sqlbase.ScanLockingStrength_FOR_NONE, sqlbase.ScanLockingWaitPolicy{LockLevel: sqlbase.ScanLockingWaitLevel_BLOCK}, fetcherArgs...); err != nil {
		return nil, err
	}
...
func TestNextRowSingle(t *testing.T) {
	...
	for tableName, table := range tables {
		t.Run(tableName, func(t *testing.T) {
			tableDesc := sqlbase.GetImmutableTableDescriptor(kvDB, sqlutils.TestDB, "public", tableName)
...
func TestNextRowInterleaved(t *testing.T) {
	...
	for _, idxs := range generateIdxSubsets(len(interleaveEntries)-1, nil) {
		...
		t.Run(testName, func(t *testing.T) {
			...
			for i, entry := range entries {
				tableDesc := sqlbase.GetImmutableTableDescriptor(kvDB, sqlutils.TestDB, "public", entry.tableName)
...
func TestRowFetcherReset(t *testing.T) {
	...
	tableDesc := sqlbase.GetImmutableTableDescriptor(kvDB, sqlutils.TestDB, "public", "foo")
	var valNeededForCol util.FastIntSet
	...
	if err := resetFetcher.Init(false, false /*reverse*/, false, /* isCheck */
		&da, sqlbase.ScanLockingStrength_FOR_NONE, sqlbase.ScanLockingWaitPolicy{LockLevel: sqlbase.ScanLockingWaitLevel_BLOCK}, fetcherArgs...); err != nil {
		t.Fatal(err)
	}
