...
system "grep -qF '\[config\] clusterID:' logs/db/logs/znbase.log"
end_test
...
# Verify that "znbase.log" is not the file where the server reports
# it just started.
system "if grep -q 'restarted pre-existing node' logs/db/logs/znbase.log; then false; fi"
