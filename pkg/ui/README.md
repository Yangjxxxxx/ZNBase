...
This directory contains the client-side code for ZNBaseDB's web-based admin
UI, which provides details about a cluster's performance and health. See the
[Admin UI docs](https://www.znbaselabs.com/docs/stable/explore-the-admin-ui.html)
for an expanded overview.

## Getting Started

To start developing the UI, be sure you're able to build and run a ZNBaseDB
node. Instructions for this are located in the top-level README. Every ZNBase
...
build` in the parent directory, `embedded.go` is linked into the `znbase`
binary so that it can serve the admin UI when you run `znbase start`.
...
directory, while proxying all API requests to the specified ZNBaseDB node.

To use this proxy, in ZNBase's root directory run:
```shell
...
To proxy to a cluster started up in secure mode, in ZNBase's root directory run:
```shell
...
submodule that points at the [znbasedb/yarn-vendored] repository.
...
```bash
$ cd $GOPATH/src/github.com/znbasedb/znbase/pkg/ui
$ yarn add FOO
```
...
Be sure to push to [znbasedb/yarn-vendored] directly instead of a personal
...
impossible for future `git clone`s to build that version of ZNBase. GitHub
...
[znbasedb/yarn-vendored]: https://github.com/znbasedb/yarn-vendored
