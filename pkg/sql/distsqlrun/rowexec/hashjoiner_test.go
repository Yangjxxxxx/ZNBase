...
func TestHashOptJoiner(t *testing.T) {
	defer leaktest.AfterTest(t)()
	...
	for _, c := range testCases {
		...
		for _, memLimit := range []int64{1, 256, 512, 1024, 2048} {
			t.Run(fmt.Sprintf("MemLimit=%d", memLimit), func(t *testing.T) {
				if err := testFunc(t, func(f *runbase.FlowCtx) {
					//f.TestingKnobs().MakeTestingKnobsMemoryLimitBytes(memLimit)
					f.Cfg.TestingKnobs.MakeTestingKnobsMemoryLimitBytes(memLimit)
				}, nil); err != nil {
					t.Fatal(err)
				}
...
func checkExpectedRows(
	types []sqlbase.ColumnType, expectedRows sqlbase.EncDatumRows, results *runbase.RowBuffer,
) error {
	var expected []string
