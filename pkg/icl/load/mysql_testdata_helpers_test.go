...
// [mysqld]
// secure_file_priv=""
var rewriteMysqlTestData = envutil.EnvOrDefaultBool("ZNBASE_REWRITE_MYSQL_TESTDATA", false)

const mysqlTestDB = "znbasetestdata"
...
var everythingTestRows = func() []everythingTestRow {
	if util.EnableAQKK {
		return []everythingTestRow{
			{1, "Small", "c", []byte("bin"), timeutil.Unix(946684800, 0), 1, -2, float64(float32(-1.5)), "-12.345", `{"a": "b", "c": {"d": ["e", 11, null]}}`},
			{2, "Large", "c2", []byte("bin2"), timeutil.Unix(946684800, 0), 3525343334, 3, float64(float32(1.2)), "12.345", `{}`},
		}
	}
	return []everythingTestRow{
...
func mysqldump(t *testing.T, dest string, table string) {
	...
	args := []string{`-u`, `root`, `znbasetestdata`}
	if table != "" {
