...
func (n *splitNode) startExec(params runParams) error {
	...
	// recorded in the same transaction as the table and index descriptor update.
	params.p.curPlan.auditInfo = &server.AuditInfo{
		EventTime: timeutil.Now(),
		EventType: string(EventLogSplit),
		Info:      "User name:" + params.SessionData().User + " SHOW SYNTAX:" + params.p.curPlan.AST.String(),
	}

	// when alter table split at, the index is primary index, when alter index with split at,
	// the index maybe secondary index, but it's impossible primary index.
	if n.index.Name != "primary" {
		params.p.curPlan.auditInfo.TargetInfo = &server.TargetInfo{
			TargetID: int32(n.index.ID),
			Desc: struct {
				IndexName string
			}{
				n.index.Name,
