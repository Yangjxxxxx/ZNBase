...
var demoCmd = &cobra.Command{
	...
	Long: `
Start an in-memory, standalone, single-node ZNBaseDB instance, and open an
interactive SQL prompt to it. Various datasets are available to be preloaded as
subcommands: e.g. "znbase demo startrek". See --help for a full list.
`,
	Example: `  znbase demo`,
	Args:    cobra.NoArgs,
...
func setupTransientServer(
	...
	if !df.Changed && !sf.Changed {
		...
		// Otherwise, the demo command would cause a znbase-data
	...
	options.Add("application_name", sql.ReportableAppNamePrefix+"znbase demo")
	url := url.URL{
		Scheme:   "postgres",
...
func runDemo(cmd *cobra.Command, gen workload.Generator) error {
	...
	if cliCtx.isInteractive {
		fmt.Printf(`#
# Welcome to the ZNBaseDB demo database!
#
# You are connected to a temporary, in-memory ZNBaseDB
