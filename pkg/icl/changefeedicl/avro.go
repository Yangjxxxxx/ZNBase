...
type avroSchemaField struct {
	...
	Metadata   string         `json:"__exdb__,omitempty"`
...
type avroEnvelopeOpts struct {
	...
	beforeField, afterField, typeField, haveDdl bool
}
...
func envelopeToAvroSchema(
	...
	//It should exist
	if opts.haveDdl || opts.beforeField {
	...
	if opts.typeField {
		typeField := &avroSchemaField{
			SchemaType: []avroSchemaType{avroSchemaNull, avroSchemaString},
			Name:       `operate`,
			Default:    nil,
		}
		schema.Fields = append(schema.Fields, typeField)
	}
	if opts.haveDdl {
		typeField := &avroSchemaField{
			SchemaType: []avroSchemaType{avroSchemaNull, avroSchemaString},
...
func (r *avroEnvelopeRecord) BinaryFromRow(
	buf []byte, meta avroMetadata, beforeRow, afterRow sqlbase.EncDatumRow, ddl bool,
) ([]byte, error) {
	...
	if ddl {
		...
		if u, ok := meta[`ddl`]; ok {
			...
			opt, ok := u.(string)
			if !ok {
				return nil, errors.Errorf(`unknown metadata ddl type: %T`, u)
			}
			native[`ddl`] = goavro.Union(avroUnionKey(avroSchemaString), opt)
		}
	}
	...
	if r.opts.typeField {
		native[`operate`] = nil
		if u, ok := meta[`operate`]; ok {
			delete(meta, `operate`)
			opt, ok := u.(string)
			if !ok {
				return nil, errors.Errorf(`unknown metadata operate type: %T`, u)
			}
			native[`operate`] = goavro.Union(avroUnionKey(avroSchemaString), opt)
		}
	}
