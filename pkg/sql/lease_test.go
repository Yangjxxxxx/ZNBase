...
`
	...
	tableDesc := sqlbase.GetTableDescriptor(t.kvDB, "test", "public", "t")
...
`); err != nil {
	...
	tableDesc := sqlbase.GetTableDescriptor(kvDB, "t", "public", "foo")
...
`); err != nil {
	...
	_, err = tx.Exec("SAVEPOINT znbase_restart")
	...
	if _, err := tx.Exec(
		"SELECT zbdb_internal.force_retry('100s':::INTERVAL)"); !testutils.IsError(
		err, `forced by zbdb_internal\.force_retry\(\)`) {
		t.Fatal(err)
	}

	_, err = tx.Exec("ROLLBACK TO SAVEPOINT znbase_restart")
	require.NoError(t, err)
...
`); err != nil {
	...
	checkDeadlineErr := func(err error, t *testing.T) {
		pqe, ok := err.(*pq.Error)
		if !ok || pgcode.MakeCode(string(pqe.Code)) != pgcode.SerializationFailure ||
	...
	testutils.SucceedsSoon(t, func() error {
		if tableDesc := sqlbase.GetTableDescriptor(kvDB, "t", "public", "kv"); len(tableDesc.GCMutations) != 0 {
...
`); err != nil {
	...
	tableDesc := sqlbase.GetTableDescriptor(t.kvDB, "t", "public", "test")
...
`); err != nil {
	...
	test2Desc := sqlbase.GetTableDescriptor(t.kvDB, "t", "public", "test2")
...
`); err != nil {
	...
	go func() {
		...
		if err := txRetry.Commit(); !testutils.IsError(err,
			`TransactionRetryWithProtoRefreshError: cannot publish new versions for tables: \[\{kv1 56 1\}\], old versions still in use`,
		) {
			t.Errorf("err = %v", err)
		}
		wg.Done()
...
`); err != nil {
	...
	beforeDesc := sqlbase.GetTableDescriptor(t.kvDB, "t", "public", "before")
	afterDesc := sqlbase.GetTableDescriptor(t.kvDB, "t", "public", "after")
