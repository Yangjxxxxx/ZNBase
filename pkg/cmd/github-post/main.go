...
func listFailures(
	...
	trimmedPkgName := strings.TrimPrefix(packageName, issues.ZNBasePkgPrefix)
	...
	if timedOutTestName != "" {
		...
		if timedOutTestName == slowest.Test {
			...
			if err := f(
				ctx, title, packageName, "(unknown)" /* testName */, report, "somebody@znbaselabs.com",
			); err != nil {
				return errors.Wrap(err, "failed to post issue")
			}
		}
...
func getAuthorEmail(ctx context.Context, packageName, testName string) (string, error) {
	...
	// commits in znbasedb/znbase for commits authored by the address.
	subtests := strings.Split(testName, "/")
	testName = subtests[0]
	packageName = strings.TrimPrefix(packageName, "github.com/znbasedb/znbase/")
	cmd := exec.Command(`/bin/bash`, `-c`,
		fmt.Sprintf(`git grep -n "func %s" $(git rev-parse --show-toplevel)/%s/*_test.go`,
			testName, packageName))
	// This command returns output such as:
	// ../icl/storageccl/export_test.go:31:func TestExportCmd(t *testing.T) {
