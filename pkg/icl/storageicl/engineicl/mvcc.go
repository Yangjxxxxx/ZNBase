...
// ZNBaseDB uses that as a sentinel for key metadata anyway.
//
// Expected usage:
...
// DumpSeek advances the iterator to the first key in the engine which is >= the
// provided key.
func (i *MVCCIncrementalIterator) DumpSeek(startKey engine.MVCCKey) {
	i.iter.Seek(startKey)
	i.err = nil
	i.valid = true
	i.dumpAdvance()
...
// DumpNext advances the iterator to the next key/value in the iteration. After this
// call, Valid() will be true if the iterator was not positioned at the last
// key.
func (i *MVCCIncrementalIterator) DumpNext() {
	i.iter.Next()
	i.dumpAdvance()
}

// DumpNextKey advances the iterator to the next MVCC key. This operation is
...
// key.
func (i *MVCCIncrementalIterator) DumpNextKey() {
	i.iter.NextKey()
	i.dumpAdvance()
}

// DumpReset advances the iterator to the next key/value in the iteration. After this
// call, Valid() will be true if the iterator was not positioned at the last
// key.
func (i *MVCCIncrementalIterator) DumpReset() {
	i.err = nil
	i.valid = true
}

func (i *MVCCIncrementalIterator) dumpAdvance() {
	...
	for {
		...
		if i.meta.Txn != nil {
			...
			if i.startTime.Less(metaTimestamp) && !i.endTime.Less(metaTimestamp) {
				intent := roachpb.MakeIntent(i.meta.Txn, i.iter.Key().Key)
				intent.IntentTimestamp = i.meta.Txn.WriteTimestamp
				intents.Intents = append(intents.Intents, intent)
			}
			i.iter.Next()
			continue
		}
