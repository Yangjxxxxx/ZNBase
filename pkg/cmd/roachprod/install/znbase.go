...
// ZNBase TODO(peter): document
type ZNBase struct{}

func znbaseNodeBinary(c *SyncedCluster, i int) string {
	...
	if err != nil {
		...
		// look in the znbase repo.
		gopath := os.Getenv("GOPATH")
		if gopath == "" {
			return config.Binary
		}
		path = gopath + "/src/github.com/znbasedb/znbase/" + config.Binary
		var err2 error
...
func getZNBaseVersion(c *SyncedCluster, i int) (*version.Version, error) {
	...
	cmd := znbaseNodeBinary(c, i) + " version"
	...
	if len(matches) != 2 {
		return nil, fmt.Errorf("unable to parse znbase version output:%s", out)
	}
...
func (r ZNBase) Start(c *SyncedCluster, extraArgs []string) {
...
`, znbaseNodeBinary(c, 1), strings.Join(nodeNames, " "))
	...
	c.Parallel(display, len(nodes), p, func(i int) ([]byte, error) {
		vers, err := getZNBaseVersion(c, nodes[i])
		if err != nil {
			return nil, err
		}
		...
		binary := znbaseNodeBinary(c, nodes[i])
		...
		// of a znbase process sometimes doesn't release file locks immediately.
		...
		cmd += keyCmd +
			...
			"GOTRACEBACK=crash " +
			"ZNBASE_SKIP_ENABLING_DIAGNOSTIC_REPORTING=1 " +
			c.Env + " " + binary + " start " + strings.Join(args, " ") +
			" >> " + logDir + "/znbase.stdout 2>> " + logDir + "/znbase.stderr" +
			" || (x=$?; cat " + logDir + "/znbase.stderr; exit $x)"
		if out, err := sess.CombinedOutput(cmd); err != nil {
			return nil, errors.Wrapf(err, "~ %s\n%s", cmd, out)
		}
		// NB: if znbase started successfully, we ignore the output as it is
		// some harmless start messaging.
		return nil, nil
	})

	if bootstrapped {
		license := envutil.EnvOrDefaultString("ZNBASE_DEV_LICENSE", "")
		if license == "" {
			fmt.Printf("%s: ZNBASE_DEV_LICENSE unset: enterprise features will be unavailable\n",
				c.Name)
		}
...
if ! test -e ` + dir + `/settings-initialized ; then
  ZNBASE_CONNECT_TIMEOUT=0 ` + znbaseNodeBinary(c, 1) + " sql --url " +
				r.NodeURL(c, "localhost", r.NodePort(c, 1)) + " -e " +
				fmt.Sprintf(`"
SET CLUSTER SETTING server.remote_debugging.mode = 'any';
SET CLUSTER SETTING cluster.organization = 'ZNBase Labs - Production Testing';
...
func (ZNBase) NodeDir(c *SyncedCluster, index int) string {
	if c.IsLocal() {
		return os.ExpandEnv(fmt.Sprintf("${HOME}/local/%d/data", index))
	}
	return "/mnt/data1/znbase"
}

// LogDir implements the ClusterImpl.NodeDir interface.
func (ZNBase) LogDir(c *SyncedCluster, index int) string {
	dir := "${HOME}/logs"
	if c.IsLocal() {
...
func (ZNBase) NodeURL(c *SyncedCluster, host string, port int) string {
...
func (ZNBase) NodePort(c *SyncedCluster, index int) int {
...
func (r ZNBase) NodeUIPort(c *SyncedCluster, index int) int {
	return GetAdminUIPort(r.NodePort(c, index))
}

// SQL implements the ClusterImpl.NodeDir interface.
func (r ZNBase) SQL(c *SyncedCluster, args []string) error {
	if len(args) == 0 || len(c.Nodes) == 1 {
		...
		binary := znbaseNodeBinary(c, c.Nodes[0])
	...
	c.Parallel(display, len(c.Nodes), 0, func(i int) ([]byte, error) {
		...
		cmd += znbaseNodeBinary(c, c.Nodes[i]) + " sql --url " +
