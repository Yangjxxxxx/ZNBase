...
func TestClusterFlow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		s := tc.Server(i)
		conn, err := s.RPCContext().GRPCDialNode(s.ServingAddr(), tc.Server(i).NodeID(), rpc.DefaultClass).Connect(ctx)
		if err != nil {
			t.Fatal(err)
		}
...
func TestLimitedBufferingDeadlock(t *testing.T) {
	...
	leftValuesSpec, err := runbase.GenerateValuesSpec(sqlbase.ColumnTypesToDatumTypes(types), leftRows, 10 /* rows per chunk */)
	if err != nil {
		t.Fatal(err)
	}
	...
	rightValuesSpec, err := runbase.GenerateValuesSpec(sqlbase.ColumnTypesToDatumTypes(types), rightRows, 10 /* rows per chunk */)
	if err != nil {
		t.Fatal(err)
	}
	...
	s := tc.Server(0)
	conn, err := s.RPCContext().GRPCDialNode(s.ServingAddr(), tc.Server(0).NodeID(), rpc.DefaultClass).Connect(context.Background())
	if err != nil {
		t.Fatal(err)
	}
				...
				Knobs: base.TestingKnobs{Store: &storage.StoreTestingKnobs{
					EvalKnobs: storagebase.BatchEvalTestingKnobs{
						TestingEvalFilter: func(filterArgs storagebase.FilterArgs) *roachpb.Error {
							...
							if !strings.HasPrefix(scanReq.Key.String(), "/Table/56/1") {
								return nil
							}
			...
			for _, numRows := range []int{1, 100, 10000} {
				b.Run(fmt.Sprintf("r%d", numRows), func(b *testing.B) {
					...
					for i := range valSpecs {
						se := StreamEncoder{}
						se.Init(sqlbase.ToTypes(sqlbase.ThreeIntCols))
						for j := 0; j < numRows; j++ {
					...
					for i := 0; i < numNodes; i++ {
						s := tc.Server(i)
						conn, err := s.RPCContext().GRPCDialNode(s.ServingAddr(), tc.Server(i).NodeID(), rpc.DefaultClass).Connect(context.Background())
						if err != nil {
							b.Fatal(err)
						}
						clients = append(clients, distsqlpb.NewDistSQLClient(conn))
