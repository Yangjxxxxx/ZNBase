...
func (b *Builder) buildRelational(e memo.RelExpr) (execPlan, error) {
	...
	if opt.IsMutationOp(e) {
		if b.evalCtx.TxnReadOnly {
			return execPlan{}, pgerror.NewErrorf(pgcode.ReadOnlySQLTransaction,
	...
	b.factory.ConstructDataStoreEngineInfo(e, ep.root)
	if err != nil {
		return execPlan{}, err
	}
...
func (b *Builder) buildScan(scan *memo.ScanExpr) (execPlan, error) {
	...
	if scan.Flags.IndexHintType == keys.IndexHintForce && scan.Flags.HintIndexs[0] != scan.Index {
		idx := tab.Index(scan.Flags.HintIndexs[0])
		var err error
		if idx.IsInverted() {
			err = pgerror.NewErrorf(pgcode.Syntax, "index \"%s\" is inverted and cannot be used for this query", idx.Name())
		} else if _, isPartial := idx.Predicate(); isPartial {
			err = pgerror.NewErrorf(pgcode.Syntax,
				"index \"%s\" is a partial index that does not contain all the rows needed to execute this query",
				idx.Name())
		} else if scan.Locking != nil {
			err = pgerror.NewErrorf(pgcode.Syntax, "index \"%s\" with inner join cannot be used with for update", idx.Name())
		} else {
			// This should never happen.
			err = pgerror.NewErrorf(pgcode.Syntax, "index \"%s\" cannot be used for this query", idx.Name())
		}
		return execPlan{}, err
	}
...
func (b *Builder) buildProject(prj *memo.ProjectExpr) (execPlan, error) {
	...
	prj.Passthrough.ForEach(func(colID int) {
		res.outputCols.Set(int(colID), len(exprs))
		indexedVar := b.indexedVar(&ctx, md, opt.ColumnID(colID))
		exprs = append(exprs, indexedVar)
		meta := md.ColumnMeta(opt.ColumnID(colID))
		cols = append(cols, sqlbase.ResultColumn{
			Name: meta.Alias,
			Typ:  meta.Type,
		})
	...
	res.root, err = b.factory.ConstructRender(input.root, cols, exprs, reqOrdering, md.AllFunctions())
	if err != nil {
		return execPlan{}, err
	}
	return res, nil
...
func (b *Builder) buildHashJoin(join memo.RelExpr) (execPlan, error) {
	...
	if f := join.Private().(*memo.JoinPrivate).Flags; f.DisallowParallel {
		ParallelFlag = false
	}
	...
	case *memo.SelectExpr, *memo.ScanExpr:
		lok = true
	}
	switch rightExpr.(type) {
	case *memo.ScanExpr, *memo.SelectExpr:
		rok = true
	}
	if rok && lok {
		if leftExpr.Relational().Stats.RowCount > rightExpr.Relational().Stats.RowCount {
			forceSide = 1
		} else {
			forceSide = 0
		}
...
func (b *Builder) buildMergeJoin(join *memo.MergeJoinExpr) (execPlan, error) {
	...
	if leftOrd == nil && rightOrd == nil {
		if !join.Flags.DisallowMergeJoin && join.Flags.DisallowLookupJoin && join.Flags.DisallowHashJoin {
			colOrder := make(sqlbase.ColumnOrdering, 1)
			colOrder[0].Direction = encoding.Ascending
			leftOrd = colOrder
			rightOrd = colOrder
		}
	}
...
func (b *Builder) buildGroupBy(groupBy memo.RelExpr) (execPlan, error) {
	...
	if groupBy.Op() == opt.ScalarGroupByOp {
		...
		ep.root, err = b.factory.ConstructGroupBy(
			input.root, groupingColIdx, orderedInputCols, aggInfos, reqOrdering, groupingColOrder,
		)
	}
	if err != nil {
		return execPlan{}, err
...
func (b *Builder) buildDistinct(distinct *memo.DistinctOnExpr) (execPlan, error) {
	...
	node, err := b.factory.ConstructDistinct(input.root, distinctCols, orderedCols, reqOrdering)
	if err != nil {
		return execPlan{}, err
	}
...
func (b *Builder) buildSort(sort *memo.SortExpr) (execPlan, error) {
	...
	for i := range inputOrdering {
		if i == len(ordering) {
			return execPlan{}, errutil.AssertionFailedf("sort ordering already provided by input")
		}
...
func (b *Builder) buildInsert(ins *memo.InsertExpr) (execPlan, error) {
	if ins.IsMerge {
		b.evalCtx.SessionData.IsInternalSQL = true
	}
	...
	node, err := b.factory.ConstructInsert(
		...
		checkOrds,
		ins.NeedResults(),
		b.mem.Metadata().GetDepUpperOwner(tab.Name(), tab),
	)
	if err != nil {
		return execPlan{}, err
	}
...
func (b *Builder) buildUpsert(ups *memo.UpsertExpr) (execPlan, error) {
	...
	if ups.AdditionalCol != 0 {
		colList = append(colList, ups.AdditionalCol)
	}
	...
	additionalCol := exec.ColumnOrdinal(-1)
	if ups.CanaryCol != 0 {
		canaryCol = input.getColumnOrdinal(ups.CanaryCol)
	}
	if ups.AdditionalCol != 0 {
		additionalCol = input.getColumnOrdinal(ups.AdditionalCol)
	}
...
func (b *Builder) buildDelete(del *memo.DeleteExpr) (execPlan, error) {
	...
	node, err := b.factory.ConstructDelete(input.root, tab, fetchColOrds, del.NeedResults(), md.GetDepUpperOwner(tab.Name(), tab))
	if err != nil {
		return execPlan{}, err
	}
...
func (b *Builder) canUseDeleteRange(del *memo.DeleteExpr) bool {
	...
	if b.factory.IsContainTrigger(tab) {
		return false
	}
...
func (b *Builder) buildCreateView(cv *memo.CreateViewExpr) (execPlan, error) {
	...
	for i := range cols {
		...
		cols[i].VisibleTypeName = cv.Columns[i].VisibleType
	}
...
func (b *Builder) buildSortedInput(input execPlan, ordering opt.Ordering) (execPlan, error) {
	node, err := b.factory.ConstructSort(input.root, input.sqlOrdering(ordering), -1)
	if err != nil {
		return execPlan{}, err
	}
	return execPlan{root: node, outputCols: input.outputCols}, nil
...
func (b *Builder) buildWith(with *memo.WithExpr) (execPlan, error) {
	...
	if sel, ok := with.OriginalExpr.(*tree.Select); ok {
		stmt = sel.Select
	}
...
func (b *Builder) buildWithScan(withScan *memo.WithScanExpr) (execPlan, error) {
	...
	if e == nil {
		return execPlan{}, errors.Errorf("couldn't find With expression with ID %d", id)
	}
	...
	if maxVal, _ := e.outputCols.MaxValue(); len(withScan.InCols) == maxVal+1 {
		...
		for i := range withScan.InCols {
			...
			if !ok {
				panic(errors.Errorf("column %d not in input", log.Safe(withScan.InCols[i])))
			}
...
func (b *Builder) buildRecursiveCTE(rec *memo.RecursiveCTEExpr) (execPlan, error) {
	...
	fn := func(bufferRef exec.Node) (exec.Plan, error) {
		...
		return innerBld.factory.ConstructPlan(plan.root, innerBld.subqueries)
	}

	label := fmt.Sprintf("working buffer (%s)", rec.Name)
	var ep execPlan
	ep.root, err = b.factory.ConstructRecursiveCTE(initial.root, fn, label, rec.UnionAll)
	if err != nil {
		return execPlan{}, err
	}
	for i, col := range rec.OutCols {
...
func (b *Builder) buildFrame(input execPlan, w *memo.WindowsItem) (*tree.WindowFrame, error) {
	...
	newDef := &tree.WindowFrame{
		...
		IsIgnoreNulls: w.Frame.IsIgnoreNulls,
	}
	if boundExpr, ok := b.extractFromOffset(w.Function); ok {
		if !b.isOffsetMode(w.Frame.StartBoundType) {
			panic(pgerror.NewAssertionErrorf("expected offset to only be present in offset mode"))
		}
...
func (b *Builder) buildWindow(w *memo.WindowExpr) (execPlan, error) {
	...
	for i := range w.Windows {
		...
		if ok {
			...
			if !ok {
				panic(pgerror.NewAssertionErrorf("expected FILTER expression to be a VariableExpr"))
			}
