...
import "github.com/znbasedb/znbase/pkg/sql/sqltelemetry"
...
type FunctionDefinition struct {
	...
	Name string
	// Path is the fun path in db (0 database 1 schema 2 funcname)
	Path [4]string
...
// ResultData store result from a query execute in plgolang
type ResultData struct {
	// Datum stands for result of this function
	// TODO: need to consider return array
	Result Datum
	Err    error
}

// ExecUdrArgs used to transfer args to UDR function(not procedure)
type ExecUdrArgs struct {
	Ctx           *EvalContext
	Args          Datums
	Name          string
	Ret           chan ResultData
...
// ExecChan is a chan for execute UDR function, to transfer args from package tree to package sql
var ExecChan chan ExecUdrArgs
