...
import (
	...
	"github.com/znbasedb/pebble/bloom"
...
// MVCCKeyCompare compares znbase keys, including the MVCC timestamps.
...
func (t *pebbleTimeBoundPropCollector) Finish(userProps map[string]string) error {
	...
	userProps["znbase.ts.min"] = string(t.min)
	userProps["znbase.ts.max"] = string(t.max)
	return nil
}
...
//		true /* enableGC */, false /* forceSyncWrites */)
//	return pebbleLog
//}
...
//TODO It may need to be changed later
func (p *Pebble) UpdateState(state roachpb.StoreDescriptor_StoreState) {
	p.state = state
}

//State get current store or engine state
//TODO It may need to be changed later
func (p *Pebble) State() roachpb.StoreDescriptor_StoreState {
	return p.state
}

// LoadWAL loads the wal files from specify location using Pebble
// haven't implemented
func (p *Pebble) LoadWAL(dir string) error {
	panic("unimplemented")
}
...
func NewPebble(ctx context.Context, cfg PebbleConfig) (*Pebble, error) {
	...
	if cfg.Dir == "" {
		...
		auxDir = cfg.Opts.FS.PathJoin(cfg.Dir, AuxiliaryDir)
...
func iterateOnReader(
	...
	if start.Key.Compare(end.Key) >= 0 {
		return nil
	}
...
func (p *Pebble) LogLogicalOp(op MVICLogicalOpType, details MVICLogicalOpDetails) {
...
func (p *Pebble) GetStats() (*Stats, error) {
	...
	return &Stats{
		...
		BaseLevelFileCount:             0,
		L0Bytes:                        0,
		BaseLevelBytes:                 0,
		L0CompactionScore:              0,
		BaseLevelCompactionScore:       0,
		// 增加写失速指标
		StallL0SlowdownMicros:         0,
		StallMemtableCompactionMicros: 0,
		StallL0NumFilesMicros:         0,
		StallMicros:                   0,
		StallL0SlowdownCount:          0,
		StallMemtableCompactionCount:  0,
		StallL0NumFilesCount:          0,
		WriteStall:                    0,
	}, nil
}
...
func (p *Pebble) PreIngestDelay(ctx context.Context) bool {
	return preIngestDelay(ctx, p, p.settings)
}
...
// pebbleFile implements DBFile interface. It is used to interact with the
...
func (f *pebbleFile) Append(data []byte) error {
	_, err := f.file.Write(data)
	return err
}

// Close implements the DBFile interface.
func (f *pebbleFile) Close() error {
	return f.file.Close()
}

// Sync implements the DBFile interface.
func (f *pebbleFile) Sync() error {
	return f.file.Sync()
}

// OpenFile implements the FS interface.
func (p *Pebble) OpenFile(name string) (DBFile, error) {
	if p.InMem() {
		_ = p.fs.MkdirAll(p.fs.PathDir(name), 0755)
	}
	//var file vfs.File
	//var err error
	//if _, err = p.fs.Stat(name);
	//	testutils.IsError(err, "file does not exist") {
	...
	return &pebbleFile{db: p.db, file: file}, nil
}
...
func (p *Pebble) GetSSTables() (sstables SSTableInfos) {
	...
	for level, tables := range sstInfos {
...
func (p *Pebble) NewSortedDiskMap() diskmap.SortedDiskMap {
	return newPebbleMap(p.db, false /* allowDuplications */)
}

// NewSortedDiskMultiMap implements the diskmap.Factory interface.
func (p *Pebble) NewSortedDiskMultiMap() diskmap.SortedDiskMap {
	return newPebbleMap(p.db, true /* allowDuplicates */)
}

// UpdateOnlineParams in Pebble is not supported
func (p *Pebble) UpdateOnlineParams(params string) error {
	// Pebble目前不支持在线修改运行参数
	return nil
}
...
func (p *pebbleReadOnly) LogLogicalOp(op MVICLogicalOpType, details MVICLogicalOpDetails) {
	panic("not implemented")
}
