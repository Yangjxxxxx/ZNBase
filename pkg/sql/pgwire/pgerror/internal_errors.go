...
const assertionErrorHint = `You have encountered an unexpected error inside ZNBaseDB.

Please check docs to check whether this problem is already tracked. If you cannot 
find it there, please contact us and report the error with details. 
...
func NewAssertionErrorWithDepthf(depth int, format string, args ...interface{}) *Error {
	err := NewErrorWithDepthf(depth+1, pgcode.Internal, internalErrorPrefix+format, args...)
...
func NewAssertionErrorWithWrappedErrf(err error, format string, args ...interface{}) error {
	return WrapWithDepthf(1, err, pgcode.Internal, format, args...)
}
