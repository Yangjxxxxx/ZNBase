...
import (
	...
	"github.com/znbasedb/znbase/pkg/util/limit"
...
var storeTickSchedulerConcurrency = envutil.EnvOrDefaultInt(
	"ZNBASE_TICK_SCHEDULER_CONCURRENCY", util.Min(2*runtime.NumCPU(), 24))

var storeReadyRequestSchedulerConcurrency = envutil.EnvOrDefaultInt(
	"ZNBASE_REQUEST_SCHEDULER_CONCURRENCY", util.Min(6*runtime.NumCPU(), 72))

var logSSTInfoTicks = envutil.EnvOrDefaultInt(
	"ZNBASE_LOG_SST_INFO_TICKS_INTERVAL", 60,
)

var testingDisableConsistencyQueue = envutil.EnvOrDefaultBool("ZNBASE_DISABLE_CONSISTENCY_QUEUE", false)
...
var engineOnlineParameters = settings.RegisterStringSetting(
	"engine.online.adjustable.parameters",
	"engine.online.adjustable.parameters",
	"",
)
...
type Store struct {
	...
	tickScheduler         *raftScheduler
	readyrequestScheduler *raftScheduler
...
type StoreConfig struct {
	...
	// DumpSink creates ExternalStorage objects which allows access to external files
...
func (sc *StoreConfig) SetDefaults() {
	...
	if sc.concurrentSnapshotApplyLimit == 0 {
		...
		sc.concurrentSnapshotApplyLimit =
			envutil.EnvOrDefaultInt("ZNBASE_CONCURRENT_SNAPSHOT_APPLY_LIMIT", 1)
	}
...
func NewStore(cfg StoreConfig, eng engine.Engine, nodeDesc *roachpb.NodeDescriptor) *Store {
	...
	s.tickScheduler = newRaftScheduler(s.metrics, s, storeTickSchedulerConcurrency, typeTick)
	s.readyrequestScheduler = newRaftScheduler(s.metrics, s, storeReadyRequestSchedulerConcurrency, typeReadyRequest)
	...
	// fix bug that prevented changes to kv.bulk_io_write.concurrent_addsstable_requests from taking effect.
	...
	if s.cfg.Gossip != nil {
		...
		if testingDisableConsistencyQueue {
...
func (s *Store) SetDraining(drain bool) {
	...
	transferAllAway := func() int {
		...
		sem := quotapool.NewIntPool("Store.SetDraining", 100)
			...
			if err := s.stopper.RunLimitedAsyncTask(
				...
				func(ctx context.Context) {
					...
					if needsLeaseTransfer {
						...
						ctx = setLocateSpace(ctx, r.GetLocateSpace())
	...
	if err := contextutil.RunWithTimeout(ctx, "wait for raft leadership transfer", raftLeadershipTransferWait,
		func(ctx context.Context) error {
			...
			// Avoid retry.ForDuration because of https://github.com/znbasedb/znbase/issues/25091.
...
func (s *Store) Start(ctx context.Context, stopper *stop.Stopper) error {
	...
	if envutil.EnvOrDefaultBool("ZNBASE_ENABLE_COMPACTOR", true) {
...
func (s *Store) systemGossipUpdate(sysCfg *config.SystemConfig) {
	...
	newStoreReplicaVisitor(s).Visit(func(repl *Replica) bool {
		...
		oldZone := repl.Zone()
		replicateQueueNeeded := checkZoneReplicateQueueNeeded(*zone, *oldZone)
		repl.SetZoneConfig(zone)
		if locateSpace, err := sysCfg.GetLocateSpace(key); err != nil {
			log.Infof(context.TODO(), "failed to get locate space for key %s", key)
		} else {
			repl.SetLocateSpace(locateSpace)
		}
		...
		if replicateQueueNeeded {
			s.replicateQueue.MaybeAddAsync(ctx, repl, s.cfg.Clock.Now())
		}
		return true // more
	})
}

// checkZoneReplicateQueueNeeded checks if zone update need replicate queue to work.
func checkZoneReplicateQueueNeeded(zone, oldZone config.ZoneConfig) bool {
	if zone.NumReplicas != nil && oldZone.NumReplicas != nil && *zone.NumReplicas != *oldZone.NumReplicas ||
		!reflect.DeepEqual(zone.Constraints, oldZone.Constraints) ||
		!reflect.DeepEqual(zone.LeasePreferences, oldZone.LeasePreferences) {
		return true
	}
	return false
}
...
func splitPreApply(ctx context.Context, eng engine.ReadWriter, split roachpb.SplitTrigger) {
	...
	// replica is initialized (comznbaseng it with existing or default
	// Term and Vote).
...
func splitPostApply(
	...
	// might require the range to be split again. EnqueueTxn both the left and right
	...
	// queue may not have picked it up (due to the need for a split). EnqueueTxn
...
func (s *Store) Send(
	...
	if ba.IsSingleAddSSTableRequest() {
		...
		triggerFlag := s.engine.PreIngestDelay(ctx)
		if triggerFlag {
			s.metrics.WriteStallTime.Inc(1)
		}
...
func (s *Store) uncoalesceBeats(
	...
	s.readyrequestScheduler.EnqueueRaftRequests(toEnqueue...)
}
...
func (s *Store) HandleRaftRequest(
	...
	if enqueue {
		s.readyrequestScheduler.EnqueueRaftRequest(req.RangeID)
	}
	return nil
}
...
func (s *Store) processRaftSnapshotRequest(
	...
	if snapHeader.IsPreemptive() {
		return roachpb.NewError(errors.Errorf(`expected a raft or learner snapshot`))
	}
...
func (s *Store) enqueueRaftUpdateCheck(rangeID roachpb.RangeID) {
	s.readyrequestScheduler.EnqueueRaftReady(rangeID)
}
...
func (s *Store) processRaft(ctx context.Context) {
	...
	s.tickScheduler.Start(ctx, s.stopper, s)
	s.readyrequestScheduler.Start(ctx, s.stopper, s)
	// Wait for the scheduler worker goroutines to finish.
	s.stopper.RunWorker(ctx, s.tickScheduler.Wait)
	s.stopper.RunWorker(ctx, s.readyrequestScheduler.Wait)
...
func (s *Store) raftTickLoop(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			s.tickScheduler.EnqueueRaftTicks(rangeIDs...)
...
func (s *Store) tryGetOrCreateReplica(
	...
	repl := newReplica(rangeID, uninitializedDesc, s)
...
func (s *Store) ComputeMetrics(ctx context.Context, tick int) error {
	...
	if err := s.engine.UpdateOnlineParams(engineOnlineParameters.Get(&s.cfg.Settings.SV)); err != nil {
		return err
	}
...
func (s *Store) AllocatorDryRun(
	...
	_, err := s.replicateQueue.processOneChange(
		ctx, repl, sysCfg, canTransferLease, true /* dryRun */, false)
	if err != nil {
