...
func validateCheckExpr(
	...
	tblref := tree.TableRef{TableID: int64(tableDesc.ID), As: tree.AliasClause{Alias: "t"}, IsOnly: true}
...
func validateNotNull(
	...
	exprStr := fmt.Sprintf("%s is null", sqlutil.AddExtraMark(col.Name, sqlutil.DoubleQuotation))
	expr, err := parser.ParseExpr(exprStr)
	if err != nil {
		return err
	}
	sel := new(tree.SelectClause)
	from := new(tree.From)
	val := ctx.Value(contextStatementKey{})
	switch t := val.(type) {
	case *tree.AlterTable:
		switch t.Cmds[0].(type) {
		case *tree.AlterTableSetNotNull:
			if !t.IsAuto {
				tblref := tree.MakeUnqualifiedTableName(tree.Name(tableDesc.Name))
				from = &tree.From{Tables: []tree.TableExpr{&tblref}}
			}
		}
	}
	...
	sel = &tree.SelectClause{
		Exprs: sqlbase.ColumnsSelectors(tableDesc.Columns, false /* forUpdateOrDelete */),
		From:  from,
		Where: &tree.Where{Type: tree.AstWhere, Expr: expr.(tree.Expr)},
	}
	lim := &tree.Limit{Count: tree.NewDInt(1)}
	stmt := &tree.Select{Select: sel, Limit: lim}
	queryStr := tree.AsStringWithFlags(stmt, tree.FmtParsable)
	rows, err := ie.QueryRow(ctx, "validate not null", txn, queryStr)
	if err != nil {
		return err
	}
	if rows.Len() > 0 {
		return errors.Errorf(`column "%s" contains null values`, col.Name)
	}
	return nil
}
...
func (p *planner) validateForeignKey(
	...
	if prefix > 1 && srcIdx.ForeignKey.Match == sqlbase.ForeignKeyReference_FULL {
		...
		if rows.Len() > 0 {
			return pgerror.NewErrorf(pgcode.ForeignKeyViolation,
