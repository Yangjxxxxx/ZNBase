	...
	for _, insecure := range [...]bool{true, false} {
		...
		if err := trivialQuery(pgBaseURL); err != nil {
			if insecure {
				...
				// authentication. There is no default password for security.RootUser.
				if !testutils.IsError(err, fmt.Sprintf("pq: password authentication failed for user %s", security.RootUser)) {
...
func TestPGWireDrainOngoingTxns(t *testing.T) {
	...
	t.Run("CancelResponseSuccess", func(t *testing.T) {
		...
		if err := pgServer.DrainImpl(
			0 /* drainWait */, 20*time.Second, /* cancelWait */
		); err != nil {
			t.Fatal(err)
		}
...
func TestPGPrepareFail(t *testing.T) {
	...
	testFailures := map[string]string{
		...
		//"SELECT $1 + $1":                            "pq: could not determine data type of placeholder $1",
		"SELECT CASE WHEN TRUE THEN $1 END":         "pq: please check your return type or value in case expression!",
		"SELECT CASE WHEN TRUE THEN $1 ELSE $2 END": "pq: please check your return type or value in case expression!",
		...
		"SELECT 3 + CASE (4) WHEN 4 THEN $1 END":    "pq: please check your return type or value in case expression!",
...
func TestPGPreparedQuery(t *testing.T) {
	...
	queryTests := []struct {
		...
		{"SELECT descriptor FROM system.descriptor WHERE descriptor != $1 LIMIT 1", []preparedQueryTest{
			baseTest.SetArgs([]byte("abc")).Results([]byte("\x12=\n\x06system\x10\x01\x1a1\n\x14\n\x05admin\x12\x0b\n\x05admin\x10\x04\x18\x01\n\x13\n\x04root\x12\x0b\n\x05admin\x10\x04\x18\x01\x12\x04node")),
		}},
		{"SHOW COLUMNS FROM system.users", []preparedQueryTest{
			baseTest.
				...
				Results("usersearchpath", "STRING[]", false, "ARRAY['public']", "", "{}", false).
				Results("dbsearchpath", "STRING", true, gosql.NullBool{}, "", "{}", false),
		}},
		{"SHOW DATABASES", []preparedQueryTest{
			baseTest.Results("d", "root").Results("defaultdb", "root").Results("postgres", "root").Results("system", "node"),
		}},
		{"SHOW GRANTS ON system.users", []preparedQueryTest{
			baseTest.Results("system", "public", "users", gosql.NullString{}, sqlbase.AdminRole, sqlbase.AdminRole, "DELETE", "YES").
				Results("system", "public", "users", gosql.NullString{}, sqlbase.AdminRole, sqlbase.AdminRole, "INSERT", "YES").
				Results("system", "public", "users", gosql.NullString{}, sqlbase.AdminRole, sqlbase.AdminRole, "SELECT", "YES").
				Results("system", "public", "users", gosql.NullString{}, sqlbase.AdminRole, sqlbase.AdminRole, "UPDATE", "YES").
				Results("system", "public", "users", gosql.NullString{}, sqlbase.AdminRole, security.RootUser, "DELETE", "YES").
				Results("system", "public", "users", gosql.NullString{}, sqlbase.AdminRole, security.RootUser, "INSERT", "YES").
				Results("system", "public", "users", gosql.NullString{}, sqlbase.AdminRole, security.RootUser, "SELECT", "YES").
				Results("system", "public", "users", gosql.NullString{}, sqlbase.AdminRole, security.RootUser, "UPDATE", "YES"),
		}},
		{"SHOW INDEXES FROM system.users", []preparedQueryTest{
			baseTest.Results("users", "primary", false, 1, "username", "ASC", false, false, "global", "NO"),
		}},
		{"SHOW TABLES FROM system", []preparedQueryTest{
			baseTest.Results("authentication", gosql.NullString{}).Others(23),
		}},
		{"SHOW SCHEMAS FROM system", []preparedQueryTest{
			baseTest.Results("information_schema", gosql.NullString{}).Others(3),
		}},
		...
		{"CREATE USER IF NOT EXISTS $1 WITH PASSWORD $2", []preparedQueryTest{
			baseTest.SetArgs("abc", "Def&*123"),
			baseTest.SetArgs("woo", "wAA123^^"),
		}},
...
func TestPGWireAuth(t *testing.T) {
	...
	defer s.Stopper().Stop(context.TODO())
	{
		...
		t.Run("RootUserAuth", func(t *testing.T) {
			...
			if _, err := db.Exec(fmt.Sprintf("CREATE USER %s WITH PASSWORD '蟑♫螂Abc123';", unicodeUser)); err != nil {
				t.Fatal(err)
			}
		})
		t.Run("UnicodeUserAuth", func(t *testing.T) {
			...
			unicodeUserPgURL.User = url.UserPassword(unicodeUser, "蟑♫螂Abc123")
...
func TestHBA(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			confErr: "unimplemented: per-database HBA rules are not supported",
		},
		{
			// quoted all isn't ok since it strips the special meaning
			conf:    `host "all" all 0.0.0.0/0 cert`,
			confErr: "per-database HBA rules are not supported",
		},
		{
			// only the all hostname is supported
			conf:    `host all all hostname cert`,
			confErr: "unimplemented: hostname-based HBA rules are not supported",
		},
		{
