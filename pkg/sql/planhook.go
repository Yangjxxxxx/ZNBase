...
type PlanHookState interface {
	...
	CreateUserNode(
		ctx context.Context, nameE tree.Expr, ifNotExists bool, isRole bool, opName string, kvOptions tree.KVOptions,
	...
	GrantRole(ctx context.Context, n *tree.GrantRole) (planNode, error)
	RevokeRole(ctx context.Context, n *tree.RevokeRole) (planNode, error)
	ResolveTableDescriptor(
		ctx context.Context, tn *ObjectName, required bool,
	) (table *MutableTableDescriptor, err error)
}

//*
func (p *planner) ResolveTableDescriptor(
	ctx context.Context, tn *ObjectName, required bool,
) (table *MutableTableDescriptor, err error) {
	return p.ResolveMutableTableDescriptor(ctx, tn, required, requireTableDesc)
}
...
func (f *hookFnNode) startExec(params runParams) error {
	...
	go func() {
		tempExecInUdr := ExecInUDR
		ExecInUDR = false
		err := f.f(params.ctx, f.subplans, f.run.resultsCh)
		ExecInUDR = tempExecInUdr
		select {
