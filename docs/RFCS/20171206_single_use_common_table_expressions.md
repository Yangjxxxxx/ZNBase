...
infrastructure, which is currently missing from ZNBaseDB. This proposal aims
...
CTEs are a frequently requested feature for ZNBaseDB, both for compatibility
...
infrastructure for temporary tables to ensure that ZNBaseDB doesn't run out
of memory while executing them.

However, many CTE use cases only need the syntactic sugar provided in points 1
and 2 above. None of the queries mentioned in ZNBaseDB's CTE issue #7029 use
...
An
[initial implementation](https://github.com/znbasedb/znbase/pull/20359)
...
ZNBaseDB to execute the complete sample query suggested by the popular CTE
blog post
...
ZNBaseDB already supports using arbitrary plans as plan data sources, so the
...
it just like an ordinary ZNBaseDB plan tree.
