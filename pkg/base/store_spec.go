...
type StoreSpec struct {
	...
	// This is set by ICL code when encryption-at-rest is in use.
...
func (ss StoreSpec) String() string {
	...
	fmt.Fprintf(&buffer, "state=%s,", ss.State)
	...
	for _, split := range strings.Split(value, ",") {
		...
		case "state":
			...
			if !IsAvailable(value) {
				return StoreSpec{}, fmt.Errorf("field %s is not a valid store state", "state")
			}
			ss.State = TransformStringToStoreState(value)
		case "type":
			if value == "mem" {
				ss.InMemory = true
			} else {
...
// TransformStringToStoreState transform String to StoreDescriptor_StoreState.
func TransformStringToStoreState(state string) roachpb.StoreDescriptor_StoreState {
	IntState := roachpb.StoreDescriptor_StoreState_value[state]
	State := roachpb.StoreDescriptor_StoreState(IntState)
	return State
}

// IsAvailable judges whether s is available.
