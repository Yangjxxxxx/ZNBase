...
func TestUnresolvedIntentQueue(t *testing.T) {
	...
	adv := uiq.IncRef(txn1, txn1Key, hlc.Timestamp{}, txn1TS, false)
	require.False(t, adv)
	...
	adv = uiq.DecrRef(txn2, txn2TS, false)
	require.False(t, adv)
	...
	txn3 := uuid.MakeV4()
	adv = uiq.UpdateTS(txn3, hlc.Timestamp{WallTime: 5}, false)
	require.False(t, adv)
	require.Equal(t, 2, uiq.Len())
	...
	adv = uiq.UpdateTS(txn1, hlc.Timestamp{WallTime: 0}, false)
	require.False(t, adv)
	require.Equal(t, 2, uiq.Len())
	require.Equal(t, txn1, uiq.Oldest().txnID)
	require.Equal(t, txn1TS, uiq.Oldest().timestamp)
	...
	adv = uiq.UpdateTS(txn1, hlc.Timestamp{WallTime: 1}, false)
	require.False(t, adv)
	require.Equal(t, 2, uiq.Len())
	require.Equal(t, txn1, uiq.Oldest().txnID)
	require.Equal(t, txn1TS, uiq.Oldest().timestamp)
	...
	adv = uiq.UpdateTS(txn1, newTxn1TS, false)
	require.True(t, adv)
	require.Equal(t, 2, uiq.Len())
	require.Equal(t, txn1, uiq.Oldest().txnID)
	...
	adv = uiq.IncRef(txn1, nil, hlc.Timestamp{}, newTxn1TS, false)
	require.False(t, adv)
	require.Equal(t, 2, uiq.Len())
	...
	adv = uiq.DecrRef(txn1, newTxn1TS, false)
	require.False(t, adv)
	require.Equal(t, 2, uiq.Len())
	...
	txn5 := uuid.MakeV4()
	adv = uiq.IncRef(txn5, nil, hlc.Timestamp{}, hlc.Timestamp{WallTime: 10}, false)
	require.False(t, adv)
	...
	adv = uiq.IncRef(txn2, nil, hlc.Timestamp{}, txn2TS, false)
	require.True(t, adv)
	require.Equal(t, 1, uiq.Len())
	require.Equal(t, txn1, uiq.Oldest().txnID)
	require.Equal(t, newTxn1TS, uiq.Oldest().timestamp)
	...
	adv = uiq.DecrRef(txn1, hlc.Timestamp{}, false)
	require.True(t, adv)
	require.Equal(t, 0, uiq.Len())

	// Add new txn. Immediately decrement ref count. Should be empty again.
	txn6 := uuid.MakeV4()
	adv = uiq.IncRef(txn6, nil, hlc.Timestamp{}, hlc.Timestamp{WallTime: 20}, false)
	require.False(t, adv)
	require.Equal(t, 1, uiq.Len())
	require.Equal(t, txn6, uiq.Oldest().txnID)
	adv = uiq.DecrRef(txn6, hlc.Timestamp{}, false)
	require.True(t, adv)
	require.Equal(t, 0, uiq.Len())
