...
type joinPlanningInfo struct {
	...
	joinResultTypes     []sqlbase.ColumnType
	...
	leftPlanDistribution, rightPlanDistribution distsqlplan.PlanDistribution
	forceSide                                   int
	ParallelFlag                                bool
}
...
func (h *joinPlanningHelper) remapOnExpr(
	...
	return distsqlplan.MakeExpression(onCond, planCtx, joinColMap)
}

func (info *joinPlanningInfo) makeCoreSpec() distsqlpb.ProcessorCoreUnion {
	...
	if len(info.leftMergeOrd.Columns) == 0 {
		...
		core.HashJoiner = &distsqlpb.HashJoinerSpec{
			...
			ForceSide:            uint32(info.forceSide),
			ParallelFlag:         info.ParallelFlag,
		}
	} else {
