...
func TestTableReaderOpt(t *testing.T) {
		...
		for i := 0; i < 2; i++ {
			t.Run("", func(t *testing.T) {
				testutils.RunTrueAndFalse(t, "row-source", func(t *testing.T, rowSource bool) {
					...
					TableReaderOpt.Override(&flowCtx.Cfg.Settings.SV, true)
					TableReaderParallelNums.Override(&flowCtx.Cfg.Settings.SV, 4)
					var out runbase.RowReceiver
					var buf *runbase.RowBuffer
					if !rowSource {
						buf = &runbase.RowBuffer{}
						out = buf
					}
...
func TestMisplannedRangesMetadataOpt(t *testing.T) {
	defer leaktest.AfterTest(t)()
	ctx := context.Background()
...
func TestLimitScansOpt(t *testing.T) {
	defer leaktest.AfterTest(t)()
	ctx := context.Background()
...
func BenchmarkTableReaderOpt(b *testing.B) {
