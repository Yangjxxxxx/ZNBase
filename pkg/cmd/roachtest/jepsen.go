...
var jepsenNemeses = []struct {
	...
	// https://github.com/znbasedb/znbase/issues/35599
...
func initJepsen(ctx context.Context, t *test, c *cluster) {
	...
	c.GitClone(ctx, "https://github.com/znbasedb/jepsen", "/mnt/data1/jepsen", "tc-nightly", controller)
	...
	// Jepsen expects a tarball that expands to znbase/znbase
	// (which is not how our official builds are laid out).
	c.Run(ctx, c.All(), "tar --transform s,^,znbase/, -c -z -f znbase.tgz znbase")
...
func runJepsen(ctx context.Context, t *test, c *cluster, testName, nemesis string) {
	...
	t.Status("running")
	run(c, ctx, controller, "rm -f /mnt/data1/jepsen/znbasedb/store/latest")

	// Install the jepsen package (into ~/.m2) before running tests in
	// the znbase package. Clojure doesn't really understand
...
cd /mnt/data1/jepsen/znbasedb && set -eo pipefail && \
 ~/lein run test \
   --tarball file://${PWD}/znbase.tgz \
   --username ${USER} \
...
"`, nodesStr, testName, nemesis))
	...
	if testErr != nil {
		...
		run(c, ctx, controller, "tail -n 100 /mnt/data1/jepsen/znbasedb/invoke.log")
		...
		if err := runE(c, ctx, controller,
			`grep "Oh jeez, I'm sorry, Jepsen broke. Here's why" /mnt/data1/jepsen/znbasedb/invoke.log -A1 `+
		...
		cmd := exec.CommandContext(ctx, roachprod, "run", c.makeNodes(controller),
			...
			"tar -chj --ignore-failed-read -f- /mnt/data1/jepsen/znbasedb/store/latest /mnt/data1/jepsen/znbasedb/invoke.log /var/log/")
		...
		for _, file := range collectFiles {
			cmd := c.LoggedCommand(ctx, roachprod, "get", c.makeNodes(controller),
				"/mnt/data1/jepsen/znbasedb/store/latest/"+file,
		...
		if anyFailed {
			...
			cmd := c.LoggedCommand(ctx, roachprod, "get", c.makeNodes(controller),
				"/mnt/data1/jepsen/znbasedb/invoke.log",
