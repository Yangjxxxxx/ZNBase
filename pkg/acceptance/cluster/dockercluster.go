...
const (
	...
	networkPrefix = "znbasedb_acceptance"
)
...
// ZNBaseBinaryInContainer is the container-side path to the ZNBaseDB
// binary.
const ZNBaseBinaryInContainer = "/znbase/znbase"

var znbaseImage = flag.String("i", defaultImage, "the docker image to run")
var znbaseEntry = flag.String("e", "", "the entry point for the image")
...
// ZNBaseBinary is the path to the host-side binary to use.
var ZNBaseBinary = flag.String("b", func() string {
	rootPkg, err := build.Import("github.com/znbasedb/znbase", "", build.FindOnly)
	if err != nil {
		panic(err)
	}
	// NB: This is the binary produced by our linux-gnu build target. Changes
	// to the Makefile must be reflected here.
	return filepath.Join(rootPkg.Dir, "znbase-linux-2.6.32-gnu-amd64")
...
// DockerCluster manages a local znbase cluster running on docker. The
// cluster is composed of a "volumes" container which manages the
// persistent volumes used for certs and node data and N znbase nodes.
...
// CreateDocker creates a Docker-based znbase cluster. The stopper is used to
...
// specified volumesDir, including logs and znbase stores. If volumesDir is
...
func CreateDocker(
	...
	if *znbaseImage == defaultImage && !exists(*ZNBaseBinary) {
		log.Fatalf(ctx, "\"%s\": does not exist", *ZNBaseBinary)
	}
	...
	if volumesDir == "" {
		volumesDir, err = ioutil.TempDir("", fmt.Sprintf("znbase-acceptance-%s", clusterIDS))
		maybePanic(err)
	} else {
...
func (l *DockerCluster) initCluster(ctx context.Context) {
	...
	binds := []string{
		...
		filepath.Join(pwd, "..") + ":/go/src/github.com/znbasedb/znbase",
		filepath.Join(l.volumesDir, "logs") + ":/logs",
	}

	if *znbaseImage == defaultImage {
		path, err := filepath.Abs(*ZNBaseBinary)
		maybePanic(err)
		binds = append(binds, path+":"+ZNBaseBinaryInContainer)
	}
...
// znbaseEntryPoint returns the value to be used as
// container.Config.Entrypoint for a container running the znbase
// binary under test.
// TODO(bdarnell): refactor this to minimize globals
func znbaseEntrypoint() []string {
	var entrypoint []string
	if *znbaseImage == defaultImage {
		entrypoint = append(entrypoint, ZNBaseBinaryInContainer)
	} else if *znbaseEntry != "" {
		entrypoint = append(entrypoint, *znbaseEntry)
	}
	return entrypoint
}
...
func (l *DockerCluster) startNode(ctx context.Context, node *testNode) {
	...
	env := []string{
		"ZNBASE_SCAN_MAX_IDLE_TIME=200ms",
		"ZNBASE_CONSISTENCY_CHECK_PANIC_ON_FAILURE=true",
  ...
  logs:      %[3]s/znbase.INFO
  pprof:     docker exec -it %[4]s pprof https+insecure://$(hostname):%[5]s/debug/pprof/heap
  znbase: %[6]s

  cli-env:   ZNBASE_INSECURE=false ZNBASE_CERTS_DIR=%[7]s ZNBASE_HOST=%s:%d`,
...
// RunInitCommand runs the `znbase init` command. Normally called
...
func (l *DockerCluster) RunInitCommand(ctx context.Context, nodeIdx int) {
	...
	// [0]: https://github.com/znbasedb/znbase/pull/19753#issuecomment-341561452
...
// ExecCLI runs ./znbase <args> with sane defaults.
func (l *DockerCluster) ExecCLI(ctx context.Context, i int, cmd []string) (string, string, error) {
	cmd = append([]string{ZNBaseBinaryInContainer}, cmd...)
