...
import (
	...
	"github.com/znbasedb/znbase/pkg/icl/storageicl/engineicl/enginepbicl"
...
// 定义特定于icl的调试命令， 将加密标志添加到调试命令中
// `pkg/cli/debug.go`, 并注册回调以生成加密选项.

const (
	// 这些常量在libroach中定义。它们不应该被改变。
	plaintextKeyID       = "plain"
	keyRegistryFilename  = "INSPURCLOUDDB_DATA_KEYS"
	fileRegistryFilename = "INSPURCLOUDDB_REGISTRY"
)
...
`,
	...
	cli.VarFlag(f, &storeEncryptionSpecs, cliflagsicl.EnterpriseEncryption)
	// And other flags.
	...
	for _, cmd := range cli.DebugCmdsForRocksDB {
		...
		cli.VarFlag(cmd.Flags(), &storeEncryptionSpecs, cliflagsicl.EnterpriseEncryption)
	}
...
func fillEncryptionOptionsForStore(cfg *engine.RocksDBConfig) error {
	opts, err := baseicl.GetEncryptedOptionsForStore(cfg.Dir, storeEncryptionSpecs)
	if err != nil {
		return err
	}
...
type keyInfoByAge []*enginepbicl.KeyInfo
...
func runEncryptionStatus(cmd *cobra.Command, args []string) error {
	...
	var keyRegistry enginepbicl.DataKeysRegistry
	...
	for name, entry := range fileRegistry.Files {
		...
		if entry.EnvType != enginepb.EnvType_Plaintext && len(entry.EncryptionSettings) > 0 {
			var setting enginepbicl.EncryptionSettings
...
func getActiveEncryptionkey(dir string) (string, string, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return enginepbicl.EncryptionType_Plaintext.String(), "", nil
		}
