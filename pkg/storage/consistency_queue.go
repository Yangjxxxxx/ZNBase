...
// ConsistencyQueueConcurrency is used to adjust range consistency. We allow to
// adjust the parallelism of replicate queue process.
var ConsistencyQueueConcurrency = settings.RegisterPositiveIntSetting(
	"kv.consistency.queue_concurrency",
	"consistencyQueue is used to check range consistency. "+
		"This cluster setting is used to adjust "+
		"the degree of parallelism dynamically.",
	1,
)

var testingAggressiveConsistencyChecks = envutil.EnvOrDefaultBool("ZNBASE_CONSISTENCY_AGGRESSIVE", false)
...
func newConsistencyQueue(store *Store, gossip *gossip.Gossip) *consistencyQueue {
	...
	q.baseQueue = newBaseQueue(
		...
		queueConfig{
			...
			maxConcurrency:       ConsistencyQueueConcurrency.Get(&store.ClusterSettings().SV),
			needsLease:           true,
	...
	confChanged := func() {
		q.maxConcurrency = ConsistencyQueueConcurrency.Get(&store.ClusterSettings().SV)
		q.confCh <- struct{}{}
	}
	ConsistencyQueueConcurrency.SetOnChange(&store.ClusterSettings().SV, confChanged)
	return q
}
...
func (q *consistencyQueue) process(
	...
	resp, pErr := repl.CheckConsistency(ctx, req, false)
	if pErr != nil {
