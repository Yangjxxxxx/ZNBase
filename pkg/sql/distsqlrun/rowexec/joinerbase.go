...
func (jb *joinerBase) init(
	...
	jb.eqCols[leftSide] = runbase.Columns(leftEqColumns)
	jb.eqCols[rightSide] = runbase.Columns(rightEqColumns)
...
func (jb *joinerBase) render1(
	lrow, rrow sqlbase.EncDatumRow, rows []sqlbase.EncDatumRow, j uint32,
) error {
	n := jb.numMergedEqualityColumns
	if rows[j] == nil {
		rows[j] = make(sqlbase.EncDatumRow, n+len(lrow)+len(rrow))
	}
	for i := 0; i < n; i++ {
		// This function is called only when lrow and rrow match on the equality
		// columns which can never happen if there are any NULLs in these
		// columns. So we know for sure the lrow value is not null
		rows[j][i] = lrow[jb.eqCols[leftSide][i]]
	}
	copy(rows[j][n:], lrow)
	copy(rows[j][n+len(lrow):], rrow)

	if jb.onCond.Expr != nil {
		res, err := jb.onCond.EvalFilter(rows[j])
		if !res || err != nil {
			rows[j] = nil
			return err
		}
...
func (jb *joinerBase) renderUnmatchedRow1(
	row sqlbase.EncDatumRow, side joinSide, rows []sqlbase.EncDatumRow, j uint32,
) {
	...
	rows[j] = rows[j][:0]
	for idx := 0; idx < jb.numMergedEqualityColumns; idx++ {
		rows[j] = append(rows[j], row[jb.eqCols[side][idx]])
	}
	rows[j] = append(rows[j], lrow...)
	rows[j] = append(rows[j], rrow...)
}
