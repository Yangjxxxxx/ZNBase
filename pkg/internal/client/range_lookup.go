...
// distinct range in the cluster. They map the logical keyspace in znbase to
// its physical replicas, allowing a node to send requests for a certain key to
// the replicas that contain that key.
//
// RangeDescriptors are stored as values in the znbase cluster's key-value
...
//
//
// It is often useful to think of ZNBase's ranges as existing in a three
// level tree:
//
