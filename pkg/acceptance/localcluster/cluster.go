...
func repoRoot() string {
	root, err := build.Import("github.com/znbasedb/znbase", "", build.FindOnly)
	if err != nil {
		panic(fmt.Sprintf("must run from within the znbase repository: %s", err))
	}
	return root.Dir
}

// SourceBinary returns the path of the znbase binary that was built with the
// local source.
func SourceBinary() string {
	return filepath.Join(repoRoot(), "znbase")
}

const listeningURLFile = "znbasedb-url"
...
type ClusterConfig struct {
	...
	Binary      string             // path to znbase, defaults go <znbase_repo>/znbase
	AllNodeArgs []string           // args to pass to ./znbase on all nodes
...
type NodeConfig struct {
	...
	ExtraArgs         []string // extra arguments for ./znbase start
...
func (c *Cluster) isReplicated() (bool, string) {
	db := c.Nodes[0].DB()
	rows, err := db.Query(`SELECT range_id, start_key, end_key, array_length(replicas, 1) FROM zbdb_internal.ranges`)
	if err != nil {
		// Versions <= 1.1 do not contain the zbdb_internal table, which is what's used
		...
		if testutils.IsError(err, "(table|relation) \"zbdb_internal.ranges\" does not exist") {
			return true, ""
		}
		log.Fatal(context.Background(), err)
	}
...
func (n *Node) startAsyncInnerLocked(ctx context.Context, joins ...string) error {
	...
	n.cmd.Env = append(n.cmd.Env, "ZNBASE_SCAN_MAX_IDLE_TIME=5ms") // speed up rebalancing
...
func (n *Node) advertiseAddrFile() string {
	return filepath.Join(n.Cfg.DataDir, "znbase.advertise-addr")
}

func (n *Node) httpAddrFile() string {
	return filepath.Join(n.Cfg.DataDir, "znbase.http-addr")
}
	...
	for r := retry.Start(opts); r.Next(); {
...

		{
			...
			if err := n.db.QueryRow(
				`SELECT value FROM zbdb_internal.node_runtime_info WHERE component='UI' AND field = 'URL'`,
			).Scan(&uiStr); err != nil {
				log.Info(ctx, err)
				return nil
			}
