...
import (
	...
	clog "github.com/znbasedb/znbase/pkg/util/log"
...
rm -fr /mnt/data*/{auxiliary,local,tmp,cassandra,znbase,znbase-temp*,mongo-data} \; ;
rm -fr {certs*,logs} ;
`
		}
		return sess.CombinedOutput(cmd)
...
func (c *SyncedCluster) Status() {
	...
	c.Parallel(display, len(c.Nodes), 0, func(i int) ([]byte, error) {
		...
		binary := znbaseNodeBinary(c, c.Nodes[i])
...
if [ -n "${out}" -a -n "${vers}" ]; then
  echo ${out} | sed "s/znbase/znbase-${vers}/g"
else
  echo ${out}
fi
`
...
func (c *SyncedCluster) Monitor() chan NodeMonitorInfo {
	...
	for i := range nodes {
		go func(i int) {
			...
			// On each monitored node, we loop looking for a znbase process. In
...
// ZNBaseVersions TODO(peter): document
func (c *SyncedCluster) ZNBaseVersions() map[string]int {
	sha := make(map[string]int)
	var mu syncutil.Mutex

	display := fmt.Sprintf("%s: znbase version", c.Name)
	nodes := c.ServerNodes()
	c.Parallel(display, len(nodes), 0, func(i int) ([]byte, error) {
		sess, err := c.newSession(c.Nodes[i])
		if err != nil {
...
// `znbase debug merge-logs <dest>/*/*` with the optional flag for filter.
...
func (c *SyncedCluster) Logs(
	...
	mergeLogs := func(ctx context.Context, prev, t time.Time) error {
		cmd := exec.CommandContext(ctx, "znbase", "debug", "merge-logs",
			dest+"/*/*",
		...
		if err := cmd.Run(); err != nil && ctx.Err() == nil {
			return fmt.Errorf("failed to run znbase debug merge-logs:%v\n%v",
				err, errBuf.String())
		}
		return nil
	}
	...
	go func() { <-ch; cancel() }()
	// TODO(ajwerner): consider SIGHUP-ing znbase before the rsync to avoid the delays
	prev := from
	if prev.IsZero() {
