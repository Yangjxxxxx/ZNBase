...
The magic of ZNBaseDB is its ability to stitch together physically-disjoint
...
ZNBaseDB’s default range allocation policy might be a poor fit:
...
but would bring back some of the pain of manual sharding that ZNBaseDB was
...
preserving the magic of ZNBaseDB: that is, a logical table that hides the
...
ships live znbasees. RoachMart processes several hundred requests per second,
each of which might generate several queries to RoachMart’s nine-node
ZNBaseDB cluster. (Live znbasees are very popular these days.) To keep
...
In ZNBaseDB, partitions are cheap and easy to redefine at will, with one
...
So, RoachMart restarts each of its ZNBaseDB nodes with a `--locality` flag
that indicates its data center:

```
$ ./znbase start --locality=datacenter=au1
$ ./znbase start --locality=datacenter=us1
```
...
$ ./znbase zone set roachmart.users --partition=australia -f australia.zone.yml
$ ./znbase zone set roachmart.users --partition=north_america -f north_america.zone.yml
```
...
per day. (ZNBase enthusiasts are prolific.)
...
```
$ ./znbase start --store=path=/mnt/znbase,attrs=ssd
$ ./znbase start --store=path=/mnt/znbase,attrs=hdd
```
...
constraints: [+hdd]
$ ./znbase zone set roachblog.articles --partition=recent -f recent.zone.yml
$ ./znbase zone set roachblog.articles --partition=archived -f archived.zone.yml
```
...
- After a table is partitioned, its partitions can be targeted by zone
  constraints via the existing CLI, e.g.  `./znbase zone set
  database.table --partition=partition -f zone.yml`.

- As before, ZNBaseDB uses the most granular zone config available. Zone
...
```bash
# Before:
$ ./znbase zone {set|get|rm} DATABASE[.TABLE[@INDEX]] [--partition=PARTITION] -f zone.yml

# After:
$ ./znbase zone {set|get|rm} DATABASE[.TABLE] f zone.yml
```

Omitting the index but specifying the `--partition` flag (e.g., `./znbase
zone set db.tbl --partition=p0`) implies a partition of the primary index. Note,
however, that `./znbase set db.tbl` and `./znbase set db.tbl@primary` are
...
- Partition configs outside of the database+table scoping. This could be useful
  for allowing partitioning in a shared ZNBaseDB offering with static, preset
  zone configs.
...
The ZNBaseDB unit of replication and rebalancing is the range. So for zone
...
Currently, ZNBaseDB asynchronously splits each newly created table into its
...
ZNBaseDB has a number of pieces that can be combined to deal with this. NB:
...
3. A list partitioning without a `DEFAULT` is an enum. The user can use the
   ...
   version of ZNBaseDB supports enum types, the user would get this behavior
...
4. This is not a full table scan in other partitioning implementations because
   ...
   in ZNBaseDB by introducing and partitioning on a derived `partition_id`
...
The unit of replication in ZNBaseDB is the range, so partitioning necessarily
...
[#14113]: https://github.com/znbasedb/znbase/issues/14113
[#18683]: https://github.com/znbasedb/znbase/pull/18683
[#19141]: https://github.com/znbasedb/znbase/issues/19141
[computed columns]: https://github.com/znbasedb/znbase/pull/20735
[example: date partitioning]: #example-date-partitioning
[example: geographic partitioning]: #example-geographic-partitioning
[index key prefix]: https://github.com/znbasedb/znbase/blob/1f3c72f17546f944490e0a4dcd928fd96a375987/docs/RFCS/sql_partitioning.md#key-encoding
[interleaved tables]: https://www.znbaselabs.com/docs/stable/interleave-in-parent.html
[leases follow the sun]: https://github.com/znbasedb/znbase/blob/763d21e6fad69728a523d3cdd8b449c8513094b7/docs/RFCS/20170125_leaseholder_locality.md
...
[system.subzones]: https://github.com/znbasedb/znbase/blob/1f3c72f17546f944490e0a4dcd928fd96a375987/docs/RFCS/sql_partitioning.md#table-subzones
[zone config]: https://www.znbaselabs.com/docs/stable/configure-replication-zones.html
