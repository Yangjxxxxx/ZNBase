// Copyright 2018 The Drdb Authors.
//
...
func (ef *execFactory) ConstructScan(
	...
	scan.storeEngine = table.DataStoreEngineInfo()
	if locking != nil {
...
func (ef *execFactory) ConstructRender(
	...
	for i, expr := range exprs {
		exprs[i] = rb.r.ivarHelper.Rebind(expr, false, true)
	}
	rb.setOutput(exprs, columns)

	// append function descriptor to render node
	for _, udrFunctions := range functions {
		if udrFunc, ok := udrFunctions.(*optUdrFunction); ok {
			for _, Group := range udrFunc.desc.FuncGroup {
				rb.r.FunctionDescCollection = append(rb.r.FunctionDescCollection, *Group.Desc)
			}
		}
	}
...
func (ef *execFactory) ConstructHashJoin(
	...
	return p.makeJoinNode(leftSrc, rightSrc, pred, forceSide, ParallelFlag), nil
}
...
func (ef *execFactory) ConstructMergeJoin(
	...
	node := p.makeJoinNode(leftSrc, rightSrc, pred, 2, false)
...
func (ef *execFactory) addAggregations(n *groupNode, aggregations []exec.AggInfo) error {
	...
	for i := range aggregations {
		...
		builtin := agg.Builtin
		var renderIdx int
		var aggFn func(*tree.EvalContext, tree.Datums, *mon.BoundAccount) tree.AggregateFunc

		switch len(agg.ArgCols) {
		case 0:
			renderIdx = noRenderIdx
			aggFn = func(evalCtx *tree.EvalContext, arguments tree.Datums, account *mon.BoundAccount) tree.AggregateFunc {
				return builtin.AggregateFunc([]types.T{}, evalCtx, arguments, account)
			}

		case 1:
			renderIdx = int(agg.ArgCols[0])
			aggFn = func(evalCtx *tree.EvalContext, arguments tree.Datums, account *mon.BoundAccount) tree.AggregateFunc {
				return builtin.AggregateFunc([]types.T{inputCols[renderIdx].Typ}, evalCtx, arguments, account)
			}
...
func (ef *execFactory) ConstructIndexJoin(
	...
	//Skip UDR function & UPSERT privilege check (they has been checked in opt builder)
	if !ef.planner.skipSelectPrivilegeChecks {
		ef.planner.skipSelectPrivilegeChecks = true
		defer func() { ef.planner.skipSelectPrivilegeChecks = false }()
	}
	if ef.planner.skipSelectPrivilegeChecks {
	...
	// inner join的lock信息传递
	tableScan.lockingStrength = scan.lockingStrength
	tableScan.lockingWaitPolicy = scan.lockingWaitPolicy
...
func (ef *execFactory) ConstructLookupJoin(
	...
	n := &lookupJoinNode{
		...
		reqOrdering: sqlbase.ColumnOrdering(reqOrdering),
	}
...
func (ef *execFactory) ConstructPlan(
	...
	return constructPlan(ef.planner, root, subqueries)
}

func constructPlan(
	planner *planner, root exec.Node, subqueries []exec.Subquery,
) (exec.Plan, error) {
	res := &planComponents{}
	assignPlan := func(plan *planMaybePhysical, node exec.Node) {
		...
		case planMaybePhysical:
			*plan = n
		default:
			panic(errors.Errorf("unexpected node type %T", node))
		}
	}
...
func (ef *execFactory) showEnv(plan string, envOpts exec.ExplainEnvData) (exec.Node, error) {
	...
	// Show the version of ZNBase running.
...
func (ef *execFactory) ConstructInsert(
	...
	fkTables, err := ef.makeFkMetadata(tabDesc, row.CheckInserts, checkHelper, upperOwner)
	if err != nil {
		return nil, err
	}
...
func (ef *execFactory) ConstructUpdate(
	...
	var isAlterHash bool
	// var alterHashIndex int
	for _, index := range tabDesc.Indexes {
		if index.Name == "hashpartitionidx" && tabDesc.IsHashPartition {
			isAlterHash = true
			// alterHashIndex = i
			break
		}
	...
	if tabDesc.IsHashPartition {
		for _, desc := range updateColDescs {
			hashIdx := 0
			if !isAlterHash {
				if desc.Name == tabDesc.PrimaryIndex.ColumnNames[1] {
					for idx, col := range fetchColDescs {
						if col.Name == "hashnum" {
							hashIdx = idx
							break
						}
					}
					updateColDescs = append(updateColDescs, fetchColDescs[hashIdx])
				}
			}
			// else {
			// 	isContainHashIndexColumn := false
			// 	hashIndexColumn := fetchColDescs[tabDesc.Indexes[alterHashIndex].ColumnIDs[0]-1]
			// 	for _, col := range updateColDescs {
			// 		if reflect.DeepEqual(col, hashIndexColumn) {
			// 			isContainHashIndexColumn = true
			// 		}
			// 	}
			// 	if !isContainHashIndexColumn {
			// 		updateColDescs = append(updateColDescs, hashIndexColumn)
			// 	}
			// }
		}
	}
	...
	cols := tree.NameList{}
	for _, d := range updateColDescs {
		cols = append(cols, d.ColName())
	}
	...
	fkTables, err := row.MakeFkMetadata(
		...
		ef.planner.checkPrivilegeAccessToTable,
...
func (ef *execFactory) ConstructUpsert(
	...
	// Determine the foreign key tables involved in the upsert.
	fkTables, err := ef.makeFkMetadata(tabDesc, row.CheckUpdates, checkHelper, upperOwner)
	if err != nil {
		return nil, err
	}
	...
	*ups = upsertNode{
		...
		run: upsertRun{
			...
			tw: &optTableUpserter{
				...
				additionalOrdinal: int(additionalCol),
				fkTables:          fkTables,
...
func (ef *execFactory) ConstructDelete(
	...
	// Determine the foreign key tables involved in the update.
	fkTables, err := ef.makeFkMetadata(tabDesc, row.CheckDeletes, nil, upperOwner)
	if err != nil {
		return nil, err
	}

	fastPathInterleaved := canDeleteFastInterleaved(tabDesc, fkTables)
	if !ef.IsContainTrigger(table) {
...
func (ef *execFactory) ConstructCreateTable(
	...
	nd := &createTableNode{n: ct, dbDesc: schema.(*optSchema).DbDesc()}
	if input != nil {
...
func (ef *execFactory) ConstructCreateView(
	...
	for _, d := range deps {
		...
		if d.SpecificIndex {
			...
			if _, isPartial := idx.Predicate(); isPartial {
	...
	return &createViewNode{
		n:        cv,
		dbDesc:   schema.(*optSchema).desc.(*DatabaseDescriptor),
		columns:  columns,
		planDeps: planDeps,
		hasStar:  hasStar,
	}, nil
...
func constructOpaque(metadata opt.OpaqueMetadata) (planNode, error) {
	...
	if !ok {
		return nil, errors.Errorf("unexpected OpaqueMetadata object type %T", metadata)
	}
	return o.plan, nil
}

func (ef *execFactory) ConstructDataStoreEngineInfo(e memo.RelExpr, object exec.Node) {
	//switch t := e.(type) {
	//case *memo.ScanExpr:
	//	//sc := object.(*scanNode)
	//	object.(*scanNode).StoreEngine = t.StoreEngine
	//case *memo.MergeJoinExpr:
	//	object.(*joinNode).StoreEngine = t.StoreEngine
	//case * memo.IndexJoinExpr:
	//	object.(*indexJoinNode).StoreEngine = t.StoreEngine
	//case *memo.LookupJoinExpr:
	//	object.(*lookupJoinNode).StoreEngine = t.StoreEngine
	//case *memo.ZigzagJoinExpr:
	//	object.(*zigzagJoinNode).StoreEngine = t.StoreEngine
	//case *memo.SortExpr:
	//	object.(*sortNode).StoreEngine = t.StoreEngine
	//case *memo.GroupByExpr, *memo.ScalarGroupByExpr:
	//	object.(*groupNode).StoreEngine = t.GetDataStoreEngine()
	//case *memo.DistinctOnExpr:
	//	object.(*distinctNode).StoreEngine = t.StoreEngine
	//case *memo.SelectExpr:
	//	object.(*filterNode).StoreEngine = t.StoreEngine
	//case *memo.ProjectExpr:
	//	object.(*renderNode).StoreEngine = t.StoreEngine
	//case *memo.LimitExpr, *memo.OffsetExpr:
	//	object.(*limitNode).StoreEngine = t.GetDataStoreEngine()
	//default:
	//}

	//only some nodes inherited from DataStoreEngine, such as scanNode, joinNode and so on.
	_, ok := object.(DataStoreEngine)
	if ok {
		object.(DataStoreEngine).SetDataStoreEngine(e.GetDataStoreEngine())
	}
}
...
func (ef *execFactory) ConstructAlterTableSplit(
	...
	return &splitNode{
		...
		tableDesc: index.Table().(*optTable).desc.TableDesc(),
...
func (rb *renderBuilder) init(n exec.Node, reqOrdering exec.OutputOrdering) {
	...
	rb.r = &renderNode{
		...
		render:     make([]tree.TypedExpr, 0),
		columns:    make([]sqlbase.ResultColumn, 0),
	}
	rb.r.ivarHelper = tree.MakeIndexedVarHelper(rb.r, len(src.info.SourceColumns))

	switch src.plan.(type) {
	case *limitNode:
		switch src.plan.(*limitNode).plan.(type) {
		case *scanBufferNode:
			str := src.plan.(*limitNode).plan.(*scanBufferNode).label
			if str == "buffer 2 (recursive_temp)" {
				//rb.r.ivarHelper = tree.MakeIndexedVarHelper(rb.r, 4)
				rb.r.ivarHelper = tree.MakeIndexedVarHelper(rb.r, len(src.info.SourceColumns))
			}
		}
	}
...
func (ef *execFactory) IsContainTrigger(table cat.Table) bool {
	tabDesc := table.(*optTable).desc
	trigger, err := MakeTriggerDesc(ef.planner.extendedEvalCtx.Ctx(), ef.planner.txn, tabDesc)
	if err != nil {
		return true
	}
