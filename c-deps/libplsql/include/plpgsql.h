...
#include "postgres.h"
// 内存相关接口
#include "mmg/memutils.h"
#include "mmg/memdebug.h"
// 兼容znbase 相关数据结构接口
#include "znbase/expr.h"
#include "znbase/fmgr.h"
#include "znbase/tuple.h"
#include "znbase/znbase.h"
#include "nodes/nodes.h"
#include "cson/cJSON.h"
#include "../utils/bepitime.h"
#ifndef addRoach
extern "C"
...
/*
 * add for cursor(by zh)
 * */
typedef enum Cursor_state {
	NO_CURSOR,                /* 没有游标 */
	CURSOR_DECLARED,          /* 游标已声明 */
	CURSOR_OPENED,            /* 游标已打开 */
	CURSOR_CLOSED             /* 游标已关闭 */
} Cursor_state;
...
typedef struct PLpgSQL_expr
{
	char *query;
	void *plan;
	// planIndex is key of this PLpgSQL_expr's logic plan cache
	BEPIPlanIndex planIndex;
	// 需要引入bitmap set 相关源码 放入plsql/utils 内部
	// include/nodes/bitmapset.h
	// backend/nodes/bitmapset.c
	...
	bool expr_simple_in_use;	  /* true if eval tree is active */
								  // znbase comment
								  // just a uint32 可定义于postgres.h
	LocalTransactionId expr_simple_lxid;

	bool in_using; /* parameter in using, for dynamic sql */
} PLpgSQL_expr;
...
typedef struct PLpgSQL_exception_block
{
	...
	int sqlerrm_varno;
	// znbase comment
	//  List 定义里面的NodeTag 放入postgres.h
...
typedef struct PLpgSQL_execstate
{
	...
	Handler_t handler;		/* UDR Runtim Params */
	...
	Cursor_state cur_state[100]; /* cusor state */
 ...
 */
extern PLpgSQL_function *plpgsql_compile(Handler_t handler, FunctionCallInfo fcinfo,
										 bool forValidator);
...
extern PLpgSQL_type *plpgsql_build_datatype(char *typeName, Oid typeOid, int32 typmod,
											Oid collation);
...
extern Node *resolve_column_ref(ParseState *pstate, PLpgSQL_expr *expr, ColumnRef *cref, bool error_if_no_field);
extern Node *make_datum_param(PLpgSQL_expr *expr, int dno, int location);
/*
 * Functions in pl_handler.c
 */
extern void _PG_init(void);

extern Datum plpgsql_exec_function(Handler_t handler,
...
extern char* ReplaceSubStr(const char* str, const char* srcSubStr, const char* dstSubStr, char* out);
//extern void plpgsql_exec_get_datum_type_info(PLpgSQL_execstate *estate,
//PLpgSQL_datum *datum,
...
extern Datum plpgsql_validator(FunctionCallInfo, Handler_t);
extern Datum plpgsql_call_handler(FunctionCallInfo, Handler_t);
extern void plpgsql_notsupport_action();
extern bool __attribute__((weak)) DeclareDrDBVarible(Handler_t, char *, char*, int, bool, bool, char*, char**);
extern char* __attribute__((weak))AssignTheValue(Handler_t,int target,int value, bool inFetch, bool inForEach, int index);
extern int __attribute__((weak))BepiGetResultLen(Handler_t);
extern void __attribute__((weak))BepiSetResultInt(Handler_t,int);
extern char*__attribute__((weak))LookUpParams(Handler_t);
