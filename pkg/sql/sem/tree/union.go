...
//GetLeftSelectClauseIsJoin get flag IsJion easily
func (u *UnionClause) GetLeftSelectClauseIsJoin() bool {
	leftSel, ok := u.Left.Select.(*SelectClause)
	if ok {
		return leftSel.StartWithIsJoin
	}
	return false
}

// Format implements the NodeFormatter interface.
func (u *UnionClause) Format(ctx *FmtCtx) {
	ctx.FormatNode(u.Left)
	ctx.WriteByte(' ')
	ctx.WriteString(u.Type.String())
	if u.All {
		ctx.WriteString(" ALL")
	}
	ctx.WriteByte(' ')
	ctx.FormatNode(u.Right)
}
