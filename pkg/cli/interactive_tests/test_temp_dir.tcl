...
start_test "Check that temp directory does not get wiped upon subsequent failed znbase start attempt and that a znbase instance can be subsequently started up after a shutdown"
send "$argv start --insecure --store=$storedir --background\r"
eexpect ":/# "
# Try to start up a second znbase instance with the same store path.
send "$argv start --insecure --store=$storedir\r"
...
glob_exists "$storedir/$tempprefix*"
send "pkill -9 znbase\r"
# We should be able to start the znbase instance again.
send "$argv start --insecure --store=$storedir\r"
eexpect "node starting"
interrupt
eexpect "shutdown completed"
