...
func (b *Builder) buildScalar(
	...
	case *tree.CastExpr:
		...
		out = b.factory.ConstructCast(arg, t.Type.(coltypes.T), t.SyntaxMode == tree.CastManual)
	...
	case *tree.CharExpr:
		...
		out = b.factory.ConstructChar(args)
	...
	case *tree.NvlExpr:
		...
		out = b.factory.ConstructNvl(args)
	...
	case *tree.AdditionalExpr:
		out = b.factory.ConstructAdditional(t)

	case *tree.PlpgsqlVar:
...
func (b *Builder) buildFunction(
	...
	def, err := f.Func.Resolve(b.semaCtx.SearchPath, b.CaseSensitive)
	if err != nil {
		panic(builderError{err})
	}
	...
	def, ok := f.Func.FunctionReference.(*tree.FunctionDefinition)
	if !ok {
		panic(pgerror.NewAssertionErrorf("function has not been resolved!"))
	}
...
func (b *Builder) constructBinary(
	...
	case tree.Xor:
		return b.factory.ConstructXor(left, right)
	case tree.Plus:
