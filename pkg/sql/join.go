...
func (p *planner) makeJoinNode(
	left planDataSource, right planDataSource, pred *joinPredicate, forceSide int, ParallelFlag bool,
) *joinNode {
	n := &joinNode{
		left:         left,
		right:        right,
...
func (p *planner) makeJoin(
	...
	n := p.makeJoinNode(left, right, pred, 2, false)
	...
	for i := range usingColumns {
		...
		if n.joinType == sqlbase.InnerJoin || n.joinType == sqlbase.LeftOuterJoin {
			...
			expr, err = c.TypeCheck(&p.semaCtx, types.Any, false)
...
func (n *joinNode) SetDataStoreEngine(ds cat.DataStoreEngine) {
	n.storeEngine = ds
}

func (n *joinNode) GetDataStoreEngine() cat.DataStoreEngine {
