...
import (
	...
	"github.com/znbasedb/znbase/pkg/util/syncutil/singleflight"
...
// tableVersionState holds the state for a table version or function version according LeaseManger's mode
// This includes the lease information for a table version.
...
// TableVersionState is the exported type for tableVersionState
type TableVersionState struct {
	table *tableVersionState
}

// functionVersionGrp is a group of tableVersionState, for tableVersionState contains functionDescriptor
type functionVersionGrp struct {
	funcGroup     []*tableVersionState
...
type LeaseStore struct {
	// LeaseMode stands for mode of leaseMangaer
	LeaseMode LeaseManagerMode
...
func (s LeaseStore) acquire(
	...
	err := s.execCfg.DB.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		var functionDesc *sqlbase.FunctionDescriptor
		var storeLeaseID sqlbase.ID
		...
		// according Lease mode, will return different desc from tableID
		// Function Mode will return function desc, and table id is actual a function id
		if s.LeaseMode == TableLeaseMgr {
			...
			storeLeaseID = tableDesc.ID
			version = tableDesc.Version
			table.ImmutableTableDescriptor = *sqlbase.NewImmutableTableDescriptor(*tableDesc)
			...
			functionDesc, err = sqlbase.GetFunctionDescFromID(ctx, txn, tableID)
			if err != nil {
				return err
			}
			storeLeaseID = functionDesc.ID
			table.ImmutableTableDescriptor.FunctionDesc = functionDesc
			table.ID = sqlbase.InvalidID
			table.Version = functionDesc.Version
			table.TableDescriptor.ModificationTime = table.FunctionDesc.ModificationTime
		}
...
func (s LeaseStore) WaitForOneVersion(
	...
	for r := retry.Start(retryOpts); r.Next(); {
		...
		ts, err := s.execCfg.DB.GetProtoTs(ctx, descKey, desc)
		if err != nil {
			return 0, err
		}
...
// WaitForOneVersionFunction returns once there are no unexpired leases on the
// previous version of the table descriptor. It returns the current version.
// After returning there can only be versions of the descriptor >= to the
// returned version. Lease acquisition (see acquire()) maintains the
// invariant that no new leases for tableDesc.Version-1 will be granted once
// tableDesc.Version exists.
func (s LeaseStore) WaitForOneVersionFunction(
	ctx context.Context, tableID sqlbase.ID, retryOpts retry.Options,
) (sqlbase.DescriptorVersion, error) {
	desc := &sqlbase.Descriptor{}
	descKey := sqlbase.MakeDescMetadataKey(tableID)
	...
	for r := retry.Start(retryOpts); r.Next(); {
		...
		// version of the descriptor.
		now := s.execCfg.Clock.Now()
		tables := []IDVersion{NewFunctionIDVersionPrev(funcDesc)}
		count, err := CountLeases(ctx, s.execCfg.InternalExecutor, tables, now)
		if err != nil {
			return 0, err
		}
		...
		log.Infof(context.TODO(), "publish (%d leases): funcDesc=%v", count, tables)
	}
	return funcDesc.Version, nil
}
...
func maybeIncrementFunctionVersion(
	ctx context.Context, desc *sqlbase.MutableFunctionDescriptor, txn *client.Txn,
) error {
	// Already incremented, no-op.
	if desc.Version == desc.ClusterVersion.Version+1 {
		return nil
	...
	for r := retry.Start(base.DefaultRetryOptions()); r.Next(); {
		...
		err := s.execCfg.DB.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
			...
			for _, id := range tableIDs {
				...
				if err := maybeIncrementVersion(ctx, descsToUpdate[id], txn); err != nil {
					return err
				}
				if err := descsToUpdate[id].ValidateTable(s.execCfg.Settings); err != nil {
					return err
				}
			...
			for tableID, tableDesc := range tableDescs {
				if err := WriteDescToBatch(ctx, false /* kvTrace */, s.execCfg.Settings, b, tableID, tableDesc.TableDesc()); err != nil {
					return err
				}
			}
			if logEvent != nil {
...
func (s LeaseStore) Publish(
	...
	for r := retry.Start(base.DefaultRetryOptions()); r.Next(); {
		...
		// a race now to update to the next version.
		err = s.execCfg.DB.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
			txn.InternalSetPriority(enginepb.MaxTxnPriority)
...
// PublishFunction updates a table descriptor. It also maintains the invariant that
// there are at most two versions of the descriptor out in the wild at any time
// by first waiting for all nodes to be on the current (pre-update) version of
// the table tableDesc.
// The update closure is called after the wait, and it provides the new version
...
// not have side effects.
// Returns the updated version of the descriptor.
func (s LeaseStore) PublishFunction(
	ctx context.Context,
	funcID sqlbase.ID,
	update func(*sqlbase.MutableFunctionDescriptor) error,
	logEvent func(*client.Txn) error,
	...
	for r := retry.Start(base.DefaultRetryOptions()); r.Next(); {
		// Wait until there are no unexpired leases on the previous version
		// of the table.
		expectedVersion, err := s.WaitForOneVersionFunction(ctx, funcID, base.DefaultRetryOptions())
		if err != nil {
			return nil, err
		}

		var funcDesc *sqlbase.MutableFunctionDescriptor
		// There should be only one version of the descriptor, but it's
		// a race now to update to the next version.
		err = s.execCfg.DB.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
			// Re-read the current version of the table descriptor, this time
			// transactionally.
			var err error
			funcDesc, err = sqlbase.GetMutableFunctionDescFromID(ctx, txn, funcID)
			if err != nil {
				return err
			}

			if expectedVersion != funcDesc.Version {
				// The version changed out from under us. Someone else must be
				// performing a schema change operation.
				if log.V(3) {
					log.Infof(ctx, "publish (version changed): %d != %d", expectedVersion, funcDesc.Version)
				}
				return errLeaseVersionChanged
			}

			// Run the update closure.
			version := funcDesc.Version
			if err := update(funcDesc); err != nil {
				return err
			}
			if version != funcDesc.Version {
				return errors.Errorf("updated version to: %d, expected: %d",
					funcDesc.Version, version)
			}

			if err := maybeIncrementFunctionVersion(ctx, funcDesc, txn); err != nil {
				return err
			}
			if err := funcDesc.ValidateFunction(s.execCfg.Settings); err != nil {
				return err
			}
			...
			b := txn.NewBatch()
			b.Put(sqlbase.MakeDescMetadataKey(funcID), sqlbase.WrapDescriptor(funcDesc))
			if logEvent != nil {
				// If an event log is required for this update, ensure that the
				// descriptor change occurs first in the transaction. This is
				// necessary to ensure that the System configuration change is
		...
		switch err {
		case nil, errDidntUpdateDescriptor:
			return &funcDesc.FunctionDescriptor, nil
		case errLeaseVersionChanged:
			// will loop around to retry
		default:
			return nil, err
...
//NewFunctionIDVersionPrev a function
func NewFunctionIDVersionPrev(desc *sqlbase.FunctionDescriptor) IDVersion {
	return IDVersion{name: desc.Name, id: desc.ID, version: desc.Version - 1}
}
...
func acquireFuncNodeLease(
	...
	if tbvs, ok := result.Val.(leaseToken); ok {
		return tbvs, didAcquire, nil
	}
	return nil, didAcquire, nil
}
...
// from the store. The cache maintains the latest version for each table name.
// All methods are thread-safe.
type funcNameCache struct {
	mu        syncutil.Mutex
	functions map[tableNameCacheKey]*functionVersionGrp
}
...
// The table's refcount is incremented before returning, so the caller
// is responsible for releasing it to the leaseManager.
func (c *funcNameCache) get(
	dbID sqlbase.ID, funcGrpName string, timestamp hlc.Timestamp,
) *functionVersionGrp {
	c.mu.Lock()
	function, ok := c.functions[makeTableNameCacheKey(dbID, funcGrpName)]
	c.mu.Unlock()
	if !ok {
		return nil
	}

	for _, f := range function.funcGroup {
		f.mu.Lock()
		if f.mu.lease == nil {
			f.mu.Unlock()
			// This get() raced with a release operation. Remove this cache
	...
	if !nameParentIDMatches(function.funcGroupName, function.ParentID, dbID, funcGrpName) {
		funcIDGrp := make([]sqlbase.ID, len(function.funcGroup))
		for _, f := range function.funcGroup {
			funcIDGrp = append(funcIDGrp, f.ID)
		}
		panic(fmt.Sprintf("Out of sync entry in the function name cache. "+
			"Cache entry: %d.%q -> %v. Lease: %d.%q.",
			dbID, funcGrpName, funcIDGrp, function.ParentID, function.funcGroupName))
	}

	// Expired function. Don't hand it out.
	//for _, f := range function.funcGroup {
	//	if f.hasExpired(timestamp) {
	//		return nil
	//	}
	//	f.incRefcountLocked()
	//}
	for _, tbvs := range function.funcGroup {
		tbvs.incRefcountLocked()
	}
	// function.incRefcountLocked()
	return function
}

func (c *funcNameCache) insert(functionGrp *functionVersionGrp) {
	c.mu.Lock()
	defer c.mu.Unlock()

	key := makeTableNameCacheKey(functionGrp.ParentID, functionGrp.funcGroupName)
	existing, ok := c.functions[key]
	if !ok {
		c.functions[key] = functionGrp
		return
	}
	...
	isUpdate := false
	var findFunc bool
	ready2AddFuncs := make([]*tableVersionState, 0, 4)
	for i, Func := range functionGrp.funcGroup {
		findFunc = false
		for _, existingFunc := range existing.funcGroup {
			if existingFunc.FunctionDesc.FullFuncName == Func.FunctionDesc.FullFuncName {
				findFunc = true
				if Func.FunctionDesc.Version > existingFunc.FunctionDesc.Version ||
					(Func.FunctionDesc.Version == existingFunc.FunctionDesc.Version && Func.hasValidExpiration(existingFunc)) {
					// Overwrite the old table. The new one is better. From now on, we want
					// clients to use the new one.
					updatedExisting.funcGroup[i] = Func
					isUpdate = true
				}
				break
			}
		...
		if !findFunc {
			ready2AddFuncs = append(ready2AddFuncs, Func)
		}
	}
	if len(ready2AddFuncs) != 0 {
		updatedExisting.funcGroup = append(updatedExisting.funcGroup, ready2AddFuncs...)
		isUpdate = true
	}
	if isUpdate {
		c.functions[key] = updatedExisting
	}
}

func (c *funcNameCache) remove(functionGrp *functionVersionGrp) {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	// If this was the lease that the cache had for the function group name, remove it.
	// If the cache had some other function, this remove is a no-op.
	if existing == functionGrp {
		delete(c.functions, key)
	}
}

func (c *funcNameCache) removeFuncGrpByIDAndName(fgParentID sqlbase.ID, fgName string) {
	c.mu.Lock()
	defer c.mu.Unlock()

	key := makeTableNameCacheKey(fgParentID, fgName)
	existing, ok := c.functions[key]
	if !ok {
		// Table for lease not found in table name cache. This can happen if we had
		// a more recent lease on the table in the tableNameCache, then the table
...
// LeaseManagerMode is used for distinct with function mode from table mode.
type LeaseManagerMode int

const (
	// TableLeaseMgr means this LeaseManager will manager the version or desc of table.
	TableLeaseMgr LeaseManagerMode = iota

	// FunctionLeaseMgr means this LeaseManager will manager the version or desc of a function.
	FunctionLeaseMgr
)
...
// Attention: LeaseManager also charge for function Lease manger when leaseManger switch to function mode.
type LeaseManager struct {
	...
	// functionNames is a cache for functionName -> ids mappings. A mapping for the cache
	// should only be used if we currently have an active lease on the respective id;
	// otherwise, the mapping may well be stale.
	// for functionNames, a function name will map a list of function those have same parentID.
	// so we should find a list of functions before insert into this map
	functionNames funcNameCache
}
...
func NewLeaseManager(
	...
	lm := &LeaseManager{
		...
		stopper:    stopper,
		functionNames: funcNameCache{
			functions: make(map[tableNameCacheKey]*functionVersionGrp),
		},
	}
...
func nameParentIDMatches(name string, parentID sqlbase.ID, dbID sqlbase.ID, tableName string) bool {
	return parentID == dbID && name == tableName
}
...
func (m *LeaseManager) AcquireByName(
	...
	var err error
	tableIDs, err := m.resolveName(ctx, timestamp, dbID, tableName)
	if err != nil {
		return nil, hlc.Timestamp{}, err
	}
	if len(tableIDs) != 1 {
		return nil, hlc.Timestamp{}, pgerror.NewAssertionErrorf("Can not find an valid tableIDs or id exceed one!")
	}
	tableID := tableIDs[0]
...
// AcquireByFunctionName will return a group of function desc for function Name.
func (m *LeaseManager) AcquireByFunctionName(
	ctx context.Context, timestamp hlc.Timestamp, parentID sqlbase.ID, functionName string,
) (*sqlbase.ImmutableFunctionDescriptor, hlc.Timestamp, error) {
	immutableFuncDesc := &sqlbase.ImmutableFunctionDescriptor{
		FuncGroup:         make([]sqlbase.FuncDesc, 0),
		FunctionParentID:  parentID,
		FunctionGroupName: functionName,
	}

	// TODO add function name cache, so that will not resolve function name to get function Group id
	// read from cache, from LeaseManager.functionNames

	// Check if we have cached an ID for this name.
	funcVersionGrp := m.functionNames.get(parentID, functionName, timestamp)
	if funcVersionGrp != nil {
		for _, function := range funcVersionGrp.funcGroup {
			if !timestamp.Less(function.ModificationTime) {
				// If this lease is nearly expired, ensure a renewal is queued.
				durationUntilExpiry := time.Duration(function.expiration.WallTime - timestamp.WallTime)
				if durationUntilExpiry < m.LeaseStore.leaseRenewalTimeout {
					if t := m.findTableState(function.ID, false /* create */); t != nil {
						if err := t.maybeQueueLeaseRenewal(
							ctx, m, function.ID, functionName); err != nil {
							return nil, hlc.Timestamp{}, err
						}
					}
				}
				tempFunction := sqlbase.FuncDesc{
					Desc:       function.FunctionDesc,
					Expiration: function.expiration,
					TableDesc:  &function.ImmutableTableDescriptor,
				}
				immutableFuncDesc.FuncGroup = append(immutableFuncDesc.FuncGroup, tempFunction)
				continue
				// return &function.ImmutableTableDescriptor, function.expiration, nil
			}
			if err := m.Release(&function.ImmutableTableDescriptor); err != nil {
				return nil, hlc.Timestamp{}, err
			}
			// Return a valid table descriptor for the timestamp.
			acquireFunc, _, expiration, err := m.AcquireFuncs(ctx, timestamp, function.FunctionDesc.ID)
			if err != nil {
				return nil, hlc.Timestamp{}, err
			}
			tempFunction := sqlbase.FuncDesc{
				Desc:       function.FunctionDesc,
				Expiration: expiration,
				TableDesc:  acquireFunc,
			}
			immutableFuncDesc.FuncGroup = append(immutableFuncDesc.FuncGroup, tempFunction)
			// return table, expiration, nil
		}
		// hlc.MinTimestamp may have problem
		return immutableFuncDesc, hlc.MinTimestamp, nil
	}

	// read from store, txn.scan through parentID and function group name
	var err error
	functionIDs, err := m.resolveName(ctx, timestamp, parentID, functionName)
	if err != nil {
		return nil, hlc.Timestamp{}, err
	}
	tbVersionStates := make([]*tableVersionState, 0)

	for _, functionID := range functionIDs {
		funcDesc, tbvs, expiration, err := m.AcquireFuncs(ctx, timestamp, functionID)
		if err != nil {
			return nil, hlc.Timestamp{}, err
		}
		function := sqlbase.FuncDesc{
			Desc:       funcDesc.FunctionDesc,
			Expiration: expiration,
			TableDesc:  funcDesc,
		}
		immutableFuncDesc.FuncGroup = append(immutableFuncDesc.FuncGroup, function)
		tbVersionStates = append(tbVersionStates, tbvs.table)
	}
	m.functionNames.insert(&functionVersionGrp{
		funcGroup:     tbVersionStates,
		funcGroupName: functionName,
		ParentID:      parentID,
	})
	return immutableFuncDesc, hlc.MinTimestamp, nil
}
...
func (m *LeaseManager) resolveName(
	...
	var tableMode bool
	var key roachpb.Key
	if m.LeaseMode == TableLeaseMgr {
		nameKey := tableKey{parentID: dbID, name: tableName}
		key = nameKey.Key()
		tableMode = true
	} else {
		nameKey := functionKey{parentID: dbID, name: "", funcName: tableName}
		key = nameKey.Key()
		tableMode = false
	}
	ids := []sqlbase.ID{}
	...
	if len(ids) == 0 {
		return ids, sqlbase.ErrDescriptorNotFound
	}
	for _, id := range ids {
...
// AcquireFuncs acquires a read lease for the specified function ID valid for
// the timestamp. This function is similar with (m *LeaseManager) Acquire,
// the difference is that this function is for user define function, but above if for table
func (m *LeaseManager) AcquireFuncs(
	ctx context.Context, timestamp hlc.Timestamp, funcID sqlbase.ID,
) (*sqlbase.ImmutableTableDescriptor, *TableVersionState, hlc.Timestamp, error) {
	for {
		t := m.findTableState(funcID, true /*create*/)
		table, latest, err := t.findForTimestamp(ctx, timestamp)
		if err == nil {
			// If the latest lease is nearly expired, ensure a renewal is queued.
			if latest {
				durationUntilExpiry := time.Duration(table.expiration.WallTime - timestamp.WallTime)
				if durationUntilExpiry < m.LeaseStore.leaseRenewalTimeout {
					if err := t.maybeQueueLeaseRenewal(ctx, m, funcID, table.Name); err != nil {
						return nil, nil, hlc.Timestamp{}, err
					}
				}
			}
			tempTab := &TableVersionState{table}
			return &table.ImmutableTableDescriptor, tempTab, table.expiration, nil
		}
		switch err {
		case errRenewLease:
			// Renew lease and retry. This will block until the lease is acquired.
			if _, _, errLease := acquireFuncNodeLease(ctx, m, funcID); errLease != nil {
				return nil, nil, hlc.Timestamp{}, errLease
			}
			if m.testingKnobs.LeaseStoreTestingKnobs.LeaseAcquireResultBlockEvent != nil {
				m.testingKnobs.LeaseStoreTestingKnobs.LeaseAcquireResultBlockEvent(LeaseAcquireBlock)
			}
		...
		case errReadOlderTableVersion:
			// Read old table versions from the store. This can block while reading
			// old table versions from the store.
			versions, errRead := m.readOlderVersionForTimestamp(ctx, funcID, timestamp)
			if errRead != nil {
				return nil, nil, hlc.Timestamp{}, errRead
			}
			m.insertTableVersions(funcID, versions)
			...
			case <-gossipUpdateC:
				...
				cfgFilter.ForModified(cfg, func(kv roachpb.KeyValue) {
					...
					case *sqlbase.Descriptor_Function:
						...
						// need to remove current function in leaseMgr
						// first to find out parasitic table desc of this function
						if err := m.SwicthLeaseManager(ctx, TableLeaseMgr, FunctionLeaseMgr); err != nil {
							log.Warningf(ctx, "error purging leases for function %d(%s):%s",
								function.ID, function.Name, err,
							)
							return
						}
						defer func() {
							if err := m.SwicthLeaseManager(ctx, FunctionLeaseMgr, TableLeaseMgr); err != nil {
								log.Warningf(ctx, "error purging leases for function %v(%s):%s",
									function.ID, function.Name, err,
								)
								return
							}
						...
						if err := purgeOldVersions(
							ctx, db, function.ID, function.Dropped(), function.Version, m); err != nil {
							log.Warningf(ctx, "error purging leases for table %v(%s): %s",
								function.ID, function.Name, err)
						}
...
func (m *LeaseManager) refreshSomeLeases(ctx context.Context) {
	...
	sem := quotapool.NewIntPool("Lease Manager", 5)
	m.stopper.AddCloser(sem.Closer("stopper"))
...
// SwicthLeaseManager will change current's lease manger's state to goal model
func (m *LeaseManager) SwicthLeaseManager(ctx context.Context, from, to LeaseManagerMode) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	LeaseModeDesc := []string{"Table mode.", "Function mode"}
	if from == to {
		// no possiable to here
		return nil
	} else if m.LeaseMode != from {
		return pgerror.NewAssertionErrorf("Current's state is not %s", LeaseModeDesc[from])
	} else {
		m.LeaseMode = to
		log.Warningf(ctx, "now lease manger switch to %s mode", LeaseModeDesc[to])
	}
	return nil
}

// ClientAcquire is the function to be using in client package.
func ClientAcquire(
	ctx context.Context, m interface{}, timestamp hlc.Timestamp, tableID uint32,
) (hlc.Timestamp, error) {
	var ok bool
	var lmr *LeaseManager
	if lmr, ok = m.(*LeaseManager); !ok {
		return hlc.Timestamp{}, errors.Errorf("Wrong type: Type should be *LeaseManager")
	}

	_, expiration, err := lmr.Acquire(ctx, timestamp, sqlbase.ID(tableID))
	if err != nil {
...
func init() {
	client.LeaseMgrAcquire = ClientAcquire
}
