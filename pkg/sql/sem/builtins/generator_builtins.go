...
var generators = map[string]builtinDefinition{
	"generate_series": makeBuiltin(genProps(seriesValueGeneratorLabels),
		...
		makeGeneratorOverload(
			tree.ArgTypes{{Name: "start", Typ: types.Int}, {Name: "end", Typ: types.Int}},
			seriesValueGeneratorType,
			makeSeriesGenerator,
			"Produces a virtual table containing the integer values from `start` to `end`, inclusive.",
		),
		makeGeneratorOverload(
			tree.ArgTypes{{Name: "start", Typ: types.Int}, {Name: "end", Typ: types.Int}, {Name: "step", Typ: types.Int}},
			seriesValueGeneratorType,
			makeSeriesGenerator,
			"Produces a virtual table containing the integer values from `start` to `end`, inclusive, by increment of `step`.",
		),
		makeGeneratorOverload(
			tree.ArgTypes{{Name: "start", Typ: types.Timestamp}, {Name: "end", Typ: types.Timestamp}, {Name: "step", Typ: types.Interval}},
		...
		makeGeneratorOverload(
			...
			seriesTZSValueGeneratorType,
			makeTZSSeriesGenerator,
	...
	"unnest": makeBuiltin(genProps(arrayValueGeneratorLabels),
		...
		makeGeneratorOverloadWithReturnType(
			tree.ArgTypes{{Name: "input", Typ: types.AnyArray}},
	...
	"zbdb_internal.unary_table": makeBuiltin(genProps(nil /* no labels */),
		makeGeneratorOverload(
			tree.ArgTypes{},
			unaryValueGeneratorType,
			makeUnaryGenerator,
			"Produces a virtual table containing a single row with no values.\n\n"+
				"This function is used only by ZNBaseDB's developers for testing purposes.",
		),
	),
	...
	"generate_subscripts": makeBuiltin(genProps(subscriptsValueGeneratorLabels),
		...
		makeGeneratorOverload(
			tree.ArgTypes{{Name: "array", Typ: types.AnyArray}},
			subscriptsValueGeneratorType,
			makeGenerateSubscriptsGenerator,
			"Returns a series comprising the given array's subscripts.",
		),
		makeGeneratorOverload(
			tree.ArgTypes{{Name: "array", Typ: types.AnyArray}, {Name: "dim", Typ: types.Int}},
			subscriptsValueGeneratorType,
			makeGenerateSubscriptsGenerator,
			"Returns a series comprising the given array's subscripts.",
		),
		makeGeneratorOverload(
			tree.ArgTypes{{Name: "array", Typ: types.AnyArray}, {Name: "dim", Typ: types.Int}, {Name: "reverse", Typ: types.Bool}},
			subscriptsValueGeneratorType,
			makeGenerateSubscriptsGenerator,
			"Returns a series comprising the given array's subscripts.\n\n"+
	...
	"zbdb_internal.check_consistency": makeBuiltin(
		...
		makeGeneratorOverload(
			...
			"Runs a consistency check on ranges touching the specified key range. "+
				...
				"and verbose detail.\n\n"+
				"Example usage:\n"+
				"SELECT * FROM zbdb_internal.check_consistency(true, '\\x02', '\\x04')",
		),
	),
}
...
var seriesTZSValueGeneratorType = types.TimestampTZ

var errStepCannotBeZero = pgerror.NewError(pgcode.InvalidParameterValue, "step cannot be 0")
...
func seriesTZSNext(s *seriesValueGenerator) (bool, error) {
	step := s.step.(duration.Duration)
	start := s.start.(time.Time)
	stop := s.stop.(time.Time)
...
func seriesGenTZSValue(s *seriesValueGenerator) tree.Datums {
	return tree.Datums{tree.MakeDTimestampTZ(s.value.(time.Time), time.Microsecond)}
}
...
func makeTZSSeriesGenerator(ctx *tree.EvalContext, args tree.Datums) (tree.ValueGenerator, error) {
	...
	return &seriesValueGenerator{
		...
		stop:      stop,
		step:      step,
		genType:   seriesTZSValueGeneratorType,
		genValue:  seriesGenTZSValue,
		next:      seriesTZSNext,
	}, nil
}
...
// unaryValueGenerator supports the execution of zbdb_internal.unary_table().
...
var (
	errJSONObjectKeysOnArray         = pgerror.NewError(pgcode.InvalidParameterValue, "cannot call json_object_keys on an array")
	errJSONObjectKeysOnScalar        = pgerror.NewErrorf(pgcode.InvalidParameterValue, "cannot call json_object_keys on a scalar")
	errJSONDeconstructArrayAsObject  = pgerror.NewError(pgcode.InvalidParameterValue, "cannot deconstruct an array as an object")
	errJSONDeconstructScalarAsObject = pgerror.NewErrorf(pgcode.InvalidParameterValue, "cannot deconstruct a scalar")
)

var jsonArrayElementsImpl = makeGeneratorOverload(
	tree.ArgTypes{{Name: "input", Typ: types.JSON}},
...
var errJSONCallOnNonArray = pgerror.NewError(pgcode.InvalidParameterValue,
