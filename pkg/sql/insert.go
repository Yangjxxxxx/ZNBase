...
type insertNode struct {
	...
	isKv   bool
	kvNums int
	//kvRedundancy chan sqlbase.KvRedundancy
	kvs []sqlbase.KvRedundancy
}
...
func (p *planner) Insert(
	...
	for i, col := range insertCols {
		...
		if requiredTypesFromSelect[i] == types.Float && col.Type.Precision == 16 {
			requiredTypesFromSelect[i] = types.Float4
		}
		if requiredTypesFromSelect[i] == types.Int && col.Type.Width == 16 {
			requiredTypesFromSelect[i] = types.Int2
		}
		if requiredTypesFromSelect[i] == types.Int && col.Type.Width == 32 {
			requiredTypesFromSelect[i] = types.Int4
		}
	}
	...
	ri, err := row.MakeInserter(nil, p.txn, nil, desc, fkTables, insertCols,
	...
	var canaryColNum, addtionalColNum int
	var onlyMatch bool

	if n.OnConflict != nil {
		if n.OnConflict.IsMergeStmt() {
			var errForMerge error
			rows, canaryColNum, addtionalColNum, onlyMatch, errForMerge = p.constructJoinNode(ctx, n)
			if errForMerge != nil {
				return nil, errForMerge
			}
		}
		/* when merge into stmt it will construct a joinNode here to replace simple rendernode */
		...
		node, err = p.newUpsertNode(
			...
			defaultExprs, computeExprs, computedCols, fkTables, desiredTypes, canaryColNum, addtionalColNum, onlyMatch)
		if err != nil {
			return nil, err
		}
	} else {
...
func (n *insertNode) startExec(params runParams) error {
	...
	// trigger constructor
	tgDesc, err := MakeTriggerDesc(params.ctx, params.p.txn, n.run.ti.tableDesc())
	if err != nil {
		return err
	}
	n.run.Trigger = tgDesc

	err = ExecuteTriggers(params.ctx, params.p.txn, params, n.run.Trigger, nil, nil, nil,
		TriggerTypeInsert, TriggerTypeBefore, TriggerTypeStatement)
	if err != nil {
		return err
	}

	// relationKey := tableKey{n.run.ti.tableDesc().ParentID, n.run.ti.tableDesc().Name, ""}
	// id, err := getDescriptorID(params.ctx, params.p.txn, relationKey)
	// if err != nil {
	// 	return err
	// }
	// if id == sqlbase.InvalidID {
	// 	return errors.Errorf("table does not exist")
	// }
	// if id != n.run.ti.tableDesc().ID {
	// 	n.run.ti.tableDesc().ID = id
	// }
...
func (n *insertNode) BatchedNext(params runParams) (bool, error) {
	...
	// for _, tg := range n.run.Trigger.Triggers {
	// 	if tg.IsAfterStmt(TriggerTypeAfter, TriggerTypeStatement) {
	// 		n.run.ti.isAfter = true
	// 		break
	// 	}
	// }
	...
	for {
		...
		if n.isKv {
			...
			n.run.ti.batchSize++
			err := n.run.ti.ri.InsertKv(params.ctx, n.run.ti.b, kvRed)
			if err != nil {
				return false, nil
			}
		} else {
			...
			err := ExecuteTriggers(params.ctx, params.p.txn, params, n.run.Trigger, n.run.ti.tableDesc(), n.source.Values(),
				nil, TriggerTypeInsert, TriggerTypeBefore, TriggerTypeRow)
			if err != nil {
				return false, err
			}
			...
			err = ExecuteTriggers(params.ctx, params.p.txn, params, n.run.Trigger, n.run.ti.tableDesc(), n.source.Values(),
				n.run.ti.tableWriterBase.b, TriggerTypeInsert, TriggerTypeAfter, TriggerTypeRow)
			if err != nil {
				return false, err
			}
	...
	if lastBatch {
		if _, err := n.run.ti.finalize(params.ctx, params, n.run.traceKV, n.run.Trigger); err != nil {
			return false, err
		}
...
func (n *insertNode) processSourceRow(params runParams, sourceVals tree.Datums) error {
	...
	partialIndexOrds := n.run.ti.tableDesc().PartialIndexOrds()
	if !partialIndexOrds.Empty() {
		partialIndexPutVals := rowVals[len(n.run.insertCols)+n.run.checkOrds.Len():]

		err := pm.Init(partialIndexPutVals, tree.Datums{}, n.run.ti.tableDesc())
		if err != nil {
			return err
		}
		...
		// values.
		rowVals = rowVals[:len(n.run.insertCols)+n.run.checkOrds.Len()]
	}
	...
	if n.run.ti.ri.Helper.Indexes != nil {
		for _, index := range n.run.ti.ri.Helper.TableDesc.Indexes {
			if index.IsFunc != nil {
	...
	if exprstrings != nil {
		...
		typeexprs, err := sqlbase.MakeFuncExprs(exprstrings, n.run.ti.ri.Helper.TableDesc, tree.NewUnqualifiedTableName(tree.Name(n.run.ti.ri.Helper.TableDesc.Name)), &txCtx, params.EvalContext(), true /* addingCols */)
		if err != nil {
			return err
		}
		for _, typeexpr := range typeexprs {
			for i := 0; i < len(typeexpr.(*tree.FuncExpr).Exprs); i++ {
				...
				typeexpr.(*tree.FuncExpr).Exprs[i] = rowVals[typeexpr.(*tree.FuncExpr).Exprs[i].(*tree.IndexedVar).Idx]
			}
			newrow, err := typeexpr.Eval(params.EvalContext())
			if err != nil {
				return err
			}
			rowVals = append(rowVals, newrow)
		}
	}

	// Queue the insert in the KV batch.
	if err = n.run.ti.row(params.ctx, rowVals, n.run.traceKV, pm); err != nil {
		return err
	}
...
func GenerateInsertRow(
	...
	if len(rowVals) < len(insertCols) || len(computeExprs) > 0 {
	...
	// check this table is whether alterHashPartition table or CreateHashPartition table.
	var isAlterHash bool
	for _, index := range tableDesc.Indexes {
		if index.Name == "hashpartitionidx" && tableDesc.IsHashPartition {
			isAlterHash = true
			break
		}
	}
	...
	// get original primary key's index
	var hashkey int
	if tableDesc.IsHashPartition && !isAlterHash {
		primaryName := ""
		if len(tableDesc.PrimaryIndex.ColumnNames) > 1 {
			primaryName = tableDesc.HashField
		} else {
			primaryName = tableDesc.PrimaryIndex.ColumnNames[0]
	...
	for i := 0; i < len(insertCols); i++ {
		if tableDesc.IsHashPartition && insertCols[i].Name == "hashnum" {
			val := util.CRC32Origin([]byte(rowVals[hashkey].String()))
			mod := uint32(tableDesc.HashParts)
			if mod == 0 {
				return nil, errors.New("zero mod")
			}
			val = val % mod
			outVal, err := sqlbase.LimitValueWidth(insertCols[i].Type, tree.Datum(tree.NewDInt(tree.DInt(val))), &insertCols[i].Name, false)
			if err != nil {
				return nil, err
			}
			rowVals[i] = outVal
...
func (n *insertNode) BuildKv1(
	i int, wg *sync.WaitGroup, rows [][]tree.Datum, kvs []sqlbase.KvRedundancy, params runParams,
) error {
	for j, sourceVals := range rows {
		rowVals, err := GenerateInsertRow(
			n.run.defaultExprs,
			n.run.computeExprs,
			n.run.insertCols,
		...
		if n.run.checkHelper != nil {
			if n.run.checkHelper.NeedsEval() {
				...
				if err := n.run.checkHelper.CheckEval(params.EvalContext().Copy()); err != nil {
					wg.Done()
					return err
				}
			} else {
		...
		n.run.ti.ri.BuildKv1(params.ctx, i, j, rowVals, kvs)
...
func (n *insertNode) GetKvs(i int) sqlbase.KvRedundancy {
	if i == len(n.kvs) {
		return sqlbase.KvRedundancy{}
	}
	return n.kvs[i]
}
