...
func ResultColumnsFromColDescs(colDescs []ColumnDescriptor) ResultColumns {
	...
	for _, colDesc := range colDescs {
		...
		cols = append(cols, ResultColumn{Name: colDesc.Name, Typ: typ, Hidden: hidden, VisibleTypeName: colDesc.Type.VisibleTypeName})
	}
	return cols
}

// ResultColumnsFromFuncInoutArgDescs converts FuncArgDescriptors to ResultColumns.
func ResultColumnsFromFuncInoutArgDescs(funcArgDescs []FuncArgDescriptor) ResultColumns {
	cols := make(ResultColumns, 0, len(funcArgDescs))
	for _, funcArgDesc := range funcArgDescs {
		if funcArgDesc.InOutMode != "inout" {
			continue
		}
		// Convert the FuncArgDescriptor to ResultColumn.
		typ := funcArgDesc.Type.ToDatumType()
		if typ == nil {
			panic(fmt.Sprintf("unsupported column type: %s", funcArgDesc.Type.SemanticType))
		}
		if funcArgDesc.Name == "" {
			funcArgDesc.Name = fmt.Sprintf("column%d", len(cols)+1)
		}
		cols = append(cols, ResultColumn{Name: funcArgDesc.Name, Typ: typ, Hidden: false, VisibleTypeName: funcArgDesc.Type.VisibleTypeName})
	}
	return cols
}
