...
export function PrintReplicaID(
  rangeID: Long,
  rep: protos.znbase.roachpb.IReplicaDescriptor,
  nodeID?: number,
  storeID?: number,
  replicaID?: Long,
) {
...
export function PrintTimestamp(
  timestamp: protos.znbase.util.hlc.ITimestamp |
    protos.google.protobuf.ITimestamp,
) {
  let time: moment.Moment = null;
  if (_.has(timestamp, "wall_time")) {
    time = LongToMoment((timestamp as protos.znbase.util.hlc.ITimestamp).wall_time);
...
export function PrintTimestampDelta(
  newTimestamp: protos.znbase.util.hlc.ITimestamp,
  oldTimestamp: protos.znbase.util.hlc.ITimestamp,
) {
