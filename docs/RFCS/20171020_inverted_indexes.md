...
Add infrastructure for inverted indexes in ZNBaseDB, which will permit
...
We’ve committed to supporting JSON column types to make ZNBaseDB useful for
...
in ZNBaseDB they will be referred to as either `JSON` or `JSONB` depending
...
way to index them, and ZNBaseDB currently only provides indexes that permit
...
questions that are not possible to efficiently answer with ZNBaseDB’s
...
- Filtering text columns based on what words they contain. For example, “give
  me all rows whose text column contain the words “ZNBaseDB rocks”.

None of the questions posed by these use cases are efficiently answerable by
ZNBaseDB’s conventional indexes without full table scans, since they are not
...
----------
## Review: Ordinary Indexes in ZNBaseDB

An ordinary primary or secondary index in ZNBaseDB maps one or several
...
ZNBaseDB's inverted index for JSON will encode the full JSON path for every
...
using their internal row id representations in the list. Because ZNBaseDB
...
Fortunately, ZNBaseDB has already solved this problem for non-unique
...
tag them with types. ZNBaseDB already uses type tags for its value encoding,
