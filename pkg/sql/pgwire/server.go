...
var baseSQLMemoryBudget = envutil.EnvOrDefaultInt64("ZNBASE_BASE_SQL_MEMORY_BUDGET",
...
var noteworthySQLMemoryUsageBytes = envutil.EnvOrDefaultInt64("ZNBASE_NOTEWORTHY_SQL_MEMORY_USAGE", 100*1024*1024)
...
// usage growth in the log.
var noteworthyConnMemoryUsageBytes = envutil.EnvOrDefaultInt64("ZNBASE_NOTEWORTHY_CONN_MEMORY_USAGE", 2*1024*1024)
...
func MakeServer(
	...
	connAuthConf.SetOnChange(&st.SV, func() {
		loadLocalAuthConfigUponRemoteSettingChange(ambientCtx.AnnotateCtx(context.Background()), server, st)
	})
...
// https://github.com/znbasedb/znbase/blob/master/docs/RFCS/20160425_drain_modes.md
...
func (s *Server) ServeConn(ctx context.Context, conn net.Conn) error {
	...
	if errSSLRequired {
		return sendErr(pgerror.NewError(pgcode.ProtocolViolation, ErrSSLRequired))
	}
	if draining {
...
func parseOptions(ctx context.Context, data []byte) (*sql.SessionArgs, error) {
	args := sql.SessionArgs{
		SessionDefaults:       sql.SessionDefaults{SessionDefaultsMp: make(map[string]string)},
	...
	for {
		...
		if err != nil {
			return &sql.SessionArgs{}, pgerror.NewErrorf(pgcode.ProtocolViolation,
		...
		default:
			...
			if exists && configurable {
				args.SessionDefaults.Set(key, value)
			} else {
				if !exists {
					...
					return &sql.SessionArgs{}, pgerror.NewErrorf(pgcode.CantChangeRuntimeParam,
	...
	if dbName := args.SessionDefaults.Get("database"); dbName == "" {
		// ZNBaseDB-specific behavior: if no database is specified,
...
func newAdminShutdownErr(err error) error {
	return pgerror.NewErrorf(pgcode.AdminShutdown, err.Error())
}
