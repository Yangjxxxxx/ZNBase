...
func (s *scanner) scanIdent(lval *sqlSymType) {
	...
	if isLower {
		...
		if lex.GetResKeywordID(lval.str) == lex.SELECT {
			s.isToChange = true
		} else if lex.GetResKeywordID(lval.str) == lex.FROM {
			s.isToChange = false
		}
	} else if isASCII {
		...
		lower := *(*string)(unsafe.Pointer(&b))

		if lex.GetResKeywordID(lower) == lex.FROM {
			s.isToChange = false
		}
		if s.isFirstSelect && s.isToChange && s.isClientQuery {
			lval.str = s.in[start:s.pos]
		} else if (s.CaseSensitive || tree.CaseSensitiveG) && lex.GetResKeywordID(lower) == lex.IDENT {
			lval.str = s.in[start:s.pos]
		} else {
			lval.str = lower
		}
		if lex.GetResKeywordID(lower) == lex.SELECT {
			s.isToChange = true
		}

		lval.id = lex.GetKeywordID(lower)
		//lval.str = *(*string)(unsafe.Pointer(&b))
	} else {
		if s.isFirstSelect && s.isToChange && s.isClientQuery && !s.CaseSensitive && !tree.CaseSensitiveG {
			lval.str = s.in[start:s.pos]
			lval.id = lex.GetKeywordID(lval.str)
		} else {
