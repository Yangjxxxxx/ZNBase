...
func (a *aggregateInfo) TypeCheck(
	...
	if _, err := a.FuncExpr.TypeCheck(ctx, desired, useOrigin); err != nil {
		return nil, err
	}
	return a, nil
}
...
func (b *Builder) constructAggregate(name string, args []opt.ScalarExpr) opt.ScalarExpr {
	...
	case "group_concat":
		switch len(args) {
		case 1:
			return b.factory.ConstructGroupConcat(args[0])
		case 2:
	...
	case "stddev_samp":
		return b.factory.ConstructStddevSamp(args[0])
	case "max":
		return b.factory.ConstructMax(args[0])
	case "min":
		return b.factory.ConstructMin(args[0])
	case "median":
		return b.factory.ConstructMedian(args[0])
	case "sum_int":
	...
	case "listagg":
		...
		return b.factory.ConstructListAgg(args[0], args[1])
	case "stats_mode":
		return b.factory.ConstructStatsMode(args[0])
	case "any_value":
		return b.factory.ConstructAnyValue(args[0])
	}
