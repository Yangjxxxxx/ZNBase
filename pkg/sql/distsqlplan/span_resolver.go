...
type spanResolver struct {
	...
	oracleFactory replicastrategy.StrategyFactory
}
...
func NewSpanResolver(
	...
	nodeDesc roachpb.NodeDescriptor,
	rpcCtx *rpc.Context,
	policy replicastrategy.Policy,
) SpanResolver {
	return &spanResolver{
		st:       st,
		nodeDesc: nodeDesc,
		oracleFactory: replicastrategy.NewStrategyFactory(policy, replicastrategy.Config{
			Settings:         st,
			Gossip:           gossip,
			NodeDesc:         nodeDesc,
			RPCContext:       rpcCtx,
...
func (sr *spanResolver) NewSpanResolverIterator(txn *client.Txn) SpanResolverIterator {
	return &spanResolverIterator{
		...
		queryState: replicastrategy.MakeQueryState(),
	}
}
