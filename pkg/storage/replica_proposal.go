...
type ProposalData struct {
	...
	// ec.finish is called after command execution to update the
...
// proposed.
//
// It first invokes the ec function and then sends the specified
// proposalResult on the proposal's done channel. ec is invoked here in
...
// counted on to invoke ec itself.)
...
func addSSTablePreApply(
	...
	checksum := util.CRC32Origin(sst.Data)
	...
	triggerFlag := eng.PreIngestDelay(ctx)
...
func (r *Replica) handleReadWriteLocalEvalResult(ctx context.Context, lResult result.LocalResult) {
	...
	if lResult.EncounteredIntents != nil {
		log.Fatalf(ctx, "LocalEvalResult.Intents should be nil: %+v", lResult.EncounteredIntents)
	}
