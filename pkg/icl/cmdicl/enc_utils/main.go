...
const fileRegistryPath = "INSPURCLOUDDB_REGISTRY"
const keyRegistryPath = "INSPURCLOUDDB_DATA_KEYS"
...
type fileEntry struct {
	...
	settings enginepbicl.EncryptionSettings
}

func (f fileEntry) String() string {
	ret := fmt.Sprintf(" env type: %d, %s\n",
		f.envType, f.settings.EncryptionType)
	if f.settings.EncryptionType != enginepbicl.EncryptionType_Plaintext {
...
type keyEntry struct {
	encryptionType enginepbicl.EncryptionType
	rawKey         []byte
}
...
func loadFileRegistry() {
	...
	for name, entry := range reg.Files {
		var encSettings enginepbicl.EncryptionSettings
...
func loadStoreKey() {
	...
	switch len(data) {
	case 48:
		k.encryptionType = enginepbicl.EncryptionType_AES128_CTR
	case 56:
		k.encryptionType = enginepbicl.EncryptionType_AES192_CTR
	case 64:
		k.encryptionType = enginepbicl.EncryptionType_AES256_CTR
	default:
...
func loadKeyRegistry() {
	...
	var reg enginepbicl.DataKeysRegistry
	if err := protoutil.Unmarshal(data, &reg); err != nil {
...
func readFile(filename string) ([]byte, error) {
	...
	if !ok || reg.settings.EncryptionType == enginepbicl.EncryptionType_Plaintext {
