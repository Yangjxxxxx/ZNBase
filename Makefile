...
## Space delimited list of logic test files to run, for make testlogic/testicllogic/testoptlogic/testplannerlogic.
FILES :=

## Name of a logic test configuration to run, for make testlogic/testicllogic/testoptlogic/testplannerlogic.
...
DUPLFLAGS    := -t 100
GOFLAGS      :=
TAGS         := -Qunused-arguments -Wunused-command-line-argument
ARCHIVE      := znbase.src.tgz
STARTFLAGS   := -s type=mem,size=1GiB --logtostderr
BUILDTARGET  := ./pkg/cmd/znbase
SUFFIX       := $(GOEXE)
INSTALL      := install
prefix       := /usr/local
bindir       := $(prefix)/bin

RAFTPKG := ./vendor/go.etcd.io/etcd/...
...
help: ## Print help for targets with comments.
	...
	@printf "  $(cyan)%s$(term-reset)\n    %s\n\n" \
		...
		"make testicllogic" "Run all ICL SQL logic tests." \
		"make testoptlogic" "Run all opt builder SQL logic tests." \
		...
		"make check-libroach TESTS=icl" "Run the libroach tests matching .*icl.*"
...
CXXFLAGS += -g1
CXXFLAGS += -fPIC -Wno-unused-parameter
LDFLAGS ?=
...
# toolchain.
override LINKFLAGS = -X github.com/znbasedb/znbase/pkg/build.typ=$(BUILDTYPE) -extldflags "$(LDFLAGS)"
...
bin/.bootstrap: $(GITHOOKS) Gopkg.lock | bin/.submodules-initialized
	@$(GO_INSTALL) -v \
		...
		./vendor/github.com/znbasedb/crlfmt \
		./vendor/github.com/znbasedb/gostdlib/cmd/gofmt \
		./vendor/github.com/znbasedb/gostdlib/x/tools/cmd/goimports \
		./vendor/github.com/znbasedb/stress \
...
LIBINJECTION_SRC_DIR    := $(C_DEPS_DIR)/libinjection
LIBPLSQL_SRC_DIR := $(C_DEPS_DIR)/libplsql
...
LIBINJECTION_DIR    := $(BUILD_DIR)/libinjection$(if $(use-msan),_msan)
LIBPLSQL_DIR    := $(BUILD_DIR)/libplsql$(if $(use-msan),_msan)
#LIBPYTHON2_DIR := /usr/lib/python2.7/config-x86_64-linux-gnu
LIBPYTHON3_DIR := /usr/lib/python3.8/config-3.8-x86_64-linux-gnu
ifdef NOPLPYTHON
LIBPYTHON3_DIR = 
endif
...
LIBROACHICL := $(LIBROACH_DIR)/libroachicl.a
LIBINJECTION := $(LIBINJECTION_DIR)/libinjection.a
LIBPLSQL := $(LIBPLSQL_DIR)/libplsql.a
LIBKRB5     := $(KRB5_DIR)/lib/libgssapi_krb5.a
PROTOC 		 := $(PROTOC_DIR)/protoc

C_LIBS_COMMON = $(if $(use-stdmalloc),,$(LIBJEMALLOC)) $(LIBPROTOBUF) $(LIBSNAPPY) $(LIBROCKSDB) $(LIBINJECTION) $(LIBPLSQL)
C_LIBS_OSS = $(C_LIBS_COMMON) $(LIBROACH) 
C_LIBS_ICL = $(C_LIBS_COMMON) $(LIBCRYPTOPP) $(LIBROACHICL)
...
CGO_PKGS := cli server/status storage/engine icl/storageicl/engineicl icl/gssapiicl sql/pgwire sql udr
...
$(CGO_FLAGS_FILES): Makefile
	...
	@echo '// #cgo LDFLAGS: $(addprefix -L,$(CRYPTOPP_DIR) $(PROTOBUF_DIR) $(JEMALLOC_DIR)/lib $(SNAPPY_DIR) $(ROCKSDB_DIR) $(LIBROACH_DIR) $(LIBINJECTION_DIR) $(LIBPLSQL_DIR) $(LIBPYTHON3_DIR) $(KRB_DIR))' >> $@
	@echo 'import "C"' >> $@
...
# in GCM mode (not currently in use by znbase).
C_COMPILE_FLAG ?= true
ifeq ($(C_COMPILE_FLAG), true)
...
$(KRB5_DIR)/Makefile: $(C_DEPS_DIR)/krb5-rebuild $(KRB5_SRC_DIR)/src/configure
	...
	cd $(KRB5_DIR) && $(KRB5_SRC_DIR)/src/configure $(xconfigure-flags) --enable-static --disable-shared
CFLAGS := $(CTEMP)
CXXFLAGS := $(CXXTEMP)
...
$(LIBINJECTION_DIR)/Makefile: $(C_DEPS_DIR)/libinjection-rebuild | bin/.submodules-initialized
	rm -rf $(LIBINJECTION_DIR)
	mkdir -p $(LIBINJECTION_DIR)
	@# NOTE: If you change the CMake flags below, bump the version in
	@# $(C_DEPS_DIR)/libinjection-rebuild. See above for rationale.
	cd $(LIBINJECTION_DIR) && cmake $(xcmake-flags) $(LIBINJECTION_SRC_DIR) -DCMAKE_BUILD_TYPE=Release

c-deps/libplsql/parser/pl_gram.tab.c: c-deps/libplsql/parser/pl_gram.y
	cd c-deps/libplsql/parser && bison -Wno-deprecated -d pl_gram.y

$(LIBPLSQL_DIR)/Makefile: $(C_DEPS_DIR)/libplsql-rebuild | bin/.submodules-initialized c-deps/libplsql/parser/pl_gram.tab.c
	rm -rf $(LIBPLSQL_DIR)
	mkdir -p $(LIBPLSQL_DIR)
	@# NOTE: If you change the CMake flags below, bump the version in
	@# $(C_DEPS_DIR)/libinjection-rebuild. See above for rationale.
	cd $(LIBPLSQL_DIR) && cmake $(xcmake-flags) $(LIBPLSQL_SRC_DIR) -DCMAKE_BUILD_TYPE=Release


# TODO(benesch): make it possible to build libroach without ICL code. Because
# libroach and libroachicl are defined in the same CMake project, CMake requires
# that the ICL code be present even if only the OSS target will be built.
$(LIBROACH_DIR)/Makefile: $(C_DEPS_DIR)/libroach-rebuild | bin/.submodules-initialized bin/.cpp_protobuf_sources bin/.cpp_icl_protobuf_sources
	rm -rf $(LIBROACH_DIR)
...
$(LIBROACHICL): $(LIBROACH_DIR)/Makefile bin/uptodate .ALWAYS_REBUILD
	@uptodate $@ $(libroach-inputs) || $(MAKE) --no-print-directory -C $(LIBROACH_DIR) roachicl

$(LIBINJECTION): $(LIBINJECTION_DIR)/Makefile bin/uptodate .ALWAYS_REBUILD
	@uptodate $@ $(LIBINJECTION_SRC_DIR) || $(MAKE) --no-print-directory -C $(LIBINJECTION_DIR) injection

$(LIBPLSQL): $(LIBPLSQL_DIR)/Makefile bin/uptodate .ALWAYS_REBUILD
	@uptodate $@ $(LIBPLSQL_SRC_DIR) || $(MAKE) --no-print-directory -C $(LIBPLSQL_DIR)
...
.PHONY: protoc libcryptopp libjemalloc libprotobuf libsnappy librocksdb libroach libinjection libroachicl libkrb5 libplsql
protoc:      $(PROTOC)
...
ZNBase      := ./znbase$(SUFFIX)
ZNBaseOSS   := ./znbaseoss$(SUFFIX)
ZNBaseSHORT := ./znbaseshort$(SUFFIX)
...
PROTOBUF_TARGETS := bin/.go_protobuf_sources bin/.gw_protobuf_sources bin/.cpp_protobuf_sources bin/.cpp_icl_protobuf_sources

DOCGEN_TARGETS := bin/.docgen_bnfs bin/.docgen_functions

EXECGEN_TARGETS = \
  pkg/col/coldata/vec.eg.go \
  pkg/sql/distsqlrun/vecexec/and_or_projection.eg.go \
  pkg/sql/distsqlrun/vecexec/any_not_null_agg.eg.go \
  pkg/sql/distsqlrun/vecexec/avg_agg.eg.go \
  pkg/sql/distsqlrun/vecexec/cast.eg.go \
  pkg/sql/distsqlrun/vecexec/const.eg.go \
  pkg/sql/distsqlrun/vecexec/distinct.eg.go \
  pkg/sql/distsqlrun/vecexec/hashjoiner.eg.go \
  pkg/sql/distsqlrun/vecexec/like_ops.eg.go \
  pkg/sql/distsqlrun/vecexec/mergejoinbase.eg.go \
  pkg/sql/distsqlrun/vecexec/mergejoiner_fullouter.eg.go \
  pkg/sql/distsqlrun/vecexec/mergejoiner_inner.eg.go \
  pkg/sql/distsqlrun/vecexec/mergejoiner_leftanti.eg.go \
  pkg/sql/distsqlrun/vecexec/mergejoiner_leftouter.eg.go \
  pkg/sql/distsqlrun/vecexec/mergejoiner_leftsemi.eg.go \
  pkg/sql/distsqlrun/vecexec/mergejoiner_rightouter.eg.go \
  pkg/sql/distsqlrun/vecexec/min_max_agg.eg.go \
  pkg/sql/distsqlrun/vecexec/overloads_test_utils.eg.go \
  pkg/sql/distsqlrun/vecexec/proj_const_left_ops.eg.go \
  pkg/sql/distsqlrun/vecexec/proj_const_right_ops.eg.go \
  pkg/sql/distsqlrun/vecexec/proj_non_const_ops.eg.go \
  pkg/sql/distsqlrun/vecexec/rank.eg.go \
  pkg/sql/distsqlrun/vecexec/row_number.eg.go \
  pkg/sql/distsqlrun/vecexec/quicksort.eg.go \
  pkg/sql/distsqlrun/vecexec/rowstovec.eg.go \
  pkg/sql/distsqlrun/vecexec/selection_ops.eg.go \
  pkg/sql/distsqlrun/vecexec/select_in.eg.go \
  pkg/sql/distsqlrun/vecexec/sort.eg.go \
  pkg/sql/distsqlrun/vecexec/sum_agg.eg.go \
  pkg/sql/distsqlrun/vecexec/tuples_differ.eg.go \
  pkg/sql/distsqlrun/vecexec/vec_comparators.eg.go \
  pkg/sql/distsqlrun/vecexec/zerocolumns.eg.go

execgen-exclusions = $(addprefix -not -path ,$(EXECGEN_TARGETS))

$(info Cleaning old generated files.)
$(shell find pkg/sql/distsqlrun/vecexec -type f -name '*.eg.go' $(execgen-exclusions) -delete)
$(shell find pkg/sql/distsqlrun/rowexec -type f -name '*.eg.go' $(execgen-exclusions) -delete 2>/dev/null)
...
go-targets := $(go-targets-icl) $(ZNBaseOSS)
...
$(ZNBase) go-install generate: pkg/ui/disticl/bindata.go

$(ZNBaseOSS): BUILDTARGET = ./pkg/cmd/znbase-oss
$(ZNBaseOSS): $(C_LIBS_OSS) pkg/ui/distoss/bindata.go

$(ZNBaseSHORT): BUILDTARGET = ./pkg/cmd/znbase-short

$(go-targets-icl): $(C_LIBS_ICL)
...
$(go-targets): override LINKFLAGS += \
	-X "github.com/znbasedb/znbase/pkg/build.tag=$(if $(BUILDINFO_TAG),$(BUILDINFO_TAG),$(shell cat .buildinfo/tag))" \
	-X "github.com/znbasedb/znbase/pkg/build.rev=$(shell cat .buildinfo/rev)" \
	-X "github.com/znbasedb/znbase/pkg/build.cgoTargetTriple=$(TARGET_TRIPLE)" \
	$(if $(BUILDCHANNEL),-X "github.com/znbasedb/znbase/pkg/build.channel=$(BUILDCHANNEL)") \
	$(if $(BUILD_TAGGED_RELEASE),-X "github.com/znbasedb/znbase/pkg/util/log.crashReportEnv=$(if $(BUILDINFO_TAG),$(BUILDINFO_TAG),$(shell cat .buildinfo/tag))")
...
# every rebuild.
$(ZNBase) $(ZNBaseOSS) $(ZNBaseSHORT) go-install: override LINKFLAGS += \
	-X "github.com/znbasedb/znbase/pkg/build.utcTime=$(shell date -u '+%Y/%m/%d %H:%M:%S')"
...
.PHONY: go-install
$(ZNBase) $(ZNBaseOSS) $(ZNBaseSHORT) go-install: bin/g++
	 $(xgo) $(build-mode) -v $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LINKFLAGS)' $(BUILDTARGET)

# The build targets, in addition to producing a ZNBase binary, silently
...
build: ## Build the ZNBaseDB binary.
buildoss: ## Build the ZNBaseDB binary without any ICL-licensed code.
buildshort: ## Build the ZNBaseDB binary without the admin UI.
build: $(ZNBase)
buildoss: $(ZNBaseOSS)
buildshort: $(ZNBaseSHORT)
build buildoss buildshort: $(DOCGEN_TARGETS)
build buildshort: $(if $(is-cross-compile),,$(SETTINGS_DOC_PAGE))

# For historical reasons, symlink znbase to znbaseshort.
# TODO(benesch): see if it would break anyone's workflow to remove this.
buildshort:
	ln -sf $(ZNBaseSHORT) $(ZNBase)

.PHONY: install
install: ## Install the ZNBaseDB binary.
install: $(ZNBase)
	$(INSTALL) -d -m 755 $(DESTDIR)$(bindir)
	$(INSTALL) -m 755 $(ZNBase) $(DESTDIR)$(bindir)/znbase

.PHONY: start
start: $(ZNBase)
start:
	$(ZNBase) start $(STARTFLAGS)
...
.PHONY: check test testshort testrace testlogic testbaselogic testplannerlogic testprivilegelogic testicllogic testoptlogic bench benchshort
test: ## Run tests.
check test testshort testrace bench benchshort: bin/g++
	$(xgo) test $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LINKFLAGS)' -run "$(TESTS)" $(if $(BENCHES),-bench "$(BENCHES)") -timeout $(TESTTIMEOUT) $(PKG) $(RAFTPKG) $(TESTFLAGS)
...
testlogic: testbaselogic testplannerlogic testprivilegelogic testoptlogic testicllogic
...
testprivilegelogic: ## Run SQL Logic Tests for the privilege.
testprivilegelogic: bin/logictest

testicllogic: ## Run SQL ICL Logic Tests.
testicllogic: bin/logictesticl
...
testicllogic: TESTS := TestICLLogic/$(logic-test-selector)
testplannerlogic: TESTS := TestPlannerLogic/$(logic-test-selector)
testprivilegelogic: TESTS := TestPrivilegeLogic/$(logic-test-selector)
...
testbaselogic testicllogic testplannerlogic testprivilegelogic testoptlogic: TESTFLAGS := -test.v $(if $(FILES),-show-sql) $(if $(TESTCONFIG),-config $(TESTCONFIG))
testbaselogic testicllogic testplannerlogic testprivilegelogic testoptlogic:
...
# $(ARCHIVE_BASE)/src/github.com/znbasedb/znbase to allow the extracted
...
ARCHIVE_EXTRAS = \
	...
	$(EXECGEN_TARGETS) \
	$(OPTGEN_TARGETS) \
	pkg/ui/disticl/bindata.go pkg/ui/distoss/bindata.go
...
$(ARCHIVE).tmp: ARCHIVE_BASE = znbase-$(if $(BUILDINFO_TAG),$(BUILDINFO_TAG),$(shell cat .buildinfo/tag))
$(ARCHIVE).tmp: $(ARCHIVE_EXTRAS)
	echo "$(if $(BUILDINFO_TAG),$(BUILDINFO_TAG),$(shell cat .buildinfo/tag))" > .buildinfo/tag
	scripts/ls-files.sh | $(TAR) -cf $@ -T - $(TAR_XFORM_FLAG),^,$(ARCHIVE_BASE)/src/github.com/znbasedb/znbase/, $^
...
CPP_PROTO_ICL_ROOT := $(LIBROACH_SRC_DIR)/protosicl
...
ERRORS1_PATH := ./vendor/github.com/znbasedb/errors
ERRORS_PROTO := $(ERRORS1_PATH)/errorspb/errors.proto
...
JS_PROTOS_ICL := $(filter %/icl/storageicl/engineicl/enginepbicl/stats.proto,$(GO_PROTOS))
UI_JS_ICL := pkg/ui/icl/src/js/protos.js
UI_TS_ICL := pkg/ui/icl/src/js/protos.d.ts
UI_PROTOS_ICL := $(UI_JS_ICL) $(UI_TS_ICL)

UI_JS_OSS := pkg/ui/src/js/protos.js
UI_TS_OSS := pkg/ui/src/js/protos.d.ts
UI_PROTOS_OSS := $(UI_JS_OSS) $(UI_TS_OSS)

CPP_PROTOS := $(filter %/roachpb/api.proto %/roachpb/pushdown.proto %/roachpb/metadata.proto %/roachpb/data.proto %/roachpb/internal.proto %/roachpb/errors.proto %/roachpb/api.proto %util/tracing/recorded_span.proto %/concurrency/lock/locking.proto %/engine/enginepb/mvcc.proto %/engine/enginepb/mvcc3.proto %/engine/enginepb/file_registry.proto %/engine/enginepb/rocksdb.proto %/hlc/legacy_timestamp.proto %/hlc/timestamp.proto %/log/log.proto %/unresolved_addr.proto,$(GO_PROTOS))
CPP_HEADERS := $(subst ./pkg,$(CPP_PROTO_ROOT),$(CPP_PROTOS:%.proto=%.pb.h))
CPP_SOURCES := $(subst ./pkg,$(CPP_PROTO_ROOT),$(CPP_PROTOS:%.proto=%.pb.cc))

CPP_PROTOS_ICL := $(filter %/icl/baseicl/encryption_options.proto %/icl/storageicl/engineicl/enginepbicl/key_registry.proto %/icl/storageicl/engineicl/enginepbicl/stats.proto,$(GO_PROTOS))
CPP_HEADERS_ICL := $(subst ./pkg,$(CPP_PROTO_ICL_ROOT),$(CPP_PROTOS_ICL:%.proto=%.pb.h))
CPP_SOURCES_ICL := $(subst ./pkg,$(CPP_PROTO_ICL_ROOT),$(CPP_PROTOS_ICL:%.proto=%.pb.cc))
...
bin/.go_protobuf_sources: $(PROTOC) $(GO_PROTOS) $(GOGOPROTO_PROTO) $(ERRORS_PROTO) bin/.bootstrap bin/protoc-gen-gogoroach
	...
	set -e; for dir in $(sort $(dir $(GO_PROTOS))); do \
	  build/werror.sh $(PROTOC) -Ipkg:./vendor/github.com:$(GOGO_PROTOBUF_PATH):$(PROTOBUF_PATH):$(COREOS_PATH):$(GRPC_GATEWAY_GOOGLEAPIS_PATH):$(ERRORS1_PATH) --gogoroach_out=$(PROTO_MAPPINGS),plugins=grpc,import_prefix=github.com/znbasedb/znbase/pkg/:./pkg $$dir/*.proto; \
	done
	$(SED_INPLACE) -E \
		-e '/import _ /d' \
		-e 's!import (fmt|math) "github.com/znbasedb/znbase/pkg/(fmt|math)"! !g' \
		-e 's!github\.com/znbasedb/znbase/pkg/(etcd)!go.etcd.io/\1!g' \
		-e 's!github.com/znbasedb/znbase/pkg/((bytes|encoding/binary|errors|fmt|io|math|github\.com|(google\.)?golang\.org)([^a-z]|$$))!\1!g' \
    		-e 's!github.com/znbasedb/znbase/pkg/errorspb!github.com/znbasedb/errors/errorspb!g' \
...
bin/.gw_protobuf_sources: $(PROTOC) $(GW_SERVER_PROTOS) $(GW_TS_PROTOS) $(GO_PROTOS) $(GOGOPROTO_PROTO) $(ERRORS_PROTO) bin/.bootstrap
	...
	build/werror.sh $(PROTOC) -Ipkg:./vendor/github.com:$(GOGO_PROTOBUF_PATH):$(PROTOBUF_PATH):$(ERRORS1_PATH):$(COREOS_PATH):$(GRPC_GATEWAY_GOOGLEAPIS_PATH) --grpc-gateway_out=logtostderr=true,request_context=true:./pkg $(GW_SERVER_PROTOS)
	build/werror.sh $(PROTOC) -Ipkg:./vendor/github.com:$(GOGO_PROTOBUF_PATH):$(PROTOBUF_PATH):$(ERRORS1_PATH):$(COREOS_PATH):$(GRPC_GATEWAY_GOOGLEAPIS_PATH) --grpc-gateway_out=logtostderr=true,request_context=true:./pkg $(GW_TS_PROTOS)
...
bin/.cpp_icl_protobuf_sources: $(PROTOC) $(CPP_PROTOS_ICL)
	rm -rf $(CPP_PROTO_ICL_ROOT)
	mkdir -p $(CPP_PROTO_ICL_ROOT)
	build/werror.sh $(PROTOC) -Ipkg:$(GOGO_PROTOBUF_PATH):$(PROTOBUF_PATH) --cpp_out=lite:$(CPP_PROTO_ICL_ROOT) $(CPP_PROTOS_ICL)
	$(SED_INPLACE) -E '/gogoproto/d' $(CPP_HEADERS_ICL) $(CPP_SOURCES_ICL)
	touch $@

# The next two rules must be kept exactly the same except the ICL one depends
...
ENCRYPTION_ICL := $(CPP_PROTOS_ICL) $(JS_PROTOS_ICL)

.SECONDARY: $(UI_JS_ICL)
$(UI_JS_ICL): $(GW_PROTOS) $(GO_PROTOS) $(ENCRYPTION_ICL) pkg/ui/yarn.installed | bin/.submodules-initialized
	# Add comment recognized by reviewable.
	echo '// GENERATED FILE DO NOT EDIT' > $@
	$(PBJS) -t static-module -w es6 --strict-long --keep-case --path pkg --path ./vendor/github.com --path $(GOGO_PROTOBUF_PATH) --path $(ERRORS1_PATH) --path $(COREOS_PATH) --path $(GRPC_GATEWAY_GOOGLEAPIS_PATH) $(filter %.proto,$(GW_PROTOS) $(ENCRYPTION_ICL)) >> $@

.SECONDARY: $(UI_JS_OSS)
$(UI_JS_OSS): $(GW_PROTOS) $(GO_PROTOS) $(ENCRYPTION_ICL) pkg/ui/yarn.installed | bin/.submodules-initialized
	# Add comment recognized by reviewable.
	echo '// GENERATED FILE DO NOT EDIT' > $@
	$(PBJS) -t static-module -w es6 --strict-long --keep-case --path pkg --path ./vendor/github.com --path $(GOGO_PROTOBUF_PATH) --path $(ERRORS1_PATH) --path $(COREOS_PATH) --path $(GRPC_GATEWAY_GOOGLEAPIS_PATH) $(filter %.proto,$(GW_PROTOS) $(ENCRYPTION_ICL)) >> $@
...
.PHONY: ui-generate
ui-generate: pkg/ui/disticl/bindata.go
...
.PHONY: ui-lint
ui-lint: pkg/ui/yarn.installed $(UI_PROTOS_OSS) $(UI_PROTOS_ICL)
...
UI_ICL_DLLS := pkg/ui/dist/protos.icl.dll.js pkg/ui/dist/vendor.oss.dll.js
UI_ICL_MANIFESTS := pkg/ui/protos.icl.manifest.json pkg/ui/vendor.oss.manifest.json
UI_OSS_DLLS := $(subst .icl,.oss,$(UI_ICL_DLLS))
UI_OSS_MANIFESTS := $(subst .icl,.oss,$(UI_ICL_MANIFESTS))
...
.SECONDARY: $(UI_ICL_DLLS) $(UI_ICL_MANIFESTS) $(UI_OSS_DLLS) $(UI_OSS_MANIFESTS)
...
pkg/ui/dist/%.icl.dll.js pkg/ui/%.icl.manifest.json: pkg/ui/webpack.%.js pkg/ui/yarn.installed $(UI_PROTOS_ICL)
	$(NODE_RUN) -C pkg/ui $(WEBPACK) -p --config webpack.$*.js --env.dist=icl

.PHONY: ui-test
ui-test: $(UI_ICL_DLLS) $(UI_ICL_MANIFESTS)
	$(NODE_RUN) -C pkg/ui $(KARMA) start

.PHONY: ui-test-watch
ui-test-watch: $(UI_ICL_DLLS) $(UI_ICL_MANIFESTS)
...
pkg/ui/disticl/bindata.go: $(UI_ICL_DLLS) $(UI_ICL_MANIFESTS) $(UI_JS_ICL) $(shell find pkg/ui/icl -type f)
...
pkg/ui/dist%/bindata.go: pkg/ui/webpack.app.js $(shell find pkg/ui/src pkg/ui/styl -type f) | bin/.bootstrap
	...
	set -e; shopt -s extglob; for dll in $(notdir $(filter %.dll.js,$^)); do \
	  ln -s ../dist/$$dll pkg/ui/dist$*/$${dll/@(.icl|.oss)}; \
	done
...
ui-watch ui-watch-secure: $(UI_ICL_DLLS) pkg/ui/yarn.opt.installed
	cd pkg/ui && $(WEBPACK_DASHBOARD) -- $(WEBPACK_DEV_SERVER) --config webpack.app.js --env.dist=icl --port $(PORT) $(WEBPACK_DEV_SERVER_FLAGS)

.PHONY: ui-clean
ui-clean: ## Remove build artifacts.
	find pkg/ui/dist* -mindepth 1 -not -name dist*.go -delete
	rm -f $(UI_PROTOS_ICL) $(UI_PROTOS_OSS)
...
settings-doc-gen := $(if $(filter buildshort,$(MAKECMDGOALS)),$(ZNBaseSHORT),$(ZNBase))
...
pkg/sql/distsqlrun/vecexec/and_or_projection.eg.go: pkg/sql/distsqlrun/vecexec/and_or_projection_tmpl.go
pkg/sql/distsqlrun/vecexec/any_not_null_agg.eg.go: pkg/sql/distsqlrun/vecexec/any_not_null_agg_tmpl.go
pkg/sql/distsqlrun/vecexec/avg_agg.eg.go: pkg/sql/distsqlrun/vecexec/avg_agg_tmpl.go
pkg/sql/distsqlrun/vecexec/cast.eg.go: pkg/sql/distsqlrun/vecexec/cast_tmpl.go
pkg/sql/distsqlrun/vecexec/const.eg.go: pkg/sql/distsqlrun/vecexec/const_tmpl.go
pkg/sql/distsqlrun/vecexec/distinct.eg.go: pkg/sql/distsqlrun/vecexec/distinct_tmpl.go
pkg/sql/distsqlrun/vecexec/hashjoiner.eg.go: pkg/sql/distsqlrun/vecexec/hashjoiner_tmpl.go
pkg/sql/distsqlrun/vecexec/mergejoinbase.eg.go: pkg/sql/distsqlrun/vecexec/mergejoinbase_tmpl.go
pkg/sql/distsqlrun/vecexec/mergejoiner_fullouter.eg.go: pkg/sql/distsqlrun/vecexec/mergejoiner_tmpl.go
pkg/sql/distsqlrun/vecexec/mergejoiner_inner.eg.go: pkg/sql/distsqlrun/vecexec/mergejoiner_tmpl.go
pkg/sql/distsqlrun/vecexec/mergejoiner_leftanti.eg.go: pkg/sql/distsqlrun/vecexec/mergejoiner_tmpl.go
pkg/sql/distsqlrun/vecexec/mergejoiner_leftouter.eg.go: pkg/sql/distsqlrun/vecexec/mergejoiner_tmpl.go
pkg/sql/distsqlrun/vecexec/mergejoiner_leftsemi.eg.go: pkg/sql/distsqlrun/vecexec/mergejoiner_tmpl.go
pkg/sql/distsqlrun/vecexec/mergejoiner_rightouter.eg.go: pkg/sql/distsqlrun/vecexec/mergejoiner_tmpl.go
pkg/sql/distsqlrun/vecexec/min_max_agg.eg.go: pkg/sql/distsqlrun/vecexec/min_max_agg_tmpl.go
pkg/sql/distsqlrun/vecexec/proj_const_left_ops.eg.go: pkg/sql/distsqlrun/vecexec/proj_const_ops_tmpl.go
pkg/sql/distsqlrun/vecexec/proj_const_right_ops.eg.go: pkg/sql/distsqlrun/vecexec/proj_const_ops_tmpl.go
pkg/sql/distsqlrun/vecexec/proj_non_const_ops.eg.go: pkg/sql/distsqlrun/vecexec/proj_non_const_ops_tmpl.go
pkg/sql/distsqlrun/vecexec/quicksort.eg.go: pkg/sql/distsqlrun/vecexec/quicksort_tmpl.go
pkg/sql/distsqlrun/vecexec/rank.eg.go: pkg/sql/distsqlrun/vecexec/rank_tmpl.go
pkg/sql/distsqlrun/vecexec/row_number.eg.go: pkg/sql/distsqlrun/vecexec/row_number_tmpl.go
pkg/sql/distsqlrun/vecexec/rowstovec.eg.go: pkg/sql/distsqlrun/vecexec/rowstovec_tmpl.go
pkg/sql/distsqlrun/vecexec/select_in.eg.go: pkg/sql/distsqlrun/vecexec/select_in_tmpl.go
pkg/sql/distsqlrun/vecexec/selection_ops.eg.go: pkg/sql/distsqlrun/vecexec/selection_ops_tmpl.go
pkg/sql/distsqlrun/vecexec/sort.eg.go: pkg/sql/distsqlrun/vecexec/sort_tmpl.go
pkg/sql/distsqlrun/vecexec/sum_agg.eg.go: pkg/sql/distsqlrun/vecexec/sum_agg_tmpl.go
pkg/sql/distsqlrun/vecexec/tuples_differ.eg.go: pkg/sql/distsqlrun/vecexec/tuples_differ_tmpl.go
pkg/sql/distsqlrun/vecexec/vec_comparators.eg.go: pkg/sql/distsqlrun/vecexec/vec_comparators_tmpl.go
pkg/sql/distsqlrun/vecexec/zerocolumns.eg.go: pkg/sql/distsqlrun/vecexec/zerocolumns_tmpl.go

$(EXECGEN_TARGETS): bin/execgen
	@# Remove generated files with the old suffix to avoid conflicts.
	@# See https://github.com/znbasedb/znbase/pull/32265.
	@rm -f pkg/sql/distsqlrun/rowexec/*.og.go
	@rm -f pkg/sql/distsqlrun/vecexec/*.og.go
	execgen $@
...
unsafe-clean-c-deps:
	...
	git -C $(LIBINJECTION_SRC_DIR)     clean -dxf
	git -C $(KRB5_SRC_DIR)     clean -dxf

.PHONY: clean-execgen-files
clean-execgen-files:
	find ./pkg/sql/distsqlrun/vecexec -type f -name '*.eg.go' -exec rm {} +
	test -d ./pkg/sql/distsqlrun/rowexec && find ./pkg/sql/distsqlrun/vecexec -type f -name '*.eg.go' -exec rm {} + || true
...
clean: clean-c-deps clean-execgen-files
	...
	$(GO) clean $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LINKFLAGS)' -i -cache github.com/znbasedb/znbase...
	$(FIND_RELEVANT) -type f \( -name 'zcgo_flags*.go' -o -name '*.test' \) -exec rm {} +
	for f in znbase*; do if [ -f "$$f" ]; then rm "$$f"; fi; done
...
maintainer-clean: clean ui-maintainer-clean
	rm -f $(SQLPARSER_TARGETS) $(EXECGEN_TARGETS) $(OPTGEN_TARGETS) $(UI_PROTOS_OSS) $(UI_PROTOS_ICL)
...
execgen-package = ./pkg/sql/distsqlrun/vecexec/execgen/cmd/execgen
...
logictesticl-package = ./pkg/icl/logictesticl
logictestopt-package = ./pkg/sql/opt/exec/execbuilder

logictest-bins := bin/logictest bin/logictestopt bin/logictesticl
...
bin/workload bin/roachtest $(logictest-bins): $(EXECGEN_TARGETS) bin/g++
bin/roachtest $(logictest-bins): $(C_LIBS_ICL) $(CGO_FLAGS_FILES)
