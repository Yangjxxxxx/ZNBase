...
func hookVisitor(v reflect.Value, hook func(v reflect.Value, i int)) {
	if v.Kind() == reflect.Struct {
		for i, n := 0, v.NumField(); i < n; i++ {
			...
			tag, ok := field.Tag.Lookup("znbasedb")
...
// ZeroInsertingVisitor replaces all nil struct fields which have the `znbasedb:"randnullable"`
...
// `znbasedb:"randnullable"` struct tag set. "Randomly" here means the following: upon first
