...
var TxnLivenessHeartbeatMultiplier = envutil.EnvOrDefaultInt(
	"ZNBASE_TXN_LIVENESS_HEARTBEAT_MULTIPLIER", 5)
...
var deadlocks = make([]deadlockMessage, 0, 100)
...
func ShouldPushImmediately(req *roachpb.PushTxnRequest) bool {
	...
	if req.PusherTxn.IsolationLevel == util.ReadCommittedIsolation && req.PushType == roachpb.PUSH_TIMESTAMP {
		return true
	}
	return false
}
	...
	for {
		...
		case <-pusheeTxnTimer.C:
			...
			if updatedPushee.Status.IsFinalized() {
				...
				if updatedPushee.Status == roachpb.ABORTED {
					// 通知其他等待推送者该交易现在最终确定。凭直觉，我们希望如果有任何推动器
					// 卡住，等待交易完成，那么它将交易记录移动时已听说过更新进入最终状态。
					// 这对于以下情况是正确的：该命令显式地写入了带有终结的事务记录状态。
					//
					// 但是，这并不说明交易的情况由于商店的分辨率下降而变得无法承诺时间戳缓存。
					// 在这种情况下，交易可能突然变成在没有相关记录写入的情况下不可提交。什么时候
					// 发生这种情况时，没有其他人会立即通知对方关于不可提交交易的推动者。最终
					// Pushee的协调员会过来并回滚其记录，但这只是在被压制者本身不等待结果的情况下
					// 其中一个推动器之一。如果有这样的依赖周期那么其他推动者可能不得不等待交易完成
					// 过期以再次查询pushee，并注意到pushee现在是不可提交的。
		...
		case updatedPusher := <-queryPusherCh:
			...
			case roachpb.ABORTED:
				...
				if updatedPusher.IsDeadlock {
					return nil, roachpb.NewErrorWithTxn(&roachpb.TransactionDeadlockError{}, updatedPusher)
				}
...
func (q *Queue) forcePushAbort(
	...
	//save deadlock message in cache
	deadlocks = append(deadlocks, deadlockMessage{
		PusherIDs: req.PusherTxn.ID,
		PusheeIDs: req.PusheeTxn.ID,
	})
	if !(len(deadlocks) < 100) {
		deadlocks = deadlocks[1:]
	}
	log.VEventf(
		ctx,
		2,
		"deadlock happened : %s , pushing %s aborted",
		req.PusherTxn.ID.Short(),
		req.PusheeTxn.ID.Short(),
	)
