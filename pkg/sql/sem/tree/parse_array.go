...
var enclosingError = pgerror.NewErrorf(pgcode.InvalidTextRepresentation, "array must be enclosed in { and }")
var extraTextError = pgerror.NewErrorf(pgcode.InvalidTextRepresentation, "extra text after closing right brace")
var nestedArraysNotSupportedError = pgerror.UnimplementedWithIssueDetailError(32552, "strcast", "nested arrays not supported")
var malformedError = pgerror.NewErrorf(pgcode.InvalidTextRepresentation, "malformed array")
...
//AddQuotation return the string with double quotation
func AddQuotation(s string) string {
	return "\"" + s + "\""
}

//ParseDArrayFromStringForDump mainly to provide this implementation for import and export
func ParseDArrayFromStringForDump(evalCtx *EvalContext, s string, t coltypes.T) (*DArray, error) {
	// check if there is a string like 'ARRAY[]'
	s = strings.TrimSpace(s)
	if len(s) < 7 || s[5] != '[' || s[len(s)-1] != ']' || s[0:5] != "ARRAY" {
		return nil, fmt.Errorf(" %s is not an array", s)
	}
	newS := "{" + s[6:len(s)-1] + "}"
	// call the function which can parse string like '{1,2,3}'
	return ParseDArrayFromString(evalCtx, newS, t)
}

//ParseDArrayForLoad parses the string-form of constructing arrays
func ParseDArrayForLoad(evalCtx *EvalContext, s string, t coltypes.T) (*DArray, error) {
	parser := parseState{
		s:       s,
		evalCtx: evalCtx,
	...
	bt[len(bt)-1] = 44
	head, tail := 0, 0
	for i := 1; i < len(bt)-1; i++ {
		if bt[i] == 34 && head == 0 {
			head = i + 1
			continue
		}
		if bt[i] == 34 && bt[i+1] == 44 && head != 0 {
			tail = i
			_ = parser.result.Append(NewDString(string(bt[head:tail])))
			head, tail = 0, 0
		}
	}
