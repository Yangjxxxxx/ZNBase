...
func TestVerifyPassword(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"azure_diamond", "aaAA!!11", false, "crypto/bcrypt"},
		{"azure_diamond", "*******", false, "crypto/bcrypt"},
		{"druidia", "aaAA!!11", true, ""},
		{"druidia", "11aa!!AA", false, "crypto/bcrypt"},
		{"root", "", false, "crypto/bcrypt"},
		{"", "", false, "does not exist"},
		{"doesntexist", "zxcvbn", false, "does not exist"},
	} {
		t.Run("", func(t *testing.T) {
			valid, _, err := ts.authentication.verifyPassword(context.TODO(), tc.username, tc.password)
			if err != nil {
				t.Errorf(
