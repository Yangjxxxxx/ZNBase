 ...
 *-------------------------------------------------------------------------
 */
#include "../include/plpgsql.h"
#include "../include/parser/parser.h"
#include "../include/parser/scanner.h"
#include "../include/parser/scansup.h"
				  ...
				  decl_cursor_args decl_is_for decl_cursor_query
					{
						...
						new = (PLpgSQL_var *)
							plpgsql_build_variable($1.name, $1.lineno,
												   plpgsql_build_datatype("cursor", REFCURSOROID,
																		  -1,
																		  InvalidOid),
												   true, false);
						...
						*cp2++ = '\'';
						...
						curname_def->query = $7->query;
						new->default_val = curname_def;

						new->cursor_explicit_expr = $7;
						new->cursor_explicit_expr->ns = plpgsql_ns_top();
						if ($5 == NULL)
...
stmt_getdiag	: K_GET getdiag_area_opt K_DIAGNOSTICS getdiag_list ';'
					{
						ereport(ERROR,
							(errcode(ERRCODE_SYNTAX_ERROR),
							 errmsg("don't support diagnostics"),
							 parser_errposition(@1)));
							...
							if (tok == DOT_DOT)
							{
								...
								fvar = (PLpgSQL_var *)
									plpgsql_build_variable($1.name,
														   $1.lineno,
														   plpgsql_build_datatype("int4", INT4OID,
																				  -1,
																				  InvalidOid),
														   true, false);
...
foreach_slice	:
                ...
                // current not support slice in for each because of znbasedb dose not support array[[]]
				/*| K_SLICE ICONST*/
					/*{*/
						/*$$ = $2;*/
					/*}*/
...
stmt_raise		: K_RAISE
					{
						...
						if (tok != ';')
						{
                            ...
                            // only support exception , becasue znbasedb not support notice or warning in pgwire
                            /*
							else if (tok_is_keyword(tok, &yylval,
													K_WARNING, "warning"))
							{
...
stmt_commit		: K_COMMIT opt_transaction_chain ';'
					{
						ereport(ERROR,
                                                        	(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                                                        	 errmsg("procedure does not support transaction"),
                                                        	 parser_errposition(@1)));
				...
				| K_EXCEPTION
					{
						...
						var = plpgsql_build_variable("sqlstate", lineno,
													 plpgsql_build_datatype("string", TEXTOID,
																			-1,
				   ...
				   int *endtoken)
{
	...
	expr->plan			= NULL;
	expr->planIndex		= BEPI_NOCACHED_PLAN;
	expr->paramnos		= NULL;
	expr->rwparam		= -1;
...
check_sql_expr(const char *stmt, int location, int leaderlen)
{
	...
	char *err_check_inBini = CheckStmtParse((char*)stmt);
	if (strlen(err_check_inBini) != 0) {
	    ereport(ERROR, (errmsg("%s", err_check_inBini)));
	}
...
parse_datatype(const char *string, int location)
{
	...
	Handler_t handler;
	handler.ErrMsg = NULL;
	type_id = SearchTypeOidByName(handler, (char*)string);
	if (type_id == 0)
        ereport(ERROR, (errcode(ERRCODE_DATATYPE_MISMATCH),
                   errmsg("type \"%s\" does not exist", string)));
	typmod = -1; //useless . get 'typmod' attr latar
	error_context_stack = syntax_errcontext.previous;
	/* Okay, build a PLpgSQL_type data structure for it */
	return plpgsql_build_datatype((char*)string, type_id, typmod,
								  plpgsql_curr_compile->fn_input_collation);
}
	...
	if (t_expr)
	{
		...
		t_var = (PLpgSQL_var *)
			plpgsql_build_variable(varname, new->lineno,
								   plpgsql_build_datatype("int8", INT8OID,
														  -1,
														  InvalidOid),
								   true, false);
