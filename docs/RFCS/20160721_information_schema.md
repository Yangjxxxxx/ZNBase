...
- RFC PR: [#7965](https://github.com/znbasedb/znbase/pull/7965)
- ZNBase Issue: [#5194](https://github.com/znbasedb/znbase/issues/5194),
                   [#5583](https://github.com/znbasedb/znbase/issues/5583)
...
specification in ZNBaseDB.

# Motivation

Up until this point, ZNBaseDB has only supported limited schema reflection through
...
proper support for these meta table, many tools simply cannot be used with ZNBaseDB.
This is an issue, as support for these tools is critical for user accessibility
of ZNBaseDB. Because of this widespread usage of `information_schema`, adding support
for the standard will be a major step towards supporting a group of ORMs and other tools,
and by extension, expanding the ecosystem around ZNBaseDB.
...
implementation efforts towards adding support for VIEWs to ZNBaseDB may touch
...
introduction of `information_schema` into ZNBaseDB. The first concern is that the
...
except there is no need to actually store the descriptors in an underlying ZNBase KV store
...
[`DatabaseAccessor`](https://github.com/znbasedb/znbase/blob/4d45696ea776a5b912262e5eef9889eacb6abe41/sql/database.go#L90)
and
[`SchemaAccessor`](https://github.com/znbasedb/znbase/blob/4d45696ea776a5b912262e5eef9889eacb6abe41/sql/table.go#L89),
...
[`planner.getDataSource`](https://github.com/znbasedb/znbase/blob/4d45696ea776a5b912262e5eef9889eacb6abe41/sql/data_source.go#L191),
...
are not relevant for our implementation, and add ZNBaseDB-specific extension columns
if any specific need for them arises.

ZNBaseDB-specific information which makes sense to add to our `information_schema`
...
### Information_Schema Across Different Versions of ZNBaseDB

Because this proposal suggests the use of static virtual descriptors instead of real
descriptors persisted to an underlying ZNBaseDB store, the descriptors used for a
given `information_schema` query will depend on the version of the ZNBaseDB instance
