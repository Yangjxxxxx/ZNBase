...
import (
	...
	"encoding/binary"
	"fmt"
	"log" // Don't bring znbase/util/log into this low-level package.
	"math/rand"
...
// ZNBASE_RANDOM_SEED=x to reuse the same value later. This function should
// be called from TestMain; individual tests should not touch the seed
// of the global random number generator.
func SeedForTests() {
	seed := envutil.EnvOrDefaultInt64("ZNBASE_RANDOM_SEED", NewPseudoSeed())
	rand.Seed(seed)
