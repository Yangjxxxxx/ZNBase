...
type formattedSeries = {
  values: protos.znbase.ts.tspb.ITimeSeriesDatapoint[],
  key: string,
  area: boolean,
  fillOpacity: number,
};
...
export function InitLineChart(chart: nvd3.LineChart) {
    chart
      .x((d: protos.znbase.ts.tspb.TimeSeriesDatapoint) => new Date(NanoToMilli(d && d.timestamp_nanos.toNumber())))
      .y((d: protos.znbase.ts.tspb.TimeSeriesDatapoint) => d && d.value)
