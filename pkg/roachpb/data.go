...
var (
	...
	// implemented in package git.com/znbasedb/znbase/keys to avoid
...
// messages which refer to ZNBase keys.
type Key []byte
...
func (v *Value) SetProto(msg protoutil.Message) error {
	...
	// All of the ZNBase protos implement MarshalTo and Size. So we marshal
...
// 如果事务没有被初始化的话，那么就通过AssertInitialized方法抛出错误信息
...
func (t *Transaction) Restart(
	...
	t.BumpEpoch()
	t.RefreshAll(userPriority, upgradePriority, timestamp)
}

// RefreshAll 事务刷新
func (t *Transaction) RefreshAll(
	userPriority UserPriority, upgradePriority enginepb.TxnPriority, timestamp hlc.Timestamp,
) {
...
// epochs.
// BumpEpoch增加了事务的epoch，允许原地重启。BumpEpoch使的低于epoch的写意图都被作废
...
// copied from o.
// Update使的事务的优先级，时间戳和事务初始时间戳的值得到升级更新。如果方法调用的事务对象的ID值为空，那么这个事务对象就会从Update方法的参数事务对象进行拷贝
func (t *Transaction) Update(o *Transaction) {
	if o == nil {
		return
	}

	//通过AssertInitialized方法判断事务是否被初始化
...
// IsLocking returns whether the transaction has begun writing intents.
...
func PrepareTransactionForRetry(
	...
	case *TransactionAbortedError, *TransactionDeadlockError:
	...
	if !aborted {
		...
		if txn.IsolationLevel == util.ReadCommittedIsolation {
			txn.RefreshAll(pri, txn.Priority, txn.WriteTimestamp)
		} else {
...
func CanTransactionRetryAtRefreshedTimestamp(
	...
	case *TransactionRetryError:
		...
		wto = err.Reason == RETRY_WRITE_TOO_OLD
	case *WriteTooOldError:
	...
	if wto && txn.IsolationLevel == util.ReadCommittedIsolation {
		return false, nil
	}
