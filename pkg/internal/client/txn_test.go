...
func TestUpdateDeadlineMaybe(t *testing.T) {
	...
	if ts, _ := txn.deadline(ctx); ts != nil {
		t.Errorf("txn.LeaseMgr:%v", txn.GetLeaseMgr())
		t.Errorf("txn.Deadline:%v", txn.Deadline())
		t.Errorf("txn.TableIDLise::%v", txn.GetTableIDList())
		t.Errorf("unexpected initial deadline: %s", ts)
	}
	...
	if d, _ := txn.deadline(ctx); *d != deadline {
	...
	if d, _ := txn.deadline(ctx); *d != pastDeadline {
		t.Errorf("unexpected deadline: %s", d)
	}
}
...
func TestSetIsolationLevel(t *testing.T) {
	...
	testCase := struct {
		input  []int
		output []util.IsolationLevel
	}{
		input: []int{0, 1, 2, 3},
		output: []util.IsolationLevel{util.UnspecifiedIsolation, util.ReadCommittedIsolation,
			util.SerializableIsolation, util.UnreachableIsolation},
	}

	for _, i := range testCase.input {
		isolation := util.IsolationLevel(i)
		txn.SetIsolationLevel(isolation)
		if d := txn.IsolationLevel(); d != testCase.output[i] {
			t.Errorf("expected setting the transaction's isolation level is %v,"+
				" but the isolaton level is %v", testCase.output[i], d)
		}
	}
}
