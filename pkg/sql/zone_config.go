...
func getZoneConfig(
	...
	if descVal, err := getKey(sqlbase.MakeDescMetadataKey(sqlbase.ID(id))); err != nil {
		...
		case *sqlbase.Descriptor_Schema:
			schemaDesc := desc.GetSchema()
			dbID, zone, _, _, err := getZoneConfig(uint32(schemaDesc.ParentID), getKey, false /* getInheritedDefault */)
			if err != nil {
				return 0, nil, 0, nil, err
			}
			return dbID, zone, placeholderID, placeholder, nil
...
// GenerateSubzoneSpans is a hook point for a ICL function that constructs from
// a TableDescriptor the entries mapping zone config spans to subzones for use
// in the SubzoneSpans field of config.ZoneConfig. If no ICL hook is installed,
// it returns an error that directs users to use a ICL binary.
var GenerateSubzoneSpans = func(
	...
	return nil, nil // no longer generate subzone span as we donot support zone feature
}
...
func resolveSubzone(
	...
	if partitionName := string(zs.Partition); partitionName != "" {
		if partitioning := sqlbase.FindIndexPartitionByName(index, partitionName); partitioning == nil {
			return nil, "", fmt.Errorf("partition %q does not exist on index %q", partitionName, index.Name)
		}
