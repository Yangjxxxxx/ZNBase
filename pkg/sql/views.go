...
func (p *planner) analyzeViewQuery(
...
) (planDependencies, sqlbase.ResultColumns, bool, error) {
	var hasStar = false
	...
	return p.curPlan.deps, planColumns(sourcePlan), hasStar, nil
}
...
func checkViewMatchesMaterialized(
	...
	if isMaterialized && !wantMaterialized {
		err := errors.Newf("%q is a materialized view", desc.GetName())
		return errors.WithHint(err, "use the corresponding MATERIALIZED VIEW command")
	}
	if !isMaterialized && wantMaterialized {
		return errors.Newf("%q is not a materialized view", desc.GetName())
	}
	return nil
}
