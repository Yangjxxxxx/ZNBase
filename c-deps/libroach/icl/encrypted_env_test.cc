...
rocksdb::Status checkFileEntry(FileRegistry& registry, const std::string& filename,
                               enginepbicl::EncryptionType enc_type) {
...
TEST(EncryptedEnv, FileOps) {
  ...
  EXPECT_OK(checkFileEntry(*file_registry, file1, enginepbicl::AES128_CTR));

  // Rename file.
  ASSERT_OK(encrypted_env->RenameFile(file1, file2));
  EXPECT_OK(checkNoFileEntry(*file_registry, file1));
  EXPECT_OK(checkFileEntry(*file_registry, file2, enginepbicl::AES128_CTR));

  // Link file (hard link).
  ASSERT_OK(encrypted_env->LinkFile(file2, file3));
  EXPECT_OK(checkFileEntry(*file_registry, file2, enginepbicl::AES128_CTR));
  EXPECT_OK(checkFileEntry(*file_registry, file3, enginepbicl::AES128_CTR));
  ...
  EXPECT_OK(checkFileEntry(*file_registry, file1, enginepbicl::Plaintext));
  EXPECT_OK(checkFileEntry(*file_registry, file2, enginepbicl::AES128_CTR));
