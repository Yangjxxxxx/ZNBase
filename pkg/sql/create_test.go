...
func TestDatabaseDescriptor(t *testing.T) {
	...
	//create database test and the schema test.schema
	expectedCounter += 2
...
func createTestTable(
	...
	for {
		if _, err := db.Exec(tableSQL); err != nil {
			...
			// detail in
			// https://reviewable.io/reviews/znbasedb/znbase/10251#-KVGGLbjhbPdlR6EFlfL
...
func TestSetUserPasswordInsecure(t *testing.T) {
	...
	testCases := []struct {
		...
		{"CREATE USER user2 WITH PASSWORD 'znbase'", errFail},
	...
	testCases = []struct {
		...
		{"ALTER USER $1 WITH PASSWORD $2", "role/user user3 does not exist"},
	}
