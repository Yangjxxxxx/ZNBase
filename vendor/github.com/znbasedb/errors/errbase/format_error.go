...
func FormatError(err error, s fmt.State, verb rune) {
	...
	// CHANGE (znbasedb/errors): FormatError() accepts `error` and
...
loop:
	for {
		// CHANGE (znbasedb/errors): we only emit the inter-error
		...
		case fmt.Formatter:
			...
			break loop
		default:
			// CHANGE (znbasedb/errors): if the error did not
		...
		// CHANGE (znbasedb/errors): if there was nothing printed
	...
	///////////////////////////////////////////////////////////////////////////////
	// znbasedb/errors: the remainder of the file is unchanged from the
