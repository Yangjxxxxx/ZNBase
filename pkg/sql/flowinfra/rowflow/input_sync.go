...
var _ runbase.RowSource = &orderedSynchronizer{}

// OutputTypes is part of the RowSource interface.
func (s *orderedSynchronizer) OutputTypes() []sqlbase.ColumnType {
	cts, _ := sqlbase.DatumTypesToColumnTypes(s.types)
	return cts
}
...
func (s *orderedSynchronizer) Less(i, j int) bool {
	...
	cts, _ := sqlbase.DatumTypesToColumnTypes(s.types)
	cmp, err := si.row.Compare(cts, &s.alloc, s.ordering, s.evalCtx, sj.row)
	if err != nil {
		s.err = err
		return false
	}
...
func (s *orderedSynchronizer) advanceRoot() error {
	...
	if src.row == nil {
		...
		cts, _ := sqlbase.DatumTypesToColumnTypes(s.types)
		if cmp, err := oldRow.Compare(cts, &s.alloc, s.ordering, s.evalCtx, src.row); err != nil {
			return err
		} else if cmp > 0 {
			return errors.Errorf(
				"incorrectly ordered stream %s after %s (ordering: %v)",
				src.row.String(cts), oldRow.String(cts), s.ordering,
			)
		}
	}
	// heap operations might set s.err (see Less)
...
func (s *orderedSynchronizer) ConsumerDone() {
	...
	if s.state != draining {
		s.consumerStatusChanged(draining, runbase.RowSource.ConsumerDone)
	}
}
...
func (s *orderedSynchronizer) ConsumerClosed() {
	...
	s.consumerStatusChanged(drainBuffered, runbase.RowSource.ConsumerClosed)
}

// consumerStatusChanged calls a RowSource method on all the non-exhausted
// sources.
func (s *orderedSynchronizer) consumerStatusChanged(
	newState orderedSynchronizerState, f func(runbase.RowSource),
) {
	if s.state == notInitialized {
...
// GetBatch
func (s *orderedSynchronizer) GetBatch(
...
func (s *orderedSynchronizer) SetChan() {}
...
func makeOrderedSync(
	ordering sqlbase.ColumnOrdering, evalCtx *tree.EvalContext, sources []runbase.RowSource,
	...
	s := &orderedSynchronizer{
		...
		types:    sqlbase.ColumnTypesToDatumTypes(sources[0].OutputTypes()),
