...
// TimeSeriesMaintenanceQueueConcurrency identifies replicas that contain time series
// data and performs necessary data maintenance on the time series located in
// the replica. We allow to adjust the parallelism of replicate queue process.
var TimeSeriesMaintenanceQueueConcurrency = settings.RegisterPositiveIntSetting(
	"kv.timeseries_maintenance.queue_concurrency",
	"timeSeriesMaintenanceQueue identifies replicas that contain time series "+
		"data and performs necessary data maintenance on the time series located in the replica. "+
...
func newTimeSeriesMaintenanceQueue(
	...
	q.baseQueue = newBaseQueue(
		...
		queueConfig{
			...
			maxConcurrency:       TimeSeriesMaintenanceQueueConcurrency.Get(&store.ClusterSettings().SV),
			needsLease:           true,
	...
	confChanged := func() {
		q.maxConcurrency = TimeSeriesMaintenanceQueueConcurrency.Get(&store.ClusterSettings().SV)
		q.confCh <- struct{}{}
	}
	TimeSeriesMaintenanceQueueConcurrency.SetOnChange(&store.ClusterSettings().SV, confChanged)
	return q
}
