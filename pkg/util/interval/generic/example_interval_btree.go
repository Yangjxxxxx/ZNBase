...
func (b keyBound) contains(a *example) bool {
	c := bytes.Compare(a.Key(), b.key)
	if c == 0 {
		return b.inc
	}
...
func upperBound(c *example) keyBound {
	if len(c.EndKey()) != 0 {
...
func (i *iterator) FirstOverlap(item *example) {
	i.reset()
	if i.n == nil {
		return
	}
...
func (i *iterator) NextOverlap(item *example) {
	if i.n == nil {
		return
	}
	i.pos++
...
func (i *iterator) findNextOverlap(item *example) {
	for {
		if i.pos > i.n.count {
			// Iterate up tree.
			i.ascend()
