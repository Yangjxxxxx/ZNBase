...
// GCQueueConcurrency manages a queue of replicas slated to be scanned in their
// entirety using the MVCC versions iterator.We allow to adjust the
// parallelism of merge queue process.
var GCQueueConcurrency = settings.RegisterPositiveIntSetting(
	"kv.gc.queue_concurrency",
	"gcQueue manages a queue of replicas slated to be "+
		"scanned in their entirety using the MVCC versions iterator. "+
...
func newGCQueue(store *Store, gossip *gossip.Gossip) *gcQueue {
	...
	gcq.baseQueue = newBaseQueue(
		...
		queueConfig{
			...
			maxConcurrency:       GCQueueConcurrency.Get(&store.ClusterSettings().SV),
			needsLease:           true,
	...
	confChanged := func() {
		gcq.maxConcurrency = GCQueueConcurrency.Get(&store.ClusterSettings().SV)
		gcq.confCh <- struct{}{}
	}
	GCQueueConcurrency.SetOnChange(&store.ClusterSettings().SV, confChanged)
	return gcq
}
...
func (gcq *gcQueue) process(ctx context.Context, repl *Replica, sysCfg *config.SystemConfig) error {
	...
	info, err := RunGC(ctx, desc, snap, now, *zone.GC, &replicaGCer{repl: repl},
		func(ctx context.Context, intents []roachpb.Intent) error {
			intentCount, err := repl.store.intentResolver.CleanupIntents(ctx, intents, now, roachpb.PUSH_ABORT, false)
			if err == nil {
		...
		func(ctx context.Context, txn *roachpb.Transaction) error {
			err := repl.store.intentResolver.CleanupTxnIntentsOnGCAsync(ctx, repl.RangeID, txn, now, func(pushed, succeeded bool) {
				if pushed {
...
type GCer interface {
	...
	GetSnapShots(context.Context) ([]Snapshot, error)
	GetFlashback(context.Context) (int32, error)
}
...
func RunGC(
	...
	var snapTime []hlc.Timestamp
	snapshots, err := gcer.GetSnapShots(ctx)
	if err != nil {
		log.Errorf(ctx, "unable to GetSnapShots %s", err)
	}
	for _, s := range snapshots {
		snapTime = append(snapTime, hlc.Timestamp{WallTime: s.Time})
	}
	TTLDaysSeconds, err := gcer.GetFlashback(ctx) //直接ttldays
	if err != nil {
		log.Errorf(ctx, "unable to GetFlashback %s", err)
	}
	...
	gc := engine.MakeGarbageCollector(now, policy, TTLDaysSeconds)
	...
	snapTime = append(snapTime, gc.Threshold)
	...
	processKeysAndValues := func() {
		...
		if len(keys) > 1 {
			...
			if err := protoutil.Unmarshal(vals[0], meta); err != nil {
				...
				endIdx := len(keys)
				from := hlc.Timestamp{}
				for i, st := range snapTime {
					var idx int
					var gcTS hlc.Timestamp
					//*/
					var gcDeleted bool
					if i == 0 || len(snapTime) == 1 {
						//1.第一个快照点的delete数据可以删除
						//2.没有快照的delete数据可以删除
						gcDeleted = true
					} else {
						//其他快照：临近快照点的delete数据不能GC，否则会访问到更旧的数据
						gcDeleted = false
					}
					var deleted bool
					if idx, gcTS, deleted = gc.FilterOfSnap(keys[startIdx:endIdx], vals[startIdx:endIdx], st, gcDeleted); gcTS != (hlc.Timestamp{}) {
						...
						for i := endIdx - 1; i >= startIdx+idx; i-- {
							...
							if batchGCKeysBytes >= gcKeyVersionChunkBytes {
								batchGCKeys = append(batchGCKeys, roachpb.GCRequest_GCKey{Key: expBaseKey, From: from, Timestamp: keys[i].Timestamp})
								from = keys[i].Timestamp.Next()
						...
						if batchGCKeysBytes != 0 {
							batchGCKeys = append(batchGCKeys, roachpb.GCRequest_GCKey{Key: expBaseKey, From: from, Timestamp: gcTS})
							//from = gcTS.Next()
							//batchGCKeysBytes = 0
						}
