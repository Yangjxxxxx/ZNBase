...
SELECT string_agg(source_id::TEXT || ':' || target_id::TEXT, ',')
  FROM (SELECT * FROM zbdb_internal.gossip_network ORDER BY source_id, target_id)
`
...
func runGossipRestartNodeOne(ctx context.Context, t *test, c *cluster) {
	args := startArgs("--env=ZNBASE_SCAN_MAX_IDLE_TIME=5ms", "--encrypt=false")
	c.Put(ctx, znbase, "./znbase")
	...
	if err := retry.ForDuration(30*time.Second, func() error {
		const query = `SELECT count(*) FROM zbdb_internal.gossip_nodes`
		var count int
...
`
	...
	err := c.RunE(ctx, c.Node(1),
		`./znbase start --insecure --background --store={store-dir} `+
	...
	// current infrastructure which doesn't know about znbase nodes started on
	// non-standard ports.
	g := newGossipUtil(ctx, c)
...
func runCheckLocalityIPAddress(ctx context.Context, t *test, c *cluster) {
	...
	for i := 1; i <= c.nodes; i++ {
		...
		rows, err := db.Query(
			`SELECT node_id, advertise_address FROM zbdb_internal.gossip_nodes`,
		)
		if err != nil {
			t.Fatal(err)
		}
