...
//
// - components in ZNBaseDB that wish to have their allocations tracked
...
//
//
// In ZNBaseDB this is integrated as follows:
//
...
// other ZNBaseDB components. The monitor must be set up via Start/Stop
...
//BytesMonitor an object that can track and limit memory/disk
...
var maxAllocatedButUnusedBlocks = envutil.EnvOrDefaultInt("ZNBASE_MAX_ALLOCATED_UNUSED_BLOCKS", 10)

// DefaultPoolAllocationSize specifies the unit of allocation used by a monitor
// to reserve and release bytes to a pool.
var DefaultPoolAllocationSize = envutil.EnvOrDefaultInt64("ZNBASE_ALLOCATION_CHUNK_SIZE", 10*1024)
...
//SetUdrState 处理udr失败，将mu的状态置成udr，防止后面close的时候panic
func (mm *BytesMonitor) SetUdrState() {
	mm.mu.state = udrmu
}
...
// Allocated returns the number of bytes currently Allocated through this account.
func (b BoundAccount) Allocated() int64 {
	return b.used + b.reserved
}

// SetUsed set used to the given number
func (b *BoundAccount) SetUsed(used int64) {
	b.used = used
}

// SetReserved set reserved to the given number
func (b *BoundAccount) SetReserved(reserved int64) {
	b.reserved = reserved
}

// SetCurAlocated set used to the given number
func (b *BoundAccount) SetCurAlocated(curAllocated int64) {
	b.mon.mu.curAllocated = curAllocated
}
