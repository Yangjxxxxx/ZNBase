...
export CLOUDSDK_CORE_PROJECT=${CLOUDSDK_CORE_PROJECT-${GCEWORKER_PROJECT-znbase-workers}}
...
case "${cmd}" in
    ...
    ;;
    create)
    if [[ "$ZNBASE_DEV_LICENSE" ]]; then
      echo "Using dev license key from \$ZNBASE_DEV_LICENSE"
    else
      echo -n "Enter your dev license key (if any): "
      read ZNBASE_DEV_LICENSE
    fi
    ...
    if [[ "$ZNBASE_DEV_LICENSE" ]]; then
      gcloud compute ssh "${NAME}" --command="echo ZNBASE_DEV_LICENSE=$ZNBASE_DEV_LICENSE >> ~/.bashrc_bootstrap"
    fi
    ...
    ;;
    scp)
    # Example: $0 scp gceworker-youruser:go/src/github.com/znbasedb/znbase/znbase-data/logs gcelogs --recurse
    ...
    if (( $# == 0 )); then
      host=.  # Sync the ZNBase repo by default.
      worker=go/src/github.com/znbasedb/znbase
    elif (( $# == 2 )); then
      host=$1
      worker=$2
    else
