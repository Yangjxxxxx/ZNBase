...
interface DatabaseSummaryConnectedData {
  ...
  dbResponse: protos.znbase.server.serverpb.DatabaseDetailsResponse;
  grants: protos.znbase.server.serverpb.DatabaseDetailsResponse.Grant[];
}
...
export class DatabaseSummaryBase extends React.Component<DatabaseSummaryProps, {}> {
    ...
    if (props.tableInfos && props.tableInfos.length > 0) {
      _.each(props.tableInfos, (tblInfo) => {
        if (_.isUndefined(tblInfo.numColumns)) {
          props.refreshTableDetails(new protos.znbase.server.serverpb.TableDetailsRequest({
            database: props.name,
            table: tblInfo.name,
          }));
        }
        if (_.isUndefined(tblInfo.physicalSize)) {
          props.refreshTableStats(new protos.znbase.server.serverpb.TableStatsRequest({
            database: props.name,
            table: tblInfo.name,
          }));
        }
  ...
  componentWillMount() {
    this.props.refreshDatabaseDetails(new protos.znbase.server.serverpb.DatabaseDetailsRequest({ database: this.props.name }));
