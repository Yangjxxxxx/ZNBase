...
type Event = protos.znbase.server.serverpb.EventsResponse.Event;

function makeEventBox(
  events: protos.znbase.server.serverpb.EventsResponse.IEvent[],
...
describe("<EventBox>", function() {
  ...
  describe("attach", function() {
    it("attaches event data to contained component", function () {
      const eventsResponse = new protos.znbase.server.serverpb.EventsResponse({
        events: [
          {
...
describe("<EventRow>", function () {
  describe("attach", function () {
    it("correctly renders a known event", function () {
      const e = new protos.znbase.server.serverpb.EventsResponse.Event({
        target_id: Long.fromNumber(1),
...
describe("getEventInfo", function () {
  it("covers every currently known event", function () {
    _.each(allEvents, (eventType) => {
      const event = new protos.znbase.server.serverpb.EventsResponse.Event({ event_type: eventType });
