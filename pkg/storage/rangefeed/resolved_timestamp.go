...
func (rts *resolvedTimestamp) ConsumeLogicalOp(op enginepb.MVICLogicalOp) bool {
...
func (rts *resolvedTimestamp) consumeLogicalOp(op enginepb.MVICLogicalOp) bool {
	...
	case *enginepb.MVCCWriteIntentOp:
		rts.assertOpAboveRTS(op, t.Timestamp)
		return rts.intentQ.IncRef(t.TxnID, t.TxnKey, t.TxnMinTimestamp, t.Timestamp, t.BackFill)

	case *enginepb.MVCCUpdateIntentOp:
		return rts.intentQ.UpdateTS(t.TxnID, t.Timestamp, t.BackFill)

	case *enginepb.MVCCCommitIntentOp:
		return rts.intentQ.DecrRef(t.TxnID, t.Timestamp, t.BackFill)
	...
	case *enginepb.MVCCAbortIntentOp:
		...
		return rts.intentQ.DecrRef(t.TxnID, hlc.Timestamp{}, t.BackFill)
...
func (rts *resolvedTimestamp) assertOpAboveRTS(op enginepb.MVICLogicalOp, opTS hlc.Timestamp) {
...
func (uiq *unresolvedIntentQueue) IncRef(
	txnID uuid.UUID, txnKey roachpb.Key, txnMinTS, ts hlc.Timestamp, backFill bool,
) bool {
	return uiq.updateTxn(txnID, txnKey, txnMinTS, ts, +1, backFill)
}

// DecrRef decrements the reference count of the specified transaction. It
// returns whether the update advanced the timestamp of the oldest transaction
// in the queue.
func (uiq *unresolvedIntentQueue) DecrRef(txnID uuid.UUID, ts hlc.Timestamp, backFill bool) bool {
	return uiq.updateTxn(txnID, nil, hlc.Timestamp{}, ts, -1, backFill)
}
...
func (uiq *unresolvedIntentQueue) UpdateTS(txnID uuid.UUID, ts hlc.Timestamp, backFill bool) bool {
	return uiq.updateTxn(txnID, nil, hlc.Timestamp{}, ts, 0, backFill)
}

func (uiq *unresolvedIntentQueue) updateTxn(
	txnID uuid.UUID, txnKey roachpb.Key, txnMinTS, ts hlc.Timestamp, delta int, backFill bool,
) bool {
