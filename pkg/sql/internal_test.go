...
func TestInternalExecutor(t *testing.T) {
	...
	row, err = ie.QueryRow(
		ctx, "test", nil, /* txn */
		"select case nextval('test.seq') when 1 then zbdb_internal.force_retry('1s') else 99 end",
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = s.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		row, err = ie.QueryRow(
			ctx, "test", txn,
			"select case nextval('test.seq') when 2 then zbdb_internal.force_retry('180s') else 99 end",
		)
		if cnt > 99 {
			return nil
		}
...
func testInternalExecutorAppNameInitialization(
	...
	if rows, err := ie.Query(context.TODO(), "find-query", nil,
		"SELECT application_name FROM zbdb_internal.node_queries WHERE query LIKE '%337' || '666%'"); err != nil {
		t.Fatal(err)
	} else if len(rows) != 1 {
	...
	if rows, err := ie.Query(context.TODO(), "find-query", nil,
		"SELECT application_name FROM zbdb_internal.node_statement_statistics WHERE key LIKE 'SELECT' || ' pg_sleep(%'"); err != nil {
		t.Fatal(err)
	} else if len(rows) != 1 {
		t.Fatalf("expected 1 query, got: %+v", rows)
