...
//MemAccClose only cut memAcc.mu.curAllocated
func (c *RowContainer) MemAccClose(ctx context.Context) {
	c.memAcc.Close(ctx)
}
...
// MonitorCurAllocated returns the BytesMonitor to which this account is bound.
func (c *RowContainer) MonitorCurAllocated() int64 {
	return c.memAcc.Monitor().AllocBytes()
}

// Allocated returns the number of bytes currently Allocated through this account.
func (c *RowContainer) Allocated() int64 {
	return c.memAcc.Allocated()
}

// SetUsed set used to the given number
func (c *RowContainer) SetUsed(used int64) {
	c.memAcc.SetUsed(used)
}

// SetReserved set reserved to the given number
func (c *RowContainer) SetReserved(reserved int64) {
	c.memAcc.SetReserved(reserved)
}

// SetCurAlocated set used to the given number
func (c *RowContainer) SetCurAlocated(curAllocated int64) {
	c.memAcc.SetCurAlocated(curAllocated)
}
