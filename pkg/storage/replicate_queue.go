...
// ReplicateQueueConcurrency manages a queue of replicas which may need to add an
// additional replica to their range.We allow to adjust the parallelism
// of replicate queue process.
var ReplicateQueueConcurrency = settings.RegisterPositiveIntSetting(
	"kv.replicate.queue_concurrency",
	"replicateQueue manages a queue of replicas which may need to "+
		"add an additional replica to their range. "+
...
func newReplicateQueue(store *Store, g *gossip.Gossip, allocator Allocator) *replicateQueue {
	...
	rq.baseQueue = newBaseQueue(
		...
		queueConfig{
			...
			maxConcurrency:       ReplicateQueueConcurrency.Get(&store.ClusterSettings().SV),
			needsLease:           true,
	...
	confChanged := func() {
		rq.maxConcurrency = ReplicateQueueConcurrency.Get(&store.ClusterSettings().SV)
		rq.confCh <- struct{}{}
	}
	ReplicateQueueConcurrency.SetOnChange(&store.ClusterSettings().SV, confChanged)
	updateFn := func() {
		select {
...
func (rq *replicateQueue) shouldQueue(
	...
	ctx = setLocateSpace(ctx, repl.GetLocateSpace())
...
func (rq *replicateQueue) process(
	...
	for r := retry.StartWithCtx(ctx, retryOpts); r.Next(); {
		if requeue, err := rq.processOneChange(ctx, repl, sysCfg, rq.canTransferLease, false /* dryRun */, false); err != nil {
			...
			// EnqueueTxn this replica again to see if there are more changes to be made.
...
func (rq *replicateQueue) processOneChange(
	...
	case AllocatorAdd:
		...
		enableClusterNodes := rq.allocator.storePool.ClusterEnableNodeCount()
		need := GetNeededReplicas(*zone.NumReplicas, enableClusterNodes)
...
func (rq *replicateQueue) addReplica(
	...
	if _, err := repl.changeReplicas(ctx, roachpb.ADD_REPLICA, target, desc, priority, reason, details, backFill); err != nil {
		return err
	}
	rangeInfo := rangeInfoForRepl(repl, desc)
...
func (rq *replicateQueue) removeReplica(
	...
	if _, err := repl.ChangeReplicas(ctx, roachpb.REMOVE_REPLICA, target, desc, reason, details, false); err != nil {
		return err
	}
	rangeInfo := rangeInfoForRepl(repl, desc)
