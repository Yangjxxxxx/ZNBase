...
var _ runbase.LocalProcessor = &planNodeToRowSource{}

// InitWithOutput implements the LocalProcessor interface.
func (p *planNodeToRowSource) InitWithOutput(
	post *distsqlpb.PostProcessSpec, output runbase.RowReceiver,
) error {
...
// metadata.
func (p *planNodeToRowSource) SetInput(ctx context.Context, input runbase.RowSource) error {
...
func (p *planNodeToRowSource) GetBatch(
...
func (p *planNodeToRowSource) SetChan() {}
...
func (p *planNodeToRowSource) Next() (sqlbase.EncDatumRow, *distsqlpb.ProducerMetadata) {
	var tempData tree.Datums
	if p.State == runbase.StateRunning && p.fastPath {
		tempData = make(tree.Datums, 0)
		var count int
	...
	for p.State == runbase.StateRunning {
		valid, err := p.node.Next(p.params)
		if err != nil || !valid {
			if _, ok := p.node.(*bufferNode); ok {
				//optbuilder.AllData = []interface{}{}
				isUnionClauseFitstJoinDone = true
			}
			p.MoveToDraining(err)
			return nil, p.DrainHelper()
		}
		...
		if _, ok := p.node.(*bufferNode); ok {
			for i, datum := range p.node.Values() {
				if datum != nil {
					...
					tempData = append(tempData, p.row[i].Datum)

				}
			}
			//optbuilder.AllData = append(optbuilder.AllData, tempData)
			//tempData = []tree.Datum{}
		} else {
			for i, datum := range p.node.Values() {
				if datum != nil {
		...
		if outRow := p.ProcessRowHelper(p.row); outRow != nil {
			...
			if node, ok := p.node.(*ordinalityNode); ok && p.EvalCtx.SessionData.Rownum.IsRownum {
				node.run.curCnt--
			}
		}
	}
