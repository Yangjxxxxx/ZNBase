...
type aggregatorBase struct {
	...
	isScalar         bool
	groupCols        runbase.Columns
	orderedGroupCols runbase.Columns
...
func (ag *aggregatorBase) init(
	...
	memMonitor := runbase.NewMonitor(ctx, flowCtx.EvalCtx.Mon, "aggregator-mem")
...
func (as *AggregatorStats) StatsForQueryPlan() []string {
	return append(
		...
		fmt.Sprintf("%s: %t", "isParallel", as.IsParallel),
		fmt.Sprintf("%s: %d", "ParallelNum", as.ParallelNum),
	)
}
		...
		if ag.InternalOpt {
			tracing.SetSpanStats(
				...
				&AggregatorStats{
					...
					IsParallel:      true,
					ParallelNum:     int32(ag.parallelNum),
				},
			)
		} else {
			tracing.SetSpanStats(
...
func (ag *hashAggregator) SetChan() {
}
...
type orderedAggregator struct {
	...
	agrows             []sqlbase.EncDatumRow
	AgStructChannel    chan AgStruct
	agDuplicateChannel chan []sqlbase.EncDatumRow
	agi                int
	index              int
	newrows            AgStruct
}

var _ runbase.Processor = &hashAggregator{}
var _ runbase.RowSource = &hashAggregator{}

var _ runbase.Processor = &parallehashAggregator{}
var _ runbase.RowSource = &parallehashAggregator{}

const hashAggregatorProcName = "hash aggregator"

var _ runbase.Processor = &orderedAggregator{}
var _ runbase.RowSource = &orderedAggregator{}
...
func newAggregator(
	...
	if len(spec.GroupCols) == 0 &&
		...
		if tr, ok := input.(*tableReader); ok && tr.InternalOpt {
			if tr.InternalOpt && tr.Out.GetFilter() == nil {
				tr.switchCount = true
...
func (ag *hashAggregator) GetBatch(
...
func (ag *hashAggregator) start(ctx context.Context, procName string) context.Context {
	ag.input.Start(ctx)
	ctx = ag.StartInternal(ctx, procName)
	ag.cancelChecker = sqlbase.NewCancelChecker(ctx)
	ag.runningState = aggAccumulating
...
func (ag *hashAggregator) closeOpt() {
	if ag.acc != nil {
		ag.acc.Close(ag.Ctx)
	}
...
func (ag *orderedAggregator) Next() (sqlbase.EncDatumRow, *distsqlpb.ProducerMetadata) {
	...
	for {
		if ag.newrows.row != nil {
			for ag.index < ag.newrows.index {
				outRow := ag.newrows.row[ag.index]
				ag.index++
				if outRow != nil {
					return outRow, nil
				}
			...
			ag.index = 0
			ag.newrows.index = 0
			if len(ag.agDuplicateChannel) < 90 {
				ag.agDuplicateChannel <- ag.newrows.row
			}
			ag.newrows.row = nil
		} else {
			var ok bool
			ag.newrows, ok = <-ag.AgStructChannel
			if ok {
				if ag.newrows.meta != nil {
					if ag.newrows.meta.Err != nil {
						ag.MoveToDraining(nil /* err */)
					}
					return nil, ag.newrows.meta
				}
				continue
			} else {
				if ag.newrows.row == nil && !ok {
					ag.MoveToDraining(nil /* err */)
					for len(ag.agDuplicateChannel) > 1 {
						_ = <-ag.agDuplicateChannel
					}
					_, ok := <-ag.agDuplicateChannel
					if ok {
						close(ag.agDuplicateChannel)
					}
					break
				}
			}
...
type aggregateFuncHolder struct {
	create func(*tree.EvalContext, tree.Datums, *mon.BoundAccount) tree.AggregateFunc
...
func (ag *aggregatorBase) createAggregateFuncs() (aggregateFuncs, error) {
	...
	for i, f := range ag.funcs {
		agg := f.create(ag.FlowCtx.EvalCtx, f.arguments, ag.acc)
