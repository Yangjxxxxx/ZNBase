...
/* elog file related */
#define LOGFILE_SIZE 1024*1024*32 // 32MB
typedef FILE ELOG_File;
...
#define MAKE_SQLSTATE_WITHCHARS(chs) \
	(PGSIXBIT(chs[0]) + (PGSIXBIT(chs[1]) << 6) + (PGSIXBIT(chs[2]) << 12) + \
	 (PGSIXBIT(chs[3]) << 18) + (PGSIXBIT(chs[4]) << 24))
...
extern bool elogfile_create(const char *elog_path);
extern void elog_output(int elevel, const char *filename, int lineno, const char *funcname, const char *fmt, ...);
...
#define elog(elevel, ...) \
    do {\
        elog_output(elevel, __FILE__, __LINE__, PG_FUNCNAME_MACRO, __VA_ARGS__); \
    } while (0)
#if 0
/*----------
...
#define elog(elevel, ...)  \
	do { \
		pg_prevent_errno_in_scope(); \
		elogdata = elog_start(__FILE__, __LINE__, PG_FUNCNAME_MACRO); \
...
#define PG_TRY()  \
	do { \
		...
		sigjmp_buf *save_exception_stack = plsql_data->plsql_sigjum_buf[plsql_data->index]; \
		ErrorContextCallback *save_context_stack = error_context_stack; \
		sigjmp_buf local_sigjmp_buf; \
		if (sigsetjmp(local_sigjmp_buf, 0) == 0) \
		{ \
			plsql_data->plsql_sigjum_buf[plsql_data->index] = &local_sigjmp_buf

#define PG_CATCH()	\
		} \
		else \
		{ \
			plsql_data->plsql_sigjum_buf[plsql_data->index] = save_exception_stack; \
...
typedef struct ErrorData
{
    ...
    int         message_len;    /* error message length */
...
extern char* EmitErrorReport2ZNBase(void);
extern char* FetchErrorMessage(void);
...
extern bool* ifFatalorPanicStar;
extern sigjmp_buf*  local_sigjmp_buf_star;
extern void (*FatalMessageBuildStar) (void);
