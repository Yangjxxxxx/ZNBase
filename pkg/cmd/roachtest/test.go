...
import (
	...
	"github.com/znbasedb/znbase/pkg/cmd/internal/issues"
...
type testSpec struct {
	...
	// MinVersion indicates the minimum znbase version that is required for
	// the test to be run. If MinVersion is less than the version specified
	// --znbase-version, Skip will be populated causing the test to be
	// skipped.
	MinVersion string
...
// depending on the znbase version it is running against. Note that the
// versions are ZNBase build tag version numbers, not the internal cluster
// version number.
...
func getAuthorEmail(file string, line int) string {
	const repo = "github.com/znbasedb/znbase/"
	...
	// author-mail <somebody@znbaselabs.com>
	out, err := cmd.CombinedOutput()
	if err != nil {
		return ""
	}
