...
func NewInitialDatabaseDescriptor(id sqlbase.ID, name string, owner string) *DatabaseDescriptor {
	return NewInitialDatabaseDescriptorWithPrivileges(
		id, name, sqlbase.NewDefaultObjectPrivilegeDescriptor(privilege.Database, owner))
}
...
func NewInitialDatabaseDescriptorWithPrivileges(
	id sqlbase.ID, name string, privileges *sqlbase.PrivilegeDescriptor,
) *DatabaseDescriptor {
...
/*func makeDatabaseDesc(p *tree.CreateDatabase) sqlbase.DatabaseDescriptor {
	desc := sqlbase.DatabaseDescriptor{
		Name:       string(p.Name),
		Privileges: sqlbase.NewDefaultDataBasePrivilegeDescriptor(),
	}
	// todo(sh): cancel grant usage privilege on database for public role.
...
func (p *planner) renameDatabase(
	...
	if err := p.txn.Run(ctx, b); err != nil {
		if _, ok := err.(*roachpb.ConditionFailedError); ok {
			return pgerror.NewErrorf(pgcode.DuplicateDatabase,
