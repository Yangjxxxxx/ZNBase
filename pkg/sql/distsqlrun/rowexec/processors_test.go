...
func TestPostProcess(t *testing.T) {
	...
	for tcIdx, tc := range testCases {
		t.Run(strconv.Itoa(tcIdx), func(t *testing.T) {
			inBuf := runbase.NewRowBuffer(sqlbase.ThreeIntCols, input, runbase.RowBufferArgs{})
			outBuf := &runbase.RowBuffer{}

			var out runbase.ProcOutputHelper
...
func TestProcessorBaseContext(t *testing.T) {
	...
	runTest := func(t *testing.T, f func(noop *noopProcessor)) {
		...
		input := runbase.NewRepeatableRowSource(sqlbase.ColumnTypesToDatumTypes(sqlbase.OneIntCol), sqlbase.MakeIntRows(10, 1))
		noop, err := newNoopProcessor(flowCtx, 0 /* processorID */, input, &distsqlpb.PostProcessSpec{}, &runbase.RowDisposer{})
		if err != nil {
			t.Fatal(err)
		}
		noop.Start(ctx)
