...
TEST(Libroach, DBSSTPebbleRocksdb) {


  DBSstFileWriter* fw = DBSstFileWriterNew(false);
  DBStatus status = DBSstFileWriterOpen(fw);
  if(status.data != NULL){
    std::cout << status.data <<std::endl;
  }

  char ckey[] = {-20,-119,-119};
  char cValue[] = {0,0,0,0,3,87,64,52,118,96,40,97,101,44,103,42,87,76,69};


//  DBKey key = ToDBKey("foo");
//  DBSlice value = ToDBSlice("foo's value");
  DBKey key = ToDBKey(ckey, 3, 73, 573868525);
  DBSlice value = DBSlice{cValue, 19};

  DBSstFileWriterAdd(fw, key, value);
  DBString sst;
  DBSstFileWriterFinish(fw, &sst);
  DBSstFileWriterClose(fw);
//  std::cout << sst.data << std::endl;
//
//  std::ofstream outfile("/data/sst/rocksdb-roach.sst", std::ios::app | std::ios::binary);
//  outfile.write(sst.data, sst.len);
//  outfile.close();
