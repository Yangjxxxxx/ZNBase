...
import (
	...
	"github.com/znbasedb/znbase/pkg/util/search"
	...
	func() {
		...
		if opts.ZFS {
			if err := c.RunE(ctx, c.Node(1), "test -d /mnt/data1/.zfs/snapshot/pristine"); err != nil {
				...
				// used in a repro.
				c.Reformat(ctx, znbaseNodes, "zfs")
				...
				c.Stop(ctx, znbaseNodes)

				c.Run(ctx, znbaseNodes, "test -e /sbin/zfs && sudo zfs snapshot data1@pristine")
			}
			t.Status(`restoring store dumps`)
			c.Run(ctx, znbaseNodes, "sudo zfs rollback data1@pristine")
			c.Start(ctx, t, znbaseNodes)
		} else {
			c.Start(ctx, t, znbaseNodes)
			c.Run(ctx, workloadNode, tpccFixturesCmd(t, cloud, opts.Warehouses, true /* checks */))
...
type tpccBenchSpec struct {
	...
	// change (i.e. ZNBaseDB gets faster!).
	EstimatedMax int
...
// tpccbench is a suite of benchmarking tools that run TPC-C against ZNBaseDB
...
// expresses how well ZNBaseDB performance scales.
//
...
func runTPCCBench(ctx context.Context, t *test, c *cluster, b tpccBenchSpec) {
	...
	// ZNBase nodes and a single load generator.
	...
	if useHAProxy {
		...
		c.Put(ctx, znbase, "./znbase", loadNodes)
		c.Run(ctx, loadNodes, "./znbase gen haproxy --insecure --url {pgurl:1}")
...
func registerTPCCBench(r *registry) {
	specs := []tpccBenchSpec{
		...
		// See https://github.com/znbasedb/znbase/issues/31409 for the next three specs.
		{
			Nodes: 6,
			CPUs:  16,
