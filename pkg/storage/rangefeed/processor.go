...
func newErrBufferCapacityExceeded() *roachpb.Error {
	return roachpb.NewError(
		roachpb.NewRangeFeedRetryError(roachpb.RangeFeedRetryError_REASON_SLOW_CONSUMER, nil),
	)
}
...
type Processor struct {
	...
	FilterEvents []BackfillEvent
}

//BackfillEvent 触发回填的事件
		...
		for {
			...
			case r := <-p.regC:
				...
				runOutputLoop := func(ctx context.Context) {
					...
					if p.FilterEvents != nil && len(p.FilterEvents) != 0 {
						filter = p.FilterEvents[0].Ts
					}
					r.runOutputLoop(ctx, filter)
					select {
					case p.unregC <- &r:
					case <-p.stoppedC:
					}
			...
			case <-txnPushTickerC:
				...
				if len(oldTxns) > 0 {
					...
					pushTxns := newTxnPushAttempt(p, toPush, now, txnPushAttemptC, backFill)
	...
	for _, op := range ops {
		...
		case *enginepb.MVCCWriteValueOp:
			...
			if !t.BackFill {
				...
				_, tableID, _ := keys.DecodeTablePrefix(t.Key)
				if tableID == keys.DescriptorTableID {
					p.publishValue(ctx, t.Key, t.Timestamp, t.Value, t.PrevValue)
				}
				p.FilterEvents = make([]BackfillEvent, 1)
				p.FilterEvents[0] = BackfillEvent{
					Ts:       t.Timestamp,
					Backfill: t.BackFill,
					TxnID:    t.TxnID,
				}
