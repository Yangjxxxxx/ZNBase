...
func init() {
	if runtime.GOOS == "windows" {
		...
		skipPermissionChecks = envutil.EnvOrDefaultBool("ZNBASE_SKIP_KEY_PERMISSION_CHECK", false)
	}
}
...
func parseCertificate(ci *CertInfo) error {
	...
	for i, c := range derCerts {
		...
		if i == 0 {
			...
			if err := validateZNBaseCertificate(ci, x509Cert); err != nil {
...
// validateZNBaseCertificate takes a CertInfo and a parsed certificate and checks the
// values of certain fields.
func validateZNBaseCertificate(ci *CertInfo, cert *x509.Certificate) error {
