...
import (
	...
	cld "github.com/znbasedb/znbase/pkg/cmd/roachprod/cloud"
	"github.com/znbasedb/znbase/pkg/cmd/roachprod/config"
	"github.com/znbasedb/znbase/pkg/cmd/roachprod/install"
	"github.com/znbasedb/znbase/pkg/cmd/roachprod/ssh"
	"github.com/znbasedb/znbase/pkg/cmd/roachprod/ui"
	"github.com/znbasedb/znbase/pkg/cmd/roachprod/vm"
	_ "github.com/znbasedb/znbase/pkg/cmd/roachprod/vm/aws"
	"github.com/znbasedb/znbase/pkg/cmd/roachprod/vm/gce"
	"github.com/znbasedb/znbase/pkg/cmd/roachprod/vm/local"
	"github.com/znbasedb/znbase/pkg/util/flagutil"
...
Examples:
  ...
  roachprod sql local:2 -- -e "select * from zbdb_internal.node_runtime_info"
  roachprod stop local
  roachprod wipe local
  roachprod destroy local

The above commands will create a "local" 3 node cluster, start a znbase
...
var (
	...
	clusterType    = "znbase"
	secure         = false
	nodeEnv        = "ZNBASE_ENABLE_RPC_COMPRESSION=false"
	nodeArgs       []string
	tag            string
	external       = false
	adminurlOpen   = false
...
starting znbase nodes and "roachprod {run,ssh}" for running arbitrary
...
The --details flag adjusts the output format to include per-node details:
  ...
  marc-test: [aws gce] 5h33m57s remaining
    marc-test-0001	marc-test-0001.us-east1-b.znbase-ephemeral	10.142.0.18	35.229.60.91
    marc-test-0002	marc-test-0002.us-east1-b.znbase-ephemeral	10.142.0.17	35.231.0.44
    marc-test-0003	marc-test-0003.us-east1-b.znbase-ephemeral	10.142.0.19	35.229.111.100
    marc-test-0004	marc-test-0004.us-east1-b.znbase-ephemeral	10.142.0.20	35.231.102.125
  Syncing...
...
and others in insecure mode is not a supported ZNBase configuration.
...
znbase software is installed by default on a newly created cluster.

The --args and --env flags can be used to pass arbitrary command line flags and
environment variables to the znbase process.
` + tagHelp + `
...
If the ZNBASE_DEV_LICENSE environment variable is set the enterprise.license
...
shutdown znbase. The --wait flag causes stop to loop waiting for all
...
var monitorCmd = &cobra.Command{
	...
	Long: `Monitor the status of znbase nodes in a cluster.
...
directory of a previous test. For example:
	...
	roachprod test denim kv_0.znbase-6151ae1

will restart the kv_0 test on denim using the znbase binary with the build
tag 6151ae1.`,
...
var stageCmd = &cobra.Command{
	...
	Short: "stage znbase binaries",
	Long: `Stages release and edge binaries to the cluster.

Currently available application options are:
  znbase - ZNBase Unofficial. Can provide an optional SHA, otherwise
              latest build version is used.
  workload  - ZNBase workload application.
  release   - Official ZNBaseDB Release. Must provide a specific release
              version.

Some examples of usage:
  -- stage edge build of znbase build at a specific SHA:
  roachprod stage my-cluster znbase e90e6903fee7dd0f88e20e345c2ddfe1af1e5a97

  -- Stage the most recent edge build of the workload tool:
  roachprod stage my-cluster workload

  -- Stage the official release binary of ZNBaseDB at version 2.0.5
	...
	Run: wrap(func(cmd *cobra.Command, args []string) error {
		...
		case "znbase":
			return install.StageRemoteBinary(
				c, applicationName, "znbase/znbase.linux-gnu-amd64", versionArg,
			)
		case "workload":
			return install.StageRemoteBinary(
				c, applicationName, "znbase/workload", versionArg,
			)
		case "release":
			return install.StageZNBaseRelease(c, versionArg)
		default:
...
var sqlCmd = &cobra.Command{
	...
	Short: "run `znbase sql` on a remote cluster",
	Long:  "Run `znbase sql` on a remote cluster.\n",
	Args:  cobra.MinimumNArgs(1),
	Run: wrap(func(cmd *cobra.Command, args []string) error {
		c, err := newCluster(args[0], false /* reserveLoadGen */)
		if err != nil {
		...
		znbase, ok := c.Impl.(install.ZNBase)
		if !ok {
			return errors.New("sql is only valid on znbase clusters")
		}
		return znbase.SQL(c, args[1:])
	}),
}
...
func main() {
	...
	for _, cmd := range []*cobra.Command{
		...
		case startCmd, testCmd:
			...
			cmd.Flags().StringVarP(
				&clusterType, "type", "t", clusterType, `cluster type ("znbase" or "cassandra")`)
			cmd.Flags().BoolVar(
		...
		case sqlCmd:
			cmd.Flags().StringVarP(
				...
				"the remote znbase binary to use")
			fallthrough
