...
import (
	...
	"github.com/znbasedb/circuitbreaker"
...
func makeTestConfig(st *cluster.Settings) Config {
	...
	cfg.CursorTempStorageConfig = base.DefaultTestTempStorageConfig(st)
...
func makeTestConfigFromParams(params base.TestServerArgs) Config {
	...
	cfg.LocationName = params.LocationName
...
// A TestServer encapsulates an in-memory instantiation of a znbase node with
...
type TestServer struct {
	Cfg *Config
	// server is the embedded ZNBase server struct.
	*Server
...
// GetStatus is that get TestServer's status
func (ts *TestServer) GetStatus() serverpb.StatusServer {
	return ts.Server.status
}
...
func (ts *TestServer) Start(params base.TestServerArgs) error {
	...
	ts.Server.mailServer.TestConfig()
	ts.Server.auditServer.TestConfig()
	if err != nil {
		return err
	}
...
func ExpectedInitialRangeCount(db *client.DB) (int, error) {
	...
	schemaRangeCount := int(2)
	// `n` splits create `n+1` ranges.
	return len(config.StaticSplits()) + systemTableSplits + schemaRangeCount + 1, nil
}
...
// MailServer returns mail server shared by all service
func (ts *TestServer) MailServer() *mail.Server {
	return ts.mailServer
}
...
func (ts *TestServer) DistSender() interface{} {
