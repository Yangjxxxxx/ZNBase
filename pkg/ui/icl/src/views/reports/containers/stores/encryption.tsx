...
export default class EncryptionStatus {
  ...
  renderStoreKey(key: protos.znbase.icl.storageicl.engineicl.enginepbicl.IKeyInfo) {
    // Get the enum name from its value (eg: "AES128_CTR" for 1).
    const encryptionType = protos.znbase.icl.storageicl.engineicl.enginepbicl.EncryptionType[key.encryption_type];
  ...
  renderDataKey(key: protos.znbase.icl.storageicl.engineicl.enginepbicl.IKeyInfo) {
    // Get the enum name from its value (eg: "AES128_CTR" for 1).
    const encryptionType = protos.znbase.icl.storageicl.engineicl.enginepbicl.EncryptionType[key.encryption_type];
  ...
  renderFileStats(stats: protos.znbase.server.serverpb.IStoreDetails) {
  ...
  getEncryptionRows() {
    ...
    try {
      decodedStatus = protos.znbase.icl.storageicl.engineicl.enginepbicl.EncryptionStatus.decode(rawStatus);
    } catch (e) {
