...
func (b *logicalPropsBuilder) buildProjectProps(prj *ProjectExpr, rel *props.Relational) {
	...
	for i := range prj.Projections {
		...
		} else if originVar, IsManualCast := IsManualCastVariable(item.Element); IsManualCast {
			if inputProps.NotNullCols.Contains(int(originVar.Col)) {
				rel.NotNullCols.Add(int((item.Col)))
			}
		}
	}
...
func (b *logicalPropsBuilder) buildFiltersItemProps(item *FiltersItem, scalar *props.Scalar) {
	...
	if eq, ok := item.Condition.(*EqExpr); ok {
		originVarLeft, IsManualCastL := IsManualCastVariable(eq.Left)
		if leftVar, ok := eq.Left.(*VariableExpr); ok || IsManualCastL {
			if IsManualCastL {
				leftVar = originVarLeft
			}
			OriginVarRight, IsManualCastR := IsManualCastVariable(eq.Right)
			if rightVar, ok := eq.Right.(*VariableExpr); ok || IsManualCastR {
				if IsManualCastR {
					rightVar = OriginVarRight
				}
				scalar.FuncDeps.AddEquivalency(leftVar.Col, rightVar.Col)
			}
		}
		// if castLExpr, ok := eq.Left.(*CastExpr); ok {
		// 	if leftVar, ok2 := castLExpr.Input.(*VariableExpr); ok2 {
		// 		if rightVar, ok := eq.Right.(*VariableExpr); ok {
		// 			scalar.FuncDeps.AddEquivalency(leftVar.Col, rightVar.Col)
		// 		} else if castExpr, ok1 := eq.Right.(*CastExpr); ok1 {
		// 			if originVar, ok2 := castExpr.Input.(*VariableExpr); ok2 {
		// 				scalar.FuncDeps.AddEquivalency(leftVar.Col, originVar.Col)
		// 			}
		// 		}
		// 	}
		// }
...
func (b *logicalPropsBuilder) buildZipItemProps(item *ZipItem, scalar *props.Scalar) {
	...
	BuildSharedProps(b.mem, item.Fn, &scalar.Shared)
}
...
func ensureLookupJoinInputProps(join *LookupJoinExpr, sb *statisticsBuilder) *props.Relational {
	...
	if relational.OutputCols.Empty() {
		...
		for i := range join.KeyCols {
			...
			relational.OutputCols.Add(int(indexColID))
		}
