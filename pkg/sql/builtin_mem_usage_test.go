...
func TestAggregatesMonitorMemory(t *testing.T) {
	...
	for _, statement := range statements {
		...
		if _, err := sqlDB.Exec(statement); pgcode.MakeCode(string(err.(*pq.Error).Code)) != pgcode.OutOfMemory {
...
func TestEvaluatedMemoryIsChecked(t *testing.T) {
	...
	testData := []string{
		...
		`SELECT zbdb_internal.no_constant_folding(length(repeat('abc', 70000000)))`,
	}
	...
	for _, statement := range testData {
		t.Run("", func(t *testing.T) {
			...
			if _, err := sqlDB.Exec(
				statement,
			); pgcode.MakeCode(string(err.(*pq.Error).Code)) != pgcode.ProgramLimitExceeded {
