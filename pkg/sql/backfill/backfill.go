...
func (cb *ColumnBackfiller) Init(
	...
	return cb.fetcher.Init(
		false /* reverse */, false /* returnRangeInfo */, false /* isCheck */, &cb.alloc,
		sqlbase.ScanLockingStrength_FOR_NONE, sqlbase.ScanLockingWaitPolicy{LockLevel: sqlbase.ScanLockingWaitLevel_BLOCK}, tableArgs,
	)
}
...
func (cb *ColumnBackfiller) RunColumnBackfillChunk(
	...
	for i := int64(0); i < chunkSize; i++ {
		...
		// locate primarykey's index
		idx := 0
		if tableDesc.IsHashPartition {
			for i, col := range tableDesc.Columns {
				if col.Name == tableDesc.HashField {
					idx = i
				}
			}
		}
		...
		for j, e := range cb.updateExprs {
			if tableDesc.IsHashPartition {
				if cb.added != nil && cb.added[0].IsHashPartitionCol {
					left := tree.IndexedVar{Idx: idx, Used: true}
					left.SetIntType()
					castExpr := &tree.CastExpr{
						Expr:       &left,
						Type:       &coltypes.TString{Variant: coltypes.TStringVariantSTRING, N: 0, VisibleType: coltypes.VisSTRING},
						SyntaxMode: tree.CastShort,
					}
					tmp, err := castExpr.Eval(cb.evalCtx)
					if err != nil {
						return roachpb.Key{}, err
					}
					byteInput := []byte(tmp.String())
					hashVal := util.CRC32Origin(byteInput)
					if tableDesc.HashParts == 0 {
						return roachpb.Key{}, errors.New("hashParts should not be zero")
					}
					res := hashVal % uint32(tableDesc.HashParts)
					defaultVal := tree.NewDInt(tree.DInt(res))
					e = defaultVal
				}
			}
		...
		if _, err := ru.UpdateRow(
			ctx, b, oldValues, updateValues, row.CheckFKs, traceKV, pm); err != nil {
			return roachpb.Key{}, err
		}
	}
	b.Header.BackFill = true
...
func (ib *IndexBackfiller) Init(desc *sqlbase.ImmutableTableDescriptor) error {
	...
	for _, m := range desc.Mutations {
		...
		if index != nil {
			...
			// If it is a functional index, all columns need to be added.
			if haveFuncIndex && IndexMutationFilter(m) {
				ib.added = append(ib.added, *index)
				for i := range cols {
		...
		if IndexMutationFilter(m) {
			...
			for i, col := range cols {
				if idx.ContainsColumnID(col.ID) || idx.EncodingType == sqlbase.PrimaryIndexEncoding {
					valNeededForCol.Add(i)
				}
			}
		}
	...
	return ib.fetcher.Init(
		false /* reverse */, false /* returnRangeInfo */, false /* isCheck */, &ib.alloc,
		sqlbase.ScanLockingStrength_FOR_NONE, sqlbase.ScanLockingWaitPolicy{LockLevel: sqlbase.ScanLockingWaitLevel_BLOCK}, tableArgs,
	)
}
...
func (ib *IndexBackfiller) BuildIndexEntriesChunk(
	...
	var txCtx transform.ExprTransformContext
	if len(ib.added) > 0 {
		if ib.added[0].PredExpr != "" {
			var err error
			expr, err := sqlbase.MakeIndexExprs(ib.added, tableDesc,
				tree.NewUnqualifiedTableName(tree.Name(tableDesc.Name)), &txCtx, ib.EvalCtx, true /* addingCols */)
			if err != nil {
				return nil, nil, err
			}
			if err != nil {
			...
			eh := runbase.ExprHelper{}
			eh.EvalCtx = ib.EvalCtx
			switch expr[0].(type) {
			case *tree.ComparisonExpr:
				eh.Expr = expr[0].(*tree.ComparisonExpr)
			case *tree.AndExpr:
				eh.Expr = expr[0].(*tree.AndExpr)
			case *tree.OrExpr:
				eh.Expr = expr[0].(*tree.OrExpr)
			default:
				return nil, nil, pgerror.Unimplemented("unimplement partition index type", "")
			}

			eh.Types = tableDesc.ColumnTypes()
			...
			for i := int64(0); i < chunkSize; i++ {
				...
				passes, err := eh.EvalFilter(encRow)
				if err != nil {
					return nil, nil, err
				}
				if !passes {
				...
				if ib.added != nil {
					// When backfilling, need to calculate the old funcExpr value first.
					for _, index := range tableDesc.TableDescriptor.Indexes {
						if index.IsFunc != nil {
				...
				if exprString != nil {
					...
					typedExprs, err := sqlbase.MakeFuncExprs(exprString, tableDesc, tree.NewUnqualifiedTableName(tree.Name(tableDesc.Name)),
						&txCtx, &evalCtx, true)
					if err != nil {
						return nil, nil, err
					}
					for _, typedExpr := range typedExprs {
						for i := 0; i < len(typedExpr.(*tree.FuncExpr).Exprs); i++ {
							if _, ok := typedExpr.(*tree.FuncExpr).Exprs[i].(*tree.IndexedVar); ok {
								typedExpr.(*tree.FuncExpr).Exprs[i] = ib.rowVals[typedExpr.(*tree.FuncExpr).Exprs[i].(*tree.IndexedVar).Idx]
							}
						}
						funcRes, err := typedExpr.Eval(&evalCtx)
						if err != nil {
							return nil, nil, err
						}
						//result := make(tree.Datums,0)
						//result = append(result, funcRes)
						//for i := 0; i < len(ib.rowVals); i++ {
						//	result = append(result, ib.rowVals[i])
						//}
						//ib.rowVals = result
						////ib.rowVals = append(ib.rowVals, funcRes)
						ib.rowVals = append(ib.rowVals, funcRes)
					}
				}
				...
				if buffer, err = sqlbase.EncodeSecondaryIndexes(
					...
					ib.rowVals, buffer, ignoreIndexes); err != nil {
					return nil, nil, err
				}
			...
			for i := int64(0); i < chunkSize; i++ {
				...
				if err := sqlbase.EncDatumRowToDatums(ib.types, ib.rowVals[:len(encRow)], encRow, &ib.alloc); err != nil {
					return nil, nil, err
				}
				...
				entries = append(entries, buffer...)
				ib.rowVals = ib.rowVals[:len(encRow)]
			}
		}
	} else {
		buffer := make([]sqlbase.IndexEntry, len(ib.added))
