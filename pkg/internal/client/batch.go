...
func (b *Batch) fillResults(ctx context.Context) {
	...
	for i := range b.Results {
		...
		for k := 0; k < result.calls; k++ {
			...
			case *roachpb.RevertRequest:
				if result.Err == nil {
					result.Keys = reply.(*roachpb.RevertResponse).Keys
				}

			case *roachpb.ClearIntentRequest:
				if result.Err == nil {
					result.Keys = reply.(*roachpb.ClearIntentResponse).Keys
				}
...
func (b *Batch) scan(s, e interface{}, isReverse bool, forUpdate bool) {
...
// RevertRange is that revert range
func (b *Batch) RevertRange(
	s, e interface{}, ts int64, returnKeys bool, clearIntents []roachpb.Intent,
) {
	begin, err := marshalKey(s)
	if err != nil {
		b.initResult(0, 0, notRaw, err)
	...
	b.appendReqs(roachpb.NewRevertRange(begin, end, ts, returnKeys, clearIntents))
	b.initResult(1, 0, notRaw, nil)
}

// ClearIntentRange is that clear intent
func (b *Batch) ClearIntentRange(s, e interface{}, returnKeys bool, clearIntents []roachpb.Intent) {
	begin, err := marshalKey(s)
	if err != nil {
		b.initResult(0, 0, notRaw, err)
		return
	...
	b.appendReqs(roachpb.NewClearIntentRange(begin, end, returnKeys, clearIntents))
	b.initResult(1, 0, notRaw, nil)
}
...
// QueryLock Encapsulate key, endkey, txnid, and scanIntent into a new request,
// and add the request to the batch
func (b *Batch) QueryLock(key, endKey []byte, txnID string, scanIntent bool) {
	sk, err := marshalKey(key)
	if err != nil {
		b.initResult(0, 0, notRaw, err)
		return
	}
	ek, err := marshalKey(endKey)
	if err != nil {
		b.initResult(0, 0, notRaw, err)
		return
	}
	b.appendReqs(roachpb.NewQueryLock(sk, ek, txnID, scanIntent))
	b.initResult(1, 0, notRaw, nil)
}
