...
type Manager interface {
	...
	GetSpanLockStatus(key roachpb.Span, transactionID string) map[lock.Durability]bool
}
...
type LockManager interface {
	...
	//OnLockUpdated通知并发管理器，事务已经更新或释放了它以前持有的一个或多个锁。
	//锁更新结构的耐久性字段被忽略
...
type lockTable interface {
	...
	String() string
	//GetSpanLockMsg 返回传入span持有的锁的durability信息
	GetSpanLockMsg(span roachpb.Span, transactionID string) map[lock.Durability]bool
}
...
// lockTableGuard是一个请求的句柄，它在lockTable中等待有冲突的锁，或者在求值时在锁等待队列中占有一个位置。
type lockTableGuard interface {
	...
	// ShouldWait必须在每个ScanAndEnqueue之后调用。如果返回false，
	// 请求应该继续进行评估，否则它释放锁并监听NewStateChan返回的通道。
	ShouldWait() bool
	...
	// state transition.
	// NewStateChan返回要侦听的通道，以获取状态可能已更改的通告。
	// 如果ShouldWait返回true，此通道将有一个初始通告。
	// 请注意，如果未检索到通告，则会折叠，因为waiter不必查看每个状态转换。
...
// delay.
// lockTableWaiter关心的是在锁等待队列中等待冲突事务所持有的锁。
// 它确保等待的请求继续向前进展，即使存在错误的事务协调器和事务死锁。
// waiter实现了请求等待lockTable中冲突锁的逻辑，直到它们被释放。
// 类似地，在任何锁等待队列中，它实现了在调用者的请求之前等待冲突请求的逻辑，该锁等待队列是调用者的一部分。
// 这个等待状态响应锁表中的一组状态转换:
//-释放一个冲突的锁
//-一个冲突的锁被更新，使其不再冲突
//-锁等待队列中的冲突请求获得锁
//-锁等待队列中有冲突的请求退出锁等待队列
// 这些状态转换通常是反应性的——waiter可以简单地等待锁被释放或者锁等待队列被其他参与者退出。
// 对于冲突锁，响应状态转换是由LockManager提供的，
// 而对于冲突锁等待队列，响应状态转换是由RequestSequencer接口提供的。
// 但是，在事务协调器失败或事务死锁的情况下，如果没有waiter的干预，
// 状态转换可能永远不会发生。为了确保向前推进，
// waiter可能需要主动推送冲突锁的持锁者或冲突锁等待队列的头。
// 此活动推入需要一个RPC到冲突事务记录的租赁者，通常会导致RPC在租赁者的txnWaitQueue中排队。
// 因为这样做的代价很高，所以推送不会立即执行。相反，它只在延迟之后执行。
type lockTableWaiter interface {
	...
	// again.
	// WaitOn接受并等待已从shouldwait返回true的lockTableGuard。
	// 应在删除请求获取的任何锁存之后调用该方法。
	// 当请求位于所有锁等待队列的最前面时，它将返回并且可以安全地重新获取锁存器并再次扫描lockTable。
	WaitOn(context.Context, Request, lockTableGuard) *Error

	WaitOnTime(context.Context, Request, lockTableGuard) *Error
	...
	// WaitOnLock等待负责指定锁的事务，然后确保该锁被清除出请求的路径。
	// 应在删除请求获取的任何锁之后调用该方法。当锁被解决时返回。
	// 注意：当禁用lockTable（例如在跟随者副本上）并且发现必须等待的锁时（例如在跟随者读取期间），
	// 将使用此方法。
	// 如果/当通过在ReplicatedEvalResult中的Raft日志而不是通过
	//（仅供租约持有者使用）LocalResult传播Raft日志来传播lockTable状态转换
	// 而在跟随者副本上维护lockTables时，我们应该能够删除lockTable的“ disabled”状态，
	// 并依次删除这个方法。 这可能是由于将所有复制的锁拉入lockTable而导致的。
	...
	// 清除所有由lockTableWaiter维护的缓存。
	// 这主要用于在副本失去租约时恢复内存。但是，它还用于在测试中重置lockTableWaiter的状态。
	ClearCaches()
}
...
type txnWaitQueue interface {
	...
	// EnqueueTxn创建一个与所提供的事务相关联的队列。一旦建立了队列，
	// 事务的pusher就可以在队列中等待，并将被告知事务所经历的状态转换。
	...
	// UpdateTxn通知队列所提供的事务已经经历了状态转换。这将被传达给任何等待的推动者。
	UpdateTxn(context.Context, *roachpb.Transaction)

	// GetDependents returns a set of transactions waiting on the specified
	// transaction either directly or indirectly. The method is used to perform
	// deadlock detection.
	// GetDependents返回一组直接或间接等待指定事务的事务。该方法用于执行死锁检测。
	GetDependents(uuid.UUID) []uuid.UUID
	...
	// 检查是否已经为被提供的请求推入的事务建立了队列。如果不是，或者PushTxn请求不能排队，该方法立即返回。
	// 如果有一个队列，该方法将该请求作为一个侍者进入队列，并等待事务被推入/完成。
	// 如果在此方法等待时成功地推送了事务，则第一个返回值是非nil的PushTxnResponse对象。
	...
	// MaybeWaitForQuery检查正在查询的事务是否已经建立了队列。如果没有，
	// 或者QueryTxn请求没有指定WaitForUpdate，
	// 该方法将立即返回。如果有一个队列，该方法将此请求作为一个waiter排队，并等待对目标事务的任何更新。
