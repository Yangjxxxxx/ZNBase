...
func mustParseOne(s string) parser.Statement {
	stmts, err := parser.Parse(s, false)
	if err != nil {
...
func startConnExecutor(
	...
	nodeID.Set(ctx, 1)
	distSQLMetrics := runbase.MakeDistSQLMetrics(time.Hour /* histogramWindow */)
	// Just init temp cursor engine for test
	cursorEngines := make([]engine.Engine, 0)
	tempCursorEngine, err := engine.NewTempEngine(ctx, enginepb.EngineTypeRocksDB, base.TempStorageConfig{InMemory: true}, base.DefaultTestStoreSpec)
	if err != nil {
		return nil, nil, nil, nil, err
	}
	stopper.AddCloser(tempCursorEngine)
	cursorEngines = append(cursorEngines, tempCursorEngine)
	cfg := &ExecutorConfig{
		...
		DistSQLPlanner: NewDistSQLPlanner(
			ctx, distsql.Version, st, roachpb.NodeDescriptor{},
			nil, /* rpcCtx */
			distsql.NewServer(ctx, runbase.ServerConfig{
