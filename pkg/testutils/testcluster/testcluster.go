...
// StartTestCluster starts a test cluster
func StartTestCluster(t testing.TB, nodes int, args base.TestClusterArgs) *TestCluster {
	...
	for i := 0; i < nodes; i++ {
		...
		//default.
		if nodes > 0 {
			Names := serverArgs.LocationName.Names
			name := fmt.Sprintf("Dongli%d", i+1)
			Names = append(Names, name)
			Names = append(Names, "TJ")
			serverArgs.LocationName = roachpb.LocationName{Names: Names}
...
func (tc *TestCluster) changeReplicas(
	...
	desc, err := tc.Servers[0].DB().AdminChangeReplicas(
		ctx, startKey.AsRawKey(), changeType, targets, beforeDesc, false,
	)
	if err != nil {
...
func (tc *TestCluster) WaitForNodeStatuses(t testing.TB) {
	testutils.SucceedsSoon(t, func() error {
		...
		conn, err := tc.Server(0).RPCContext().GRPCDialNode(url, nodeID, rpc.DefaultClass).Connect(context.Background())
		if err != nil {
			return err
		}
...
func (tc *TestCluster) UpdateAudit() {
	if tc != nil {
		for i := 0; i < tc.NumServers(); i++ {
			tc.Server(i).AuditServer().TestSyncConfig()
		}
	}
}
