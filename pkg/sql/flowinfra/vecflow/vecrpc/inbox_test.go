...
func TestInboxCancellation(t *testing.T) {
	...
	t.Run("ReaderWaitingForStreamHandler", func(t *testing.T) {
		inbox, err := NewInbox(testAllocator, typs, distsqlpb.StreamID(0))
		require.NoError(t, err)
		ctx, cancelFn := context.WithCancel(context.Background())
		// Cancel the context.
		cancelFn()
		// Next should not block if the context is canceled.
		err = execerror.CatchVecRuntimeError(func() { inbox.Next(ctx) })
...
func TestInboxNextPanicDoesntLeakGoroutines(t *testing.T) {
	...
	inbox, err := NewInbox(testAllocator, []coltypes.T{coltypes.Int64}, distsqlpb.StreamID(0))
	require.NoError(t, err)
...
func TestInboxTimeout(t *testing.T) {
	...
	go func() {
		readerErrCh <- execerror.CatchVecRuntimeError(func() { inbox.Next(ctx) })
	}()
...
func TestInboxShutdown(t *testing.T) {
	...
	var (
		...
		typs               = []coltypes.T{coltypes.Int64}
		batch              = vecexec.RandomBatch(testAllocator, rng, typs, int(coldata.BatchSize()), 0 /* length */, rng.Float64())
	)
		...
		for _, runNextGoroutine := range []bool{false, true} {
			for _, runRunWithStreamGoroutine := range []bool{false, true} {
				...
				t.Run(fmt.Sprintf(
					...
					inbox, err := NewInbox(
						vecexec.NewAllocator(inboxCtx, &inboxMemAccount),
						...
						},
						{
							...
							asyncOperation: func() chan error {
								...
								go func() {
									...
									for !done && err == nil {
										err = execerror.CatchVecRuntimeError(func() { b := inbox.Next(inboxCtx); done = b.Length() == 0 })
									}
									errCh <- err
								}()
								return errCh
