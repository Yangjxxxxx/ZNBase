...
func (m *mysqldumpReader) closeRejectCh(ctx context.Context) {
	if m.fileContext.rejected != nil {
		close(m.fileContext.rejected)
	}
}

func (m *mysqldumpReader) saveRejectRecord(
...
func newMysqldumpReader(
	...
	res := &mysqldumpReader{evalCtx: evalCtx, kvCh: kvCh, metrics: metrics}
...
func (m *mysqldumpReader) readFiles(
	...
	return readInputFiles(ctx, cp, m.readFile, &m.fileContext, nil, "", details)
}
...
func mysqlValueToDatum(
	...
	case *mysql.SQLVal:
		...
		case mysql.StrVal:
			s := string(v.Val)
			// https://github.com/znbasedb/znbase/issues/29298
			...
			return tree.ParseStringAs(desired, s, evalContext, true)
		case mysql.IntVal:
			return tree.ParseStringAs(desired, string(v.Val), evalContext, true)
		case mysql.FloatVal:
			return tree.ParseStringAs(desired, string(v.Val), evalContext, true)
		case mysql.HexVal:
			v, err := v.HexDecode()
...
func readMysqlCreateTable(
	...
	for {
		...
		if i, ok := stmt.(*mysql.DDL); ok && i.Action == mysql.CreateStr {
			...
			tbl, moreFKs, err := mysqlTableToCockroach(ctx, evalCtx, parentID, id, name, i.TableSpec, fks, seqVals, statusServer, user, p)
			if err != nil {
				return nil, err
			}
...
func mysqlTableToCockroach(
	...
	if seqName != "" {
		...
		priv := sqlbase.NewDefaultObjectPrivilegeDescriptor(privilege.Table, user)
		...
		if p != nil {
			...
			desc, err = sql.MakeSequenceTableDesc(
				seqName, opts, parentID, id, time, priv, nil, nil, /* params */
			)
		}
		if err != nil {
			return nil, nil, err
	...
	desc, err := MakeSimpleTableDescriptor(evalCtx.Ctx(), nil, stmt, parentID, id, fks, time.WallTime, statusServer, user)
	if err != nil {
		return nil, nil, err
	}
...
func mysqlColToCockroach(
	...
	case mysqltypes.Blob:
		def.Type = coltypes.Blob
	case mysqltypes.VarBinary:
		def.Type = coltypes.Bytea
