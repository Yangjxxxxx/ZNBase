...
func (sb *statisticsBuilder) colStatFromInput(
	...
	case *SelectExpr:
		return sb.colStatFromChild(colSet, t, 0), sb.statsFromChild(e, 0 /* childIdx */)
...
func (sb *statisticsBuilder) colStat(colSet opt.ColSet, e RelExpr) *props.ColumnStatistic {
	...
	case opt.WithScanOp, opt.RecursiveCTEOp:
		// colStatWithScan暂时注释
		//return sb.colStatWithScan(colSet, e.(*WithScanExpr))
		return sb.colStatUnknown(colSet, e.Relational())

	case opt.OpaqueDDLOp, opt.OpaqueMutationOp, opt.OpaqueRelOp:
		return sb.colStatUnknown(colSet, e.Relational())
	}
	...
	if tab.StatisticCount() == 0 {
		...
		for i := 0; i < tab.StatisticCount(); i++ {
			...
			for i := 0; i < stat.ColumnCount(); i++ {
				colID := tabID.ColumnID(stat.ColumnOrdinal(i))
				cols.Add(int(colID))
			}
			if colStat, ok := stats.ColStats.Add(cols); ok {
				...
				if cols.Len() == 1 && stat.Histogram() != nil {
					...
					histogram.Init(sb.evalCtx, opt.ColumnID(col), stat.Histogram())
					colStat.Histogram = histogram
				}
	...
	default:
		...
		default:
			...
			case *FullJoinApplyExpr:
				e = e.Memo().MemoizeFullJoinApply(t.Left, t.Right, on, &t.JoinPrivate)
			case *RightJoinApplyExpr:
				e = e.Memo().MemoizeRightJoinApply(t.Left, t.Right, on, &t.JoinPrivate)
			default:
...
func (sb *statisticsBuilder) updateNullCountsFromNotNullCols(
	...
	notNullCols.ForEach(func(col int) {
...
func (sb *statisticsBuilder) buildSelect(sel *SelectExpr, relProps *props.Relational) {
	...
	// Adjust the selectivity so we don't double-count the histogram columns.
	s.ApplySelectivity(1.0 / sb.selectivityFromDistinctCounts(histCols, sel, s))
...
func (sb *statisticsBuilder) applyFilter(
	...
	applyConjunct := func(conjunct *FiltersItem) {
		...
		if scalarProps.Constraints != nil {
			histColsLocal := sb.applyConstraintSet(scalarProps.Constraints, scalarProps.TightConstraints, e, relProps)
		...
		if conjunct.Condition.Op() == opt.LikeOp {
			numUnappliedConjuncts++
		}
	}
...
func (sb *statisticsBuilder) applyIndexConstraint(
	...
	// Calculate histogram.
	constrainedCols.ForEach(func(col int) {
		colSet := util.MakeFastIntSet(col)
...
//	}
//
//	applied, _ := sb.updateDistinctCountsFromConstraint(c, e, relProps)
...
func (sb *statisticsBuilder) applyConstraintSet(
	...
	for i := 0; i < cs.Length(); i++ {
		...
		applied, lastColMinDistinct := sb.updateDistinctCountsFromConstraint(cs.Constraint(i), e, s)
		if applied == 0 {
		...
		// Calculate histogram.
		cols := util.MakeFastIntSet(int(col))
		// Calculate histogram.
	...
	for col := 0; col <= prefix; col++ {
		...
		for i := 0; i < c.Spans.Count(); i++ {
			...
			if startVal.Compare(sb.evalCtx, endVal) != 0 {
				...
				// besides int?
				if (startVal.ResolvedType() == types.Int && endVal.ResolvedType() == types.Int) || (startVal.ResolvedType() == types.Int2 && endVal.ResolvedType() == types.Int2) || (startVal.ResolvedType() == types.Int4 && endVal.ResolvedType() == types.Int4) {
		...
		sb.ensureColStat(util.MakeFastIntSet(int(colID)), distinctCount, e, s)
		applied = col + 1
	}
...
func (sb *statisticsBuilder) selectivityFromMultiColDistinctCounts(
	...
	for col, ok := cols.Next(0); ok; col, ok = cols.Next(col + 1) {
		colStat, ok := s.ColStats.Lookup(opt.MakeColSet(opt.ColumnID(col)))
		if !ok {
	...
	if maxNewDistinct > 1 && multiColSet.Len() > 2 {
		...
		multiColSet.ForEach(func(col int) {
			// We already know the column stat exists if it's in multiColSet.
			colStat, _ := s.ColStats.Lookup(opt.MakeColSet(opt.ColumnID(col)))
	...
	return (1-0.9)*singleColSelectivity + 0.9*multiColSelectivity
}
...
func (sb *statisticsBuilder) selectivityFromNullsRemoved(
	...
	relProps.NotNullCols.ForEach(func(col int) {
		if !ignoreCols.Contains(col) {
			inputColStat, inputStats := sb.colStatFromInput(util.MakeFastIntSet(col), e)
...
//finalizeFromRowCountAndDistinctCounts 将colStat中列的统计信息缩放为应用过滤条件后的统计信息
func (sb *statisticsBuilder) finalizeFromRowCountAndDistinctCounts(
	...
	if colStat.Cols.Len() > 1 && rowCount > 1 {
		product := 1.0
		maxDistinct := 0.0
		colStat.Cols.ForEach(func(col int) {
			var cs opt.ColSet
			cs.Add(col)
			if singleColStat, ok := s.ColStats.Lookup(cs); ok {
...
func (sb *statisticsBuilder) updateDistinctCountFromUnappliedConjuncts(
	...
	colSet := util.MakeFastIntSet(int(colID))
...
//func (sb *statisticsBuilder) splitFilter() []filterList {
