...
// This test runs psycopg full test suite against an single znbase node.
...
func registerPsycopg(r *registry) {
	runPsycopg := func(
		...
		if expectedFailureList == nil {
			t.Fatalf("No psycopg blacklist defined for znbase version %s", version)
		}
		if ignoredlist == nil {
			t.Fatalf("No psycopg ignorelist defined for znbase version %s", version)
		}
		c.l.Printf("Running znbase version %s, using blacklist %s, using ignoredlist %s",
