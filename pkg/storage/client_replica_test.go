...
//    operation, and znbase doesn't update its read timestamp cache.
...
func TestLeaseNotUsedAfterRestart(t *testing.T) {
	...
	t.Skip("https://github.com/znbasedb/znbase/issues/34111")
...
func TestDrainRangeRejection(t *testing.T) {
	...
	if _, err := repl.ChangeReplicas(
		...
		"",
		false); !testutils.IsError(err, "store is draining") {
...
func TestSystemZoneConfigs(t *testing.T) {
	...
	expectedSchemaRanges := 2
	expectedUserRanges := 1 + expectedSchemaRanges
	expectedSystemRanges = expectedSystemRanges - expectedUserRanges
...
func TestConcurrentAdminChangeReplicasRequests(t *testing.T) {
	...
	go func() {
		res1, err1 = db.AdminChangeReplicas(
			ctx, key, roachpb.ADD_REPLICA, targets1, expects1, false)
		wg.Done()
	}()
	go func() {
		res2, err2 = db.AdminChangeReplicas(
			ctx, key, roachpb.ADD_REPLICA, targets2, expects2, false)
		wg.Done()
	}()
	wg.Wait()
...
func TestRandomConcurrentAdminChangeReplicasRequests(t *testing.T) {
	...
	addReplicas := func() error {
		_, err := db.AdminChangeReplicas(
			ctx, key, roachpb.ADD_REPLICA, pickTargets(), rangeInfo.Desc, false)
		return err
	}
	wg.Add(actors)
	for i := 0; i < actors; i++ {
...
func TestAdminRelocateRangeSafety(t *testing.T) {
	...
	change := func() {
		...
		changedDesc, changeErr = r1.ChangeReplicas(ctx, roachpb.REMOVE_REPLICA,
			makeReplicationTargets(2)[0], &expDescAfterAdd, "replicate", "testing", false)
	}
	relocate := func() {
...
func TestDiscoverIntentAcrossLeaseTransferAwayAndBack(t *testing.T) {
	...
	knobs.EvalKnobs.TestingPostEvalFilter = func(args storagebase.FilterArgs) *roachpb.Error {
		if txn := args.Hdr.Txn; txn != nil && txn.ID == txn2ID.Load() {
			txn2BBlockOnce.Do(func() {
				if _, ok := args.Err.(*roachpb.WriteIntentError); !ok {
	...
	knobs.TestingRequestFilter = func(ba roachpb.BatchRequest) *roachpb.Error {
	...
	// being aborted.
	hb, hbH := heartbeatArgs(txn3.Serialize(), kvDB.Clock().Now())
	_, pErr := client.SendWrappedWith(ctx, kvDB.GetFactory().NonTransactionalSender(), hbH, hb)
