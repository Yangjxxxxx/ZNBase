...
var backwardCompatibleMigrations = []migrationDescriptor{
	...
	{
		// Introduced in inspur 1.0
		name:                "create system.snapshots table",
		workFn:              createSnapshotTable,
		includedInBootstrap: true,
		newDescriptorIDs:    staticIDs(keys.SnapshotsTableID),
	},
	{
		// Introduced in inspur 1.2
		name:                "create system.flashback table",
		workFn:              createFlashbackTable,
		includedInBootstrap: true,
		newDescriptorIDs:    staticIDs(keys.FlashbackTableID),
	},
	{
		// Defaultdb Schema ID
		name:             "create defaultdb.public schema",
		workFn:           createDefaultdbSchema,
		newDescriptorIDs: staticIDs(keys.DefaultdbSchemaID),
	},
	{
		// Postgres Schema ID
		name:             "create postgres.public schema",
		workFn:           createPostgresSchema,
		newDescriptorIDs: staticIDs(keys.PostgresSchemaID),
	},
	{
		// Introduced in v20.2.
	...
	{
		// Introduced in v20.2.
		name:   "add created_by_type columns to system.jobs",
		workFn: addJobsCreatedByType,
	},
	//{
	//	// Introduced in v20.2.
	//	name:   "add created_by_id columns to system.jobs",
	//	workFn: addJobsCreatedByID,
	//},
	{
		// add column privileges to table jobs.
		name:   "add column privileges to system.jobs",
		workFn: addJobsColumnPrivileges,
	},
}
...
// some part of the cluster state when the ZNBaseDB version is upgraded.
...
func createSnapshotTable(ctx context.Context, r runner) error {
	return createSystemTable(ctx, r, sqlbase.SnapshotsTable)
}

func createFlashbackTable(ctx context.Context, r runner) error {
	return createSystemTable(ctx, r, sqlbase.FlashbackTable)
}

func createSystemSchema(ctx context.Context, r runner, desc sqlbase.SchemaDescriptor) error {
	// We install the table at the KV layer so that we can choose a known ID in
	// the reserved ID space. (The SQL layer doesn't allow this.)
	err := r.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		b := txn.NewBatch()
...
func createDefaultdbSchema(ctx context.Context, r runner) error {
	return createSystemSchema(ctx, r, sqlbase.DefaultdbPublicSchema)
}

func createPostgresSchema(ctx context.Context, r runner) error {
	return createSystemSchema(ctx, r, sqlbase.PostgresPublicSchema)
}

var reportingOptOut = envutil.EnvOrDefaultBool("ZNBASE_SKIP_ENABLING_DIAGNOSTIC_REPORTING", false)
...
func ensureMaxPrivileges(ctx context.Context, r runner) error {
	tableDescFn := func(desc *sqlbase.TableDescriptor) (bool, error) {
		...
		return desc.Privileges.MaybeFixPrivileges(desc.ID, priv), nil
	}
	databaseDescFn := func(desc *sqlbase.DatabaseDescriptor) (bool, error) {
		return desc.Privileges.MaybeFixPrivileges(desc.ID, privilege.DatabasePrivileges), nil
	}
	schemaDescFn := func(desc *sqlbase.SchemaDescriptor) (bool, error) {
		return desc.Privileges.MaybeFixPrivileges(desc.ID, privilege.DatabasePrivileges), nil
	}
	functionDescFn := func(desc *sqlbase.FunctionDescriptor) (bool, error) {
		return desc.Privileges.MaybeFixPrivileges(desc.ID, privilege.FunctionPrivileges), nil
	}
	return upgradeDescsWithFn(ctx, r, tableDescFn, databaseDescFn, schemaDescFn, functionDescFn)
}
...
func upgradeDescsWithFn(
	...
	upgradeSchemaDescFn func(desc *sqlbase.SchemaDescriptor) (upgraded bool, err error),
	upgradeFunctionDescFn func(desc *sqlbase.FunctionDescriptor) (upgraded bool, err error),
) error {
		...
		if err := r.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
			...
			for _, kv := range kvs {
				...
				case *sqlbase.Descriptor_Table:
					deprecatedKey := sqlbase.MakeDescMetadataKey(sqlDesc.GetID())
					...
					if table := deprecatedDesc.Table(ts); table != nil && upgradeTableDescFn != nil {
				...
				case *sqlbase.Descriptor_Schema:
					if schema := sqlDesc.GetSchema(); schema != nil && upgradeSchemaDescFn != nil {
						if upgraded, err := upgradeSchemaDescFn(schema); err != nil {
							return err
						} else if upgraded {
							...
							b.Put(kv.Key, sqlbase.WrapDescriptor(schema))
						}
					}
				case *sqlbase.Descriptor_Function:
					if function := sqlDesc.GetFunction(); function != nil && upgradeFunctionDescFn != nil {
						if upgraded, err := upgradeFunctionDescFn(function); err != nil {
							return err
						} else if upgraded {
							...
							b.Put(kv.Key, sqlbase.WrapDescriptor(function))
						}
					}
	...
	for retry := retry.Start(retry.Options{MaxRetries: 5}); retry.Next(); {
		...
		if isRole {
			return fmt.Errorf(`found a role named %s which is now a reserved name. Please drop the role `+
				`(DROP ROLE %s) using a previous version of ZNBaseDB and try again`,
		...
		return fmt.Errorf(`found a user named %s which is now a reserved name. Please drop the role `+
			`(DROP USER %s) using a previous version of ZNBaseDB and try again`,
			sqlbase.PublicRole, sqlbase.PublicRole)
	}
	return nil
}
...
func addJobsCreatedByType(ctx context.Context, r runner) error {
	// Ideally to add a column progress, we'd just run a query like:
	//  ALTER TABLE system.jobs ADD COLUMN progress BYTES CREATE FAMLIY progress;
	// However SQL-managed schema changes use jobs tracking internally, which will
	// fail if the jobs table's schema has not been migrated. Rather than add very
	...
	return r.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		if _, err := desc.FindActiveColumnByName("created_by_type"); err == nil {
			return nil
		}
		desc.AddColumn(sqlbase.ColumnDescriptor{
			Name: "created_by_type",
			Type: sqlbase.ColumnType{SemanticType: sqlbase.ColumnType_STRING,
				VisibleTypeName: coltypes.VisibleTypeName[coltypes.VisSTRING]},
			Nullable: true,
		})
		if err := desc.AddColumnToFamilyMaybeCreate("created_by_type", "fam_0_id_status_created_payload", true, true); err != nil {
			return err
		}
		if _, err := desc.FindActiveColumnByName("created_by_id"); err == nil {
			return nil
		}
		desc.AddColumn(sqlbase.ColumnDescriptor{
			Name: "created_by_id",
		...
		if err := desc.AddColumnToFamilyMaybeCreate("created_by_id", "fam_0_id_status_created_payload", true, true); err != nil {
			return err
		}
		if err := desc.AllocateIDs(); err != nil {
			return err
...
//func addJobsCreatedByID(ctx context.Context, r runner) error {
//	// Ideally to add a column progress, we'd just run a query like:
//	//  ALTER TABLE system.jobs ADD COLUMN progress BYTES CREATE FAMLIY progress;
//	// However SQL-managed schema changes use jobs tracking internally, which will
//	// fail if the jobs table's schema has not been migrated. Rather than add very
...
//			Nullable: true,
//		})
//		if err := desc.AddColumnToFamilyMaybeCreate("created_by_id", "fam_0_id_status_created_payload", true, false); err != nil {
//			return err
//		}
//		if err := desc.AllocateIDs(); err != nil {
//			return err
...
func addJobsColumnPrivileges(ctx context.Context, r runner) error {
	return r.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		if err := txn.SetSystemConfigTrigger(); err != nil {
			return err
		}
		...
		for i, col := range desc.Columns {
			if (col.Name == "progress" || col.Name == "created_by_type" || col.Name == "created_by_id") && col.Privileges == nil {
				desc.Columns[i].Privileges = colPrivs
			}
		}
