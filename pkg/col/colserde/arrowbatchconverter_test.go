...
func randomBatch(allocator *vecexec.Allocator) ([]coltypes.T, coldata.Batch) {
	const maxTyps = 16
	...
	b := vecexec.RandomBatch(allocator, rng, typs, capacity, length, rng.Float64())
	return typs, b
}
...
func BenchmarkArrowBatchConverter(b *testing.B) {
	...
	for typIdx, typ := range typs {
		batch := vecexec.RandomBatch(testAllocator, rng, []coltypes.T{typ}, int(coldata.BatchSize()), 0 /* length */, 0 /* nullProbability */)
		if batch.Width() != 1 {
