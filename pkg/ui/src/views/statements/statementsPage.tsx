...
type ICollectedStatementStatistics = protos.znbase.server.serverpb.StatementsResponse.ICollectedStatementStatistics;
...
class StatementsPage extends React.Component<StatementsPageProps & RouteProps, StatementsPageState> {
  ...
  renderStatements = () => {
    ...
    const appOptions = [{ value: "", label: "所有" }];
  ...
  render() {
    return (
      <React.Fragment>
        <Helmet>
          <title>
            { this.props.params[appAttr] ? this.props.params[appAttr] + " 应用 | 语句" : "语句"}
          </title>
        </Helmet>
...
export const selectApps = createSelector(
  ...
  (state: CachedDataReducerState<StatementsResponseMessage>) => {
    ...
    return [].concat(sawInternal ? ["内部"] : []).concat(sawBlank ? ["(unset)"] : []).concat(Object.keys(apps));
  },
);
