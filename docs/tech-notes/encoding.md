Structured data encoding in ZNBaseDB SQL
===========================================

Like many databases, ZNBaseDB (ZNBase) encodes SQL data into key-value
...
for how ZNBase encodes primitive values ([pkg/util/encoding/encoding.go]).

The ZNBase Labs blog post [SQL in ZNBaseDB: Mapping Table Data to
...
ZNBaseDB]. See also the [column families RFC] and the [interleaving
RFC].

This document was originally written by David Eisenstat
&lt;<eisen@znbaselabs.com>&gt;.
...
family as needed (see subsection NULL below). ZNBase stores primary key
...
ZNBase encodes these fields individually and concatenates the resulting
...
share an encoding. The relationship will become many-to-many when ZNBase
...
sentinel KV pairs. ZNBase emits sentinel KV pairs regardless of whether
...
On upgrading to format version 2 or higher, ZNBase puts each existing
...
Instead, ZNBase encodes each row as if the columns where that row is null
...
`znbase debug rocksdb scan --value_hex`, with annotations.
...
To unify the handling of SQL tables and indexes, ZNBase stores the
...
By default, indexes (in ZNBase terminology, so both primary and secondary)
...
Example dump:
  ...
  [pkg/util/encoding/encoding.go]: https://github.com/znbasedb/znbase/blob/master/pkg/util/encoding/encoding.go
  [SQL in ZNBaseDB: Mapping Table Data to Key-Value Storage]: https://www.znbaselabs.com/blog/sql-in-znbasedb-mapping-table-data-to-key-value-storage/
  [Implementing Column Families in ZNBaseDB]: https://www.znbaselabs.com/blog/sql-znbasedb-column-families/
  [column families RFC]: https://github.com/znbasedb/znbase/blob/master/docs/RFCS/20151214_sql_column_families.md
  [interleaving RFC]: https://github.com/znbasedb/znbase/blob/master/docs/RFCS/20160624_sql_interleaved_tables.md
  [pkg/sql/sqlbase/structured.proto]: https://github.com/znbasedb/znbase/blob/master/pkg/sql/sqlbase/structured.proto
  [pkg/sql/rowwriter.go]: https://github.com/znbasedb/znbase/blob/master/pkg/sql/sqlbase/rowwriter.go
  [pkg/sql/sqlbase/multirowfetcher.go]: https://github.com/znbasedb/znbase/blob/master/pkg/sql/sqlbase/multirowfetcher.go
  [prefix-free]: https://en.wikipedia.org/wiki/Prefix_code
  [new `DECIMAL` encoding]: https://github.com/znbasedb/znbase/issues/13384#issuecomment-277120394
  [pkg/sql/sqlbase/table.go]: https://github.com/znbasedb/znbase/blob/master/pkg/sql/sqlbase/table.go
  [pkg/sql/sqlbase/encoded\_datum.go]: https://github.com/znbasedb/znbase/blob/master/pkg/sql/sqlbase/encoded_datum.go
  [pkg/roachpb/data.proto]: https://github.com/znbasedb/znbase/blob/master/pkg/roachpb/data.proto
