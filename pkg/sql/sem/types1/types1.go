...
// nullable and non-nullable  It is up to the caller to store that
...
// for a subset of  See the method comments for more details.
//
...
//
// Implementation-wise, T wraps a protobuf-generated InternalType struct.
// The generated protobuf code defines the struct fields, marshals/unmarshals
// them, formats a string representation, etc. Meanwhile, the wrapper T
...
// Convenience list of pre-constructed  Caller code can use any of these
...
var (
	...
	// that matches any other type, including scalar, array, and tuple
...
// IntervalTypeMetadata returns the IntervalTypeMetadata for interval
...
// example, when ZNBase encodes values, it maintains routines for each family,
...
// (Int). Doing this vastly reduces the required number of operator
// overloads.
...
//
// Precision is always 0 for other
...
// than the native ZNBase name for it (i.e. the Name function). It is used when
...
func (t *T) PGName() string {
	...
	// Postgres does not have an UNKNOWN[] type. However, ZNBase does, so
...
//  And a wildcard collation (empty string) matches any other collation.
...
// been serialized by any previous version of ZNBase. It upgrades the object
...
func (t *T) upgradeType() error {
	...
	case ArrayFamily:
		if t.ArrayContents() == nil {
			// This array type was serialized by a previous version of ZNBase,
...
// version of ZNBase so that clusters can run in mixed version mode during
// upgrade.
//
...
// of the latest ZNBase version. It updates the fields so that they will be
// marshaled into a format that is compatible with the previous version of
// ZNBase. This is necessary to preserve backwards-compatibility in mixed-version
