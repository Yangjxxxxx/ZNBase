...
func TestSpanSetBatch(t *testing.T) {
	...
	insideKey3 := engine.MakeMVCCMetadataKey(roachpb.Key("e"))
	...
	if err := batch.Put(insideKey3, []byte("value3")); err != nil {
		t.Fatalf("failed to write inside the range: %s", err)
	}
	...
	func() {
		...
		if !reflect.DeepEqual(iter.Key(), insideKey3) {
			t.Fatalf("expected key %s, got %s", insideKey3, iter.Key())
		}
