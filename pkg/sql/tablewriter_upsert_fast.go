...
func (tu *fastTableUpserter) row(
	ctx context.Context, d tree.Datums, traceKV bool, pm schemaexpr.PartialIndexUpdateHelper,
) error {
	tu.batchSize++
	// Use the fast path, ignore conflicts.
	return tu.ri.InsertRow(ctx, tu.b, d, true /* ignoreConflicts */, row.CheckFKs, traceKV, pm)
}
