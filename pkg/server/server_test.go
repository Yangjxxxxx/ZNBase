		...
		t.Run("", func(t *testing.T) {
			...
			for start := 0; start < len(tc.keys); start++ {
				...
				for maxResults := 1; maxResults <= len(tc.keys)-start+1; maxResults++ {
					scan := roachpb.NewScan(tc.keys[start], tc.keys[len(tc.keys)-1].Next(), false)
...
func TestListenerFileCreation(t *testing.T) {
	...
	files, err := filepath.Glob(filepath.Join(dir, "znbase.*"))
	if err != nil {
		t.Fatal(err)
	}
...
`
	...
	t.Run("Insecure mode", func(t *testing.T) {
		s, _, _ := serverutils.StartServer(t, base.TestServerArgs{
			...
			// `ZNBASE_EXPERIMENTAL_REQUIRE_WEB_SESSION` to false, or not setting it.
