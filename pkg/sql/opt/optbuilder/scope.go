...
type scope struct {
	...
	// UpdatableViewDepends store properties related to updatable view.
	UpdatableViewDepends updatableViewDepends
}

// updatableViewDepends store properties related to updatable view.
type updatableViewDepends struct {
	ViewDependsColName map[string]string

	ViewDependsColExpr map[string]tree.Expr

	TableName []tree.Name

	ChangedCol map[string]colType
}
...
func (s *scope) resolveType(expr tree.Expr, desired types.T) tree.TypedExpr {
	isInsert := false
	if s.builder.semaCtx.IVarContainer != nil {
		isInsert = s.builder.semaCtx.IVarContainer.GetForInsertOrUpdate()
	}
	expr = s.walkExprTree(expr)
	s.builder.semaCtx.IVarContainer.SetForInsertOrUpdate(isInsert)
	texpr, err := tree.TypeCheck(expr, s.builder.semaCtx, desired, false)
	if err != nil {
		if s.UpdatableViewDepends.ViewDependsColExpr != nil {
...
func (s *scope) resolveAndRequireType(expr tree.Expr, desired types.T) tree.TypedExpr {
	expr = s.walkExprTree(expr)
	texpr, err := tree.TypeCheckAndRequire(expr, s.builder.semaCtx, desired, s.context, false)
	if err != nil {
		if s.UpdatableViewDepends.ViewDependsColExpr != nil {
			panic(pgerror.NewErrorf(pgcode.Syntax, err.Error()))
		} else {
...
func (s *scope) FindSourceProvidingColumn(
	_ context.Context, colName tree.Name, originalName string,
	...
	stmt := s.GetScopeStmt()
	if stmt != nil {
		if sel, ok := stmt.(*tree.Select); ok {
			startWithIsJoin = sel.GetStartWithIsJoin()
			for i := range s.cols {
				col := &s.cols[i]
				name := strings.Split(string(col.name), ".")
				if len(name) == 2 {
					if name[1] == string(colName) {
						isAmbiguous = true
					}
				}
			}
	...
	// above.
	var tmpName tree.Name
	if originalName == "" {
		tmpName = colName
	} else {
		tmpName = tree.Name(originalName)
	}
	if startWithIsJoin && isAmbiguous {
		//return nil, nil, -1, sqlbase.NewUndefinedColumnError(tree.ErrString(&tmpName))
		return nil, nil, -1, pgerror.NewErrorf(pgcode.AmbiguousColumn,
			"column reference %q is ambiguous ", tmpName,
		)
	} else if startWithIsJoin && isExit {
		return nil, nil, -1, sqlbase.NewUndefinedColumnError(tree.ErrString(&tmpName))
	}
...
// ResolveUDRFunction will resolve user define function when build-in function finish
func (s *scope) ResolveUDRFunction(
	ctx context.Context, function *tree.FunctionDefinition,
) (*tree.FunctionDefinition, error) {
	b := s.builder
	catalog := s.builder.catalog
	funcName := tree.MakeUnqualifiedFuncTableName(function.Path)
	funcName.DefinedAsFunction()
	var funcDescGroup *sqlbase.ImmutableFunctionDescriptor
	objectDesc, udrFunc, err := catalog.ResolveUDRFunctionDesc(&funcName)
	if err != nil {
		return nil, err
	}
	var ok bool
	funcDescGroup, ok = objectDesc.(*sqlbase.ImmutableFunctionDescriptor)
	if !ok {
		return nil, pgerror.NewAssertionErrorf("resolve udr desc assertion error")
	}

	if funcDescGroup == nil || len(funcDescGroup.FuncGroup) == 0 {
		return nil, sqlbase.NewUndefinedFunctionError(funcName.TableName.String())
	}

	// add function to meta data
	fID := b.factory.Metadata().AddFunction(udrFunc)
	if fID > 0 {
		b.factory.Metadata().AddFunctionDependency(&funcName, udrFunc, privilege.EXECUTE)
	}
	var funcDef tree.FunctionDefinition
	funcDef.Name = function.String()
	funcDef.Definition = []tree.OverloadImpl{}

	errs := s.builder.catalog.CheckFunctionPrivilege(ctx, udrFunc)
	//	if len(errs) == 1 && errs[0] != nil { // no database or schema USAGE privilege
	/*	if errs[0] != nil {
		return nil, errs[0]
	}*/

	for i, function := range funcDescGroup.FuncGroup {
...
func (s *scope) Resolve(
	...
	for i := range inScope.cols {
		col := &inScope.cols[i]
		if s.builder != nil && !s.builder.CaseSensitive && !tree.CaseSensitiveG {
			if col.expr != nil {
				if strings.ToLower(string(col.name)) == strings.ToLower(string(colName)) && sourceNameMatches(*prefix, col.table) {
					return col, nil
				}
			} else {
	...
	if stmt != nil {
		if sel, ok := stmt.(*tree.Select); ok {
			if sel.GetStartWithIsJoin() {
				for i := range inScope.cols {
					col := &inScope.cols[i]
					name := strings.Split(string(col.name), ".")
					if len(name) == 2 {
						if name[1] == string(colName) {
							return nil, pgerror.NewErrorf(pgcode.AmbiguousColumn,
								"column reference %q is ambiguous ", colName,
							)
						}
					}
				}
	...
	if !tree.CaseSensitiveG && s.builder != nil && !s.builder.CaseSensitive {
		stmt := s.GetScopeStmt()
		if stmt != nil {
			if s, ok := stmt.(*tree.Select); ok {
				if sel, ok := s.Select.(*tree.SelectClause); ok {
					for _, value := range sel.Exprs {
						switch t := value.Expr.(type) {
						case *tree.UnresolvedName:
							if string(colName) == t.Parts[0] && t.OriginalName != "" {
								colName = tree.Name(t.OriginalName)
							}
						}
					}
				}
...
func (s *scope) VisitPre(expr tree.Expr) (recurse bool, newExpr tree.Expr) {
	...
	case *tree.ColumnItem:
		...
		if err != nil {
			if s.builder.semaCtx.InUDR {
				varName := string(t.ColumnName)
				i := len(s.builder.evalCtx.Params) - 1
				for ; i >= 0; i-- {
					if (s.builder.evalCtx.Params[i].VarName == varName || tree.FindNameInNames(varName, s.builder.evalCtx.Params[i].AliasNames)) && !s.builder.evalCtx.Params[i].IsCursorArg {
						v := s.builder.evalCtx.Params[i]
						if s.builder.semaCtx.IsWhere {
							s.builder.HadPlaceholders = true
							return true, v.VarDatum
						}
						return true, &tree.PlpgsqlVar{VarName: tree.Name(varName), VarType: types.OidToType[v.VarOid], Index: i}
					}
				}
			}
			panic(builderError{err})
	...
	case *tree.FuncExpr:
		def, err := t.Func.Resolve(s.builder.semaCtx.SearchPath, s.builder.CaseSensitive)
		var part [4]string
		if funcReference, ok := t.Func.FunctionReference.(*tree.UnresolvedName); ok {
			part = funcReference.Parts
		}
		if err != nil {
			if !util.EnableUDR {
				panic(builderError{err})
			}
			// first check error's code type, if NewUndefinedFunctionError, need to continue search
			// User define function if exists then return or not report error
			if errMsg, ok := err.(*pgerror.Error); ok && errMsg.Code == pgcode.UndefinedFunction {
				// using pre resolve result for function Ref
				t.Func.FunctionReference = def
				def.Path = part
				UDRdef, UDRerr := t.ResolveUDRFunction(s.builder.ctx, s.builder.semaCtx.SearchPath, s)
				if UDRerr != nil {
					if pgError, ok := UDRerr.(*pgerror.Error); ok &&
						(pgError.Code == pgcode.UndefinedFunction || pgError.Code == pgcode.InsufficientPrivilege) {
						panic(builderError{UDRerr})
					}
				}
				t.IsUDF = true
				t.Func.FunctionReference = UDRdef
				t.Type = tree.UDRFuncType
			} else {
				panic(builderError{err})
			}
		}
...
func (s *scope) replaceSRF(f *tree.FuncExpr, def *tree.FunctionDefinition) *srf {
	...
	expr := f.Walk(s)
	typedFunc, err := tree.TypeCheck(expr, s.builder.semaCtx, types.Any, false)
	if err != nil {
		panic(builderError{err})
	}
...
func (s *scope) replaceAggregate(f *tree.FuncExpr, def *tree.FunctionDefinition) tree.Expr {
	...
	if f.Filter != nil {
		func() {
			...
			_, err := tree.TypeCheck(expr.(*tree.FuncExpr).Filter, s.builder.semaCtx, types.Any, false)
			if err != nil {
				panic(builderError{err})
			}
...
func (s *scope) replaceWindowFn(f *tree.FuncExpr, def *tree.FunctionDefinition) tree.Expr {
	...
	info.col = &scopeColumn{
		...
		typ:  f.ResolvedType(),
		id:   s.builder.factory.Metadata().AddColumn(def.Name, f.ResolvedType(), nil),
		expr: &info,
	}
	...
	if strings.EqualFold(def.Name, "count") && f.Type == 0 {
		if _, ok := vn.(tree.UnqualifiedStar); ok {
			if f.Filter != nil {
				...
				newDef, err := e.Func.Resolve(s.builder.semaCtx.SearchPath, s.builder.CaseSensitive)
				if err != nil {
					panic(builderError{err})
				}
			...
			if _, err := e.TypeCheck(&tree.SemaContext{}, types.Any, false); err != nil {
				panic(builderError{err})
			}
			newDef, err := e.Func.Resolve(s.builder.semaCtx.SearchPath, s.builder.CaseSensitive)
			if err != nil {
...
func (s *scope) replaceSubquery(
	...
	outScope := s.builder.buildStmt(sub.Select, nil, s)
...
func (s *scope) GetVisibleType(idx int) string {
	return ""
}
...
func (s *scope) SetForInsertOrUpdate(b bool) {
	s.IsInsertOrUpdate = b
}

// GetForInsertOrUpdate implements the parser.IndexedVarContainer interface.
func (s *scope) GetForInsertOrUpdate() bool {
	return s.IsInsertOrUpdate
}

// IndexedVarNodeFormatter is part of the IndexedVarContainer interface.
func (s *scope) GetScopeStmt() tree.Statement {
	if s.builder != nil {
...
func (s *scope) newAmbiguousColumnError(
	...
	return pgerror.NewErrorf(pgcode.AmbiguousColumn,
...
func newAmbiguousSourceError(tn *tree.TableName) error {
	if tn.Catalog() == "" {
		return pgerror.NewErrorf(pgcode.AmbiguousAlias,
