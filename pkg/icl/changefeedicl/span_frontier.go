...
func (s *spanFrontier) insert(span roachpb.Span, ts hlc.Timestamp) {
	...
	entryCov := intervalicl.Ranges{{Low: span.Key, High: span.EndKey, Payload: ts}}
	overlapCov := make(intervalicl.Ranges, len(overlapping))
	for i, o := range overlapping {
		spe := o.(*spanFrontierEntry)
		overlapCov[i] = intervalicl.Range{
			Low: spe.span.Key, High: spe.span.EndKey, Payload: spe,
		}
	}
	merged := intervalicl.OverlapCoveringMerge([]intervalicl.Ranges{entryCov, overlapCov})
	...
	for _, m := range merged {
		...
		if tracked {
			toInsert = append(toInsert, spanFrontierEntry{
				id:   s.idAlloc,
				keys: interval.Range{Start: m.Low, End: m.High},
				span: roachpb.Span{Key: m.Low, EndKey: m.High},
				ts:   mergedTs,
			})
			s.idAlloc++
		}
