...
func (a *Allocator) ComputeAction(
	...
	enableClusterNodes := a.storePool.ClusterEnableNodeCount()
...
func (a *Allocator) allocateTargetFromList(
	...
	candidates := al.allocateCandidates(
...
func (a Allocator) RemoveTarget(
	...
	al := allocatorLocation{ctx: ctx}
	rankedCandidates := al.removeCandidates(
		sl,
		analyzedConstraints,
		rangeInfo,
...
func (a Allocator) RebalanceTarget(
	...
	options := a.scorerOptions()
	al := allocatorLocation{ctx: ctx}
	results := al.rebalanceCandidates(
		sl,
		analyzedConstraints,
		rangeInfo,
		a.storePool.getLocalities(rangeInfo.Desc.Replicas),
...
func (a *Allocator) TransferLeaseTarget(
	...
	// 筛选器存储在包含现有副本的节点上，但已离开
	// 包含现有副本的存储。这个不包括存储
	// 我们无法重新平衡，避免在3节点集群中出现问题
	// 每个节点有多个存储。
	//
	...
	for _, s := range sl.stores {
		...
		for _, r := range existing {
			if r.NodeID == s.Node.NodeID && r.StoreID != s.StoreID {
				log.Infof(ctx, "Store rebalance ignoew %v ", s)
				exclude = true
				break
			}
		}
	...
	if checkTransferLeaseSource {
		preferred = al.preferredLeaseholders(zone, a.storePool, existing)
	} else {
		...
		preferred = al.preferredLeaseholders(zone, a.storePool, candidates)
	}
	if len(preferred) == 1 {
