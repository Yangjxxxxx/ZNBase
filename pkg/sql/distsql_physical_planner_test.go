	...
	tc.Stopper().RunWorker(context.TODO(), func(ctx context.Context) {
		...
		for {
			...
			default:
				...
				desc := sqlbase.GetTableDescriptor(cdb, "test", "public", "t")
...
func TestDistSQLRangeCachesIntegrationTest(t *testing.T) {
	...
	tc.Server(3).DistSender().(*kv.DistSender).DisableFirstRangeUpdates()
	db3 := tc.ServerConn(3)
...
func TestPartitionSpansSkipsIncompatibleNodes(t *testing.T) {
	...
	testCases := []struct {
		...
		// is to simulate ZNBase 1.0 nodes which don't advertise this information.
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			dsp := DistSQLPlanner{
				...
				nodeHealth: distSQLNodeHealth{
					gossip: mockGossip,
					connHealth: func(_ roachpb.NodeID, _ rpc.ConnectionClass) error {
