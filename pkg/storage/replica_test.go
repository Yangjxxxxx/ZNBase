...
func (tc *testContext) SendWrappedWith(
	...
	var err error
	tc.repl, err = tc.store.GetReplica(h.RangeID)
	if err != nil {
		panic(err)
	}
	tc.rangeID = tc.repl.RangeID
...
func TestInitPutUpdatesTSCacheOnError(t *testing.T) {
	...
	txn.ReadTimestamp, txn.WriteTimestamp = makeTS(1, 0), makeTS(1, 0)
	pArgs := putArgs(key, []byte("value"))
	...
	if br.Txn.WriteTimestamp.WallTime != t0.Nanoseconds() {
		t.Errorf("expected write timestamp to upgrade to 1s; got %s", br.Txn.WriteTimestamp)
	}
	...
	h := roachpb.Header{Timestamp: txn.WriteTimestamp}
...
func TestReplicaAbortSpanTxnIdempotency(t *testing.T) {
	...
	txn.WriteTimestamp.Forward(txn.WriteTimestamp.Add(10, 10)) // can't hurt
	if pErr := try(); pErr != nil {
		t.Fatal(pErr)
	}
...
// TestAbortSpanPoisonOnResolve 验证当一个意图被中止时，相应范围上的中止平移被下毒，
// 并且在下一次与同一时间段内的范围接触时向被推者显示一个txn中止。
func TestAbortSpanPoisonOnResolve(t *testing.T) {
	...
	run := func(abort bool) {
		...
		}
		{
			...
			// 由于修改写后读不检查abort已提升tpcc场景性能
			if err := assert(pErr); !abort && err != nil {
				t.Fatal(err)
			}
		}
		{
...
func TestChangeReplicasDuplicateError(t *testing.T) {
	...
	if _, err := tc.repl.ChangeReplicas(
		...
		"",
		false); err == nil || !strings.Contains(err.Error(), "node already has a replica") {
...
func TestReplicaCancelRaft(t *testing.T) {
	...
	for _, cancelEarly := range []bool{true, false} {
		func() {
			...
			_, pErr := tc.repl.executeBatchWithConcurrencyRetries(ctx, &ba, (*Replica).executeWriteBatch, false)
			if cancelEarly {
...
func TestReplicaIDChangePending(t *testing.T) {
	...
	_, _, _, err := repl.evalAndPropose(context.Background(), &ba, allSpansGuard(), &lease)
	if err != nil {
		t.Fatal(err)
	}
	<-proposedOnOld
...
func TestReplicaBurstPendingCommandsAndRepropose(t *testing.T) {
	...
	chs := func() []chan proposalResult {
		...
		for i := 0; i < num; i++ {
			...
			ch, _, _, err := tc.repl.evalAndPropose(ctx, &ba, allSpansGuard(), &lease)
			if err != nil {
				t.Fatal(err)
			}
			chs = append(chs, ch)
...
func TestCommandTimeThreshold(t *testing.T) {
	defer leaktest.AfterTest(t)()
	// todo jdx
	// skip by gzq, cannot fix gc
	// requestCanProceed is changed by snapshot
	t.Skip()
	tc := testContext{}
	stopper := stop.NewStopper()
	defer stopper.Stop(context.TODO())
...
func TestErrorInRaftApplicationClearsIntents(t *testing.T) {
	...
	ch, _, _, pErr := repl.evalAndPropose(
		context.Background(), &ba, allSpansGuard(), &exLease,
	)
	if pErr != nil {
		t.Fatal(pErr)
	}
	...
	testCases := []struct {
		...
		// 1PC serializable transaction will retry locally.
		{
			...
			batchFn: func(ts hlc.Timestamp) (ba roachpb.BatchRequest, expTS hlc.Timestamp) {
				...
				ba.CanForwardRead = true // necessary to indicate serverside-refresh is possible
				cput := cPutArgs(ba.Txn.Key, []byte("cput"), []byte("put"))
				et, _ := endTxnArgs(ba.Txn, true /* commit */)
				ba.Add(&cput, &et)
				assignSeqNumsForReqs(ba.Txn, &cput, &et)
...
// cause of https://github.com/znbasedb/znbase/issues/23176
