...
const (
	...
	githubUser           = "znbasedb"
	githubRepo           = "znbase"
	// ZNBasePkgPrefix is the znbase package prefix.
	ZNBasePkgPrefix = "github.com/znbasedb/znbase/pkg/"
)
...
func DefaultStressFailureTitle(packageName, testName string) string {
	trimmedPkgName := strings.TrimPrefix(packageName, ZNBasePkgPrefix)
...
func (p *poster) post(
	...
	const bodyTemplate = `To repro, try:
...
cd ~/go/src/github.com/znbasedb/znbase && \
stdbuf -oL -eL \
