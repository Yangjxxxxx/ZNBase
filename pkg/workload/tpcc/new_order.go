...
func (n *newOrder) run(ctx context.Context, wID int) (interface{}, error) {
	...
	err = zbdb.ExecuteInTx(
		ctx, (*workload.PgxTx)(tx),
		func() error {
			if n.config.rc {
				if _, err := (*workload.PgxTx)(tx).ExecContext(
					ctx, "SET TRANSACTION ISOLATION LEVEL READ COMMITTED;"); err != nil {
					return err
				}
			}
			// Select the district tax rate and next available order number, bumping it.
			...
			if _, err := tx.ExecEx(
				...
				fmt.Sprintf(`
					UPDATE stock
					SET
						s_quantity = CASE (s_i_id, s_w_id) %[1]s ELSE zbdb_internal.force_error('', 'unknown case') END,
