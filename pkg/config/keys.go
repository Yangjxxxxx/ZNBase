...
// MakeLocationKey returns the key for id's entry in the system.location table.
func MakeLocationKey(id uint32) roachpb.Key {
	k := keys.MakeTablePrefix(uint32(keys.LocationTableID))
	k = encoding.EncodeUvarintAscending(k, uint64(keys.LocationTablePrimaryIndexID))
	k = encoding.EncodeUvarintAscending(k, uint64(id))
	return keys.MakeFamilyKey(k, uint32(keys.LocationTableBufColumnID))
}
