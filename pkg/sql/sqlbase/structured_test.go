...
func TestAllocateIDs(t *testing.T) {
	...
	desc := NewMutableCreatedTableDescriptor(TableDescriptor{
		...
		Privileges:    NewDefaultObjectPrivilegeDescriptor(privilege.Table, AdminRole),
...
func TestAbleConstraint(t *testing.T) {
	defer leaktest.AfterTest(t)()
	intEncodedSize := 10 // 1 byte tag + 9 bytes max varint encoded size
	const ConstraintvalidityValidating = 2
	makeTestTableDescriptor := func(familyTypes [][]ColumnType) *MutableTableDescriptor {
		nextColumnID := ColumnID(8)
		var desc TableDescriptor
		for _, fTypes := range familyTypes {
	...
	desc := makeTestTableDescriptor(nil)
	detail1 := ConstraintDetail{}
	err := desc.AbleConstraint("check_a", detail1, true)
	want := pgerror.Unimplemented(fmt.Sprintf("drop-constraint-%s", detail1.Kind),
		"constraint %q has unsupported type", tree.ErrNameString("check_a"))
	if err.Error() != want.Message {
		t.Errorf("expected fit for the column but didn't get one")
	}
	detail2 := ConstraintDetail{Kind: ConstraintTypeCheck, CheckConstraint: &TableDescriptor_CheckConstraint{Validity: ConstraintvalidityValidating}}
	err1 := desc.AbleConstraint("check_a", detail2, true)
	want1 := pgerror.Unimplemented("rename-constraint-check-mutation",
		"constraint %q in the middle of being added, try again later",
		tree.ErrNameStringP(&detail2.CheckConstraint.Name))
	if err1.Error() != want1.Message {
		t.Errorf("expected fit for the column but didn't get one")
	}
	detail3 := ConstraintDetail{Kind: ConstraintTypeCheck, CheckConstraint: &TableDescriptor_CheckConstraint{Validity: 1}}
	desc.Checks = append(desc.Checks, &TableDescriptor_CheckConstraint{Name: "check_b", Able: false})
	_ = desc.AbleConstraint("check_b", detail3, true)
	want2 := true
	if desc.Checks[len(desc.Checks)-1].Able != want2 {
		t.Errorf("expected fit for the column but didn't get one")
	}
}
