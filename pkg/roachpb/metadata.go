...
// NodeID is a custom type for a znbase node ID. (not a raft node ID)
...
// StoreID is a custom type for a znbase store ID.
type StoreID int32
...
func (sc StoreCapacity) FractionUsed() float64 {
	...
	// capacity is really just the available space plus znbase's usage.
	//
...
// String returns a string representation of the LocationName.
func (m *LocationName) String() string {
	return strings.Join(m.Names, ",")
}

// Type returns the underlying type in string form. This is part of pflag's
// value interface.
func (*LocationName) Type() string {
	return "LocationName"
}

// Set sets the value of the LocationName. It is the important part of
// pflag's value interface.
func (m *LocationName) Set(value string) error {
	m.Names = append(m.Names, strings.Split(value, ",")...)
	return nil
}

// Duplicate the LocationName after start node
func (m *LocationName) Duplicate() *LocationName {
	p := map[string]struct{}{}
...
// Empty returns bool
func (m *LocationName) Empty() bool {
	return len(m.Names) == 0
}

// Have returns whether the specified location in the node and its diversity score
func (m *LocationName) Have(locateSpace *LocationValue) (bool, float64) {
	// locateSpace must be not nil
	match := 0
	for _, space := range locateSpace.Spaces {
		for _, name := range m.Names {
			if space == name {
				match++
			}
	...
	return true, float64(match) / float64(len(locateSpace.Spaces))
}

// HaveLease returns whether the node is a lease node that specifies a location
func (m *LocationName) HaveLease(locateSpace *LocationValue) bool {
	// locateSpace must be not nil
	for _, space := range locateSpace.Leases {
		for _, name := range m.Names {
			if space == name {
				return true
			}
...
func NewLocationMap() *LocationMap {
	res := &LocationMap{}
	res.IndexSpace = map[uint32]*LocationValue{}
	return res
}
