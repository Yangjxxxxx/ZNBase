...
func findBinary(binary, defValue string) (string, error) {
	...
	if err != nil {
		...
		dirs := []string{
			filepath.Join(gopath, "/src/github.com/znbasedb/znbase/"),
			filepath.Join(gopath, "/src/github.com/znbasedb/znbase/bin"+binSuffix),
...
func initBinaries() {
	...
	znbaseDefault := "znbase"
	if !local {
		znbaseDefault = "znbase-linux-2.6.32-gnu-amd64"
	}
	var err error
	znbase, err = findBinary(znbase, znbaseDefault)
	if err != nil {
...
// starting and stopping a znbase cluster on a subset of those machines, and
...
func (c *cluster) FetchDebugZip(ctx context.Context) error {
	...
	return contextutil.RunWithTimeout(ctx, "debug zip", 5*time.Minute, func(ctx context.Context) error {
		...
		// `./znbase debug zip` is noisy. Suppress the output unless it fails.
		output, err := execCmdWithBuffer(ctx, c.l, roachprod, "ssh", c.name+":1", "--",
			"./znbase", "debug", "zip", "--url", "{pgurl:1}", zipName)
		if err != nil {
			c.l.Printf("./znbase debug zip failed: %s", output)
			return err
		}
...
// StartE starts znbase nodes on a subset of the cluster. The nodes parameter
...
// StopE znbase nodes running on a subset of the cluster. See cluster.Start()
...
func waitForFullReplication(t *test, db *gosql.DB) {
	for ok := false; !ok; time.Sleep(time.Second) {
		if err := db.QueryRow(
			"SELECT min(array_length(replicas, 1)) >= 3 FROM zbdb_internal.ranges",
		).Scan(&ok); err != nil {
			t.Fatal(err)
		}
	}
