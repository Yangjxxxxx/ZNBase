...
// PgxTx is a thin wrapper that implements the zbdb.Tx interface, allowing pgx
// transactions to be used with ExecuteInTx.
type PgxTx pgx.Tx

var _ zbdb.Tx = &PgxTx{}

// ExecContext is part of the zbdb.Tx interface.
func (tx *PgxTx) ExecContext(
	ctx context.Context, sql string, args ...interface{},
) (gosql.Result, error) {
	_, err := (*pgx.Tx)(tx).ExecEx(ctx, sql, nil /* QueryExOptions */, args...)
	// zbdb.ExecuteInTx doesn't actually care about the Result, just the error.
	return nil, err
}

// Commit is part of the zbdb.Tx interface.
func (tx *PgxTx) Commit() error {
	return (*pgx.Tx)(tx).Commit()
}

// Rollback is part of the zbdb.Tx interface.
