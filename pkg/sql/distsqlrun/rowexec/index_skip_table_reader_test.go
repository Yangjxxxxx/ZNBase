...
func TestIndexSkipTableReader(t *testing.T) {
	...
	td1 := sqlbase.GetTableDescriptor(kvDB, "test", "public", "t1")
	td2 := sqlbase.GetTableDescriptor(kvDB, "test", "public", "t2")
	td3 := sqlbase.GetTableDescriptor(kvDB, "test", "public", "t3")
	td4 := sqlbase.GetTableDescriptor(kvDB, "test", "public", "t4")
	td5 := sqlbase.GetTableDescriptor(kvDB, "test", "public", "t5")
	td6 := sqlbase.GetTableDescriptor(kvDB, "test", "public", "t6")
	td7 := sqlbase.GetTableDescriptor(kvDB, "test", "public", "t7")
...
func BenchmarkIndexScanTableReader(b *testing.B) {
	...
	for _, numRows := range []int{1 << 4, 1 << 8, 1 << 12, 1 << 16, 1 << 18} {
		...
		for _, valueRatio := range []int{1, 100, 500, 1000, 5000, 10000} {
			...
			runner := func(reader runbase.RowSource, b *testing.B) {
				reader.Start(ctx)
				count := 0
				for {
			...
			flowCtxTableReader := runbase.FlowCtx{
				EvalCtx: &evalCtx,
				Cfg:     &runbase.ServerConfig{Settings: s.ClusterSettings()},
				Txn:     client.NewTxn(ctx, s.DB(), s.NodeID(), client.RootTxn),
				NodeID:  s.NodeID(),
			...
			flowCtxIndexSkipTableReader := runbase.FlowCtx{
				EvalCtx: &evalCtx,
				Cfg:     &runbase.ServerConfig{Settings: s.ClusterSettings()},
				Txn:     client.NewTxn(ctx, s.DB(), s.NodeID(), client.RootTxn),
				NodeID:  s.NodeID(),
