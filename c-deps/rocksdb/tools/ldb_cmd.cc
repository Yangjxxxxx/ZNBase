...
void DumpWalFileToDB(Options options, DB* db, std::string wal_dir, LDBCommandExecuteResult* exec_state);

void DumpSstFile(Options options, std::string filename, bool output_hex,
                 bool show_properties);
bool DecodeTableIDKey(const rocksdb::Slice& key, uint64_t* table_id);
};
...
class InMemoryHandler : public WriteBatch::Handler {
 ...
 private:
  ...
  class WalDumpHandler : public WriteBatch::Handler {
  public:
    WalDumpHandler(std::stringstream& row, DB * db)
            : Handler(),
              row_(row),
              db_(db){}
    ...
    Status MergeCF(uint32_t cf, const Slice& key, const Slice& value) override {
      //row_ << "MERGE(" << cf << ") : ";
      //commonPutMerge(key, value);
      db_->Merge(WriteOptions(),key, value);
      return Status::OK();
    }
...
//    row_ << LDBCommand::StringToHex(key.ToString()) << " ";
      uint64_t table_id = 0;
      DecodeTableIDKey(key, &table_id);
      if(table_id==62){
        row_<<"Delete:" << table_id << " " ;
      }
//    row_ << table_id << " ";
    ...
    Status SingleDeleteCF(uint32_t cf, const Slice& key) override {
      row_ << "SINGLE_DELETE(" << cf << ") : ";
      row_ << LDBCommand::StringToHex(key.ToString()) << " ";
      db_->SingleDelete(WriteOptions(), key);
      return Status::OK();
    }
    ...
    Status DeleteRangeCF(uint32_t cf, const Slice& begin_key,
      ...
      row_ << LDBCommand::StringToHex(begin_key.ToString()) << " ";
      row_ << LDBCommand::StringToHex(end_key.ToString()) << " ";
      db_->DeleteRange(WriteOptions(),db_->DefaultColumnFamily(), begin_key, end_key);
      return Status::OK();
    }
...
void DumpWalFileToDB(Options options, DB* db, std::string wal_dir, LDBCommandExecuteResult *exec_state) {
    ...
    filenames.clear();
    s = env->GetChildren(wal_dir, &filenames);
    if (s.IsNotFound() || !s.ok()) {
      *exec_state = LDBCommandExecuteResult::Failed("wal dir not found" + s.ToString());
      return;
    }

    std::vector<uint64_t> walLogs;
    for (size_t i = 0; i < filenames.size(); i++) {
      uint64_t number;
      FileType type;
      if (ParseFileName(filenames[i], &number, &type) && type == kLogFile) {
        walLogs.push_back(number);
      }
    }

    if (!walLogs.empty()) {
      // Recover in the order in which the logs were generated
      std::sort(walLogs.begin(), walLogs.end());
    }

    for(auto& walLog : walLogs){
      std::string fname = LogFileName(wal_dir, walLog);

      Status status;
      {
        std::unique_ptr<SequentialFile> file;
        status = env->NewSequentialFile(fname, &file, env_options);
        if (status.ok()) {
          wal_file_reader.reset(
                  new SequentialFileReader(std::move(file), wal_dir));
        }
      }
      if (!status.ok()) {
        if (exec_state) {
  ...
  bool DecodeUvarint64(rocksdb::Slice* buf, uint64_t* value) {
    ...
    if (length < 0 || length > 8) {
      return false;
    } else if (buf->size() < size_t(length)) {
      return false;
    }
    *value = 0;
  ...
  bool DecodeTableIDKey(const rocksdb::Slice& key, uint64_t* table_id) {
    ...
    uint p_type = key[0];
    if (p_type < 180 && p_type > 0xfd) {
      return false;
    }
    // DecodeUvarint64会修改原始值
    // rocksdb::Slice buf(static_cast<char*>(malloc(key.size())), key.size());
    // memcpy((void*)buf.data_, key.data_, key.size());
    rocksdb::Slice buf = key;

    bool ok = DecodeUvarint64(&buf, table_id);
    if (int(*table_id) < 0) {
      *table_id = 0;
    }
    // delete buf.data_;
...
const std::string WALDumperCommand::ARG_LOAD_WAL = "load_wal";
...
WALDumperCommand::WALDumperCommand(
  ...
  load_wal = IsFlagPresent(flags, ARG_LOAD_WAL);
}
...
void WALDumperCommand::DoCommand() {
  if(load_wal){
    DumpWalFileToDB(options_, load_wal_db_, wal_file_, &exec_state_);
  }else{
