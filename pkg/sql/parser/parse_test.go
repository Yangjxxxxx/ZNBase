...
func TestParse(t *testing.T) {
	...
	testData := []struct {
		...
		{`PREPARE a AS DUMP DATABASE a TO SST 'b'`},
		{`PREPARE a (STRING) AS DUMP DATABASE a TO SST $1`},
		...
		{`SELECT 'foo'::STRING(3)`},
		{`SELECT 'foo'::TIMESTAMP(3)`},
		{`SELECT 'foo'::TIMESTAMPTZ(3)`},
		...
		{`ALTER PARTITION p OF TABLE db.t LOCATE IN ("TJ")`},
		{`EXPLAIN ALTER PARTITION p OF TABLE db.t LOCATE IN ("TJ")`},
		...
		{`ALTER PARTITION p OF TABLE t LOCATE IN ("TJ")`},
		...
		{`ALTER PARTITION p OF TABLE db.t`},
		{`ALTER TABLE t CONFIGURE ZONE DISCARD`},
		{`ALTER PARTITION p OF TABLE t`},
		...
		{`DUMP TABLE foo TO SST 'bar'`},
		{`EXPLAIN DUMP TABLE foo TO SST 'bar'`},
		{`DUMP TABLE foo.foo, baz.baz TO SST 'bar'`},

		{`SHOW DUMP 'bar'`},
		{`EXPLAIN SHOW DUMP 'bar'`},
		{`SHOW DUMP RANGES 'bar'`},
		{`SHOW DUMP FILES 'bar'`},

		{`DUMP TABLE foo TO SST 'bar' AS OF SYSTEM TIME '1' INCREMENTAL FROM 'baz'`},
		{`DUMP TABLE foo TO SST $1 INCREMENTAL FROM 'bar', $2, 'baz'`},

		{`DUMP DATABASE foo TO SST 'bar'`},
		{`EXPLAIN DUMP DATABASE foo TO SST 'bar'`},
		{`DUMP DATABASE foo, baz TO SST 'bar'`},
		{`DUMP DATABASE foo TO SST 'bar' AS OF SYSTEM TIME '1' INCREMENTAL FROM 'baz'`},
		...
		{`DUMP TABLE foo TO SST 'bar' WITH key1, key2 = 'value'`},
...
func TestParse2(t *testing.T) {
	...
	testData := []struct {
		...
		{`CREATE TABLE a (b FLOAT, c FLOAT(10), d FLOAT(40), e REAL, f DOUBLE PRECISION)`,
			`CREATE TABLE a (b FLOAT, c FLOAT4, d FLOAT8, e REAL, f DOUBLE PRECISION)`},
		...
		{`CREATE TABLE a (b TIMESTAMP WITH TIME ZONE)`,
			`CREATE TABLE a (b TIMESTAMP WITH TIME ZONE)`},
		{`CREATE TABLE a (b TIMESTAMP(3) WITH TIME ZONE)`,
			`CREATE TABLE a (b TIMESTAMPTZ(3))`},
		{`CREATE TABLE a (b TIMESTAMP(3) WITHOUT TIME ZONE)`,
			`CREATE TABLE a (b TIMESTAMP(3))`},
		{`CREATE TABLE a (b TIMESTAMPTZ(3))`,
			`CREATE TABLE a (b TIMESTAMPTZ(3))`},
		{`CREATE TABLE a (b TIMESTAMP(3))`,
			`CREATE TABLE a (b TIMESTAMP(3))`},
		...
		{`DUMP DATABASE foo TO SST bar`,
			`DUMP DATABASE foo TO SST 'bar'`},
		{`DUMP DATABASE foo TO SST "bar.12" INCREMENTAL FROM "baz.34"`,
			`DUMP DATABASE foo TO SST 'bar.12' INCREMENTAL FROM 'baz.34'`},
		//{`RESTORE DATABASE foo FROM bar`,
		//	`RESTORE DATABASE foo FROM 'bar'`},
		...
		{`SHOW ALL CLUSTER SETTINGS`, `SHOW ALL CLUSTER SETTINGS`},
		...
		{`DUMP foo TO SST 'bar'`,
			`DUMP TABLE foo TO SST 'bar'`},
		{`DUMP foo.foo, baz.baz TO SST 'bar'`,
			`DUMP TABLE foo.foo, baz.baz TO SST 'bar'`},
		{`DUMP foo TO SST 'bar' AS OF SYSTEM TIME '1' INCREMENTAL FROM 'baz'`,
			`DUMP TABLE foo TO SST 'bar' AS OF SYSTEM TIME '1' INCREMENTAL FROM 'baz'`},
		{`DUMP foo TO SST $1 INCREMENTAL FROM 'bar', $2, 'baz'`,
			`DUMP TABLE foo TO SST $1 INCREMENTAL FROM 'bar', $2, 'baz'`},
		// Tables named "role" are handled specially to support SHOW GRANTS ON ROLE,
		// but that special handling should not impact DUMP.
		{`DUMP TABLE role TO SST 'bar'`,
			`DUMP TABLE role TO SST 'bar'`},
		...
		//	`RESTORE TABLE foo, baz FROM 'bar' AS OF SYSTEM TIME '1'`},
		{`DUMP foo TO SST 'bar' WITH key1, key2 = 'value'`,
			`DUMP TABLE foo TO SST 'bar' WITH key1, key2 = 'value'`},
	...
	for _, d := range testData {
		t.Run(d.sql, func(t *testing.T) {
			stmts, err := parser.Parse(d.sql, false)
			if err != nil {
				t.Errorf("%s: expected success, but found %s", d.sql, err)
				return
			}
			s := stmts.StringWithFlags(tree.FmtShowPasswords | tree.FmtVisableType)
			if d.expected != s {
				t.Errorf("%s: expected %s, but found (%d statements): %s", d.sql, d.expected, len(stmts), s)
			}
			if _, err := parser.Parse(s, false); err != nil {
...
func TestParseSyntax(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.sql, func(t *testing.T) {
			if _, err := parser.Parse(d.sql, false); err != nil {
...
`,
	...
	for _, d := range testData {
		t.Run(d.sql, func(t *testing.T) {
			_, err := parser.Parse(d.sql, false)
			if err == nil {
...
func TestParseOne(t *testing.T) {
	_, err := parser.ParseOne("SELECT 1; SELECT 2", false)
...
func BenchmarkParse(b *testing.B) {
	...
	for _, tc := range testCases {
		b.Run(tc.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				if _, err := parser.Parse(tc.query, false); err != nil {
					b.Fatal(err)
				}
			}
		})
