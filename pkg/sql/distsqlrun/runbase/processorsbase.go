...
//OptTableReader is a switch
var OptTableReader bool

//OptTableReaderParallelSetting is is parallel nums
var OptTableReaderParallelSetting int

//ParallelSetting is parallel nums
var ParallelSetting int

//OptOrderedSync is a switch
var OptOrderedSync bool
...
type ProcOutputHelper struct {
	...
	outputRows []sqlbase.EncDatumRow
...
//GetFilter return the filter
func (h *ProcOutputHelper) GetFilter() *ExprHelper {
	return h.filter
}
...
func (h *ProcOutputHelper) Init(
	...
	if OptJoin {
		h.renderExprs1 = make([][]ExprHelper, ParallelSetting)
	}
	h.output = output
	h.numInternalCols = len(types)
	if post.Filter != (distsqlpb.Expression{}) {
		h.filter = NewExprHelper(post.FuncDesc)
		if err := h.filter.Init(post.Filter, types, evalCtx); err != nil {
			return err
		}
		if OptJoin {
			h.filters = make([]ExprHelper, ParallelSetting)
			for i := range h.filters {
				h.filter.FunctionDescCollection = post.FuncDesc
				if err := h.filters[i].Init(post.Filter, types, evalCtx.Copy()); err != nil {
					return err
				}
			}
		}
	...
	if post.Projection {
		...
		if cap(h.renderExprs) >= nRenders {
			...
			if OptJoin {
				for i := 0; i < ParallelSetting; i++ {
					h.renderExprs1[i] = make([]ExprHelper, nRenders)
				}
			}
		} else {
		...
		for i, expr := range post.RenderExprs {
			if OptJoin {
				for j := range h.renderExprs1 {
					h.renderExprs1[j][i].FunctionDescCollection = post.FuncDesc
					if err := h.renderExprs1[j][i].Init(expr, types, evalCtx.Copy()); err != nil {
						return err
					}
				}
			}
			h.renderExprs[i] = *NewExprHelper(post.FuncDesc)
	...
	if h.outputCols != nil || len(h.renderExprs) > 0 {
		...
		if OptJoin {
			h.outputRows = make([]sqlbase.EncDatumRow, ParallelSetting)
			for i := 0; i < ParallelSetting; i++ {
				h.outputRows[i] = make(sqlbase.EncDatumRow, len(h.outputRow))
			}
		}
	}
...
// OptProcessRow is for TableReader parallel
func (h *ProcOutputHelper) OptProcessRow(
	ctx context.Context, row sqlbase.EncDatumRow, index int,
) (_ sqlbase.EncDatumRow, moreRowsOK bool, _ error) {
	if h.rowIdx >= h.maxRowIdx {
		return nil, false, nil
	}
	...
	if h.filter != nil {
		// Filtering.
		passes, err := h.filters[index].EvalFilter(row)
		if err != nil {
			return nil, false, err
		}
		if !passes {
	...
	if len(h.renderExprs1[index]) > 0 {
		// Rendering.
		for i := range h.renderExprs1[index] {
			datum, err := h.renderExprs1[index][i].Eval(row)
			if err != nil {
				return nil, false, err
			}
			h.outputRows[index][i] = sqlbase.DatumToEncDatum(h.OutputTypes[i], datum)
		}
	} else if h.outputCols != nil {
		// Projection.
		for i, col := range h.outputCols {
			h.outputRows[index][i] = row[col]
		}
	} else {
		// No rendering or projection.
		return row, h.rowIdx < h.maxRowIdx, nil
	...
	// If this row satisfies the limit, the caller is told to drain.
	return h.outputRows[index], h.rowIdx < h.maxRowIdx, nil
}
...
// Outputforfuncindex returns the output of the ProcOutputHelper.
func (h *ProcOutputHelper) Outputforfuncindex() {
	for i := 0; i < len(h.outputCols); i++ {
		h.outputCols[i]++
	}
}
...
// package into sql/vecexec package.
...
type ProcessorBase struct {
	...
	BlockRespones chan roachpb.BlockStruct
}

//IsCycle use for adding data that generates loops to RowContainer
func (pb *ProcessorBase) IsCycle(tem sqlbase.EncDatumRow, virtualColID int) {
	if s, ok := tem[virtualColID].Datum.(*tree.DString); ok {
		parts := strings.Split(string(*s), "||startwithvirtualpathcolumn+123456||")
		for key, value := range parts {
			for i := key; i < len(parts)-1; i++ {
				if value == parts[i+1] && pb.EvalCtx.RecursiveData.Nocycle && parts[i+1] != "" {
					//the flag use for stopping addrow
					optbuilder.CycleStopFlag = true
				}
			}
		}
	}
...
// MoveToDraining1 switches the processor to the StateDraining. Only metadata is
// returned from now on. In this state, the processor is expected to drain its
// inputs (commonly by using DrainHelper()).
//
// If the processor has no input (ProcStateOpts.intputToDrain was not specified
...
// An error can be optionally passed. It will be the first piece of metadata
// returned by DrainHelper().
func (pb *ProcessorBase) MoveToDraining1(err error) {
	if pb.State != StateRunning {
		// Calling MoveToDraining in any state is allowed in order to facilitate the
		// ConsumerDone() implementations that just call this unconditionally.
		// However, calling it with an error in states other than StateRunning is
...
// ProcessRowHelperOpt for tablereader parallel
func (pb *ProcessorBase) ProcessRowHelperOpt(
	row sqlbase.EncDatumRow,
	...
	// Note that outRow might be nil here.
	return outRow, ok, nil
}

// ProcessRowHelperOptForCount for tablereader parallel
func (pb *ProcessorBase) ProcessRowHelperOptForCount() bool {
	return pb.Out.rowIdx >= pb.Out.maxRowIdx
}

// OptProcessRowHelper is for TableReader Parallel
func (pb *ProcessorBase) OptProcessRowHelper(
	row sqlbase.EncDatumRow, index int,
) sqlbase.EncDatumRow {
	outRow, ok, err := pb.Out.OptProcessRow(pb.Ctx, row, index)
	if err != nil {
		pb.MoveToDraining(err)
		return nil
	}
...
const _procStateName = "StateRunningstateDrainingstateTrailingMetastateExhausted"

var _procStateIndex = [...]uint8{0, 12, 25, 42, 56}

func (i procState) String() string {
	if i < 0 || i >= procState(len(_procStateIndex)-1) {
		return "procState(" + strconv.FormatInt(int64(i), 10) + ")"
	}
	return _procStateName[_procStateIndex[i]:_procStateIndex[i+1]]
}
