...
func RemoveAllZoneConfigs(t testing.TB, sqlDB *SQLRunner) {
	t.Helper()
	for _, zone := range sqlDB.QueryStr(t, "SELECT cli_specifier FROM zbdb_internal.zones") {
...
func ZoneConfigExists(t testing.TB, sqlDB *SQLRunner, cliSpecifier string) bool {
	...
	var exists bool
	sqlDB.QueryRow(
		t, "SELECT EXISTS (SELECT 1 FROM zbdb_internal.zones WHERE cli_specifier = $1)", cliSpecifier,
	).Scan(&exists)
	return exists
}
