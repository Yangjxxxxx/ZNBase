...
func DecodeTableKey(
	...
	default:
		if _, ok := valType.(types.TtSet); ok {
			var r string
			if dir == encoding.Ascending {
				rkey, r, err = encoding.DecodeUnsafeStringAscending(key, nil)
			} else {
...
func FromColumnType(ct ColumnType) types1.T {
	switch ct.SemanticType {
	case ColumnType_BOOL:
		return *types1.Bool
	case ColumnType_BYTES, ColumnType_STRING, ColumnType_NAME:
		return *types1.Bytes
...
//ToType convert  ColumnType to types.T
func ToType(ct *ColumnType) types.T {
	switch ct.SemanticType {
	case ColumnType_BOOL:
		return types.Bool
	case ColumnType_BYTES:
	...
	case ColumnType_COLLATEDSTRING:
		return types.FamCollatedString
	case ColumnType_UUID:
		return types.UUID
	case ColumnType_TIME:
		return types.Time
...
//ToTypes convert ColumnType to types.T
func ToTypes(cts []ColumnType) []types.T {
...
// resulting slice.
//func FromColumnTypes(cts []ColumnType) []types1.T {
//	typs := make([]types1.T, len(cts))
//	for i := range typs {
...
// FromColumnTypeToT returns the T that corresponds to the input ColumnType.
func FromColumnTypeToT(ct ColumnType) *types1.T {
	switch ct.SemanticType {
	case ColumnType_BOOL:
		return types1.Bool
	case ColumnType_BYTES, ColumnType_NAME:
		return types1.Bytes
	case ColumnType_STRING:
...
//ToType1sWithoutErr convert ColumnType to types1.T
func ToType1sWithoutErr(cts []ColumnType) []types1.T {
	typs := make([]types1.T, len(cts))
	for i := range cts {
		typs[i] = *ToType1(&cts[i])
	}

	return typs
}

//ToType1 convert ColumnType to types1.T
func ToType1(ct *ColumnType) *types1.T {
	switch ct.SemanticType {
	case ColumnType_BOOL:
		return types1.Bool
	case ColumnType_BYTES, ColumnType_NAME:
...
//ToType1s convert ColumnType to types1.T
func ToType1s(cts []ColumnType) ([]types1.T, error) {
	typs := make([]types1.T, len(cts))
	for i := range typs {
		typs[i] = *FromColumnTypeToT(cts[i])
	}
