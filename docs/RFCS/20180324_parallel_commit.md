...
Consensus latencies are the price ZNBaseDB pays for its consistency guarantees; they are not usually incurred on monolithic or NoSQL databases, and can be used as an argument against the use of ZNBaseDB.
...
But, it is a restriction for the future: We must never issue [no-op writes](https://github.com/znbasedb/znbase/issues/23942) in the final batch of a transaction, and appropriate care must be taken to preserve this invariant. As of https://github.com/znbasedb/znbase/commit/9d7c35f, we assert against these no-op writes for successful point writes. Even with this added level of protection, we'll need to remember never to issue no-op writes as promised writes.
