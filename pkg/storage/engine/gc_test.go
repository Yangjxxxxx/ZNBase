...
func TestGarbageCollectorFilter(t *testing.T) {
	...
	gcA := MakeGarbageCollector(hlc.Timestamp{WallTime: 0, Logical: 0}, config.GCPolicy{TTLSeconds: 1}, 0)
	gcB := MakeGarbageCollector(hlc.Timestamp{WallTime: 0, Logical: 0}, config.GCPolicy{TTLSeconds: 2}, 0)
	n := []byte("data")
	d := []byte(nil)
	testData := []struct {
		gc       GarbageCollector
