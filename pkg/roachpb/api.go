...
const (
	...
	skipLeaseCheck                    // Requests for acquiring a lease skip the (proposal-time) check that the proposing replica has a valid lease.
...
// IsBlindWrite returns true if the request both reads and writes
...
// IsLocking returns true if the request produces write
...
func (rh *ResponseHeader) combine(otherRH ResponseHeader) error {
	...
	if rh.ResumeSpan != nil {
		return errors.Errorf("comznbaseng %+v with %+v", rh.ResumeSpan, otherRH.ResumeSpan)
	}
...
// Combine implements the combinable interface.
func (er *DumpResponse) combine(c combinable) error {
	if er != nil {
		otherER := c.(*DumpResponse)
		if err := er.ResponseHeader.combine(otherER.Header()); err != nil {
			return err
		}
		er.Files = append(er.Files, otherER.Files...)
...
// Method implements the Request interface.
func (*RevertRequest) Method() Method { return Revert }
func (*RevertRequest) flags() int {
	return isWrite | isTxn | isLocking | isIntentWrite | isRange | needsRefresh | consultsTSCache //DelRange
	//return isWrite | isTxn | isLocking | isRange | consultsTSCache //PUT
}

// ShallowCopy implements the Request interface.
func (gr *RevertRequest) ShallowCopy() Request {
	shallowCopy := *gr
	return &shallowCopy
}

// Method implements the Request interface.
func (*ClearIntentRequest) Method() Method { return ClearIntent }
func (*ClearIntentRequest) flags() int {
	return isWrite | isTxn | isLocking | isIntentWrite | isRange | needsRefresh | consultsTSCache //DelRange
	//return isWrite | isTxn | isLocking | isRange | consultsTSCache //PUT
}

// ShallowCopy implements the Request interface.
func (gr *ClearIntentRequest) ShallowCopy() Request {
	shallowCopy := *gr
	return &shallowCopy
}

// Method implements the Request interface.
func (*DumpRequest) Method() Method { return Dump }

// Method implements the Request interface.
func (*DumpOnlineRequest) Method() Method { return DumpOnline }

// Method implements the Request interface.
func (*LoadRequest) Method() Method { return Load }
...
// ShallowCopy implements the Request interface.
func (ekr *DumpRequest) ShallowCopy() Request {
	shallowCopy := *ekr
	return &shallowCopy
}

// ShallowCopy implements the Request interface.
func (ekr *DumpOnlineRequest) ShallowCopy() Request {
	shallowCopy := *ekr
	return &shallowCopy
}

// ShallowCopy implements the Request interface.
func (r *LoadRequest) ShallowCopy() Request {
	shallowCopy := *r
	return &shallowCopy
}
...
// NewQueryLock returns a Request initialized to delete the value at key.
func NewQueryLock(startKey, endKey Key, tID string, scanIntent bool) Request {
...
// NewRevertRange returns a Request initialized to revert snapshot.
func NewRevertRange(
	startKey, endKey Key, ts int64, returnKeys bool, clearIntents []Intent,
) Request {
	return &RevertRequest{
		...
		SnapshotTime: hlc.Timestamp{WallTime: ts},
		ReturnKeys:   returnKeys,
		ClearIntents: clearIntents,
	}
}

// NewClearIntentRange returns a Request initialized to clear intents.
func NewClearIntentRange(startKey, endKey Key, returnKeys bool, clearIntents []Intent) Request {
	return &ClearIntentRequest{
		RequestHeader: RequestHeader{
			Key:    startKey,
			EndKey: endKey,
		},
...
func (*PutRequest) flags() int { return isWrite | isTxn | isLocking | isIntentWrite | consultsTSCache }
...
func (*ConditionalPutRequest) flags() int {
	return isRead | isWrite | isTxn | isLocking | isIntentWrite | updatesReadTSCache | updatesTSCacheOnError | consultsTSCache
}
...
func (*IncrementRequest) flags() int {
	return isRead | isWrite | isTxn | isLocking | isIntentWrite | consultsTSCache
}

func (*DeleteRequest) flags() int {
	return isWrite | isTxn | isLocking | isIntentWrite | consultsTSCache
}
func (drr *DeleteRangeRequest) flags() int {
	...
	return isRead | isWrite | isTxn | isLocking | isIntentWrite | isRange | consultsTSCache | updatesReadTSCache | needsRefresh
}
...
func (sr *ScanRequest) flags() int {
	...
	return isRead | isRange | isTxn | maybeLocking | updatesReadTSCache | needsRefresh
}
...
func (rsr *ReverseScanRequest) flags() int {
	...
	return isRead | isRange | isReverse | isTxn | maybeLocking | updatesReadTSCache | needsRefresh
}
...
func (*QueryIntentRequest) flags() int {
	return isRead | isPrefix | updatesReadTSCache | updatesTSCacheOnError
}
...
func (*DumpRequest) flags() int             { return isRead | isRange | updatesReadTSCache }
func (*DumpOnlineRequest) flags() int       { return isRead | isRange }
func (*LoadRequest) flags() int             { return isAdmin | isAlone }
...
// Keys returns credentials in an aws.Config.
func (b *DumpSink_S3) Keys() *aws.Config {
	return &aws.Config{
		Credentials: credentials.NewStaticCredentials(b.AccessKey, b.Secret, b.TempToken),
	}
}
...
// Verify verifies the integrity of the get response value.
func (m *QueryLockResponse) Verify(req Request) error {
	return nil
}

// Method implements the Request interface.
func (m *QueryLockRequest) Method() Method {
	return QueryLock
}

// ShallowCopy implements the Request interface.
func (m *QueryLockRequest) ShallowCopy() Request {
	return m
}

func (m *QueryLockRequest) flags() int {
	return isRead | isRange
}

// Method implements the Request interface.向量扫描请求
func (*VecScanRequest) Method() Method { return VecScan }

// ShallowCopy implements the Request interface.
func (sr *VecScanRequest) ShallowCopy() Request {
	shallowCopy := *sr
	return &shallowCopy
}

func (sr *VecScanRequest) flags() int {
	if sr.OverrideFlags != 0 {
		return int(sr.OverrideFlags)
	}
	return isRead | isRange | isTxn | updatesReadTSCache | needsRefresh
}

// ForUpdate is setted which in order to achieve the "select for update" statement
func (sr *VecScanRequest) ForUpdate() {
	sr.OverrideFlags = isWrite | isRange | isTxn | updatesWriteTSCache | updatesTSCacheOnError | consultsTSCache
}

// ForRC is setting ScanRequest.OverrideFlags`s values
func (sr *VecScanRequest) ForRC() {
	sr.OverrideFlags = isRead | isRange | isTxn
}

// ForPushDown is setting pushdown
func (sr *VecScanRequest) ForPushDown() {
