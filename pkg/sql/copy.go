...
func (c *copyMachine) preparePlanner(ctx context.Context) func(context.Context, error) error {
	...
	autoCommit := false
	if txn == nil {
		txn = client.NewTxnWithSteppingEnabled(ctx, c.p.execCfg.DB, c.p.execCfg.NodeID.Get(), client.RootTxn)
...
func (c *copyMachine) addRow(ctx context.Context, line []byte) error {
	...
	for i, part := range parts {
		...
		d, err := tree.ParseStringAs(c.resultColumns[i].Typ, s, c.parsingEvalCtx, false)
		if err != nil {
			return err
		}
