...
- RFC PR: [#5985](https://github.com/znbasedb/znbase/pull/5985)
- ZNBase Issue:
...
restart in that way, but we may prefer to keep it explicit (`./znbase update
latest`?).
...
binary which runs the individual steps (`znbase-update latest`) one after
another.
