...
import (
	...
	sqlstats "github.com/znbasedb/znbase/pkg/sql/stats"
...
func (dsp *DistSQLPlanner) createPlanForCreateStats(
	...
	histogramCollectionEnabled := sqlstats.HistogramClusterMode.Get(&dsp.st.SV)
	for i := 0; i < len(reqStats); i++ {
		histogram := details.ColumnLists[i].HasHistogram && histogramCollectionEnabled
		histogramMaxBuckets := defaultHistogramBuckets
		if details.ColumnLists[i].HistogramMaxBuckets > 0 {
			histogramMaxBuckets = int(details.ColumnLists[i].HistogramMaxBuckets)
		}
