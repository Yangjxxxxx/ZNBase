...
//	txnC1 := txn(kC, ts1)
//
//	db := NewInMemByType(ctx, DefaultStorageEngine, roachpb.Attributes{}, 10<<20)
//	defer db.Close()
//
...
//	err := MVCCResolveWriteIntent(ctx, db, nil, intent(txnA1))
//	require.NoError(t, err)
//	err = MVCCResolveWriteIntent(ctx, db, nil, intent(txnB1))
//	require.NoError(t, err)
...
func TestMVCCIncrementalIteratorIntentStraddlesSStables(t *testing.T) {
	...
	ctx := context.Background()
	db1 := NewInMemByType(ctx, DefaultStorageEngine, base.StoreSpec{}, 10<<20 /* 10 MB */)
	defer db1.Close()
	...
	//   SSTable 2:
	//     b@2
	db2 := NewInMemByType(ctx, DefaultStorageEngine, base.StoreSpec{}, 10<<20 /* 10 MB */)
	defer db2.Close()
	...
	ingest := func(it Iterator, count int) {
		...
		for i := 0; i < count; i++ {
			...
			if err := sst.Add(MVCCKeyValue{it.Key(), it.Value()}); err != nil {
				t.Fatal(err)
			}
			it.Next()
		}
