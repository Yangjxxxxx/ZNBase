...
type Node struct {
	...
	clusterID   *base.ClusterIDContainer // UUID for ZNBase cluster
...
// NodeStatusRecorder and persists them to the znbase data store.
...
// server stops.
func (n *Node) recordJoinEvent(joinDuration int64) {
	...
	n.stopper.RunWorker(context.Background(), func(bgCtx context.Context) {
		...
		for r := retry.Start(retryOpts); r.Next(); {
			if err := n.storeCfg.DB.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
				return n.auditServer.LogAudit(
					ctx,
					false,
					&server.AuditInfo{
						EventTime:  timeutil.Now(),
						EventType:  string(logEventType),
						TargetInfo: &server.TargetInfo{TargetID: int32(n.Descriptor.NodeID)},
						Opt:        string(logEventType),
						OptTime:    joinDuration,
						Result:     "OK",
						Info: &infos.NodeInfo{
...
func (n *Node) setupSpanForIncomingRPC(
	...
	const opName = "/znbase.roachpb.Internal/Batch"
