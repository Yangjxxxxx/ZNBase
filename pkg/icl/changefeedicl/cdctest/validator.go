...
// Validator checks for violations of our cdc ordering and delivery
...
// fingerprintValidator verifies that recreating a table from its cdc
...
func (v *fingerprintValidator) NoteResolved(partition string, resolved hlc.Timestamp) error {
	...
	for len(v.buffer) > 0 {
		...
		var args []interface{}
		_, _ = fmt.Fprintf(&stmtBuf, `UPSERT INTO %s (`, v.fprintTable)
