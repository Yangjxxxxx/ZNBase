...
var rocksdbConcurrency = envutil.EnvOrDefaultInt(
	"ZNBASE_ROCKSDB_CONCURRENCY", func() int {
...
var rocksdbOnlineParameters = settings.RegisterStringSetting(
	"rocksdb.online.adjustable.parameters",
	"rocksdb.online.adjustable.parameters",
	"",
)

// ParamListOpt list the parameters can be update online by rocksdb.SetOptions() API
// This is used to determine which API(SetOptions() or SetDBOptions())to use
var ParamListOpt = []string{
...
// ParamListDBOpt list the parameters can be update online by rocksdb.SetDBOptions() API
// This is used to determine which API(SetOptions() or SetDBOptions())to use
var ParamListDBOpt = []string{
	"max_open_files",
	"max_total_wal_size",
...
type RocksDBConfig struct {
	...
	// ExtraOptions is a serialized protobuf set by Go ICL code and passed through
	// to C ICL code.
	ExtraOptions []byte
}
...
func (r *RocksDB) State() roachpb.StoreDescriptor_StoreState {
	return r.cfg.State
}

// UpdateState update engine state
func (r *RocksDB) UpdateState(state roachpb.StoreDescriptor_StoreState) {
	r.cfg.State = state
}
...
// RocksDB initialization. It is intended to be called by ICL code.
...
func newRocksDBInMem(spec base.StoreSpec, cacheSize int64) *RocksDB {
	...
	db, err := newMemRocksDB(spec, cache, 512<<20 /* MaxSize: 512 MiB */)
	if err != nil {
		panic(err)
	}
	return db
...
func newMemRocksDB(spec base.StoreSpec, cache RocksDBCache, MaxSizeBytes int64) (*RocksDB, error) {
	r := &RocksDB{
		cfg: RocksDBConfig{
	...
	auxDir, err := ioutil.TempDir(os.TempDir(), "znbase-auxiliary")
	if err != nil {
		return nil, err
	}
...
// LoadWAL loads the wal files from specify location using RocksDB
func (r *RocksDB) LoadWAL(dir string) error {
	status := C.DBLoadWAL(r.rdb, goToCSlice([]byte(dir)))
	return errors.Wrap(statusToError(status), "unable to load wal")
}
...
func (r *RocksDB) LogLogicalOp(op MVICLogicalOpType, details MVICLogicalOpDetails) {
...
func (r *rocksDBReadOnly) LogLogicalOp(op MVICLogicalOpType, details MVICLogicalOpDetails) {
	panic("not implemented")
}
...
func (r *RocksDB) GetStats() (*Stats, error) {
	...
	return &Stats{
		...
		BaseLevelFileCount:             int64(s.base_level_file_count),
		L0Bytes:                        int64(s.l0_bytes),
		BaseLevelBytes:                 int64(s.base_level_bytes),
		L0CompactionScore:              int64(s.l0_compaction_score),
		BaseLevelCompactionScore:       int64(s.base_level_compaction_score),
		//增加写失速指标
		StallL0SlowdownMicros:         int64(s.stall_l0_slowdown_micros),
		StallMemtableCompactionMicros: int64(s.stall_memtable_compaction_micros),
		StallL0NumFilesMicros:         int64(s.stall_l0_num_files_micros),
		StallMicros:                   int64(s.stall_micros),
		StallL0SlowdownCount:          int64(s.stall_l0_slowdown_count),
		StallMemtableCompactionCount:  int64(s.stall_memtable_compaction_count),
		StallL0NumFilesCount:          int64(s.stall_l0_num_files_count),
		WriteStall:                    int64(s.write_stall),
	}, nil
}
...
// UpdateOnlineParams in RocksDB is  supported
func (r *RocksDB) UpdateOnlineParams(params string) error {
	if params != r.currentParams {
		...
		for _, param := range paramSlice {
			...
			if strings.Contains(param, "=") {
				...
				for _, elOpt := range ParamListOpt {
					if temp[0] == elOpt {
						paramKey = temp[0]
						paramValue = temp[1]
						opt = true
				...
				for _, elDBOpt := range ParamListDBOpt {
					if temp[0] == elDBOpt {
						paramKey = temp[0]
						paramValue = temp[1]
						opt = false
				...
				paramRes := paramKey + "=" + paramValue
				if opt {
					status := C.DBUpdateOnlineParams(r.rdb, goToCSlice([]byte(paramRes)), C.bool(true))
					if statusToError(status) == nil {
						continue
					} else {
						return errors.Errorf("Parameter %v modified failed", paramKey)
					}
				} else {
					status := C.DBUpdateOnlineParams(r.rdb, goToCSlice([]byte(paramRes)), C.bool(false))
					if statusToError(status) == nil {
						continue
					} else {
						return errors.Errorf("Parameter %v modified failed", paramKey)
...
func (r *distinctBatch) LogLogicalOp(op MVICLogicalOpType, details MVICLogicalOpDetails) {
	// No-op. Logical logging disabled.
}
...
func (r *rocksDBBatch) LogLogicalOp(op MVICLogicalOpType, details MVICLogicalOpDetails) {
	// No-op. Logical logging disabled.
}
...
func (r *rocksDBIterator) MVCCScan(
	...
	rc := opts.Txn != nil && opts.Txn.IsolationLevel == util.ReadCommittedIsolation
	var maxTimestamp hlc.Timestamp
	if rc {
		// 暴力解决ReadWithinUncertaintyIntervalError
		maxTimestamp = opts.Txn.MaxTimestamp
		opts.Txn.MaxTimestamp = opts.Txn.WriteTimestamp
	}
	r.clearState()
	state := C.MVCCScan(
	...
	if rc {
		// 暴力解决ReadWithinUncertaintyIntervalError
		opts.Txn.MaxTimestamp = maxTimestamp
	}
	if err := statusToError(state.status); err != nil {
...
) (values roachpb.VecResults, resumeSpan *roachpb.Span, intents []roachpb.Intent, err error) {
	panic("implement me")
}
func (r *rocksDBReadOnly) VecScan(
	...
	if opts.Inconsistent && opts.Txn != nil {
		return roachpb.VecResults{}, nil, nil, errors.Errorf("cannot allow inconsistent reads within a transaction")
	}
	if len(end) == 0 {
		return roachpb.VecResults{}, nil, nil, emptyKeyError()
	}
	if max <= 0 {
		resumeSpan = &roachpb.Span{Key: start, EndKey: end}
		return roachpb.VecResults{}, resumeSpan, nil, nil
	}
	...
	pushExpr, _ := protoutil.Marshal(pd)
	state := C.VecScan(
		r.parent.rdb, goToCSlice(start), goToCSlice(end),
		goToCTimestamp(timestamp),
		goToCSlice(pushExpr), // 下推的表达式
		C.int64_t(max),
		goToCTxn(opts.Txn), C.bool(opts.Inconsistent),
		C.bool(opts.Reverse), C.bool(opts.Tombstones),
		C.bool(opts.IgnoreSequence),
	...
	if err := statusToError(state.status); err != nil {
		return roachpb.VecResults{}, nil, nil, err
	}
	if err := writeTooOldToError(timestamp, state.write_too_old_timestamp); err != nil {
		//return roachpb.VecResults{}, nil,nil, err
	}
	...
	if err := protoutil.Unmarshal(cSliceToUnsafeGoBytes(state.data), &values); err != nil {
		return roachpb.VecResults{}, nil, nil, err
	}
	// C层DBVecResults.data是malloc分配，需要在go层释放
	C.free(unsafe.Pointer(state.data.data))
	...
	if !opts.Inconsistent && len(intents) > 0 {
		return roachpb.VecResults{}, nil, nil, &roachpb.WriteIntentError{Intents: intents}
	}

	return values, resumeSpan, intents, nil
}
...
func goToCPushdown(pd *roachpb.PushDownExpr) (*C.DBPushdown, error) {
	// 检查schema信息完整性
	if len(pd.ColIds) < 1 {
		return nil, errors.Errorf("Pushdown schema no column.")
	}
	if len(pd.ColIds) != len(pd.ColTypes) {
		return nil, errors.Errorf("Pushdown schema column type not match .")
	}
	// 数据表schema信息
	cPush := C.DBPushdown{col_count: C.int(len(pd.ColIds)),
		col_ids:   (*C.uint32_t)(unsafe.Pointer(&pd.ColIds[0])),
		col_types: (*C.uint32_t)(unsafe.Pointer(&pd.ColTypes[0])),
	}
	if len(pd.PrimaryCols) > 0 {
		cPush.primary_count = C.int(len(pd.PrimaryCols))
		cPush.primary_cols = (*C.uint32_t)(unsafe.Pointer(&pd.PrimaryCols[0]))
	}
	if len(pd.SecondaryCols) > 0 {
		cPush.secondary_count = C.int(len(pd.SecondaryCols))
		cPush.secondary_cols = (*C.uint32_t)(unsafe.Pointer(&pd.SecondaryCols[0]))
	}
	// 列裁剪
	if len(pd.RequiredCols) > 0 {
		cPush.required_count = C.int(len(pd.RequiredCols))
		cPush.required_cols = (*C.uint32_t)(unsafe.Pointer(&pd.RequiredCols[0]))
	}
	// 条件下推
	if len(pd.Filters) > 0 {
		// TODO 构建C结构体的数组，以下写法报错
		//cPush.filter_count = C.int(len(pd.Filters))
		//filters := make([]C.DBSlice, len(pd.Filters))
		//for i := 0; i < len(pd.Filters); i++ {
		//	filters[i] = goToCSlice([]byte(pd.Filters[i]))
		//}
		//cPush.filters = (*C.DBSlice)(&filters[0])
		// todo 解决办法：可以在C层加一个创建DBSlice[]的方法，然后在Go层转换为切片，再赋值
		// p := (*[1 << 30]C.DBSlice)(unsafe.Pointer(filters))[:filter_count:filter_count]
	}
	return &cPush, nil
}
...
func MakeRocksDBSstFileReader() RocksDBSstFileReader {
	...
	return RocksDBSstFileReader{rocksDB: newRocksDBInMem(base.StoreSpec{}, cacheSize)}
}
...
// configuration for backup SSTs.
// These SSTs have bloom filters disabled and format set to LevelDB.
func MakeRocksDBSstFileWriter() (RocksDBSstFileWriter, error) {
	fw := C.DBSstFileWriterNew(C.bool(false))
	err := statusToError(C.DBSstFileWriterOpen(fw))
	return RocksDBSstFileWriter{fw: fw}, err
}

// MakeRocksDBIngestSstFileWriter creates a new RocksDBSstFileWriter for ingestion SSTs.
// These SSTs have bloom filters enabled (as set in default) and format set to RocksDBv2.
func MakeRocksDBIngestSstFileWriter() (RocksDBSstFileWriter, error) {
	fw := C.DBSstFileWriterNew(C.bool(true))
	err := statusToError(C.DBSstFileWriterOpen(fw))
	return RocksDBSstFileWriter{fw: fw}, err
}
...
func (r *RocksDB) PreIngestDelay(ctx context.Context) bool {
	if r.cfg.Settings == nil {
		return false
	}
	...
	targetDelay := calculatePreIngestDelay(r.cfg.Settings, stats)
	...
	if targetDelay == maxDelay {
		//fmt.Println("-------------- write stall time +1 --------------")
		return true
	}
...
func calculatePreIngestDelay(settings *cluster.Settings, stats *Stats) time.Duration {
	...
	if stats.L0FileCount > l0Filelimit {
		...
		if targetDelay > maxDelay {
			//fmt.Println(" =============== pending compaction mbytes: ", stats.PendingCompactionBytesEstimate)
			//fmt.Println(" =============== compaction limit mbytes: ", compactionLimit>>20)
			return maxDelay
		}
		return targetDelay
	}
...
// UpdateOnlineParam change the parameters online
func (r *RocksDB) UpdateOnlineParam() error {
	return nil
}
