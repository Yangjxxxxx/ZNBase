...
const (
	znbaseIndexEncoding = "prefix"
	defaultCollationTag = "en-US"
)

var znbaseIndexEncodingOid *tree.DOid

func init() {
	h := makeOidHasher()
	h.writeStr(znbaseIndexEncoding)
	znbaseIndexEncodingOid = h.getOid()
}
...
var pgCatalog = virtualSchema{
	...
	tableDefs: map[sqlbase.ID]virtualSchemaDef{
		...
		sqlbase.PgCatalogNodeStatusTableID:          pgCatalogNodeStatusTable,
...
CREATE TABLE pg_catalog.pg_am (
	...
	populate: func(_ context.Context, p *planner, _ *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		return addRow(
			...
			tree.NewDName(znbaseIndexEncoding), // amname - all versions
...
var pgCatalogNodeStatusTable = virtualSchemaTable{
	comment: `show all node status`,
	schema: `
CREATE TABLE pg_catalog.pg_nodeStatus (
	...
	populate: func(ctx context.Context, p *planner, _ *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		return forEachNodeInTable(nodes, func(node_id, nodeAddress, nodeName tree.Datum) error {
			return addRow(
				node_id,
				nodeAddress,
				nodeName,
...
CREATE TABLE pg_catalog.pg_collation (
  ...
  collctype STRING
	...
	populate: func(ctx context.Context, p *planner, dbContext *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		h := makeOidHasher()
		return forEachDatabaseDesc(ctx, p, dbContext, true, func(db *DatabaseDescriptor) error {
...
CREATE TABLE pg_catalog.pg_constraint (
	...
	conbin STRING,
	consrc STRING,
	-- condef is a ZNBaseDB extension that provides a SHOW CREATE CONSTRAINT
		...
		return forEachTableDescWithTableLookup(ctx, p, dbContext, hideVirtual /*virtual tables have no constraints*/, func(
			...
			for conName, con := range conInfo {
				...
				case sqlbase.ConstraintTypeFK:
					var ifError error
					if referencedSC, err := tableLookup.getSchemaByID(con.ReferencedTable.ParentID); err != nil {
						if _, err := tableLookup.getDatabaseByID(con.ReferencedTable.ParentID); err != nil {
							ifError = err
						} else {
							oid = h.ForeignKeyConstraintOid(db, tree.PublicSchema, table, con.FK)
							conindid = h.IndexOid(con.ReferencedTable.ID, con.ReferencedIndex.ID)
						}
					} else {
						if _, err := tableLookup.getDatabaseByID(referencedSC.ParentID); err != nil {
							ifError = err
						} else {
							oid = h.ForeignKeyConstraintOid(db, referencedSC.Name, table, con.FK)
							conindid = h.IndexOid(con.ReferencedTable.ID, con.ReferencedIndex.ID)
						}
					}
					...
					referencedTable, err := tableLookup.getTableByID(con.FK.Table)
					if err != nil {
						return err
					}
					if idx, err := referencedTable.FindIndexByID(con.FK.Index); err != nil {
					...
					if err := printForeignKeyConstraint(ctx, &buf, db.Name, scName, con.Index, tableLookup); err != nil {
						return err
					}
				...
				case sqlbase.ConstraintTypeCheck:
					...
					conbin = consrc
					if con.CheckConstraint.Able {
						condef = tree.NewDString(fmt.Sprintf("CHECK (%s) DISABLE", con.Details))
					} else {
						condef = tree.NewDString(fmt.Sprintf("CHECK (%s) ENABLE", con.Details))
					}
				}
...
CREATE TABLE pg_catalog.pg_database (
	...
	populate: func(ctx context.Context, p *planner, _ *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		return forEachDatabaseDesc(ctx, p, nil /*all databases*/, true, func(db *sqlbase.DatabaseDescriptor) error {
			return addRow(
	...
	populate: func(ctx context.Context, p *planner, dbContext *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		return forEachTableDescWithTableLookup(ctx, p, dbContext, hideVirtual /*virtual tables have no constraints*/, func(
			...
			for _, con := range conInfo {
				...
				var constraintOid, refObjID *tree.DOid
				var ifError error
				if referencedSC, err := tableLookup.getSchemaByID(con.ReferencedTable.ParentID); err != nil {
					if _, err := tableLookup.getDatabaseByID(con.ReferencedTable.ParentID); err != nil {
						ifError = err
						...
						refObjID = h.IndexOid(con.ReferencedTable.ID, con.ReferencedIndex.ID)
					}
				} else {
					if _, err := tableLookup.getDatabaseByID(referencedSC.ParentID); err != nil {
						ifError = err
					} else {
						constraintOid = h.ForeignKeyConstraintOid(db, referencedSC.Name, table, con.FK)
						refObjID = h.IndexOid(con.ReferencedTable.ID, con.ReferencedIndex.ID)
					}
				}
...
func getComments(ctx context.Context, p *planner) ([]tree.Datums, error) {
    ...
    FULL JOIN (SELECT * FROM zbdb_internal.predefined_comments) AS pc
	...
	populate: func(
		...
		for _, comment := range comments {
			...
			case keys.DatabaseCommentType:
				...
				objID = defaultOid(sqlbase.ID(*comment[0].(*tree.DInt)))
			...
			case keys.ConstraintCommentType:
				h := makeOidHasher()
				err = forEachTableDescWithTableLookup(ctx, p, dbContext, hideVirtual, func(
					db *sqlbase.DatabaseDescriptor,
					scName string,
					table *sqlbase.TableDescriptor,
					tableLookup tableLookupFn,
					...
					for _, con := range conInfo {
						switch con.Kind {
						case sqlbase.ConstraintTypePK:
							if objSubID.String() == fmt.Sprint(transformConstraint(table.ID, con)) {
								objID = h.PrimaryKeyConstraintOid(db, scName, table, con.Index)
							}
						case sqlbase.ConstraintTypeFK:
							if objSubID.String() == fmt.Sprint(transformConstraint(table.ID, con)) {
								referencedSC, err := tableLookup.getSchemaByID(con.ReferencedTable.ParentID)
								if err != nil {
									if _, err := tableLookup.getDatabaseByID(con.ReferencedTable.ParentID); err != nil {
										return err
									}
									objID = h.ForeignKeyConstraintOid(db, tree.PublicSchema, table, con.FK)
								}
								if referencedSC != nil {
									if _, err := tableLookup.getDatabaseByID(referencedSC.ParentID); err != nil {
										return err
									}
									objID = h.ForeignKeyConstraintOid(db, referencedSC.Name, table, con.FK)
								}
							}
						case sqlbase.ConstraintTypeUnique:
							if objSubID.String() == fmt.Sprint(transformConstraint(table.ID, con)) {
								objID = h.UniqueConstraintOid(db, scName, table, con.Index)
							}
						case sqlbase.ConstraintTypeCheck:
							if objSubID.String() == fmt.Sprint(transformConstraint(table.ID, con)) {
								objID = h.CheckConstraintOid(db, scName, table, con.CheckConstraint)
							}
						}
					}
					return nil
				...
				if o, ok := objID.(*tree.DInt); ok {
					objID = tree.NewDOid(tree.MustBeDInt(o))
				}
				objSubID = tree.DZero
				classOid = tree.NewDOid(sqlbase.PgCatalogClassTableID)
			}
			...
			func(db *sqlbase.DatabaseDescriptor, scName string, table *sqlbase.TableDescriptor) error {
				...
				return forEachIndexInTable(table, func(index *sqlbase.IndexDescriptor) error {
					...
					for i, columnID := range index.ColumnIDs {
						if index.IsFunc != nil && index.IsFunc[i] {
							continue
						}
...
https://www.postgresql.org/docs/9.5/view-pg-indexes.html`,
	// Note: znbase_oid is an extension of the schema to much more easily map
...
func indexDefFromDescriptor(
	...
	if len(index.Interleave.Ancestors) > 0 {
		...
		var parentDb *DatabaseDescriptor
		if parentSc, err := tableLookup.getSchemaByID(parentTable.ParentID); err != nil {
			if parentDb, err = tableLookup.getDatabaseByID(parentTable.ParentID); err != nil {
				return "", err
			}
		} else {
			scName = parentSc.Name
			if parentDb, err = tableLookup.getDatabaseByID(parentSc.ParentID); err != nil {
				return "", err
			}
		}
		...
		intlDef := &tree.InterleaveDef{
			Parent: tree.MakeTableNameWithSchema(tree.Name(parentDb.Name), tree.Name(scName), tree.Name(parentTable.Name)),
...
var pgCatalogNamespaceTable = virtualSchemaTable{
	comment: `available namespaces (incomplete; namespaces and databases are congruent in ZNBaseDB)
...
CREATE TABLE pg_catalog.pg_namespace (
	...
	populate: func(ctx context.Context, p *planner, dbContext *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		return forEachDatabaseDesc(ctx, p, dbContext, true, func(db *sqlbase.DatabaseDescriptor) error {
			return forEachSchemaName(ctx, p, db, false, func(sc *sqlbase.SchemaDescriptor) error {
				return addRow(
					h.NamespaceOid(db, sc.Name), // oid
					tree.NewDString(sc.Name),    // nspname
					getOwnerOID(sc),             // nspowner
					tree.DNull,                  // nspacl
				)
			})
		})
...
CREATE TABLE pg_catalog.pg_sequence (
	...
	populate: func(ctx context.Context, p *planner, dbContext *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		return forEachTableDesc(ctx, p, dbContext, hideVirtual, /* virtual schemasMap do not have indexes */
			func(db *sqlbase.DatabaseDescriptor, scName string, table *sqlbase.TableDescriptor) error {
				if !table.IsSequence() {
					return nil
				}
	...
	populate: func(_ context.Context, p *planner, _ *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		for _, vName := range varNames {
			...
			if gen.Set == nil && gen.RuntimeSet == nil {
				...
				switch vName {
				case "search_path":
					bootDatum = tree.NewDString(sqlbase.DefaultSearchPath.String())
					resetDatum = bootDatum
				default:
...
var pgCatalogTablespaceTable = virtualSchemaTable{
	comment: `available tablespaces (incomplete; concept inapplicable to ZNBaseDB)
...
var pgCatalogStatActivityTable = virtualSchemaTable{
	comment: `backend access statistics (empty - monitoring works differently in ZNBaseDB)
...
var datumToTypeCategory = map[reflect.Type]*tree.DString{
	...
	reflect.TypeOf(types.Void):        typCategoryPseudo,
	reflect.TypeOf(types.Cursor):      typCategoryPseudo,
...
CREATE TABLE pg_catalog.pg_views (
	...
	populate: func(ctx context.Context, p *planner, dbContext *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		return forEachTableDesc(ctx, p, dbContext, hideVirtual, /*virtual schemasMap do not have views*/
...
// Because ZNBase does not have an equivalent concept, we generate arbitrary
...
func getOwnerOID(desc sqlbase.DescriptorProto) tree.Datum {
	owner := getOwnerOfDesc(desc)
	h := makeOidHasher()
	return h.UserOid(owner)
}

func getOwnerName(desc sqlbase.DescriptorProto) tree.Datum {
	owner := getOwnerOfDesc(desc)
