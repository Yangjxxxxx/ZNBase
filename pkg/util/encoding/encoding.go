...
//   running ZNBaseDB are little-endian, so the bytes would need
...
func EncodeUntaggedTimeValue(appendTo []byte, t time.Time) []byte {
	...
	appendTo = EncodeNonsortingStdlibVarint(appendTo, int64(t.Nanosecond()))
	appendTo = EncodeNonsortingStdlibVarint(appendTo, int64(abbv/3600))
	return appendTo
}
...
func DecodeUntaggedTimeValue(b []byte) (remaining []byte, t time.Time, err error) {
	...
	if len(b) != 0 {
		b, _, zone, err = DecodeNonsortingStdlibVarint(b)
		if err != nil {
			return b, time.Time{}, err
		}
		if zone == 0 {
		...
		return b, timeutil.UnixNew(sec, nsec, zone), nil
	}
...
// DeocdeValue 解码列值，返回GO原生数据类型
func DeocdeValue(b []byte, typeOffset, dataOffset int, typ Type) ([]byte, []byte, error) {
	//*SQL层返回的是类型的[]byte //sql层转换为：EncDatum{encoding: enc,	encoded:  encoded,Datum:    nil,}
	//  只在需要时才解码成具体的数据类型
	encLen, err := PeekValueLengthWithOffsetsAndType(b, dataOffset, typ)
	if err != nil {
		return b, nil, err
	}
	...
	/*
		...
		case IPAddr:
			...
			return b, ipAddr, nil
		default:
			return b, nil, errors.Errorf("unknown type %s", typ)
		}
		//*/
}
