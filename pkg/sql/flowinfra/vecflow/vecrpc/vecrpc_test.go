...
func TestOutboxInbox(t *testing.T) {
	...
	_, mockServer, addr, err := distsqlpb.StartMockDistSQLServer(clock, stopper, runbase.StaticNodeID)
	...
	t.Run(fmt.Sprintf("cancellationScenario=%s", cancellationScenarioName), func(t *testing.T) {
		var (
			...
			inputBuffer = vecexec.NewBatchBuffer()
		...
		input := vecexec.NewRandomDataOp(
			vecexec.NewAllocator(ctx, &inputMemAcc), rng, args,
		)

		outboxMemAcc := testMemMonitor.MakeBoundAccount()
		defer outboxMemAcc.Close(ctx)
		outbox, err := NewOutbox(
			vecexec.NewAllocator(ctx, &outboxMemAcc), input, typs, nil,
		)
		require.NoError(t, err)
		...
		inbox, err := NewInbox(
			vecexec.NewAllocator(ctx, &inboxMemAcc), typs, distsqlpb.StreamID(0),
		)
		require.NoError(t, err)
		...
		inputBatches := vecexec.NewDeselectorOp(
			vecexec.NewAllocator(ctx, &deselectorMemAcc), inputBuffer, typs,
		)
		inputBatches.Init()
		outputBatches := vecexec.NewBatchBuffer()
		var readerErr error
		for {
			var outputBatch coldata.Batch
			if err := execerror.CatchVecRuntimeError(func() {
...
func TestOutboxInboxMetadataPropagation(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			defer inboxMemAcc.Close(ctx)
			inbox, err := NewInbox(
				vecexec.NewAllocator(ctx, &inboxMemAcc),
				typs, distsqlpb.StreamID(0),
			)
			require.NoError(t, err)
...
func BenchmarkOutboxInbox(b *testing.B) {
	...
	input := vecexec.NewRepeatableBatchSource(batch)
...
func TestOutboxStreamIDPropagation(t *testing.T) {
	...
	input := &vecexec.CallbackOperator{NextCb: func(ctx context.Context) coldata.Batch {
