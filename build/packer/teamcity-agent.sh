...
serverUrl=https://teamcity.znbasedb.com
name=
workDir=../work
tempDir=../temp
systemDir=../system
...
wget https://teamcity.znbasedb.com/update/buildAgent.zip
unzip buildAgent.zip
rm buildAgent.zip

# Cache the current version of the main ZNBase repository on the agent to
...
# alternate system.
git clone --bare https://github.com/znbasedb/znbase system/git/znbase.git
cat > system/git/map <<EOS
https://github.com/znbasedb/znbase = znbase.git
EOS

# For master and the last two release, download the builder and acceptance
# containers.
repo="$GOPATH"/src/github.com/znbasedb/znbase
git clone --shared system/git/znbase.git "$repo"
cd "$repo"
...
do
  git checkout "$branch"
  ZNBASE_BUILDER_CCACHE=1 build/builder.sh make test testrace TESTS=-
  # TODO(benesch): store the acceptanceversion somewhere more accessible.
  docker pull $(git grep znbasedb/acceptance -- '*.go' | sed -E 's/.*"([^"]*).*"/\1/') || true
done
cd -
EOF
write_teamcity_config
