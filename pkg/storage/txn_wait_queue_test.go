	...
	for i := 0; i < len(reqs); i++ {
		...
		if ret.pErr != nil {
			if !testutils.IsPError(ret.pErr, context.Canceled.Error()) {
				require.Regexp(t, `deadlock cause txn aborted`, ret.pErr)
			}
		} else {
			pushed = true
...
func TestTxnWaitQueueDependencyCycleWithPriorityInversion(t *testing.T) {
	...
	txnA := newTransaction("txnA", roachpb.Key("a"), -1, tc.Clock())
	...
	txnB := newTransaction("txnB", roachpb.Key("a"), -2, tc.Clock())
