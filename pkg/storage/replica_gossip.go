...
func (r *Replica) loadSystemConfig(ctx context.Context) (*config.SystemConfigEntries, error) {
	...
	if intents := result.Local.DetachEncounteredIntents(); len(intents) > 0 {
		...
		// This is called from handleReadWriteLocalEvalResult (with raftMu locked),
		// so disallow synchronous processing (which blocks that mutex for
		// too long and is a potential deadlock).
		if err := r.store.intentResolver.CleanupIntentsAsync(ctx, intents, false /* allowSync */, false); err != nil {
			log.Warning(ctx, err)
		}
