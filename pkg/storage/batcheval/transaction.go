...
func CanCreateTxnRecord(rec EvalContext, txn *roachpb.Transaction, rc bool) error {
	...
	var epochZeroOrigTS hlc.Timestamp
	if rc {
		epochZeroOrigTS = hlc.MaxTimestamp
	} else {
		epochZeroOrigTS = txn.MinTimestamp
	}
	ok, minTS, reason := rec.CanCreateTxnRecord(txn.ID, txn.Key, epochZeroOrigTS)
	if !ok {
		return roachpb.NewTransactionAbortedError(reason)
	}
	txn.WriteTimestamp.Forward(minTS)
	return nil
}
