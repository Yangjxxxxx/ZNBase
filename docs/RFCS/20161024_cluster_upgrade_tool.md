...
- RFC PR: [#9404](https://github.com/znbasedb/znbase/pull/9404)
- ZNBase Issue: [#4267](https://github.com/znbasedb/znbase/issues/4267)

# Summary

A series of hooks to perform any necessary bookkeeping for a ZNBaseDB
version upgrade.
...
software as complex as ZNBaseDB.
...
Some versions of ZNBaseDB will require that a "system migration hook" be run.
...
of the ZNBaseDB nodes in a cluster to modify the state of the cluster in
some way.

Simple migrations can be discussed in terms of which versions of the ZNBase
...
For simplicity, we assume that at most two versions of ZNBaseDB are ever
...
Some migrations most naturally match a model where the ZNBaseDB version with
...
and compatible with previous versions of ZNBaseDB that could be running on
...
migrations. It would then automatically be run whenever a version of ZNBaseDB
...
what ZNBaseDB versions are being used by other active nodes in the cluster.
...
a new CLI command `./znbase cluster-upgrade` that gives the DB administrator
...
* Listing the available migrations, which migrations they depend on, and the
  ZNBaseDB versions at which it's safe to run each of them
...
The idea is that before upgrading to a new version of ZNBaseDB, the admin
...
If a ZNBaseDB node starts up and the cluster that it joins has not run all
...
The CLI tool will be the recommended way of upgrading ZNBaseDB versions for
...
`./znbase cluster-upgrade` command.
...
add an automatically run migration to the next release of ZNBaseDB because
...
1. Release new version of ZNBaseDB with:
...
1. Decide to upgrade ZNBaseDB cluster to new version
1. Verify that all migrations for the current version have been completed by
   running `./znbase cluster-upgrade` with the current binary
...
1. Once the upgrade has completed and rollbacks are deemed no longer necessary,
   perform the new version's migrations by running `./znbase cluster-upgrade`
...
the ZNBaseDB version (or alternatively the known migrations) for all nodes
...
version leases for this](https://github.com/znbasedb/znbase/issues/10212).
...
The migration will look fairly similar from the perspective of ZNBaseDB
developers:

1. Release new version of ZNBaseDB with code that can handle both encodings.
...
[attempted `system.jobs` PR](https://github.com/znbasedb/znbase/pull/7073)
for an example.
