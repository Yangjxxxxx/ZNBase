...
func VerifyNoImports(
	...
	short := func(in string) string {
		return strings.Replace(in, "github.com/znbasedb/znbase/pkg/", "./pkg/", -1)
	}
	...
	check = func(path string) error {
		...
		for _, imp := range pkg.Imports {
			for _, forbidden := range forbiddenPkgs {
				...
				if forbidden == "c-deps" && imp == "C" && strings.HasPrefix(path, "github.com/znbasedb/znbase/pkg") {
					// don't check plsql package
					if pkg.Name == "plsql" {
						continue
					}
