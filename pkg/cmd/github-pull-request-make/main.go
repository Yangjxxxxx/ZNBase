...
func main() {
	...
	znbase, err := build.Import(fmt.Sprintf("github.com/%s/%s", org, repo), "", build.FindOnly)
	if err != nil {
		log.Fatal(err)
	}
	...
	if target == "checkdeps" {
		...
		if vendorChanged {
			...
			// Check for diffs.
			var foundDiff bool
			for _, dir := range []string{filepath.Join(znbase.Dir, "vendor"), znbase.Dir} {
		...
		if len(pkgs) > 0 {
			...
			for name, pkg := range pkgs {
				...
				cmd.Env = append(os.Environ(), "ZNBASE_NIGHTLY_STRESS=true")
				cmd.Dir = znbase.Dir
				cmd.Stdout = os.Stdout
				cmd.Stderr = os.Stderr
				log.Println(cmd.Args)
