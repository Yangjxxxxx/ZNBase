...
// ReaderAppearsEncrypted checks if passed bytes begin with an encryption preamble.
func ReaderAppearsEncrypted(input io.Reader) (io.Reader, bool, error) {
	text, err := ioutil.ReadAll(input)
	if err != nil {
		return nil, false, err
	}
	input = bytes.NewReader(text)
	return input, bytes.HasPrefix(text, encryptionPreamble), err
}
