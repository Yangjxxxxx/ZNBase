...
import (
	...
	"github.com/znbasedb/znbase/pkg/workload/ledger"
)
...
func TestEncoders(t *testing.T) {
	...
	expecteds := map[string]struct {
		...
		`format=json,envelope=key_only`: {
			insert:   `[1]->`,
			delete:   `[1]->`,
			resolved: `{"__exdb__":{"resolved":"1.0000000002"}}`,
		},
		...
		`format=json,envelope=row,updated`: {
			insert:   `[1]->{"__exdb__": {"updated": "1.0000000002"}, "a": 1, "b": "bar"}`,
			delete:   `[1]->`,
			resolved: `{"__exdb__":{"resolved":"1.0000000002"}}`,
		},
	...
	for _, o := range opts {
		...
		t.Run(name, func(t *testing.T) {
			...
			case string(optFormatAvro):
				...
				defer reg.Close()
				o[optSchemaRegistry] = reg.server.URL
			...
			valueInsert, _, _, _, _, _, err := e.EncodeValue(nil, rowInsert, nil)
			require.NoError(t, err)
			...
			valueDelete, _, _, _, _, _, err := e.EncodeValue(nil, rowDelete, nil)
			require.NoError(t, err)
...
func (r *testSchemaRegistry) encodedAvroToNative(b []byte) (interface{}, error) {
	if len(b) == 0 || b[0] != avroWireFormatMagic {
...
func TestAvroEncoder(t *testing.T) {
	...
	testFn := func(t *testing.T, db *gosql.DB, f cdctest.TestFeedFactory) {
		...
		assertPayloadsAvro(t, reg, foo, []string{
			`d.public.foo: {"a":{"long":1}}->{"after":{"foo":{"a":{"long":1},"b":{"string":"bar"}}},"before":null}`,
			`d.public.foo: {"a":{"long":2}}->{"after":{"foo":{"a":{"long":2},"b":null}},"before":null}`,
		})
		...
		var ts2 string
		require.NoError(t, zbdb.ExecuteTx(ctx, db, nil /* txopts */, func(tx *gosql.Tx) error {
			return tx.QueryRow(
				`INSERT INTO foo VALUES (3, 'baz') RETURNING cluster_logical_timestamp()`,
			).Scan(&ts2)
		}))
		assertPayloadsAvro(t, reg, fooUpdated, []string{
			`d.public.foo: {"a":{"long":3}}->{"after":{"foo":{"a":{"long":3},"b":{"string":"baz"}}},` +
				`"before":null,` +
				`"updated":{"string":"` + ts2 + `"}}`,
		})
	}
...
func TestAvroMigrateToUnsupportedColumn(t *testing.T) {
	...
	testFn := func(t *testing.T, db *gosql.DB, f cdctest.TestFeedFactory) {
		...
		defer closeFeed(t, foo)
		assertPayloadsAvro(t, reg, foo, []string{
			`d.public.foo: {"a":{"long":1}}->{"after":{"foo":{"a":{"long":1}}}}`,
		})
